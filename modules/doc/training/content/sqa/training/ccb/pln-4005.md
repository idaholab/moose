...
- Static document created to detail the process for development of MOOSE and MOOSE-based
  applications to meet the NQA-1 standard using [!ac](INL) templates and procedures.
...
- In short, we follow the most stringent processes but do not provide software at that level, it is
  necessary for the end-users to complete the process before using our software in a safety
  capacity
...
- +Safety-software Determination (SSD)+: [!ac](INL) form to declare if the application is safety
  software
- +Quality-level Determination (QLD)+: [!ac](INL) form to declare the safety software level
- +[!ac](EA) Entry+: [!ac](INL) from that lists [!ac](SQA) documentation for application

!alert note
The SDD, QLD, and EA Entry are the only [!ac](INL) level documents that are necessary for satisfying the
NQA-1 standard and are that is created when the [!ac](SQA) process begins.

\**VVP is referring verification and validation of the NQA-1 process, not the science.*

!---

### PLN-4005: Repository Documentation

All documentation stored in the repository, the following [!ac](SQA) specific documents must be
...
- +Static Documentation Review+: Changes to the static documentation follow the review procedure
  native to the INL system (SQA Lead should handle these)
...
- Static Documentation is managed using the [!ac](INL) tools
...
[!ac](INL) has specific instructions for acquiring software; however, "zero-dollar" acquisitions
and upgrades are not required to follows those procedures (e.g., PETSc). These shall be maintained
as git submodules and follow the regular change control process.
...
1. +Development Branch Testing+: ensure changes work together
1. +Documentation+: build current website/docs
1. +Release+: final review and release by Project Lead (0INL1862)
