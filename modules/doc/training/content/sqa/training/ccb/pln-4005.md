...
- In short, we follow the most stringent processes but do not provide software at that level, and it is
...
- +[!ac](EA) Entry+: [!ac](INL) form that lists [!ac](SQA) documentation for application

!alert note
The SDD, QLD, and EA Entry are the only [!ac](INL) level documents that are necessary for satisfying the
NQA-1 standard, and they are created when the [!ac](SQA) process begins.

\**VVP is referring to verification and validation of the NQA-1 process, not the science.*
...
and upgrades are not required to follow those procedures (e.g., PETSc). These shall be maintained
