...
The [framework:syntax/Materials/index.md] in MOOSE is one for defining spatially and/or temporally varying properties and storing their values in a single object. The types of terms that belong to this system are the coefficients in the governing equations, though they may or may not be continuously differentiable with respect to the problem domain. The values are always indexed at individual [!ac](QPs) and the associated class methods tend to be invoked at every time step. The properties may then be accessed by other MOOSE systems. And, like many systems in MOOSE, Materials can be coupled to variables if a property being calculated depends on them.
