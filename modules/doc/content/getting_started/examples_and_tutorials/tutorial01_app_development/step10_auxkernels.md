...
The [syntax/AuxKernels/index.md] in MOOSE is one for computing and setting the explicitly known values of auxiliary variables. The main advantage of [!file text=`AuxKernel`](src/auxkernels/AuxKernel.C) (or `VectorAuxKernel`) objects is that the variables they operate on are treated like any other MOOSE variable and can be used to decouple systems of equations or for various postprocessing tasks. These objects have access to the following variable members:
