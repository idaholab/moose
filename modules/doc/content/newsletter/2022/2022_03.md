...
[framework:/MultiAppUserObjectTransfer.md] gained a new option that allows transferring from the *nearest* sub-app app from a multiapp.  This will help in cases where the sub-app is a lower-dimensional manifold and therefore doesn't overlap with the domain of the parent app.
...
- The `ElementPropertyReadFile` user object was deprecated and replaced by [framework:PropertyReadFile.md].
  It can now load nodal data, and is no longer restricted to contiguous IDs for elements or nodes.
- The [framework:PiecewiseConstantFromCSV.md] function was added to use the `PropertyReadFile` user object
