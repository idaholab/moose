...
some of the documentation in [MFEMSolverBase.md] here. LOR solvers work by taking a problem and
...
A [VectorJumpIndicator.md] was added to perform adaptivity based on the jump in values of a vector variable between neighboring cells.
...
The [DirectCentralDifference.md] time integrator was improved to support mixed order time integration scheme, using central difference for some variables and first order Euler for others. This mixed-order approach can avoid using an instable second order scheme for equations known to have these issues.
...
- The Makefile for moose was improved to avoid errors on missing libtorch or libmfm on unrelated targets.
- JSON output of the perf graph was enabled. This is intended for automated examination of solver performance by the continuous integration platform.
- The [ADEqualValueEmbeddedConstraint.md] was added to MOOSE. This facilitates the use of AD in that system, enabling numerically-perfect Jacobians without hand-coding the Jacobian.
- The [LibtorchController.md] can now be used for recover simulations.
...
- The reporting of the preconditioner in the simulaiton header has been improved to correctly report the preconditioner when modified using the command line.
- The `FParser` epsilon, a constant used for removing terms during the function optimization phase, has been set to 0 to prevent undesirable behavior.
