...
The material property outputs sytem was reworked to perform consistently using both specific `Outputs`
...
- verification for inputs comparing subchannel simulations to analytical solutions or simulations using
  other subchannel codes,
- validation for inputs of simulations compared to experimental data, and
- examples for inputs that simply feature useful techniques used in subchannel, outside of any V&V
  effort.

### Reactor

The [HexagonalGridPositions.md] was extended to allow the nesting of another positions object within
the hexagonal grid. This can be used for example to obtain in a single object all the pin positions
in hexagonal core consisting of multiple hexagonal lattices of pins.
...
- CheckpointIO can now handle some of the Mesh caches if needed. MOOSE often skips partitioning when
  doing checkpoint restart, so now at the libMesh level we `recalculate_n_partitions()` and
