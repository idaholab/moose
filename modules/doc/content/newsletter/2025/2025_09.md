...
- Patched [MFEMTransient.md] to fix an issue causing time step information in the console output to lag by one time step
- Added the ability to define cut transition submeshes via [MFEMCutTransitionSubMesh](MFEMTransitionSubMesh.md), e.g., to model an induced
  electromotive force on a closed loop
- Added an auxiliary kernel, [MFEMSumAux.md], to sum an arbitrary number of variables
- Added two mixed form kernels, [MFEMMixedGradGradKernel.md] and [MFEMMixedVectorMassKernel.md], akin to the diffusion
  and vector mass kernels, respectively, for when test and trial variables may differ
- Added the ability to set an initial condition on a scalar variable on a boundary via [MFEMScalarBoundaryIC.md]
- Added a postprocessor, [MFEMVectorFEInnerProductIntegralPostprocessor.md], to integrate the dot product of two vector
  variables over a given domain
