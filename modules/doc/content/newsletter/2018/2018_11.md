...
[!file](ad_material.i), [!file](ad_coupled_convection/ad_coupled_convection.i), and [!file](ad_simple_diffusion/ad_simple_diffusion.i). For
examples of writing `ADKernels`, see [!file](ADCoupledConvection.h), [!file](ADMatDiffusionTest.h)
headers and [!file](ADCoupledConvection.C), [!file](ADMatDiffusionTest.C) source files. An example
of an `ADMaterial` can be found in [!file](ADCoupledMaterial.h), [!file](ADCoupledMaterial.C).
...
`VectorConstantIC` exists (see [!file](VectorConstantIC.h), [!file](VectorConstantIC.C),
[!file](vector_constant_ic.i)), but more can be easily added by overriding the `value` method.
