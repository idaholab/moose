...
The [hpc_remote.md] instructions have been updated for a better user experience in accessing web-based INL HPC resources. The previous instructions involved using an additional browser with a proxy to access said resources (hpcgitlab, moosebuild, hpcweb, etc). The new instructions utilize a proxy configuration that is much easier to setup for the end-user and do not require an additional browser. For more information, see [hpc_remote.md#socks-proxy].
...
The pictured example is an anisotropic point source that emits in a cone of directions. See [flashlight_source.md] for more information.
