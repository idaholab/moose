...
```
 ...
 You do not need the `relap5 -i input.i` and you need the full path to your equivalent of `input.i`. The "Select File" button should open a dialog that will allow you to browse your INL HPC folder structure and manually select your input file to prevent typos in the full path to the input file (see Figure 2).
...
```bash
relap5 -t -i /home/user/input.i
relap5 --timing -i /home/user/input.i
```

 You are allowed to put more than one in this box. Everything that you put in this box will be entered between  `relap5` and your input file `-i /home/user/input.i`.
...
This option allows you to run a specific build of the herd code. To get a list of the different version hashes that are allowed here, you can open a terminal on the appropriate cluster and run `module spider relap5` and you will be presented with multiple versions of each herd application.
