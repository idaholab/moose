...
AEFVBC::computeQpResidual()
{
  ...
  // calculate the flux
  const auto & flux = _flux.getFlux(_current_side, _current_elem->id(), uvec1, dwave, _tid);
...
AEFVBC::computeQpJacobian()
{
  ...
  // calculate the flux
  auto & fjac1 = _flux.getJacobian(_current_side, _current_elem->id(), uvec1, dwave, _tid);
