...
validParams<MeshCut3DUserObject>()
{
  ...
  params.addRequiredParam<MeshFileName>("mesh_file", "Mesh file for XFEM geometric cuts");
    ...
    _n_step_growth(getParam<unsigned int>("n_step_growth"))
{
  ...
  // only the old libmesh file type is supported
...
// TODO: Time evolving cuts not yet supported in 3D (hence the lack of use of the time variable)
{
  ...
  const auto elem_begin = _cut_mesh->elements_begin();
  auto elem_end = _cut_mesh->elements_end();
  auto elem_it = _cut_mesh->elements_begin();
  ...
  for (unsigned int i = 0; i < elem->n_sides(); ++i)
  {
    ...
    for (unsigned int j = 0; j < n_edges; j++)
    {
      ...
      for (; elem_it != elem_end; ++elem_it)
      {
        ...
        for (unsigned int k = 0; k < cut_elem->n_nodes(); k++)
        {
          Node * node = cut_elem->get_node(k);
                                       ...
                                       Point & pint) const
{
  ...
  std::vector<Real> plane_point = {center(0), center(1), center(2)};
  std::vector<Real> planenormal = {normal(0), normal(1), normal(2)};
  std::vector<Real> edge_point1 = {p1(0), p1(1), p1(2)};
  std::vector<Real> edge_point2 = {p2(0), p2(1), p2(2)};
  std::vector<Real> cut_point = {0.0, 0.0, 0.0};
                                      ...
                                      Point & pint) const
{
  ...
  std::vector<Real> plane_point = {center(0), center(1), center(2)};
  std::vector<Real> planenormal = {normal(0), normal(1), normal(2)};
  std::vector<Real> p_begin = {p1(0), p1(1), p1(2)};
  std::vector<Real> p_end = {p2(0), p2(1), p2(2)};
  std::vector<Real> cut_point = {0.0, 0.0, 0.0};
...
MeshCut3DUserObject::findBoundaryNodes()
{
  ...
  std::vector<Real> angle(n_nodes, 0);
  std::vector<dof_id_type> node_id(_cut_elem_nnode);

  const auto elem_begin = _cut_mesh->elements_begin();
  auto elem_end = _cut_mesh->elements_end();
  auto elem_it = elem_begin;

  for (; elem_it != elem_end; ++elem_it)
  {
    Elem * cut_elem = *elem_it;

    std::vector<std::vector<Real>> vertices(_cut_elem_nnode, std::vector<Real>(LIBMESH_DIM));
  ...
  {
    if (!MooseUtils::relativeFuzzyEqual(angle[i], libMesh::pi * 2))
    {
      auto ptr = std::make_shared<CutPoint>();
      _boundary_map[i] = ptr;
      Node * this_node = _cut_mesh->node_ptr(i);
      mooseAssert(this_node, "Node is NULL");
      Point & this_point = *this_node;

      mooseAssert(_boundary_map.find(i) != _boundary_map.end(),
                  "_boundary_map does not have this key");

      for (unsigned int j = 0; j < LIBMESH_DIM; ++j)
        _boundary_map.find(i)->second->coord[j] = this_point(j);
    }
  }
}
    ...
    else
    {
      ...
      for (unsigned int i = 0; i < _cut_elem_nnode; ++i)
      {
        ...
        if (is_node_on_boundary[i] && is_node_on_boundary[(i + 1 <= 2) ? i + 1 : 0])
        {
          ...
          auto ptr = std::make_shared<CutEdge>();
          _boundary_edges.push_back(ptr);
          _boundary_edges.back()->id1 = node1;
          _boundary_edges.back()->id2 = node2;
        }
      }
    }
    ++counter;
...
MeshCut3DUserObject::sortBoundaryNodes()
{
  ...
  {
    BoundaryID node1 = (*it)->id1;
    BoundaryID node2 = (*it)->id2;
    ...
    _boundary_map.find(node1)->second->neighbor.push_back(node2);
    _boundary_map.find(node2)->second->neighbor.push_back(node1);
  }
  ...
  BoundaryID node1 = _boundary_edges[0]->id1;
  BoundaryID node2 = _boundary_edges[0]->id2;
  ...
  for (unsigned int i = 0; i < _boundary_edges.size() - 1; ++i)
  {
    ...
    BoundaryID node3 = _boundary_map.find(node2)->second->neighbor[0];
    BoundaryID node4 = _boundary_map.find(node2)->second->neighbor[1];
...
{
  std::vector<BoundaryID> new_boundary_order(_boundary.begin(), _boundary.end());

  mooseAssert(_boundary.size() >= 2, "Boundary should have at least two nodes");

  for (unsigned int i = _boundary.size() - 1; i >= 1; --i)
  {
    BoundaryID node1 = _boundary[i - 1];
    BoundaryID node2 = _boundary[i];
    ...
    if (distance > _size_control)
    {
      ...
      for (unsigned int j = 0; j < n; ++j)
      {
        ...
        BoundaryID id = _cut_mesh->n_nodes() - 1;
...
// find growth direction of each boundary node; this is currently assgined by a hard-coded growth
// function;
...
// and/or (2) growth direction determined by fracture mechanics
{
  ...
  for (unsigned int i = 0; i < _active_boundary.size(); ++i)
  {
    ...
    for (unsigned int j = i1; j < i2; ++j)
    {
      ...
      mooseAssert(this_node, "Node is NULL");
      Point & this_point = *this_node;
      std::array<Real, LIBMESH_DIM> x;
      for (unsigned int j = 0; j < LIBMESH_DIM; ++j)
        x[j] = this_point(j);

      // Here is the predefined crack growth direction
      dir(0) = 5 * (x[0] - 0.3) + x[2];
...
// TBD; it is not needed for the current problems but will be useful for fracture growth
{
}
  ...
  for (unsigned int i = 0; i < _front.size(); ++i)
  {
    if (_front[i].size() >= 2)
    {
      ...
      if (length2.size() != 0 && do_inter2)
      {
        ...
        BoundaryID n = _cut_mesh->n_nodes() - 1;
...
{
  std::vector<std::vector<BoundaryID>> new_front(_front.begin(), _front.end());
  ...
  for (unsigned int ifront = 0; ifront < _front.size(); ++ifront)
  {
    ...
    for (unsigned int i = i1; i >= 1; --i)
    {
      ...
      BoundaryID node1 = _front[ifront][i - 1];
      BoundaryID node2 = _front[ifront][i2];
      Real distance = findDistance(node1, node2);
...
MeshCut3DUserObject::triangulation()
{
  ...
  for (unsigned int k = 0; k < _front.size(); ++k)
  {
    ...
    {
      std::vector<BoundaryID> elem;

      BoundaryID p1 = _active_boundary[k][i1]; // node in the old front
      BoundaryID p2 = _front[k][i2];           // node in the new front

      if (i1 != n1 - 1 && i2 != n2 - 1)
      {
        BoundaryID p3 = _active_boundary[k][i1 + 1]; // next node in the old front
        BoundaryID p4 = _front[k][i2 + 1];           // next node in the new front
...
MeshCut3DUserObject::joinBoundary()
{
  ...
  std::vector<BoundaryID> full_front;
  ...
  for (unsigned int i = 0; i < size1; ++i)
  {
    ...
    BoundaryID bd1 = _active_boundary[i][size2 - 1];
    BoundaryID bd2 = _active_boundary[i + 1 < size1 ? i + 1 : 0][0];
