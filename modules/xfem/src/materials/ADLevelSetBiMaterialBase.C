...
InputParameters
ADLevelSetBiMaterialBase::validParams()
{
  InputParameters params = ADMaterial::validParams();
  params.addClassDescription("Compute an AD material property for bi-materials (consisting of two  "
...
ADLevelSetBiMaterialBase::ADLevelSetBiMaterialBase(const InputParameters & parameters)
  : ADMaterial(parameters),
    _base_name(isParamValid("base_name") ? getParam<std::string>("base_name") + "_" : ""),
    _prop_name(getParam<std::string>("prop_name")),
    _level_set_var_number(this->_subproblem
                              .getVariable(this->_tid,
                                           parameters.get<VariableName>("level_set_var"),
                                           Moose::VarKindType::VAR_ANY,
                                           Moose::VarFieldType::VAR_FIELD_STANDARD)
                              .number()),
    _system(this->_subproblem.getSystem(getParam<VariableName>("level_set_var"))),
    _solution(this->_system.current_local_solution.get()),
    _use_positive_property(false)
{
  FEProblemBase * fe_problem = dynamic_cast<FEProblemBase *>(&(this->_subproblem));
...
void
ADLevelSetBiMaterialBase::computeProperties()
{
  ...
  ADMaterial::computeProperties();
}

void
ADLevelSetBiMaterialBase::computeQpProperties()
{
