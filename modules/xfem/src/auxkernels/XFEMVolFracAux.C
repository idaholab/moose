...
{
  if (isNodal())
    mooseError2("XFEMVolFracAux must be run on an element variable");
  FEProblemBase * fe_problem = dynamic_cast<FEProblemBase *>(&_subproblem);
  if (fe_problem == NULL)
    mooseError2("Problem casting _subproblem to FEProblemBase in XFEMVolFracAux");
  _xfem = MooseSharedNamespace::dynamic_pointer_cast<XFEM>(fe_problem->getXFEM());
  if (_xfem == NULL)
    mooseError2("Problem casting to XFEM in XFEMVolFracAux");
}
