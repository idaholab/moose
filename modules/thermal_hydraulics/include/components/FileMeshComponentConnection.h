...
/**
 * Base class for mesh-based component junctions and boundaries
 */
class FileMeshComponentConnection : public Component
{
public:
  FileMeshComponentConnection(const InputParameters & params);
...
template <typename T>
void
FileMeshComponentConnection::checkSizeEqualsNumberOfConnections(const std::string & param) const
{
...
template <typename T>
void
FileMeshComponentConnection::checkAllConnectionsHaveSame(const std::vector<T> & objects,
                                                         const std::string & description) const
{
...
{
template <>
FileMeshComponentConnection::EEndType stringToEnum(const std::string & s);
}

namespace Moose
{
std::string
stringify(FileMeshComponentConnection::EEndType end_type)
{
  if (end_type == FileMeshComponentConnection::EEndType::IN)
    return "IN";
  else if (end_type == FileMeshComponentConnection::EEndType::OUT)
    return "OUT";
  else
    return std::to_string(end_type) + " (unrecognized)";
}
}
