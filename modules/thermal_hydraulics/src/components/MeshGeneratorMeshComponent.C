...
#include "MeshGeneratorMeshComponent.h"
#include "THMMesh.h"
#include "MooseUtils.h"
#include "libmesh/exodusII_io.h"
#include "libmesh/exodusII_io_helper.h"

registerMooseObject("ThermalHydraulicsApp", MeshGeneratorMeshComponent);

InputParameters
MeshGeneratorMeshComponent::validParams()
{
  InputParameters params = GeometricalComponent::validParams();

  params.addRequiredParam<MeshGeneratorName>("generator", "The mesh generator providing the mesh");
  params.addRequiredParam<Point>("position", "Translation vector for the file mesh [m]");

  params.addClassDescription("Loads a mesh from a mesh generator without adding physics.");

  return params;
}

MeshGeneratorMeshComponent::MeshGeneratorMeshComponent(const InputParameters & parameters)
  : GeometricalComponent(parameters),
    _mg_name(getParam<MeshGeneratorName>("generator")),
    _position(getParam<Point>("position"))
{
}

void
MeshGeneratorMeshComponent::setupMesh()
{
  buildMesh();
...
MeshGeneratorMeshComponent::buildMesh()
{
...
bool
MeshGeneratorMeshComponent::hasBoundary(const BoundaryName & boundary_name) const
{
...
MeshGeneratorMeshComponent::getBoundaryInfo(const BoundaryName & boundary_name) const
{
  checkSetupStatus(MESH_PREPARED);
