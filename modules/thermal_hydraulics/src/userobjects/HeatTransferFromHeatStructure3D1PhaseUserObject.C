...
registerMooseObject("ThermalHydraulicsApp", HeatTransferFromHeatStructure3D1PhaseUserObject);

InputParameters
HeatTransferFromHeatStructure3D1PhaseUserObject::validParams()
{
...
HeatTransferFromHeatStructure3D1PhaseUserObject::HeatTransferFromHeatStructure3D1PhaseUserObject(
    const InputParameters & parameters)
  : DerivativeMaterialInterfaceTHM<ElementUserObject>(parameters),
...
void
HeatTransferFromHeatStructure3D1PhaseUserObject::initialize()
{
}

void
HeatTransferFromHeatStructure3D1PhaseUserObject::execute()
{
  ...
  for (auto elem_id : _hs_elem_ids)
  {
    ...
    if (nearest_elem_id == _current_elem->id())
    {
      ...
      {
        _T_fluid_jacobian[_current_elem->id()].resize(n_qpts_fch);
        _T_fluid_jacobian[elem_id].resize(n_qpts_hs);
        for (unsigned int qp_hs = 0; qp_hs < n_qpts_hs; qp_hs++)
        {
          unsigned int nearest_qp = _elem_qp_map[elem_id][qp_hs];
          DenseVector<Real> jac(4);
          jac(THM3Eqn::EQ_MASS) = _dT_drhoA[nearest_qp];
          jac(THM3Eqn::EQ_MOMENTUM) = _dT_drhouA[nearest_qp];
          jac(THM3Eqn::EQ_ENERGY) = _dT_drhoEA[nearest_qp];
          jac(THM3Eqn::EQ_ENERGY + 1) = 0.0;

          _T_fluid_jacobian[_current_elem->id()][nearest_qp] = jac;
          _T_fluid_jacobian[elem_id][qp_hs] = jac;
        }
      }
    }
  }
...
void
HeatTransferFromHeatStructure3D1PhaseUserObject::finalize()
{
}

void
HeatTransferFromHeatStructure3D1PhaseUserObject::threadJoin(const UserObject & y)
{
  const HeatTransferFromHeatStructure3D1PhaseUserObject & uo =
      static_cast<const HeatTransferFromHeatStructure3D1PhaseUserObject &>(y);
  for (auto & it : uo._heated_perimeter)
    _heated_perimeter[it.first] = it.second;
  for (auto & it : uo._T_fluid)
    _T_fluid[it.first] = it.second;
  for (auto & it : uo._T_fluid_jacobian)
    _T_fluid_jacobian[it.first] = it.second;
  for (auto & it : uo._htc)
    _htc[it.first] = it.second;
}

const std::vector<Real> &
HeatTransferFromHeatStructure3D1PhaseUserObject::getHeatedPerimeter(dof_id_type element_id) const
{
...
const std::vector<Real> &
HeatTransferFromHeatStructure3D1PhaseUserObject::getHeatTransferCoeff(dof_id_type element_id) const
{
  Threads::spin_mutex::scoped_lock lock(Threads::spin_mtx);
...
const std::vector<Real> &
HeatTransferFromHeatStructure3D1PhaseUserObject::getTfluid(dof_id_type element_id) const
{
  Threads::spin_mutex::scoped_lock lock(Threads::spin_mtx);
...
HeatTransferFromHeatStructure3D1PhaseUserObject::getTfluidJacobian(dof_id_type element_id) const
{
  Threads::spin_mutex::scoped_lock lock(Threads::spin_mtx);
  auto it = _T_fluid_jacobian.find(element_id);
  if (it != _T_fluid_jacobian.end())
    return it->second;
  else
    mooseError(name(),
               ": Requested fluid temperature jacobian for element ",
               element_id,
               " was not computed.");
}
