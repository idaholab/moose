...
registerMooseObject("ThermalHydraulicsApp", OneDMomentumStagnationPandTBC);
...
OneDMomentumStagnationPandTBC::computeQpResidual()
{
  ...
  return _area[_qp] * (rho * _vel[_qp] * _vel[_qp] + p) * _normal * _test[_i][_qp];
}
...
OneDMomentumStagnationPandTBC::computeQpJacobian()
{
  ...
  return _area[_qp] * (drho_darhouA * _vel[_qp] * _vel[_qp] + rho * du2_darhouA + dp_darhouA) *
         _phi[_j][_qp] * _normal * _test[_i][_qp];
}
...
OneDMomentumStagnationPandTBC::computeQpOffDiagJacobian(unsigned jvar)
{
  if (jvar == _arhoA_var_number)
  {
    ...
    return _area[_qp] * (drho_darhoA * _vel[_qp] * _vel[_qp] + rho * du2_darhoA + dp_darhoA) *
           _phi[_j][_qp] * _normal * _test[_i][_qp];
  }
