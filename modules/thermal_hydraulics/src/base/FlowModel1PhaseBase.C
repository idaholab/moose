...
FlowModel1PhaseBase::validParams()
{
  ...
  params.addRequiredParam<FunctionName>("A_function", "Cross-sectional area function");
  ...
  params.addRequiredParam<Point>("start_point", "Start point for the flow channel");
  params.addRequiredParam<Point>("end_point", "End point for the flow channel");
  return params;
}
...
void
FlowModel1PhaseBase::addTHMNormalizedVariableStep(const std::string & variable, Real normalization)
{
  const std::string class_name = "THMNormalizedVariableStep";
  InputParameters params = _factory.getValidParams(class_name);
  params.set<std::vector<SubdomainName>>("block") = _flow_channel.getSubdomainNames();
  params.set<MooseEnum>("value_type") = "max";
  params.set<MooseFunctorName>("functor") = variable + "_change";
  params.set<Real>("normalization") = normalization;
                                                     ...
                                                     const std::string & equation)
{
  ...
  params.set<FunctionName>("A") = getParam<FunctionName>("A_function");
