...
#include "ADWallHeatTransferCoefficientKazimiFunctorMaterial.h"
#include "SinglePhaseFluidProperties.h"
#include "FlowModelSinglePhase.h"
#include "Numerics.h"

registerMooseObject("ThermalHydraulicsApp", ADWallHeatTransferCoefficientKazimiFunctorMaterial);

InputParameters
ADWallHeatTransferCoefficientKazimiFunctorMaterial::validParams()
{
...
ADWallHeatTransferCoefficientKazimiFunctorMaterial::
    ...
    _PoD(getParam<Real>("PoD"))
{
  addFunctorProperty<ADReal>(
      ...
      [this](const auto & r, const auto & t) -> ADReal
      {
        ...
        {
          mooseDoOnce(
              mooseWarning("The Kazimi-Carelli correlation is valid when Pe is between 10 and "
        ...
        ADReal Nu = 4.0 + 0.33 * std::pow(_PoD, 3.8) * std::pow(Pe / 100, 0.86) +
                    0.16 * std::pow(_PoD, 5.0);
