...
MeshAlignment::MeshAlignment(const MooseMesh & mesh, bool require_same_translation)
  : MeshAlignmentBase(mesh), _require_same_translation(require_same_translation), _meshes_are_coincident(false), _meshes_have_same_translation(false)
{
}
...
MeshAlignment::buildMapping()
{
  ...
  if (_primary_node_points.size() > 0 && _secondary_node_points.size() > 0)
  {
    ...
    for (std::size_t i_secondary = 0; i_secondary < _secondary_node_points.size(); i_secondary++)
    {
      ...
      const Point translation_vector = _primary_node_points[i_primary] - _secondary_node_points[i_secondary];

      if (i_secondary == 0)
        reference_translation_vector = translation_vector;
      else
      {
        if (!MooseUtils::absoluteFuzzyEqual((reference_translation_vector - translation_vector).norm(), 0))
    ...
    if (_meshes_have_same_translation && MooseUtils::absoluteFuzzyEqual(reference_translation_vector.norm(), 0))
      _meshes_are_coincident = true;
    else
      _meshes_are_coincident = false;
