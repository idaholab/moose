...
This kernel is intended for the use case of creating a closed loop flow path with one or more complementary instances of [`IncompressibleMomentumSPScalarKernel`](scalarkernels/IncompressibleMomentumSPScalarKernel.md) and a [`ParsedODEKernel`](scalarkernels/ParsedODEKernel.md) defining the relationship between mass flow rates in the different flow paths.
Please note, due to the intended use, the pressure gradient term in the residual is opposite that of the [`IncompressibleMomentumSPScalarKernel`](scalarkernels/IncompressibleMomentumSPScalarKernel.md).
Furthermore, use of this kernel also necessitates the use of a [`CoupledODETimeDerivative`](scalarkernels/CoupledODETimeDerivative.md) with v being the coupled reference mass flow rate, which adds the time derivative of the mass flow rate to the residual:
\begin{equation}
