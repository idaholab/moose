...
plt.plot(
    time, rp_turb, linestyle="--", marker="", color="cornflowerblue", label="Turbine"
)
...
ax.axvline(x=t2, linestyle=":", color="black", label="$t = t_2$")
plt.plot(
    time, mfr_comp, linestyle="-", marker="", color="indianred", label="Compressor"
)
plt.plot(
    time, mfr_turb, linestyle="--", marker="", color="cornflowerblue", label="Turbine"
)
ax.legend(frameon=False, prop={"size": 12}, loc="center left")
plt.tight_layout()
...
plt.plot(
    time,
    (power_turb + power_comp) / 1e3,
    linestyle="--",
    marker="",
    color="magenta",
    label="$\\dot{W}_{turb} + \\dot{W}_{comp}$",
)
