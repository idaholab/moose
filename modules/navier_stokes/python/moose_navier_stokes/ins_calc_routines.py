...
def gradVec2(u_vec, x, y):
    return sp.Matrix(
        [
            [sp.diff(u_vec[0], x), sp.diff(u_vec[1], x)],
            [sp.diff(u_vec[0], y), sp.diff(u_vec[1], y)],
        ]
    )
...
def L_momentum_traction(uvec, p, k, eps, x, y):
    ...
    visc_term = (
        -mu * divTen2(gradVec2(uvec, x, y) + gradVec2(uvec, x, y).transpose(), x, y)
    ).transpose()
    ...
    turbulent_visc_term = -(
        divTen2(
            ...
            * k**2
            / eps
            * (gradVec2(uvec, x, y) + gradVec2(uvec, x, y).transpose()),
            x,
            y,
        )
    ).transpose()
...
def L_momentum_laplace(uvec, p, k, eps, x, y):
    ...
    pressure_term = gradScalar2(p, x, y).transpose()
    turbulent_visc_term = -(
        divTen2(rho * cmu * k**2 / eps * (gradVec2(uvec, x, y)), x, y)
    ).transpose()
    source = conv_term + visc_term + pressure_term + turbulent_visc_term
    return source
...
def L_kin(uvec, k, eps, x, y):
    ...
    diff_term = -divVec2(
        (mu + rho * cmu * k**2 / eps / sigk) * gradScalar2(k, x, y), x, y
    )
...
def L_eps(uvec, k, eps, x, y):
    ...
    diff_term = -divVec2(
        (mu + rho * cmu * k**2 / eps / sigeps) * gradScalar2(eps, x, y), x, y
    )
...
def wall_function_momentum_traction(
    uvec, nvec, p, k, eps, x, y, tau_type, symbolic=True, parts=True
):
    if symbolic:
        cmu = sp.var("c_{\mu}")
    ...
    if tau_type == "vel":
        ...
        uTau = cmu**0.25 * sp.sqrt(k)
    else:
        raise ValueError("Must either pass 'vel' or 'kin' for tau_type")

    mu, rho = sp.var("mu rho")
    normal_stress_term = (
        -nvec.transpose() * mu * strain_rate(uvec, x, y) * nvec * nvec.transpose()
    ).transpose()
    tangential_stress_term = uTau / yStarPlus * uvec
    muT = rho * cmu * k * k / eps
    turbulent_stress_term = (
        -nvec.transpose() * muT * strain_rate(uvec, x, y)
    ).transpose()
    if parts:
        pressure_term = (nvec.transpose() * eye2() * p).transpose()
    else:
...
def ins_epsilon_wall_function_bc(nvec, k, eps, x, y):
    ...
    mu, rho = sp.var("mu rho")
    muT = rho * cmu * k**2 / eps
    return -(mu + muT / sigEps) * kappa * cmu**0.25 * sp.sqrt(k) * eps * rho / muT
