...
ConservedVarMaterial::validParams()
{
  ...
  params.addCoupledVar(
      NS::T_fluid, "optionally use for approximating second spatial derivatives for stabilization");
...
ConservedVarMaterial::ConservedVarMaterial(const InputParameters & params)
  : VarMaterialBase(params),
    ...
    _var_grad_grad_rho_v(isCoupled(NS::momentum_y) ? adCoupledSecond(NS::momentum_y)
                                                   : _ad_second_zero),
    _var_grad_grad_rho_w(isCoupled(NS::momentum_z) ? adCoupledSecond(NS::momentum_z)
                                                   : _ad_second_zero),
    ...
    _var_rho_w_dot(isCoupled(NS::momentum_z) && _is_transient ? adCoupledDot(NS::momentum_z)
                                                              : _ad_zero),
    _var_grad_grad_T_fluid(isCoupled(NS::T_fluid) ? adCoupledSecond(NS::T_fluid) : _ad_second_zero),
    ...
    _mass_flux(declareADProperty<RealVectorValue>(NS::mass_flux))
{
  ...
  if (_have_porosity)
    if (!isParamValid("velocity_is_superficial"))
      mooseError("If the porosity parameter is supplied, then a value for "
                 "'velocity_is_superficial' must be supplied");
}
...
ConservedVarMaterial::coupledAuxiliaryVariables() const
{
  ...
  return sys.hasVariable(NS::density) || sys.hasVariable(NS::total_energy_density) ||
         sys.hasVariable(NS::momentum_x) || sys.hasVariable(NS::momentum_y) ||
         sys.hasVariable(NS::momentum_z);
}
...
ConservedVarMaterial::computeQpProperties()
{
  ...
  _grad_grad_vel_x[_qp] =
      (_var_grad_grad_rho_u[_qp] - outer_product(_grad_rho[_qp], _grad_vel_x[_qp]) -
       outer_product(_grad_vel_x[_qp], _grad_rho[_qp]) -
       _velocity[_qp](0) * _var_grad_grad_rho[_qp]) /
      _rho[_qp];
  _grad_grad_vel_y[_qp] =
      (_var_grad_grad_rho_v[_qp] - outer_product(_grad_rho[_qp], _grad_vel_y[_qp]) -
       outer_product(_grad_vel_y[_qp], _grad_rho[_qp]) -
       _velocity[_qp](1) * _var_grad_grad_rho[_qp]) /
      _rho[_qp];
  _grad_grad_vel_z[_qp] =
      (_var_grad_grad_rho_w[_qp] - outer_product(_grad_rho[_qp], _grad_vel_z[_qp]) -
       outer_product(_grad_vel_z[_qp], _grad_rho[_qp]) -
       _velocity[_qp](2) * _var_grad_grad_rho[_qp]) /
      _rho[_qp];

  const auto vel2_prefactor = (_have_porosity && _velocity_is_superficial)
                                  ? 1. / ((*_epsilon)[_qp] * (*_epsilon)[_qp])
                                  : Real(1);
  ...
  auto grad_e = _grad_rho_et[_qp] * _v[_qp] + _total_energy_density[_qp] * grad_vol -
                vel2_prefactor *
                    (_velocity[_qp](0) * _grad_vel_x[_qp] + _velocity[_qp](1) * _grad_vel_y[_qp] +
                     _velocity[_qp](2) * _grad_vel_z[_qp]);
  ...
  auto de_dt = _drho_et_dt[_qp] / _rho[_qp] -
               _total_energy_density[_qp] / (_rho[_qp] * _rho[_qp]) * _drho_dt[_qp] -
               vel2_prefactor * (_velocity[_qp](0) * dvelx_dt + _velocity[_qp](1) * dvely_dt +
                                 _velocity[_qp](2) * dvelz_dt);
