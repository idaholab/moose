...
PrimitiveVarMaterial::computeQpProperties()
{
  ...
  _specific_internal_energy[_qp] = _fluid.e_from_p_T(_pressure[_qp], _T_fluid[_qp]);
  ...
  _rho[_qp] = rho;
  _total_energy_density[_qp] = _rho[_qp] * (_specific_internal_energy[_qp] + 0.5 * (_velocity[_qp] * _velocity[_qp]));
  ...
  _grad_rho_et[_qp] = _rho[_qp] * (grad_e + _velocity[_qp](0) * _grad_vel_x[_qp] + _velocity[_qp](1) * _grad_vel_y[_qp] + _velocity[_qp](2) * _grad_vel_z[_qp]) +
                    _total_energy_density[_qp] / _rho[_qp] * _grad_rho[_qp];
  ...
  _drho_et_dt[_qp] =
      ...
      _total_energy_density[_qp] / _rho[_qp] * _drho_dt[_qp];
