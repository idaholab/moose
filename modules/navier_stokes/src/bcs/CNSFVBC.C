...
InputParameters validParams<CNSFVBC>()
{
  ...
  params.addParam<MooseEnum>("component", component,
  "Choose one of the conservation equations");
  ...
  params.addRequiredParam<UserObjectName>("flux",
  "The name of boundary flux object to use");
...
CNSFVBC::computeQpResidual()
{
  ...
  std::vector<Real> uvec1 = {_rho1[_qp],
                             _rhou1[_qp],
                             _rhov1[_qp],
                             _rhow1[_qp],
                             _rhoe1[_qp]};
...
{
  std::vector<Real> uvec1 = {_rhoc1[_qp],
                             _rhouc1[_qp],
                             _rhovc1[_qp],
                             _rhowc1[_qp],
                             _rhoec1[_qp]};
  ...
  const DenseMatrix<Real> & fjac1 = _flux.getJacobian(_current_side,
                                                      _current_elem->id(),
                                                      uvec1,
                                                      _normals[_qp],
                                                      _tid);
