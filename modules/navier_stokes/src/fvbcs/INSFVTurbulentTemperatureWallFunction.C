...
INSFVTurbulentTemperatureWallFunction::INSFVTurbulentTemperatureWallFunction(const InputParameters & parameters)
  : FVFluxBC(parameters),
...
INSFVTurbulentTemperatureWallFunction::computeQpResidual()
{
  ...
  {
    alpha = _kappa(current_argument) / (_rho(current_argument) * _cp(current_argument));
  }
  else if (y_plus >= 30.0)
  {
    auto Pr = _cp(current_argument) * _mu(current_argument) / _kappa(current_argument);
    auto Pr_ratio = Pr / _Pr_t(current_argument);
    auto jayatilleke_P = 9.24 * (std::pow(Pr_ratio, 0.75) - 1.0) * (1.0 + 0.28 * std::exp(-0.007*Pr_ratio));
    auto wall_scaling = 1.0/_von_karman * std::log(_E*y_plus) + jayatilleke_P;
    alpha = u_tau * wall_dist / (_Pr_t(current_argument) * wall_scaling);
  }
  else
  {
    auto alpha_lam = _kappa(current_argument) / (_rho(current_argument) * _cp(current_argument));
    auto Pr = _cp(current_argument) * _mu(current_argument) / _kappa(current_argument);
    auto Pr_ratio = Pr / _Pr_t(current_argument);
    auto jayatilleke_P = 9.24 * (std::pow(Pr_ratio, 0.75) - 1.0) * (1.0 + 0.28 * std::exp(-0.007*Pr_ratio));
    auto wall_scaling = 1.0/_von_karman * std::log(_E*y_plus) + jayatilleke_P;
    auto alpha_turb = u_tau * wall_dist / (_Pr_t(current_argument) * wall_scaling);
  ...
  return - _rho(current_argument) * _cp(current_argument) * alpha * (_T_w(face_arg) - _var(current_argument)) / wall_dist;
}
