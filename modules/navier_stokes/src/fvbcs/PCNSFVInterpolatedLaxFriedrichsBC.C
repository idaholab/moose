...
#include "PCNSFVInterpolatedLaxFriedrichsBC.h"
#include "NS.h"
#include "SinglePhaseFluidProperties.h"
#include "Function.h"
#include "Limiter.h"

registerMooseObject("MooseApp", PCNSFVInterpolatedLaxFriedrichsBC);

using namespace Moose::FV;

InputParameters
PCNSFVInterpolatedLaxFriedrichsBC::validParams()
{
...
PCNSFVInterpolatedLaxFriedrichsBC::PCNSFVInterpolatedLaxFriedrichsBC(const InputParameters & params)
  : FVFluxBC(params),
...
void
PCNSFVInterpolatedLaxFriedrichsBC::computeResidual(const FaceInfo & fi)
{
  _face_info = &fi;
  _normal = fi.normal();
  ...
  if (ft == FaceInfo::VarFaceNeighbors::ELEM)
    ...
    mooseError("A PCNSFVInterpolatedLaxFriedrichsBC is being triggered on an internal face with "
               "centroid: ",
               fi.faceCentroid());
  else
    mooseError("A PCNSFVInterpolatedLaxFriedrichsBC is being triggered on a face which does not "
               "connect to a block ",
...
void
PCNSFVInterpolatedLaxFriedrichsBC::computeJacobian(const FaceInfo & fi)
{
  _face_info = &fi;
  _normal = fi.normal();
  auto ft = fi.faceType(_var.name());
...
ADReal
PCNSFVInterpolatedLaxFriedrichsBC::computeQpResidual()
{
