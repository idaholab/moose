...
INSFVTurbulentViscosityWallFunction::boundaryValue(const FaceInfo & fi) const
{
  ...
  ADRealVectorValue velocity(_u_var(current_argument));
  if (_v_var)
    velocity(1) = (*_v_var)(current_argument);
  if (_w_var)
    velocity(2) = (*_w_var)(current_argument);
  ...
  if (_linearized_yplus)
  {
    ...
    const ADReal b_c = 1 / karman_cte * (std::log(E * wall_dist / _mu(current_argument)) + 1.0);
    const ADReal c_c = parallel_speed;
    u_tau = (-b_c + std::sqrt(std::pow(b_c, 2) + 4.0 * a_c * c_c)) / (2.0 * a_c);
  }
  else
    u_tau = NS::findUStar(_mu(current_argument), _rho(current_argument), parallel_speed, wall_dist);

  if (_non_equilibrium_treatement)
    u_tau = std::pow(_C_mu(current_argument), -0.25) * std::pow(_k(current_argument), -0.5) *
            std::pow(u_tau, 2);

  ADReal y_plus = wall_dist * u_tau * _rho(current_argument) / _mu(current_argument);

  if (y_plus <= 5.0) // sub-laminar layer
  {
    return _mu(current_argument);
  }
  else if (y_plus >= 30.0)
  {
    auto wall_val = Utility::pow<2>(u_tau) * _rho(current_argument) * wall_dist / parallel_speed;
    return wall_val + _mu(current_argument);
  }
  else
  {
    auto wall_val_log =
        Utility::pow<2>(u_tau) * _rho(current_argument) * wall_dist / parallel_speed +
        _mu(current_argument);
    auto blending_function = (y_plus - 5.0) / 25.0;
    auto wall_val =
        blending_function * wall_val_log + (1.0 - blending_function) * _mu(current_argument);
    return wall_val;
  }
}
