...
VolumetricFlowRate::computeQpIntegral()
{
  ...
  if (_fv)
  {
    ...
    if (fi->isBoundary())
      mooseError(
          "Use BoundaryVolumetricFlowRate instead of VolumetricFlowRate for domain boundaries");
    ...
    if (parameters().isParamSetByUser("advected_variable"))
    {
      ...
      // If user did not request an interpolation method, use what the kernels are most likely to
      // use
