...
isPorosityJumpFace(const Moose::Functor<ADReal> & porosity, const FaceInfo & fi)
{
  ...
  const Moose::ElemArg elem{fi.elemPtr(), false};
  const Moose::ElemArg neighbor{fi.neighborPtr(), false};
  const auto eps_elem = porosity(elem), eps_neighbor = porosity(neighbor);
