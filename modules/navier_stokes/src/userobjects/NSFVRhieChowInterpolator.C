...
registerMooseObject("NavierStokesApp", NSFVRhieChowInterpolator);

std::unordered_map<const MooseApp *,
                   std::vector<std::unordered_map<const Elem *, VectorValue<ADReal>>>>
    NSFVRhieChowInterpolator::_rc_a_coeffs;

InputParameters
NSFVRhieChowInterpolator::validParams()
{
...
NSFVRhieChowInterpolator::NSFVRhieChowInterpolator(const InputParameters & params)
  : UserObject(params),
...
void
NSFVRhieChowInterpolator::initialize()
{
...
void
NSFVRhieChowInterpolator::setupFlowBoundaries(const BoundaryID bnd_id)
{
...
template <typename T>
void
NSFVRhieChowInterpolator::setupBoundaries(const BoundaryID bnd_id,
...
NSFVRhieChowInterpolator::rcCoeff(const Elem & elem, const ADReal & mu) const
{
...
VectorValue<ADReal>
NSFVRhieChowInterpolator::coeffCalculator(const Elem & elem, const ADReal & mu) const
{
  ...
  auto action_functor = [&coeff,
    ...
    if (onBoundary(*fi))
    {
      ...
      mooseError("The NSFVRhieChowInterpolator object ",
                 this->name(),
...
void
NSFVRhieChowInterpolator::interpolate(Moose::FV::InterpMethod m,
                                      ADRealVectorValue & v,
                                      const ADRealVectorValue & elem_v,
                                      const ADRealVectorValue & neighbor_v,
                                      const FaceInfo * face_info) const
{
  const Elem * const elem = &face_info->elem();
  const Elem * const neighbor = face_info->neighborPtr();

  if (onBoundary(*face_info))
  {
#ifndef NDEBUG
    bool flow_boundary_found = false;
    for (const auto b_id : face_info->boundaryIDs())
    ...
    v(0) = _u_var->getBoundaryFaceValue(*face_info);
    if (_v_var)
      v(1) = _v_var->getBoundaryFaceValue(*face_info);
    if (_w_var)
      v(2) = _w_var->getBoundaryFaceValue(*face_info);

    return;
  }

  Moose::FV::interpolate(
      Moose::FV::InterpMethod::Average, v, elem_v, neighbor_v, *face_info, true);
  ...
  const VectorValue<ADReal> & grad_p = _p_var->adGradSln(*face_info);

  // Get uncorrected pressure gradient. This will use the element centroid gradient if we are
  // along a boundary face
  const VectorValue<ADReal> & unc_grad_p = _p_var->uncorrectedAdGradSln(*face_info);

  const Point & elem_centroid = face_info->elemCentroid();
  const Point * const neighbor_centroid = neighbor ? &face_info->neighborCentroid() : nullptr;
  Real elem_volume = face_info->elemVolume();
  Real neighbor_volume = neighbor ? face_info->neighborVolume() : 0;
  const auto & elem_mu = _mu_elem[0];
  ...
  {
    const auto & neighbor_mu = _mu_neighbor[0];
    ...
    Moose::FV::interpolate(
        Moose::FV::InterpMethod::Average, face_D, elem_D, neighbor_D, *face_info, true);
  }
  else
    face_D = elem_D;
...
VectorValue<ADReal>
NSFVRhieChowInterpolator::coeffCalculator(const Elem &, const ADReal &) const
{
  mooseError("NSFVRhieChowInterpolator only works with global AD indexing");
}

void
NSFVRhieChowInterpolator::interpolate(Moose::FV::InterpMethod,
                                      ADRealVectorValue &,
                                      const ADRealVectorValue &,
                                      const ADRealVectorValue &) const
{
  mooseError("NSFVRhieChowInterpolator only works with global AD indexing");
}
#endif
...
bool
NSFVRhieChowInterpolator::onBoundary(const FaceInfo & fi) const
{
  const bool internal =
...
void
NSFVRhieChowInterpolator::clearRCCoeffs()
{
  std::cout << "EXECUTED NOW!!!! -> once a NL solve????" << std::endl;
  auto it = _rc_a_coeffs.find(&_app);
  mooseAssert(it != _rc_a_coeffs.end(),
              "No RC coeffs structure exists for the given MooseApp pointer");
  mooseAssert(_tid < it->second.size(),
...
void
NSFVRhieChowInterpolator::addDependencies(MooseObject * moose_object) const
{
  // Cast object to the right interfaces and
  // - add pressure dependency
  // moose_object->addMooseVariableDependency("pressure");

  // - add viscosity dependency
