...
RhieChowMassFlux::computeCellVelocity()
{
  ...
  const auto & pressure_gradient =
      _grad_p_corrected.empty() ? _pressure_system->linearFVGradientContainer() : _grad_p_corrected;
...
RhieChowMassFlux::selectPressureGradient(const bool updated_pressure)
{
  if (updated_pressure)
  {
    ...
    const auto & source =
        use_corrected ? _grad_p_corrected : _pressure_system->linearFVGradientContainer();
