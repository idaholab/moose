...
INSFVRhieChowInterpolator::validParams()
{
  ...
  params.addParam<VariableName>("disp_z", "The z-component of displacement");
      ...
      params.addParam<bool>(
          ...
          false,
          "Whether to pull all nonlocal 'a' coefficient data to our process. Note that 'nonlocal' "
          "means elements that we have access to (this may not be all the elements in the mesh if "
          "the "
          "mesh is distributed) but that we do not own.");
      ...
      if (_a_data_provided)
      {
        for (const auto tid : make_range(libMesh::n_threads()))
        {
          ...
          _a_aux[tid] = std::make_unique<Moose::VectorCompositeFunctor<ADReal>>(
              "RC_a_coeffs",
              UserObject::_subproblem.getFunctor<ADReal>(
                  deduceFunctorName("a_u"), tid, name(), true),
              *v_comp,
              *w_comp);
...
{
      _elem_range = std::make_unique<ConstElemRange>(
          _mesh.active_local_subdomain_set_elements_begin(blockIDs()),
...
INSFVRhieChowInterpolator::finalize()
{
      ...
      {
        for (const auto * const elem : as_range(_mesh.active_not_local_elements_begin(),
      ...
      // Then pull
      {
                                     ...
                                     const std::vector<VectorValue<ADReal>> & filled_data)
        {
          ...
          mooseAssert(elem_ids.size() == filled_data.size(),
...
INSFVRhieChowInterpolator::ghostADataOnBoundary(const BoundaryID boundary_id)
{
      ...
      for (auto elem_id : _moose_mesh.getBoundaryActiveSemiLocalElemIds(boundary_id))
      {
        ...
        if (elem->processor_id() != this->processor_id() &&
            elem->is_semilocal(this->processor_id()))
                                       ...
                                       const bool subtract_mesh_velocity) const
{
      ...
      for (const auto side : make_range(elem->n_sides()))
      {
        ...
        {
          mooseDoOnce(
              mooseWarning("Cannot compute Rhie Chow coefficients if not solving. Returning "
        ...
        // simply mark the face add the reverse pressure interpolation for these faces In brief,
        // this function is just marking the faces where the Rhie-Chow interpolation is inconsistent
            ...
            [this, &elem, &neighbor, &time, &fi, &correct_skewness](Point & loc_normal)
        {
          ...
          for (const auto side : make_range(elem->n_sides()))
          {
            ...
            if (Moose::FV::onBoundary(*this, *fi_loc))
            {
              const Elem * const boundary_elem =
                  hasBlocks(elem->subdomain_id()) ? elem : loc_neighbor;
          ...
          // Uncorrected interpolation - Step 3: interpolate element and neighbor reconstructed
          // values to the face
          interpolate(Moose::FV::InterpMethod::Average,
                      uncorrected_interp_vf,
                      elem_value,
                      neigh_value,
                      fi,
                      true);
            ...
            [this, &elem, &neighbor, &time, &fi, &correct_skewness](Point & loc_normal)
        {
          ...
          {
            const Elem * const boundary_elem = hasBlocks(elem->subdomain_id()) ? elem : neighbor;
            const Moose::FaceArg loc_boundary_face{&fi,
                                                   Moose::FV::LimiterType::CentralDifference,
                                                   true,
                                                   correct_skewness,
                                                   boundary_elem};
            ...
            for (const auto i : make_range(_volumetric_force.size()))
              value +=
                  (*_volumetric_force[i])(loc_boundary_face, time) * (loc_normal * fi.normal());
          }
          else
          {
        ...
        for (const auto i : make_range(_dim))
        {
          ...
          if (_bool_correct_vf)
          {
            // To solve the volume force incorrect interpolation, we add back the pressure gradient
            // to the RC-inconsistent faces regarding the marking method
            Point loc_direction;
            loc_direction(i) = 1.0;
            ...
            velocity(i) +=
                face_D(i) * face_eps * (grad_p(i) - unc_grad_p(i)) * correction_indicator;
          }
        }
