...
InputParameters validParams<CNSFVLeastSquaresSlopeReconstruction>()
{
  ...
  params.addClassDescription("A user object that performs the least-squares slope reconstruction to get the slopes of the P0 primitive variables.");
...
CNSFVLeastSquaresSlopeReconstruction::reconstructElementSlope()
{
  ...
  u[0][2] = rhovc * rhomc;
  u[0][3] = rhowc * rhomc;
  Real eintc  = rhoec * rhomc - 0.5 * (u[0][1] * u[0][1] +
                                       u[0][2] * u[0][2] +
                                       u[0][3] * u[0][3]);
    ...
    else
    {
      ...
      for (unsigned int ib = 0; ib < bndID.size(); ib++)
      {
        ...
        if (pos != _bnd_uo_name_map.end())
        {
          ...
          Real eintg = rhoeg * rhomg - 0.5 * (u[in][1] * u[in][1] +
                                              u[in][2] * u[in][2] +
                                              u[in][3] * u[in][3]);
