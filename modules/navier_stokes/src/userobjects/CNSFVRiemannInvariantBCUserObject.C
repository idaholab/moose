...
InputParameters validParams<CNSFVRiemannInvariantBCUserObject>()
{
  ...
  params.addClassDescription("A user object that computes the ghost cell values based on the Riemann invariant boundary condition.");
...
CNSFVRiemannInvariantBCUserObject::CNSFVRiemannInvariantBCUserObject(const InputParameters & parameters)
                                                     ...
                                                     const RealVectorValue & dwave) const
{
  ...
  else if (mach1 > -1. && mach1 <= 0.)
  {
    ...
    Real rplus = (uadv1 * nx + vadv1 * ny + wadv1 * nz)
               + 2. * csou1 / gamm1;

    Real rmins = (_inf_uadv * nx + _inf_vadv * ny + _inf_wadv * nz)
               - 2. * inf_csou / gamm1;
    ...
    urigh[2] = rhob * vadvb;
    urigh[3] = rhob * wadvb;
    urigh[4] = rhob * _fp.e(presb, rhob)
             + rhob * 0.5 * (uadvb * uadvb + vadvb * vadvb + wadvb * wadvb);
  }
