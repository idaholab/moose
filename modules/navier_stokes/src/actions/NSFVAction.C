...
NSFVAction::NSFVAction(InputParameters parameters)
  : Action(parameters),
    ...
    _smooth_porosity_name(isParamValid("smoothing_layers")
...
NSFVAction::addINSMomentumAdvectionKernels()
{
  ...
  if (_porous_medium_treatment)
    params.set<MooseFunctorName>(NS::porosity) = _smooth_porosity_name;
  ...
  if (_porous_medium_treatment)
  {
    ...
    for (unsigned int block_i = 0; block_i < num_used_blocks; ++block_i)
    {
      ...
      if (_use_friction_correction)
      {
        ...
        corr_params.set<MooseFunctorName>(NS::porosity) = _smooth_porosity_name;
