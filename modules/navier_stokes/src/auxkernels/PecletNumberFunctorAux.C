...
PecletNumberFunctorAux::computeValue()
{
  ...
  if (_use_qp_arg)
  {
    ...
    return _current_elem->hmax() * raw_value(_speed(qp_arg, current_time)) /
           raw_value(_alpha(qp_arg, current_time));
  }
  else
  {
    const auto elem_arg = makeElemArg(_current_elem);
    return _current_elem->hmax() * raw_value(_speed(elem_arg, current_time)) /
           raw_value(_alpha(elem_arg, current_time));
  }
}
