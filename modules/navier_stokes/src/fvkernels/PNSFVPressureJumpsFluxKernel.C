...
#include "PNSFVPressureJumpsFluxKernel.h"
#include "FVUtils.h"
#include "NS.h"

registerMooseObject("NavierStokesApp", PNSFVPressureJumpsFluxKernel);

InputParameters
PNSFVPressureJumpsFluxKernel::validParams()
{
...
PNSFVPressureJumpsFluxKernel::PNSFVPressureJumpsFluxKernel(const InputParameters & params)
  : FVFluxKernel(params),
...
ADReal
PNSFVPressureJumpsFluxKernel::computeQpResidual()
{
  return -_normal(_index) * (_pressure_neighbor[_qp] - _pressure_elem[_qp]) / 2;
}

void
PNSFVPressureJumpsFluxKernel::computeResidual(const FaceInfo & fi)
{
  ...
  if (ft == FaceInfo::VarFaceNeighbors::ELEM || ft == FaceInfo::VarFaceNeighbors::BOTH)
  {
    ...
    _local_re(0) = r * MetaPhysicL::raw_value(_eps_elem[_qp]);
  ...
  if (ft == FaceInfo::VarFaceNeighbors::NEIGHBOR || ft == FaceInfo::VarFaceNeighbors::BOTH)
  {
    ...
    _local_re(0) = r * MetaPhysicL::raw_value(_eps_neighbor[_qp]);
    accumulateTaggedLocalResidual();
  }
}

void
PNSFVPressureJumpsFluxKernel::computeJacobian(const FaceInfo & fi)
{
  if (skipForBoundary(fi))
    return;
