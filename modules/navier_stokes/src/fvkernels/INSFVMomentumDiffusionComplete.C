...
#include "INSFVMomentumDiffusionComplete.h"
...
registerMooseObject("NavierStokesApp", INSFVMomentumDiffusionComplete);

InputParameters
INSFVMomentumDiffusionComplete::validParams()
{
  auto params = INSFVMomentumDiffusion::validParams();
  params.addClassDescription(
      "Implements the full Laplace form of the viscous stress in the Navier-Stokes equation.");
...
INSFVMomentumDiffusionComplete::INSFVMomentumDiffusionComplete(const InputParameters & params)
...
ADReal
INSFVMomentumDiffusionComplete::computeStrongResidual()
{
  /// ------------------ Useful variables ----------------------
  ...
  auto mu_elem = has_elem ? _mu(elem_face) : _mu(neighbor_face);
  auto mu_neighbor = has_neighbor ? _mu(neighbor_face) : _mu(elem_face);
  ...
  ADReal mu_face;
  Real protection_mu = 1e-12;
  if (_harmonic_interpolation)
  {
    const auto inv_mu_neighbor = 1.0 / (mu_neighbor + protection_mu);
    mu_face = 1.0 / (coef_elem * (1/(mu_elem + protection_mu) - inv_mu_neighbor) + inv_mu_neighbor);
  }
  else
    mu_face = coef_elem * (mu_elem - mu_neighbor) + mu_neighbor;

  // if (mu_elem < 1e10)
  //   _console << "Mu Elem: " << mu_elem << std::endl;
  // if (mu_neighbor < 1e10)
  //   _console << "Mu Neigh: " << mu_neighbor << std::endl;
  // if (mu_face < 1e10)
  //   _console << "Mu Face: " << mu_face << std::endl;
  ...
  //const ADReal laplacian_part = gradient.row(_index) * _normal;
  const ADReal laplacian_part = _var.gradient(Moose::FV::makeCDFace(*_face_info, faceArgSubdomains())) * _face_info->normal();
