...
ADReal
PCNSFVKTDC::computeFlux(const bool upwind)
{
  Limiter & limiter = upwind ? *_upwind_limiter : *_limiter;
  auto & flux_container = upwind ? _current_upwind_fluxes : _current_ho_fluxes;

  // Perform primitive interpolations
  const auto pressure_elem = interpolate(limiter,
                                         _pressure_elem[_qp],
                                         _pressure_neighbor[_qp],
                                         _grad_pressure_elem[_qp],
                                         *_face_info,
                                         /*elem_is_up=*/true);
  const auto pressure_neighbor = interpolate(limiter,
                                             _pressure_neighbor[_qp],
                                             _pressure_elem[_qp],
                                             _grad_pressure_neighbor[_qp],
                                             *_face_info,
                                             /*elem_is_up=*/false);
  const auto T_fluid_elem = interpolate(limiter,
                                        _T_fluid_elem[_qp],
                                        _T_fluid_neighbor[_qp],
                                        _grad_T_fluid_elem[_qp],
                                        *_face_info,
                                        /*elem_is_up=*/true);
  const auto T_fluid_neighbor = interpolate(limiter,
                                            _T_fluid_neighbor[_qp],
                                            _T_fluid_elem[_qp],
                                            _grad_T_fluid_neighbor[_qp],
                                            *_face_info,
                                            /*elem_is_up=*/false);
  const auto sup_vel_x_elem = interpolate(limiter,
                                          _sup_vel_x_elem[_qp],
                                          _sup_vel_x_neighbor[_qp],
                                          _grad_sup_vel_x_elem[_qp],
                                          *_face_info,
                                          /*elem_is_up=*/true);
  const auto sup_vel_x_neighbor = interpolate(limiter,
                                              _sup_vel_x_neighbor[_qp],
                                              _sup_vel_x_elem[_qp],
                                              _grad_sup_vel_x_neighbor[_qp],
                                              *_face_info,
                                              /*elem_is_up=*/false);
  const auto sup_vel_y_elem = interpolate(limiter,
                                          _sup_vel_y_elem[_qp],
                                          _sup_vel_y_neighbor[_qp],
                                          _grad_sup_vel_y_elem[_qp],
                                          *_face_info,
                                          /*elem_is_up=*/true);
  const auto sup_vel_y_neighbor = interpolate(limiter,
                                              _sup_vel_y_neighbor[_qp],
                                              _sup_vel_y_elem[_qp],
                                              _grad_sup_vel_y_neighbor[_qp],
                                              *_face_info,
                                              /*elem_is_up=*/false);
  const auto sup_vel_z_elem = interpolate(limiter,
                                          _sup_vel_z_elem[_qp],
                                          _sup_vel_z_neighbor[_qp],
                                          _grad_sup_vel_z_elem[_qp],
                                          *_face_info,
                                          /*elem_is_up=*/true);
  const auto sup_vel_z_neighbor = interpolate(limiter,
                                              _sup_vel_z_neighbor[_qp],
                                              _sup_vel_z_elem[_qp],
  ...
  ADReal flux;
  if (_eqn == "mass")
    flux = 0.5 * (sup_vel_elem_normal * rho_elem + sup_vel_neighbor_normal * rho_neighbor -
  ...
  else if (_eqn == "momentum")
  {
    ...
    flux = 0.5 * (sup_vel_elem_normal * rhou_elem + sup_vel_neighbor_normal * rhou_neighbor +
  ...
  else if (_eqn == "energy")
  {
    ...
    flux = 0.5 * (sup_vel_elem_normal * rho_ht_elem + sup_vel_neighbor_normal * rho_ht_neighbor -
                  a * (rho_ht_neighbor - rho_ht_elem));
  }
  else if (_eqn == "scalar")
    flux = sup_vel_elem_normal * rho_elem * _scalar_elem[_qp] +
  ...
  auto pr =
#endif
      flux_container.emplace(_face_info->id(), flux.value());
...
PCNSFVKTDC::computeQpResidual()
{
  ...
  const auto current_upwind_flux = computeFlux(/*upwind=*/true);
  const auto current_ho_flux = computeFlux(/*upwind=*/false);
