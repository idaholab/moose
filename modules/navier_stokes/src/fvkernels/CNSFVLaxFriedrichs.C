...
#include "CNSFVLaxFriedrichs.h"
#include "NS.h"
#include "SinglePhaseFluidProperties.h"

registerMooseObject("MooseApp", CNSFVLaxFriedrichs);

InputParameters
CNSFVLaxFriedrichs::validParams()
{
...
CNSFVLaxFriedrichs::CNSFVLaxFriedrichs(const InputParameters & params)
  : FVFluxKernel(params),
...
void
CNSFVLaxFriedrichs::computeResidual(const FaceInfo & fi)
{
...
void
CNSFVLaxFriedrichs::computeJacobian(const FaceInfo & fi)
{
  if (skipForBoundary(fi))
    return;
...
void
CNSFVLaxFriedrichs::computeAValues()
{
  _Sf = _face_info->faceArea() * _face_info->faceCoord() * _face_info->normal();
  _vSf_elem = _vel_elem[_qp] * _Sf;
  _vSf_neighbor = _vel_neighbor[_qp] * _Sf;
...
ADReal
CNSFVLaxFriedrichs::computeQpResidual()
{
  computeAValues();
  ...
  if (_eqn == "mass")
    ...
    return _adjusted_vSf_elem * _rhou_elem[_qp](_index) +
           ...
           (_alpha_elem * _pressure_elem[_qp] + _alpha_neighbor * _pressure_neighbor[_qp]) *
               _Sf(_index);
  else if (_eqn == "energy")
    return _adjusted_vSf_elem * _rho_ht_elem[_qp] + _adjusted_vSf_neighbor * _rho_ht_neighbor[_qp] +
           // This term removes artifacts at boundaries in the particle velocity solution.
