#include "WCNSLinearFVSharpInterfaceFlowPhysics.h"
...
registerWCNSFVFlowPhysicsBaseTasks("NavierStokesApp", WCNSLinearFVSharpInterfaceFlowPhysics);
registerMooseAction(
    "NavierStokesApp", WCNSLinearFVSharpInterfaceFlowPhysics, "add_linear_fv_kernel");
registerMooseAction(
    "NavierStokesApp", WCNSLinearFVSharpInterfaceFlowPhysics, "add_linear_fv_bc");
registerMooseAction(
    "NavierStokesApp", WCNSLinearFVSharpInterfaceFlowPhysics, "add_functor_material");
...
InputParameters
WCNSLinearFVSharpInterfaceFlowPhysics::validParams()
{
  InputParameters params = WCNSFVFlowPhysicsBase::validParams();

  params.addClassDescription(
      "Define a linear-FV segregated sharp-interface Navier-Stokes flow solve with a reduced-"
      "pressure correction path, OpenFOAM-like curvature reconstruction, and optional extra "
      "pressure-predictor face fluxes.");
  ...
  params.transferParam<bool>(RhieChowMassFlux::validParams(), "split_momentum_predictor_operator");
  ...
  params.addParamNamesToGroup(
      "pressure_formulation split_momentum_predictor_operator add_transient_projection_flux "
      "add_capillary_hydrostatic_flux transient_projection_face_acceleration "
...
WCNSLinearFVSharpInterfaceFlowPhysics::WCNSLinearFVSharpInterfaceFlowPhysics(
        ...
        getParam<bool>("create_dynamic_contact_angle_functor_material"))
{
  ...
  if (_split_momentum_predictor_operator &&
    ...
    paramError("split_momentum_predictor_operator",
               "split_momentum_predictor_operator is only supported for the reduced-pressure "
               "dynamic-pressure path.");

  if (_split_momentum_predictor_operator && !getParam<bool>("use_cached_momentum_predictor_operator"))
    paramError("use_cached_momentum_predictor_operator",
               "split_momentum_predictor_operator requires "
               "use_cached_momentum_predictor_operator = true so the cached predictor operator "
               "remains the source of truth for HbyA/Ainv reconstruction.");
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::initializePhysicsAdditional()
{
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::addSolverVariables()
{
  if (!_has_flow_equations)
    return;
    ...
    else if (_define_variables)
    {
      ...
      paramError("velocity_variable",
                 "Variable (" + _velocity_names[d] +
                     ") supplied to the WCNSLinearFVSharpInterfaceFlowPhysics does not exist!");
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::addFVKernels()
{
  if (!_has_flow_equations)
    return;
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::addUserObjects()
{
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::addMomentumFluxKernels()
{
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::addMomentumPressureKernels()
{
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::addMomentumGravityKernels()
{
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::addMomentumReducedPressureKernels()
{
  if (_split_momentum_predictor_operator)
    return;
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::addMomentumFrictionKernels()
{
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::addMomentumBoussinesqKernels()
{
        ...
        momentum_outlet_type == "fixed-pressure-zero-gradient")
    {
      for (const auto d : make_range(dimension()))
      {
        ...
        const std::string bc_type = use_pressure_inlet_outlet_velocity
                                        ? "LinearFVPressureInletOutletVelocityBC"
...
MooseFunctorName
WCNSLinearFVSharpInterfaceFlowPhysics::generatedGeometryFunctorName(const std::string & base_name) const
{
  return prefix() + base_name;
}

bool
WCNSLinearFVSharpInterfaceFlowPhysics::shouldCreateGeometryFunctorMaterial() const
{
...
bool
WCNSLinearFVSharpInterfaceFlowPhysics::shouldCreateCurvatureProducer() const
{
...
bool
WCNSLinearFVSharpInterfaceFlowPhysics::shouldCreateDynamicContactAngleFunctorMaterial() const
{
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::addCurvatureUserObject()
{
  ...
  const std::string object_type = "SharpInterfaceCurvatureCalculator";
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::addRhieChowUserObjects()
{
  ...
  for (const auto & obj : objs)
    if (dynamic_cast<RhieChowMassFlux *>(obj))
    {
      ...
      if (dynamic_cast<SharpInterfaceRhieChowMassFlux *>(obj))
        have_sharp_rc_uo = true;
      else
        mooseError("Sharp-interface flow physics '",
                   name(),
                   "' requires a SharpInterfaceRhieChowMassFlux on blocks ",
  ...
  const std::string u_names[3] = {"u", "v", "w"};
  const auto object_type = "SharpInterfaceRhieChowMassFlux";
  ...
  params.set<bool>("split_momentum_predictor_operator") = _split_momentum_predictor_operator;
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::addDynamicContactAngleFunctorMaterial()
{
...
void
WCNSLinearFVSharpInterfaceFlowPhysics::addFunctorMaterials()
{
  if (parameters().isParamValid("gravity"))
  {
    const auto gravity_vector = getParam<RealVectorValue>("gravity");
  ...
  const std::string class_name = "SharpInterfaceGeometryFunctorMaterial";
  auto params = getFactory().getValidParams(class_name);
  assignBlocks(params, _blocks);
...
unsigned short
WCNSLinearFVSharpInterfaceFlowPhysics::getNumberAlgebraicGhostingLayersNeeded() const
{
  return 2;
}
