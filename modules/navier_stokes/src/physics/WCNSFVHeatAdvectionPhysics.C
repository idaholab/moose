                                                     ...
                                                     bool warn) const
{
  ...
  consistent =
      (consistent && parameterConsistent<FunctionName>(other_params, "initial_temperature", warn) &&
       parameterConsistent<MooseFunctorName>(other_params, "specific_heat", warn) &&
       parameterConsistent<MooseFunctorName>(other_params, "external_heat_source", warn) &&
       parameterConsistent<Real>(other_params, "external_heat_source_coeff", warn) &&
       parameterConsistent<bool>(other_params, "use_external_enthalpy_material", warn) &&
...
void
WCNSFVHeatAdvectionPhysics::processAdditionalParameters(const InputParameters & other_params)
{
  // Concatenates the boundary information
  ...
  _thermal_conductivity_blocks.insert(
      _thermal_conductivity_blocks.end(),
      other_params.get<std::vector<std::vector<SubdomainName>>>("thermal_conductivity_blocks")
          .begin(),
      other_params.get<std::vector<std::vector<SubdomainName>>>("thermal_conductivity_blocks")
          .end());
  _thermal_conductivity_name.insert(
      _thermal_conductivity_name.end(),
      other_params.get<std::vector<MooseFunctorName>>("thermal_conductivity_name").begin(),
      other_params.get<std::vector<MooseFunctorName>>("thermal_conductivity_name").end());

  // Add the block restricted ambient convection parameters
  _ambient_convection_blocks.insert(
      _ambient_convection_blocks.end(),
      other_params.get<std::vector<std::vector<SubdomainName>>>("ambient_convection_blocks")
          .begin(),
      other_params.get<std::vector<std::vector<SubdomainName>>>("ambient_convection_blocks").end());
  _ambient_convection_alpha.insert(
      _ambient_convection_alpha.end(),
      other_params.get<std::vector<MooseFunctorName>>("ambient_convection_alpha").begin(),
      other_params.get<std::vector<MooseFunctorName>>("ambient_convection_alpha").end());
  _ambient_temperature.insert(
      _ambient_temperature.end(),
      other_params.get<std::vector<MooseFunctorName>>("ambient_temperature").begin(),
      other_params.get<std::vector<MooseFunctorName>>("ambient_temperature").end());
}
