  ...
  for (const auto i : index_range(num_elem))
  {
    ...
    for (const auto & fi : all_fi)
    {
      auto moukalled_reconstruct = [&fi](auto & functor, auto & container)
      {
        ...
        const RealVectorValue uf(functor(face, Moose::currentTimeFunctorArg()));
    ...
    Moose::FV::interpolateReconstruct(up_weller, u, 1, true, faces, Moose::currentTimeFunctorArg());
    Moose::FV::interpolateReconstruct(up_tano, u, 1, false, faces, Moose::currentTimeFunctorArg());
    ...
    for (auto * const elem : lm_mesh.active_element_ptr_range())
    {
      ...
      const RealVectorValue analytic(u(elem_arg, Moose::currentTimeFunctorArg()));
    ...
    up_tano.clear();
    Moose::FV::interpolateReconstruct(up_tano, u, 2, false, faces, Moose::currentTimeFunctorArg());
