  ...
  for (const auto i : index_range(num_elem))
  {
    ...
    for (const auto & fi : all_fi)
    {
      auto moukalled_reconstruct = [&fi](auto & functor, auto & container)
      {
        ...
        const RealVectorValue uf(functor(face, determineState()));
    ...
    Moose::FV::interpolateReconstruct(up_weller, u, 1, true, faces, determineState());
    Moose::FV::interpolateReconstruct(up_tano, u, 1, false, faces, determineState());
    ...
    for (auto * const elem : lm_mesh.active_element_ptr_range())
    {
      ...
      const RealVectorValue analytic(u(elem_arg, determineState()));
          ...
          [elem_id, current_h, elem, &analytic](auto & container, auto & error)
      {
        ...
        const auto elem_point_eval =
            container(Moose::ElemPointArg({elem, elem->vertex_average(), false}), determineState());
    ...
    up_tano.clear();
    Moose::FV::interpolateReconstruct(up_tano, u, 2, false, faces, determineState());
    ...
    try
    {
      unrestricted_error_test(Moose::ElemArg({lm_mesh.elem_ptr(0), false}), determineState());
      EXPECT_TRUE(false);
    }
    ...
    try
    {
      restricted_error_test(Moose::ElemArg({lm_mesh.elem_ptr(0), false}), determineState());
      EXPECT_TRUE(false);
    }
    catch (std::runtime_error & e)
    {
