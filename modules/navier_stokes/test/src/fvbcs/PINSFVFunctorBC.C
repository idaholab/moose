...
PINSFVFunctorBC::computeQpResidual()
{
  ...
  const VectorValue<ADReal> sup_vel(
      ...
      _sup_vel_y ? (*_sup_vel_y)(boundary_face, current_time) : ADReal(0),
      _sup_vel_z ? (*_sup_vel_z)(boundary_face, current_time) : ADReal(0));
  const auto rho = _rho(boundary_face, current_time);

  if (_eqn == "mass")
    return rho * sup_vel * normal;
  else if (_eqn == "momentum")
  {
    const auto eps = _eps(boundary_face, current_time);
    ...
    return rhou * sup_vel * normal + eps * _pressure(boundary_face, current_time) * normal(_index);
  }
  else
