...
class TestSymmetricVortexOrthogonal(unittest.TestCase):
    def test(self):
        ...
        df1 = run_spatial('2d-symmetric-vortex.i', 4, y_pp=labels, mpi=2, file_base='2d-symmetric-vortex')
        ...
        for key,value in fig.label_to_slope.items():
            ...
            if key in velocity_labels:
                ...
                self.assertTrue(fuzzyAbsoluteEqual(value, 1.0, .25))
...
class TestSymmetricVortexNonorthogonal(unittest.TestCase):
    def test(self):
        ...
        for key,value in fig.label_to_slope.items():
            print("%s, %f" % (key, value))
            if key in velocity_labels:
                self.assertTrue(fuzzyAbsoluteEqual(value, 2., .25))
            else:
                self.assertTrue(fuzzyAbsoluteEqual(value, 1.0, .25))
