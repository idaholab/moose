...
class TestVortexNewton(unittest.TestCase):
    def test(self):
        ...
        fig.plot(df1, label=labels, marker='o', markersize=8, num_fitted_points=2, slope_precision=1)
...
class TestVortexNewtonDeviatoric(unittest.TestCase):
    def test(self):
        ...
        pressure_labels = ['L2p']
        labels = velocity_labels + pressure_labels
        df1 = run_spatial('newton.i', 4, "FVKernels/u_viscosity/complete_expansion=true FVKernels/v_viscosity/complete_expansion=true FVKernels/u_forcing/functor=forcing_u_deviatoric FVKernels/v_forcing/functor=forcing_v_deviatoric", y_pp=labels, mpi=4, file_base='newton-deviatoric')
...
class TestVortexSNL(unittest.TestCase):
    def test(self):
        ...
        for key,value in fig.label_to_slope.items():
            print("%s, %f" % (key, value))
            if key in velocity_labels:
                self.assertTrue(fuzzyAbsoluteEqual(value, 2.0, .31))
            else:
...
class TestVortexSNLDeviatoric(unittest.TestCase):
    def test(self):
        ...
        for key,value in fig.label_to_slope.items():
            print("%s, %f" % (key, value))
            if key in velocity_labels:
                self.assertTrue(fuzzyAbsoluteEqual(value, 2.0, .26))
            else:
                self.assertTrue(fuzzyAbsoluteEqual(value, 1.5, .25))
