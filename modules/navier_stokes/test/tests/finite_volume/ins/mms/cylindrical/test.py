...
class TestRZViscousSource(unittest.TestCase):
    def test(self):
        labels = ['L2u', 'L2v', 'L2p']
        df = run_spatial('2d-rc-rz-viscous-source.i', [0, 1, 2, 3], y_pp=labels, mpi=2)

        fig = mms.ConvergencePlot(xlabel='Element Size ($h$)', ylabel='$L_2$ Error')
        fig.plot(df, label=labels, marker='s', markersize=8, num_fitted_points=3, slope_precision=1)
