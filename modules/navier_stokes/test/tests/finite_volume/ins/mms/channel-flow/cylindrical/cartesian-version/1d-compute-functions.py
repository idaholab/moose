...
flux_u_left, _ = mms.evaluate(
    "(vel*rho*u).dot(-e_i)", u, variable="u", vel=vel, scalars=["mu", "rho"]
)
flux_u_right, _ = mms.evaluate(
    "(vel*rho*u).dot(e_i)", u, variable="u", vel=vel, scalars=["mu", "rho"]
)
flux_u_diffusion_left, _ = mms.evaluate(
    "(- mu * grad(u)).dot(-e_i)", u, variable="u", vel=vel, scalars=["mu", "rho"]
)
flux_u_diffusion_right, _ = mms.evaluate(
    "(- mu * grad(u)).dot(e_i)", u, variable="u", vel=vel, scalars=["mu", "rho"]
)
flux_p_left, _ = mms.evaluate(
    "(vel*rho).dot(-e_i)", p, variable="p", vel=vel, scalars=["rho"]
)
flux_p_right, _ = mms.evaluate(
    "(vel*rho).dot(e_i)", p, variable="p", vel=vel, scalars=["rho"]
)
...
mms.print_hit(
    flux_u_diffusion_right, "flux_u_diffusion_right", mu="${mu}", rho="${rho}"
)
