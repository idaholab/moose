# Turbulent flow in a heated channel where fluid enters cold and is heated by hot walls.
# Momentum and heat transport are modeled using a k-epsilon turbulence model with
# wall functions for both velocity and temperature.
...
[Variables]
  ...
  [TKE]
    ...
    initial_condition = ${fparse 3*k_init}
  []
  [TKED]
    type = MooseLinearVariableFVReal
    solver_sys = TKED_system
    initial_condition = ${fparse 3*eps_init}
  []
  [temperature]
    type = MooseLinearVariableFVReal
    solver_sys = energy_system
    initial_condition = ${temperature_init}
  []
[]
...
[LinearFVBCs]
  ...
  []
  [walls_k_t]
    type = LinearFVTemperatureWallFunctionBC
    boundary = 'bottom top'
    variable = 'k_eff'
    u = vel_x
    v = vel_y
...
[AuxKernels]
  ...
  []
  [compute_k_eff]
    type = EffectiveTurbulentThermalConductivityAux
    variable = k_eff
    Pr_t = ${Pr_t}
    cp = ${cp}
    k = ${k}
...
[Executioner]
  ...
  turbulence_equation_relaxation = '0.35 0.35'
  turbulence_field_relaxation = '0.35 0.35'
