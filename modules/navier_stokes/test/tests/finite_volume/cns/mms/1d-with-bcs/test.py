...
class Test1DFreeFlowHLLC(unittest.TestCase):
    def test(self):
        ...
        for key, value in fig.label_to_slope.items():
            print("%s, %f" % (key, value))
            self.assertTrue(fuzzyAbsoluteEqual(value, 1.0, 0.05))
...
class TestBasic1DPorousKTPrimitiveCD(unittest.TestCase):
    def test(self):
        ...
        df1 = run_spatial(
            "basic-primitive-pcnsfv-kt.i",
            7,
            "GlobalParams/limiter='central_difference'",
            "--error",
            y_pp=labels,
        )
        ...
        for key, value in fig.label_to_slope.items():
            ...
            self.assertTrue(fuzzyAbsoluteEqual(value, 2.0, 0.05))
...
class TestBasic1DVaryingPorousKNPPrimitiveCD(unittest.TestCase):
    def test(self):
        labels = ["L2pressure", "L2sup_vel_x", "L2T_fluid"]
        df1 = run_spatial(
            "varying-eps-basic-kt-primitive.i",
            7,
            "GlobalParams/limiter='central_difference'",
            y_pp=labels,
        )
...
class TestBasic1DVaryingPorousKTPrimitiveCD(unittest.TestCase):
    def test(self):
        ...
        df1 = run_spatial(
            ...
            7,
            "GlobalParams/limiter='central_difference'",
            "GlobalParams/knp_for_omega=false",
            "--error",
            y_pp=labels,
        )
