...
def writeGMSH(filename_base, ref, Q, TriFlag, E, V, NC, ni, nj):
    ...
    if Q == 1:
        GmshLineType = 1  # 2-node line
    if Q == 2:
        GmshLineType = 8  # 3-node line
    ...
    for j in xrange(nOutflow):
        f.write(
            str(nInflow + j + 5) + " " + str(GmshLineType) + " 2 " + str(BC) + " 0 "
        )
        # Write end points
    ...
    for i in xrange(nWall):
        f.write(
            str(nInflow + nOutflow + nWall + i + 5)
            + " "
            + str(GmshLineType)
            + " 2 "
            + str(BC)
