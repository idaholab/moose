 ...
 * physics for use by incompressible and weakly compressible flow models
 */
class INSFVAdvectionBase : public FVMatAdvection
{
public:
...
protected:
   ...
   * @param mu The dynamic viscosity
   */
  /// Note: coeffCalculator is defined in the subclass where the density variable is
  ///       available, it must be declared as a pure virtual method here.
  virtual VectorValue<ADReal> coeffCalculator(const Elem & elem, const ADReal & mu) const = 0;
