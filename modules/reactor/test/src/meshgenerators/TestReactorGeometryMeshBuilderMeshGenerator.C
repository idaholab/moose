...
TestReactorGeometryMeshBuilderMeshGenerator::TestReactorGeometryMeshBuilderMeshGenerator(
    const InputParameters & params)
  : MeshGenerator(params),
    _input(getParam<MeshGeneratorName>("input")),
    _input_mesh(&getMesh("input"))
{
  // Assumes input has been generated by CoreMeshGenerator and contains valid metadata for
  // generation
  const auto reactor_params_name = getMeshProperty<std::string>(RGMB::reactor_params_name, _input);

  // Get assembly pitch from metadata and define a circular mesh that preserves volume of assembly
  // structure
