...
If the [!param](/Mesh/AzimuthalBlockSplitGenerator/input) mesh is generated by [`PolygonConcentricCircleMeshGenerator`](/PolygonConcentricCircleMeshGenerator.md) with [!param](/Mesh/PolygonConcentricCircleMeshGenerator/quad_center_elements) set as true, and if the block that contains these quadrilateral central elements is selected in [!param](/Mesh/AzimuthalBlockSplitGenerator/old_blocks), an error message is issued because no nodes in that block should be moved.
