...
The external boundary has a default boundary id of 10000. The user may instead set specific boundary id and/or name values through the use of `external_boundary_id` and `external_boundary_name`. Additionally, for each half side of the polygon, an independent boundary id is generated for side-specific applications by default. The generation of these side-specific boundaries can be disabled by setting [!param](/Mesh/PolygonConcentricCircleMeshGenerator/generate_side_specific_boundaries) as `false`.
