...
Users can also set [!param](/Mesh/PolygonConcentricCircleMeshGenerator/replace_inner_ring_with_delaunay_mesh) to true to replace the inner ring with an unstructured Delaunay triangular mesh as illustrated in [Figure 4](#inner_delaunay).

!media reactor/meshgenerators/tri_delaunay.png
      style=display: block;margin-left:auto;margin-right:auto;width:40%;
      id=inner_delaunay
      caption=Example meshes when [!param](/Mesh/PolygonConcentricCircleMeshGenerator/replace_inner_ring_with_delaunay_mesh) is set as `false` and `true`.

The left figure is generated with two intervals for the inner ring and the right figure is generated with [!param](/Mesh/PolygonConcentricCircleMeshGenerator/replace_inner_ring_with_delaunay_mesh) equal to true. This feature is useful in a situation when the number of total sectors is large making the center point connected with too many elements. When this parameter is true, [!param](/Mesh/PolygonConcentricCircleMeshGenerator/quad_center_elements) cannot be set otherwise an error will be thrown. The first element of [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_intervals) is ignored and the element areas are controlled by [!param](/Mesh/PolygonConcentricCircleMeshGenerator/inner_ring_desired_area) function. The function can be a constant value. The default empty string means that the code will pick a default desired area which is the square of the smallest side length on the ring circle. The unstructured Delaunay mesh of the inner ring will have the triangle element type specified in [!param](/Mesh/PolygonConcentricCircleMeshGenerator/tri_element_type). It will also have the subdomain ID/name of the original subdomain ID/name of the triangular elements of the inner ring, the first value in [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_block_ids) and [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_block_names). If the first element of [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_intervals) is greater than one, the second elements of [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_block_ids) and [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_block_names) for the inner ring are ignored. When [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_id_name) is specified, the element ring IDs will be all equal to one. When [!param](/Mesh/PolygonConcentricCircleMeshGenerator/sector_id_name) is specified, the element sector IDs will be all equal to zero. If the new feature is used, the first elements of [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_inner_boundary_layer_biases), [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_inner_boundary_layer_intervals), [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_inner_boundary_layer_widths), [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_inner_boundary_layer_biases), [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_inner_boundary_layer_intervals), [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_inner_boundary_layer_widths) and [!param](/Mesh/PolygonConcentricCircleMeshGenerator/ring_radial_biases) are all ignored.
