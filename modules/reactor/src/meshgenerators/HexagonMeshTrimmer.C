...
HexagonMeshTrimmer::validParams()
{
  ...
  params.addRangeCheckedParam<std::vector<unsigned short>>(
      ...
      "Whether the peripheral region on each of the six sidesto will be trimmed in an assembly "
      "mesh."); // need a test
  params.addParam<BoundaryName>("peripheral_trimming_section_boundary",
                                "Boudary formed by peripheral trimming.");
  ...
  params.addParam<BoundaryName>("center_trimming_section_boundary",
                                "Boudary formed by center trimming.");
  ...
  params.addParam<SubdomainName>("tri_elem_subdomain_name_suffix",
                                 "trimmer_tri",
                                 "SubdomainName postfix used to rename the converted triangular "
                                 "elements to avert degenerate quad elements.");
  ...
  params.addClassDescription(
      "This HexagonMeshTrimmer object perform peripheral and/or across-center trimming for "
      "assembly or core mesh generated by PatternedHexMG.");
                                          ...
                                          const std::set<subdomain_id_type> subdomain_ids_set)
{
  ...
  {
    auto elem_angles = vertex_angles(*elem);
