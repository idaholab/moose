...
ReactorMeshParams::validParams()
{
  ...
  params.addParam<bool>("flexible_assembly_stitching", false, "Use FlexiblePatternGenerator for stitching dissimilar assemblies together");
  params.addRangeCheckedParam<unsigned int>("num_sectors_at_flexible_boundary", 6, "num_sectors_at_flexible_boundary>2", "Number of sectors to use at assembly boundary interface when flexible patterning is used (Defaults to 6)");
    ...
    _assembly_pitch(getParam<Real>("assembly_pitch"))
{
  ...
  if (flexible_assembly_stitching)
    this->declareMeshProperty(RGMB::num_sectors_flexible_stitching, getParam<unsigned int>("num_sectors_at_flexible_boundary"));
  if (parameters.isParamSetByUser("num_sectors_at_flexible_boundary") && !flexible_assembly_stitching)
    paramWarning("num_sectors_at_flexible_boundary", "This parameter is only relevant when ReactorMeshParams/flexible_assembly_stitching is set to true. This value will be ignored");
