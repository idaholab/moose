...
CoreMeshGenerator::validParams()
{
  ...
  params.addRequiredParam<MeshGeneratorName>("empty_position_name",
  ...
  params.addClassDescription("This CoreMeshGenerator object is designed to generate a core-like structure, with IDs, from a reactor geometry. "
                             "The core-like structure consists of a pattern of assembly-like structures generated with AssemblyMeshGenerator "
                             "and is permitted to have \"empty\" locations. The size and spacing of the assembly-like structures is defined, and "
...
{  
  /* Below, we get the ReactorMeshParams info from one of the pin inputs.
     ...
     We then create the PatternedMesh subgenerators and pass the parameter info needed for the inputs.
     Finally, if the the mesh is to be extruded we create the FancyExtruderGenerator subgenerator and the 
     PlaneIDMeshGenerator subgenerator to handle this.
  */
  MeshGeneratorName _reactor_params = MeshGeneratorName(getMeshProperty<std::string>("reactor_params_name", _inputs[0]));
  ...
  for(auto assembly:_inputs){
    if(assembly != _empty_key){
      ...
      if(getMeshProperty<bool>("extruded", assembly)){
        mooseError("Assemblies that have already been extruded cannot be used in CoreMeshGenerator definition.\n");
      }
    }
    else{
      make_empty = true;
  ...
  if(_geom_type == "Square"){
    ...
    if(make_empty){
      ...
      params.set<std::vector<unsigned int>>("subdomain_id") = std::vector<unsigned int>{UINT16_MAX-1};
    ...
    }        
    {
      ...
      params.set<MooseEnum>("assign_type") = "cell"; //give elems IDs relative to position in assembly
      params.set<std::vector<MeshGeneratorName>>("inputs") = _inputs;
      params.set<std::vector<std::vector<unsigned int>>>("pattern") = _pattern;
      if(make_empty){
        params.set<std::vector<MeshGeneratorName>>("exclude_id") = std::vector<MeshGeneratorName>{_empty_key};
      }
    ...
    if(make_empty){      
      {
        auto params = _app.getFactory().getValidParams("HexagonConcentricCircleAdaptiveBoundaryMeshGenerator");
        
        params.set<Real>("hexagon_size") = getMeshProperty<Real>("assembly_pitch", _reactor_params)/2.0;
        params.set<std::vector<unsigned int>>("num_sectors_per_side") = std::vector<unsigned int>(6, 2);
        params.set<std::vector<unsigned int>>("sides_to_adapt") = std::vector<unsigned int>{0};
        params.set<std::vector<MeshGeneratorName>>("inputs") = std::vector<MeshGeneratorName>{_inputs[0]};
        params.set<std::vector<subdomain_id_type>>("background_block_ids") = std::vector<subdomain_id_type>{UINT16_MAX-1};
        
        addMeshSubgenerator("HexagonConcentricCircleAdaptiveBoundaryMeshGenerator", std::string(_empty_key), params);
      }
    }
    {
      ...
      _build_mesh = &addMeshSubgenerator("HexIDPatternedMeshGenerator", name() + "_pattern", params);
    }
  ...
  for(auto assembly:_inputs){
    if(assembly!=_empty_key){
      std::map<subdomain_id_type, std::vector<std::vector<subdomain_id_type>>> pin_map =
          getMeshProperty<std::map<subdomain_id_type, std::vector<std::vector<subdomain_id_type>>>>("pin_id_map", assembly);
      for(auto pin = pin_map.begin(); pin !=pin_map.end(); ++pin){
        if(_id_map.find(pin->first) == _id_map.end()){        
          _id_map.insert(std::pair<subdomain_id_type, std::vector<std::vector<subdomain_id_type>>>(pin->first, pin->second));
        }
      ...
      if(_geom_type == "Hex"){
        subdomain_id_type assembly_type = getMeshProperty<subdomain_id_type>("assembly_type_id", assembly);
        if(_background_id_map.find(assembly_type) == _background_id_map.end()){
          std::vector<subdomain_id_type> background_regions = getMeshProperty<std::vector<subdomain_id_type>>("background_region_ids", assembly);
          std::vector<std::vector<subdomain_id_type>> duct_regions = getMeshProperty<std::vector<std::vector<subdomain_id_type>>>("duct_region_ids", assembly);
          _background_id_map.insert(std::pair<subdomain_id_type, std::vector<subdomain_id_type>>(assembly_type, background_regions));
          _duct_id_map.insert(std::pair<subdomain_id_type, std::vector<std::vector<subdomain_id_type>>>(assembly_type, duct_regions));
        }
      }  
    }
  }
  ...
  if(_extrude && getMeshProperty<int>("mesh_dimensions", _reactor_params) == 3){  
    std::vector<Real> axial_boundaries = getMeshProperty<std::vector<Real>>("axial_boundaries", _reactor_params);
	{
      ...
      params.set<MeshGeneratorName>("input") = empty_pos ? name() + "_deleted" : name() + "_pattern";
  ...
  else declareMeshProperty("extruded", false);
}
    ...
    for(auto & elem : (*_build_mesh)->active_element_ptr_range()){
      ...
      if(_id_map.find(pt_id) == _id_map.end()){
        ...
        for(size_t i = 0; i < (_id_map.at(pt_id))[0].size(); ++i){
          ...
          if(r_id == _id_map.at(pt_id)[0][i] && _id_map.at(pt_id)[0][i] != _id_map.at(pt_id)[z_id][i]){
