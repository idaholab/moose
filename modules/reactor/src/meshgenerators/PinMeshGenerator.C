...
PinMeshGenerator::validParams()
{
  ...
  params.addRequiredParam<MeshGeneratorName>("reactor_params",
      "The ReactorMeshParams MeshGenerator that is the basis for this component conformal mesh.");
  
  params.addRequiredParam<subdomain_id_type>("pin_type", "The integer ID for this pin type definition");
  
  params.addRequiredRangeCheckedParam<Real>("pitch", "pitch>0.0", "The pitch for the outermost boundary polygon");
  
  params.addRequiredRangeCheckedParam<unsigned int>("num_sectors", "num_sectors>0",
                                        "Number of azimuthal sectors in each quadrant");
  
  params.addRangeCheckedParam<std::vector<Real>>("ring_radii", "ring_radii>0.0", "Radii of major concentric circles");
  
  params.addRangeCheckedParam<std::vector<Real>>("duct_radii", "duct_radii>0.0", "Apothem of the ducts");
  
  params.addRangeCheckedParam<std::vector<unsigned int>>("mesh_intervals", std::vector<unsigned int>{1}, "mesh_intervals>0", 
      "The number of meshing intervals for each region starting at the center (length ring_radii + duct_radii + 1)");

  params.addParam<std::vector<std::vector<subdomain_id_type>>>("region_ids",
      "IDs for each radial and axial zone for assignment of unique physical characteristics"); 
  
  params.addParam<bool>("extrude", false, "Determines if this is the final step in the geometry construction"
  " and extrudes the 2D geometry to 3D. If this is true then this mesh cannot be used in further mesh building"); 

  params.addParam<bool>("quad_center_elements", true, "Whether the center elements are quad or triangular.");
  
  params.addClassDescription("This PinMeshGenerator object is designed to generate pin-like structures, with IDs, from a reactor geometry. "
                             "Whether it be a square or hexagonal pin, they are divided into three substructures - the innermost "
                             "radial pin regions, the single bridging background region, and the square or hexagonal ducts regions.");
...
PinMeshGenerator::PinMeshGenerator(
  ...
  _ring_radii(isParamValid("ring_radii") ? getParam<std::vector<Real>>("ring_radii")
                                         : std::vector<Real>()),
  _duct_radii(isParamValid("duct_radii") ? getParam<std::vector<Real>>("duct_radii")
                                         : std::vector<Real>()),
  _intervals(getParam<std::vector<unsigned int>>("mesh_intervals")),
  _region_ids(isParamValid("region_ids") ? getParam<std::vector<std::vector<subdomain_id_type>>>("region_ids")
...
{    
  ...
  if(!hasMeshProperty("mesh_dimensions", _reactor_params) || !hasMeshProperty("mesh_geometry", _reactor_params)){
  ...
  mooseError("This is a 2 dimensional mesh, you cannot extrude it. Check you ReactorMeshParams inputs\n");
  }
  if(_intervals.size() != (_ring_radii.size() + _duct_radii.size() + 1)){
    mooseError("The number of mesh intervals must be equal to the number of annular regions + the number of duct regions + 1"
      " for the region between the rings and ducts\n");
  }
  
  if(isParamValid("region_ids")){      
      if( _region_ids[0].size() != (_ring_radii.size() + _duct_radii.size() + 1)){
        mooseError("The number of region IDs given needs to be one more than the number of ring_radii + the number of duct_radii\n"); 
      }
  }
  else if(!getMeshProperty<bool>("procedural_ids", _reactor_params)){
    mooseError("If procedural ID generation is not being used then region IDs must be assigned with parameter region_ids"); 
  }
  else{
    ...
    if(_intervals[0] > 1) num_rings++; //background center -> generate id but don't break interval yet
    ...
    if(_pin_type*pow(10, digits_r) + num_rings > UINT16_MAX-1){
      if(_pin_type*pow(10, 5-digits_id) + num_rings > UINT16_MAX-1){
        ident = _pin_type*pow(10, 4-digits_id) - int(_pin_type*pow(10, 4-digits_id)) % int(digits_r);
      }
      else{
        ident = _pin_type*pow(10, 5-digits_id) - int(_pin_type*pow(10, 5-digits_id)) % int(digits_r);
      }
    }
    else{
    ...
    for(subdomain_id_type i = 0; i < num_rings; i++){
      if(!(_intervals[0] > 1 && i==0)){ //for center background skip the first to assign in params
        regions_0.push_back(ident + i+1);
      }          
    }
    
    _region_ids.push_back(regions_0);
    if(getMeshProperty<int>("mesh_dimensions", _reactor_params) == 3){
      for(size_t i = 1; i < getMeshProperty<std::vector<Real>>("axial_boundaries", _reactor_params).size(); i++){
        _region_ids.push_back(regions_0);
      }
    }
  }      
  ...
  if(getMeshProperty<std::string>("mesh_geometry", _reactor_params) == "Square"){
    {
      ...
      params.set<std::vector<unsigned int>>("num_sectors_per_side") = std::vector<unsigned int>(4, _num_sectors);
      params.set<bool>("preserve_volumes") = true;
      params.set<bool>("quad_center_elements") = _quad_center;
      
      //Break the mesh intervals and region ids up by construct for PolygonConcentricCircleMeshGenerator
      ...
      if(ring_intervals.size() > 0 ){
        ...
        if(ring_intervals.front() != 1) ring_ids.insert(ring_ids.begin(), ring_ids.front());       
        ...
        if(getMeshProperty<bool>("procedural_ids", _reactor_params)){
      ...
      if(duct_intervals.size() > 0){
        ...
        params.set<std::vector<Real>>("duct_sizes") = _duct_radii;      
      ...
      params.set<boundary_id_type>("interface_boundary_id_shift") = 30000; //need to shift interface boundaries to avoid clashing
  ...
  //Rotate pin to be square rather than diamond, like what is generated using ConcentricCircleMeshGenerator
    {
    ...
    if(hasMeshProperty("background_intervals_meta", name() + "_circle"))
      declareMeshProperty("background_intervals_meta",getMeshProperty<unsigned int>("background_intervals_meta", name() + "_circle"));
    if(hasMeshProperty("node_id_background_meta", name() + "_circle"))
      declareMeshProperty("node_id_background_meta",getMeshProperty<unsigned int>("node_id_background_meta", name() + "_circle"));
    if(hasMeshProperty("pattern_pitch_meta", name() + "_circle"))
      declareMeshProperty("pattern_pitch_meta",getMeshProperty<Real>("pattern_pitch_meta", name() + "_circle"));
    if(hasMeshProperty("azimuthal_angle_meta", name() + "_circle"))
      declareMeshProperty("azimuthal_angle_meta",getMeshProperty<std::vector<Real>>("azimuthal_angle_meta", name() + "_circle"));
    if(hasMeshProperty("num_sectors_per_side_meta", name() + "_circle"))
      declareMeshProperty("num_sectors_per_side_meta",getMeshProperty<std::vector<unsigned int>>("num_sectors_per_side_meta", name() + "_circle"));    
    if(hasMeshProperty("max_radius_meta", name() + "_circle"))
      declareMeshProperty("max_radius_meta",getMeshProperty<Real>("max_radius_meta", name() + "_circle"));
  
    {
      auto params = _app.getFactory().getValidParams("RenameBoundaryGenerator");

      params.set<MeshGeneratorName>("input") = name() + "trans";
      params.set<std::vector<BoundaryName>>("old_boundary") = {"15001","15002","15003","15004"}; //hard coded boundary IDs in patterned mesh generator
    ...
    //Hex geometry
    {
      ...
      params.set<std::vector<unsigned int>>("num_sectors_per_side") = std::vector<unsigned int>(6, _num_sectors);
      params.set<bool>("preserve_volumes") = true;
      params.set<bool>("quad_center_elements") = _quad_center;
      params.set<boundary_id_type>("external_boundary_id") = 20000 + _pin_type; //The boundary id for pins as set by the Reactor Geometry Meshing convention
      params.set<std::string>("external_boundary_name") = "outer_pin_" + std::to_string(_pin_type);
      ...
      if(duct_intervals.size() > 0){
       ...
       _build_mesh = &addMeshSubgenerator("PolygonConcentricCircleMeshGenerator", name() + "_2D", params);
    }
    ...
    if(hasMeshProperty("background_intervals_meta", name() + "_2D"))
      declareMeshProperty("background_intervals_meta",getMeshProperty<unsigned int>("background_intervals_meta", name() + "_2D"));
    if(hasMeshProperty("node_id_background_meta", name() + "_2D"))
      declareMeshProperty("node_id_background_meta",getMeshProperty<unsigned int>("node_id_background_meta", name() + "_2D"));
    if(hasMeshProperty("pattern_pitch_meta", name() + "_2D"))
      declareMeshProperty("pattern_pitch_meta",getMeshProperty<Real>("pattern_pitch_meta", name() + "_2D"));
    if(hasMeshProperty("azimuthal_angle_meta", name() + "_2D"))
      declareMeshProperty("azimuthal_angle_meta",getMeshProperty<std::vector<Real>>("azimuthal_angle_meta", name() + "_2D"));
    if(hasMeshProperty("num_sectors_per_side_meta", name() + "_2D"))
      declareMeshProperty("num_sectors_per_side_meta",getMeshProperty<std::vector<unsigned int>>("num_sectors_per_side_meta", name() + "_2D"));
    if(hasMeshProperty("max_radius_meta", name() + "_2D"))
      declareMeshProperty("max_radius_meta",getMeshProperty<Real>("max_radius_meta", name() + "_2D"));
  }    
  
  
  //id swap info after extrusion
  std::map<subdomain_id_type, std::vector<std::vector<subdomain_id_type>>> id_map { {_pin_type, _region_ids}};
  ...
  if(_extrude && getMeshProperty<int>("mesh_dimensions", _reactor_params) == 3){  
    std::vector<Real> axial_boundaries = getMeshProperty<std::vector<Real>>("axial_boundaries", _reactor_params);
    {
      ...
      params.set<std::vector<unsigned int>>("num_layers") =  getMeshProperty<std::vector<unsigned int>>("axial_mesh_intervals", _reactor_params);
    ...
    
    {
      ...
      params.set<std::vector<BoundaryName>>("old_boundary") = {"201", "202"}; //hard coded boundary IDs in patterned mesh generator
...
std::unique_ptr<MeshBase>  PinMeshGenerator::generate()
{
  ...
  for (auto & elem : (*_build_mesh)->active_element_ptr_range()){
    ...
    if(subdomain_name_map.find(elem->subdomain_id()) == subdomain_name_map.end()){
       subdomain_name_map.insert(std::pair<subdomain_id_type, std::string>(elem->subdomain_id(), std::to_string(elem->subdomain_id())));
    }        
  }
