...
AssemblyMeshGenerator::validParams()
{
  ...
  params.addRequiredParam<std::vector<std::vector<unsigned int>>>("pattern",
      "A double-indexed array starting with the upper-left corner"); 
                                            
  params.addRangeCheckedParam<std::vector<Real>>( "duct_radii", "duct_radii>0.0",
      "Distance(s) from center to duct(s) inner boundaries.");
             
  params.addRangeCheckedParam<unsigned int>("background_intervals", "background_intervals>0",
      "Radial intervals in the assembly peripheral region."); 

  params.addRangeCheckedParam<std::vector<unsigned int>>("duct_intervals", "duct_intervals>0",
      "Number of meshing intervals in each enclosing duct.");      
      
  params.addParam<std::vector<subdomain_id_type>>("background_region_id",
    "The region id for the background area between the pins and the ducts");  

  params.addParam<std::vector<std::vector<subdomain_id_type>>>("duct_region_ids",
    "The region id for the ducts from innermost to outermost.");
  ...
  params.addClassDescription("This AssemblyMeshGenerator object is designed to generate assembly-like structures, with IDs, from a reactor geometry. "
                             "The assembly-like structures must consist of a full pattern of equal sized pins from PinMeshGenerator"
                             "A hexagonal assembly will be placed inside of a bounding hexagon consisting of a background region and, optionally,"
                             " duct regions.");
...
AssemblyMeshGenerator::AssemblyMeshGenerator(
  ...
  _duct_sizes(isParamValid("duct_radii")
                              ? getParam<std::vector<Real>>("duct_radii")
                              : std::vector<Real>()),
  _background_intervals(isParamValid("background_intervals")
                                      ? getParam<unsigned int>("background_intervals")
                                      : 0),
  ...
  */
  ...
  for(auto pin:_inputs){
    if(getMeshProperty<bool>("extruded", pin)){
      mooseError("Pins that have already been extruded cannot be used in AssemblyMeshGenerator definition.\n");
    }
  } 
  if((_geom_type == "Square") &&
    (_duct_sizes.size() != 0 || _duct_intervals.size() != 0 ||
     _background_intervals != 0)){
  
    mooseError("Ducts and background regions are not currently supported for square assemblies");   
  }
  if((_geom_type == "Hex") &&
     ((_background_region_id.size() == 0 && !getMeshProperty<bool>("procedural_ids", _reactor_params)) || _background_intervals == 0)){
    
    mooseError("Hexagonal assemblies must have a background region defined");     
  }
  if(_duct_sizes.size() != 0 && _duct_intervals.size() == 0 ){
    mooseError("If ducts are defined then \"duct_intervals\" must also be defined.");
  }
  if(_duct_sizes.size() != _duct_intervals.size()){
    mooseError("If ducts are defined then \"duct_intervals\" and \"duct_region_ids\" must also be defined and of equal size.");
  }  
  if( _duct_sizes.size() != 0 ){
    if(!getMeshProperty<bool>("procedural_ids", _reactor_params)){
      if(_duct_region_ids.size() == 0){
        mooseError("If ducts are defined, and procedural ID generation is not used, then \"duct_intervals\" and \"duct_region_ids\" must also be defined and of equal size.");
      }    
  ...
  if(getMeshProperty<bool>("procedural_ids", _reactor_params) && (_duct_sizes.size() != 0 || _background_intervals != 0)){
    ...
    if(_assembly_type*pow(10, 5-digits_id) + num_regions > UINT16_MAX-1){
      ident = _assembly_type*pow(10, 4-digits_id) - int(_assembly_type*pow(10, 4-digits_id)) % int(digits_r);
    }
    else{
      ident = _assembly_type*pow(10, 5-digits_id) - int(_assembly_type*pow(10, 5-digits_id)) % int(digits_r);
    }
  ...
  if(_geom_type == "Square"){
    {
      ...
      params.set<BoundaryName>("right_boundary") = "10004";
       ...
       _build_mesh = &addMeshSubgenerator("CartesianIDPatternedMeshGenerator", name() + "_pattern", params);
    }    
    ...
    //Hex Geometry
    {
      ...
      params.set<subdomain_id_type>("background_block_id") = UINT16_MAX-1; //place holder subdomain id to indicate background region
      _peripheral_regions++;
      ...
      if(hasMeshProperty("background_intervals_meta", name() + "_pattern")){
        declareMeshProperty("background_intervals_meta",getMeshProperty<unsigned int>("background_intervals_meta", name() + "_pattern"));
      }
      if(hasMeshProperty("node_id_background_meta", name() + "_pattern")){
        declareMeshProperty("node_id_background_meta",getMeshProperty<unsigned int>("node_id_background_meta", name() + "_pattern"));
      }
      if(hasMeshProperty("pattern_pitch_meta", name() + "_pattern")){        
        declareMeshProperty("pattern_pitch_meta", getMeshProperty<Real>("assembly_pitch", _reactor_params));
        //declareMeshProperty("pattern_pitch_meta",getMeshProperty<Real>("pattern_pitch_meta", name() + "_pattern"));
      }
      if(hasMeshProperty("azimuthal_angle_meta", name() + "_pattern")){
        declareMeshProperty("azimuthal_angle_meta",getMeshProperty<std::vector<Real>>("azimuthal_angle_meta", name() + "_pattern"));
      }
      if(hasMeshProperty("num_sectors_per_side_meta", name() + "_pattern")){
        declareMeshProperty("num_sectors_per_side_meta",getMeshProperty<std::vector<unsigned int>>("num_sectors_per_side_meta", name() + "_pattern"));    
      }
      if(hasMeshProperty("max_radius_meta", name() + "_pattern")){
        declareMeshProperty("max_radius_meta",getMeshProperty<Real>("max_radius_meta", name() + "_pattern"));
      }
      if(hasMeshProperty("is_control_drum_meta", name() + "_pattern")){           
         declareMeshProperty("is_control_drum_meta", getMeshProperty<bool>("is_control_drum_meta", name() + "_pattern"));
      }
      if(hasMeshProperty("control_drum_positions", name() + "_pattern")){
         declareMeshProperty("control_drum_positions", getMeshProperty<std::vector<Point>>("control_drum_positions", name() + "_pattern"));
      }
      if(hasMeshProperty("control_drum_angles", name() + "_pattern")){
         declareMeshProperty("control_drum_angles", getMeshProperty<std::vector<Real>>("control_drum_angles", name() + "_pattern"));
      }
      if(hasMeshProperty("control_drums_azimuthal_meta", name() + "_pattern")){
        declareMeshProperty("control_drums_azimuthal_meta", getMeshProperty<std::vector<std::vector<Real>>>("control_drums_azimuthal_meta", name() + "_pattern"));  
      }
      if(hasMeshProperty("position_file_name", name() + "_pattern")){
         declareMeshProperty("position_file_name", getMeshProperty<std::string>("position_file_name", name() + "_pattern"));
      }
  ...
  for(auto pinMG:_inputs){
    std::map<subdomain_id_type, std::vector<std::vector<subdomain_id_type>>> pin_map =
        getMeshProperty<std::map<subdomain_id_type, std::vector<std::vector<subdomain_id_type>>>>("pin_region_ids", pinMG);
    _id_map.insert(std::pair<subdomain_id_type, std::vector<std::vector<subdomain_id_type>>>(pin_map.begin()->first, pin_map.begin()->second));
  }
...
std::unique_ptr<MeshBase>  AssemblyMeshGenerator::generate()
{
  ...
  if(_geom_type == "Square"){
    ...
    (*_build_mesh)->get_boundary_info().sideset_name(_assembly_boundary_id) = _assembly_boundary_name;
    (*_build_mesh)->get_boundary_info().nodeset_name(_assembly_boundary_id) = _assembly_boundary_name;
  }
