...
AssemblyMeshGenerator::validParams()
{
  ...
  params.addRangeCheckedParam<std::vector<Real>>(
      "duct_radii", "duct_radii>0.0", "Distance(s) from center to duct(s) inner boundaries.");
...
AssemblyMeshGenerator::AssemblyMeshGenerator(const InputParameters & parameters)
  : MeshGenerator(parameters),
    ...
    _duct_sizes(isParamValid("duct_radii") ? getParam<std::vector<Real>>("duct_radii")
                                           : std::vector<Real>()),
    ...
    _extrude(getParam<bool>("extrude"))
{
  ...
  */
  ...
  if ((_geom_type == "Hex") && ((_background_region_id.size() == 0 &&
                                 !getMeshProperty<bool>("procedural_ids", _reactor_params)) ||
  ...
  else
  {
    // Hex Geometry
    {
      ...
      if (hasMeshProperty("pattern_pitch_meta", name() + "_pattern"))
      {
        ...
        // declareMeshProperty("pattern_pitch_meta",getMeshProperty<Real>("pattern_pitch_meta",
        // name() + "_pattern"));
      }
