...
PatternedHexPeripheralModifier::validParams()
{
  ...
  params.addClassDescription("This PatternedHexPeripheralModifier object removes the outmost layer "
                             "of the input mesh and add a "
...
PatternedHexPeripheralModifier::generate()
{
  ...
  for (unsigned i_side = 0; i_side < 6; i_side++)
  {
    ...
    MeshTools::Modification::rotate(*input_mesh_origin, -60.0, 0, 0);
    ...
    const Point outer_end_1 =
        Point(_pattern_pitch_meta / 2.0, -_pattern_pitch_meta / 2.0 / std::sqrt(3.0), 0.0);
    const Point outer_end_2 =
        Point(_pattern_pitch_meta / 2.0, _pattern_pitch_meta / 2.0 / std::sqrt(3.0), 0.0);
    ...
    addExtraElemIntegers(*mesh, del_elem_extra_ids);
...
void
PatternedHexPeripheralModifier::addExtraElemIntegers(
    ReplicatedMesh & mesh,
    const std::vector<std::pair<Point, std::vector<dof_id_type>>> ref_extra_ids)
{
  ...
  for (const auto & elem : as_range(mesh.active_elements_begin(), mesh.active_elements_end()))
  {
    ...
    {
      tmp_dist.push_back((pt_extra_id.first - elem->true_centroid()).norm());
    }
