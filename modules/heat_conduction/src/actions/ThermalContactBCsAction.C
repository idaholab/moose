...
ThermalContactBCsAction::act()
{
  ...
  if (isParamValid("parser_syntax"))
    ...
    mooseError("The 'parser_syntax' parameter is not valid, which indicates that this actions was "
               "not created by the Parser, which is not currently supported.");
  ...
  _problem->addBoundaryCondition(
      getParam<std::string>("type"), "gap_bc_" + Moose::stringify(n), params);
  ...
  if (quadrature)
  {
    ...
    _problem->addBoundaryCondition(
        getParam<std::string>("type"), "gap_bc_master_" + Moose::stringify(n), params);
  }
