...
FunctorThermalResistanceBC::computeQpResidual()
{
  ...
  _T_surface = 0.5 * (_T(face_arg, autoState()) + _T_ambient);
  ...
  if (_emissivity > 1e-8)
    while (norm > (_tolerance * _alpha))
    {
      ...
      flux = (_T(face_arg, autoState()) - _T_surface) / _conduction_resistance;
  ...
  flux = (_T(face_arg, autoState()) - _T_ambient) /
...
FunctorThermalResistanceBC::computeParallelResistance()
{
  ...
  _parallel_resistance = 1.0 / (hr + _h(face_arg, autoState())) / _outer_radius;
}
