...
InputParameters validParams<BulkCoolantBC>()
{
  ...
  params.addParam<Real>("alpha",2E3, "heat transfer coefficient W/m^2-K"); //should be optionally a material property
...
BulkCoolantBC::BulkCoolantBC(const std::string & name, InputParameters parameters)
  :IntegratedBC(name, parameters),
   ...
   _conductivity(getMaterialProperty<Real>("thermal_conductivity")) //should give option for constant value
