...
InputParameters validParams<AnisoHeatConductionMaterial>()
{
  ...
  params.addParam<PostprocessorName>("thermal_conductivity_x_pp", "The thermal conductivity PP name in the x direction");
  params.addParam<PostprocessorName>("thermal_conductivity_y_pp", "The thermal conductivity PP name in the y direction");
  params.addParam<PostprocessorName>("thermal_conductivity_z_pp", "The thermal conductivity PP name in the z direction");
...
AnisoHeatConductionMaterial::computeProperties()
{
  for (unsigned int qp(0); qp < _qrule->n_points(); ++qp)
  {
    ...
    {
      (*_thermal_conductivity_y)[qp] = _thermal_conductivity_y_pp ? *_thermal_conductivity_y_pp : _my_thermal_conductivity_y;
      (*_thermal_conductivity_y_dT)[qp] = 0;
    }
    if (_thermal_conductivity_z)
    {
      (*_thermal_conductivity_z)[qp] = _thermal_conductivity_z_pp ? *_thermal_conductivity_z_pp : _my_thermal_conductivity_z;
