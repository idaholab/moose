#include "OneDEnergyFriction.h"

registerMooseObject("THMApp", OneDEnergyFriction);

template <>
InputParameters
validParams<OneDEnergyFriction>()
{
  ...
  params.addClassDescription("Computes energy dissipation caused by wall friction");
  return params;
}

OneDEnergyFriction::OneDEnergyFriction(const InputParameters & parameters)
...
Real
OneDEnergyFriction::computeQpResidual()
{
  return -_mult[_qp] * 0.5 * _f_D[_qp] * _alpha[_qp] * _rho[_qp] * _vel[_qp] * _vel[_qp] *
         std::abs(_vel[_qp]) * _A[_qp] / _D_h[_qp] * _test[_i][_qp];
}

Real
OneDEnergyFriction::computeQpJacobian()
{
  return computeQpOffDiagJacobian(_var.number());
}

Real
OneDEnergyFriction::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  else if (jvar == _arhoA_var_number)
  {
    ...
    return -(_dmult_darhoA[_qp] * _f_D[_qp] * _rho[_qp] * vel3 +
             _mult[_qp] * _df_D_darhoA[_qp] * _rho[_qp] * vel3 +
             _mult[_qp] * _f_D[_qp] * _drho_darhoA[_qp] * vel3 +
             _mult[_qp] * _f_D[_qp] * _rho[_qp] * dvel3_darhoA) *
  ...
  else if (jvar == _arhouA_var_number)
  {
    ...
    return -(_dmult_darhouA[_qp] * _f_D[_qp] * vel3 + _mult[_qp] * _df_D_darhouA[_qp] * vel3 +
             _mult[_qp] * _f_D[_qp] * dvel3_darhouA) *
           0.5 * _alpha[_qp] * _rho[_qp] * _A[_qp] / _D_h[_qp] * _phi[_j][_qp] * _test[_i][_qp];
  }
  else if (jvar == _arhoEA_var_number)
  {
    return -(_dmult_darhoEA[_qp] * _f_D[_qp] + _mult[_qp] * _df_D_darhoEA[_qp]) * 0.5 *
           _alpha[_qp] * _rho[_qp] * _vel[_qp] * _vel[_qp] * std::abs(_vel[_qp]) * _A[_qp] /
