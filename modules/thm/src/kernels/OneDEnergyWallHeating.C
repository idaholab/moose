...
{
  return _heat_transfer_coefficient[_qp] * _P_hf[_qp] * (_temperature[_qp] - _T_wall[_qp]) *
         _test[_i][_qp];
}
...
{
  return _heat_transfer_coefficient[_qp] * _P_hf[_qp] * _dT_drhoEA[_qp] * _phi[_j][_qp] *
         _test[_i][_qp];
}
...
OneDEnergyWallHeating::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _rhoA_var_number)
    return _heat_transfer_coefficient[_qp] * _P_hf[_qp] * _dT_drhoA[_qp] * _phi[_j][_qp] *
           _test[_i][_qp];

  else if (jvar == _rhouA_var_number)
    return _heat_transfer_coefficient[_qp] * _P_hf[_qp] * _dT_drhouA[_qp] * _phi[_j][_qp] *
           _test[_i][_qp];
