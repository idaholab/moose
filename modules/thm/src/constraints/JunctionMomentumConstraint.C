#include "JunctionMomentumConstraint.h"
...
registerMooseObject("THMApp", JunctionMomentumConstraint);

template <>
InputParameters
validParams<JunctionMomentumConstraint>()
{
...
JunctionMomentumConstraint::JunctionMomentumConstraint(const InputParameters & parameters)
...
JunctionMomentumConstraint::computeResidual(NumericVector<Number> & /*residual*/)
{
  ...
  for (unsigned int i = 0; i < dofs.size(); i++)
  {
    if (_rhouA_old[i] * _normals[i] > 0)
    {
      ...
      re(i) = (_rhouA[i] * _vel[i] + p * _A[i]) * _normals[i];
    }
    else
    {
      const Real H_junction = _H_junction_uo.getJunctionStagnationEnthalpy();
      ...
      re(i) = (rho * _vel[i] * _vel[i] + p) * _A[i] * _normals[i];
    }
  }
...
Real JunctionMomentumConstraint::computeQpResidual(Moose::ConstraintType /*type*/) { return 0; }

void
JunctionMomentumConstraint::computeJacobian(SparseMatrix<Number> & /*jacobian*/)
{
  ...
  for (unsigned int i = 0; i < dofs.size(); i++)
  {
    if (_rhouA_old[i] * _normals[i] > 0)
    {
      ...
      J_s_junction(i, 0) = dp_ds_junction * An;

      // derivatives w.r.t. rhoA, rhouA, rhoEA
      for (unsigned int j = 0; j < dofs.size(); j++)
      {
        Real drhouA_drhouA = 0;
        Real dvel_drhoA = 0;
        Real dvel_drhouA = 0;
        Real dh_drhoA = dH_junction_drhoA[j];
        Real dh_drhouA = dH_junction_drhouA[j];
        Real dh_drhoEA = dH_junction_drhoEA[j];
        if (j == i)
        {
          drhouA_drhouA = 1;
          dvel_drhoA = THM::dvel_darhoA(_rhoA[i], _rhouA[i]);
          dvel_drhouA = THM::dvel_darhouA(_rhoA[i]);
          dh_drhoA += -_vel[i] * dvel_drhoA;
          dh_drhouA += -_vel[i] * dvel_drhouA;
        }
        ...
        J_rhoA(i, j) = (_rhouA[i] * dvel_drhoA + dp_drhoA * _A[i]) * _normals[i];
        J_rhouA(i, j) =
            (drhouA_drhouA * _vel[i] + _rhouA[i] * dvel_drhouA + dp_drhouA * _A[i]) * _normals[i];
        J_rhoEA(i, j) = dp_drhoEA * An;
      }
    }
    else
    {
      ...
      // derivatives w.r.t. s_junction
      J_s_junction(i, 0) = (drho_ds_junction * _vel[i] * _vel[i] + dp_ds_junction) * An;
      ...
      for (unsigned int j = 0; j < dofs.size(); j++)
      {
        ...
        J_rhoA(i, j) =
            (drho_drhoAj * _vel[i] * _vel[i] + rho * 2 * _vel[i] * dvel_drhoAj + dp_drhoAj) * An;
        J_rhouA(i, j) =
            (drho_drhouAj * _vel[i] * _vel[i] + rho * 2 * _vel[i] * dvel_drhouAj + dp_drhouAj) * An;
        J_rhoEA(i, j) = (drho_drhoEAj * _vel[i] * _vel[i] + dp_drhoEAj) * An;
      }
    }
  }
...
Real JunctionMomentumConstraint::computeQpJacobian(Moose::ConstraintJacobianType /*type*/)
{
  return 0;
}
