...
RayTracingMeshOutput::fillFields()
{
      ...
      [&set_solution](const DofObject * const dof, const unsigned int start_var, const auto & data)
  {
    ...
    for (const auto i : index_range(data))
      set_solution(dof, start_var + i, data[i]);
  };
  ...
  for (const auto & trace_data : _study.getCachedTraces())
  {
    ...
    if (_output_data_nodal && _study.hasRayData() && !trace_data.stationary())
      fill_data(elem->node_ptr(0), _data_start_var, trace_data._point_data[0]._data);
    ...
    for (const auto i : make_range(start, trace_data._point_data.size()))
    {
      ...
      if (_output_data && _study.hasRayData())
        fill_data(elem, _data_start_var, point_data._data);
      if (_output_aux_data && _study.hasRayAuxData())
        fill_data(elem, _aux_data_start_var, point_data._aux_data);
      // Nodal data fields
      if (_output_data_nodal && _study.hasRayData() && !trace_data.stationary())
        fill_data(elem->node_ptr(1), _data_start_var, point_data._data);
