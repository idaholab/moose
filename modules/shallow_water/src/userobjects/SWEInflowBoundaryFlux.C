...
{
  mooseAssert(UL.size() >= 4, "Expected [h,hu,hv,(b),g]");
  const Real nx = n(0), ny = n(1);
  const bool has_b = (UL.size() >= 5);
  const unsigned int idx_g = has_b ? 4 : 3;
  const Real g_here = UL[idx_g];
                                    ...
                                    DenseMatrix<Real> & jac1) const
{
  ...
  if (hL > _h_eps)
  {
    ...
    jac1(1, 0) = huL * d_un_dh + g_here * hL * nx;
    jac1(1, 1) = un + huL * d_un_dhu;
    jac1(1, 2) = huL * d_un_dhv;

    jac1(2, 0) = hvL * d_un_dh + g_here * hL * ny;
