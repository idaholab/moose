...
InputParameters validParams<ThermalGraphiteProperties>()
{
  ...
  params.addRangeCheckedParam<Real>("density_room_temp", 1600.0,
    "density_room_temp > 0.0", "Density at room temperature");
  params.addParam<MooseEnum>("surface", getSurfaceEnum("oxidized"),
    "The state of the solid surface");
  params.addRangeCheckedParam<Real>("emissivity", 1.0,
...
ThermalGraphiteProperties::cp_from_T(Real T) const
{
  return 4184.0 * (0.54212 - 2.42667e-6 * T - 90.2725 / T -
    43449.3 * std::pow(T, - 2.0) + 1.59309e7 * std::pow(T, - 3.0) -
    1.43688e9 * std::pow(T, - 4.0));
}
...
{
  cp = cp_from_T(T);
  dcp_dT = 4184.0 * (- 2.42667e-6 + 90.2725 * std::pow(T, - 2.0) +
    86898.6 * std::pow(T, - 3.0) - 4.77927e7 * std::pow(T, - 4.0) +
    5.74752e9 * std::pow(T, - 5.0));
}
...
ThermalGraphiteProperties::rho_from_T(Real T, Real & rho, Real & drho_dT) const
{
  ...
  if (T > 373.15)
  {
    Real d_betaT = _beta0 + (2.1e-9 * 4.0 * std::pow(T, 3.0) -
      1.23726e-5 * 3.0 * std::pow(T, 2.0) + 3.05359e-2 * 2.0 * T - 9.73349) * 1e-7;
    Real d_beta = (2.1e-9 * 3.0 * std::pow(T, 2.0) - 1.23726e-5 * 2.0 * T +
      3.05359e-2) * 1e-7;
...
ThermalGraphiteProperties::beta_from_T(Real T) const
{
  ...
  if (T > 373.15)
    term = (2.1e-9 * std::pow(T, 3.0) - 1.23726e-5 * std::pow(T, 2.0) +
      3.05359e-2 * T - 9.73349) * 1e-7;
