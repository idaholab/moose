...
const std::string ThermalSiliconCarbideProperties::_name =
  std::string("thermal_silicon_carbide");
...
InputParameters validParams<ThermalSiliconCarbideProperties>()
{
  ...
  params.addParam<MooseEnum>("thermal_conductivity_model", ThermalSiliconCarbidePropertiesKModel,
    "Thermal conductivity model to be used");
  params.addRangeCheckedParam<Real>("density", 3216.0, "density > 0.0",
    "(Constant) density");
  return params;
}
...
ThermalSiliconCarbideProperties::ThermalSiliconCarbideProperties(const InputParameters & parameters)
  : ThermalSolidProperties(parameters),
    _k_model(getParam<MooseEnum>("thermal_conductivity_model").getEnum<ThermalSiliconCarbidePropertiesKModel>()),
...
Real
ThermalSiliconCarbideProperties::rho_from_T(Real /* T */) const
{
  return _rho;
}
