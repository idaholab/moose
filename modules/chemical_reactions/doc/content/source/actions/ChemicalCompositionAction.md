...
!alert note
Use [!param](/ChemicalComposition/prefix) to namespace all variables generated by this action instance. If provided and not ending in `_`, an underscore is appended automatically. For example, `prefix = salt` creates `salt_Mo`, `salt_BCCN`, `salt_BCCN:Mo`, `salt_mu:Mo`, `salt_vp:gas_ideal:Mo`, and `salt_ep:BCCN:Mo`.
...
When [!param](/ChemicalComposition/initial_values) is used with [!param](/ChemicalComposition/prefix), the CSV `name` entries must match the prefixed variable names exactly.
...
!alert note
Use [!param](/ChemicalComposition/output_name_separator) to choose the output-name separator (`colon`, `slash`, or `underscore`). The default is `colon` (which uses `:`) for backward compatibility.

!alert note
The format for entries in `output_species` must be `phase_name<sep>species_name` or `ALL`, and both must match the specified thermodynamic database.

!alert note
The format for entries in `output_element_potentials` must be `mu<sep>element_name`  or `ALL`.

!alert note
The format for entries in `output_vapor_pressures` must be `vp<sep>gas_phase_name<sep>species_name`  or `ALL`. The phase name must correspond to the gas phase in the thermodynamic database.

!alert note
The format for entries in `output_element_phases` must be `ep<sep>phase_name<sep>element_name`  or `ALL`.
