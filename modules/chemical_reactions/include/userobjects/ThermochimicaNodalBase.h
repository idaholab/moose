 ...
 */
class ThermochimicaNodalBase : public NodalUserObject
{
public:
  static InputParameters validParams();
  ThermochimicaNodalBase(const InputParameters & parameters);
  ...
  /// Function to set state variables and run Thermochimica
  int setParamsAndRun(const std::vector<Real> element_values);

  /// Function to set specific output variables after Thermochimica run
  void setOutputNodalValues();

  /// Function to set outputs for chemical potentials of elements after Themochimica run
  Real getElementalChemicalPotentials(const int idx);
