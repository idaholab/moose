...
# This test is for FLiNaK and two different noble gases: helium and argon
...
[Postprocessors]
  ...
  [helium_gold_750]
    ...
    value = ${fparse 4.1e-8/(1e-6*101325)}
    outputs = none
  []
  ...
  [helium_gold_900]
    type = ConstantPostprocessor
    value = ${fparse 9.1e-8/(1e-6*101325)}
    outputs = none
  []
  [check_helium_900]
    type = PostprocessorComparison
  ...
  [helium_gold_1100]
    type = ConstantPostprocessor
    value = ${fparse 1.9e-7/(1e-6*101325)}
    outputs = none
  []
  [check_helium_1100]
    type = PostprocessorComparison
  ...
  [argon_gold_750]
    type = ConstantPostprocessor
    value = ${fparse 2.5e-9/(1e-6*101325)}
    outputs = none
  []
  # Set tolerance to second sig fig
  [check_argon_750]
  ...
  [argon_gold_900]
    type = ConstantPostprocessor
    value = ${fparse 1.1e-8/(1e-6*101325)}
    outputs = none
  []
  [check_argon_900]
    type = PostprocessorComparison
  ...
  [argon_gold_1100]
    type = ConstantPostprocessor
    value = ${fparse 4.0e-8/(1e-6*101325)}
    outputs = none
  []
  [check_argon_1100]
    type = PostprocessorComparison
