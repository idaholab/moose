...
ReactionKineticsPhysicsBase::validParams()
{
  ...
  params.addClassDescription(
      "Base class to create a reaction network Physics from.");

  // Variables parameters
  params.addRequiredParam<std::vector<VariableName>>("solver_variables", "Species to solve for");
  params.addRequiredParam<std::vector<AuxVariableName>>("auxiliary_variables",
      "Additional species to output the concentration of, which do not require an additional equation to obtain");
  params.transferParam<MooseEnum>(MooseVariableBase::validParams(), "order", "variable_order");
  params.addParam<std::vector<Real>>("equation_scaling", "Scaling factor to apply to each equation");
  ...
  params.addParam<std::vector<MooseFunctorName>>(
      "additional_source_functors", "Additional volumetric source terms in each species' equations");
  params.addParam<std::vector<Real>>("additional_source_coefs", {1}, "Coefficients multiplying each source");
    ...
    _reactions(ReactionNetworkUtils::parseReactionNetwork(getParam<std::string>("reactions"), _verbose))
{
  ...
  if (isParamValid("additional_source_functors"))
    checkVectorParamsSameLength<VariableName, MooseFunctorName>("solver_variables", "additional_source_functors");
...
ReactionKineticsPhysicsBase::addInitialConditions()
{
  ...
  const auto & initial_functors = isParamValid("initial_conditions") ?
      getParam<std::vector<MooseFunctorName>>("initial_conditions") : empty_functor_vector;
