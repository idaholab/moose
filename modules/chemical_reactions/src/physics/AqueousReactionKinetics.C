...
AqueousReactionKinetics::validParams()
{
  ...
  params.addClassDescription("Forms the equations for the chemical reaction networks in an aqueous"
  ...
  params.renameParam("reactions", "reactions", "The list of aqueous equilibrium reactions");
...
AqueousReactionKinetics::addAuxiliaryVariables()
{
  ...
  for (const auto j : make_range(_num_reactions))
  {
    // Loop over each species involved in the reaction
