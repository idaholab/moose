...
HenryGasConstantScalarAux::validParams()
{
  ...
  params.addParam<Real>(
      "KH0", "Reference Henry parameter in Henry model, if defining custom fluid [mol/m^3/Pa]");
...
HenryGasConstantScalarAux::computeValue()
{
  ...
  Real value = _alpha * 4.0 * libMesh::pi * std::pow(_radius * m_to_ang, 2) *
                   (_gamma_0 + _dgamma_dT * (_temperature[0] - 273.15)) +
               _beta * 4.0 / 3.0 * libMesh::pi * std::pow(_radius * m_to_ang, 3) *
                   HenryGasConstantAux::_Rgas * _temperature[0];
