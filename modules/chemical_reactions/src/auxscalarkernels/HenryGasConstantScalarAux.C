#include "HenryGasConstantScalarAux.h"
...
InputParameters
HenryGasConstantScalarAux::validParams()
{
  ...
  params.addParam<MooseEnum>("salt", salt_list, "Salt");
  ...
  params.addParam<Real>("gamma_0", "gamma_0 fit parameter in Henry model, if defining custom fluidl");
  params.addParam<Real>("dgamma_dT", "gamma derivative fit parameter in Henry model, if defining custom fluid");
  params.addParam<Real>("KH0", "Reference Henry parameter in Henry model, if defining custom fluid [mol/m^3/Pa]");
  params.addClassDescription("Calculates Henry gas constant of a noble gas [mol/m^3/Pa]");
  return params;
}

HenryGasConstantScalarAux::HenryGasConstantScalarAux(const InputParameters & parameters)
    ...
    _KH0(isParamSetByUser("KH0") ? getParam<Real>("KH0") : 0.0)
{
  ...
  {
    case Saltlist::FLIBE:
      _alpha     = HenryGasConstantAux::_alpha_FLiBe;
      _beta      = HenryGasConstantAux::_beta_FLiBe;
      _KH0       = HenryGasConstantAux::_KH0_FLiBe;
      _gamma_0   = HenryGasConstantAux::_gamma_0_FLiBe;
      _dgamma_dT = HenryGasConstantAux::_dgamma_dT_FLiBe;
      break;

    case Saltlist::FLINAK:
      _alpha     = HenryGasConstantAux::_alpha_FLiNaK;
      _beta      = HenryGasConstantAux::_beta_FLiNaK;
      _KH0       = HenryGasConstantAux::_KH0_FLiNaK;
      _gamma_0   = HenryGasConstantAux::_gamma_0_FLiNaK;
      _dgamma_dT = HenryGasConstantAux::_dgamma_dT_FLiNaK;
      break;
...
Real
HenryGasConstantScalarAux::computeValue()
{
  const Real m_to_ang = 1e10;

  // Equations result in units of mol/cm^3/atm, so convert to mol/m^3/atm
  Real value = _alpha * 4.0*libMesh::pi*std::pow(_radius*m_to_ang, 2)*(_gamma_0+_dgamma_dT*(_temperature[0]-273.15)) +
          _beta * 4.0/3.0*libMesh::pi*std::pow(_radius*m_to_ang, 3)*HenryGasConstantAux::_Rgas*_temperature[0];
  value = exp(value/(HenryGasConstantAux::_Rgas * _temperature[0])) * _KH0;
  return value;
}
