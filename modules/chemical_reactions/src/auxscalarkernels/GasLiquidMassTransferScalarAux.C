...
GasLiquidMassTransferScalarAux::computeValue()
{
  ...
  {
    case Equationlist::STOKESEINSTEIN:
      Diffusivity = GasLiquidMassTransferAux::_kB * _temperature[0] /  (6.0 * libMesh::pi * mu * _radius);
      break;
    ...
    case Equationlist::WILKECHANG:
      ...
      Diffusivity = _wc * _temperature[0] * std::sqrt( _phi * molar_weight ) /  ( mu_cgs * std::pow(molar_volume, 0.6));
