                                               ...
                                               const std::string & origin)
{
  ...
  {
    if (it->second == origin)
      paramError(origin, "Duplicate generated variable name '", name, "'.");
    else
      paramError(origin,
    ...
    const std::vector<std::vector<std::string>> & database_species)
{
  ...
  auto phaseSystemIndex = [&](const std::string & phase, const std::string & parameter)
  {
    const auto it = std::find(database_phases.begin(), database_phases.end(), phase);
    if (it == database_phases.end())
      paramError(parameter, "Phase '", phase, "' was not found in the configured system.");
    return static_cast<int>(std::distance(database_phases.begin(), it));
  };

  auto speciesPhaseIndex = [&](const int phase_index, const std::string & species)
  {
  ...
  for (const auto & value : species)
  {
    ...
    const auto phase_index = phaseSystemIndex(split[0], "output_species");
    if (phase_index >= static_cast<int>(database_species.size()))
      paramError("output_species", "Phase '", split[0], "' does not contain solution species.");
    if (std::find(database_species[phase_index].begin(),
                  database_species[phase_index].end(),
                  split[1]) == database_species[phase_index].end())
      paramError(
  ...
  for (const auto & value : vapors)
  {
    ...
    const auto phase_index = phaseSystemIndex(split[0], "output_vapor_pressures");
    if (!Thermochimica::isPhaseGas(phase_index))
      paramError("output_vapor_pressures", "Phase '", split[0], "' is not a gas phase.");
    if (std::find(database_species[phase_index].begin(),
                  database_species[phase_index].end(),
                  split[1]) == database_species[phase_index].end())
  ...
  for (const auto & value : phase_elements)
  {
    ...
    const auto phase_index = phaseSystemIndex(split[0], "output_element_phases");
    if (std::find(config.elements.begin(), config.elements.end(), split[1]) ==
        config.elements.end())
      paramError("output_element_phases", "Element '", split[1], "' is not configured.");
    const auto [element_index, info] =
        Thermochimica::getElementIndex(Thermochimica::atomicNumber(split[1]));
    if (info != 0)
