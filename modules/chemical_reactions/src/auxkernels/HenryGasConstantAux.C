...
HenryGasConstantAux::validParams()
{
  ...
  params.addParam<Real>("KH0", "Reference Henry parameter in Henry model, if defining custom fluid [mol/m3/Pa]");
...
HenryGasConstantAux::computeValue()
{
  ...
  Real value = _alpha * 4*libMesh::pi*std::pow(_radius*m_to_ang, 2)*(_gamma_0+_dgamma_dT*(_temperature[_qp]-273.15)) +
