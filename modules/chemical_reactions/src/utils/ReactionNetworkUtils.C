...
std::vector<Reaction> parseReactionNetwork(const std::string & reaction_network_string, bool output_to_cout)
{
  ...
  parser["Float"] = [](const SemanticValues & vs) {
    return std::stod( std::string(vs.token()));
  };

  parser["Coefficient"] = [](const SemanticValues & vs) {
    return std::any_cast<double>(vs[0]);
  };

  parser["BaseSpecies"] = [](const SemanticValues & vs) {
    return vs.token();
  };
  ...
  parser["Charge"] = [](const SemanticValues & vs) {
    return vs.token();
  };
  ...
  if (output_to_cout)
  {
    for (size_t i = 0; i < reactions.size(); ++i) {
      ...
      for (const auto& t : r.reactants) {
        ...
        if (t.charge) std::cout << t.charge.value();
        if (t.state) std::cout << " (" << t.state.value() << ")";
        std::cout << "\n";
      }
      std::cout << "  Products:\n";
