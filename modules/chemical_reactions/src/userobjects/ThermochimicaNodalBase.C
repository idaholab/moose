...
InputParameters
ThermochimicaNodalBase::validParams()
{
...
ThermochimicaNodalBase::ThermochimicaNodalBase(const InputParameters & parameters)
...
void
ThermochimicaNodalBase::initialize()
{
}
...
ThermochimicaNodalBase::setParamsAndRun(const std::vector<Real> element_values)
{
  ...
  for (const auto i : make_range(_n_elements))
    Thermochimica::setElementMass(_el_ids[i], element_values[i]);
...
ThermochimicaNodalBase::setOutputNodalValues()
{
  ...
  if (_output_vapor_pressures)
    for (const auto i : make_range(_n_vapor_species))
    {
      ...
      if (idbg == 0)
        _vp[i]->setNodalValue(fraction * _pressure[_qp], _qp);
      else if (idbg == 1)
...
void
ThermochimicaNodalBase::reinitDataMooseFromTc()
{
#ifdef THERMOCHIMICA_ENABLED
...
void
ThermochimicaNodalBase::reinitDataMooseToTc()
{
#ifdef THERMOCHIMICA_ENABLED
...
const ThermochimicaNodalBase::Data &
ThermochimicaNodalBase::getNodalData(dof_id_type node_id) const
{
