...
# porepressure = -(Fluid bulk modulus)*t*log(1 - 0.01)  (if dt=1, see below)
...
# stress_zz = -0.04*t
#
# Regarding the "log", which is not standard in poroelasticity.  In this
# test we're solving (rho - rho_old)/dt + rho*strain_increment/dt = 0
# because porosity and saturation are constant.  In the limit as dt->0
# this becomes dot{P} + dot{strain} = 0   (here strain=volumetric_strain)
# which is the standard poroelasticity equation (in this example).  However,
# for finite dt, with rho=exp(P/bulk), we get
# P = P_old - bulk*log(1 + strain_increment) at each timestep.
