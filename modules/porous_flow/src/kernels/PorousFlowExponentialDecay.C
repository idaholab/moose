...
validParams<PorousFlowExponentialDecay>()
{
  ...
  params.addClassDescription("Residual = rate * (variable - reference).  Useful for modelling exponential decay of a variable");
  return params;
}

PorousFlowExponentialDecay::PorousFlowExponentialDecay(const InputParameters & parameters)
  : Kernel(parameters),
    _rate(coupledValue("rate")),
    _reference(coupledValue("reference"))
{
}
