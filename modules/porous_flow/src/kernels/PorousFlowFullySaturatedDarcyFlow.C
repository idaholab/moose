...
InputParameters validParams<PorousFlowFullySaturatedDarcyFlow>()
{
  ...
  params.addClassDescription("Darcy flux suitable for models involving a fully-saturated single phase, multi-component fluid.  No upwinding is used");
  return params;
}
...
PorousFlowFullySaturatedDarcyFlow::PorousFlowFullySaturatedDarcyFlow(const InputParameters & parameters) :
    ...
    _dmfrac_dvar(getMaterialProperty<std::vector<std::vector<std::vector<Real>>>>("dPorousFlow_mass_frac_qp_dvar")),
...
Real PorousFlowFullySaturatedDarcyFlow::dmobility(unsigned pvar) const
{
  ...
  return _dmfrac_dvar[_qp][ph][_fluid_component][pvar] * darcy_mob + _mfrac[_qp][ph][_fluid_component] * ddarcy_mob;
}
