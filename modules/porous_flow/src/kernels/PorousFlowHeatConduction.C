...
PorousFlowHeatConduction::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  return _grad_test[_i][_qp] * ((_dla_dvar[_qp][pvar] * _grad_t[_qp] + _la[_qp] * _dgrad_t_dvar[_qp][pvar]) * _phi[_j][_qp] + _la[_qp] * _dgrad_t_dgradvar[_qp][pvar] * _grad_phi[_j][_qp]);
}
