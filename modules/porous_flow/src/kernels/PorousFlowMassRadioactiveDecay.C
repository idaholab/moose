...
InputParameters validParams<PorousFlowMassRadioactiveDecay>()
{
  ...
  params.addRequiredParam<Real>("decay_rate", "The decay rate (units 1/time) for the fluid component");
    ...
    _dmass_frac_dvar(getMaterialProperty<std::vector<std::vector<std::vector<Real>>>>("dPorousFlow_mass_frac_nodal_dvar"))
{
  if (_fluid_component >= _dictator.numComponents())
    mooseError2("The Dictator proclaims that the number of components in this simulation is ", _dictator.numComponents(), " whereas you have used the Kernel PorousFlowComponetMassRadioactiveDecay with component = ", _fluid_component, ".  The Dictator does not take such mistakes lightly");
}
...
PorousFlowMassRadioactiveDecay::computeQpJac(unsigned int pvar)
{
  ...
  Real dmass = 0.0;
  for (unsigned ph = 0; ph < _num_phases; ++ph)
    dmass += _fluid_density[_i][ph] * _fluid_saturation_nodal[_i][ph] * _mass_frac[_i][ph][_fluid_component] * _dporosity_dgradvar[_i][pvar] * _grad_phi[_j][nearest_qp];
