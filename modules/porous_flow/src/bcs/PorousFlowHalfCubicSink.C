...
PorousFlowHalfCubicSink::multiplier()
{
  ...
  if (x >= 0)
    return PorousFlowSinkPTDefiner::multiplier() * _maximum;

  const Real cutoff = _cutoff.value(_t, _q_point[_qp]);
  if (x <= cutoff)
    return 0.0;

  return PorousFlowSinkPTDefiner::multiplier() * _maximum * (2 * x + cutoff) * (x - cutoff) * (x - cutoff) / Utility::pow<3>(cutoff);
}
...
PorousFlowHalfCubicSink::dmultiplier_dvar(unsigned int pvar)
{
  ...
  if (x >= 0)
    return PorousFlowSinkPTDefiner::dmultiplier_dvar(pvar) * _maximum;
  ...
  return PorousFlowSinkPTDefiner::dmultiplier_dvar(pvar) * str + PorousFlowSinkPTDefiner::multiplier() * deriv * dptVar(pvar);
}
