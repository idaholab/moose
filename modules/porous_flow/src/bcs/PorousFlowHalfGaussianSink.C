...
InputParameters validParams<PorousFlowHalfGaussianSink>()
{
  ...
  params.addRequiredParam<Real>("max", "Maximum of the Gaussian flux multiplier.  Flux out is multiplied by max*exp((-0.5*(p - center)/sd)^2) for p<center, and by = max for p>center.  Here p is the nodal porepressure for the fluid_phase specified (or, for heat fluxes, it is the temperature).");
  params.addRequiredParam<Real>("sd", "Standard deviation of the Gaussian flux multiplier (measured in Pa (or K for heat fluxes)).");
  params.addRequiredParam<Real>("center", "Center of the Gaussian flux multiplier (measured in Pa (or K for heat fluxes)).");
  return params;
}
...
PorousFlowHalfGaussianSink::multiplier()
{
  ...
  return PorousFlowSink::multiplier() * _maximum * std::exp(-0.5 * std::pow((ptVar() - _center) / _sd, 2));
}
