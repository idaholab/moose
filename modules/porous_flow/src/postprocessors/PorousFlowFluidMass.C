  ...
  _mass_frac(getMaterialProperty<std::vector<std::vector<Real> > >("PorousFlow_mass_frac"))
{
  if (_component_index >= _porflow_name_UO.num_components())
    mooseError("The Dictator proclaims that the number of components in this simulation is " << _porflow_name_UO.num_components() << " whereas you have used the Postprocessor PorousFlowFluidMass with component = " << _component_index << ".  The Dictator does not take such mistakes lightly");
}
...
{
  Real mass = 0.;
  for (unsigned ph = 0; ph < _porflow_name_UO.num_phases(); ++ph)
