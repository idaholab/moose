...
InputParameters validParams<PorousFlow2PhasePP_VG>()
{
  ...
  params.addClassDescription("This Material calculates the 2 porepressures and the 2 saturations in a 2-phase isothermal situation, and derivatives of these with respect to the PorousFlowVariables. Calculates the 1 porepressure and the 1 saturation in a 1-phase isothermal situation, and derivatives of these with respect to the PorousFlowVariables.  A van Genuchten effective saturation (1 + (-al * p)^(1 / (1 - m)))^(-m) is assumed, where p = phase0_porepressure - phase1_porepressure <= 0");
  return params;
}
