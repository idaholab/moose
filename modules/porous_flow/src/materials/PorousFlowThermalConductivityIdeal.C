...
InputParameters validParams<PorousFlowThermalConductivityIdeal>()
{
  ...
  params.addRequiredParam<RealTensorValue>("dry_thermal_conductivity", "The thermal conductivity of the rock matrix when the aqueous saturation is zero");
...
PorousFlowThermalConductivityIdeal::PorousFlowThermalConductivityIdeal(const InputParameters & parameters) :
    ...
    _saturation_qp(_aqueous_phase ? &getMaterialProperty<std::vector<Real> >("PorousFlow_saturation_qp") : NULL),
    _dsaturation_qp_dvar(_aqueous_phase ? &getMaterialProperty<std::vector<std::vector<Real> > >("dPorousFlow_saturation_qp_dvar") : NULL),
...
PorousFlowThermalConductivityIdeal::computeQpProperties()
{
  ...
  if (_aqueous_phase && _wet_and_dry_differ)
    _la_qp[_qp] += std::pow((*_saturation_qp)[_qp][_aqueous_phase_number], _exponent) * (_la_wet - _la_dry);

  _dla_qp_dvar[_qp].assign(_num_var, RealTensorValue());
  if (_aqueous_phase && _wet_and_dry_differ)
    for (unsigned v = 0; v < _num_var; ++v)
      _dla_qp_dvar[_qp][v] = _exponent * std::pow((*_saturation_qp)[_qp][_aqueous_phase_number], _exponent - 1.0) * (*_dsaturation_qp_dvar)[_qp][_aqueous_phase_number][v] * (_la_wet - _la_dry);
}
