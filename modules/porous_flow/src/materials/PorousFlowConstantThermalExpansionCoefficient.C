...
InputParameters validParams<PorousFlowConstantThermalExpansionCoefficient>()
{
  ...
  params.addRangeCheckedParam<Real>("fluid_coefficient", 2.1E-4, "fluid_coefficient>=0", "Volumetric coefficient of thermal expansion for the fluid");
  params.addRequiredRangeCheckedParam<Real>("drained_coefficient", "drained_coefficient>=0.0", "Volumetric coefficient of thermal expansion of the drained porous skeleton (ie the porous rock without flulid, or with a fluid that is free to move in and out of the rock)");
  params.addClassDescription("Computes the effective thermal expansion coefficient, (biot_coeff - porosity) * drained_coefficient + porosity * fluid_coefficient.");
  return params;
}
...
{
  _coeff[_qp] = (_biot_coefficient - _porosity[_qp]) * _drained_coefficient + _porosity[_qp] * _fluid_coefficient;
}
