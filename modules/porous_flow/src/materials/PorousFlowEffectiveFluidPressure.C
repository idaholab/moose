...
PorousFlowEffectiveFluidPressure::PorousFlowEffectiveFluidPressure(const InputParameters & parameters) :
    ...
    _pf_old(_nodal_material ? declarePropertyOld<Real>("PorousFlow_effective_fluid_pressure_nodal") : declarePropertyOld<Real>("PorousFlow_effective_fluid_pressure_qp")),
...
PorousFlowEffectiveFluidPressure::computeQpProperties()
{
  ...
  for (unsigned ph = 0; ph < _num_phases; ++ph)
  {
    ...
    for (unsigned v = 0; v < _num_var; ++v)
      _dpf_dvar[_qp][v] += _dsaturation_dvar[_qp][ph][v] * _porepressure[_qp][ph] + _saturation[_qp][ph] * _dporepressure_dvar[_qp][ph][v];
  }
}
