...
PorousFlowSingleComponentFluid::PorousFlowSingleComponentFluid(const InputParameters & parameters) :
    ...
    _viscosity_nodal(_nodal_material ? &declareProperty<Real>("PorousFlow_viscosity" + _phase) : nullptr),
    _dviscosity_nodal_dp(_nodal_material ? &declarePropertyDerivative<Real>("PorousFlow_viscosity" + _phase, _pressure_variable_name) : nullptr),
    _dviscosity_nodal_dT(_nodal_material ? &declarePropertyDerivative<Real>("PorousFlow_viscosity" + _phase, _temperature_variable_name) : nullptr),
...
PorousFlowSingleComponentFluid::computeQpProperties()
{
  ...
  if (_nodal_material)
  {
    ...
    (*_dviscosity_nodal_dp)[_qp] = dmu_drho * drho_dp;
