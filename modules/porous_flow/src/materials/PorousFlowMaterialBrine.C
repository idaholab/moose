...
{
  InputParameters params = validParams<PorousFlowMaterialWater>();
...
PorousFlowMaterialBrine::PorousFlowMaterialBrine(const InputParameters & parameters) :
    PorousFlowMaterialWater(parameters),
...
void
PorousFlowMaterialBrine::initQpStatefulProperties()
{
  _density_nodal[_qp] = density(_porepressure_nodal[_qp][_phase_num], _temperature_nodal[_qp][_phase_num], _xnacl);
}

void
PorousFlowMaterialBrine::computeQpProperties()
{
  /// Density and derivatives wrt pressure and temperature at the nodes
  _density_nodal[_qp] = density(_porepressure_nodal[_qp][_phase_num], _temperature_nodal[_qp][_phase_num], _xnacl);
  _ddensity_nodal_dp[_qp] = dDensity_dP(_temperature_nodal[_qp][_phase_num], _temperature_nodal[_qp][_phase_num], _xnacl);
...
Real
PorousFlowMaterialBrine::density(Real pressure, Real temperature, Real xnacl) const
{
  ...
  if (Tv >= 0. && Tv <= 350.)
  {
    if (pressure > psat && pressure <= 100.e6)
    {
      ...
      water_density = PorousFlowMaterialWater::densityRegion1(pressure, Tv);
    }

    else if (pressure <= psat)
    {
      /// Region 2: vapour phase
      water_density = PorousFlowMaterialWater::densityRegion2(pressure, Tv);
    }
...
Real
PorousFlowMaterialBrine::dDensity_dP(Real pressure, Real temperature, Real /*xnacl*/) const
{
  //TODO: implement brine density derivative wrt P
  return PorousFlowMaterialWater::dDensity_dP(pressure, temperature);
}

Real
PorousFlowMaterialBrine::dDensity_dT(Real /*pressure*/, Real /*temperature*/, Real /*xnacl*/) const
{
  //TODO: implement brine density derivative wrt T
  return 0.;}

Real
PorousFlowMaterialBrine::viscosity(Real temperature, Real water_density, Real xnacl) const
{
  ...
  return a * PorousFlowMaterialWater::viscosity(temperature, water_density);
}

Real
PorousFlowMaterialBrine::dViscosity_dT(Real /*temperature*/, Real /*density*/, Real /*xnacl*/) const
{
  return 0.; // TODO: not implemented yet
}

Real
PorousFlowMaterialBrine::pSat(Real temperature, Real xnacl) const
{
  // Temperature in K
  ...
  return PorousFlowMaterialWater::pSat(th20);
}

Real
PorousFlowMaterialBrine::dViscosity_dDensity(Real temperature, Real water_density, Real xnacl) const
{
  // Correlation requires molar concentration (mol/kg)
  Real mol = xnacl / ((1.0 - xnacl) * _Mnacl);
  Real mol2 = mol * mol;
  ...
  return a * PorousFlowMaterialWater::dViscosity_dDensity(temperature, water_density);
}

Real
PorousFlowMaterialBrine::haliteDensity(Real pressure, Real temperature) const
{
...
Real
PorousFlowMaterialBrine::haliteSolubility(Real temperature) const
{
