...
InputParameters validParams<PorousFlowRelativePermeabilityConst>()
{
  ...
  params.addClassDescription("This class sets the relative permeability to a constant value (default = 1)");
  return params;
}

PorousFlowRelativePermeabilityConst::PorousFlowRelativePermeabilityConst(const InputParameters & parameters) :
    PorousFlowRelativePermeabilityBase(parameters),
    _relperm(getParam<Real>("kr"))
{
}

Real
PorousFlowRelativePermeabilityConst::relativePermeability(Real /*seff*/) const
{
  return _relperm;
}
