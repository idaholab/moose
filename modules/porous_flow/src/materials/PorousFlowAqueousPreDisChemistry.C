...
validParams<PorousFlowAqueousPreDisChemistry>()
{
  ...
  params.addCoupledVar(
      ...
      "Initial concentrations for the mineral species (dimensionless).  Default = 0");
  ...
  params.addRequiredParam<std::vector<Real>>(
      "molar_volume",
      "Volume occupied by one mole of the secondary species (L/mol) (one for each reaction)");
  ...
  : PorousFlowMaterialVectorBase(parameters),
    ...
    _dsec_conc_dvar(_nodal_material ? declareProperty<std::vector<std::vector<Real>>>(
                                    ...
                                    : declareProperty<std::vector<std::vector<Real>>>(
                                          "dPorousFlow_mineral_concentration_qp_dvar")),
    ...
    _mineral_sat(
        _nodal_material
            ? declareProperty<std::vector<Real>>("PorousFlow_mineral_equilibrium_saturation_nodal")
            : declareProperty<std::vector<Real>>("PorousFlow_mineral_equilibrium_saturation_qp")),
                                               ...
                                               : std::vector<Real>(_num_reactions, 1.0))
{
  ...
  if (_num_initial_conc != _num_reactions)
    mooseError("PorousFlowAqueousPreDisChemistry: The number of initial concentrations is ",
               _num_initial_conc,
               " which must be equal to the number of reactions (",
               _num_reactions,
               ")");
...
{
  _mineral_sat[_qp].resize(_num_reactions);
...
void
PorousFlowAqueousPreDisChemistry::initQpSecondaryConcentrations()
{
...
void
PorousFlowAqueousPreDisChemistry::computeQpSecondaryConcentrations()
{
  for (unsigned r = 0; r < _num_reactions; ++r)
  {
    _mineral_sat[_qp][r] = 1.0 / _equilibrium_constants[r];
    for (unsigned j = 0; j < _num_primary; ++j)
    {
      ...
      if (gamp <= 0.0)
      {
        if (stoichiometry(r, j) < 0.0)
          _mineral_sat[_qp][r] = std::numeric_limits<Real>::max();
        else if (stoichiometry(r, j) == 0.0)
          _mineral_sat[_qp][r] *= 1.0;
        else
        {
          _mineral_sat[_qp][r] = 0.0;
          break;
        }
      }
      else
        _mineral_sat[_qp][r] *= std::pow(gamp, stoichiometry(r, j));
    }
    const Real fac = 1.0 - std::pow(_mineral_sat[_qp][r], _theta_exponent[r]);
    // if fac > 0 then dissolution occurs; if fac < 0 then precipitation occurs.
    const Real sgn = (fac < 0 ? -1.0 : 1.0);
    const Real kinetic_rate = -sgn * rateConstantQp(r) * _r_area[r] * _molar_volume[r] *
                              std::pow(std::abs(fac), _eta_exponent[r]);
    _sec_conc[_qp][r] = std::min(std::max(_sec_conc_old[_qp][r] + kinetic_rate * _dt, 0.0), 1.0);
  }
}

void
PorousFlowAqueousPreDisChemistry::dQpSecondaryConcentration_dprimary(unsigned reaction_num,
                                                                     std::vector<Real> & dsc) const
{
  dsc.assign(_num_primary, 0.0);

  // handle corner case
  if (_sec_conc[_qp][reaction_num] <= 0.0 || _sec_conc[_qp][reaction_num] >= 1.0)
    return;

  /*
   * Form the derivative of _mineral_sat, and store it in dsc for now.
  ...
  {
    for (unsigned i = 0; i < _num_primary; ++i)
      dsc[i] = stoichiometry(reaction_num, i) * _mineral_sat[_qp][reaction_num] /
  ...
  else
  {
    ...
    if (zero_count == 1 && stoichiometry(reaction_num, zero_conc_index) == 1.0)
    {
      ...
      // all other cases have dsc = 0, so return without performing any other calculations
      return;
  }

  // At the moment _dsc = d(mineral_sat)/d(primary)
  // Multiply by appropriate term to give _dsc = d(sec_conc)/d(primary)
  const Real fac = 1.0 - std::pow(_mineral_sat[_qp][reaction_num], _theta_exponent[reaction_num]);
  const Real dfac = -_theta_exponent[reaction_num] *
                    std::pow(_mineral_sat[_qp][reaction_num], _theta_exponent[reaction_num] - 1.0);
  const Real multiplier = -_dt * rateConstantQp(reaction_num) * _r_area[reaction_num] *
...
Real
PorousFlowAqueousPreDisChemistry::dQpSecondaryConcentration_dT(unsigned reaction_num) const
{
  // handle corner case
  if (_sec_conc[_qp][reaction_num] <= 0.0 || _sec_conc[_qp][reaction_num] >= 1.0)
    return 0.0;
