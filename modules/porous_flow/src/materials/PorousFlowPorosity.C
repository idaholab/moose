...
validParams<PorousFlowPorosity>()
{
  ...
  params.addCoupledVar("reference_chemistry",
                       "Reference values of the solid mineral concentrations (m^3/m^3), entered as "
...
PorousFlowPorosity::PorousFlowPorosity(const InputParameters & parameters)
  : PorousFlowPorosityExponentialBase(parameters),
    ...
    _mineral_conc(_chemical ? (_nodal_material ? &getMaterialProperty<std::vector<Real>>(
                                                     "PorousFlow_mineral_concentration_nodal")
                                               : &getMaterialProperty<std::vector<Real>>(
                                                     "PorousFlow_mineral_concentration_qp"))
                            : nullptr),
    _dmineral_conc_dvar(_chemical ? (_nodal_material
...
PorousFlowPorosity::atNegInfinityQp() const
{
  ...
  if (_chemical)
    for (unsigned i = 0; i < _num_c_ref; ++i)
      result -= _c_weights[i] * (*_mineral_conc)[_qp][i];
  return result;
}
...
PorousFlowPorosity::datNegInfinityQp(unsigned pvar) const
{
  ...
  if (_chemical)
    for (unsigned i = 0; i < _num_c_ref; ++i)
      result -= _c_weights[i] * (*_dmineral_conc_dvar)[_qp][i][pvar];
  return result;
}
...
PorousFlowPorosity::atZeroQp() const
{
  ...
  if (_chemical)
    for (unsigned i = 0; i < _num_c_ref; ++i)
      result -= _c_weights[i] * ((*_mineral_conc)[_qp][i] - (*_c_reference[i])[_qp]);
  return result;
}
