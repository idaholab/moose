    ...
    _num_components(getParam<unsigned int>("number_fluid_components"))
{
  ...
  _pf_var_num.assign(_fe_problem.getNonlinearSystemBase().nVariables(), _num_variables); // Note: the _num_variables assignment indicates that "this is not a PorousFlow variable"
  for (unsigned int i = 0; i < _num_variables; ++i)
