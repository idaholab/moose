...
InputParameters validParams<PorousFlowPeacemanBorehole>()
{
  ...
  params.addRequiredParam<Real>("bottom_p_or_t", "For function_of=pressure, this parameter is the pressure at the bottom of the borehole, otherwise it is the temperature at the bottom of the borehole");
...
PorousFlowPeacemanBorehole::PorousFlowPeacemanBorehole(const InputParameters & parameters) :
    ...
    _has_permeability(hasMaterialProperty<RealTensorValue>("PorousFlow_permeability_qp") && hasMaterialProperty<std::vector<RealTensorValue>>("dPorousFlow_permeability_qp_dvar")),
    ...
    _perm_or_cond(_p_or_t == pressure ? getMaterialProperty<RealTensorValue>("PorousFlow_permeability_qp") : getMaterialProperty<RealTensorValue>("PorousFlow_thermal_conductivity_qp")),
    _dperm_or_cond_dvar(_p_or_t == pressure ? getMaterialProperty<std::vector<RealTensorValue>>("dPorousFlow_permeability_qp_dvar") : getMaterialProperty<std::vector<RealTensorValue>>("dPorousFlow_thermal_conductivity_qp_dvar"))
{
...
Real
PorousFlowPeacemanBorehole::wellConstant(const RealTensorValue & perm, const RealTensorValue & rot, const Real & half_len, const Elem * ele, const Real & rad) const
// Peaceman's form for the borehole well constant
{
  ...
  if (eig_val1 <= 0.0)
    ...
    r0 = _re_constant * std::sqrt(std::sqrt(eig_val1 / eig_val2) * std::pow(ll2, 2) + std::sqrt(eig_val2 / eig_val1) * std::pow(ll1, 2)) / ( std::pow(eig_val1 / eig_val2, 0.25) + std::pow(eig_val2 / eig_val1, 0.25) );
...
PorousFlowPeacemanBorehole::computeQpBaseOutflow(unsigned current_dirac_ptid) const
{
  ...
  // contribution from half-segment "behind" this point (must have >1 point for current_dirac_ptid>0)
  {
    if ((character < 0.0 && pp < bh_pressure) || (character > 0.0 && pp > bh_pressure))
    {
      ...
      const Real wc = wellConstant(_perm_or_cond[_qp], _rot_matrix[current_dirac_ptid - 1], _half_seg_len[current_dirac_ptid - 1], _current_elem, _rs[current_dirac_ptid]);
  ...
  // contribution from half-segment "ahead of" this point, or we only have one point
  {
    ...
    {
      // injection, so outflow<0 || // production, so outflow>0
      const Real wc = wellConstant(_perm_or_cond[_qp], _rot_matrix[current_dirac_ptid], _half_seg_len[current_dirac_ptid], _current_elem, _rs[current_dirac_ptid]);
      outflow += wc * (pp - bh_pressure);
    }
  }
...
void
PorousFlowPeacemanBorehole::computeQpBaseOutflowJacobian(unsigned jvar, unsigned current_dirac_ptid, Real & outflow, Real & outflowp) const
{
  outflow = 0.0;
  outflowp = 0.0;
