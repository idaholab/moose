...
//
#include "PorousFlowCapillaryRSC.h"

Real
PorousFlowCapillaryRSC::seff(Real pc, Real shift, Real scale)
{
...
Real
PorousFlowCapillaryRSC::dseff(Real pc, Real shift, Real scale)
{
  Real x = (pc - shift)/scale;
  Real ex = std::exp(x);
...
Real
PorousFlowCapillaryRSC::d2seff(Real pc, Real shift, Real scale)
{
  Real x = (pc - shift)/scale;
  Real ex = std::exp(x);
