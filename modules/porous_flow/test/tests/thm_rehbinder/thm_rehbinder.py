...
def rehbinder(r):
    ...
    Tfirst_pr = (
        2 * sigmahat0 * That0 * xi * (np.log(xi) - np.log(xi1)) / np.log(xi1) ** 2
    )
    ...
    uzeroth_pr = (
        (That0 - sigmahat0 * (1 - 2 * poisson) / (1 - poisson))
        * (
            0.5 * (xi**2 - 1)
            - 0.25 * (1 - xi**2 + 2 * xi**2 * np.log(xi)) / np.log(xi1)
        )
        / xi
    )
    uzeroth_pr_xi1 = (
        ...
        * (
            0.5 * (xi1**2 - 1)
            - 0.25 * (1 - xi1**2 + 2 * xi1**2 * np.log(xi1)) / np.log(xi1)
        )
        / xi1
    )
    # fixed outer boundary
    Bzeroth = -((1 - 2 * poisson) * sigmahat0 + uzeroth_pr_xi1 / xi1) / (
        1 - 2 * poisson + 1.0 / xi1
    )
    ...
    ufirst_pr = (1.0 / xi) * (
        0.5 * (xi**2 - 1) * (2 * Cone - Done)
        + 0.5 * Done * xi**2 * np.log(xi)
        + 2
        * sigmahat0
        * That0
        / np.log(xi1) ** 2
        * (xi**3 * np.log(xi) / 3 + (1 - xi**3) / 9 + 0.5 * np.log(xi1) * (1 - xi**2))
    )
    ufirst_pr_xi1 = (1.0 / xi1) * (
        0.5 * (xi1**2 - 1) * (2 * Cone - Done)
        + 0.5 * Done * xi1**2 * np.log(xi1)
        + 2
        * sigmahat0
        * That0
        / np.log(xi1) ** 2
    ...
    fixed_u = (
        thermal_expansion * Tref * r0 * fixed_uhat * (1 + poisson) / (1 - poisson)
    )  # Corrected Eqn(16)
    free_uhat = free_uzeroth + Peclet * free_ufirst
    free_u = (
        thermal_expansion * Tref * r0 * free_uhat * (1 + poisson) / (1 - poisson)
    )  # Corrected Eqn(16)
...
plt.figure()
plt.plot(
    rpoints, [1e-6 * p for p in expected[1]], "k-", linewidth=3.0, label="expected"
)
...
plt.plot(
    rpoints,
    [1000 * u for u in expected[2]],
    "k-",
    linewidth=3.0,
    label="expected (fixed)",
)
plt.plot(
    fixedu[0],
    [1000 * u for u in fixedu[1]],
    "rs",
    markersize=10.0,
    label="MOOSE (fixed)",
)
plt.plot(
    rpoints,
    [1000 * u for u in expected[3]],
    "b-",
    linewidth=2.0,
    label="expected (free)",
)
plt.plot(
    freeu[0], [1000 * u for u in freeu[1]], "g*", markersize=13.0, label="MOOSE (free)"
)
