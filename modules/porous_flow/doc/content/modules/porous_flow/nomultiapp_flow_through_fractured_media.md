...
!media media/porous_flow/fracture_flow_diffusion_mesh.png style=width:100%;margin-left:10px caption=The 2D meshes used in the diffusion simulation.  Each mesh comprises a thin central 2D fracture (light blue with red element edges, barely visible in the left-hand panels) embedded within a large, coarsely meshed matrix (gray with dark blue element edges).  (A): fracture width meshed with 1 element; matrix with 2 elements.  (B): fracture width meshed with 5 elements; matrix with 2 elements.  (C): fracture width meshed with 1 element; matrix with 8 elements.  id=retard_mesh_fig
