...
def expected(x, t):
    ...
    biot_modulus = 1.0 / (
        initial_porosity / fluid_bulk_modulus
        + (biot_coeff - initial_porosity) * (1.0 - biot_coeff) / soil_drained_bulk
    )
    ...
    drained_poisson = (3.0 * soil_drained_bulk - 2.0 * soil_shear_modulus) / (
        6.0 * soil_drained_bulk + 2.0 * soil_shear_modulus
    )
    undrained_poisson = (3.0 * undrained_bulk_modulus - 2.0 * soil_shear_modulus) / (
        6.0 * undrained_bulk_modulus + 2.0 * soil_shear_modulus
    )
    consolidation_coeff = (
        2.0
        * fluid_mobility
        ...
        * soil_shear_modulus
        * (1.0 - drained_poisson)
        * (1 + undrained_poisson) ** 2
        / 9.0
        / (1.0 - undrained_poisson)
        / (undrained_poisson - drained_poisson)
    )

    roots = [
        1.419988120304100e00,
        4.666177581823210e00,
        7.826417353528760e00,
        1.097591703059930e01,
        1.412188800507350e01,
        1.726626279765500e01,
        2.040978005325610e01,
        2.355278342938330e01,
        2.669545454962390e01,
        2.983789845132980e01,
        3.298018011077390e01,
        3.612234188229790e01,
    ]
    expr1 = [np.sin(v) / (v - np.sin(v) * np.cos(v)) for v in roots]
    expr2 = [np.sin(v) * np.cos(v) / (v - np.sin(v) * np.cos(v)) for v in roots]

    d_terms = [
        expr2[i] * np.exp(-roots[i] ** 2 * consolidation_coeff * t / soil_width**2)
    ...
    vert_disp = -normal_stress * (
        1.0 - drained_poisson
    ) * soil_height / 2.0 / soil_shear_modulus / soil_width + normal_stress * (
        1.0 - undrained_poisson
    ) * soil_height / soil_shear_modulus / soil_width * sum(
        d_terms
    )
    hor_disp = (
        normal_stress * drained_poisson / 2.0 / soil_shear_modulus
        + normal_stress * (1.0 - undrained_poisson) / soil_shear_modulus * sum(d_terms)
    )

    p_terms = [
        (expr1[i] * np.cos(roots[i] * x / soil_width) - expr2[i])
        * np.exp(-((roots[i] / soil_width) ** 2) * consolidation_coeff * t)
        for i in range(len(roots))
    ]
    porepressure = (
        2.0
...
moose_hm = get_moose_results(
    "../../../../../../test/tests/poro_elasticity/gold/mandel.csv"
)
moose_constM = get_moose_results(
    "../../../../../../test/tests/poro_elasticity/gold/mandel_constM.csv"
)
moose_fs = get_moose_results(
    "../../../../../../test/tests/poro_elasticity/gold/mandel_fully_saturated.csv"
)
moose_fsv = get_moose_results(
    "../../../../../../test/tests/poro_elasticity/gold/mandel_fully_saturated_volume.csv"
)

plt.figure()
plt.plot(
    tpoints, expected(0.0, tpoints)[2], "k-", linewidth=2.0, label="expected, x = 0"
)
plt.plot(
    tpoints, expected(0.3, tpoints)[2], "r-", linewidth=2.0, label="expected, x = 0.3"
)
plt.plot(
    tpoints, expected(0.8, tpoints)[2], "b-", linewidth=2.0, label="expected, x = 0.8"
)
...
plt.plot(
    moose_constM[0], moose_constM[1], "ks", markersize=6.0, label="MOOSE ConstM, x = 0"
)
plt.plot(
    moose_constM[0],
    moose_constM[2],
...
plt.plot(
    moose_fs[0], moose_fs[2], "rs", markersize=6.0, label="MOOSE FullySat, x = 0.3"
)
plt.plot(
    moose_fs[0], moose_fs[3], "bs", markersize=6.0, label="MOOSE FullySat, x = 0.8"
)
plt.legend(loc="upper right")
plt.xlabel("time")
plt.ylabel("Porepressure")
...
plt.plot(
    moose_fsv[0], moose_fsv[1], "ks", markersize=6.0, label="MOOSE FullySatVol, x = 0"
)
plt.plot(
    moose_fsv[0], moose_fsv[2], "rs", markersize=6.0, label="MOOSE FullySatVol, x = 0.3"
)
plt.plot(
    moose_fsv[0], moose_fsv[3], "bs", markersize=6.0, label="MOOSE FullySatVol, x = 0.8"
)
plt.legend(loc="upper right")
plt.xlabel("time")
plt.ylabel("Porepressure")
