...
soil_initial_storativity = (
    soil_initial_porosity / fluid_bulk_modulus
    + (biot_coefficient - soil_initial_porosity)
    * (1.0 - biot_coefficient)
    / soil_bulk_modulus
)
consolidation_coefficient = fluid_mobility / (
    soil_initial_storativity + soil_confined_compressibility * biot_coefficient**2
)
initial_porepressure = (
    biot_coefficient
    * soil_confined_compressibility
    * normal_stress_on_top
    / (soil_initial_storativity + soil_confined_compressibility * biot_coefficient**2)
)
...
def expectedU(t):
    ...
    def coeff(n, ctoverh2):
        return np.power(1.0 / (2.0 * n - 1.0), 2) * np.exp(
            -np.power(2.0 * n - 1.0, 2) * np.pi**2 * ctoverh2 / 4.0
        )
...
def expectedP(t, z):
    ...
    def coeff(n, zoverh):
        return (
            np.power(-1.0, n - 1)
            / (2.0 * n - 1.0)
            * np.cos((2.0 * n - 1.0) * np.pi * zoverh / 2.0)
            * np.exp(-np.power(2 * n - 1, 2) * np.pi**2 * ctoverh2 / 4.0)
        )
    ...
    approx = round(
        ctoverh2, -int(np.floor(np.sign(ctoverh2) * np.log10(abs(ctoverh2)))) + 1
    )
...
def get_moose_results(fn):
    ...
    t = [d[0] for d in data]
    u = [
        (-d[12] - initial_displacement) / (final_displacement - initial_displacement)
        for d in data
    ]
...
moose = get_moose_results(
    "../../../../../../test/tests/poro_elasticity/gold/terzaghi_constM.csv"
)
...
plt.plot(
    zoverh,
    np.array(moose[2][1:-1]) / initial_porepressure,
    "ko",
    label="MOOSE " + str(ex[0]),
)
...
plt.plot(
    zoverh,
    np.array(moose[3][1:-1]) / initial_porepressure,
    "bo",
    label="MOOSE " + str(ex[0]),
)
plt.legend(loc="best")
...
plt.plot(
    zoverh,
    np.array(moose[4][1:-1]) / initial_porepressure,
    "ro",
    label="MOOSE " + str(ex[0]),
)
plt.legend(loc="best")
...
plt.plot(
    zoverh,
    np.array(moose[5][1:-1]) / initial_porepressure,
    "go",
    label="MOOSE " + str(ex[0]),
)
plt.legend(loc="best")
