...
def squarepulse1_expected():
    ...
    m = np.array(
        [
            initial_mass,
            initial_mass,
            initial_mass - 0.1 * 200,
            initial_mass - 0.1 * 200,
            initial_mass - 0.1 * 1000,
            initial_mass - 0.1 * 1000,
            initial_mass,
        ]
    )
...
plt.figure(0)
plt.plot(
    squarepulse1_expected()[0],
    squarepulse1_expected()[1],
    "k-",
    linewidth=3.0,
    label="expected",
)
...
def theis_expected(r, t):
    ...
    reciprocal_biot_modulus = (
        porosity / fluid_bulk
    )  # Biot coefficient is assumed to be unity
...
plt.plot(
    rpoints, theis_expected(rpoints, 1000.0), "k-", linewidth=3.0, label="expected"
)
...
tdata = np.genfromtxt(
    "../../../../../../test/tests/dirackernels/gold/theis3_line_0011.csv",
    delimiter=",",
    names=True,
    dtype=float,
)
rdata = np.genfromtxt(
    "../../../../../../test/tests/dirackernels/gold/theis3.csv",
    delimiter=",",
    names=True,
    dtype=float,
)
