    ...
    return [
        (
            ...
            if (x >= 0)
                ...
                0
                if (x <= cutoff)
                else fcn / pow(cutoff, 3) * (2 * x + cutoff) * pow(x - cutoff, 2)
            )
        )
        for x in xx
    ]
...
def s06():
    ...
    prate = sorted(
        list(
            set(
                [(d[11], d[1]) for d in data]
                + [(d[12], d[2]) for d in data]
                + [(d[13], d[3]) for d in data]
                + [(d[14], d[4]) for d in data]
            )
        ),
        key=lambda x: x[0],
    )
...
plt.plot(
    s09_fully_saturated_01()[0],
    s09_fully_saturated_01()[1],
    "r-",
    label="no upwinding, t=0.1",
)
plt.plot(s09_05()[0], s09_05()[1], "k--", label="upwinded, t=0.5")
plt.plot(
    s09_fully_saturated_05()[0],
    s09_fully_saturated_05()[1],
    "r--",
    label="no upwinding, t=0.5",
)
plt.plot(s09_10()[0], s09_10()[1], "k.", label="upwinded, t=1.0")
plt.plot(
    s09_fully_saturated_10()[0],
    s09_fully_saturated_10()[1],
    "r.",
    label="no upwinding, t=1.0",
)
plt.legend()
plt.xlabel("Mass fraction")
plt.ylabel("x (m)")
...
def ip_outflowBC():
    f = open(
        "../../../../../../test/tests/sinks/gold/injection_production_eg_outflowBC_out.csv"
    )
    data = [
        list(map(float, line.strip().split(",")))
        for line in f.readlines()[1:]
    ...
    return (
        [d[0] for d in data],
        [1000 * d[1] for d in data],
        [1000 * d[2] for d in data],
    )
