...
protected:
  ...
  /// mineral concentration at the quadpoints or nodes
  const MaterialProperty<std::vector<Real>> * const _mineral_conc;

  /// d(mineral_conc)/d(porflow variable)
  const MaterialProperty<std::vector<std::vector<Real>>> * const _dmineral_conc_dvar;
};
