...
protected:
  ...
  /// Strain (first const means we never want to dereference and change
  /// the value, second means we'll always be pointing to the same address after initialization (like a reference))
  const GenericMaterialProperty<Real, is_ad> * const _vol_strain_qp;

  /// d(strain)/(dvar) (first const means we never want to dereference and
  /// change the value, second means we'll always be pointing to the same address after initialization (like a reference))
