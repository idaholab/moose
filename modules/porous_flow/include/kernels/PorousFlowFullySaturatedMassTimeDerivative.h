...
protected:
  ...
  /// Jacobian contribution for PorousFlow variable pvar
  ...
  /// Old quadpoint pore pressure
  const MaterialProperty<std::vector<Real>> & _pp_old;

  /// d(pore pressure)/d(PorousFlow variable)
  ...
  /// Old quadpoint temperature
  const MaterialProperty<Real> * const _temperature_old;

  /// d(temperature)/d(PorousFlow variable)
  const MaterialProperty<std::vector<Real>> * const _dtemperature_dvar;

  /// Volumetric strain rate (only when _includes_mechanical)
  const GenericMaterialProperty<Real, is_ad> * const _strain_rate;
