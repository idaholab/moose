...
/**
 * Base class for PorousFlow actions.  This simply defines
 * the name of the PorousFlowDictator.  However, it also
 * contains a number of utility functions that may be used
 * by derived classes
 */
class PorousFlowActionBase : public Action
{
public:
...
protected:
   ...
   */
  void addSingleComponentFluidMaterial(unsigned phase, const UserObjectName & fp);
   ...
   */
  void addRelativePermeabilityCorey(unsigned phase, Real n, Real s_res, Real sum_s_res);
   ...
   * @param sum_s_res The sum of residual saturations over all phases
   */
  void addRelativePermeabilityFLAC(unsigned phase, Real m, Real s_res, Real sum_s_res);
