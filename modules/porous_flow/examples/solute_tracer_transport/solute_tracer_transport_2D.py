...
def C_a(C0, alphal, t, r, Qinj, n, b):
    ...
    for i in range(1, len(t)):
        ...
        C_a.append(
            0.5
            * C0
            * (mt.erfc((r**2 - R_inj**2) / mt.sqrt(16 * alphal * (R_inj**3) / 3)))
        )
    return C_a
...
s01 = np.genfromtxt(
    "solute_tracer_transport_2D_0.1.csv", delimiter=",", names=True, dtype=float
)
s03 = np.genfromtxt(
    "solute_tracer_transport_2D_0.3.csv", delimiter=",", names=True, dtype=float
)
s05 = np.genfromtxt(
    "solute_tracer_transport_2D_0.5.csv", delimiter=",", names=True, dtype=float
)
s07 = np.array(
    np.genfromtxt(
        "solute_tracer_transport_2D_0.7.csv", delimiter=",", names=True, dtype=float
    )
)
s1 = np.genfromtxt(
    "solute_tracer_transport_2D_1.csv", delimiter=",", names=True, dtype=float
)
s3 = np.genfromtxt(
    "solute_tracer_transport_2D_3.csv", delimiter=",", names=True, dtype=float
)
s5 = np.array(
    np.genfromtxt(
        "solute_tracer_transport_2D_5.csv", delimiter=",", names=True, dtype=float
    )
)
s7 = np.genfromtxt(
    "solute_tracer_transport_2D_7.csv", delimiter=",", names=True, dtype=float
)
s10 = np.genfromtxt(
    "solute_tracer_transport_2D_10.csv", delimiter=",", names=True, dtype=float
)
...
plt.plot(
    ...
    "ro",
    markevery=5,
    label="MOOSE (dispersivity=0.7)",
)
plt.plot(
    s07["time"][1:] / (3600 * 24),
    np.array(C_a_07) * 1000,
    "r",
    markevery=5,
    label="Analytical (dispersivity=0.7)",
)
plt.plot(
    s5["time"] / (3600 * 24),
    np.array(s5["C"]) * 1e6,
...
plt.plot(
    ...
    "g",
    markevery=5,
    label="Analytical (dispersivity=5)",
)
