...
p.add_option(
    ...
    default=3.0,
    dest="seam_thickness",
    help="Thickness of the coal seam.  Default=%default",
)
p.add_option(
    "--bias",
    action="store",
    ...
    default=1.1,
    dest="bias",
    help="Bias of the vertical sizes of the elements.  b>=1.  Default=%default",
)
p.add_option(
    "--base_elevation",
    action="store",
...
if opts.bias < 1.0:
    ...
    num_eles = int(
        round(-opts.base_elevation - opts.seam_thickness) / opts.seam_thickness
    )
    ...
    floor_elevations += [
        opts.base_elevation + opts.seam_thickness + i * ele_height
    ...
    num_eles = int(
        round(
            math.log(
                1.0
                - (
                    (-opts.base_elevation - opts.seam_thickness)
                    / opts.seam_thickness
                    * (1.0 - opts.bias)
                )
            )
    ...
    h = (
        (-opts.base_elevation - opts.seam_thickness)
        * (1.0 - opts.bias)
        / (1.0 - math.pow(opts.bias, num_eles))
    )
...
for vp in floor_elevations:
    ...
    vtps += fn
    if opts.verbose:
        sys.stdout.write(" Creating vtp at elevation " + str(vp) + "\n")
    if subprocess.call(cmd, shell=True) != 0:
        sys.stderr.write("Failed to create vtp at elevation " + str(vp) + "\n")
        sys.exit(1)

if opts.verbose:
    sys.stdout.write("Creating 3D mesh " + output_base_name + ".vtu\n")
...
if opts.verbose:
    sys.stdout.write("Removing temporary vtp files\n")
for vp in floor_elevations:
    os.remove("tmp" + str(vp) + ".vtp")

if opts.verbose:
    sys.stdout.write("Creating 3D mesh " + output_base_name + ".e\n")
