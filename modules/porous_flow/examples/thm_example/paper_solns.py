...
def integrate_over_r(rmin, rmax, fcn, n=0, bias_to_small_r=True, num_rs=100):
    ...
    if bias_to_small_r:
        r_points = [
            np.log10(rmin) + (np.log10(rmax) - np.log10(rmin)) * i / float(num_rs - 1)
...
def df1_dss1(ss1, mu1, mu2):
    ...
    return (
        ...
        * (mu1 / mu2)
        * (
            dkr2_dss1(ss1) / kr1(ss1)
            - kr2(ss1) * dkr1_dss1(ss1) / np.power(kr1(ss1), 2)
        )
    )
...
def co2_saturation(t, mu1, mu2, num_saturations=1000):
    ...
    water_saturations = [
        local_ss1ss * i / float(num_saturations - 1) for i in range(num_saturations)
    ]
...
def tt_result(t, mu1, mu2, num_rs=10):
    ...
    for xd in xds:
        ...
        integrand = [
            -gammaR * uu_given_t(t) / (f1(s_water_interp(xd), mu1, mu2) + alpha)
            for xd in xds
        ]
        integral.append(
            sum(integrand) * (finite_difference_points[1] - finite_difference_points[0])
        )  # edges of integral probably don't matter if num_rs is large enough
...
def ggp(t, mu1, mu2, num_rs=10, rmin=rw):
    ...
    rs = [
        np.log10(rmin) + (np.log10(shock_pos) - np.log10(rmin)) * i / float(num_rs - 1)
        for i in range(num_rs)
    ]
    ...
    for r in rs:
        r_points = [
            np.log10(r) + (np.log10(rs[-1]) - np.log10(r)) * i / float(num_rs - 1)
            for i in range(num_rs)
        ]
        r_points = [np.power(10, x) for x in r_points]
        this_integral = 0.0
        for i in range(len(r_points) - 1):
            ...
            this_integral += (
                ...
                * (
                    2.0
                    / b
                    / (kr1(s_water_interp(b)) / mu1 + kr2(s_water_interp(b)) / mu2)
                    + 2.0
                    / a
                    / (kr1(s_water_interp(a)) / mu1 + kr2(s_water_interp(a)) / mu2)
                )
            )
...
def ggb(t, r, mu1):
    ...
    return -scipy.special.expi(
        -(q / rho2) * mu1 * comp1 * xd_given_r(r) / 4 / np.pi / h / k / td_given_t(t)
    )
...
def ggll(t, r, mu1):
    ...
    return -scipy.special.expi(
        -(q / rho2) * mu1 * comp1 * xd_given_r(ll) / 4 / np.pi / h / k / td_given_t(t)
    )
...
def pp_increase(t, mu1, mu2, num_rs=10, rmin=rw):
    ...
    shock_pos = saturation_shock_position(t, mu1, mu2)
    rs = [
        np.log10(rmin) + (np.log10(ll) - np.log10(rmin)) * i / float(num_rs - 1)
        for i in range(num_rs)
    ]
    rs = [np.power(10, r) for r in rs]
    ...
    for r in rs:
        if r <= shock_pos:
            pp_inc.append(
                ggp_interp(r) + mu1 * ggb(t, shock_pos, mu1) - mu1 * ggll(t, r, mu1)
            )
        else:
...
def upr(t, temp_soln, pp_inc, num_rs=10, rmin=rw):
    ...
    for r in rs[1:]:
        r_points = [
            np.log10(rmin) + (np.log10(r) - np.log10(rmin)) * i / float(num_rs - 1)
            for i in range(num_rs)
        ]
        r_points = [np.power(10, x) for x in r_points]
        this_integral = 0.0
        for i in range(len(r_points) - 1):
            ...
            contrib = (
                -0.5
                * (b - a)
                * (b * ttd_given_tt(temp_interp(b)) + a * ttd_given_tt(temp_interp(a)))
            )
...
def aa_bb(upr_soln, pp_inc):
    ...
    phatzero = (
        (pp_inc[1][0] + stress_eff_hor_ini)
        * (1 - poisson)
        / thermal_expansion
        / ttr
        / young
    )  # Probably tara had -stress_eff_hor_ini instead as that gives similar displacements, but incorrect effective stresses near the origin
    # ASSUME that upr_soln[1][-1] = u_{pr}(xi=xi1), ie, that upr has been evaluated up to xi=xi1 (which is r=L)
    bb = ((1 - 2 * poisson) * phatzero - upr_soln[1][-1] * rw / ll) / (
        1 - 2 * poisson + rw * rw / ll / ll
    )  # Eqn (A2) of Tara's second paper (possibly with fixed sign) with fixed denominator (that was wrong in Rehbinder too)
    aa = (
        -bb * rw * rw / ll / ll - upr_soln[1][-1] * rw / ll
    )  # Eqn (A2) of Tara's second paper with fixed sign on first term
    return (aa, bb)
...
def eff_rr(upr_soln, pp_inc, num_rs=10, rmin=rw):
    ...
    result = [
        stress_eff_hor_ini
        + (thermal_expansion * ttr * young / (1 - poisson))
        * (
            aa / (1 - 2 * poisson)
            - bb * rw * rw / pp_inc[0][i] / pp_inc[0][i]
            + (1 - poisson) * pp_inc[1][i] / thermal_expansion / ttr / young
            - upr_soln[1][i] * rw / pp_inc[0][i]
        )
        for i in range(len(pp_inc[0]))
    ]
...
def eff_tt(upr_soln, pp_inc, tt_soln, num_rs=10, rmin=rw):
    ...
    result = [
        ...
        * (
            aa / (1 - 2 * poisson)
            + bb * rw * rw / pp_inc[0][i] / pp_inc[0][i]
            + pp_inc[1][i] * poisson / thermal_expansion / ttr / young
            + upr_soln[1][i] * rw / pp_inc[0][i]
            + ttd_given_tt(tt_interp(pp_inc[0][i]))
        )
        for i in range(len(pp_inc[0]))
    ]
...
pp_increase_five_years = pp_increase(
    five_years, mu1_moose, mu2_moose, num_rs=resolution
)
plt.semilogx(
    pp_increase_one_hour[0],
    [p / 1e6 for p in pp_increase_one_hour[1]],
    "b-",
    label="1 hour",
)
plt.semilogx(
    pp_increase_one_day[0],
    [p / 1e6 for p in pp_increase_one_day[1]],
    "r-",
    label="1 day",
)
plt.semilogx(
    pp_increase_one_month[0],
    [p / 1e6 for p in pp_increase_one_month[1]],
    "g-",
    label="1 month",
)
plt.semilogx(
    pp_increase_five_years[0],
    [p / 1e6 for p in pp_increase_five_years[1]],
    "k-",
    label="5 years",
)
plt.legend(loc="best")
...
sg_one_month = co2_saturation(
    one_month, mu1_moose, mu2_moose, num_saturations=resolution
)
sg_five_years = co2_saturation(
    five_years, mu1_moose, mu2_moose, num_saturations=resolution
)
...
plt.figure()
upr_one_hour = upr(
    one_hour, tt_one_hour, pp_increase_one_hour, num_rs=resolution, rmin=rw
)
uh_one_hour = uh(upr_one_hour, pp_increase_one_hour, num_rs=resolution, rmin=rw)
uhat_one_hour = [
    upr_one_hour[1][i] + uh_one_hour[1][i] for i in range(len(upr_one_hour[1]))
]
u_one_hour = (
    uh_one_hour[0],
    [
        1e3 * thermal_expansion * ttr * rw * (1 + poisson) / (1 - poisson) * uhat
        for uhat in uhat_one_hour
    ],
)
...
uhat_one_day = [
    upr_one_day[1][i] + uh_one_day[1][i] for i in range(len(upr_one_day[1]))
]
u_one_day = (
    uh_one_day[0],
    [
        1e3 * thermal_expansion * ttr * rw * (1 + poisson) / (1 - poisson) * uhat
        for uhat in uhat_one_day
    ],
)
upr_one_month = upr(
    one_month, tt_one_month, pp_increase_one_month, num_rs=resolution, rmin=rw
)
uh_one_month = uh(upr_one_month, pp_increase_one_month, num_rs=resolution, rmin=rw)
uhat_one_month = [
    upr_one_month[1][i] + uh_one_month[1][i] for i in range(len(upr_one_month[1]))
]
u_one_month = (
    uh_one_month[0],
    [
        1e3 * thermal_expansion * ttr * rw * (1 + poisson) / (1 - poisson) * uhat
        for uhat in uhat_one_month
    ],
)
upr_five_years = upr(
    five_years, tt_five_years, pp_increase_five_years, num_rs=resolution, rmin=rw
)
uh_five_years = uh(upr_five_years, pp_increase_five_years, num_rs=resolution, rmin=rw)
uhat_five_years = [
    upr_five_years[1][i] + uh_five_years[1][i] for i in range(len(upr_five_years[1]))
]
u_five_years = (
    uh_five_years[0],
    [
        1e3 * thermal_expansion * ttr * rw * (1 + poisson) / (1 - poisson) * uhat
        for uhat in uhat_five_years
    ],
)
...
seff_rr_one_hour = eff_rr(
    upr_one_hour, pp_increase_one_hour, num_rs=resolution, rmin=rw
)
seff_tt_one_hour = eff_tt(
    upr_one_hour, pp_increase_one_hour, tt_one_hour, num_rs=resolution, rmin=rw
)
seff_rr_one_day = eff_rr(upr_one_day, pp_increase_one_day, num_rs=resolution, rmin=rw)
seff_tt_one_day = eff_tt(
    upr_one_day, pp_increase_one_day, tt_one_day, num_rs=resolution, rmin=rw
)
seff_rr_one_month = eff_rr(
    upr_one_month, pp_increase_one_month, num_rs=resolution, rmin=rw
)
seff_tt_one_month = eff_tt(
    upr_one_month, pp_increase_one_month, tt_one_month, num_rs=resolution, rmin=rw
)
seff_rr_five_years = eff_rr(
    upr_five_years, pp_increase_five_years, num_rs=resolution, rmin=rw
)
seff_tt_five_years = eff_tt(
    upr_five_years, pp_increase_five_years, tt_five_years, num_rs=resolution, rmin=rw
)
