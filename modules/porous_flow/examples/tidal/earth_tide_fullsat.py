...
def strain_xx(t):
    return (
        5 * np.cos(t * 2 * np.pi)
        + 2 * np.cos((t - 0.5) * 2 * np.pi)
        + 1 * np.cos((t + 0.3) * 0.5 * np.pi)
    )
...
def strain_yy(t):
    return (
        7 * np.cos(t * 2 * np.pi)
        + 4 * np.cos((t - 0.3) * 2 * np.pi)
        + 7 * np.cos((t + 0.6) * 0.5 * np.pi)
    )


def strain_zz(t):
    return (
        7 * np.cos((t - 0.5) * 2 * np.pi)
        + 4 * np.cos((t - 0.8) * 2 * np.pi)
        + 7 * np.cos((t + 0.1) * 4 * np.pi)
    )


moose = np.genfromtxt(
    "gold/earth_tide_fullsat_out.csv", delimiter=",", names=True, dtype=float
)
