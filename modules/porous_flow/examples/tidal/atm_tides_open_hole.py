...
data = np.genfromtxt(
    "gold/atm_tides_open_hole_out.csv", delimiter=",", names=True, dtype=float
)
...
plt.plot(
    data["time"][1:] / 3600.0,
    (data["p0_0"][1:] - data["p0_0"][1]) / 1000.0,
    label="atmospheric variation",
)
plt.plot(
    data["time"][1:] / 3600.0,
    (data["p100_100"][1:] - data["p100_100"][1]) / 1000.0,
    label="100m deep, 100m from borehole bottom",
)
plt.xlabel("t (hr)")
...
plt.plot(
    data["time"][1:] / 3600.0, 1000 * (data["uz100"][1:]), label="100m from borehole"
)
plt.xlabel("t (hr)")
