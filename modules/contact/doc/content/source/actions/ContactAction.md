...
For the `mortar` and `mortar_penalty` formulations, the action forwards
[!param](/Contact/ContactAction/mortar_3d_qp_mapping) to its mortar objects. See
[AutomaticMortarGeneration.md#quadrature-point-mapping] for the two mapping methods. The parameter
has no effect in 2D and must be consistent across each mortar interface.
