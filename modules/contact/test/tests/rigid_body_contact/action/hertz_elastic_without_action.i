...
# HAND-ROLLED version: uses `[Physics/SolidMechanics/QuasiStatic]` for the
# disp kernels + strain material but spells out every contact object by
# hand.  Companion `hertz_elastic_with_action.i` compresses that plumbing
# into a single `[RigidContact]` sub-block.  Both CSVDiff against the
# same gold to prove the action expansion is bit-for-bit equivalent.
