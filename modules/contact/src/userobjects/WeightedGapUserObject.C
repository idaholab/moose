...
WeightedGapUserObject::includeNodalNormalDerivatives()
{
  ...
  // A valid penetration_tolerance identifies augmented-Lagrange contact, which retains frozen
  // normal directions; the runtime derivative flag set here and the sparsity decision an
  // augmented-Lagrange-configured user object already made from its own InputParameters (e.g.
  // PenaltyWeightedGapUserObject's relationship-manager configuration) must stay in agreement, so
  // this is enforced here rather than only documented.
  if (isParamValid("penetration_tolerance"))
    mooseError("Nodal-normal derivatives cannot be enabled on user object '",
               name(),
               "' because it is configured for augmented-Lagrange contact (a valid "
               "'penetration_tolerance' was supplied), which retains frozen normal directions.");
...
WeightedGapUserObject::nodalCoordinate(const Node & node, const Point & geometry_coordinate) const
{
  ...
  for (const auto & [variable, component] : displacement_variables)
    if (variable)
    {
      ...
      // A node carrying no displacement degree of freedom, such as a mapping-only node on an
      // enriched element, does not move with the solution, so leaving its coordinate derivative at
      // zero is correct rather than an omission.
