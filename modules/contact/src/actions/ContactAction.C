...
ContactAction::validParams()
{
  ...
  MooseEnum lm_space(getContactLMSpaceOptions(), "MATCH_DISPLACEMENT");
  lm_space.addDocumentation(
      "MATCH_DISPLACEMENT",
      "Use the same finite element order as the displacement variables for generated mortar "
      "Lagrange multiplier variables.");
  lm_space.addDocumentation(
      "LINEAR",
      "Use first-order LAGRANGE generated mortar Lagrange multiplier variables, independent "
      "of the displacement variable order.");
  params.addParam<MooseEnum>(
      "lm_space",
      lm_space,
      "Finite element space for mortar Lagrange multiplier variables generated by the "
      "contact action. This parameter only applies to the 'mortar' contact formulation.");
  params.addParam<bool>(
      "normalize_c",
      false,
  ...
  params.transferParam<Real>(mortar_constraint_params, "minimum_projection_angle");
  params.transferParam<MooseEnum>(mortar_constraint_params, "mortar_3d_subpatch_plane");
  ...
  // Interface ghosting
  params.addParamNamesToGroup("ghost_whole_interface", "Interface Ghosting");
...
ContactAction::ContactAction(const InputParameters & params)
  : Action(params),
    ...
    _lm_space(getParam<MooseEnum>("lm_space").getEnum<ContactLMSpace>()),
  ...
  else
  {
    if (params.isParamSetByUser("correct_edge_dropping"))
      ...
      paramError("segment_quadrature",
                 ...
                 "'mortar' formulation.");
    else if (params.isParamSetByUser("lm_space"))
      paramError("lm_space",
                 "The 'lm_space' option can only be used with the 'mortar' formulation.");
  }
...
ContactAction::act()
{
  ...
  if (_current_task == "add_contact_aux_variable")
  {
    ...
    const auto order = _problem->systemBaseNonlinear(/*nl_sys_num=*/0)
                           ...
                           .variable_type(displacements[0])
                           .order.get_order();
    const auto mortar_lm_order =
        _lm_space == ContactLMSpace::LINEAR ? static_cast<int>(FIRST) : order;
    std::unique_ptr<InputParameters> current_params;
    const auto create_aux_var_params =
        [this, order, mortar_lm_order, &current_params]() -> InputParameters &
    {
      current_params = std::make_unique<InputParameters>(_factory.getValidParams("MooseVariable"));
      // Node/face and mortar-penalty contact aux variables continue to follow the displacement
      // order. Mortar LM contact aux variables live on the same contact surface as the generated LM
      // field, so they use the selected generated LM space.
      const auto aux_order = _formulation == ContactFormulation::MORTAR ? mortar_lm_order : order;
      current_params->set<MooseEnum>("order") =
          Utility::enum_to_string<Order>(OrderWrapper{aux_order});
    ...
    {
      {
        ...
        for (const auto & disp_component : disp_components)
        {
          ...
          var_params.set<MooseEnum>("order") =
              Utility::enum_to_string<Order>(OrderWrapper{mortar_lm_order});
...
ContactAction::addMortarContact()
{
  ...
  const auto addLagrangeMultiplier = [this, &displacements](const std::string & secondary_subdomain,
                                                            ...
                                                            const bool penalty_traction) //
  {
    ...
    // The lm_space option is only valid for the mortar Lagrange multiplier formulation. Mortar
    // penalty traction variables continue to use the displacement order.
    const int lm_order =
        _formulation == ContactFormulation::MORTAR && _lm_space == ContactLMSpace::LINEAR
            ? static_cast<int>(FIRST)
            : primal_type.order.get_order();
  ...
  if (_current_task == "add_mortar_variable" && _formulation == ContactFormulation::MORTAR)
  {
    for (const auto & contact_pair : _boundary_pairs)
    {
      ...
      addLagrangeMultiplier(secondary_subdomain_name,
                            ...
                            getParam<Real>("normal_lm_scaling"),
                            false,
                            false);
  ...
  if (_current_task == "add_user_object")
  {
    ...
    for (const auto & contact_pair : _boundary_pairs)
    {
      ...
      const auto register_mortar_uo_name =
          [this, &suffix](const auto & bnd_pair, const auto & uo_prefix)
      {
