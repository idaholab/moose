...
ContactAction::validParams()
{
  ...
  lm_space.addDocumentation(
      ...
      "Lagrange multiplier and mortar traction variables.");
  lm_space.addDocumentation(
      "LINEAR",
      "Use first-order LAGRANGE generated mortar Lagrange multiplier and mortar traction "
      "variables, independent of the displacement variable order.");
  params.addParam<MooseEnum>(
      "lm_space",
      lm_space,
      "Finite element space for mortar Lagrange multiplier and mortar traction variables "
      "generated by the contact action. This parameter only applies to mortar-based contact "
      "formulations.");
  params.addParam<bool>(
      "normalize_c",
      false,
  ...
  else
  {
    if (params.isParamSetByUser("correct_edge_dropping"))
      ...
      paramError("segment_quadrature",
             ...
             _formulation != ContactFormulation::MORTAR_PENALTY)
      paramError("lm_space",
                 "The 'lm_space' option can only be used with mortar-based formulations.");
  }
...
ContactAction::act()
{
  ...
  if (_current_task == "add_contact_aux_variable")
  {
    ...
    const auto order = _problem->systemBaseNonlinear(/*nl_sys_num=*/0)
                           ...
                           .variable_type(displacements[0])
                           .order.get_order();
    const auto mortar_order =
        _lm_space == ContactLMSpace::LINEAR && (_formulation == ContactFormulation::MORTAR ||
                                                _formulation == ContactFormulation::MORTAR_PENALTY)
            ? static_cast<int>(FIRST)
            : order;
    std::unique_ptr<InputParameters> current_params;
    const auto create_aux_var_params =
        [this, order, mortar_order, &current_params]() -> InputParameters &
    {
      current_params = std::make_unique<InputParameters>(_factory.getValidParams("MooseVariable"));
      // Node/face contact aux variables continue to follow the displacement order. Mortar aux
      // variables represent mortar quantities, so they use the selected generated LM space.
      const auto aux_order = (_formulation == ContactFormulation::MORTAR ||
                              _formulation == ContactFormulation::MORTAR_PENALTY)
                                 ? mortar_order
                                 : order;
    ...
    {
      {
        ...
        for (const auto & disp_component : disp_components)
        {
          ...
          var_params.set<MooseEnum>("order") =
              Utility::enum_to_string<Order>(OrderWrapper{mortar_order});
...
ContactAction::addMortarContact()
{
                                                        ...
                                                        const bool penalty_traction) //
  {
    ...
    // MATCH_DISPLACEMENT preserves the historical action behavior. LINEAR is useful with
    // higher-order displacement/contact surfaces when the generated multiplier field should only
    // use the contact-surface corner-node interpolation.
    const int lm_order = _lm_space == ContactLMSpace::LINEAR ? static_cast<int>(FIRST)
