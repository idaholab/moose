...
{
  // each single series is assumed to be a function of a single variable so that
  // it has two bounds
  if (bounds.size() != _series_types.size() * 2)
    mooseError("Cartesian: Mismatch between the physical bounds provided and"
               " the number of series in the functional basis!");
