...
// default constructor creates a functional basis with one term. In order for
// the _series member to be initialized, we initialized it with a Legendre series.
CompositeSeriesBasisInterface::CompositeSeriesBasisInterface() : FunctionalBasisInterface(1)
{
  _series.push_back(new Legendre());
}

// the non-default constructor is where we actually loop over the series_types
// and initialize pointers to those members. Because we won't know the number of
// terms until the end of the body of the constructor, we need to call the default
// FBI constructor.
...
{
  // check that the orders matches the number of series before trying to access
  // orders[i] in the loop below, which would be an invalid memory access.
  if (orders.size() != _series_types.size())
    mooseError("CSBI: Incorrect number of 'orders' specified for 'FunctionSeries'!"
               " Check that 'orders' is the correct length and no invalid enumerations"
               " are specified for the series.");
}

void
CompositeSeriesBasisInterface::evaluateOrthonormal()
{
  // Evaluate the orthonormal versions of each of the single series, and
  // collect the results before passing them to evaluateSeries, where they will
  // be multiplied together correctly and stored in the composite series
  // basis evaluation.
...
CompositeSeriesBasisInterface::evaluateSeries(
    const std::vector<std::vector<Real>> & single_series_basis_evaluation)
{
  // appropriate number of loops based on 1-D, 2-D, or 3-D to multiply the
  // basis evaluations of the single series together to form the basis evaluation
  // of the entire composite series
  Real f1, f2, f3;
  std::size_t term = 0;

  if (single_series_basis_evaluation.size() == 1)
    for (std::size_t i = 0; i < _series[0]->getNumberOfTerms(); ++i, ++term)
      save(term, single_series_basis_evaluation[0][i]);

  if (single_series_basis_evaluation.size() == 2)
    for (std::size_t i = 0; i < _series[0]->getNumberOfTerms(); ++i)
    {
      f1 = single_series_basis_evaluation[0][i];
      for (std::size_t j = 0; j < _series[1]->getNumberOfTerms(); ++j, ++term)
      {
        f2 = single_series_basis_evaluation[1][j];
        save(term, f1 * f2);
      }
    }

  if (single_series_basis_evaluation.size() == 3)
    for (std::size_t i = 0; i < _series[0]->getNumberOfTerms(); ++i)
    {
      f1 = single_series_basis_evaluation[0][i];
      for (std::size_t j = 0; j < _series[1]->getNumberOfTerms(); ++j)
      {
        ...
        {
          f3 = single_series_basis_evaluation[2][k];
          save(term, f1 * f2 * f3);
        }
      }
    }
...
{
  // Evaluate the standard versions of each of the single series, and
  // collect the results before passing them to evaluateSeries, where they will
  // be multiplied together correctly and stored in the composite series
  // basis evaluation.
  std::vector<std::vector<Real>> single_series_basis_evaluation;
...
{
  // if any one of the single series have an invalid cache, then we need to
  // re-evaluate the entire composite series because the terms are multiplied
  for (auto & series : _series)
...
// a point is in the physical bounds of the composite series if it is in the
// physical bounds of each of the single series
bool
...
CompositeSeriesBasisInterface::setNumberOfTerms()
{
  ...
  // the length of the _basis_evaluation depends on the number of terms, so we
  // need to clear the entries because the number of terms in the composite series
  // may have changed.
...
CompositeSeriesBasisInterface::setOrder(const std::vector<std::size_t> & orders)
{
  ...
  // after changing the order of each single series, we need to recompute the
  // number of terms by looping over those single series. This also clears the
  // basis evaluation of the composite series.
  setNumberOfTerms();
}
...
{
  // return if this point is the same as the last at which the composite series
  // was evaluated
