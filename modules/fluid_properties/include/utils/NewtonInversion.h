 ...
 * @param[in] object_name name of the fluid properties appended to name of the routine calling the
 * method
            ...
            const unsigned int max_its = 100)
{
  ...
  do
  {
    ...
    if (!MooseUtils::relativeFuzzyEqual(J_differenced, dy_dz, 1e-2))
      mooseDoOnce(mooseWarning(object_name + ": Bad Jacobian in NewtonSolve"));
#endif
    ...
    // Check for NaNs
    if (std::isnan(z))
      mooseException(object_name + ": NaN detected in Newton solve");
