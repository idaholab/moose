...
#include "TabulatedBilinearFluidPropertiesTest.h"
...
TEST_F(TabulatedBilinearFluidPropertiesTest, generateTabulatedData)
{
  Real p = 1.5e6;
  Real T = 450.0;

  // Generate the tabulated data
  const_cast<TabulatedBilinearFluidProperties *>(_tab_gen_fp)->initialSetup();
...
TEST_F(TabulatedBilinearFluidPropertiesTest, passthrough)
{
  Real p = 1.5e6;
  Real T = 450.0;
  const Real tol = REL_TOL_SAVED_VALUE;

  // As the flags for interpolation in TabulatedBilinearFluidProperties default to false,
 ...
 */
TEST_F(TabulatedBilinearFluidPropertiesTest, fluidName) { EXPECT_EQ(_tab_fp->fluidName(), "co2"); }

/**
 * Test that the molar mass is correctly returned
 */
TEST_F(TabulatedBilinearFluidPropertiesTest, molarMass)
{
 ...
 */
TEST_F(TabulatedBilinearFluidPropertiesTest, combined)
{
  const Real p = 1.0e6;
  const Real T = 300.0;
