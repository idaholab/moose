...
TabulatedFluidProperties::validParams()
{
  ...
  params.addRangeCheckedParam<unsigned int>("num_h",
                                            100,
                                            "num_h > 0",
                                            "Number of points to divide specific enthalpy "
                                            "range for (v,h) lookups.");
  ...
  params.addParamNamesToGroup("num_T num_p num_v num_e num_h use_log_grid_v",
...
TabulatedFluidProperties::TabulatedFluidProperties(const InputParameters & parameters)
  : SinglePhaseFluidProperties(parameters),
    ...
    _num_h(getParam<unsigned int>("num_h")),
...
void
TabulatedFluidProperties::generatePTFromVE(std::vector<std::vector<Real>> & p_from_v_e,
...
void
TabulatedFluidProperties::generatePTFromVH(std::vector<std::vector<Real>> & p_from_v_h,
                                           std::vector<std::vector<Real>> & T_from_v_h)
{
  ...
  for (unsigned int i = 0; i < _num_v; ++i)
  {
    Real p_guess = _p_initial_guess;
    Real T_guess = _T_initial_guess;
    p_from_v_h[i].resize(_num_h);
    T_from_v_h[i].resize(_num_h);
    for (unsigned int j = 0; j < _num_h; ++j)
    {
      Real p_vh, T_vh;
      // Using an input fluid property instead of the tabulation will get more exact inversions
      ...
      else
      {
        ...
        _error_on_out_of_bounds = false;
        p_T_from_v_h(
            _specific_volume[i], _enthalpy[j], p_guess, T_guess, p_vh, T_vh, conversion_succeeded);
        _error_on_out_of_bounds = old_error_behavior;
        // track number of times convergence failed
        if (!conversion_succeeded)
          ++fail_counter_vh;
...
void
TabulatedFluidProperties::checkNaNs(Real min_1,
                                    Real max_1,
                                    Real min_2,
                                    Real max_2,
                                    unsigned int i,
...
void
TabulatedFluidProperties::checkOutofBounds(Real min,
                                           Real max,
                                           Real & variable,
...
void
TabulatedFluidProperties::outputWarnings(unsigned int num_nans_p,
