...
# Record the set of enabled modules in a generated header that ModulesApp.h includes.
#
# Adding a module to an application's Makefile only changes the -D<MODULE>_ENABLED
# compiler flags; it does not modify any source or header file. Header-based
# dependency tracking would therefore not recompile the module registration (the
# #ifdef <MODULE>_ENABLED blocks in ModulesApp.h/.C), so a module added without a
# 'make clean' would build and link but its objects would silently go unregistered.
# Writing the enabled-module set into this header (only when it changes) gives the
# registration translation units a real header dependency on that set, so they are
# recompiled automatically when a module is added or removed.
moose_enabled_modules := $(sort $(patsubst -D%_ENABLED,%,$(filter -D%_ENABLED,$(ADDITIONAL_CPPFLAGS))))
modules_enabled_header := $(MOOSE_DIR)/modules/module_loader/include/ModulesEnabled.h
$(shell mkdir -p $(dir $(modules_enabled_header)); \
  if [ ! -f $(modules_enabled_header) ] || \
     [ "$$(sed -n 's|^// Enabled modules: ||p' $(modules_enabled_header) 2>/dev/null)" != "$(moose_enabled_modules)" ]; then \
    { echo '#pragma once'; \
      echo '// Auto-generated by modules/modules.mk. Do not edit.'; \
      echo '// Records the set of enabled MOOSE modules so that changing it (for example by'; \
      echo '// adding a module to an application Makefile) recompiles the module registration'; \
      echo '// in ModulesApp.h/.C without requiring a clean rebuild.'; \
      echo '// Enabled modules: $(moose_enabled_modules)'; } > $(modules_enabled_header); \
  fi)
