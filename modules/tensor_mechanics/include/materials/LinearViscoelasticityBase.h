 ...
 * It represents the arrangement of the system (typically, in parallel or in series), the mechanical 
 * properties of each individual spring and dashpot in the model, and the internal strains associated
 * with each dashpot.
 *
 * To use a linear viscoelastic model, the user must provide a LinearViscoelasticityManager, that will
 * initialize and update the internal time-stepping scheme at the beginning and the end of each time step.
 *
 ...
 * ComputeMultipleInelasticStress material associated with a LinearViscoelasticStressUpdate material if 
 * there are multiple sources of inelastic strains (this requires to use incremental strains).
 *
 ...
 * properties. The scheme acts upon a series of internal strain variables, stored in the _viscous_strains
 * material properties. At the start of each time step, the apparent 
 * elastic properties of the assembly and the apparent creep strain are calculated. The rate-dependent
 * problem becomes then equivalent to a purely elastic system. At the end of the time step, the internal 
 * strain variables are updated according to the current
 ...
 *
 * This class does not store the true creep strain of the material, but an equivalent creep strain adapted 
 * for numerical efficiency. The true creep strain themsevelves are declared by 
...
public:
  ...
  /**
   * Compute the apparent properties at a quadrature point. This initializes the internal time-stepping
  ...
  /** 
   * Update the internal viscous strains at a quadrature point. Calling this method is required at the end
   * of each time step to update the internal time-stepping scheme correctly.
   *
   * This method is called by LinearViscoelasticityManager.
   * This method is pure virtual. Inherited classes must override it.
   *
   * This method is related to the internal time-stepping scheme. It should only be overwritten by classes
   * that inherit directly from LinearViscoelasticityBase, and that represent a different spring-dashpot
   * assembly. See GeneralizedKelvinVoigtBase or GeneralizedMaxwellBase for example.
   *
   * @param qp The quadrature point at which the viscous strains are updated.
   * @param effective_strain The sum of elastic_strain + creep_strain to which the system is subject to.
  ...
  /**
   * Computes the true creep strain resulting from a given mechanical strain. This method is required to 
   * translate the apparent creep strain stored in this class to the true creep strain.
   *
...
protected:
   ...
   * override it. See GeneralizedKelvinVoigtModel and GeneralizedKelvinVoigtBase for examples.
   */
   ...
   * inherited only if there is a faster way to compute this inverse (for example, if they are known).
   *
   * This method is related to the storage of the mechanical properties of each spring and dashpot
   * in the system, and not the internal time-stepping procedure. Only end-user classes should 
   * override it. See GeneralizedKelvinVoigtModel and GeneralizedKelvinVoigtBase for examples.
   ...
   * It is called after the mechanical properties have been set, and before the apparent creep strains
   * are calculated.
   *
   * This method is also responsible for calculating the instantaneous elasticity tensor, and the inverse
   * of both the apparent and instantaneous elasticity tensors.
   *
   * This method is purely virtual. Inherited classes must override it.
   *
   * This method is related to the internal time-stepping scheme. It should only be overwritten by classes
  ...
  /**
   * This method computes the apparent creep strain corresponding to the current viscous_strain of each 
   * dashpot. It must be called after the apparent elasticity tensors have been calculated.
   *
   * This method is purely virtual. Inherited classes must override it.
   *
   * This method is related to the internal time-stepping scheme. It should only be overwritten by classes
  ...
  /**
   * This is the number of internal variables required by the model. This must be set in the constructor
   * of an inherited class. See GeneralizedKelvinVoigtModel or GeneralizedMaxwell model for examples.
   */
  ...
  /**
   * The internal strain variables required by the time-stepping procedure (generally, on a one-on-one basis
   * with the number of dashpot).
   */
  ...
  /**
   * If activated, the time-stepping scheme will be re-initialized at each step of the solver. This may be
   * required for models in which the mechanical properties vary following other variables. If the mechanical
   * properties are constant through the time step, this can be set to false.
   */
