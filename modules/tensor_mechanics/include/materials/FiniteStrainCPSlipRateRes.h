...
class FiniteStrainCPSlipRateRes: public FiniteStrainCrystalPlasticity
{
  ...
  */
  bool lineSearchUpdateSlipRate(const Real, const std::vector<Real> & );

  /**
  * This function calculates the dot product of residual and update
  */
  Real calcResidDotProdUpdate(const std::vector<Real> &);

  std::vector<Real> _resid;
  std::vector<Real> _slip_rate;
  std::vector<Real> _dsliprate_dgss;
  std::vector< std::vector<Real> > _jacob;
  std::vector< std::vector<Real> > _dsliprate_dsliprate;
};
