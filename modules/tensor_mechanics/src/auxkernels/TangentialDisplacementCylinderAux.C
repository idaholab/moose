...
#include "TangentialDisplacementCylinderAux.h"
#include "MooseMesh.h"

registerMooseObject("TensorMechanicsApp", TangentialDisplacementCylinderAux);

InputParameters
TangentialDisplacementCylinderAux::validParams()
{
...
TangentialDisplacementCylinderAux::TangentialDisplacementCylinderAux(const InputParameters & parameters)
  : AuxKernel(parameters), _ndisp(coupledComponents("displacements")), _disp_vals(_ndisp)
{
  ...
  for (const auto & sbd : subdomains)
  {
    if (sbd == sbd_begin)
      ...
      mooseError("TangentialDisplacementCylinderAux requires that all subdomains have the same "
                 "coordinate type");
  }
  ...
  if (_coord_system == Moose::COORD_XYZ && _ndisp == 1)
    ...
    mooseError("TangentialDisplacementCylinderAux can only be used with Cartesian or axisymmetric "
               "coordinate systems");
...
Real
TangentialDisplacementCylinderAux::computeValue()
{
  Real tan_disp = 0.0;
  Real rad_disp = 0.0;
  ...
  switch (_coord_system)
  {
    case Moose::COORD_XYZ:
    {
      ...
      if (rad > 0.0)
      {
        ...
        // tan_disp = atan(rad_vec[1]/rad_vec[0]);
      }
      else
