...
CrystalPlasticityRotationOutAux::computeValue()
{
  ...
  if (_t_step % _out_freq == 0)
  {
    ...
    fileout << _t_step << ' ' << _dt << ' ' << _JxW[_qp] << ' '
            << _update_rot[_qp](0,0) << ' ' << _update_rot[_qp](0,1) << ' ' << _update_rot[_qp](0,2) << ' '
            << _update_rot[_qp](1,0) << ' ' << _update_rot[_qp](1,1) << ' ' << _update_rot[_qp](1,2) << ' '
            << _update_rot[_qp](2,0) << ' ' << _update_rot[_qp](2,1) << ' ' << _update_rot[_qp](2,2) << '\n';
  }
