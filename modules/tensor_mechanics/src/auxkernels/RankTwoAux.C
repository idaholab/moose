...
InputParameters validParams<RankTwoAux>()
{
  ...
  params.addRequiredRangeCheckedParam<unsigned int>("index_i", "index_i != 0", "The index i of ij for the tensor to output (1, 2, 3)");
  params.addRequiredRangeCheckedParam<unsigned int>("index_j", "index_j != 0", "The index j of ij for the tensor to output (1, 2, 3)");
  return params;
}
