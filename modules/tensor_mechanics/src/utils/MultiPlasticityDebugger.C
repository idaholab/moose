...
void
MultiPlasticityDebugger::checkJacobian(const RankFourTensor & E_inv, const std::vector<Real> & intnl_old)
{
  ...
  fdJacobian(_fspb_debug_stress, intnl_old, _fspb_debug_intnl, _fspb_debug_pm, delta_dp, E_inv, false, fdjac);
...
void
MultiPlasticityDebugger::fdJacobian(const RankTwoTensor & stress, const std::vector<Real> & intnl_old, const std::vector<Real> & intnl, const std::vector<Real> & pm, const RankTwoTensor & delta_dp, const RankFourTensor & E_inv, bool eliminate_ld, std::vector<std::vector<Real> > & jac)
{
  ...
  calculateRHS(stress, intnl_old, intnl, pm, delta_dp, orig_rhs, active, eliminate_ld, deactivated_due_to_ld); // this calculates RHS, and also set deactivated_due_to_ld.  The latter stays fixed for the rest of this routine
...
MultiPlasticityDebugger::checkSolution(const RankFourTensor & E_inv)
{
  ...
  Moose::err << "Before checking Ax=b is correct, check that the Jacobians given below are equal.\n";
  ...
  deactivated_due_to_ld.assign(_num_surfaces, false); // this potentially gets changed by nrStep, below
...
void
MultiPlasticityDebugger::fddyieldFunction_dstress(const RankTwoTensor & stress, const std::vector<Real> & intnl, std::vector<RankTwoTensor> & df_dstress)
{
...
void
MultiPlasticityDebugger::fddflowPotential_dstress(const RankTwoTensor & stress, const std::vector<Real> & intnl, std::vector<RankFourTensor> & dr_dstress)
{
