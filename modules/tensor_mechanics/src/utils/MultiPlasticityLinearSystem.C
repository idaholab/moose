...
MultiPlasticityLinearSystem::calculateConstraints(const RankTwoTensor & stress, const std::vector<Real> & intnl_old, const std::vector<Real> & intnl, const std::vector<Real> & pm, const RankTwoTensor & delta_dp, std::vector<Real> & f, std::vector<RankTwoTensor> & r, RankTwoTensor & epp, std::vector<Real> & ic, const std::vector<bool> & active)
{
  ...
  // ic = intnl - intnl_old + pm*h   [only for models that are active]
  //
...
// NOTE: TODO : this needs to be made a bit cleaner
void
MultiPlasticityLinearSystem::calculateJacobian(const RankTwoTensor & stress, const std::vector<Real> & intnl, const std::vector<Real> & pm, const RankFourTensor & E_inv, const std::vector<bool> & active, const std::vector<bool> & deactivated_due_to_ld, std::vector<std::vector<Real> > & jac)
{
  ...
  std::vector<bool> active_model(_num_models);
  std::vector<bool> active_maybe_deact_model(_num_models);
  for (unsigned model = 0 ; model < _num_models ; ++model)
  {
    active_model[model] = anyActiveSurfaces(model, active_surface);
    active_maybe_deact_model[model] = anyActiveSurfaces(model, active);
  }
  ...
  ind = 0;
  std::vector<unsigned int> active_maybe_deact_model_index(_num_models);
  for (unsigned model = 0 ; model < _num_models ; ++model)
    if (active_maybe_deact_model[model])
      active_maybe_deact_model_index[model] = ind++;
    else
      active_maybe_deact_model_index[model] = _num_models+1; // just a dummy, that will probably cause a crash if something goes wrong
  ...
  // In the following
  // epp = pm*r - E_inv*(trial_stress - stress) = pm*r - delta_dp
  // f = yield function    [only the "active_surface" surfaces]
  // ic = intnl - intnl_old + pm*h    [only for models with "active_surface" surfaces]
  //
  // Here pm*r = sum_{active_alpha} pm[alpha]*r[alpha].  Note that this contains all the "active" constraints,
  //             even the ones that have been deactivated_due_to_ld.  r is a std::vector containing all the
  //             active flow directions.   The derivative wrt pm that are deactivated_due_to_ld is explicitly
  //             excluded from the jacobian assembly below.  Similarly, derivatives wrt internal parameters
  //             of models with no active_surface surfaces are exlucded.
  // Also yield_function contains only the "active_surface" constraints.  f is a std::vector containing only
  //             these yield functions.  (Note that in most other functions in this Class, f contains ALL
  //             the "active" constraints.)
  // Also pm*h = sum_{active_alpha} pm[alpha]*h[alpha].  Note that this contains all the hardening potentials
  //             and pm for the "active" surfaces.  The derivative wrt pm that are deactivated_due_to_ld is
  //             explicitly exlucded from the jacobian assembly below
  //
  // The degrees of freedom are:
  //   the 6 components of stress
  //   the num_active_surface plastic multipliers, pm
  //   the num_active_model internal parameters, intnl, belonging to models with 'active_surface' surfaces
