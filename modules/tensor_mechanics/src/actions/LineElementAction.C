...
LineElementAction::beamParameters()
{
  ...
  params.addParam<std::vector<VariableName>>("rot_velocities", "Rotational velocity variables");
  params.addParam<std::vector<VariableName>>("rot_accelerations",
  ...
  params.addParam<Real>("nodal_mass", "Mass associated with the node");
  ...
  params.addParam<Real>("nodal_Ixx", "Nodal moment of inertia in the x direction.");
  params.addParam<Real>("nodal_Iyy", "Nodal moment of inertia in the y direction.");
  params.addParam<Real>("nodal_Izz", "Nodal moment of inertia in the z direction.");
    ...
    _truss(getParam<bool>("truss"))
{
  ...
  if (!_truss)
  {
    ...
    if (!isParamValid("youngs_modulus") || !isParamValid("shear_modulus"))
      mooseError("LineElementAction: youngs_modulus and shear_modulus should be provided for beam "
                 "elements.");
        ...
        _dynamic_nodal_rotational_inertia)
    {
      if (!isParamValid("velocities") || !isParamValid("accelerations") ||
          !isParamValid("rot_velocities") || !isParamValid("rot_accelerations"))
        mooseError("LineElementAction: Variable names for translational and rotational velocities "
                   "and accelerations should be provided as input to perform dynamic simulation "
                   "using beam elements using `velocities`, `accelerations`, `rot_velocities` and "
                   "`rot_accelerations`.");

      _velocities = getParam<std::vector<VariableName>>("velocities");
      _accelerations = getParam<std::vector<VariableName>>("accelerations");
      _rot_velocities = getParam<std::vector<VariableName>>("rot_velocities");
      _rot_accelerations = getParam<std::vector<VariableName>>("rot_accelerations");
    ...
    if (_dynamic_consistent_inertia &&
        (!isParamValid("density") && !isParamValid("constant_density")))
  ...
  if (_current_task == "add_variable" && getParam<bool>("add_variables"))
  {
    ...
    for (const auto & action : actions)
    {
      ...
      if (size_after != size_before + added_size)
        mooseError("The block restrictions in the LineElement actions must be "
                   "non-overlapping.");

      if (added_size == 0 && actions.size() > 1)
        mooseError("No LineElement action can be block unrestricted if more than one "
