...
InputParameters validParams<TensorMechanicsActionBase>()
{
  ...
  // Output
  params.addParam<MultiMooseEnum>("generate_output", TensorMechanicsActionBase::outputPropertiesType(), "Add scalar quantity output for stress and/or strain");
    ...
    Action(parameters)
{
  ...
  if (isParamValid("additional_generate_output"))
  {
    ...
    MultiMooseEnum additional_generate_output = getParam<MultiMooseEnum>("additional_generate_output");
  ...
  for (auto & r2a : _ranktwoaux_table)
    for (unsigned int a = 0; a < 3; ++a)
      for (unsigned int b = 0; b < 3; ++b)
        options += (options == "" ? "" : " ") + r2a.first + '_' + _component_table[a] + _component_table[b];
