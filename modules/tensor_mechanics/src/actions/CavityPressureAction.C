...
validParams<CavityPressureAction>()
{
  ...
  params.addDeprecatedParam<NonlinearVariableName>(
      "disp_x", "The x displacement", "Use displacements = 'disp_x disp_y disp_z'");
  params.addDeprecatedParam<NonlinearVariableName>(
      "disp_y", "The y displacement", "Use displacements = 'disp_x disp_y disp_z'");
  params.addDeprecatedParam<NonlinearVariableName>(
      "disp_z", "The z displacement", "Use displacements = 'disp_x disp_y disp_z'");
  ...
  params.addDeprecatedParam<std::vector<AuxVariableName>>(
      ...
      "Use save_in = 'save_in_disp_x save_in_disp_y save_in_disp_z'");
  params.addDeprecatedParam<std::vector<AuxVariableName>>(
      "save_in_disp_y",
...
CavityPressureAction::CavityPressureAction(const InputParameters & params)
  : Action(params), _boundary(getParam<std::vector<BoundaryName>>("boundary"))
{
}
...
CavityPressureAction::act()
{
  ...
  else if (isParamValid("disp_x"))
  {
    ...
    mooseError("The input file should specify a string of displacement names, e.g. 'displacements "
               "= disp_x disp_y disp_z'; these names should match the Variable block names");
