...
DynamicTensorMechanicsAction::act()
{
  ...
  {
    mooseDeprecated("DynamicStressDivergenceTensors has been updated to accept a string of displacement variable names, e.g. displacements = 'disp_x disp_y' in the input file.");
  ...
  // Retain this code 'as is' because DynamicStressDivergenceTensors inherits from Kernel.C
  ...
  InputParameters params = _factory.getValidParams("DynamicStressDivergenceTensors");
  ...
  std::string short_name = "DynamicTensorMechanics";
  ...
  for (unsigned int i = 0; i < dim; ++i)
  {
    ...
    _problem->addKernel("DynamicStressDivergenceTensors", name.str(), params);
  }
