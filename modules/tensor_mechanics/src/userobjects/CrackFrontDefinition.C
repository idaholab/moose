    ...
    if (getParam<UserObjectName>("crack_front_points_provider") == "cut_mesh")
    {
      ...
      if (_direction_method != DIRECTION_METHOD::CURVED_CRACK_FRONT)
        paramError("crack_direction_method",
                   "Using the cutter mesh requires _direction_method == "
                   "DIRECTION_METHOD::CURVED_CRACK_FRONT");
      ...
      if (_treat_as_2d)
        paramError("2d", "Using the cutter mesh requires that the problem is not treated as 2d");
    }
  }
...
CrackFrontDefinition::initialize()
{
  ...
  // This is only useful for 3D growing cracks, which are described by the mesh cutter at this
  // moment
  if (_use_mesh_cutter)
  {
    _crack_front_points =
