...
InputParameters validParams<TensorMechanicsHardeningCutExponential>()
{
  ...
  params.addParam<Real>("rate", 0, "Let p = internal_parameter.  Then value = value_0 for p<internal_0, and otherwise, value = value_residual + (value_0 - value_residual)*exp(-rate*(p - internal_0)");
...
TensorMechanicsHardeningCutExponential::TensorMechanicsHardeningCutExponential(const std::string & deprecated_name, InputParameters parameters) :
