...
InputParameters validParams<LinearViscoelasticityManager>()
{
  ...
  params.addParam<std::string>("creep_strain_name","creep_strain","name of the creep strain tensor used for the viscoelastic update");
  params.addParam<std::string>("elastic_strain_name","elastic_strain","name of the elastic strain tensor used for the viscoelastic update");
...
LinearViscoelasticityManager::execute()
{
  ...
  {
    for (unsigned int _qp = 0; _qp < _qrule->n_points(); ++_qp)
      _viscoelastic_model->updateQpApparentProperties(_qp, _creep_strain[_qp] + _elastic_strain[_qp], _stress[_qp]);
  }
}
