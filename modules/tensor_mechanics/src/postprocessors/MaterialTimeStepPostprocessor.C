...
validParams<MaterialTimeStepPostprocessor>()
{
  ...
  params.addRangeCheckedParam<Real>("tolerance",
                                    "tolerance > 0",
                                    "tolerance used to determine if an element has changed");
...
MaterialTimeStepPostprocessor::MaterialTimeStepPostprocessor(const InputParameters & parameters)
  : ElementPostprocessor(parameters),
    ...
    _matl_time_step(_use_matl_time_step
    ...
    _changed_property(_use_elements_changed
                      ? &getMaterialPropertyByName<Real>(getParam<MaterialPropertyName>("elements_changed_property"))
                      : nullptr),
    _changed_property_old(_use_elements_changed
                          ? &getMaterialPropertyOldByName<Real>(getParam<MaterialPropertyName>("elements_changed_property"))
                          : nullptr),
    _target(isParamValid("elements_changed_number")
           ? getParam<int>("elements_changed_number")
           : 0),
    _count(0),
    _tolerance(parameters.isParamSetByUser("tolerance") ? getParam<Real>("tolerance") : TOLERANCE * TOLERANCE),
    _qp(0)
{
  if (_use_elements_changed && !parameters.isParamSetByUser("elements_changed_number"))
    paramError("elements_changed_number", "needs to be set when elements_changed_property is defined");
...
MaterialTimeStepPostprocessor::execute()
{
  ...
  if (_use_elements_changed)
  {
    for (_qp = 0; _qp < _qrule->n_points(); ++_qp)
    {
      if (!MooseUtils::absoluteFuzzyEqual((*_changed_property)[_qp], (*_changed_property_old)[_qp], _tolerance))
      {
        ++_count;
        return;
      } 
