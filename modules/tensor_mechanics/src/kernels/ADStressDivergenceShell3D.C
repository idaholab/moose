...
#include "ADStressDivergenceShell3D.h"
...
registerMooseObject("TensorMechanicsApp", ADStressDivergenceShell3D);

InputParameters
ADStressDivergenceShell3D::validParams()
{
...
ADStressDivergenceShell3D::ADStressDivergenceShell3D(const InputParameters & parameters)
  : ADKernel(parameters),
    ...
    _large_strain(getParam<bool>("large_strain"))
{
  ...
  for (unsigned int i = 0; i < _t_weights.size(); ++i)
  {
    ...
    _B_mat[i] = &getADMaterialProperty<DenseMatrix<Real>>("t_points_" + std::to_string(i) + "_B");
    if (_large_strain)
      _B_nl[i] =
          &getADMaterialProperty<DenseMatrix<Real>>("t_points_" + std::to_string(i) + "_B_nl");

    _J_map[i] = &getADMaterialProperty<Real>("t_points_" + std::to_string(i) + "_J_mapping");
  }
}

ADReal
ADStressDivergenceShell3D::computeQpResidual()
{
