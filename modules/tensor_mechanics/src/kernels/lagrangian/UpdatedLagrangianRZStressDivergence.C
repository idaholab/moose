...
#include "UpdatedLagrangianRZStressDivergence.h"

registerMooseObject("TensorMechanicsApp", UpdatedLagrangianRZStressDivergence);

InputParameters
UpdatedLagrangianRZStressDivergence::validParams()
{
  InputParameters params = UpdatedLagrangianStressDivergence::validParams();

  params.addClassDescription("Updated Lagrangian stress equilibrium kernel in RZ coordinates");

  return params;
}

UpdatedLagrangianRZStressDivergence::UpdatedLagrangianRZStressDivergence(
    const InputParameters & parameters)
  : UpdatedLagrangianStressDivergence(parameters)
{
...
void
UpdatedLagrangianRZStressDivergence::initialSetup()
{
...
RankTwoTensor
UpdatedLagrangianRZStressDivergence::testGrad(unsigned int i)
{
  // R
  if (i == 0)
  {
...
RankTwoTensor
UpdatedLagrangianRZStressDivergence::trialGrad(unsigned int k, bool stabilize)
{
  // TODO: add stabilization
  if (stabilize)
    mooseError("Stabilization not yet implemented");
