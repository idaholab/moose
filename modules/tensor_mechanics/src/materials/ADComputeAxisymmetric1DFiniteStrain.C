...
#include "ADComputeAxisymmetric1DFiniteStrain.h"

registerADMooseObject("TensorMechanicsApp", ADComputeAxisymmetric1DFiniteStrain);

defineADValidParams(ADComputeAxisymmetric1DFiniteStrain,
...
ADComputeAxisymmetric1DFiniteStrain<compute_stage>::ADComputeAxisymmetric1DFiniteStrain(
    const InputParameters & parameters)
  : ADCompute1DFiniteStrain<compute_stage>(parameters),
...
template <ComputeStage compute_stage>
void
ADComputeAxisymmetric1DFiniteStrain<compute_stage>::initialSetup()
{
...
template <ComputeStage compute_stage>
ADReal
ADComputeAxisymmetric1DFiniteStrain<compute_stage>::computeGradDispYY()
{
  if (_has_scalar_out_of_plane_strain)
...
template <ComputeStage compute_stage>
Real
ADComputeAxisymmetric1DFiniteStrain<compute_stage>::computeGradDispYYOld()
{
  if (_has_scalar_out_of_plane_strain)
...
template <ComputeStage compute_stage>
ADReal
ADComputeAxisymmetric1DFiniteStrain<compute_stage>::computeGradDispZZ()
{
...
template <ComputeStage compute_stage>
Real
ADComputeAxisymmetric1DFiniteStrain<compute_stage>::computeGradDispZZOld()
{
  if (!MooseUtils::absoluteFuzzyEqual(_q_point[_qp](0), 0.0))
