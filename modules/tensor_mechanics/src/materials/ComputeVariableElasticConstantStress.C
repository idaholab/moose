...
ComputeVariableElasticConstantStress::ComputeVariableElasticConstantStress(const InputParameters & parameters) :
...
ComputeVariableElasticConstantStress::computeQpStress()
{
  ...
  // Once we introduce aniostropy into these models this check will need to be expanded/ coded more craftily
  if (!MooseUtils::relativeFuzzyEqual(_elasticity_tensor_old[_qp](0,0,0,0), _elasticity_tensor[_qp](0,0,0,0)) &&
      !MooseUtils::relativeFuzzyEqual(_elasticity_tensor_old[_qp](0,0,0,0), 0.0))
  {
    // Recover the old strain state by multiplying the old stress by the inverse old elasticity tensor
    RankFourTensor old_Cijkl_inverse = _elasticity_tensor_old[_qp].invSymm();
    RankTwoTensor intermediate_strain = old_Cijkl_inverse * _stress_old[_qp] + _strain_increment[_qp];
