...
//  params.addParam<std::string>("base_name", "Optional parameter that allows the user to define multiple mechanics material systems on the same block");
...
RecomputeRadialReturnStressIncrement::RecomputeRadialReturnStressIncrement(const InputParameters & parameters) :
  ...
  //  _return_stress_increment(declareProperty<RankTwoTensor>("radial_return_stress")),
...
RecomputeRadialReturnStressIncrement::computeStressIncrement()
{
     ...
     (norm_residual/first_norm_residual) > _relative_tolerance)
  {
    ...
    mooseError("Exceeded maximum iterations in DiscreteRadialReturn solve for material: " << _name << ".  Rerun with  'output_iteration_info_on_error = true' for more information.");
  }
