...
InputParameters validParams<InclusionProperties>()
{
  ...
  params.addRequiredParam<MaterialPropertyName>("strain_name", "Name of the material property where analytical total strains will be stored");
...
InclusionProperties::computeQpProperties()
{
  ...
  else
  {
    ...
    Real H41 = n_x * n_y * (1 - 3 * rho_a*rho_a
            + (6 * rho_a*rho_a + 2 * rho_b*rho_b + T_6) * n_x*n_x);
    Real H42 = n_x * n_y * (1 - 3 * rho_b*rho_b
            + (2 * rho_a*rho_a + 6 * rho_b*rho_b + T_6) * n_y*n_y);
    ...
    _stress[_qp](0,1) = _stress[_qp](1,0) = 4 * _mu * rho_a * rho_b / (_kappa + 1)
    ...
    Real G1211 = rho_a * rho_b * n_x*n_y / (2 * (1 - _nu))
                * (1 - 3 * rho_a*rho_a + (6 * rho_a*rho_a + 2 * rho_b*rho_b + T_6) * n_x*n_x);
    Real G1222 = rho_a * rho_b * n_x*n_y / (2 * (1 - _nu))
...
InclusionProperties::precomputeInteriorProperties()
{
  ...
  _total_strain_int(0,0) = 1 / (_kappa + 1) / (1 + t) / (1 + t)
                  * (S11 * _misfit[0] + S12 * _misfit[1]);
  _total_strain_int(1,1) = 1 / (_kappa + 1) / (1 + t) / (1 + t)
