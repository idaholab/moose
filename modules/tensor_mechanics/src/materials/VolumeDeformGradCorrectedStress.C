...
InputParameters validParams<VolumeDeformGradCorrectedStress>()
{
  ...
  params.addClassDescription("Transforms stress with volumetric term from previous configuration to this configuration");
  params.addRequiredParam<MaterialPropertyName>("pre_stress_name", "Name of stress variable from previous config.");
  params.addRequiredParam<MaterialPropertyName>("deform_grad_name", "Name of deformation gradient variable");
  params.addParam<MaterialPropertyName>("pre_jacobian_name", "Name of jacobian variable from previous config.");
...
{
  _stress[_qp] = _deformation_gradient[_qp] * _pre_stress[_qp] * _deformation_gradient[_qp].transpose()/_deformation_gradient[_qp].det();
