...
#include "ComputeEigenstrainTrussFromInitialStress.h"
#include "RankTwoTensor.h"
#include "Function.h"
#include "Conversion.h" // for stringify

registerMooseObject("TensorMechanicsApp", ComputeEigenstrainTrussFromInitialStress);

InputParameters
ComputeEigenstrainTrussFromInitialStress::validParams()
{
  ...
  params.addRequiredParam<Real>(
      "initial_stress",
      "Initial axial stress. To compute the eigenstrain correctly, your elasticity tensor should not be time-varying in the first timestep");
  // params.addCoupledVar("initial_stress_aux",
                       // "A list of 9 AuxVariables describing the initial stress.  If provided, each of these is multiplied by its corresponding initial_stress function to obtain the relevant component of initial stress.");
...
ComputeEigenstrainTrussFromInitialStress::ComputeEigenstrainTrussFromInitialStress(
    const InputParameters & parameters)
  : ComputeEigenstrainTrussBase(parameters),
    ...
    _ini_aux_provided(isParamValid("initial_stress_aux"))
{
  ...
  if (num != LIBMESH_DIM * LIBMESH_DIM)
    paramError(
        "initial_stress",
        "ComputeEigenstrainTrussFromInitialStress: " + Moose::stringify(LIBMESH_DIM * LIBMESH_DIM) +
  ...
  if (_ini_aux_provided)
  {
    ...
    if (aux_size != LIBMESH_DIM * LIBMESH_DIM)
      paramError("initial_stress_aux",
                 "ComputeEigenstrainTrussFromInitialStress: If you supply initial_stress_aux, " +
...
void
ComputeEigenstrainTrussFromInitialStress::computeQpEigenstrain()
{
  if (_t_step == 1)
  {
    Real initial_stress = _initial_stress_fcn->value(_t, _q_point[_qp]);
    _eigenstrain[_qp] = -_material_stiffness[_qp] * initial_stress;
