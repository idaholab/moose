...
InputParameters validParams<RecomputeRadialReturnIsotropicPowerLawHardening>()
{
  ...
  params.addRequiredRangeCheckedParam<Real>("strain_hardening_exponent", "strain_hardening_exponent>=0.0 & strain_hardening_exponent <=1.0", "The strain hardening exponent (n) for power law hardening");

  return params;
}


RecomputeRadialReturnIsotropicPowerLawHardening::RecomputeRadialReturnIsotropicPowerLawHardening(const InputParameters & parameters) :
...
void
RecomputeRadialReturnIsotropicPowerLawHardening::computeStressInitialize(Real effectiveTrialStress)
{
...
Real
RecomputeRadialReturnIsotropicPowerLawHardening::computeHardeningDerivative(Real scalar)
{
  const Real stress_delta = _effective_trial_stress - 3.0 * _shear_modulus * scalar;
  Real slope = std::pow(stress_delta, (1.0 / _strain_hardening_exponent - 1.0)) / _strain_hardening_exponent * 1.0 / std::pow(_K, 1.0 / _strain_hardening_exponent);
...
void
RecomputeRadialReturnIsotropicPowerLawHardening::computeYieldStress()
{
...
Real
RecomputeRadialReturnIsotropicPowerLawHardening::getIsotropicLameLambda()
{
