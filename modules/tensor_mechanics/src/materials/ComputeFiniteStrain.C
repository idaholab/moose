...
InputParameters validParams<ComputeFiniteStrain>()
{
  ...
  MooseEnum decomposition_type("Rashid Eigen", "Rashid");
...
{
  if (_decomposition_method == "Rashid")
  {
    // inverse of _Fhat
  ...
  else if (_decomposition_method == "Eigen")
  {
    ...
    const Real loglambda1 = std::log(lambda1);
    const Real loglambda2 = std::log(lambda2);
    const Real loglambda3 = std::log(lambda3);
    ...
    total_strain_increment = N1 * loglambda1 + N2 * loglambda2 + N3 * loglambda3;
  }
}
