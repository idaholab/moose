...
template <>
InputParameters
validParams<TwoParameterPlasticity>()
{
...
TwoParameterPlasticity::TwoParameterPlasticity(const InputParameters & parameters,
                                               unsigned num_yf,
                                               unsigned num_intnl)
...
void
TwoParameterPlasticity::initQpStatefulProperties()
{
...
void
TwoParameterPlasticity::updateState(RankTwoTensor & strain_increment,
                                    ...
                                    RankFourTensor & tangent_operator)
{
      ...
      while (res2 > _f_tol2 && step_iter < (float)_max_nr_its && nr_failure == 0 && ls_failure == 0)
      {
            ...
            std::abs(_rhs[1]) <= 1E-13 * std::abs(p) && std::abs(_rhs[2]) <= 1E-13 * std::abs(q))
        {
          if (_warn_about_precision_loss)
            Moose::err << "TwoParameterPlasticity: precision-loss in element "
  ...
  if (step_size < _min_step_size)
    errorHandler("TwoParameterPlasticity: Minimum step-size violated");

  // success!
  finaliseReturnProcess(rotation_increment);
...
void
TwoParameterPlasticity::dVardTrial(bool elastic_only,
                                   Real p_trial,
                                   Real q_trial,
                                   Real p,
                                   Real q,
                                   ...
                                   bool compute_full_tangent_operator)
{
  ...
  LAPACKgesv_(&gesv_num_rhs,
              ...
              &info);
  if (info != 0)
    errorHandler("TwoParameterPlasticity: PETSC LAPACK gsev routine returned with error code " +
...
TwoParameterPlasticity::f_and_derivs
TwoParameterPlasticity::smoothAllQuantities(Real p, Real q, const std::vector<Real> & intnl)
{
...
Real
TwoParameterPlasticity::ismoother(Real f_diff) const
{
  mooseAssert(f_diff <= 0.0, "TwoParameterPlasticity: ismoother called with positive argument");
...
Real
TwoParameterPlasticity::smoother(Real f_diff) const
{
  if (f_diff >= _smoothing_tol)
    return 1.0;
...
Real
TwoParameterPlasticity::dsmoother(Real f_diff) const
{
  if (f_diff >= _smoothing_tol)
    return 0.0;
  else if (f_diff <= -_smoothing_tol)
...
int
TwoParameterPlasticity::lineSearch(Real & res2,
                                   Real & gaE,
                                   Real & p,
                                   Real & q,
                                   Real p_trial,
...
void
TwoParameterPlasticity::dnRHSdVar(const f_and_derivs & smoothed_q,
...
Real
TwoParameterPlasticity::calculateRHS(
...
void
TwoParameterPlasticity::errorHandler(const std::string & message)
{
  throw MooseException(message);
}

void
TwoParameterPlasticity::initialiseReturnProcess()
{
  return;
}

void
TwoParameterPlasticity::finaliseReturnProcess(const RankTwoTensor & /*rotation_increment*/)
{
  return;
}

void
TwoParameterPlasticity::preReturnMap(Real /*p_trial*/,
                                     Real /*q_trial*/,
...
Real
TwoParameterPlasticity::yieldF(Real p, Real q, const std::vector<Real> & intnl) const
{
...
void
TwoParameterPlasticity::initialiseVars(Real p_trial,
                                       Real q_trial,
                                       const std::vector<Real> & intnl_old,
                                       Real & p,
                                       Real & q,
...
void
TwoParameterPlasticity::consistentTangentOperator(const RankTwoTensor & stress_trial,
                                                  Real /*p_trial*/,
                                                  Real /*q_trial*/,
...
void
TwoParameterPlasticity::setStressAfterReturn(const RankTwoTensor & stress_trial,
                                             Real /*p_ok*/,
                                             Real /*q_ok*/,
                                             Real gaE,
                                             const std::vector<Real> & /*intnl*/,
