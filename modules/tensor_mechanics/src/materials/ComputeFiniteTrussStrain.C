...
#include "ComputeFiniteTrussStrain.h"
#include "Assembly.h"
...
registerMooseObject("TensorMechanicsApp", ComputeFiniteTrussStrain);

InputParameters
ComputeFiniteTrussStrain::validParams()
{
  InputParameters params = ComputeIncrementalTrussStrain::validParams();
  params.addClassDescription("Compute a rotation increment for finite rotations of the truss and "
                             "computes the small/large strain increments in the current rotated "
                             "configuration of the truss.");
  return params;
}

ComputeFiniteTrussStrain::ComputeFiniteTrussStrain(const InputParameters & parameters)
  : ComputeIncrementalTrussStrain(parameters)
    // _total_rotation_old(getMaterialPropertyOld<RankTwoTensor>("total_rotation"))
{
}
/*
void
ComputeFiniteTrussStrain::computeRotation()
{
  // First - convert the differential truss displacement to the truss configuration at previous time
  // step
  ...
  // Convert average rotational displacement to the truss configuration at previous time step
  ...
  // Total rotation matrix from global configuration to truss local config at current time
