...
ComputeCappedWeakPlaneCosseratStress::ComputeCappedWeakPlaneCosseratStress(const InputParameters & parameters) :
    ComputeCappedWeakPlaneStress(parameters),
    _curvature(getMaterialPropertyByName<RankTwoTensor>("curvature")),
    _elastic_flexural_rigidity_tensor(getMaterialPropertyByName<RankFourTensor>("elastic_flexural_rigidity_tensor")),
...
void
ComputeCappedWeakPlaneCosseratStress::setStressAfterReturn(const RankTwoTensor & stress_trial, Real p_ok, Real q_ok, Real gaE, const std::vector<Real> & /*intnl*/, const f_and_derivs & smoothed_q, RankTwoTensor & stress) const
{
  stress = stress_trial;
  stress(2, 2) = p_ok;
...
void
ComputeCappedWeakPlaneCosseratStress::consistentTangentOperator(const RankTwoTensor & /*stress_trial*/, Real /*p_trial*/, Real q_trial, const RankTwoTensor & /*stress*/, Real /*p*/, Real q, Real gaE, const f_and_derivs & smoothed_q, RankFourTensor & cto) const
{
  if (!_fe_problem.currentlyComputingJacobian())
    return;
  ...
  const Real dintnl1_dq = tanpsi / Exzxz - (q_trial - q) * _tan_psi.derivative(_intnl[_qp][0]) * dintnl0_dq / Exzxz;
  const Real dintnl1_dqt = - tanpsi / Exzxz - (q_trial - q) * _tan_psi.derivative(_intnl[_qp][0]) * dintnl0_dqt / Exzxz;
  ...
  const Real dqt_dep02 = (q_trial == 0.0 ? 1.0 : _in_trial02 / q_trial) * _elasticity_tensor[_qp](0, 2, 0, 2);
  ...
  if (q_trial > 0.0)
  {
    ...
    cto(0, 2, 0, 2) = _elasticity_tensor[_qp](0, 2, 0, 2) * q / q_trial + _in_trial02 * (_dq_dqt - q / q_trial) / q_trial * dqt_dep02;
    cto(1, 2, 0, 2) = _in_trial12 * (_dq_dqt - q / q_trial) / q_trial * dqt_dep02;
  }

  const Real dqt_dep20 = (q_trial == 0.0 ? 1.0 : _in_trial02 / q_trial) * _elasticity_tensor[_qp](0, 2, 2, 0);
  cto(2, 2, 2, 0) = _dp_dqt * dqt_dep20;
  cto(0, 0, 2, 0) = cto(1, 1, 2, 0) =  poisson_effect* dqt_dep20;
  if (q_trial > 0.0)
  {
    // for q_trial=0, Jacobian_mult is just given by the elastic case
    cto(0, 2, 2, 0) = _elasticity_tensor[_qp](0, 2, 2, 0) * q / q_trial + _in_trial02 * (_dq_dqt - q / q_trial) / q_trial * dqt_dep20;
    cto(1, 2, 2, 0) = _in_trial12 * (_dq_dqt - q / q_trial) / q_trial * dqt_dep20;
  }

  const Real dqt_dep12 = (q_trial == 0.0 ? 1.0 : _in_trial12 / q_trial) * _elasticity_tensor[_qp](1, 2, 1, 2);
  ...
  if (q_trial > 0.0)
  {
    ...
    cto(1, 2, 1, 2) = _elasticity_tensor[_qp](1, 2, 1, 2) * q / q_trial + _in_trial12 * (_dq_dqt - q / q_trial) / q_trial * dqt_dep12;
  }

  const Real dqt_dep21 = (q_trial == 0.0 ? 1.0 : _in_trial12 / q_trial) * _elasticity_tensor[_qp](1, 2, 2, 1);
  ...
  if (q_trial > 0.0)
  {
    ...
    cto(1, 2, 2, 1) = _elasticity_tensor[_qp](1, 2, 2, 1) * q / q_trial + _in_trial12 * (_dq_dqt - q / q_trial) / q_trial * dqt_dep21;
  }
}
