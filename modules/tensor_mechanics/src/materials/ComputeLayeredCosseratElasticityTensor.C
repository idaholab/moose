...
InputParameters validParams<ComputeLayeredCosseratElasticityTensor>()
{
  ...
  params.addClassDescription("Computes Cosserat elasticity and flexural bending rigidity tensors relevant for simulations with layered materials.  The layering direction is assumed to be perpendicular to the 'z' direction.");
  params.addRequiredParam<Real>("young", "The Young's modulus");
  params.addRequiredParam<Real>("poisson", "The Poisson's ratio");
  params.addRequiredRangeCheckedParam<Real>("layer_thickness", "layer_thickness>=0", "The layer thickness");
  params.addRequiredRangeCheckedParam<Real>("joint_normal_stiffness", "joint_normal_stiffness>=0", "The joint normal stiffness");
  params.addRequiredRangeCheckedParam<Real>("joint_shear_stiffness", "joint_shear_stiffness>=0", "The joint shear stiffness");
  return params;
}
