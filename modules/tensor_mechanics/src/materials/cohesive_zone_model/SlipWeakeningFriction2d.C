...
SlipWeakeningFriction2d::computeInterfaceTractionAndDerivatives()
{
  ...
  Real T2 =
      -(1 / _dt) * M * (displacement_jump_rate(0) + (1 / _dt) * displacement_jump(0)) / (2 * area) +
      ((R_minus_local_y - R_plus_local_y) / (2 * area)) - T2_o;
  ...
  {
    tau_f = (mu_s - (mu_s - mu_d) * std::abs(displacement_jump(1)) / Dc) *
