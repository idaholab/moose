...
ComputeFiniteBeamStrain::computeRotation()
{
  ...
  // Fetch the solution for the two end nodes at time current time step and previous time step
  ...
  Real intermediate_length_1 =
      std::sqrt(Utility::pow<2>(_original_length[0] + delta_disp_local(0)) +
                Utility::pow<2>(delta_disp_local(2)));
  Real cos_alpha = (_original_length[0] + delta_disp_local(0)) / intermediate_length_1;
