...
#include "ADComputeFiniteShellStrain3D.h"
...
registerMooseObject("TensorMechanicsApp", ADComputeFiniteShellStrain3D);

InputParameters
ADComputeFiniteShellStrain3D::validParams()
{
...
ADComputeFiniteShellStrain3D::ADComputeFiniteShellStrain3D(const InputParameters & parameters)
  : ADComputeIncrementalShellStrain(parameters), _B_nl()
{
  _B_nl.resize(_t_points.size());

  for (unsigned int i = 0; i < _t_points.size(); ++i)
    _B_nl[i] = &declareADProperty<DenseMatrix<Real>>("t_points_" + std::to_string(i) + "_B_nl");
}

void
ADComputeFiniteShellStrain3D::initQpStatefulProperties()
{
  computeGMatrix();
...
void
ADComputeFiniteShellStrain3D::computeProperties()
{
...
void
ADComputeFiniteShellStrain3D::computeNodeNormal()
{
  // update _node_normal
  for (unsigned int k = 0; k < _nodes.size(); ++k)
  {
...
void
ADComputeFiniteShellStrain3D::updatedxyz()
{
  for (unsigned int i = 0; i < _2d_points.size(); ++i)
  {
    for (unsigned int j = 0; j < _t_points.size(); ++j)
...
void
ADComputeFiniteShellStrain3D::updateGVectors()
{
  for (unsigned int component = 0; component < 3; ++component)
  {
    _g1_a(component) +=
...
void
ADComputeFiniteShellStrain3D::computeBNLMatrix()
{
