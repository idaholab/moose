...
/****************************************************************/
#include "ComputeLinearElasticPFFracStress.h"
#include "libmesh/utility.h"

template<>
InputParameters validParams<ComputeLinearElasticPFFracStress>()
{
...
ComputeLinearElasticPFFracStress::ComputeLinearElasticPFFracStress(const InputParameters & parameters) :
...
void ComputeLinearElasticPFFracStress::computeQpStress()
{
  Real c = _c[_qp];
  ...
  RankTwoTensor Ipos, Ineg, eigval_tensor, eye;
  Ineg.zero();
  eye.zero();
  ...
  RankFourTensor IpoIp = Ipos.mixedProductIkJl(eye);
  RankFourTensor InoIn = Ineg.mixedProductIkJl(eye);

  RankFourTensor Jpos = _Jacobian_mult[_qp] * QoQ * IpoIp * QToQT;
  RankFourTensor Jneg = _Jacobian_mult[_qp] * QoQ * InoIn * QToQT;
