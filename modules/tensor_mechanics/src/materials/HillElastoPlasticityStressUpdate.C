...
HillElastoPlasticityStressUpdateTempl<is_ad>::validParams()
{
  ...
  params.addClassDescription(
      "This class uses the stress update material in a radial return plasticity"
    ...
    const GenericReal<is_ad> & delta_gamma)
{
  ...
  GenericReal<is_ad> s_y = _hardening_slope * _hardening_variable[_qp] + _yield_stress;
    ...
    const GenericReal<is_ad> & delta_gamma)
{
  ...
  GenericReal<is_ad> sy =
      _hardening_slope * computeHardeningValue(delta_gamma, omega) + _yield_stress;
  GenericReal<is_ad> sy_alpha = _hardening_slope;
