  ...
  _youngs_modulus( _youngs_modulus_set ? getParam<Real>("youngs_modulus") : -1 )
{
  ...
  {
    isotropic_constants[0] = _lambda;
    isotropic_constants[1] = _shear_modulus;
  }
  else if (_youngs_modulus_set && _poissons_ratio_set)
  {
    isotropic_constants[0] = _youngs_modulus*_poissons_ratio/((1 + _poissons_ratio)*(1 - 2*_poissons_ratio));
    isotropic_constants[1] = _youngs_modulus/(2*(1 + _poissons_ratio));
  }
  else if (_shear_modulus && _bulk_modulus)
  {
    isotropic_constants[0] = _bulk_modulus - 2/3*_shear_modulus;
    isotropic_constants[1] = _shear_modulus;
  }
  else
  ...
  _Cijkl.fillFromInputVector(isotropic_constants, RankFourTensor::symmetric_isotropic);
}
