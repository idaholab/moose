...
InputParameters validParams<ComputeCappedDruckerPragerStress>()
{
  ...
  params.addRequiredParam<UserObjectName>("DP_model", "A TensorMechanicsPlasticDruckerPrager UserObject that defines the Drucker-Prager parameters (cohesion, friction angle and dilation angle)");
  params.addRequiredParam<UserObjectName>("tensile_strength", "A TensorMechanicsHardening UserObject that defines hardening of the tensile strength.  In physical situations this is positive (and always must be greater than negative compressive-strength.");
  params.addRequiredParam<UserObjectName>("compressive_strength", "A TensorMechanicsHardening UserObject that defines hardening of the compressive strength.  In physical situations this is positive.");
  params.addRequiredRangeCheckedParam<Real>("tip_smoother", "tip_smoother>=0", "The cone vertex at J2 = 0 will be smoothed by the given amount.  Typical value is 0.1*cohesion");
  ...
  params.addParam<bool>("small_dilation", true, "If true, and if the trial stress exceeds the tensile strength, then the user gaurantees that the returned stress will be independent of the compressive strength.");
  return params;
}

ComputeCappedDruckerPragerStress::ComputeCappedDruckerPragerStress(const InputParameters & parameters) :
...
void
ComputeCappedDruckerPragerStress::initialiseReturnProcess()
{
  _stress_return_type = StressReturnType::nothing_special;
}
...
void
ComputeCappedDruckerPragerStress::preReturnMap(Real /*p_trial*/, Real q_trial, const RankTwoTensor & /*stress_trial*/, const std::vector<Real> & /*intnl_old*/, const std::vector<Real> & yf)
{
...
void
ComputeCappedDruckerPragerStress::computePQ(const RankTwoTensor & stress, Real & p, Real & q) const
{
  p = stress.trace();
...
void
ComputeCappedDruckerPragerStress::setEppEqq(const RankFourTensor & Eijkl, Real & Epp, Real & Eqq) const
{
  Epp = Eijkl.sum3x3();
  Eqq = Eijkl(0, 1, 0, 1);
}
...
void
ComputeCappedDruckerPragerStress::setIntnlDerivatives(Real /*p_trial*/, Real q_trial, Real /*p*/, Real q, const std::vector<Real> & intnl, std::vector<std::vector<Real> > & dintnl) const
{
  Real tanpsi;
...
void
ComputeCappedDruckerPragerStress::yieldFunctionValues(Real p, Real q, const std::vector<Real> & intnl, std::vector<Real> & yf) const
{
  Real aaa;
  Real bbb;
...
void
ComputeCappedDruckerPragerStress::computeAllQ(Real p, Real q, const std::vector<Real> & intnl, std::vector<f_and_derivs> & all_q) const
{
  Real aaa;
  Real bbb;
  _dp.bothAB(intnl[0], aaa, bbb);
...
void
ComputeCappedDruckerPragerStress::initialiseVars(Real p_trial, Real q_trial, const std::vector<Real> & intnl_old, Real & p, Real & q, Real & gaE, std::vector<Real> & intnl) const
{
  if (!_perfect_guess)
  {
    p = p_trial;
...
void
ComputeCappedDruckerPragerStress::setIntnlValues(Real p_trial, Real q_trial, Real p, Real q, const std::vector<Real> & intnl_old, std::vector<Real> & intnl) const
{
...
void
ComputeCappedDruckerPragerStress::setStressAfterReturn(const RankTwoTensor & stress_trial, Real p_ok, Real q_ok, Real /*gaE*/, const std::vector<Real> & /*intnl*/, const f_and_derivs & /*smoothed_q*/, RankTwoTensor & stress) const
{
  // stress = s_ij + de_ij tr(stress) / 3 = q / q_trial * s_ij^trial + de_ij p / 3 = q / q_trial * (stress_ij^trial - de_ij tr(stress^trial) / 3) + de_ij p / 3
  const Real p_trial = stress_trial.trace();
  stress = RankTwoTensor(RankTwoTensor::initIdentity) / 3.0 * (p_ok - (_in_q_trial == 0.0 ? 0.0 : p_trial * q_ok / _in_q_trial));
  if (_in_q_trial > 0)
...
RankTwoTensor
ComputeCappedDruckerPragerStress::dpdstress(const RankTwoTensor & stress) const
{
  return stress.dtrace();
}

RankFourTensor
ComputeCappedDruckerPragerStress::d2pdstress2(const RankTwoTensor & /*stress*/) const
{
  return RankFourTensor();
}

RankTwoTensor
ComputeCappedDruckerPragerStress::dqdstress(const RankTwoTensor & stress) const
{
...
RankFourTensor
ComputeCappedDruckerPragerStress::d2qdstress2(const RankTwoTensor & stress) const
{
  const Real j2 = stress.secondInvariant();
  if (j2 == 0.0)
    return RankFourTensor();
  ...
  return - 0.25 * dj2.outerProduct(dj2) / std::pow(j2, 1.5) + 0.5 * stress.d2secondInvariant() / std::sqrt(j2);
}


void
ComputeCappedDruckerPragerStress::consistentTangentOperator(const RankTwoTensor & /*stress_trial*/, Real /*p_trial*/, Real /*q_trial*/, const RankTwoTensor & stress, Real /*p*/, Real q, Real gaE, const f_and_derivs & smoothed_q, RankFourTensor & cto) const
{
  ...
  const RankTwoTensor s_over_q = (q == 0.0 ? RankTwoTensor() : (stress - stress.trace() * RankTwoTensor(RankTwoTensor::initIdentity) / 3.0) / q);
  ...
  for (unsigned i = 0; i < _tensor_dimensionality; ++i)
    for (unsigned j = 0; j < _tensor_dimensionality; ++j)
      for (unsigned k = 0; k < _tensor_dimensionality; ++k)
        ...
        {
          cto(i, j, k, l) -= (i == j) * (1.0 / 3.0) * (_Epp * (1.0 - _dp_dpt) / 3.0 * (k == l) + s_over_q(k, l) * _Eqq * (- _dp_dqt));
	  cto(i, j, k, l) -= s_over_q(i, j) * (_Epp * (- _dq_dpt) / 3.0 * (k == l) + s_over_q(k, l) * _Eqq * (1.0 - _dq_dqt));
	}

  if (smoothed_q.dg[1] != 0.0)
  {
    const RankFourTensor Tijab = _elasticity_tensor[_qp] * (gaE / _Epp) * smoothed_q.dg[1] * d2qdstress2(stress);
