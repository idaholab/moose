...
#include "ADComputeIsotropicElasticityTensorShell3D.h"
...
registerMooseObject("TensorMechanicsApp", ADComputeIsotropicElasticityTensorShell3D);

InputParameters
ADComputeIsotropicElasticityTensorShell3D::validParams()
{
...
ADComputeIsotropicElasticityTensorShell3D::ADComputeIsotropicElasticityTensorShell3D(
    ...
    _youngs_modulus(getParam<Real>("youngs_modulus"))
{
  ...
  for (unsigned int t = 0; t < _t_points.size(); ++t)
  {
    _elasticity_tensor[t] =
        &declareADProperty<RankFourTensor>("t_points_" + std::to_string(t) + "_elasticity_tensor");
    _ge[t] = &getADMaterialProperty<RankTwoTensor>("t_points_" + std::to_string(t) + "_ge");
  }
}

void
ADComputeIsotropicElasticityTensorShell3D::computeQpProperties()
{
  for (unsigned int t = 0; t < _t_points.size(); ++t)
  {
