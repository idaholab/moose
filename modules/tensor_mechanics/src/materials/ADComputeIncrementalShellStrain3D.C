...
#include "ADComputeIncrementalShellStrain3D.h"
#include "MooseMesh.h"
#include "Assembly.h"
...
registerMooseObject("TensorMechanicsApp", ADComputeIncrementalShellStrain3D);

InputParameters
ADComputeIncrementalShellStrain3D::validParams()
{
...
ADComputeIncrementalShellStrain3D::ADComputeIncrementalShellStrain3D(
    ...
    _sol_old(_nonlinear_sys.solutionOld())
{
  ...
  if (_ndisp != 3 || _nrot != 2)
    mooseError(
        "ADComputeIncrementalShellStrain3D: The number of variables supplied in 'displacements' "
        "must be 3 and that in 'rotations' must be 2.");

  if (_mesh.hasSecondOrderElements())
    mooseError("ADComputeIncrementalShellStrain3D: Shell element is implemented only for linear "
               "elements.");
  ...
  _mechanical_strain_old.resize(_t_points.size());
  ...
  {
    _strain_increment[i] =
        &declareADProperty<RankTwoTensor>("t_points_" + std::to_string(i) + "_strain_increment");
    _mechanical_strain[i] =
        &declareADProperty<RankTwoTensor>("t_points_" + std::to_string(i) + "_mechanical_strain");
    _mechanical_strain_old[i] = &getMaterialPropertyOldByName<RankTwoTensor>(
        "t_points_" + std::to_string(i) + "_mechanical_strain");
    _total_strain[i] =
        &declareADProperty<RankTwoTensor>("t_points_" + std::to_string(i) + "_total_strain");
    _total_strain_old[i] = &getMaterialPropertyOldByName<RankTwoTensor>(
        "t_points_" + std::to_string(i) + "_total_strain");
    _B[i] = &declareADProperty<DenseMatrix<Real>>("t_points_" + std::to_string(i) + "_B");
    _B_old[i] =
        &getMaterialPropertyOldByName<DenseMatrix<Real>>("t_points_" + std::to_string(i) + "_B");
    _ge[i] = &declareADProperty<RankTwoTensor>("t_points_" + std::to_string(i) + "_ge");
    _ge_old[i] =
        &getMaterialPropertyOldByName<RankTwoTensor>("t_points_" + std::to_string(i) + "_ge");
    _J_map[i] = &declareADProperty<Real>("t_points_" + std::to_string(i) + "_J_mapping");
    _J_map_old[i] =
        &getMaterialPropertyOldByName<Real>("t_points_" + std::to_string(i) + "_J_mapping");
    _dxyz_dxi[i] =
        &declareADProperty<RealVectorValue>("t_points_" + std::to_string(i) + "_dxyz_dxi");
    _dxyz_dxi_old[i] = &getMaterialPropertyOldByName<RealVectorValue>(
        "t_points_" + std::to_string(i) + "_dxyz_dxi");
    _dxyz_deta[i] =
        &declareADProperty<RealVectorValue>("t_points_" + std::to_string(i) + "_dxyz_deta");
    _dxyz_deta_old[i] = &getMaterialPropertyOldByName<RealVectorValue>(
        "t_points_" + std::to_string(i) + "_dxyz_deta");
    _dxyz_dzeta[i] =
        &declareADProperty<RealVectorValue>("t_points_" + std::to_string(i) + "_dxyz_dzeta");
    _dxyz_dzeta_old[i] = &getMaterialPropertyOldByName<RealVectorValue>(
        "t_points_" + std::to_string(i) + "_dxyz_dzeta");
    // Create rotation matrix and total strain global for output purposes only
    _rotation_matrix[i] =
        &declareADProperty<RankTwoTensor>("t_points_" + std::to_string(i) + "_rotation_increment");
    // _rotation_matrix[i] = &declareProperty<RankTwoTensor>("t_points_" + std::to_string(i) +
    // "_rotation");
    _total_global_strain[i] =
        &declareADProperty<RankTwoTensor>("t_points_" + std::to_string(i) + "_total_global_strain");
  }
...
void
ADComputeIncrementalShellStrain3D::initQpStatefulProperties()
{
  unsigned int dim = _current_elem->dim();
  if ((dim != 2))
    mooseError(
        "ADComputeIncrementalShellStrain3D: Shell element is implemented only for 2D elements");
  if (_current_elem->n_nodes() != 4)
    mooseError(
        "ADComputeIncrementalShellStrain3D: Shell element needs to have exactly four nodes.");
  computeGMatrix();
  computeBMatrix();
}

void
ADComputeIncrementalShellStrain3D::computeProperties()
{
  ...
  for (unsigned int i = 0; i < _2d_points.size(); ++i)
  {
    for (unsigned int j = 0; j < _t_points.size(); ++j)
    {
      ...
      (*_mechanical_strain[j])[i] = (*_mechanical_strain_old[j])[i] + (*_strain_increment[j])[i];
      (*_total_strain[j])[i] = (*_total_strain_old[j])[i] + (*_strain_increment[j])[i];

      for (unsigned int ii = 0; ii < 3; ++ii)
        for (unsigned int jj = 0; jj < 3; ++jj)
        {
          _unrotated_mechanical_strain(ii, jj) =
              MetaPhysicL::raw_value((*_mechanical_strain[j])[i](ii, jj));
          _unrotated_total_strain(ii, jj) = MetaPhysicL::raw_value((*_total_strain[j])[i](ii, jj));
        }
      (*_mechanical_strain[j])[i] = (*_rotation_matrix[j])[i].transpose() *
                                    _unrotated_mechanical_strain * (*_rotation_matrix[j])[i];
...
void
ADComputeIncrementalShellStrain3D::computeGMatrix()
{
  // quadrature points in isoparametric space
  _2d_points = _qrule->get_points(); // would be in 2D
...
void
ADComputeIncrementalShellStrain3D::computeNodeNormal()
{
  for (unsigned int k = 0; k < _nodes.size(); ++k)
...
void
ADComputeIncrementalShellStrain3D::computeBMatrix()
{
...
void
ADComputeIncrementalShellStrain3D::computeSolnVector()
{
  _soln_vector.zero();
