...
InputParameters validParams<ComputeCosseratElasticityTensor>()
{
  ...
  params.addParam<MooseEnum>("fill_method_bending", RankFourTensor::fillMethodEnum() = "antisymmetric_isotropic", "The fill method for the 'bending' tensor.");
  return params;
}

ComputeCosseratElasticityTensor::ComputeCosseratElasticityTensor(const InputParameters & parameters) :
    ComputeElasticityTensorBase(parameters),
    _Eijkl(getParam<std::vector<Real> >("E_ijkl"), (RankFourTensor::FillMethod)(int)getParam<MooseEnum>("fill_method")),
    _Bijkl(getParam<std::vector<Real> >("B_ijkl"), (RankFourTensor::FillMethod)(int)getParam<MooseEnum>("fill_method_bending")),
