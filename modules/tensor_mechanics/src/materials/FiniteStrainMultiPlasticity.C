...
FiniteStrainMultiPlasticity::returnMap(const RankTwoTensor & stress_old, RankTwoTensor & stress, const std::vector<Real> & intnl_old, std::vector<Real> & intnl, const RankTwoTensor & plastic_strain_old, RankTwoTensor & plastic_strain, const RankFourTensor & E_ijkl, const RankTwoTensor & strain_increment, std::vector<Real> & f, unsigned int & iter, const bool & can_revert_to_dumb)
{
  ...
  std::vector<bool> act(_num_surfaces);
  for (unsigned surface = 0 ; surface < _num_surfaces ; ++surface)
    act[surface] = (f[surface] > _f[modelNumber(surface)]->_f_tol);
  ...
  while (still_finding_solution)
  {
    ...
    bool nr_good = (nr_res2 <= 0.5 && iter <= _max_iter && single_step_success);
  ...
  Moose::out << "Returning " << successful_return << "\n";
...
FiniteStrainMultiPlasticity::checkAndModifyConstraints(const bool & nr_exit_condition, const RankTwoTensor & stress, const std::vector<Real> & intnl, const std::vector<Real> & pm, const std::vector<bool> & initial_act, const bool & can_revert_to_dumb, const RankTwoTensor & initial_stress, const std::vector<Real> & intnl_old, const std::vector<Real> & f, MooseEnum & deact_scheme, std::vector<bool> & act, int & dumb_iteration, std::vector<unsigned int> dumb_order, bool & die)
{
  ...
  // of the problem
  if (success && _num_surfaces > 1)
  {
    std::vector<Real> all_f;
    success = (success && checkAdmissible(stress, intnl, all_f));
  }
  ...
  if (success)
  {
    if (deact_scheme != "dumb")
      ...
      success = (success && checkAndApplyKuhnTucker(f, pm, act));
    else
    {
	...
	success = false;
	if (canIncrementDumb(dumb_iteration))
          incrementDumb(dumb_iteration, dumb_order, act);
        else
