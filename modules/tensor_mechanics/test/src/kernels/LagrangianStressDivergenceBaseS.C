...
InputParameters
LagrangianStressDivergenceBaseS::validParams()
{
...
void
LagrangianStressDivergenceBaseS::precalculateJacobian()
{
...
void
LagrangianStressDivergenceBaseS::precalculateOffDiagJacobian(unsigned int jvar)
{
  // Skip if we are not doing stabilization
  if (!_stabilize_strain)
    return;
...
Real
LagrangianStressDivergenceBaseS::computeQpJacobian()
{
  return computeQpJacobianDisplacement(_alpha, _alpha);
}

Real
LagrangianStressDivergenceBaseS::computeQpOffDiagJacobian(unsigned int jvar)
{
