...
Calls to this class calculates the following strain/stress quantities.

## Axial Stress

The scalar type `AxialStress` calculates the scalar value of a Rank-2 tensor,
...
\label{eq : axial_stress_scalar_type} s = \hat{a}_i T_{ ij }
\hat{a}_j \quad \text
{ where }
\quad \hat{a}_i = \frac{P^2_i - P ^ 1_i} {\left | P ^ 2_i - P ^ 1_i \right | }
\end{equation}

where $\hat{a}$ is the normalized direction vector for the axis defined by the
points $P^1$ and $P^2$.

## Direction

The scalar type `direction` calculates the scalar value of a Rank-2 tensor, $T$,
...
\label{eq : effective_strain_scalar_type} s = \sqrt
{
  \frac{2} { 3 }
  \epsilon_ { ij }
  \epsilon_ { ij }
...
{
  \frac{2} { 3 }
  \dot{\epsilon} ^ p_ { ij } \dot{\epsilon} ^ p_ { ij }
}
\mathrm{d} t
\end{equation}
...
The scalar type `HoopStress` calculates the value of a Rank -2 tensor along the
...
\label{eq : hoop_stress_scalar_type} s = \hat{n} ^ h_i T_ { ij }
\hat{n} ^ h_j
\end{equation}
...
where $P^c$ is the current sampling position point, and $\hat{n}^r$ is the
direction normal to the plane defined by the cylinder axis of rotation vector
and the direction normal to the axis of rotation at the current position $P^c$.

## Hydrostatic Stress

The scalar type `Hydrostatic` calculates the hydrostatic scalar of a Rank-2
...
\label{eq : hydrostatic_scalar_type} s = \frac{Tr \left(T_ { ij } \right)} {3} = \frac{T_{ii}} { 3 }
\end { equation }
...
The scalar type `FirstInvariant` calculates the first invariant of the specified
...
\label{eq : first_invariant_scalar_type} I_T = Tr \left(T_ { ij } \right) = T_ { ii }
\end { equation }


### Second Invariant

Similarly, the scalar type `SecondInvariant` finds the second invariant of the
...
\label{eq : second_invariant_scalar_type} II_T = T_{ii} T_{jj} - \frac{1} { 2 }
\left(T_{ij} T_{ij} + T_{ji} T_ { ji } \right)
\end { equation }


### Third Invariant

The scalar type `ThirdInvariant` computes the value of the Rank-2 tensor,
...
\label{eq : third_invariant_scalar_type} III_T =
    det \left(T_ { ij } \right) = \frac{1} {6} e_{ijk} e_{pqr} T_{ip} T_{jq} T_
{
  kr
}
\end{equation}
...
The scalar type `L2Norm` calculates the L2 normal of a Rank-2 tensor, $T_{ij}$,
...
\label{eq : l2_norm_scalar_type} s = \sqrt
{
  T_{ij} T_ { ij }
}
\end { equation }
...
The scalar type `MaxShear` calculates the maximum shear stress for a Rank-2
...
The scalar type `MaxPrincipal` calculates the largest principal value for a
...
Similarly, the scalar type `MidPrincipal` finds the second largest principal
...
The scalar type `MinPrincipal` computes the smallest principal value for a
symmetric tensor, using the calcEigenValues method from the Rank Two Tensor
utility class.
...
The scalar type `RadialStress` calculates the scalar component for a Rank-2
...
\label{eq : radial_stress_scalar_type} s = \hat{n} ^ r_i T_ { ij } \hat{n}^r_j
\end{equation}

where $\hat{n}^r$ is the direction normal to the plane defined by the cylinder
axis of rotation vector and the direction normal to the axis of rotation at the
current position $P^c$.

## Stress Intensity

The scalar type `StressIntensity` calculates the stress intensity for a Rank-2
...
The scalar type `TriaxialityStress` finds the ratio of the hydrostatic measure,
...
\label{eq : triaxiality_scalar_type} s = \frac{T_{hydrostatic}} {
    T_{vonMises}} = \frac{\frac{1} {3} T_{ii}}
{
  \sqrt
  {
    \frac{3} {2} S_{ij} S_ { ij }
  }
}
\end{equation}
...
\label{eq : volumetric_strain_small_strain} s = T_{11} + T_{22} + T_ { 33 }
\end{equation}
...
The scalar type `VonMisesStress` calculates the vonMises measure for a Rank-2
...
\label{eq : vonmises_scalar_type} s = \sqrt
{
  \frac{3} {2} S_{ij} S_ { ij }
}
\end{equation}
...
!syntax parameters /Utils/RankTwoScalarTools

!syntax inputs /Utils/RankTwoScalarTools

!syntax children /Utils/RankTwoScalarTools
