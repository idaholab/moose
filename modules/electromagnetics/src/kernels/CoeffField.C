...
Real CoeffField::computeQpResidual() {
  return -std::pow(_coefficient,2) * _func.value(_t,_q_point[_qp]) * Reaction::computeQpResidual();
}

Real CoeffField::computeQpJacobian() {
  return -std::pow(_coefficient,2) * _func.value(_t,_q_point[_qp]) * Reaction::computeQpJacobian();
}
