...
Q2PNodalMass::computeQpResidual()
{
  ...
  if (_var_is_pp)
  {
    density = _density.density(_moose_var.nodalSln()[_i]);
    mass = _porosity[_qp]*density*(1 - _other_var_nodal[_i]);
  }
  else
  {
    density = _density.density(_other_var_nodal[_i]);
    mass = _porosity[_qp]*density*_moose_var.nodalSln()[_i];
  }
...
Q2PNodalMass::computeQpJacobian()
{
  ...
  if (_var_is_pp)
  {
    ...
    Real ddensity = _density.ddensity(_moose_var.nodalSln()[_i]);
...
Q2PNodalMass::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  {
    // we're calculating the deriv wrt saturation
    Real density = _density.density(_moose_var.nodalSln()[_i]);
    mass_prime = -_porosity[_qp]*density;
  }
  else
  {
    ...
    mass_prime = _porosity[_qp]*ddensity*_moose_var.nodalSln()[_i];
  }
