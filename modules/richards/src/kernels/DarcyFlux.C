...
InputParameters validParams<DarcyFlux>()
{
  ...
  params.addClassDescription("Darcy flux.  nabla_i (k_ij/mu (nabla_j P - w_j)), where k_ij is the permeability tensor, mu is the fluid viscosity, P is the fluid pressure, and w_j is the fluid weight");
  return params;
}
...
{
  return _grad_test[_i][_qp]*(_permeability[_qp]*(_grad_u[_qp] - _fluid_weight))/_fluid_viscosity;
}
