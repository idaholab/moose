...
int LinInt::_file_number = 0;

LinInt::LinInt(const std::vector<double> & x, const std::vector<double> & y) :
    _x(x),
    _y(y)
{
...
void
LinInt::dumpSampleFile(std::string base_name, std::string x_label, std::string y_label, float xmin, float xmax, float ymin, float ymax)
{
