...
InputParameters validParams<Q2PPiecewiseLinearSink>()
{
  ...
  params.addClassDescription("Sink of fluid, controlled by (pressure, bare_fluxes) interpolation.  This is for use in Q2P models");
  return params;
}
...
Q2PPiecewiseLinearSink::prepareNodalValues()
{
  ...
  if (_var_is_pp)
  {
    ...
    {
      _pp[nodenum] = _moose_var.nodalSln()[nodenum];
  ...
  else
  {
    for (unsigned int nodenum = 0; nodenum < _num_nodes ; ++nodenum)
    {
      ...
      _sat[nodenum] = _moose_var.nodalSln()[nodenum];
    }
  }
