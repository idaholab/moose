...
InputParameters validParams<RichardsBorehole>()
{
  ...
  params.addCoupledVar("dseff_var", "Derivative of Seff wrt the pressure variable.  Usually this is calculated by RichardsMaterial: inputting this parameter allows the user greater freedom");
  params.addCoupledVar("relperm_var", "Relative permeability.  Usually this is calculated by RichardsMaterial: inputting this parameter allows the user greater freedom");
  params.addCoupledVar("drelperm_var", "Derivative of relative permeability wrt effective saturation.  Usually this is calculated by RichardsMaterial: inputting this parameter allows the user greater freedom");
  params.addCoupledVar("density_var", "Fluid density.  Usually this is calculated by RichardsMaterial: inputting this parameter allows the user greater freedom");
  params.addCoupledVar("ddensity_var", "Derivative of fluid density wrt pressure.  Usually this is calculated by RichardsMaterial: inputting this parameter allows the user greater freedom");
...
RichardsBorehole::RichardsBorehole(const std::string & name, InputParameters parameters) :
    ...
    _dseff_val(isCoupled("dseff_var") ? &coupledValue("dseff_var") : &_zero),
    _relperm_val(isCoupled("relperm_var") ? &coupledValue("relperm_var") : &_zero),
    _drelperm_val(isCoupled("drelperm_var") ? &coupledValue("drelperm_var") : &_zero),
    _density_val(isCoupled("density_var") ? &coupledValue("density_var") : &_zero),
    _ddensity_val(isCoupled("ddensity_var") ? &coupledValue("ddensity_var") : &_zero),
...
// Peaceman's form for the borehole well constant
{
  ...
  //std::cout << "half_len = " << half_len << " r0 = " << r0 << " rad = " << rad << "\n";
...
RichardsBorehole::computeQpResidual()
{
  ...
  // contribution from half-segment "ahead of" this point
  {
    ...
    //std::cout << " p= " << _u[_qp] << " relperm = " << (*_relperm_val)[_qp] << " density = " << (*_density_val)[_qp] << " mob = " << mob << "\n";
    //std::cout << " qp= " << _qp << " density = " << _density[_qp][_pvar] << " p = " << std::log(_density[_qp][_pvar]/1000)*2E9 << "\n";
    if ((character < 0.0 && _u[_qp] < bh_pressure) || (character > 0.0 && _u[_qp] > bh_pressure))
      // injection, so outflow<0 || // production, so outflow>0
      outflow += _test[_i][_qp]*std::abs(character)*wc*mob*(_u[_qp] - bh_pressure);
