...
InputParameters validParams<DarcyFluxComponent>()
{
  ...
  params.addClassDescription("Darcy flux (in m^3.s^-1.m^-2, or m.s^-1)  -(k_ij/mu (nabla_j P - w_j)), where k_ij is the permeability tensor, mu is the fluid viscosity, P is the fluid pressure, and w_j is the fluid weight.  If velocity_scaling is used then -(k_ij/mu (nabla_j P - w_j))/velocity_scaling is returned");
  params.addParam<MooseEnum>("component", component, "The component of the Darcy flux to return");
  params.addParam<Real>("velocity_scaling", 1, "Scale the result by (1/velocity_scaling).  Usually velocity_scaling = porosity.");
...
{
  return -_poro_recip*(_permeability[_qp]*(_grad_pp[_qp] - _fluid_weight)/_fluid_viscosity)(_component);
}
