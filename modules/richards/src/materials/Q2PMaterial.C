...
InputParameters validParams<Q2PMaterial>()
{
  ...
  params.addRequiredRangeCheckedParam<Real>("mat_porosity", "mat_porosity>=0 & mat_porosity<=1", "The porosity of the material.  Should be between 0 and 1.  Eg, 0.1");
  ...
  params.addCoupledVar("perm_change", "A list of auxillary variable describing permeability changes.  There must be 9 of these (in 3D), corresponding to the xx, xy, xz, yx, yy, yz, zx, zy, zz components respectively (in 3D).  Permeability = mat_permeability*10^(perm_change).");
