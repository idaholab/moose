...
InputParameters validParams<Q2PPiecewiseLinearSinkFlux>()
{
  ...
  params.addParam<UserObjectName>("fluid_density", "The fluid density as a RichardsDensity UserObject.  If this and the fluid_viscosity are given, then fluxes are multiplied by (density*permeability_nn/viscosity), where the '_nn' indicates the component normal to the boundary.  In this case bare_flux is measured in Pa.s^-1.  This can be used in conjunction with fluid_relperm.");
  params.addParam<Real>("fluid_viscosity", "The fluid dynamic viscosity.");
  params.addParam<UserObjectName>("fluid_relperm", "The fluid density as a RichardsRelPerm UserObject (eg RichardsRelPermPower for water, or Q2PRelPermPostGas for gas).  If this and the saturation variable are defined then the flux will be motiplied by relative permeability.  This can be used in conjunction with fluid_density");
...
Q2PPiecewiseLinearSinkFlux::Q2PPiecewiseLinearSinkFlux(const InputParameters & parameters) :
    ...
    _density(isParamValid("fluid_density") ? &getUserObject<RichardsDensity>("fluid_density") : NULL),
    _viscosity(isParamValid("fluid_viscosity") ? getParam<Real>("fluid_viscosity") : 1),
    _relperm(isParamValid("fluid_relperm") ? &getUserObject<RichardsRelPerm>("fluid_relperm") : NULL),
