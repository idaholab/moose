...
InputParameters validParams<RichardsRelPermPowerGas>()
{
  ...
  params.addRequiredParam<Real>("simm", "Immobile saturation.  Must be between 0 and 1.   Define s = (seff - simm)/(1 - simm).  Then relperm = 1 - (n+1)(1-s)^n + n(1-s)^(n+1)");
  params.addRequiredParam<Real>("n", "Exponent.  Must be >= 2.   Define s = (seff - simm)/(1 - simm).  Then relperm = 1 - (n+1)(1-s)^n + n(1-s)^(n+1)");
  ...
  _n(getParam<Real>("n"))
{
  ...
  if (_n < 2)
    mooseError("The exponent, n, in the powerGas relative permeability function is set to " << _n << " but it must not be less than 2");
}
