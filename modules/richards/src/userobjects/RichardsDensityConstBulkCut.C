...
InputParameters validParams<RichardsDensityConstBulkCut>()
{
  ...
  params.addParam<Real>("cut_limit", 1E5, "For porepressure > cut_limit, density = dens0*Exp(pressure/bulk).  For porepressure < cut_limie, density = cubic*dens0*Exp(pressure/bulk), where cubic=1 for pressure=cut_limit, and cubic=0 for pressure<=zero_point");
  params.addParam<Real>("zero_point", 0,  "For porepressure > cut_limit, density = dens0*Exp(pressure/bulk).  For porepressure < cut_limie, density = cubic*dens0*Exp(pressure/bulk), where cubic=1 for pressure=cut_limit, and cubic=0 for pressure<=zero_point");
...
RichardsDensityConstBulkCut::density(Real p) const
{
  ...
  Real modifier = (3*_cut_limit - 2*p - _zero_point)*(p - _zero_point)*(p - _zero_point)/_c3;
