...
\item In multiphase problems if one phase completely disappears, MOOSE
  may not converge.  To avoid this:
\begin{enumerate}
...
will be singular for $dt\rightarrow 0$, and this sometimes leads to
...
\chapter{Coupled porosity}

When porosity is a function of time and the residual saturations are
nonzero, the system can be singular when $dt\rightarrow 0$ (or for
small $dt$).  This is annoying, and i suggest using zero residual
saturations.

In the case of one phase disappearing {\em and} time-dependent
porosity, then you're stuck, sorry.  I need to implement a
pressure-saturation-saturation formulation that will get around this.
