...
RadiationTransferAction::validParams()
{
  ...
  params.addParam<VariableName>("heat_flux_variable",
                                "Heat flux aux variable name; this must be provided if "
                                "'add_heat_flux_aux' is true (otherwise has no effect)");
  params.addParam<std::vector<SubdomainName>>(
      "block",
      "Subdomains to use for heat flux aux if 'add_heat_flux_aux' is true (otherwise has no "
      "effect)");
    ...
    _add_heat_flux_aux(getParam<bool>("add_heat_flux_aux"))
{
  ...
  if (_add_heat_flux_aux)
  {
    ...
    if (isParamValid("block"))
      ...
      paramError("block", "If 'add_heat_flux_aux' is true, then this parameter must be provided.");
  }
}
