...
def build_frames(camera=None):
    ...
    parser = argparse.ArgumentParser(
        description="Process results for initial/final condition and/or area conservation."
    )
    parser.add_argument("exodus", help="The ExodusII file to process.")
    parser.add_argument(
        "--variable", "-v", default="phi", type=str, help="The variable to display."
    )
    parser.add_argument(
        "--interval", default=5, type=int, help="The image output interval."
    )
    parser.add_argument(
        "--output", "-o", type=str, help="The prefix for the outputted PNG files."
    )
    parser.add_argument(
        "--size", default=[400, 400], type=list, help="The image size, in pixels."
    )
    parser.add_argument(
        "--gif",
        action="store_true",
        help="Enable the creation of an animated gif via ImageMagick, using the --output prefix for the filename.",
    )
    parser.add_argument(
        "--delay", default=20, type=int, help="Delay between frames in animate gif."
    )
    parser.add_argument(
        "--clean",
        action="store_true",
        help="Delete existing PNG files matching the output name.",
    )
    ...
    initial_result = chigger.exodus.ExodusResult(
        initial_reader,
        variable=options.variable,
        camera=camera,
        color=[0, 0, 0],
        filters=[initial_contour],
        edge_width=2,
        layer=2,
    )
    ...
    result2 = chigger.exodus.ExodusResult(
        reader,
        variable=options.variable,
        camera=camera,
        color=[0, 1, 0],
    ...
    if options.gif:
        chigger.utils.animate(
            "{}_*.png".format(options.output),
            "{}.gif".format(options.output),
            delay=options.delay,
        )
