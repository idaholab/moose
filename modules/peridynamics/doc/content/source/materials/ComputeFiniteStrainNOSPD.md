...
The `ComputeFiniteStrainNOSPD` Material calculates the strain and rotation increments used in the peridynamic correspondence model under finite strain assumptions. The class first constructs the [deformation gradient](peridynamics/DeformationGradients.md) and uses the Rashid scheme [citep:](rashid1993incremental) to formulate a incremental corotational finite strain model.
