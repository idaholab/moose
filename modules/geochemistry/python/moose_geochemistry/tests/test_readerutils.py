...
class TestReaderUtils(unittest.TestCase):
    ...
    def testfillMissingValues(self):
        ...
        data = [-0.5426, -0.3614, -0.1267, 0.1260, 0.4402, 0.7793, 500.0000, 500.0000]
        ...
        output, note = reader_utils.fillMissingValues(
            T, data, "fourth-order", missing_value
        )
        self.assertEqual(output, gold)
        self.assertEqual(note, notegold)
        ...
        output, note = reader_utils.fillMissingValues(
            T, data, "maier-kelly", missing_value
        )
        self.assertEqual(output, gold)
        self.assertEqual(note, notegold)
