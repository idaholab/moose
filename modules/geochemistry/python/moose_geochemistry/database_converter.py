...
def command_line_options():
    ...
    parser = argparse.ArgumentParser(
        description="Utility to read data from thermodynamic database and write to MOOSE thermodynamic database (in JSON format)"
    )
    parser.add_argument(
        "-i", "--input", type=str, help="The input database", required=True
    )
    parser.add_argument(
        "--format",
        type=str,
...
def main():
    ...
    database["Header"]["original"] = (
        "moose" + os.path.abspath(opt.input).split("moose")[1]
    )
    ...
    sys.stdout.write(
        "Finished parsing " + opt.input + ".  Output written to " + opt.output + "\n"
    )
    sys.exit(0)
