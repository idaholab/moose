    ...
    while line < len(dblist):
        ...
        while parsing_header:
            ...
            if dblist[line].strip().startswith("Output package"):
                assert (
                    dblist[line].split(":")[1].strip() == "eq3"
                ), "EQ3/6 reader detected database in wrong format"
        ...
        if dblist[line].strip().startswith("Miscellaneous") and dblist[
            line + 1
        ].strip().startswith("+--"):
            ...
            while not dblist[line].strip().startswith("+--"):
                if dblist[line].strip().startswith("temperatures"):
                    ...
                    while dblist[line].strip()[0].isnumeric():
                        temperatures.extend(
                            [float(item) for item in dblist[line].split()]
                        )
                        line = line + 1
        ...
        # Read the elements
        if dblist[line].strip().startswith("elements") and dblist[
            line + 1
        ].strip().startswith("+--"):
            line = line + 2
        ...
        if dblist[line].strip().startswith("basis species") and dblist[
            line + 1
        ].strip().startswith("+--"):
            line = line + 2
            ...
            while parsing_basis:
                ...
                while not dblist[line].strip().startswith("+--"):
                    ...
                    if "DHazero" in dblist[line]:
                        basis_species[species]["radius"] = float(
                            dblist[line].split()[3]
                        )

                    # Read charge
                    if "charge" in dblist[line]:
                        basis_species[species]["charge"] = float(
                            dblist[line].split()[2]
                        )
                ...
                if dblist[line].strip().startswith("+--") and dblist[
                    line + 2
                ].strip().startswith("+--"):
                    parsing_basis = False
        ...
        if dblist[line].strip().startswith("auxiliary basis species") and dblist[
            line + 1
        ].strip().startswith("+--"):
            line = line + 2
            ...
            while parsing_auxiliary:
                ...
                while not dblist[line].strip().startswith("+--"):
                    ...
                    if "mol.wt." in dblist[line]:
                        redox_couples[species]["molecular weight"] = float(
                            dblist[line].split()[3]
                        )

                    # Read radius
                    if "DHazero" in dblist[line]:
                        redox_couples[species]["radius"] = float(
                            dblist[line].split()[3]
                        )

                    # Read charge
                    if "charge" in dblist[line]:
                        redox_couples[species]["charge"] = float(
                            dblist[line].split()[2]
                        )
                    ...
                    if "element" in dblist[line]:
                        ...
                        while len(redox_couples[species]["elements"]) < num_elements:
                            ...
                            data = dblist[line].split()
                            for i in range(0, len(data), 2):
                                redox_couples[species]["elements"][data[i + 1]] = float(
                                    data[i]
                                )
                    ...
                    if "species" in dblist[line]:
                        ...
                        while len(redox_couples[species]["species"]) < num_species - 1:
                            ...
                            for i in range(0, len(data), 2):
                                if data[i + 1] != species:
                                    redox_couples[species]["species"][data[i + 1]] = (
                                        float(data[i])
                                    )
        ...
        if dblist[line].strip().startswith("aqueous species") and dblist[
            line + 1
        ].strip().startswith("+--"):
            line = line + 2
            ...
            while parsing_secondary:
                ...
                while not dblist[line].strip().startswith("+--"):
                    ...
                    # Read radius
                    if "DHazero" in dblist[line]:
                        secondary_species[species]["radius"] = float(
                            dblist[line].split()[3]
                        )

                    # Read charge
                    if "charge" in dblist[line]:
                        secondary_species[species]["charge"] = float(
                            dblist[line].split()[2]
                        )
                    ...
                    if "element" in dblist[line]:
                        num_elements = int(dblist[line].split()[0])
                        while (
                            len(secondary_species[species]["elements"]) < num_elements
                        ):
                            line = line + 1
                            data = dblist[line].split()
                            for i in range(0, len(data), 2):
                                secondary_species[species]["elements"][data[i + 1]] = (
                                    float(data[i])
                                )
                    ...
                    if "species" in dblist[line]:
                        num_species = int(dblist[line].split()[0])
                        while (
                            len(secondary_species[species]["species"]) < num_species - 1
                        ):
                            line = line + 1
                            data = dblist[line].split()
                            for i in range(0, len(data), 2):
        ...
        if dblist[line].strip().startswith("solids") and dblist[
            line + 1
        ].strip().startswith("+--"):
            line = line + 2
            ...
            while parsing_mineral:
                ...
                while not dblist[line].strip().startswith("+--"):
                    # Read molecular weight
                    if "mol.wt." in dblist[line]:
                        mineral_species[species]["molecular weight"] = float(
                            dblist[line].split()[3]
                        )

                    # Read molar volume
                    if "V0PrTr" in dblist[line]:
                        mineral_species[species]["molar volume"] = float(
                            dblist[line].split()[2]
                        )
                    ...
                    if "element" in dblist[line]:
                        ...
                        while len(mineral_species[species]["elements"]) < num_elements:
                            ...
                            data = dblist[line].split()
                            for i in range(0, len(data), 2):
                                mineral_species[species]["elements"][data[i + 1]] = (
                                    float(data[i])
                                )
                    ...
                    if "species" in dblist[line]:
                        num_species = int(dblist[line].split()[0])
                        while (
                            len(mineral_species[species]["species"]) < num_species - 1
                        ):
                            line = line + 1
                            data = dblist[line].split()
                            for i in range(0, len(data), 2):
        ...
        if dblist[line].strip().startswith("gases") and dblist[
            line + 1
        ].strip().startswith("+--"):
            line = line + 2
            ...
            while parsing_gas:
                ...
                while not dblist[line].strip().startswith("+--"):
                    # Read molecular weight
                    if "mol.wt." in dblist[line]:
                        gas_species[species]["molecular weight"] = float(
                            dblist[line].split()[3]
                        )
                    ...
                    if "element" in dblist[line]:
                        ...
                        while len(gas_species[species]["elements"]) < num_elements:
                            ...
                            data = dblist[line].split()
                            for i in range(0, len(data), 2):
                                gas_species[species]["elements"][data[i + 1]] = float(
                                    data[i]
                                )
                    ...
                    if "species in" in dblist[line]:
                        ...
                        while len(gas_species[species]["species"]) < num_species - 1:
                            ...
                            data = dblist[line].split()
                            for i in range(0, len(data), 2):
                                if data[i + 1] != species or not data[i].startswith(
                                    "-1"
                                ):
                                    gas_species[species]["species"][data[i + 1]] = (
                                        float(data[i])
                                    )
