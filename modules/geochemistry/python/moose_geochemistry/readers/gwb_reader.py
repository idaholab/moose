...
def readDatabase(dblist):
    ...
    def fillValues(vals):
        return reader_utils.fillMissingValues(
            temperatures, vals, logk_model, missing_value
        )
    ...
    while line < len(dblist):
            ...
            while reading_elements:
                if dblist[line].split():
                    ...
                    if (
                        ...
                        raise ValueError(
                            "Element line incorrectly specified.  Offending line is: "
                            + dblist[line]
                        )
                    elements[elemlist[1]] = {}
        ...
        if "basis species\n" in dblist[line] and len(dblist[line].split()) == 3:
            ...
            while reading_basis:
                if dblist[line].strip():
                    ...
                    basis_species[species][
                        "radius"
                    ] = 0  # default for sorption sites that are counted as basis species
                    line = line + 1

                    # Charge, ionic radius and molecular weight
                    if "charge" in dblist[line]:
                        data = dblist[line][dblist[line].index("charge") + 6 :].split(
                            "="
                        )[1]
                    ...
                    if "ion size" in dblist[line]:
                        data = dblist[line][dblist[line].index("ion size") + 8 :].split(
                            "="
                        )[1]
                        if len(data.split()) <= 1 or data.split()[1] != "A":
                            raise ValueError(
                                "ion size must be measured in Angstroms.  Offending line is: "
                                + dblist[line]
                            )
                    ...
                    if "mole wt." in dblist[line]:
                        data = dblist[line][dblist[line].index("mole wt.") + 8 :].split(
                            "="
                        )[1]
                        if len(data.split()) <= 1 or data.split()[1] != "g":
                            raise ValueError(
                                "molecular weight must be measured in grams.  Offending line is: "
                                + dblist[line]
                            )
                        basis_species[species]["molecular weight"] = float(
                            data.split()[0]
                        )
                        if len(data.split()) <= 2:
                            line = line + 1
                    ...
                    if "elements in species" in dblist[line]:
                        ...
                        while len(basis_species[species]["elements"]) < num_elements:
                            ...
                            for i in range(0, len(data), 2):
                                basis_species[species]["elements"][data[i + 1]] = float(
                                    data[i]
                                )
            ...
            while reading_redox:
                if dblist[line].strip():
                    ...
                    if "mole wt." in dblist[line]:
                        ...
                        redox_couples[couple]["molecular weight"] = float(
                            data.split()[0]
                        )
                        if len(data.split()) <= 2:
                            line = line + 1
                    ...
                    if "species in reaction" in dblist[line]:
                        ...
                        while len(redox_couples[couple]["species"]) < num_species:
                            ...
                            data = dblist[line].split()
                            for i in range(0, len(data), 2):
                                redox_couples[couple]["species"][data[i + 1]] = float(
                                    data[i]
                                )
            ...
            while reading_secondary:
                if dblist[line].strip():
                    ...
                    if "mole wt." in dblist[line]:
                        ...
                        secondary_species[species]["molecular weight"] = float(
                            data.split()[0]
                        )
                        if len(data.split()) <= 2:
                            line = line + 1
                    ...
                    if "species" in dblist[line]:
                        ...
                        while len(secondary_species[species]["species"]) < num_species:
                            ...
                            data = dblist[line].split()
                            for i in range(0, len(data), 2):
                                secondary_species[species]["species"][data[i + 1]] = (
                                    float(data[i])
                                )
            ...
            while reading_freeelectron:
                if dblist[line].strip():
                    ...
                    if "mole wt." in dblist[line]:
                        ...
                        free_electron[species]["molecular weight"] = float(
                            data.split()[0]
                        )
                        if len(data.split()) <= 2:
                            line = line + 1
                    ...
                    if "species" in dblist[line]:
                        ...
                        while len(free_electron[species]["species"]) < num_species:
                            ...
                            data = dblist[line].split()
                            for i in range(0, len(data), 2):
                                free_electron[species]["species"][data[i + 1]] = float(
                                    data[i]
                                )
                ...
                if dblist[line].strip():
                    ...
                    if "surface area" in dblist[line]:
                        ...
                        if len(data.split()) <= 1 or data.split()[1] != "m2/g":
                            raise ValueError(
                                "sorbing mineral surface area must be specified in m2/g.  Offending line is: "
                                + dblist[line]
                            )
                        sorbing_minerals[species]["surface area"] = float(
                            data.split()[0]
                        )
                        line = line + 1

                    if (
                        "sorption sites" in dblist[line]
                        and len(dblist[line].split()) == 3
                    ):
                        num_sites = int(dblist[line].split()[0])
                        while (
                            len(sorbing_minerals[species]["sorbing sites"]) < num_sites
                        ):
                            line += 1
                            data = dblist[line].split()
                            if (
                                ...
                                raise ValueError(
                                    "sorbing minerals must define site density in mol/mol mineral.  Offending line is: "
                                    + dblist[line]
                                )
                            sorbing_minerals[species]["sorbing sites"][data[0]] = float(
                                data[3]
                            )
        ...
        if (
            "minerals\n" in dblist[line]
            and not ("sorbing minerals" in dblist[line])
            and len(dblist[line].split()) == 2
        ):
                ...
                if dblist[line].strip():
                    ...
                    if "mole" in dblist[line]:
                        ...
                        if (
                            ...
                            raise ValueError(
                                "molar volume must be measured in cc and molecular weight in grams.  Offending line is: "
                                + dblist[line]
                            )
                    ...
                    if "species" in dblist[line]:
                        ...
                        while len(mineral_species[species]["species"]) < num_species:
                            ...
                            data = dblist[line].split()
                            for i in range(0, len(data), 2):
                                mineral_species[species]["species"][data[i + 1]] = (
                                    float(data[i])
                                )
            ...
            while reading_gas_species:
                if dblist[line].strip():
                    ...
                    if "chi" in dblist[line]:
                        gas_species[species]["chi"] = [
                            float(item) for item in dblist[line].split()[1:]
                        ]
                        line = line + 1
                    ...
                    if "Pcrit" in dblist[line]:
                        ...
                        if (
                            ...
                            raise ValueError(
                                "Tsonopoulos parameters must be measured in bar and Kelvin.  Offending line is: "
                                + dblist[line]
                            )
                    ...
                    if "species" in dblist[line]:
                        ...
                        while len(gas_species[species]["species"]) < num_species:
                            ...
                            data = dblist[line].split()
                            for i in range(0, len(data), 2):
                                gas_species[species]["species"][data[i + 1]] = float(
                                    data[i]
                                )
                ...
                if dblist[line].strip():
                    ...
                    if "mole" in dblist[line]:
                        ...
                        if (
                            ...
                            raise ValueError(
                                "Oxide molecular weight must be measured in grams.  Offending line is: "
                                + dblist[line]
                            )
            ...
            while reading_ss:
                if dblist[line].strip():
                    ...
                    if "mole wt." in dblist[line]:
                        ...
                        surface_species[species]["molecular weight"] = float(
                            data.split()[0]
                        )
                        if len(data.split()) <= 2:
                            line = line + 1
                    ...
                    if "species" in dblist[line]:
                        ...
                        while len(surface_species[species]["species"]) < num_species:
                            ...
                            data = dblist[line].split()
                            for i in range(0, len(data), 2):
                                surface_species[species]["species"][data[i + 1]] = (
                                    float(data[i])
                                )
                        line = line + 1
                    ...
                    if "log K" in dblist[line]:
                        ...
                        if (
                            ...
                            raise ValueError(
                                "log K and dlogK/dT must be specified for surface species.  Offending line is "
                                + dblist[line]
                            )
