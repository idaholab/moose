...
steps = sorted(
    [int(s.split("bio_zoning_flow_out_react0_csv_data_")[1][:-4]) for s in steps]
)
...
def animate(idx):
    ...
    ax1.annotate(
        ...
        xy=(24, 0.15),
        xytext=(10, 0.15),
        arrowprops=dict(width=2.5, headwidth=7.0, headlength=10.0, facecolor="b"),
    )
    ...
    ax2.arrow(
        10, 0.15, 9, 0, width=0.04, head_length=5.0, head_width=0.15, facecolor="b"
    )
...
plt.show()
ani.save(
    "../../../doc/content/media/geochemistry/bio_zoning.mp4",
    writer=animation.FFMpegWriter(fps=5),
)
plt.close()

with open(
    "bio_zoning_flow_out_react0_csv_data_" + str(steps[-1]).zfill(4) + ".csv", "r"
) as f:
    data = [list(map(float, line.strip().split(","))) for line in f.readlines()[1:-1]]
xvals = [x[6] / 1000 for x in data]
methanogen = [x[0] * 1000 for x in data]
