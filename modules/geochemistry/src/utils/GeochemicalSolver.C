...
GeochemicalSolver::GeochemicalSolver(const ModelGeochemicalDatabase & mgd,
...
Real
GeochemicalSolver::computeResidual(DenseVector<Real> & residual,
                                   const DenseVector<Real> & mole_additions) const
{
  for (unsigned a_ind = 0; a_ind < _num_in_algebraic_system; ++a_ind)
    residual(a_ind) = _egs.getResidualComponent(a_ind, mole_additions);
  return residual.l1_norm();
}

void
GeochemicalSolver::solveAndUnderrelax(DenseMatrix<Real> & jacobian,
...
bool
GeochemicalSolver::swapNeeded(unsigned & swap_out_of_basis,
                               ...
                               DenseMatrix<Real> & dmole_additions)
{
  ...
  while (still_swapping && num_swaps <= _max_swaps_allowed)
  {
           ...
           iter < _ramp_max_ionic_strength)
    {
      iter += 1;
      tot_iter += 1;
      _egs.computeJacobian(_residual, _jacobian, mole_additions, dmole_additions);
    ...
    try
    {
      ...
      for (unsigned i = 0; i < _num_basis; ++i)
        _egs.addToBulkMoles(i, mole_additions(i));
