...
[Kernels]
  # Set up kernels for Gamma model
...
[Materials]
  [5DGaussian]
    # Material properties adding anisotropy to gamma and L for gamma model
    type =  SphericalGaussianMaterial
    outputs = exodus
    model_type = GAMMA # The model to use. Epsilon or Gamma model?
    ...
    base_gamma = 1.6 # (dimensionless); Base value of gamma used to specify an anisotropic material propertie gamma
    base_g_gamma = 0.482570 # (dimensionless); Base value of g_gamma associated to base_gamma from which gaussian is either added or substracted
    base_f_gamma = 0.130925 # (dimensionless); Base value of f_gamma associated to base_gamma
    ...
    add_g_gamma_gaussian = false # 'true' to add to base value; 'false' to substract from base value g_gamma
