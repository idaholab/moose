...
EpsilonModelKernelAction::act() {
  for (unsigned int op = 0; op < _op_num; ++op) {
    ...
    if (op < _op_num - 1) {
      ...
      params.set<std::vector<VariableName>>("v") = v;
      params.applyParameters(parameters());
      _problem->addKernel("EpsilonModelKernel1stGauss", "EpsilonModelKernel1stGauss" + var_name_plus, params);

      params = _factory.getValidParams("EpsilonModelKernel2ndGauss");
      params.set<NonlinearVariableName>("variable") = var_name_plus;
      params.applyParameters(parameters());
      _problem->addKernel("EpsilonModelKernel2ndGauss", "EpsilonModelKernel2ndGauss" + var_name_plus, params);
    }
    ...
    if (op > 0) {
      ...
      params.set<std::vector<VariableName>>("v") = v;
      params.applyParameters(parameters());
      _problem->addKernel("EpsilonModelKernel1stV2Gauss", "EpsilonModelKernel1stV2Gauss" + var_name_minus, params);

      params = _factory.getValidParams("EpsilonModelKernel2ndV2Gauss");
      params.set<NonlinearVariableName>("variable") = var_name_minus;
      params.applyParameters(parameters());
      _problem->addKernel("EpsilonModelKernel2ndV2Gauss", "EpsilonModelKernel2ndV2Gauss" + var_name_minus, params);
    }
  }
}
