...
GammaModelKernelAction::validParams() {
  ...
  params.addClassDescription("Set up ACGrGrPoly, ACInterface, TimeDerivative, GammaModelKernelGauss, GammaModelKernelV2Gauss kernels");
  ...
  params.addParam<bool>("variable_mobility", true, "The mobility is a function of any MOOSE variable (if this is set to false, L must be constant over the entire domain!)");
...
void GammaModelKernelAction::act() {
  for (unsigned int op : make_range(0U, _op_num)) {
    ...
    if (op > 0) {
      ...
      params.set<std::vector<VariableName>>("v") = v;
      params.applyParameters(parameters());
      _problem->addKernel("GammaModelKernelV2Gauss", "GammaModelKernelV2Gauss" + var_name_minus, params);
    }
  }
}
