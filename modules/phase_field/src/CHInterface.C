...
CHInterface::computeQpResidual()
{
  ...
  value += 2.0*_kappa[_qp]*(_second_u[_qp].tr()*(_M[_qp]*_second_test[_i][_qp].tr() + _grad_M[_qp]*_grad_test[_i][_qp]));
...
CHInterface::computeQpJacobian()
{
  ...
  value += 2.*_kappa[_qp]*_second_phi[_j][_qp].tr()*(_M[_qp]*_second_test[_i][_qp].tr() + _grad_M[_qp]*_grad_test[_i][_qp]);
