...
InputParameters validParams<EulerAngle2RGBAction>()
{
  ...
  params.addClassDescription("Set up auxvariables and auxkernels to output Euler angles as RGB values interpolated across inverse pole figure");
...
EulerAngle2RGBAction::EulerAngle2RGBAction(const InputParameters & params) :
    Action(params),
    _var_name_base(getParam<std::string>("auxvariable_name_base"))
{
}
