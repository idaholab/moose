...
validParams<PolycrystalKernelAction>()
{
  ...
  params.addRequiredParam<unsigned int>(
      "op_num", "specifies the total number of grains/variants (in the two phases) to create");
  params.addParam<unsigned int>(
      "second_phase_op_num", 0, "specifies the total number of grains/variants of the second phase");
...
PolycrystalKernelAction::act()
{
  for (unsigned int op = 0; op < _op_num; ++op)
  {
...

    {
      ...
      params.set<unsigned int>("second_phase_op_num") = getParam<unsigned int>("second_phase_op_num");
      params.set<Real>("en_ratio") = getParam<Real>("en_ratio");
      params.set<Real>("second_phase_en_ratio") = getParam<Real>("second_phase_en_ratio");
