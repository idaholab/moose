...
validParams<NonconservedAction>()
{
  ...
  params.addClassDescription("Set up the variable and the kernels needed for a nonconserved phase field variable");

  params.addRequiredParam<NonlinearVariableName>("variable_name","The name of the nonconserved variable");

  // Get MooseEnums for the possible order/family options for this variable
  MooseEnum families("LAGRANGE MONOMIAL HERMITE HIERARCHIC CLOUGH XYZ SZABAB"
                     "BERNSTEIN L2_LAGRANGE L2_HIERARCHIC","LAGRANGE");
  params.addParam<MooseEnum>("family", families, "Specifies the family of FE "
                             "shape functions to use for this variable");
  MooseEnum orders("CONSTANT FIRST SECOND THIRD FOURTH", "FIRST");
  params.addParam<MooseEnum>("order", orders, "Specifies the order of the FE "
  ...
  params.addParam<std::vector<VariableName>>("args", "Vector of variable arguments this kernel depends on");
  ...
  params.addParam<bool>("variable_mobility", true,
...
NonconservedAction::NonconservedAction(const InputParameters & params)
  : Action(params),
    _var_name(getParam<NonlinearVariableName>("variable_name")),
...
NonconservedAction::act()
{
  ...
  else if (_current_task == "add_kernel")
  {
    ...
    params1.set<bool>("implicit") = getParam<bool>("implicit");
    params1.set<bool>("use_displaced_mesh") = getParam<bool>("use_displaced_mesh");
    ...
    params2.set<bool>("implicit") = getParam<bool>("implicit");
    params2.set<bool>("use_displaced_mesh") = getParam<bool>("use_displaced_mesh");
    params2.set<std::vector<VariableName>>("args") = getParam<std::vector<VariableName>>("args");
    ...
    params3.set<bool>("implicit") = getParam<bool>("implicit");
    params3.set<bool>("use_displaced_mesh") = getParam<bool>("use_displaced_mesh");
    params3.set<std::vector<VariableName>>("args") = getParam<std::vector<VariableName>>("args");
