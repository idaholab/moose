...
InputParameters validParams<KKSMultiFreeEnergy>()
{
  ...
  params.addClassDescription("Total free energy in multi-phase KKS system, including chemical, barrier and gradient terms");
  params.addRequiredParam<std::vector<MaterialPropertyName> >("Fj_names", "List of free energies for each phase. Place in same order as hj_names and gj_names!");
  params.addRequiredParam<std::vector<MaterialPropertyName> >("hj_names", "Switching Function Materials that provide h. Place in same order as Fj_names and gj_names!");
  params.addRequiredParam<std::vector<MaterialPropertyName> >("gj_names", "Barrier Function Materials that provide g. Place in same order as Fj_names and hj_names!");
