...
#include "ADTotalFreeEnergy.h"

registerMooseObject("PhaseFieldApp", ADTotalFreeEnergy);

InputParameters
ADTotalFreeEnergy::validParams()
{
...
ADTotalFreeEnergy::ADTotalFreeEnergy(const InputParameters & parameters)
  : TotalFreeEnergyBase(parameters), _F(getADMaterialProperty<Real>("f_name")), _kappas(_nkappas)
{
  // Error check to ensure size of interfacial_vars is the same as kappa_names
  if (_nvars != _nkappas)
    mooseError(
        "Size of interfacial_vars is not equal to the size of kappa_names in ADTotalFreeEnergy");

  // Assign kappa values
  for (unsigned int i = 0; i < _nkappas; ++i)
    _kappas[i] = &getADMaterialPropertyByName<Real>(_kappa_names[i]);
}

Real
ADTotalFreeEnergy::computeValue()
{
  // Include bulk energy contribution and additional contributions
  Real total_energy = MetaPhysicL::raw_value(_F[_qp] + _additional_free_energy[_qp]);
