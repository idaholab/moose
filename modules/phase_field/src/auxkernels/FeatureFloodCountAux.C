...
InputParameters validParams<FeatureFloodCountAux>()
{
  ...
  MooseEnum field_display("UNIQUE_REGION VARIABLE_COLORING ACTIVE_BOUNDS CENTROID GHOSTED_ELEMS", "UNIQUE_REGION");
    ...
    _field_type(static_cast<FeatureFloodCount::FIELD_TYPE>(static_cast<int>(_field_display)))
{
  if (isNodal())
  {
    ...
    if (_field_display == "GHOSTED_ELEMS")
      mooseError("GHOSTED_ELEMS is only available for elemental aux variables");
...
FeatureFloodCountAux::computeValue()
{
  ...
  case 4:  // GHOSTED_ELEMS
    return _flood_counter.getEntityValue(_current_elem->id(), _field_type);
  }
