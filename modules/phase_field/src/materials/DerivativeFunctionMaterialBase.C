...
DerivativeFunctionMaterialBase::DerivativeFunctionMaterialBase(const std::string & deprecated_name, InputParameters parameters) :
    FunctionMaterialBase(deprecated_name, parameters),
    _third_derivatives(isParamValid("third_derivatives") ? getParam<bool>("third_derivatives") : (getParam<unsigned int>("derivative_order")==3))
{
  // loop counters
  unsigned int i, j, k;
