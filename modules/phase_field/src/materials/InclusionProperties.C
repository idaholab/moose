...
InputParameters validParams<InclusionProperties>()
{
  ...
  params.addRequiredParam<std::vector<Real> >("misfit_strains", "Vector of misfit strains in order eps_11, eps_22");
  params.addRequiredParam<MaterialPropertyName>("stress_name", "Name of the material property where analytical stresses will be stored");
  params.addRequiredParam<MaterialPropertyName>("strain_name", "Name of the material property where analytical strains will be stored");
  params.addRequiredParam<MaterialPropertyName>("energy_name", "Name of the material property where analytical elastic energies will be stored");
...
InclusionProperties::computeQpProperties()
{
  ...
  else
  {
    ...
    Real H11 = rho_a*_b * (_a*rho_b + _b*rho_a + 2*_a * rho_a*rho_a * rho_b + _b*std::pow(rho_a, 3))
              / std::pow((_a*rho_b + _b*rho_a), 2)
            + n_x*n_x * (2 - 6*rho_a*rho_a + (8*rho_a*rho_a + T_6) * n_x*n_x);

    Real H22 = rho_b*_a * (_a*rho_b + _b*rho_a + 2*_b * rho_a * rho_b*rho_b + _a*std::pow(rho_b, 3))
              / std::pow((_a*rho_b + _b*rho_a), 2)
    ...
    _stress[_qp](0,0) = 4*_mu * rho_a*rho_b / (_kappa + 1)
                          * (H11 * _misfit[0] + H12 * _misfit[1]);
    _stress[_qp](1,1) = 4*_mu * rho_a*rho_b / (_kappa + 1)
                          * (H12 * _misfit[0] + H22 * _misfit[1]);
    _stress[_qp](2,2) = 4*_mu * rho_a*rho_b / (_kappa + 1)
                          * _nu * (H31 * _misfit[0] + H32 * _misfit[1]);
...
InclusionProperties::precomputeInteriorProperties()
{
  ...
  _stress_int(0,0) = -4 * _mu * t / (_kappa + 1) / (1 + t) / (1 + t)
                  * (T11 * _misfit[0] + T12 * _misfit[1]);
  _stress_int(1,1) = -4 * _mu * t / (_kappa + 1) / (1 + t) / (1 + t)
                  * (T21 * _misfit[0] + T22 * _misfit[1]);
  _stress_int(2,2) = -4 * _mu * t / (_kappa + 1) / (1 + t) / (1 + t)
                  * (T31 * _misfit[0] + T32 * _misfit[1]);
  ...
  _strain_int(0,0) = 1 / (_kappa + 1) / (1+t) / (1+t)
                  * (S11 * _misfit[0] + S12 * _misfit[1]);
  _strain_int(1,1) = 1 / (_kappa + 1) / (1+t) / (1+t)
                  * (S21 * _misfit[0] + S22 * _misfit[1]);

  _elastic_energy_int = 0.5 * _stress_int.doubleContraction(_strain_int);
}
