...
SphericalGaussianMaterial::validParams()
{
  ...
  params.addClassDescription("5D-gaussian-anisotropy material object for anisotropic grain growth. "
  ...
  params.addParam<Real>("gamma_value", 1.5, "Value of gamma used to specify an isotropic material propertie gamma (dimensionless)");
  params.addParam<Real>("g_gamma_value", 0.471404, "g_gamma value associated to gamma_value (dimensionless)");
  params.addParam<Real>("f_gamma_value", 0.124961, "f_gamma value associated to gamma_value (dimensionless)");
  params.addParam<Real>("grain_boundary_width", 40, "Grain boundary width (nm)");
  params.addParam<Real>("base_energy", 2.0, "Base value of energy used to compute base value of epsilon from which gaussian is either added or substracted (J/m^2)");
  params.addParam<Real>("base_gamma", 1.6, "Base value of gamma used to specify an anisotropic material propertie gamma (dimensionless)");
  params.addParam<Real>("base_g_gamma", 0.482570, "Base value of g_gamma associated to base_gamma from which gaussian is either added or substracted (dimensionless)");
  params.addParam<Real>("base_f_gamma", 0.130925, "Base value of f_gamma associated to base_gamma (dimensionless)");
  params.addParam<Real>("base_L", 0.0007, "Base value of L from which gaussian is either added or substracted (nm^3/eV.ns)");
  params.addParam<Real>("gaussian_tolerance", 1e-30, "Value used to control gaussian effect (dimensionless)");
  params.addParam<Real>("axis_variance_range", 3, "Acceptable range for axis variance for gaussian switch calculation (Radian)");
  params.addParam<Real>("angle_variance_range", 3, "Acceptable range for angle variance for gaussian switch calculation (Radian)");
  params.addParam<Real>("sharpness", 20, "Gaussian sharpness (dimensionless)");
  params.addParam<bool>("anisotropic_epsilon",true, "If true, use anisotropic epsilon; otherwise use isotropic epsilon");
  params.addParam<bool>("anisotropic_m", true, "If true, use anisotropic m; otherwise use isotropic m");
  params.addParam<bool>("anisotropic_gamma", true, "If true, use anisotropic gamma; otherwise use isotropic gamma");
  params.addParam<bool>("anisotropic_L", true, "If true, use anisotropic L; otherwise use isotropic L");
  params.addParam<bool>("add_epsilon_gaussian", true,
                                "If true, gaussian adds to the base value of epsilon, else subtract");
  params.addParam<bool>("add_g_gamma_gaussian",true,
                                "If true, gaussian adds to the base value of g_gamma, else subtract");
  params.addParam<bool>("add_L_gaussian", true, "If true, gaussian adds to the base value of L, else subtract");
  params.addParam<bool>("compute_final_gaussian_direction", true,
  ...
  params.addParam<bool>("set_bulk_to_base_values", true,
      "If true, the anisotropic material properties are at base values within the grains"
      "If false, they are at zero");
  params.addRequiredParam<int>("library_misorientations_number", "Number of minima misorientations in the minima library file to use");
...
void SphericalGaussianMaterial::readQuaternionFile()
{
  ...
  while (File1 >> quat0 >> quat1 >> quat2 >> quat3)
  {
    ...
    ADReal qcomp2 = quat2;
    ADReal qcomp3 = quat3;
    ADReal qcomp_norm = std::sqrt(qcomp0 * qcomp0 + qcomp1 * qcomp1 + qcomp2 * qcomp2 + qcomp3 * qcomp3);
...
void SphericalGaussianMaterial::readLibraryFile()
{
  ...
  {
    // Create vectors and normalize: Axis of rotation and Gaussian direction for library misorientation
    ...
    // Verify that grain boundary energy and pre-determined value of L for library misorientation are positive
    if (lib8 <= 0)
...
SphericalGaussianMaterial::computeQpProperties()
{
  ...
  const ADReal epsilon_base = 3.0 / 4.0 * _base_energy * _energy_scale * _length_scale * _length_scale * _grain_boundary_width;
  ...
  const ADReal m_value = std::sqrt((converted_base_energy * converted_base_energy) / (_grain_boundary_width * _grain_boundary_width *
                                                 _base_f_gamma * _base_g_gamma * _base_g_gamma));
  const ADReal epsilon_value = (converted_base_energy * converted_base_energy) / (_base_g_gamma * _base_g_gamma * m_value);
  ...
  // Term used to combine all the individually computed anisotropic properties between each pair of grains into single continuous functions
        ...
        // Compute individual anisotropic properties between each pairs of grains
          if (b == op_to_grains[m])
            if (a == op_to_grains[n])
            {
              ...
              // Compute normalized gradient vector of the order parameters that specifies the orientation of the grain boundary normal between grains A and B in the current simulation domain
              continuity_term = (100000.0 * ((*_vals[m])[_qp]) * ((*_vals[m])[_qp]) + 0.01) *
                    (100000.0 * ((*_vals[n])[_qp]) * ((*_vals[n])[_qp]) + 0.01);
              ADRealVectorValue simulation_boundary_normal = (*_grad_vals[m])[_qp] - (*_grad_vals[n])[_qp];
              ADReal normvsmallbavalue = simulation_boundary_normal.norm_sq();
              ADReal normvsmallba = simulation_boundary_normal.norm();
              ADRealVectorValue normalized_simulation_boundary_normal = simulation_boundary_normal.unit();

              // Compute the simulation normalized misorientation quaternion between the two grains using the orientation quaternions from the text file
              ...
              const ADReal norm_qba = std::sqrt(qba0_init * qba0_init + qba1_init * qba1_init + qba2_init * qba2_init + qba3_init * qba3_init);
              ...
              // Convert simulation normalized misorientation quaternion to simulation normalized axis angle notation
              const ADReal simulation_rotation_angle = 2.0 * std::acos(qba0);
              ADRealVectorValue simulation_rotation_axis(qba1 / std::sin(simulation_rotation_angle / 2.0),
              ...
              // Loop through the stored minima library data to compute gaussian for each minima library misorientation
              for (std::size_t l : make_range(std::size_t(0), _library_misorientations_number))
              {
                // Compute the gaussian switch by comparing the axis angle components of the minima library misorientation to the simulation misorientation
                if (_library_rotation_angle[l] < 0.0)
                  _library_rotation_angle[l] =
                       (2 * libMesh::pi) +
                      _library_rotation_angle[l]; // _library_rotation_angle is in radians. (2 * libMesh::pi) radians is equivalent to
                ...
                const ADReal switchvalue =
                    -100.0 * ((thetadv / _axis_variance_range) * (thetadv / _axis_variance_range) +
                ...
                if (exp_switch_value < 0.001) // 0.001 is an arbitrary threshold below which the
                  ...
                  // Retrive and assign the library gaussian direction for the specific minima library misorientation
                  _library_gaussian_direction[l] = _library_gaussian_direction[l].unit();
                  // Determine whether the library gaussian direction needs to be rotated into the simulation reference frame using quaternion orientations or left as such
                  ADRealVectorValue final_gaussian_direction(0.0, 0.0, 0.0);
                  if (_compute_final_gaussian_direction)
                  {
                    const ADReal miuo0 = (_q0[a] * 0.0) - (_q1[a] * _library_gaussian_direction[l](0)) -
                                         (_q2[a] * _library_gaussian_direction[l](1)) - (_q3[a] * _library_gaussian_direction[l](2));
                    const ADReal miuo1 = (_q0[a] * _library_gaussian_direction[l](0)) + (_q1[a] * 0.0) +
                                         (_q2[a] * _library_gaussian_direction[l](2)) - (_q3[a] * _library_gaussian_direction[l](1));
                    const ADReal miuo2 = (_q0[a] * _library_gaussian_direction[l](1)) - (_q1[a] * _library_gaussian_direction[l](2)) +
                                         (_q2[a] * 0.0) + (_q3[a] * _library_gaussian_direction[l](0));
                    const ADReal miuo3 = (_q0[a] * _library_gaussian_direction[l](2)) + (_q1[a] * _library_gaussian_direction[l](1)) -
                                         (_q2[a] * _library_gaussian_direction[l](0)) + (_q3[a] * 0.0);
                    final_gaussian_direction(0) = miuo0 * _q1[a] * -1.0 + miuo1 * _q0[a] + miuo2 * _q3[a] * -1.0 -
                                 miuo3 * _q2[a] * -1.0;
                    final_gaussian_direction(1) = miuo0 * _q2[a] * -1.0 - miuo1 * _q3[a] * -1.0 + miuo2 * _q0[a] +
                                 miuo3 * _q1[a] * -1.0;
                ...
                // Compute the gaussian and its derivatives by adding or substracting adequate values from base values based on the gaussian switch and directions at/surrounding library gaussian direction
                const ADReal dot_product =  final_gaussian_direction * normalized_simulation_boundary_normal;
                ...
                if (std::abs(normvsmallba) > tol)
                {
                  ...
                  gaussianplus = (_sharpness / normvsmallba) * (final_gaussian_direction - (dot_product *  normalized_simulation_boundary_normal));
                  gaussianminus = (-1) * (_sharpness / normvsmallba) * (final_gaussian_direction - (dot_product *  normalized_simulation_boundary_normal));

                  // Update values with computed gaussian and associated derivatives
                  if (_model_type == ModelType::EPSILON)  // If model_type is "EPSILON"
                  {
                    const ADReal amplitudes_epsilon =
                        std::abs((_base_energy - _library_minima_energy[l]) * _grain_boundary_width / 1.0e9 * std::sqrt(_f_gamma_value) / _g_gamma_value);
                    const ADReal common_epsilon = s_switch_value * exp_value * amplitudes_epsilon *_energy_scale * _length_scale;
                    const ADReal finalgaussian_epsilon = _add_epsilon_gaussian ? common_epsilon : -common_epsilon;
                    finalgaussianplus_epsilon = _add_epsilon_gaussian ? (gaussianplus * common_epsilon) : -(gaussianplus * common_epsilon);
                    finalgaussianminus_epsilon = _add_epsilon_gaussian ? (gaussianminus * common_epsilon) : -(gaussianminus * common_epsilon);
                  ...
                  else if (_model_type == ModelType::GAMMA)  // If model_type is "GAMMA"
                  {
                    const ADReal library_minima_g_gamma =
                        (_library_minima_energy[l] * _energy_scale * _length_scale * _length_scale) /
                        (std::sqrt(epsilon_value * m_value));
                    const ADReal amplitudes_g_gamma = std::abs(_base_g_gamma - library_minima_g_gamma);
                    const ADReal common_g_gamma = s_switch_value * exp_value * amplitudes_g_gamma;
                    const ADReal finalgaussian_g_gamma = _add_g_gamma_gaussian ? common_g_gamma : -common_g_gamma;
                    finalgaussianplus_g_gamma = _add_g_gamma_gaussian ? (gaussianplus * common_g_gamma) : -(gaussianplus * common_g_gamma);
                    finalgaussianminus_g_gamma = _add_g_gamma_gaussian ? (gaussianminus * common_g_gamma) : -(gaussianminus * common_g_gamma);
              ...
              // Sum up all gaussians and associated derivatives after looping through all the stored minima library data to make a continuous function
  ...
  if (_model_type == ModelType::EPSILON)  // If model_type is "EPSILON"
  {
    // Compute continuous epsilon, m and L and associated derivatives at grain boundaries by specifying anisotropy or isotropy
    ...
    if (_anisotropic_m)
    {
      ...
      _dm_plus[_qp] = _depsilon_plus[_qp] / (_grain_boundary_width * _grain_boundary_width * _f_gamma_value);
      _dm_minus[_qp] = _depsilon_minus[_qp] / (_grain_boundary_width * _grain_boundary_width * _f_gamma_value);
    }
    else
    {
      // Isotropic m
  ...
  {
    // Compute continuous gamma and L and associated derivatives at grain boundaries by specifying anisotropy or isotropy
    ...
    if (_anisotropic_gamma)
    {
      ...
      _gamma[_qp] = 1.0 / p;
      _dgamma_plus[_qp] =
          -2.0 * g_gamma* _gamma[_qp] * _gamma[_qp] * dpdg2* dg_gamma_plus;
      _dgamma_minus[_qp] =
          -2.0 * g_gamma * _gamma[_qp] * _gamma[_qp] * dpdg2* dg_gamma_minus;
    }
    else
    {
      // Isotropic gamma
