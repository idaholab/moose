...
DerivativeMultiPhaseBase::DerivativeMultiPhaseBase(const std::string & deprecated_name, InputParameters parameters) :
    ...
    _W(getParam<Real>("W"))
{
  // check passed in parameter vectors
  if (_num_fi != _num_hi)
    mooseError("Need to pass in as many hi_names as fi_names in DerivativeMultiPhaseBase " << deprecated_name);
