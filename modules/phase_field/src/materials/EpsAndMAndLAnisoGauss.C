...
// Material properties adding anisotropy to epsilon [or also called k], m and L.

#include "EpsAndMAndLAnisoGauss.h"
#include "MooseMesh.h"
#include "MathUtils.h"
...
registerMooseObject("PhaseFieldApp", EpsAndMAndLAnisoGauss);

InputParameters
EpsAndMAndLAnisoGauss::validParams()
{
  InputParameters params = ADMaterial::validParams();

  params.addClassDescription("Material to provide an anisotropy field and its derivatives wrt to "
                             "gradient of order parameter");

  params.addRequiredParam<MaterialPropertyName>(
      "eps_name", "The name of the aniso property produced by this class.");
  params.addRequiredParam<MaterialPropertyName>(
      "epsenergy_name", "The name of the aniso property produced by this class.");
  ...
  params.addRequiredParam<MaterialPropertyName>("depsdx_EN_name",
                                                "Derivative of eps in x direction");

  params.addRequiredParam<MaterialPropertyName>("depsdy_EN_name",
                                                "Derivative of  eps in y direction");

  params.addRequiredParam<MaterialPropertyName>("depsdz_EN_name",
                                                "Derivative of  eps in z direction");

  params.addRequiredParam<MaterialPropertyName>(
      "VwlibR_name", "The name of the library boundary normal in simulation Reference frame R");
  params.addRequiredParam<MaterialPropertyName>(
      "VxlibR_name",
      "The name of the library boundary normal in simulation Reference frame R in x direction");
  params.addRequiredParam<MaterialPropertyName>(
      "VylibR_name",
      "The name of the library boundary normal in simulation Reference frame R in y direction");
  params.addRequiredParam<MaterialPropertyName>(
      "VzlibR_name",
      "The name of the library boundary normal in simulation Reference frame R in z direction");
  params.addRequiredParam<MaterialPropertyName>(
      "Vx_name", "The name of normmaliized gradiient in x directtion");
  params.addRequiredParam<MaterialPropertyName>(
      "Vy_name", "The name of normmaliized gradiient in y directtion");
  params.addRequiredParam<MaterialPropertyName>(
      "Vz_name", "The name of normmaliized gradiient in z directtion");
  ...
  params.addRequiredParam<MaterialPropertyName>("dmdy_EN_name",
                                                "Derivative of  eps in y direction");
  params.addRequiredParam<MaterialPropertyName>("dmdz_EN_name",
                                                "Derivative of  eps in z direction");
  params.addRequiredParam<MaterialPropertyName>("dmdxplus_name",
                                                "Derivative of eps in x direction");
  params.addRequiredParam<MaterialPropertyName>("dmdyplus_name",
                                                "Derivative of  eps in y direction");
  params.addRequiredParam<MaterialPropertyName>("dmdzplus_name",
                                                "Derivative of  eps in z direction");
  params.addRequiredParam<MaterialPropertyName>("dmdxplus_EN_name",
                                                "Derivative of eps in x direction");
  params.addRequiredParam<MaterialPropertyName>("dmdyplus_EN_name",
                                                "Derivative of  eps in y direction");
  params.addRequiredParam<MaterialPropertyName>("dmdzplus_EN_name",
                                                "Derivative of  eps in z direction");

  params.addRequiredParam<MaterialPropertyName>("depsdxplus_name",
                                                "Derivative of eps in x direction");
  params.addRequiredParam<MaterialPropertyName>("depsdyplus_name",
                                                "Derivative of  eps in y direction");
  params.addRequiredParam<MaterialPropertyName>("depsdzplus_name",
                                                "Derivative of  eps in z direction");
  params.addRequiredParam<MaterialPropertyName>("depsdxplus_EN_name",
                                                "Derivative of eps in x direction");
  params.addRequiredParam<MaterialPropertyName>("depsdyplus_EN_name",
                                                "Derivative of  eps in y direction");
  params.addRequiredParam<MaterialPropertyName>("depsdzplus_EN_name",
                                                "Derivative of  eps in z direction");
  ...
  params.addRequiredParam<MaterialPropertyName>("mCalc_name", "The name of the calculated eps");

  params.addRequiredParam<MaterialPropertyName>("m_name", "The name of the aniso property miu");
  params.addRequiredParam<MaterialPropertyName>("m_energy_name",
                                                "The name of the aniso property miu");
  ...
  params.addRequiredParam<MaterialPropertyName>("epsbar_name",
                                                "The name of the average anisotropy");
  params.addRequiredParam<MaterialPropertyName>("epsmin2grains_name",
                                                "The name of the 2grains minimum epsilon");

  params.addRequiredParam<MaterialPropertyName>("TotGauss_name",
                                                "The total gaussian to add/remove");

  params.addRequiredParam<FileName>("Library_file_name",
                                    "Name of the file containing library quaternion data");

  params.addRequiredParam<FileName>("Quaternion_file_name",
                                    "Name of the file containing anisotropy data");
  ...
  params.addRequiredParam<bool>("ADDGaussian",
                                "If this is true gaussians add to eps_base, else subtract");
  ...
  params.addParam<Real>(
      ...
      "orientations quaternions, else it is taken directly from the minima library file");

  return params;
}

EpsAndMAndLAnisoGauss::EpsAndMAndLAnisoGauss(const InputParameters & parameters)
...
void
EpsAndMAndLAnisoGauss::computeQpProperties()
{
  ...
  // eps_base = ( (_sigmaBASE *_JtoeV * _length_scale * _length_scale) * _lgb *
  // (std::sqrt(f0saddle)) ) / (g_gamma) ;
        ...
        for (unsigned int n = m + 1; n < _op_num; ++n)
        {
          ...
          if (b == op_to_grains[m])
          {
            ...
            if (a == op_to_grains[n])
            {
              ...
              Val = (((*_vals[m])[_qp]) * ((*_vals[m])[_qp])) *
                    (((*_vals[n])[_qp]) * ((*_vals[n])[_qp]));
              ...
              ADReal qbaw = (qwR[a] * qwR[b]) - (qxR[a] * (-1) * qxR[b]) -
                            (qyR[a] * (-1) * qyR[b]) - (qzR[a] * (-1) * qzR[b]);
              ADReal qbax = (qwR[a] * qxR[b]) + (qxR[a] * (-1) * qwR[b]) +
                            (qyR[a] * (-1) * qzR[b]) - (qzR[a] * (-1) * qyR[b]);
              ADReal qbay = (qwR[a] * qyR[b]) - (qxR[a] * (-1) * qzR[b]) +
                            (qyR[a] * (-1) * qwR[b]) + (qzR[a] * (-1) * qxR[b]);
              ADReal qbaz = (qwR[a] * qzR[b]) + (qxR[a] * (-1) * qyR[b]) -
                            (qyR[a] * (-1) * qxR[b]) + (qzR[a] * (-1) * qwR[b]);
              ...
              for (unsigned int l = 0; l < _libnum; ++l)
              {
                ...
                ADReal miuvalue = (miubalibx[l] * miubalibx[l]) + (miubaliby[l] * miubaliby[l]) +
                                  (miubalibz[l] * miubalibz[l]);
                ...
                ADReal miuow = (qwR[a] * (0.0)) - (qxR[a] * miubalibx_norm) -
                               (qyR[a] * miubaliby_norm) - (qzR[a] * miubalibz_norm);
                ADReal miuox = (qwR[a] * miubalibx_norm) + (qxR[a] * (0.0)) +
                               (qyR[a] * miubalibz_norm) - (qzR[a] * miubaliby_norm);
                ADReal miuoy = (qwR[a] * miubaliby_norm) - (qxR[a] * miubalibz_norm) +
                               (qyR[a] * (0.0)) + (qzR[a] * miubalibx_norm);
                ADReal miuoz = (qwR[a] * miubalibz_norm) + (qxR[a] * miubaliby_norm) -
                               (qyR[a] * miubalibx_norm) + (qzR[a] * (0.0));

                // Determine whether the boundary normal is taken from the miinima library or
                // compuuted from orientation quaterniionss
                ...
                {
                  miubalibwR = ((miuow) * (qwR[a])) - ((miuox) * (qxR[a]) * (-1)) -
                               ((miuoy) * (qyR[a]) * (-1)) - ((miuoz) * (qzR[a]) * (-1));
                  miubalibxR = ((miuow) * (qxR[a]) * (-1)) + ((miuox) * (qwR[a])) +
                               ((miuoy) * (qzR[a]) * (-1)) - ((miuoz) * (qyR[a]) * (-1));
                  miubalibyR = ((miuow) * (qyR[a]) * (-1)) - ((miuox) * (qzR[a]) * (-1)) +
                               ((miuoy) * (qwR[a])) + ((miuoz) * (qxR[a]) * (-1));
                  miubalibzR = ((miuow) * (qzR[a]) * (-1)) + ((miuox) * (qyR[a]) * (-1)) -
                               ((miuoy) * (qxR[a]) * (-1)) + ((miuoz) * (qwR[a]));
                }
                ...
                // calculate the switch for a specific pair of grains and each library
                // misorientation
                ...
                ADReal normvbalibvalue = (vbalibx[l] * vbalibx[l]) + (vbaliby[l] * vbaliby[l]) +
                                         (vbalibz[l] * vbalibz[l]);

                ADReal normvbalib = std::pow(normvbalibvalue, .5);

                ADReal cosinevalue1 =
                    ((vbalibx[l]) * vbax) + ((vbaliby[l]) * vbay) + ((vbalibz[l]) * vbaz);
                ...
                ADReal switchvalue =
                    (-100) * (((thetadv / _alphaswitch) * (thetadv / _alphaswitch)) +
                ...
                ADReal amplitudes =
                    ((_sigmaBASE - (MinimaEnergy[l])) * (_lgb / 1.0e9) * (std::sqrt(f0saddle))) /
                    g_gamma;
                ...
                if (std::fabs(normvsmallba) > tol) // if R < tol
                {
                  ...
                  ADReal Gaussian = _JtoeV * _length_scale * _amplitudeScale * amplitudes *
                                    _S_switch[_qp] * std::exp(exponent);
                  ...
                  ADReal Gaussian2 = _JtoeV * _length_scale * _amplitudeScale * amplitudes *
                                     _S_switch[_qp] * _sharpness * std::exp(exponent) *
                                     (((((-1) * miubalibxR * normvsmallba) -
                                        (miubalibxR * vsmalbax * (1 / (2 * normvsmallba)) *
                                         ((-1) * 2 * vsmalbax))) /
                                       normvsmallbavalue) +
                                      (((-1) * (miubalibyR * vsmalbay) *
                                        ((1 / (2 * normvsmallba)) * (2 * (-1) * vsmalbax))) /
                                       normvsmallbavalue) +
                                      (((-1) * (miubalibzR * vsmalbaz) *
                                        ((1 / (2 * normvsmallba)) * (2 * (-1) * vsmalbax))) /
                                       normvsmallbavalue));
                  Gaussian2 = _ADDGaussian ? Gaussian2 : -Gaussian2; // yes = add, false = subtract

                  ADReal Gaussian3 = _JtoeV * _length_scale * _amplitudeScale * amplitudes *
                                     _S_switch[_qp] * _sharpness * std::exp(exponent) *
                                     (((((-1) * miubalibyR * normvsmallba) -
                                        (miubalibyR * vsmalbay * (1 / (2 * normvsmallba)) *
                                         ((-1) * 2 * vsmalbay))) /
                                       normvsmallbavalue) +
                                      (((-1) * (miubalibxR * vsmalbax) *
                                        ((1 / (2 * normvsmallba)) * (2 * (-1) * vsmalbay))) /
                                       normvsmallbavalue) +
                                      (((-1) * (miubalibzR * vsmalbaz) *
                                        ((1 / (2 * normvsmallba)) * (2 * (-1) * vsmalbay))) /
                                       normvsmallbavalue));
                  ...
                  ADReal Gaussian4 = _JtoeV * _length_scale * _amplitudeScale * amplitudes *
                                     _S_switch[_qp] * _sharpness * std::exp(exponent) *
                                     (((((-1) * miubalibzR * normvsmallba) -
                                        (miubalibzR * vsmalbaz * (1 / (2 * normvsmallba)) *
                                         ((-1) * 2 * vsmalbaz))) /
                                       normvsmallbavalue) +
                                      (((-1) * (miubalibxR * vsmalbax) *
                                        ((1 / (2 * normvsmallba)) * (2 * (-1) * vsmalbaz))) /
                                       normvsmallbavalue) +
                                      (((-1) * (miubalibyR * vsmalbay) *
                                        ((1 / (2 * normvsmallba)) * (2 * (-1) * vsmalbaz))) /
                                       normvsmallbavalue));
                  ...
                  ADReal Gaussian2plus = _JtoeV * _length_scale * _amplitudeScale * amplitudes *
                                         _S_switch[_qp] * _sharpness * std::exp(exponent) *
                                         (((((1) * miubalibxR * normvsmallba) -
                                            (miubalibxR * vsmalbax * (1 / (2 * normvsmallba)) *
                                             ((1) * 2 * vsmalbax))) /
                                          ...
                                          (((-1) * (miubalibzR * vsmalbaz) *
                                           ...
                                           normvsmallbavalue));
                  Gaussian2plus =
                      _ADDGaussian ? Gaussian2plus : -Gaussian2plus; // yes = add, false = subtract
                  ADReal Gaussian3plus = _JtoeV * _length_scale * _amplitudeScale * amplitudes *
                                         _S_switch[_qp] * _sharpness * std::exp(exponent) *
                                         (((((1) * miubalibyR * normvsmallba) -
                                            (miubalibyR * vsmalbay * (1 / (2 * normvsmallba)) *
                                             ((1) * 2 * vsmalbay))) /
                                          ...
                                          (((-1) * (miubalibzR * vsmalbaz) *
                                           ...
                                           normvsmallbavalue));
                  Gaussian3plus =
                      _ADDGaussian ? Gaussian3plus : -Gaussian3plus; // yes = add, false = subtract
                  ADReal Gaussian4plus = _JtoeV * _length_scale * _amplitudeScale * amplitudes *
                                         _S_switch[_qp] * _sharpness * std::exp(exponent) *
                                         (((((1) * miubalibzR * normvsmallba) -
                                            (miubalibzR * vsmalbaz * (1 / (2 * normvsmallba)) *
                                             ((1) * 2 * vsmalbaz))) /
                                          ...
                                          (((-1) * (miubalibyR * vsmalbay) *
                                           ...
                                           normvsmallbavalue));
                  Gaussian4plus =
                      _ADDGaussian ? Gaussian4plus : -Gaussian4plus; // yes = add, false = subtract
                ...
                eps_min = ((MinimaEnergy[l] * _JtoeV * _length_scale * _length_scale) * _lgb *
                           (std::sqrt(f0saddle))) /
                          (g_gamma);
  ...
  _L_energy[_qp] = MetaPhysicL::raw_value(sumL / sumval);
  ...
  {
    _L[_qp] = _L_BASE;
    _L_energy[_qp] = MetaPhysicL::raw_value(_L_BASE * 1);

    _eps[_qp] = eps_base;
    _epsenergy[_qp] = MetaPhysicL::raw_value(
        (3.0 / 4.0) * (_sigmaBASE * _JtoeV * _length_scale * _length_scale) * _lgb);
