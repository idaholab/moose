...
EpsAndMAndLAnisoGauss::validParams()
{
  ...
  params.addParam<MaterialPropertyName>("eps_name", "eps", "The name of the anisotropic epsilon - AD.");
  params.addParam<MaterialPropertyName>("eps_EN_name", "kappa_op", "The name of the anisotropic epsilon - NoAD.");
  params.addParam<MaterialPropertyName>("deps_minus_name", "deps_minus", "Negative derivatives of epsilon - AD.");
  params.addParam<MaterialPropertyName>("deps_plus_name","deps_plus", "Positive derivatives of epsilon - AD.");
  ...
  params.addParam<MaterialPropertyName>("dm_minus_name","dm_minus", "Negative derivatives of m - AD.");
  params.addParam<MaterialPropertyName>("dm_plus_name", "dm_plus", "Positive derivatives of m - AD.");
  ...
  params.addRequiredParam<Real>("L_BASE", "Value of L base in nm^3/eV.ns.");
  ...
  params.addRequiredParam<bool>("ADDGaussian", "If true, Gaussians add to eps_base, else subtract.");
  params.addRequiredParam<bool>("ADDGaussianL", "If true, Gaussians add to L_BASE, else subtract.");
...
void
EpsAndMAndLAnisoGauss::readQuaternionFile()
{
...
void
EpsAndMAndLAnisoGauss::readLibraryFile()
{
...
EpsAndMAndLAnisoGauss::computeQpProperties()
{
  ...
  ADReal sumL = 0.0, sumepsxplus = 0.0, sumepsyplus = 0.0, sumepszplus = 0.0;
  ADReal aniso = 0.0, anisox = 0.0, anisoy = 0.0, anisoz = 0.0;
  ADReal anisoxplus = 0.0, anisoyplus = 0.0, anisozplus = 0.0, anisoL = 0.0;
          ...
          if (b == op_to_grains[m])
            if (a == op_to_grains[n])
            {
              ...
              for (unsigned int l : make_range(0U, _libnum))
              {
                ...
                if (exp_switch_value < 0.001) // 0.001 is an arbitrary threshold below which the switch is assumed negligible.
                  _S_switch[_qp] = 0.0;
                const ADReal exponent = _sharpness * (dot_product - 1.0);
                const ADReal amplitudes = (_sigmaBASE - _MinimaEnergy[l]) * _lgb / 1.0e9 * std::sqrt(f0saddle) / g_gamma;
                ...
                if (std::fabs(normvsmallba) > tol)
                {
                  ...
                  const ADReal gaussian_value = _JtoeV * _length_scale * _amplitudeScale * amplitudes;
  ...
  {
    _L[_qp] = _L_BASE;
    _L_EN[_qp] = MetaPhysicL::raw_value(_L_BASE * 1.0);
    _eps[_qp] = eps_base;
    _eps_EN[_qp] = MetaPhysicL::raw_value(3.0 / 4.0 * _sigmaBASE * _JtoeV * _length_scale * _length_scale * _lgb);
