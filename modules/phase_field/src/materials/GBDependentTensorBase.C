...
InputParameters validParams<GBDependentTensorBase>()
{
  ...
  params.addParam<MaterialPropertyName>("gb_normal_tensor_name", "Name of GB normal tensor property");
...
GBDependentTensorBase::GBDependentTensorBase(const InputParameters & parameters) :
    ...
    _gb_dependent_tensor(declareProperty<RealTensorValue>(getParam<MaterialPropertyName>("gb_tensor_prop_name")))
{
}
