...
DerivativeSumMaterial::DerivativeSumMaterial(const std::string & deprecated_name, InputParameters parameters) :
  ...
  // we need at least one material in the sum
  if (_num_materials == 0)
    mooseError("Please supply at least one material to sum in DerivativeSumMaterial " << deprecated_name);
