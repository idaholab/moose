...
EulerAngleUpdaterCheck::initialize()
{
  ...
  for (unsigned int i = 0; i < grain_num; ++i)
  {
    ...
    torque_rot1(0) = torque_rot(2); // Tourque about z changed to torque responsible for chaneg in angle phi1
    ...
    // Difference between the final positions of the rotated vector obtained in two different ways, should be 0.0
