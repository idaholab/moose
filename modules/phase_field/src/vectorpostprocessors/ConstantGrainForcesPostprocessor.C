...
InputParameters validParams<ConstantGrainForcesPostprocessor>()
{
  ...
  params.addClassDescription("Outputs the values from ConstantGrainForcesPostprocessor");
  params.addParam<UserObjectName>("grain_force","Specify userobject that gives center of mass and volume of grains");
  return params;
}

ConstantGrainForcesPostprocessor::ConstantGrainForcesPostprocessor(const InputParameters & parameters) :
    GeneralVectorPostprocessor(parameters),
    _grain_force_torque_vector(declareVector("grain_force_torque_vector")),
    _const_grain_force_torque(getUserObject<ConstantGrainForceAndTorque>("grain_force")),
    _grain_forces(_const_grain_force_torque.getForceValues()),
    _grain_torques(_const_grain_force_torque.getTorqueValues()),
    _grain_force_derivatives(_const_grain_force_torque.getForceDerivatives()),
    _grain_torque_derivatives(_const_grain_force_torque.getTorqueDerivatives()),
...
void
ConstantGrainForcesPostprocessor::execute()
{
