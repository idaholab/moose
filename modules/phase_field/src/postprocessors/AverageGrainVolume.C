...
InputParameters validParams<AverageGrainVolume>()
{
  ...
  // Mode 2: Calculate grain volumes directly
...
AverageGrainVolume::AverageGrainVolume(const InputParameters & parameters) :
    ...
    _feature_counter(isParamValid("feature_counter") ?
                   &getUserObject<FeatureFloodCount>("flood_counter") : nullptr)
{
  if (!_feature_counter)
  {
    if (isParamValid("v") && isParamValid("grain_num"))
    {
      ...
      _feature_volumes.resize(_nvals);

      // Build a reflexive map (ops map to grains directly)
      _static_var_to_feature.resize(_nvals);
      for (auto i = beginIndex(_static_var_to_feature); i < _nvals; ++i)
        _static_var_to_feature[i] = i;
    }
    else
      mooseError("Must supply either a flood_counter object or coupled variables and grain_num");
  }
}
...
AverageGrainVolume::execute()
{
  ...
  for (auto el = _mesh.getMesh().active_local_elements_begin(); el != end; ++el)
  {
    ...
    const std::vector<unsigned int> & var_to_feature_ptr = _feature_counter ?
      _feature_counter->getVarToFeatureVector(elem->id()) :
