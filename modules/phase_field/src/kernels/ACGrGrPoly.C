...
validParams<ACGrGrPoly>()
{
  ...
  params.addParam<unsigned int>("second_phase_op_num", 0, "specifies the total number of grains/variants of the second phase");
  params.addParam<Real>("en_ratio", 1.0, "Ratio of surface energy to GB energy, e.g., interphase energy");
  params.addParam<Real>("second_phase_en_ratio", 1.0, "Ratio of second-phase to parent-phase GB energy");
...
ACGrGrPoly::computeDFDOP(PFFunctionType type)
{
  ...
  // Calculate either the residual or Jacobian of the two-phase or single-phase grain growth free energy
  switch (type)
  {
    case Residual:
    {
      ...
      if (_second_phase_op_num > 0)
      {
        ...
        if (_op_index > _op_num - _second_phase_op_num)
          return _mob_ratio * _mu[_qp] *
                     (_u[_qp] * _u[_qp] * _u[_qp] - _u[_qp] + 2.0 * _second_phase_en_ratio * _gamma[_qp] * _u[_qp] * SumEtak + 2.0 * _en_ratio * _gamma[_qp] * _u[_qp] * (SumEtaj - SumEtak)) +
                 tgrad_correction;
        // parent-phase
        else
          return _mu[_qp] *
                     (_u[_qp] * _u[_qp] * _u[_qp] - _u[_qp] + 2.0 * _gamma[_qp] * _u[_qp] * SumEtak + 2.0 * _en_ratio * _gamma[_qp] * _u[_qp] * (SumEtaj - SumEtak)) +
                 tgrad_correction;
      }
      // single-phase case
      return _mu[_qp] *
    ...
    case Jacobian:
    {
      ...
      if (_second_phase_op_num > 0)
      {
        ...
        if (_op_index > _op_num - _second_phase_op_num)
          return  _mob_ratio * _mu[_qp] *
                     (_phi[_j][_qp] * (3.0 * _u[_qp] * _u[_qp] - 1.0  + 2.0 * _second_phase_en_ratio * _gamma[_qp] * SumEtak + 2.0 * _en_ratio * _gamma[_qp] * (SumEtaj - SumEtak))) +
                 tgrad_correction;
        // parent-phase
        else
          return _mu[_qp] *
                         (_phi[_j][_qp] * (3.0 * _u[_qp] * _u[_qp] - 1.0  + 2.0 * _gamma[_qp] * SumEtak + 2.0 * _en_ratio * _gamma[_qp] * (SumEtaj - SumEtak))) +
                     tgrad_correction;
      }
      // single-phase case
      return _mu[_qp] *
      ...
      if (_second_phase_op_num > 0)
      {
        ...
        if (_op_index > _op_num - _second_phase_op_num)
        {
          if (jvar >= _op_num - _second_phase_op_num)
            return _mob_ratio * _L[_qp] * _test[_i][_qp] * _mu[_qp] * 4.0 * _second_phase_en_ratio * _gamma[_qp] * _u[_qp] * (*_vals[i])[_qp] * _phi[_j][_qp];
          else
            return _mob_ratio * _L[_qp] * _test[_i][_qp] * _mu[_qp] * 4.0 * _en_ratio * _gamma[_qp] * _u[_qp] * (*_vals[i])[_qp] * _phi[_j][_qp];
        }
        // parent-phase
        else
        {
          if (jvar >= _op_num - _second_phase_op_num)
            return _L[_qp] * _test[_i][_qp] * _mu[_qp] * 4.0 * _gamma[_qp] * _u[_qp] * (*_vals[i])[_qp] * _phi[_j][_qp];
          else
            return _L[_qp] * _test[_i][_qp] * _mu[_qp] * 4.0 * _en_ratio * _gamma[_qp] * _u[_qp] * (*_vals[i])[_qp] * _phi[_j][_qp];
        }
      }
       // single-phase case
