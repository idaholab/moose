...
template <>
InputParameters
validParams<TwoPhaseACGrGrPoly>()
{
...
TwoPhaseACGrGrPoly::TwoPhaseACGrGrPoly(const InputParameters & parameters)
...
Real
TwoPhaseACGrGrPoly::computeDFDOP(PFFunctionType type)
{
...
Real
TwoPhaseACGrGrPoly::computeQpOffDiagJacobian(unsigned int jvar)
{
  for (unsigned int i = 0; i < _op_num; ++i)
    if (jvar == _vals_var[i])
    {
