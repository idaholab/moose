...
#include "ADACInterface2.h"

registerADMooseObject("PhaseFieldApp", ADACInterface2);

defineADValidParams(
    ADACInterface2, ADKernel, params.addClassDescription("Gradient energy Allen-Cahn Kernel");
    ...
    params.addParam<bool>("variable_L",
                          ...
                          "entire domain!)");
    params.addParam<bool>("extra_term",0,"For the extra term when kappa is not a constant, this is set to true");
    params.addParam<MaterialPropertyName>("dkappadgrad_op_name",0,"The derivative of kappa with respect to grad_op"););

template <ComputeStage compute_stage>
ADACInterface2<compute_stage>::ADACInterface2(const InputParameters & parameters)
  : ADKernel<compute_stage>(parameters),
    ...
    _dkappadgrad_op(getADMaterialProperty<RealGradient>("dkappakgrad_op_name"))
{
...
template <ComputeStage compute_stage>
ADReal
ADACInterface2<compute_stage>::computeQpResidual()
{
  ...
  if(_extra_term == 0)
    ...
    return _dkappadgrad_op[_qp] * _grad_u[_qp] * _grad_u[_qp] * nabla_Lpsi / 2;
}

adBaseClass(ADACInterface2);
