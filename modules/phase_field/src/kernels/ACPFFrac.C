...
InputParameters validParams<ACPFFrac>()
{
  ...
  params.addParam<MaterialPropertyName>("gc_prop_var", "gc_prop", "Material property name with gc value");
...
ACPFFrac::ACPFFrac(const InputParameters & parameters):
...
Real
ACPFFrac::computeQpResidual()
{
  Real x = (_l[_qp] * _beta[_qp] - _dFdc[_qp] / _gc_prop[_qp] - _u[_qp] / _l[_qp]) * _test[_i][_qp];

  return - ( std::abs(x) + x ) / 2.0 / _visco[_qp];
}

Real
ACPFFrac::computeQpJacobian()
{
  Real x = (_l[_qp] * _beta[_qp] - _dFdc[_qp] / _gc_prop[_qp] - _u[_qp] / _l[_qp]) * _test[_i][_qp];

  Real dx = (_d2Fdc2[_qp] - 1.0 / _l[_qp]) * _phi[_j][_qp] * _test[_i][_qp];

  return - (MathUtils::sign(x) + 1.0) / 2.0 * dx / _visco[_qp];
}

Real
ACPFFrac::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real x = (_l[_qp] * _beta[_qp] - _dFdc[_qp] / _gc_prop[_qp] - _u[_qp] / _l[_qp]) * _test[_i][_qp];
  ...
  if (jvar == _beta_var)
    ...
    for (unsigned int c_comp = 0; c_comp < _ndisp; ++c_comp)
      ...
      {
        Real dxddFdc = - 1.0 / _gc_prop[_qp] * _test[_i][_qp];

        Real d2Fdcdstrain_comp = (_d2Fdcdstrain[_qp].column(c_comp) + _d2Fdcdstrain[_qp].row(c_comp)) / 2.0 * _grad_phi[_j][_qp];
