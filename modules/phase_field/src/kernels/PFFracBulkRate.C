...
InputParameters validParams<PFFracBulkRate>()
{
  ...
  params.addParam<unsigned int>("component", "An integer corresponding to the direction the variable this kernel acts in. (0 for x, 1 for y, 2 for z)");
...
PFFracBulkRate::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  if (disp_flag && _dG0_pos_dstrain != NULL)
  {
    ...
    for (unsigned int i = 0; i < LIBMESH_DIM; ++i)
      val +=  ((*_dG0_pos_dstrain)[_qp](c_component,i) + (*_dG0_pos_dstrain)[_qp](i,c_component))/2.0 * _grad_phi[_j][_qp](i);
