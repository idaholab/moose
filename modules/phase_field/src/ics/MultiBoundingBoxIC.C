...
InputParameters validParams<MultiBoundingBoxIC>()
{
  ...
  params.addClassDescription("Specify variable values inside and outside a list of box shaped axis-aligned regions defined by pairs of opposing corners");
  params.addRequiredParam<std::vector<Point> >("corners", "The corner coordinates boxes");
  params.addRequiredParam<std::vector<Point> >("opposite_corners", "The coordinates of the opposite corners of the boxes");
  params.addRequiredParam<std::vector<Real> >("inside", "The value of the variable inside each box (one value per box or a single value for all boxes)");
