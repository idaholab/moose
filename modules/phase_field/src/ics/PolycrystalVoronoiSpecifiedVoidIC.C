...
InputParameters
PolycrystalVoronoiSpecifiedVoidIC::actionParameters()
{
  InputParameters params = ::validParams<SpecifiedSmoothCircleIC>();
...
registerMooseObject("PhaseFieldApp", PolycrystalVoronoiSpecifiedVoidIC);
    ...
    _file_name(getParam<FileName>("file_name"))
{
  if (_invalue < _outvalue)
    mooseError("PolycrystalVoronoiSpecifiedVoidIC requires that the voids be "
               "represented with invalue > outvalue");
}

void
PolycrystalVoronoiSpecifiedVoidIC::initialSetup()
{
  ...
  // Call initial setup from SpecifiedSmoothCircleIC to create Multiple smooth
  // circles with manually specified radii and center points
  SpecifiedSmoothCircleIC::initialSetup();
}

Real
PolycrystalVoronoiSpecifiedVoidIC::value(const Point & p)
{
  Real value = 0.0;

  // Determine value for voids
  Real void_value = SpecifiedSmoothCircleIC::value(p);
...
RealGradient
PolycrystalVoronoiSpecifiedVoidIC::gradient(const Point & p)
{
  RealGradient gradient;
  RealGradient void_gradient = SpecifiedSmoothCircleIC::gradient(p);
