...
// // Convenience function for sizing a vector to "n" given a vector with size 1 or "n"
  std::vector<Real>
  static sizeVector(std::vector<Real> v, std::size_t size);
// {
//   if (v.size() == 1)
...
validParams<NestedBoundingBoxIC>()
{
  ...
  params.addRequiredParam<std::vector<Point>>("smaller_coordinate_corners", "For 1D, these are the left points; for 2D, these are the bottom left corners; for 3D, these are the back bottom left corners. The format is (corner1_x corner1_y corner1_z corner2_x corner2_y corner2_z ...), starting from the innermost box to the outermost box");
  params.addRequiredParam<std::vector<Point>>(
      "larger_coordinate_corners", "For 1D, these are the right points; for 2D, these are the top right corners; for 3D, these are the front top right corners. The format is (corner1_x corner1_y corner1_z corner2_x corner2_y corner2_z ...), starting from the innermost box to the outermost box");
  ...
  else
  {
    for (unsigned int b = 0; b < _nbox; ++b)
    {
      if (_c1[b](0) < _c2[b](0) && p(0) >= _c1[b](0) - _int_width && p(0) <= _c2[b](0) + _int_width)
        if (_dim <= 1 || (_c1[b](1) < _c2[b](1) && p(1) >= _c1[b](1) - _int_width && p(1) <= _c2[b](1) + _int_width))
          if (_dim <= 2 || (_c1[b](2) < _c2[b](2) && p(2) >= _c1[b](2) - _int_width && p(2) <= _c2[b](2) + _int_width))
          {
            Real f_in = 1.0;
