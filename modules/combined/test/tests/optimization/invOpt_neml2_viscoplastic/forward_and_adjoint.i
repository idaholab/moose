...
[Mesh]
  ...
  []
  # Block b occupies z in [3*b, 3*b + 1]. The gap keeps the blocks disjoint, so
  # each one holds its own homogeneous uniaxial state.
  [stack]
...
[NEML2]
  ...
  [all]
    model = 'model'
    # parameter_types is ignored - the MOOSE-side type is inferred from the
    # parameter name - but its length must match parameters.
...
[Materials]
  ...
  []
  # The executioner assembles the adjoint operator as the transpose of the
  # forward Jacobian, so the adjoint variables carry no kernels. This supplies
  # only the strain the gradient inner products contract against.
  [adjoint_strain]
...
# The MOOSE-side type of a NEML2 parameter is inferred from its name, so each
# Function name must match its NEML2 parameter name exactly.
#
# The optimizer works in NORMALIZED parameters that start at 1; the coefficient
# in each expression is that parameter's SCALE, converting back to the physical
# value NEML2 consumes. The scales are the starting guess in main.i, so the two
# must be changed together. ParsedOptimizationFunction (not ParsedFunction) is
# required: its parameterGradient supplies the d(physical)/d(normalized) factor
# that the gradient inner product multiplies in.
[Functions]
  [elasticity_E]
...
[Reporters]
  ...
  []
  # Normalized starting guess, i.e. the physical guess divided by its own scale.
  # The optimization driver overwrites these values.
  [params]
...
[Executioner]
  ...
  # Computing them together overwrites the DiracKernel residual.
