...
(`EigenProblem`). Prior to the addition of this `Action`, using a
custom Problem required users to declare a `Problem` block in their
input file with a custom `type =` field, and this process has now been
streamlined.  A test of this new capability has been added in
