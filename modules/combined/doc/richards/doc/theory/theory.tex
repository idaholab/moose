...
document describes the theoretical and numerical foundations of
Wallaby.  Examples of Wallaby applications can be found in the
accompanying Wallaby Examples document.

%%%
\chapter{Governing Equations}
%%%
The Wallaby governing DE is Richards' equation~\cite{richards1931}.
This describes the movement of fluid through the connected pore space
of a porous medium.  The equation is
\begin{equation}
\phi \frac{\partial}{\partial t} \left( \rho S \right) = \nabla_{i}
\left( \frac{\rho \kappa_{ij}\kappa_{\mathrm{rel}}}{\mu} (\nabla_{j}P - \rho_{0} g_{j}) \right)
+ F \ ,
\label{richards.eqn}
\end{equation}
where the independent variable is $P$, the fluid pressure.  On the
...
\item $\phi$ is the porosity of the medium (dimensionless).  It is a
  material property (independent of the fluid pressure).  It is assumed
...
\item $\rho$ is the fluid density (measured in mass.length$^{-3}$,
  usually kg.m$^{-3}$).  $\rho_{0}$ is a reference constant reference
  density\footnote{A more physically precise version of
    Eqn~(\ref{richards.eqn}) is obtained by replacing $\rho_{0}$ with
    $\rho$.  Because most models use fluids with $\rho \sim \rho_{0}$
    over the pressure ranges encountered, it is more usual to use
    $\rho_{0}$, even though this is strictly not correct.  It would be
  very simple to implement this generalisation in MOOSE}.  $\rho$ is
  typically a function of pressure, and these two
...
\item $S$ is the fluid saturation (dimensionless).  It is bounded,
  ...
  with the fluid.  (The pore volume available to the fluid is just
  $\phi V$ where $V$ is the volume of the medium.)  The term ``fully
  ...
  actually be air, but is convenient to call it so).  This air is free
  ...
  dynamics of the fluid.  $S$ is a function of fluid pressure and is
...
\item $\mu$ is the fluid's dynamic viscosity (measured
  ...
  a function of pressure, Wallaby considers it constant.  A typical
  value for liquid water is $10^{-3}$\,Pa.s.
\item $P$ is the fluid porepressure (measured in Pa).  It is the
  independent variable.
\item ${\mathbf{g}}$ is the acceleration of gravity (measured
  in m.s$^{-2}$) as a vector pointing ``downwards''.  It is constant.
  For instance
...
bounds, $0\leq S\leq 1$, in practice it is often found that
\begin{equation}
...
much fluid as possible.  What happens is that the fluid becomes
...
the ``infinite negative pressure''.  However, further fluid may be
...
Richard's equation is only valid for $0\leq S_{\mathrm{eff}} \leq 1$,
so that in many practical simulations, modellers often set
$S_{\mathrm{res}} = 0 = S_{\mathrm{air}}$, but nevertheless Wallaby
allows a nonzero value for these parameters.  It is a moot point as to
whether $S_{\mathrm{res}}=0=S_{\mathrm{air}}$, because experiments are
hard to perform.  What is of more practical use is the ``immobile
saturation'', $S_{\mathrm{imm}}$, below which the relative permeability
is zero, and that is discussed further in Section~\ref{rel.perm.sec}.
(The physical reasons for the relative permeability going to zero below
$S_{\mathrm{imm}}$ are probably those given above: the continuous film
of fluid breaking down in the presence of large negative porepressures.)


\section{Density}
\label{sec.density.constk}

It is assumed that the fluid bulk modulus is constant.  This means
that the fluid density is
\begin{equation}
\rho = \rho_{0}e^{P/K} \ .
\end{equation}
In this expression
...
\end{equation}
in terms of the air pressure and the fluid porepressure.
$P_{\mathrm{air}}$ is deemed to be constant for the single-phase
situation described here.  In the two-phase case, it is the second
variable, and this will be discussed elsewhere.

For a wetting fluid (compared with air), $P_{\mathrm{c}} > 0$, and
...
have been proposed and used, and the most common is due to van
...
\item Both $S_{\mathrm{eff}}\rightarrow 1$ and
  ...
  $\mathrm{d}S/\mathrm{d}P$ around full saturation.  Also
  ...
  Jacobian\footnote{It appears explicitly from the time derivative in
    ...
    mass-balance characteristics so it is not used in Wallaby, but
...
\caption{$\alpha P_{c}$ as a function of $S_{\mathrm{eff}}$ as given
  ...
  of $m$ are shown: 0.5, 0.7 and 0.9.}
...
As suggested in Figure~\ref{van_genuchten_krel.fig}, $\mathrm{d}k_{\mathrm{rel}}/d P
\rightarrow 0$ as $P\rightarrow 0$ if $m>0.5$.  This is advantageous
numerically when using an implicit time-stepping solution technique,
since then both $k_{\mathrm{rel}}$ and its pressure-derivative are
continuous around full saturation.  Once again, I encourage users to
set $m>0.5$.
...
literature specifies the value of $m$.  In this case, it is extremely
advantageous to modify the relative permeability curve close to
$S_{\mathrm{eff}}=1$ to ensure continuity of its pressure derivative
in that region.  This is standard practice in other codes too.
Wallaby allows users to specify a relative permeability function of
type {\tt vg\_1}, and specify a cutoff in $\tilde{S}$, {\tt
  vg\_1\_cutoff}, above which the relative permeability is approximated
by a cubic:
\begin{equation}
...
monotonically increasing\footnote{This is true, since the
  van Genuchten $\kappa_{\mathrm{rel}}$ and its first and second
  derivatives are positive at all values of the cutoff (assuming it is
  greater than zero), and the first derivative of the cubic is
  positive at $\tilde{S}=1$.  Because cubics have at most 2 turning
...
users can easily choose $n$ large to ensure that
...
are of particular interest in groundwater flow, which have been coded
into Wallaby.  This chapter details these.  $F$ is a volumetric source
(measured in mass.length$^{-3}$.time$^{-1}$), but many of the sources
...
implemented in Wallaby by making $F$ (the surface source) a
...
surface doesn't rise appreciably above atmospheric pressure.  Then
\begin{equation}
F = \left\{
\begin{array}{ll}
...
PETSc will find it difficult to invert the stiffness matrix.  A
suitable choice is
\begin{equation}
...
In the MOOSE input file, Eqn~(\ref{rain.rechage.eqn}) is implemented
by specifying the pressure tuple $(P_{\mathrm{atm}},
P_{\mathrm{big}})$, and the flux tuple $(-3.17E-7, -3.17E-7 +
C(P_{\mathrm{big}}-P_{\mathrm{atm}}))$, for porepressure
$P_{\mathrm{big}}$ that is a little bigger than the modeller expects
to see in the model.
...
-\frac{1}{2}\left(\frac{P-P_{c}}{\sigma}\right)^{2} \right) &
\ \ \ \mbox{for } \ \ P<P_{c} \\
-E_{\mathrm{max}} & \ \ \ \mbox{for } \ \ P\geq P_{c}
\end{array}
\right.
...
Eqn~(\ref{half.gaussian.et}) also contains $P_{c}$, which is the centre
of the Gaussian.  The final parameter is $\sigma$, which
parameterises the plant root depth.  When $P = P_{c} - \sigma$, the
...
\caption{Evapotranspiration $-F/E_{\mathrm{max}}$ as a function of
  porepressure $P - P_{\mathrm{atm}}$ for different $\sigma$ parameters.  Top (blue)
...
1000\,kg.m$^{-3}$.  Usually $P_{c}$ is chosen to be atmospheric
pressure $P_{c} = P_{\mathrm{atm}}$.  Typical tree roots reach to
...
current implementation of Wallaby has $f$ being the same for all
...
\end{equation}
where $C$ is the riverbed conductance.  This is implemented in the
...
\right.
\end{equation}
where $C$ is the riverbed conductancel.  This is implemented in the
MOOSE input file using the pressure tuple $(P_{\mathrm{atm}},
...
\begin{equation}
P_{\mathrm{wellbore}}(x_{i}) = P_{\mathrm{bot}} + \rho_{0} g\cdot (x_{i} -
x_{i}^{\mathrm{bot}})
\end{equation}
The form of $f$ is
\begin{equation}
...
\item $\rho_{0}$ is the fluid reference density (see
  Sec~\ref{sec.density.constk}), and $g$ is the vector of
  gravitational acceleration which points downwards.  This means that
  $P_{\mathrm{wellbore}}(x_{i})$ will be the pressure at point $x_{i}$
  in the wellbore, due to gravitational head, assuming that the fluid
  density does change significantly in the wellbore.  If these
  gravitational effects are undesirable, the code has been written in
  such a way that the user may simply set $g=0$ for the wellbore.
...
If
  $W_{i}=0$ then the wellbore is a production well, which is, as far
  as Wallaby is concerned, a sink that produces fluid only if $P >
  P_{\mathrm{wellbore}}$.  If $W_{p}=0$ then the wellbore is an
  injection well, which is a source that injects fluid only if $P <
  P_{\mathrm{wellbore}}$.

Usually the well constants are chosen as
\begin{equation}
W = \kappa L_{\mathrm{seg}} \ ,
\end{equation}
where $\kappa$ is a relevant permeability component (eg, a horizontal
component if this is a vertical wellbore), and $L_{\mathrm{seg}}$ is
the length of this section of the wellbore that the point $x_{i}$ is
approximating.
\end{itemize}
...
The time-dependant excavation boundary is defined
succinctly through a time-dependent level-set function, that is
positive on parts of the domain where excavation has occurred, and
negative on the other region.
...
the Streamline-Upwind-Petrov-Galerkin (SUPG) upwinding used in
Wallaby, which is based on~\cite{brooks1982, hughes1986}.
...
\section{An explicit worked example: SUPG upwinding of the advection equation in one dimension}
...
\section{SUPG of the continuity equation}
...
convergence as the upwinding direction changes every timestep when
simulations start to reach steadystate.
...
This is explained in Section~\ref{sec.lumping.timde.deriv}.
In the current implementation of Wallaby, this residual depends on the
porepressure at all the quadrature points surrounding the node, and
I'm not sure how to implement lumping in Moose.
