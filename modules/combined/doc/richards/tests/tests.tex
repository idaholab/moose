...
\item Chapter~\ref{bw} demonstrates that flow in the unsaturated
  ...
  solution of Broadbridge and White for constant infiltration.
\item Chapter~\ref{forsyth} compares MOOSE with numerical results from
  ...
  reality, this is very similar to Chapter~\ref{bw}: the only
  essential differences are the capillary and relative
...
$D$ and $K$.  The hydraulic conductivity is
\begin{equation}
...
\chapter{The analytic drainage solution --- NEEDS UPDATE}

The analytic solutions of this nonlinear
diffusion-advection relevant to groundwater flow have been studied by
Broadbridge and White\footnote{P Broadbridge, I White ``Constant rate
  rainfall infiltration: A versatile nonlinear model, 1 Analytical
  solution''.  Water Resources Research 24 (1988) 145--154.} and
Warrick, Lomen and Islas\footnote{AW Warrick, DO Lomen and A Islas,
  ...
  Profile'', Water Resources Research 26 (1990) 253--258.}, and others
(see references below), and are
valid for 
\begin{eqnarray}
D(S) & = & \frac{a}{(b-S)^{2}} \ , \\
K(S) & = & \beta + \gamma(b - S) + \frac{\lambda}{2(b - S)} \ .
\end{eqnarray}
