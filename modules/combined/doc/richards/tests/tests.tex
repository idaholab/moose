...
\begin{itemize}
\item The problem is fully saturation, $S=1$, and gravitational
...
and the MOOSE implementation.  This test is not part of the automatic
test suite since it takes 17\,seconds to complete.  It is therefore
marked as ``heavy'' and is not run every time the code is updated.
However, a similar test on a coarser mesh with only 272 elements, that
also takes larger timesteps, that gives similar drawdowns (to within
0.1\,m) is part of the automatic test suite.
