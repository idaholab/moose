...
def expected(fn):
    f = open(fn, "r")
    data = sorted(
        [map(float, line.strip().split(",")[3:6]) for line in f.readlines()[1:]]
    )
    ...
    lode = [
        np.arcsin(-1.5 * np.sqrt(3) * third_inv[i] / bar[i] ** 3) / 3.0
        for i in range(len(data))
    ]
    # extend data to non-physical region for clearer visualisation
    theta_r = [[lode[i], bar[i]] for i in range(len(data))] + [
        [np.pi / 3.0 - lode[i], bar[i]] for i in range(len(data))
    ]
    theta_r = sorted(
        [[d[0], d[1]] for d in theta_r]
        + [[d[0] + 2.0 * np.pi / 3.0, d[1]] for d in theta_r]
        + [[d[0] - 2.0 * np.pi / 3.0, d[1]] for d in theta_r]
    )
