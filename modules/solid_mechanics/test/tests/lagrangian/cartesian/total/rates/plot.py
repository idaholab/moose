...
if __name__ == "__main__":
    ...
    def analytical_green_naghdi(k):
        ...
        s12 = (
            2
            * np.cos(2 * b)
            * (2 * b - 2 * np.tan(2 * b) * np.log(np.cos(b)) - np.tan(b))
        )
        return s11, s12, s22
    ...
    compare(
        ...
        "truesdell_shear_out.csv",
        analytical_truesdell,
    )

    compare(
        "Jaumann rate of Cauchy stress", "jaumann_shear_out.csv", analytical_jaumann
    )

    compare(
        "Green-Naghdi rate of Cauchy stress",
        "green_naghdi_shear_out.csv",
