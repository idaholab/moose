...
def expected(scheme, angle_degrees):
    ...
    if scheme == "native":
        ...
        coh = (
            ...
            * cohesion
            * np.cos(friction)
            / np.sqrt(9.0 + 3.0 * np.power(np.sin(friction), 2))
        )
        fric = np.arctan(
            np.sin(friction) / np.sqrt(9.0 + 3.0 * np.power(np.sin(friction), 2))
        )

    bar = np.sqrt(
        np.power(coh - mean * 3.0 * np.tan(fric), 2) - np.power(tip_smoother, 2)
    )
    x = bar * np.cos(angle)
    y = bar * np.sin(angle)
    return (x, y)
...
def sigma_bar(stress):
    ...
    return np.sqrt(
        0.5
        * (
            np.power(stress[0] - mean, 2)
            + 2 * stress[1] * stress[1]
            + 2 * stress[2] * stress[2]
            + np.power(stress[3] - mean, 2)
            + 2 * stress[4] * stress[4]
            + np.power(stress[5] - mean, 2)
        )
    )
...
plt.plot(
    expected("native", angles)[0],
    expected("native", angles)[1],
    "k-",
...
plt.plot(
    expected("outer_tip", angles)[0],
    expected("outer_tip", angles)[1],
    "g-",
    label="expected (outer_tip)",
)
mr = moose_result("gold/small_deform2_outer_tip.csv")
plt.plot(mr[0], mr[1], "g^", label="MOOSE (outer_tip)")

plt.plot(
    expected("inner_tip", angles)[0],
    expected("inner_tip", angles)[1],
    "b-",
    label="expected (inner_tip)",
)
mr = moose_result("gold/small_deform2_inner_tip.csv")
plt.plot(mr[0], mr[1], "b^", label="MOOSE (inner_tip)")

plt.plot(
    expected("lode_zero", angles)[0],
    expected("lode_zero", angles)[1],
    "c-",
    label="expected (lode_zero)",
)
mr = moose_result("gold/small_deform2_lode_zero.csv")
plt.plot(mr[0], mr[1], "c^", label="MOOSE (lode_zero)")

plt.plot(
    expected("inner_edge", angles)[0],
    expected("inner_edge", angles)[1],
    "r-",
    label="expected (inner_edge)",
)
