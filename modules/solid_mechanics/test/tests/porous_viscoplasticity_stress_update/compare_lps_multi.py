...
if len(sys.argv) < 3:
    raise SystemExit(f"usage: {sys.argv[0]} reference.csv candidate.csv [relative_tolerance]")

reference_path, candidate_path = sys.argv[1:3]
relative_tolerance = float(sys.argv[3]) if len(sys.argv) > 3 else 1.0e-10
absolute_tolerance = 1.0e-13
columns = ("disp_x", "disp_y", "avg_hydro", "avg_vonmises", "eff_creep_strain", "porosity")
...
for row_index, (ref_row, test_row) in enumerate(zip(reference, candidate)):
    for column in columns:
        ...
        if not math.isclose(ref, test, rel_tol=relative_tolerance, abs_tol=absolute_tolerance):
            raise SystemExit(
