...
[BCs]
  # Drive a non-trivial solve so a Jacobian is assembled. The NEML2 input derivative (dh_deps) is
  # evaluated only on Jacobian evaluations; without a Jacobian it would be reported as zero. The
  # aliased derivatives here do not depend on u (dh_deps = K, dh_dK = equivalent_plastic_strain, both
  # constant in u), so the checked values are unchanged.
  [left]
    type = DirichletBC
    variable = u
    boundary = left
