...
class LAROMANCE6DInterpolation(Model):
    ...
    hit = HitSchema(
        ...
        option(
            "model_file_name", str, "The name of the model file", attr="_model_file"
        ),
        option(
    ...
    def _transform(data: torch.Tensor, params: tuple[str, list[float]]) -> torch.Tensor:
        ...
        if ttype == "LOG10BOUNDED":
            factor, lowerbound, upperbound, logmin, logmax = (
                p[0],
                p[1],
                p[2],
                p[3],
            ...
            return (
                rng * (torch.log10(data + factor) - logmin) / (logmax - logmin)
                + lowerbound
            )
        if ttype == "EXP10BOUNDED":
            ...
            rng = upperbound - lowerbound
            return (
                10.0 ** (((data - lowerbound) * (logmax - logmin) / rng) + logmin)
                - factor
            )
        if ttype == "MINMAX":
    ...
    def _check_rectangular(data, key: str) -> None:
        ...
        def walk(v):
            ...
            for x in v:
                if len(x) != n:
                    raise RuntimeError(
                        f"Incorrect JSON interpolation grid size for '{key}'."
                    )
                walk(x)
