...
#include "SolidMechanicsPlasticJ2.h"
...
registerMooseObjectRenamed("SolidMechanicsApp",
                           ...
                           "01/01/2025 00:00",
                           SolidMechanicsPlasticJ2);

InputParameters
SolidMechanicsPlasticJ2::validParams()
{
  InputParameters params = SolidMechanicsPlasticModel::validParams();
  params.addRequiredParam<UserObjectName>(
      "yield_strength",
      "A SolidMechanicsHardening UserObject that defines hardening of the yield strength");
...
SolidMechanicsPlasticJ2::SolidMechanicsPlasticJ2(const InputParameters & parameters)
  : SolidMechanicsPlasticModel(parameters),
    _strength(getUserObject<SolidMechanicsHardeningModel>("yield_strength")),
...
Real
SolidMechanicsPlasticJ2::yieldFunction(const RankTwoTensor & stress, Real intnl) const
{
  return std::sqrt(3.0 * stress.secondInvariant()) - yieldStrength(intnl);
}

RankTwoTensor
SolidMechanicsPlasticJ2::dyieldFunction_dstress(const RankTwoTensor & stress, Real /*intnl*/) const
{
...
Real
SolidMechanicsPlasticJ2::dyieldFunction_dintnl(const RankTwoTensor & /*stress*/, Real intnl) const
{
  return -dyieldStrength(intnl);
}

RankTwoTensor
SolidMechanicsPlasticJ2::flowPotential(const RankTwoTensor & stress, Real intnl) const
{
  return dyieldFunction_dstress(stress, intnl);
}

RankFourTensor
SolidMechanicsPlasticJ2::dflowPotential_dstress(const RankTwoTensor & stress, Real /*intnl*/) const
{
  Real sII = stress.secondInvariant();
  if (sII == 0)
...
RankTwoTensor
SolidMechanicsPlasticJ2::dflowPotential_dintnl(const RankTwoTensor & /*stress*/,
                                               Real /*intnl*/) const
{
  return RankTwoTensor();
}
...
Real
SolidMechanicsPlasticJ2::yieldStrength(Real intnl) const
{
  return _strength.value(intnl);
}

Real
SolidMechanicsPlasticJ2::dyieldStrength(Real intnl) const
{
  return _strength.derivative(intnl);
}

std::string
SolidMechanicsPlasticJ2::modelName() const
{
  return "J2";
}

bool
SolidMechanicsPlasticJ2::returnMap(const RankTwoTensor & trial_stress,
                                   Real intnl_old,
...
RankFourTensor
SolidMechanicsPlasticJ2::consistentTangentOperator(const RankTwoTensor & trial_stress,
                                                   Real intnl_old,
...
bool
SolidMechanicsPlasticJ2::useCustomReturnMap() const
{
  return _use_custom_returnMap;
}

bool
SolidMechanicsPlasticJ2::useCustomCTO() const
{
