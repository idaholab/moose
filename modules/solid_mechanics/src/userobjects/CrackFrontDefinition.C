...
addCrackFrontDefinitionParams(InputParameters & params)
{
  ...
  params.addParam<unsigned int>("num_crack_front_points", "Number of Crack Front Points");
  ...
  params.addParam<unsigned int>("nrings", "nrings");
}
...
CrackFrontDefinition::CrackFrontDefinition(const InputParameters & parameters)
  : GeneralUserObject(parameters),
    ...
    _q_function_type(
        getParam<MooseEnum>("q_function_type")), // this can only be here; I don't know why...
    _nrings(getParam<unsigned int>("nrings")),   // this can only be here; I don't know why...
    _num_crack_front_points(getParam<unsigned int>("num_crack_front_points"))
{
...
CrackFrontDefinition::initialSetup()
{
  ...
  if (_q_function_type == "GEOMETRY")
  {
    if (_treat_as_2d)
    {
      if (_num_crack_front_points != 1)
      {
        mooseWarning("num_crack_front_points should be 1 for 2D");
      }
    }
    else
    {
