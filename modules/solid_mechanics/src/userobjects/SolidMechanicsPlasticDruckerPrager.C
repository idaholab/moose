...
registerMooseObjectRenamed("SolidMechanicsApp",
                           ...
                           "01/01/2025 00:00",
                           SolidMechanicsPlasticDruckerPrager);

InputParameters
SolidMechanicsPlasticDruckerPrager::validParams()
{
...
SolidMechanicsPlasticDruckerPrager::SolidMechanicsPlasticDruckerPrager(
    ...
    _zero_psi_hardening(_mc_psi.modelName().compare("Constant") == 0)
{
  if (_mc_phi.value(0.0) < 0.0 || _mc_psi.value(0.0) < 0.0 ||
    ...
    mooseError("SolidMechanicsPlasticDruckerPrager: MC friction and dilation angles must lie in "
               "[0, Pi/2]");
  if (_mc_phi.value(0) < _mc_psi.value(0.0))
    mooseError("SolidMechanicsPlasticDruckerPrager: MC friction angle must not be less than MC "
               "dilation angle");
  if (_mc_cohesion.value(0.0) < 0)
    mooseError("SolidMechanicsPlasticDruckerPrager: MC cohesion should not be negative");
...
Real
SolidMechanicsPlasticDruckerPrager::yieldFunction(const RankTwoTensor & stress, Real intnl) const
{
  Real aaa;
  Real bbb;
...
RankTwoTensor
SolidMechanicsPlasticDruckerPrager::df_dsig(const RankTwoTensor & stress, Real bbb) const
{
...
RankTwoTensor
SolidMechanicsPlasticDruckerPrager::dyieldFunction_dstress(const RankTwoTensor & stress,
                                                           Real intnl) const
{
  Real bbb;
...
Real
SolidMechanicsPlasticDruckerPrager::dyieldFunction_dintnl(const RankTwoTensor & stress,
                                                          Real intnl) const
{
  Real daaa;
  Real dbbb;
...
RankTwoTensor
SolidMechanicsPlasticDruckerPrager::flowPotential(const RankTwoTensor & stress, Real intnl) const
{
  Real bbb_flow;
...
RankFourTensor
SolidMechanicsPlasticDruckerPrager::dflowPotential_dstress(const RankTwoTensor & stress,
                                                           Real /*intnl*/) const
{
...
RankTwoTensor
SolidMechanicsPlasticDruckerPrager::dflowPotential_dintnl(const RankTwoTensor & stress,
                                                          Real intnl) const
{
  Real dbbb;
...
std::string
SolidMechanicsPlasticDruckerPrager::modelName() const
{
  return "DruckerPrager";
}

void
SolidMechanicsPlasticDruckerPrager::bothAB(Real intnl, Real & aaa, Real & bbb) const
{
...
void
SolidMechanicsPlasticDruckerPrager::onlyB(Real intnl, int fd, Real & bbb) const
{
...
void
SolidMechanicsPlasticDruckerPrager::donlyB(Real intnl, int fd, Real & dbbb) const
{
  if (_zero_phi_hardening && (fd == friction))
  {
    dbbb = 0;
...
void
SolidMechanicsPlasticDruckerPrager::dbothAB(Real intnl, Real & daaa, Real & dbbb) const
{
  if (_zero_cohesion_hardening && _zero_phi_hardening)
  {
    daaa = 0;
...
void
SolidMechanicsPlasticDruckerPrager::initializeAandB(Real intnl, Real & aaa, Real & bbb) const
{
  const Real C = _mc_cohesion.value(intnl);
  const Real cosphi = std::cos(_mc_phi.value(intnl));
  const Real sinphi = std::sin(_mc_phi.value(intnl));
...
void
SolidMechanicsPlasticDruckerPrager::initializeB(Real intnl, int fd, Real & bbb) const
{
  const Real s = (fd == friction) ? std::sin(_mc_phi.value(intnl)) : std::sin(_mc_psi.value(intnl));
  switch (_mc_interpolation_scheme)
  {
