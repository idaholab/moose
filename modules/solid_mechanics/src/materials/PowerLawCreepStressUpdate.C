...
{
  return _coefficient * std::pow(stress_eq, _n_exponent) * MetaPhysicL::raw_value(_exponential) *
         _exp_time;
}
    ...
    const GenericMaterialProperty<RankTwoTensor, is_ad> & strain_rate)
{
  ...
  if (!RadialReturnCreepStressUpdateBaseTempl<is_ad>::_compute_numerical_serd)
  {
    ...
    return RadialReturnCreepStressUpdateBaseTempl<is_ad>::computeStrainEnergyRateDensity(
        stress, strain_rate);
}
