...
void SymmAnisotropicElasticityTensor::intialize_material_dt_matrix()
{
...
void SymmAnisotropicElasticityTensor::form_transformed_material_dt_matrix()
{
  ...
  outputMatrix = _transpose_trans_d9_to_d6;
  outputMatrix.right_multiply(_qdmat);

  _dt = outputMatrix;
  _dt.right_multiply(_trans_d9_to_d6);
...
void
SymmAnisotropicElasticityTensor::multiply( const SymmTensor & x, SymmTensor & b )
{
  SymmElasticityTensor::generalMultiply( x, b );
}
