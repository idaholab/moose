...
InputParameters validParams<CombinedCreepPlasticity>()
{
  ...
  params.addRequiredParam<std::vector<std::string> >("submodels", "List of submodel ConstitutiveModels");
...
CombinedCreepPlasticity::initialSetup()
{
  ...
  for (unsigned i(0); i < block_id.size(); ++i)
  {
    ...
    if (_bnd)
    {
      ...
      mats_p = &_fe_problem.getMaterialWarehouse(Moose::BLOCK_MATERIAL_DATA).getActiveBlockObjects(block_id[i], _tid);
