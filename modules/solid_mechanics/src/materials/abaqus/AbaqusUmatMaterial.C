    ...
    _creep_dissipation(declareProperty<Real>("creep_dissipation"))
{
  ...
  for(unsigned int i=0;i<_NTENS; i++)
    _DDSDDT[i] = 0.0;
  ...
  for(unsigned int i=0;i<_NTENS*_NTENS; i++)
    _DDSDDE[i] = 0.0;
...
void AbaqusUmatMaterial::computeStress()
{
  ...
  Fbar(0,0) = _grad_disp_x_old[_qp](0); Fbar(0,1) = _grad_disp_x_old[_qp](1); Fbar(0,2) = _grad_disp_x_old[_qp](2);
  Fbar(1,0) = _grad_disp_y_old[_qp](0); Fbar(1,1) = _grad_disp_y_old[_qp](1); Fbar(1,2) = _grad_disp_y_old[_qp](2);
  Fbar(2,0) = _grad_disp_z_old[_qp](0); Fbar(2,1) = _grad_disp_z_old[_qp](1); Fbar(2,2) = _grad_disp_z_old[_qp](2);
