...
InputParameters validParams<RateDepSmearIsoCrackModel>()
{
  ...
  params.addParam<Real>("upper_limit_damage", 5.0 , "Upper limit of damage beyond which constitutive check is not performed");
...
RateDepSmearIsoCrackModel::calcStateIncr()
{
  ...
  if ( std::abs(_intvar_old_tmp[0]) > _zero_tol && std::abs(_intvar_old_tmp[0]) < _upper_lim_damage && std::abs(val) > _intvar_incr_tol * std::abs(_intvar_old_tmp[0]) )
  {
    _err_tol = true;
    return;
  }
