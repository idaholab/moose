...
TotalLagrangianWeakPlaneStress::computeQpJacobian()
{
  ...
  // `_d_F_stab_d_F_ust` factor REPLACED by identity in the sigma chain) for consistency.
...
TotalLagrangianWeakPlaneStress::computeQpOffDiagJacobian(unsigned int jvar)
{
  for (auto beta : make_range(_ndisp))
    ...
    {
      // Local PK1_{2,2} Jacobian via _dpk1 (includes local F-bar through the sigma chain).
      ...
      // Non-local F-bar contribution to PK1_{2,2}; helper handles the sigma-via-dL chain and
