...
StressDivergence::computeQpJacobian()
{
  ...
  Real jacobian = 0.0;
  // B^T_i * C * B_j
  jacobian +=  _Jacobian_mult[_qp].stiffness(_component, _component, _grad_test[_i][_qp], _grad_phi[_j][_qp]); //B^T_i * C *B_j
  ...
  if (_volumetric_locking_correction)
  {
    ...
    jacobian += (tmp.xx() + tmp.yy() + tmp.zz()) * (_avg_grad_test[_i][_component] - _grad_test[_i][_qp](_component)) / 3.0;
  }
...
StressDivergence::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  if (active)
  {
    ...
    RealGradient sum_C3x1 = _Jacobian_mult[_qp].sum_3x1();
    Real jacobian = 0.0;
    jacobian +=  _Jacobian_mult[_qp].stiffness(_component, coupled_component, _grad_test[_i][_qp], _grad_phi[_j][_qp]); //B^T_i * C *B_j
