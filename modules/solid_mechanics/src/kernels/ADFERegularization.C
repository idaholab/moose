...
#include "ADFERegularization.h"

registerMooseObject("SolidMechanicsApp", ADFERegularization);

InputParameters
ADFERegularization::validParams()
{
  InputParameters params = ADKernel::validParams();
  params.addClassDescription("Adds a finite-element regularization term using HuHu, LuLu, or "
                             "HuHu-LuLu Hessian/Laplacian contractions. The Jacobian is computed "
...
ADFERegularization::ADFERegularization(const InputParameters & parameters)
  : ADKernel(parameters),
...
ADReal
ADFERegularization::computeQpResidual()
{
  return _coefficient *
