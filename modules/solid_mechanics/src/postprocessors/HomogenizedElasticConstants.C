...
HomogenizedElasticConstants::HomogenizedElasticConstants(const std::string & name, InputParameters parameters)
  :ElementAverageValue(name, parameters),
   ...
   // _grad_disp_z_xx(_dim == 3 ? coupledGradient("dz_xx") : _grad_zero),
   _grad_disp_x_yy(coupledGradient("dx_yy")),
   _grad_disp_y_yy(coupledGradient("dy_yy")),
   // _grad_disp_z_yy(_dim == 3 ? coupledGradient("dz_yy") : _grad_zero),
   // _grad_disp_x_zz(_dim == 3 ? coupledGradient("dx_zz") : _grad_zero),
   //  _grad_disp_y_zz(_dim == 3 ? coupledGradient("dy_zz") : _grad_zero),
   // _grad_disp_z_zz(_dim == 3 ? coupledGradient("dz_zz") : _grad_zero),
   _grad_disp_x_xy(coupledGradient("dx_xy")),
   _grad_disp_y_xy(coupledGradient("dy_xy")),
   //_grad_disp_z_xy(_dim == 3 ? coupledGradient("dz_xy") : _grad_zero),
   //_grad_disp_x_yz(_dim == 3 ? coupledGradient("dx_yz") : _grad_zero),
   //_grad_disp_y_yz(_dim == 3 ? coupledGradient("dy_yz") : _grad_zero),
   //_grad_disp_z_yz(_dim == 3 ? coupledGradient("dz_yz") : _grad_zero),
   //_grad_disp_x_zx(_dim == 3 ? coupledGradient("dx_zx") : _grad_zero),
   //_grad_disp_y_zx(_dim == 3 ? coupledGradient("dy_zx") : _grad_zero),
   //_grad_disp_z_zx(_dim == 3 ? coupledGradient("dz_zx") : _grad_zero),
