...
def ismoother(f_diff):
    if abs(f_diff) >= opts.smoothing_tol:
        return 0.0
    return 0.5 * (
        opts.smoothing_tol - abs(f_diff)
    ) - opts.smoothing_tol / math.pi * math.cos(
        0.5 * math.pi * f_diff / opts.smoothing_tol
    )
...
def yield_function(x, y, z):
    ...
    if opts.tensile:
        yfs += [
            x - opts.tensile_strength,
            y - opts.tensile_strength,
            z - opts.tensile_strength,
        ]
    if opts.mc:
        yfs += [
            0.5 * (x - z) + 0.5 * (x + z) * sinphi - cohcos,
...
p.add_option(
    ...
    dest="name",
    help="The pointdata produced will have this name.  Default=%default",
)
p.add_option(
    "--smoothing_tol",
    action="store",
    ...
    default=0.1,
    dest="smoothing_tol",
    help="The smoothing tolerance (a) parameter.  Default=%default",
)
p.add_option(
    "-t",
    action="store_true",
    dest="tensile",
    help="Yield function will contain contributions from tensile (Rankine) failure",
)
p.add_option(
    "--tensile_strength",
    action="store",
    type="float",
    default=0.7,
    dest="tensile_strength",
    help="Tensile strength",
)
p.add_option(
    "-m",
    action="store_true",
    dest="mc",
    help="Yield function will contain contributions from Mohr-Coulomb failure",
)
p.add_option(
    "-d",
    action="store_true",
    dest="dp",
    help="Yield function will contain contributions from Drucker-Prager failure",
)
p.add_option(
    "-e",
    action="store_true",
    dest="twoD_example",
    help="Yield function will contain contributions from an example 2D yield function",
)
p.add_option(
    "-a",
    action="store_true",
