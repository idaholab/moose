...
def solid_bar2():
    f = open(
        "../../tests/static_deformations/gold/beam_cosserat_02_apply_stress_soln_0001.csv"
    )
    data = [map(float, line.strip().split(",")) for line in f.readlines()[1:12]]
    f.close()
    return data
...
def solid_bar_slippery():
    f = open(
        "../../tests/static_deformations/gold/beam_cosserat_01_slippery_soln_0001.csv"
    )
    data = [map(float, line.strip().split(",")) for line in f.readlines()[1:12]]
    f.close()
    return data
...
plt.figure()
plt.plot(
    xpoints, expected_slippery(xpoints)[0], "k-", linewidth=1.0, label="expected th_y"
)
plt.plot(
    xpoints, expected_slippery(xpoints)[1], "r-", linewidth=1.0, label="expected u_z"
)
plt.plot(
    moosex, [d[11] for d in moose_slippery], "ks", markersize=10.0, label="MOOSE wc_y"
)
plt.plot(
    moosex, [d[5] for d in moose_slippery], "r^", markersize=10.0, label="MOOSE disp_z"
)
plt.legend(loc="lower left")
plt.xlabel("x (m)")
...
plt.figure()
plt.plot(
    hpoints, expected_slippery_h(hpoints)[1], "k-", linewidth=1.0, label="expected"
)
