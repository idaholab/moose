...
To compute $C$-integrals, the [DomainIntegral Action](/DomainIntegralAction.md) should be used to set up all of the required objects. Set `integrals = CIntegral` and select exactly one source for the strain energy rate density. The `inelastic_models` parameter provides a model that implements either an analytical calculation, such as power-law creep, or Gaussian integration in effective-stress space. Set `use_existing_serd = true` when another material supplies the `strain_energy_rate_density` property. Set `use_incremental_serd = true` to use the incremental trapezoidal calculation provided by [StrainEnergyRateDensity](/StrainEnergyRateDensity.md); this approximation requires sufficiently small time steps.
