...
rotation increment for a 1D axisymmetric plane-strain or generalized-plane-strain model.
This is a non-[!ac](AD) model that has an [!ac](AD) counterpart,
...
`planar_formulation = PLANE_STRAIN` and `strain = FINITE` in the
QuasiStatic Physics block.

!listing modules/solid_mechanics/test/tests/1D_axisymmetric/axisymm_plane_strain_finite.i block=Physics/SolidMechanics/QuasiStatic
...
`planar_formulation = GENERALIZED_PLANE_STRAIN` and `strain = FINITE` in the
QuasiStatic Physics block.

!listing modules/solid_mechanics/test/tests/1D_axisymmetric/axisymm_gps_finite.i block=Physics/SolidMechanics/QuasiStatic
