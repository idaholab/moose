...
The purpose of this `NodalKernel` is to introduce Rayleigh mass damping into an [ExplicitMixedOrder](ExplicitMixedOrder.md) solid-mechanics model.  Rayleigh mass damping is used to damp low frequency, long wavelength vibratory modes of the model.  This is particularly useful in models where quasi-static solutions are of interest, as the low frequency modes can persist for many time steps if not damped.  Related aside: high frequency, short wavelength vibratory models (eg, on the scale of a single element) are damped using `stiffness_damping_coefficient` in [DynamicSolidMechanicsPhysics](DynamicSolidMechanicsPhysics.md).
