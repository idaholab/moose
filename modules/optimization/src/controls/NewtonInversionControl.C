...
NewtonInversionControl::computeUpdate(unsigned int it, Real p_used, Real y, Real y_target)
{
  ...
  // else should touch it before this solve reads it back, so p_used must equal that perturbed
  // value. If it does not, an external object changed the parameter between the two solves -- most
  // likely the parameter postprocessor was relaxed by listing it in the executioner's
  // transformed_postprocessors
  // -- so the finite-difference slope below would reflect a perturbation the model never actually
  // saw. Fail loudly rather than return a silently wrong update. The fuzzy comparison only absorbs
  // last-bit rounding (the value is written and read as the same double); a relaxation moves it by
