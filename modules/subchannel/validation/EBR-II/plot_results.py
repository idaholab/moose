...
TTC17_corrected = np.genfromtxt(
    "XX09_SCM_SS17_corrected_out.csv", skip_header=2, delimiter=","
)
TTC45R = np.genfromtxt("XX09_SCM_SS45R_out.csv", skip_header=2, delimiter=",")
TTC45R_corrected = np.genfromtxt(
    "XX09_SCM_SS45R_corrected_out.csv", skip_header=2, delimiter=","
)
...
k = (
    A48
    + A49 * inlet_temperature
    + A50 * inlet_temperature * inlet_temperature
...
A13 = -0.21390
A14 = -1.1046e-5
rho = A12 + A13 * inlet_temperature + A14 * inlet_temperature * inlet_temperature
print(rho)

plt.figure()
plt.plot(TTC31_TR17[6:, 0], TTC31_TR17[6:, 1], "g-.", label="SCM caclulation at TTC-31")
plt.plot(
    EXP[:, 0],
    EXP[:, 1],
    marker="o",
    markerfacecolor="r",
    linestyle="none",
    label="Measurement at TTC-31",
)
plt.plot(NETFLOW[:, 0], NETFLOW[:, 1], ":b", label="NETFLOW++/COBRA-IV-I at TTC-31")
...
plt.figure()
plt.plot(
    TTC31_TR45R[7:, 0], TTC31_TR45R[7:, 1], "g-.", label="SCM caclulation at TTC-31"
)
plt.plot(
    EXP45[:, 0],
    EXP45[:, 1],
...
plt.plot(NETFLOW45[:, 0], NETFLOW45[:, 1], ":b", label="NETFLOW++/COBRA-IV-I at TTC-31")
...
plt.plot(
    massflow_SHRT17[:, 0],
    massflow_SHRT17[:, 1] / 2.450,
    "k--",
    label="normalized massflow SHRT-17",
)
plt.plot(
    massflow_SHRT45[:, 0],
    massflow_SHRT45[:, 1] / 2.427,
    "r-.",
    label="normalized massflow SHRT-45R",
)
plt.plot(power_SHRT17[:, 0], power_SHRT17[:, 1], "k", label="normalized power SHRT-17")
plt.plot(power_SHRT45[:, 0], power_SHRT45[:, 1], "r", label="normalized power SHRT-45R")
plt.title(r"Transient of normalized massflow & power" "\n" "Assembly XX09", fontsize=13)
plt.xlabel(r"$time~[sec]$", fontsize=14)
...
plt.plot(
    TTC_DASSH[:, 1] + 273.15, "b-.", marker="o", markerfacecolor="b", label="DASSH"
)
plt.plot(TTC17[10:], "k-.", marker="o", markerfacecolor="k", label="SCM")
plt.plot(
    TTC17_corrected[10:],
    "g-.",
    marker="o",
    markerfacecolor="g",
    label="SCM power profile corrected",
)
plt.title(
    r"Temperature profile 0.322m downstream of heated section"
    "\n"
    "XX09 TTC SHRT-17, Initial steady state",
    fontsize=13,
)
plt.xticks([i for i in range(len(TTC_EXP[:, 0]))], [str(int(i)) for i in TTC_EXP[:, 0]])
...
plt.figure()
plt.plot(
    TTC_EXP45[:, 1], "r", marker="o", markerfacecolor="r", linestyle="none", label="EXP"
)
plt.plot(TTC45R[10:], "k-.", marker="o", markerfacecolor="k", label="SCM")
plt.plot(
    TTC45R_corrected[10:],
    "g-.",
    marker="o",
...
plt.title(
    r"Temperature profile 0.322m downstream of heated section"
    "\n"
    "XX09 TTC SHRT-45R, Initial steady state",
    fontsize=13,
)
plt.xticks(
    [i for i in range(len(TTC_EXP45[:, 0]))], [str(int(i)) for i in TTC_EXP45[:, 0]]
)
plt.xlabel(r"$Channel~\#$", fontsize=14)
plt.ylabel(r"$T~[K]$", fontsize=14)
plt.legend(fontsize=12)
