...
SCM = np.genfromtxt(output_path("FFM-3A_out.csv"), skip_header=2, delimiter=",")
...
plt.title(
    r"Temperature profile 76mm downstream of heated section" "\n" "FFM-3A Run 101",
    fontsize=13,
)
plt.xticks(
    ticks=[i for i in range(len(Channels))], labels=[str(int(i)) for i in Channels]
)
