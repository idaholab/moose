...
public:
  ...
  /**
   * Set maps that relate channel nodes to duct nodes (and back).
   */
...
protected:
  ...
  // ---- duct bookkeeping ----
  /// vector of duct nodes
  std::vector<Node *> _duct_nodes;
  /// map that relates the subchannel node to the nearest duct node
  std::map<Node *, Node *> _chan_to_duct_node_map;
  /// map that relates the duct node to the nearest subchannel node
