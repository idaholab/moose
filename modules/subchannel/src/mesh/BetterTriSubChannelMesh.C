...
registerMooseObject("SubChannelApp", BetterTriSubChannelMesh);

InputParameters
BetterTriSubChannelMesh::validParams()
{
...
void
BetterTriSubChannelMesh::rodPositions(std::vector<Point> & positions,
                                unsigned int nrings,
                                Real pitch,
                                Point center)
{
...
void
BetterTriSubChannelMesh::ductCorners(std::vector<Point> & corners, Real flat_to_flat, Point center)
{
...
void
BetterTriSubChannelMesh::ductXsec(std::vector<Point> & xsec,
...
size_t
BetterTriSubChannelMesh::ductPointIndex(unsigned int points_per_layer,
                                  unsigned int layer,
                                  unsigned int point)
{
...
void
BetterTriSubChannelMesh::ductPoints(std::vector<Point> & points,
...
void
BetterTriSubChannelMesh::ductElems(std::vector<std::vector<size_t>> & elem_point_indices,
...
void
BetterTriSubChannelMesh::buildDuct(UnstructuredMesh & mesh,
...
BetterTriSubChannelMesh::BetterTriSubChannelMesh(const InputParameters & params)
...
BetterTriSubChannelMesh::safeClone() const
{
  return libmesh_make_unique<BetterTriSubChannelMesh>(*this);
}

unsigned int
BetterTriSubChannelMesh::getSubchannelIndexFromPoint(const Point & p) const
{
...
void
BetterchannelToDuctMaps(std::map<Node *, Node *> & chan_to_duct,
...
void
BetterTriSubChannelMesh::buildMesh()
{
  ...
  mesh.reserve_elem(_n_cells * _n_channels);
  mesh.reserve_nodes((_n_cells + 1) * _n_channels);
