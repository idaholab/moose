...
validParams<SubchannelMesh>()
{
  ...
  params.addParam<unsigned int>("nx", 3, "Number of channels in the x direction");
  params.addParam<unsigned int>("ny", 2, "Number of channels in the x direction");
  params.addParam<Real>("pitch", 0.0126, "The distance between fuel pin centers");
  params.addParam<Real>("max_dz", 0.1, "The maximum element height");
...
{
  //TODO: parameterize
  std::vector<Real> spacer_z({0, 0.229, 0.457, 0.686, 0.914, 1.143, 1.372,
    1.600, 1.829, 2.057, 2.286, 2.515, 2.743, 2.972, 3.200, 3.429});
  std::vector<Real> spacer_K({1.06, 0.77, 1.41, 0.77, 1.41, 0.77, 1.41,
    0.77, 1.41, 0.77, 1.41, 0.77, 1.41, 0.77, 1.41, 0.77});
  ...
  for (auto block_size : block_sizes) {
    int n = 1;
    while (n * _max_dz < block_size) ++n;
    Real dz = block_size / n;
    for (int i = 0; i < n ; i++) z_grid_.push_back(z_grid_.back() + dz);
  }
  nz_ = z_grid_.size() - 1;
  ...
  for (unsigned int iy = 0; iy < ny_; iy++) {
    for (unsigned int ix = 0; ix < nx_; ix++) {
      ...
      for (unsigned int iz = 0; iz < nz_+1; iz++) {
        nodes_[i_ch].push_back(mesh.add_point(
          Point(pitch_*ix, pitch_*iy, z_grid_[iz]), node_id++));
      }
    }
  }
  ...
  for (unsigned int iy = 0; iy < ny_; iy++) {
    for (unsigned int ix = 0; ix < nx_; ix++) {
      for (unsigned int iz = 0; iz < nz_; iz++) {
        ...
        if (iz == 0) boundary_info.add_side(elem, 0, 0);
        if (iz == nz_-1) boundary_info.add_side(elem, 1, 1);
      }
    }
  }
