...
registerMooseObject("SubChannelApp", BetterQuadPowerIC);

InputParameters
BetterQuadPowerIC::validParams()
{
...
BetterQuadPowerIC::BetterQuadPowerIC(const InputParameters & params)
...
void
BetterQuadPowerIC::initialSetup()
{
  auto nx = _mesh.getNx();
  auto ny = _mesh.getNy();
...
Real
BetterQuadPowerIC::value(const Point & p)
{
  auto nx = _mesh.getNx();
  auto ny = _mesh.getNy();
