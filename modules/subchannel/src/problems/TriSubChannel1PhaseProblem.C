...
//                        (1 + (-0.5 * std::log(lamda) + 0.5 * std::log(4.0) - 0.25) * lamda *
//                        lamda);
      ...
      for (unsigned int i_ch = 0; i_ch < _n_channels; i_ch++)
      {
        ...
        for (auto i_gap : _subchannel_mesh.getChannelGaps(i_ch))
        {
              ...
              (wire_lead_length != 0) && (wire_diameter != 0))
          {
            ...
            {
              sweep_enthalpy += _mixing_closure->computeMixingParameter(i_gap, iz) * edge_flux_ave *
                                Sij * (*_h_soln)(node_sweep_donor);
            }
            ...
            (wire_diameter != 0.0) && (wire_lead_length != 0.0))
        {
          ...
          for (auto i_gap : _subchannel_mesh.getChannelGaps(i_ch))
          {
                ...
                (subch_type_j == EChannelType::CORNER || subch_type_j == EChannelType::EDGE))
            {
              if ((ii_ch == sweep_donor) || (jj_ch == sweep_donor))
              {
                beta_in = _mixing_closure->computeMixingParameter(i_gap, iz);
              }
              else
              {
                beta_out = _mixing_closure->computeMixingParameter(i_gap, iz);
              }
            }
          }
