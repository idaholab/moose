...
SCMTriAssemblyMeshGenerator::SCMTriAssemblyMeshGenerator(const InputParameters & params)
...
void
SCMTriAssemblyMeshGenerator::buildPinMesh(MeshBase & mesh_base)
{
  if (_npins == 0)
    return;

  mesh_base.reserve_elem(_n_cells * _npins);
  mesh_base.reserve_nodes((_n_cells + 1) * _npins);
  ...
  for (unsigned int i = 0; i < _npins; i++)
  {
    _pin_nodes[i].reserve(_n_cells + 1);
    for (unsigned int iz = 0; iz < _n_cells + 1; iz++)
...
SCMTriAssemblyMeshGenerator::generate()
{
  ...
  mesh_base->reserve_elem(_n_cells * (_n_channels + _npins));
  mesh_base->reserve_nodes((_n_cells + 1) * (_n_channels + _npins));
  ...
  sch_mesh._pin_mesh_exist = (_npins > 0);
