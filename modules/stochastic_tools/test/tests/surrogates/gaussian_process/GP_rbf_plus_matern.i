...
[Trainers]
  [GP_avg_trainer]
    ...
    covariance_function = 'covar' #Use the combined RBF + Matern kernel
...
[Covariance]
  [rbf]
    ...
    noise_variance = 1e-6 #Numerical-stability noise lives here only;
  ...
  [matern_half]
    ...
    signal_variance = 1 #Use a signal variance of 1 in the kernel
    noise_variance = 0 #Set to zero to avoid double-counting noise in the Sum
    length_factor = '0.551133 0.551133' #Select a length factor for each parameter (k and q)
  []
  [covar]
    type = CovarianceCombiner
    covariance_functions = 'rbf matern_half'
    operation = Sum
  []
[]
