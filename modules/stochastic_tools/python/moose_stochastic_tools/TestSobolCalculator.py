...
if __name__ == "__main__":
    ...
    for i in range(n):
        print(
            "  EXPECT_NEAR(sobol[{:d}], {:.9f}, 1e-9); // S_{}".format(
                idx, S[i, i], i + 1
            )
        )
        idx += 1
    ...
    for i in range(n):
        print(
            "  EXPECT_NEAR(sobol[{:d}], {:.9f}, 1e-9); // ST_{}".format(
                idx, ST[i], i + 1
            )
        )
        idx += 1
    ...
    for i in range(n):
        print("    // S_{}".format(i + 1))
        print(
            "    EXPECT_NEAR(sobol[{:d}], {:.9f}, 1e-9); // Mean".format(idx, S[i, i])
        )
        for l in range(len(levels)):
            print(
                "    EXPECT_NEAR(sobol_ci[{:d}][{:d}], {:.9f}, 1e-9); // {:.0f}% CI".format(
                    l, idx, S_ci[l, i, i], levels[l] * 100
                )
            )
        idx += 1
    ...
    for i in range(n):
        for j in range(i):
            ...
            print(
                "    EXPECT_NEAR(sobol[{:d}], {:.9f}, 1e-9); // Mean".format(
                    idx, S[i, j]
                )
            )
            for l in range(len(levels)):
