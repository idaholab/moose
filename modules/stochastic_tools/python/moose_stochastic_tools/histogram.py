...
def command_line_options():
    ...
    parser = argparse.ArgumentParser(
        description="Command-line utility for creating histograms from VectorPostprocessor data."
    )
    parser.add_argument(
        "filename",
        type=str,
        help="The VectorPostprocessor data file pattern to open, for sample 'foo_x_*.csv'.",
    )
    parser.add_argument(
        "-t",
        "--timesteps",
        ...
        nargs="+",
        type=int,
        help="List of timesteps to consider, by default only the final timestep is shown.",
    )
    parser.add_argument(
        "-v",
        "--vectors",
    ...
    parser.add_argument(
        "--ylabel", default="Probability", type=str, help="The X-axis label."
    )
    parser.add_argument(
        "--uniform",
        default=None,
        type=float,
        nargs=2,
        help="Show a uniform distribution between a and b (e.g., --uniform 8 10).",
    )
    parser.add_argument(
        "--weibull",
        default=None,
        type=float,
        nargs=2,
        help="Show a Weibull distribution with given shape and scale parameters (e.g., --uniform 1 5).",
    )
