...
def command_line_options():
    ...
    parser = argparse.ArgumentParser(
        description="Command-line utility for visualizing or tabulating statistics computed from the stochastic tools module."
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        help="List of StatisticsReporter objects to consider, by default all objects are considered.",
    )
    parser.add_argument(
        "-val",
        "--values",
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        help="List of stats to consider, by default all stats are considered.",
    )
    parser.add_argument(
        "-ci",
        "--confidence-interval",
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=float,
        help="List of times to consider, by all times are considered.",
    )
    parser.add_argument(
        "-n",
        "--names",
    ...
    parser.add_argument(
        ...
        default="{}",
        type=str,
        help='Map between the statistic name and  what is displayed in json format. Example: \'{"MEAN" : "Average", "STDDEV" : "Standard Deviation"}\'',
    )
    parser.add_argument(
        "-nf",
        "--number-format",
    ...
    parser.add_argument(
        ...
        default="Time",
        type=str,
        help="The value to use for x value in the line plot. Default is time. This is only relevant with '--line-plot'.",
    )
    parser.add_argument(
        "--xlabel",
        default=None,
        type=str,
        help="The x-axis label in the line plot. Default is whatever '--xvalue' is. This is only relevant with '--line-plot'.",
    )
    ...
    for stat in stats:
        ...
        bars.append(
            go.Bar(
                ...
                x=vnames,
                y=df.value,
                error_y=dict(type="data", array=ciplus, arrayminus=ciminus),
            )
        )
...
def statTimeLine(data, xlabel):
    ...
    for vec in vecs:
        for stat in stats:
            df = data[(data.statistic == stat) & (data.vector_name == vec)].sort_values(
                by=["time"]
            )
            ...
            if len(ci_df) and len(vecs) > 1:
                name.append(
                    "({}%, {}%) CI".format(
                        ci_df.ci_levels.iloc[0][0] * 100,
                        ci_df.ci_levels.iloc[0][1] * 100,
                    )
                )
            if len(name):
                line["name"] = " ".join(name)
    ...
    if len(data[data.ci_levels != None]) and len(vecs) == 1:
        ylabel.append(
            "({}%, {}%) CI".format(
                data[data.ci_levels != None].ci_levels.iloc[0][0] * 100,
                data[data.ci_levels != None].ci_levels.iloc[0][1] * 100,
            )
        )
    ylabel = " ".join(ylabel) if len(ylabel) else "Value"

    return go.Figure(
        data=lines, layout=go.Layout(xaxis=dict(title=xlabel), yaxis=dict(title=ylabel))
    )
...
def statLine(data, xname):
    ...
    for vec in vecs:
        for stat in stats:
            df = ydata[
                (ydata.vector_name == vec) & (ydata.statistic == stat)
            ].sort_values(by=["time"])
            for time in df.time:
                yrow = df[df.time == time].iloc(0)[0]
                xrow = xdata[(xdata.statistic == stat) & (xdata.time == time)].iloc(0)[
                    0
                ]
                ...
                if isinstance(yrow["confidence_interval"], tuple):
                    ciminus = np.asarray(yrow["value"]) - np.asarray(
                        yrow["confidence_interval"][0]
                    )
                    ciplus = np.asarray(yrow["confidence_interval"][1]) - np.asarray(
                        yrow["value"]
                    )
                    line["error_y"] = dict(
                        type="data", array=ciplus, arrayminus=ciminus
                    )

                if isinstance(xrow["confidence_interval"], tuple):
                    ciminus = np.asarray(xrow["value"]) - np.asarray(
                        xrow["confidence_interval"][0]
                    )
                    ciplus = np.asarray(xrow["confidence_interval"][1]) - np.asarray(
                        xrow["value"]
                    )
                    line["error_x"] = dict(
                        type="data", array=ciplus, arrayminus=ciminus
    ...
    if len(y_ci_level) and len(vecs) == 1:
        ylabel.append(
            "({}%, {}%) CI".format(
                y_ci_level.iloc[0][0] * 100, y_ci_level.iloc[0][1] * 100
            )
        )
    ylabel = " ".join(ylabel) if len(ylabel) else "Value"
    ...
    if len(xdata[xdata.ci_levels.notna()]):
        xlabel += " ({}%, {}%) CI".format(
            x_ci_level.iloc[0][0] * 100, x_ci_level.iloc[0][1] * 100
        )

    return go.Figure(
        data=lines,
        layout=go.Layout(
            xaxis=dict(title=xlabel), yaxis=dict(title=ylabel), showlegend=True
        ),
    )
    ...
    for file in opt.filenames:
        ...
        for time in times:
            ...
            for var in data.variables():
                # Object name
                objname = var[0]
                if (len(opt.objects) and objname not in opt.objects) or data.info(
                    objname
                ...
                frame["vector_name"].append(
                    opt.names[vecname] if vecname in opt.names else vecname
                )
                frame["statistic"].append(
                    opt.stat_names[stat] if stat in opt.stat_names else stat
                )
                ...
                if not opt.ignore_ci and "confidence_intervals" in repinfo:
                    ...
                    ci1, ci2 = (
                        val[1][levels.index(ci_levels[0])],
                        val[1][levels.index(ci_levels[1])],
                    )
