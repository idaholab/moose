...
def command_line_options():
    ...
    parser = argparse.ArgumentParser(
        description="Command-line utility for creating histograms from VectorPostprocessor or Reporter data."
    )
    parser.add_argument(
        "filenames",
        nargs="+",
        type=str,
        help="The VectorPostprocessor or Reporter data file to open. Can be pattern for distributed data 'large_data.json.*'",
    )
    parser.add_argument(
        "-v",
        "--vectors",
        ...
        nargs="+",
        type=str,
        help="List of vector names to consider, by default all vectors are shown.",
    )
    parser.add_argument(
        "--names",
        default=[],
        nargs="+",
        type=str,
        help="Name to show on legend, default is no legend.",
    )
    parser.add_argument(
        "--probability",
        default=True,
        type=bool,
        help="True to plot with probability density normalization.",
    )
    parser.add_argument(
        "--bins", default=None, type=int, help="Number of bins to consider."
    )
    parser.add_argument(
        "--alpha",
        default=0.75,
        type=float,
        help="Set the bar chart opacity alpha setting.",
    )
    parser.add_argument("--xlabel", default="Value", type=str, help="The X-axis label.")
    parser.add_argument(
        "--ylabel", default="Probability", type=str, help="The Y-axis label."
    )
    parser.add_argument("--title", default=None, type=str, help="Title of figure.")
    parser.add_argument(
        "--output", default=None, type=str, help="File name to save figure"
    )
            ...
            if ".csv" in fname:
                ...
                for vec in vars:
                    ...
                    if obj_val[-1] == "converged":
                        continue
                    val = (
                        vdata[vec][ind[obj_val[0]]].tolist()
                        if obj_val[0] in ind
                        else vdata[vec].tolist()
                    )
                    if vec in data:
                        data[vec].extend(val)
                    else:
                ...
                for vec in vars:
                    ...
                    if (
                        obj_val[-1] == "converged"
                        or rdata.info(vec)["type"] != "std::vector<double>"
                    ):
                        continue
                    val = (
                        np.array(rdata[vec])[ind[obj_val[0]]].tolist()
                        if obj_val[0] in ind
                        else rdata[vec]
                    )
                    if vec[1] in data:
