...
def command_line_options():
    ...
    parser = argparse.ArgumentParser(
        description="Command-line utility for visualizing or tabulating sobol indices computed from the stochastic tools module."
    )

    parser.add_argument("--text-table", dest="format", action="store_const", const=0)
    parser.add_argument(
        "--markdown-table", dest="format", action="store_const", const=1
    )
    ...
    parser.add_argument(
        "filename",
        type=str,
        help="The Reporter data file to open, for sample 'foo_out.json' or 'foo_out_*.json'.",
    )
    parser.add_argument(
        "-obj",
        "--object",
        default=None,
        type=str,
        help="StatisticsReporter object to consider, by default the first object found is used.",
    )
    parser.add_argument(
        "-val",
        "--values",
        ...
        nargs="+",
        type=str,
        help="List of values to consider, by default all values are considered.",
    )
    parser.add_argument(
        "-s",
        "--stat",
        default="TOTAL",
        type=str,
        help="Type of sobol index to output. Options: 'TOTAL' (default), 'FIRST_ORDER', 'SECOND_ORDER'.",
    )
    parser.add_argument(
        "-ci",
        "--confidence-interval",
        ...
        nargs=2,
        type=float,
        help="Pair of confidence interval levels to use, by default the largest and smallest are used.",
    )
    parser.add_argument(
        "-t",
        "--time",
        default=None,
        type=float,
        help="Time to consider, by default the last time is used.",
    )
    parser.add_argument(
        "-n",
        "--names",
        default="{}",
        type=str,
        help='Map between the value name and display name in json format. Example: \'{"value1_name" : "Value One", "value2_name" : "Value Two"}\'',
    )
    parser.add_argument(
        "-p",
        "--param-names",
        ...
        default=[],
        type=str,
        help="List of names to display for the parameters.",
    )
    parser.add_argument(
        "-nf",
        "--number-format",
        default=".4g",
        type=str,
        help="The number format for tables.",
    )
    parser.add_argument(
        "-o",
        "--output",
        default=None,
        type=str,
        help="File name to save figure or table",
    )
    parser.add_argument(
        "--ignore-ci",
        action="store_true",
        help="Use this argument to ignore confidence intervals.",
    )
    parser.add_argument(
        "--log-scale",
        action="store_true",
        help="Use this argument to plot on logrithmic scale.",
    )
...
def totalTable(data, num_form):
    ...
    if data["ci_levels"] is not None:
        ind_head += " ({}%, {}%) CI".format(
            data["ci_levels"][0] * 100, data["ci_levels"][1] * 100
        )
    tab[ind_head] = []
        ...
        for i, pname in enumerate(data["param_names"]):
            if data["ci_levels"] is not None:
                tab[pname].append(
                    "{:{nf}} ({:{nf}}, {:{nf}})".format(
                        data[vname]["val"][i],
                        data[vname]["ci_minus"][i],
                        data[vname]["ci_plus"][i],
                        nf=num_form,
                    )
                )
            else:
    ...
    for i, pnamei in enumerate(data["param_names"]):
        for j, pnamej in enumerate(data["param_names"]):
            ...
            if data["ci_levels"] is not None:
                ...
                error_y["arrayminus"] = [
                    data[vname]["val"].item(ind) - data[vname]["ci_minus"].item(ind)
                    for vname in data["vector_names"]
                ]
                error_y["array"] = [
                    data[vname]["ci_plus"].item(ind) - data[vname]["val"].item(ind)
                    for vname in data["vector_names"]
                ]
...
def heatmap(data, stat, log_scale):
    ...
    fig = plotly.subplots.make_subplots(
        rows=len(vnames),
        cols=1,
        subplot_titles=vnames,
        vertical_spacing=0.15 / len(vnames),
    )
    ...
    for i, vname in enumerate(vnames):
        ...
        colorbar = go.heatmap.ColorBar(
            title=dict(text=ctitle, side="right"),
            x=1,
            y=(i + 0.5) / len(vnames),
            len=1 / len(vnames),
            tickvals=ztick,
        ...
        fig.add_trace(
            go.Heatmap(
                x=data["param_names"], y=data["param_names"], z=z, colorbar=colorbar
            ),
            row=i + 1,
            col=1,
        )
...
def visualize_sobol(opt: VisualizeSobolOptions):
    ...
    if opt.object is None:
        opt.object = next(
            ...
            if data.info(var[0])["type"] == "SobolReporter"
        )
    ...
    frame["param_names"] = [
        (opt.param_names[i] if i < len(opt.param_names) else str(i))
        for i in range(repinfo["num_params"])
    ]
    ...
    if not opt.ignore_ci and "confidence_intervals" in repinfo:
        ...
        frame["ci_levels"] = (
            ...
            if opt.confidence_interval is None
            else tuple(opt.confidence_interval)
        )
    for val in opt.values:
        ...
        if levels:
            frame[vn]["ci_minus"] = np.array(
                repv[1][levels.index(frame["ci_levels"][0])]
            )
            frame[vn]["ci_plus"] = np.array(
                repv[1][levels.index(frame["ci_levels"][1])]
            )
