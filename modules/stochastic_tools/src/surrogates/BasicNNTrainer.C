...
BasicNNTrainer::initializeParameters()
{
  ...
  _t_session = std::unique_ptr<tensorflow::ClientSession>(new tensorflow::ClientSession(_t_root));
                           ...
                           double & loss)
{
  ...
  TF_CHECK_OK(
      _t_session->Run({{_input_param_batch, param_batch}, {_input_response_batch, response_batch}},
