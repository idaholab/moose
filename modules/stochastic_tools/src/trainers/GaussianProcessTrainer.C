...
GaussianProcessTrainer::postTrain()
{
  ...
  if (isParamValid("monotone_constraint_points"))
  {
    ...
    torch::Tensor virtual_params = torch::empty({(long)n_virt, (long)_n_dims}, at::kDouble);
    auto virtual_params_accessor = virtual_params.accessor<Real, 2>();
    for (unsigned int i = 0; i < n_virt; ++i)
      for (unsigned int j = 0; j < (unsigned)_n_dims; ++j)
        virtual_params_accessor[i][j] = raw_pts[i * _n_dims + j];
    LibtorchUtils::moveToLibtorchDevice(virtual_params, _app.getLibtorchDevice());
  ...
  if (isParamValid("penalty_constraint_points"))
  {
    ...
    torch::Tensor penalty_pts = torch::empty({(long)n_penalty, (long)_n_dims}, at::kDouble);
    auto penalty_pts_accessor = penalty_pts.accessor<Real, 2>();
    for (unsigned int i = 0; i < n_penalty; ++i)
      for (unsigned int j = 0; j < (unsigned)_n_dims; ++j)
        penalty_pts_accessor[i][j] = raw_pts[i * _n_dims + j];
    LibtorchUtils::moveToLibtorchDevice(penalty_pts, _app.getLibtorchDevice());
    ...
    const Real z_mean = _gp.getDataStandardizer().getMean()[0].item<Real>();
    const Real z_std_val = _gp.getDataStandardizer().getStdDev()[0].item<Real>();
