...
#include "AIDESGPryTest.h"
#include "Sampler.h"
#include "DenseMatrix.h"

registerMooseObject("StochasticToolsApp", AIDESGPryTest);

InputParameters
AIDESGPryTest::validParams()
{
...
AIDESGPryTest::AIDESGPryTest(const InputParameters & parameters)
...
void
AIDESGPryTest::computeEvidence(std::vector<Real> & evidence, const DenseMatrix<Real> & input_matrix)
{
  std::vector<Real> tmp;
  if (_var_prior)
...
//   Real std_dev1;
  ...
  for (unsigned int i = 0; i < evidence.size(); ++i)
  {
    ...
    estimated_evidence += _estimated_loglikelihood[i];
    for (unsigned int j = 0; j < _priors.size(); ++j)
      tmp[j] = _data_prev(i, j);
    if (_var_prior)
      tmp[_priors.size()] = _var_prev[i];
    estimated_evidence -= _gp_eval.evaluate(tmp);
    evidence[i] = estimated_evidence;
  }
}

void
AIDESGPryTest::computeTransitionVector(std::vector<Real> & tv,
...
void
AIDESGPryTest::nextSamples(std::vector<Real> & req_inputs,
