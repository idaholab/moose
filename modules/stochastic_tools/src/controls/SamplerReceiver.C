...
SamplerReceiver::execute()
{
  ...
  for (const std::string & param_name : _parameters)
  {
    ...
    {
      ControllableParameter<std::vector<Real>> control_param = getControllableParameterByName<std::vector<Real>>(param_name);
      if (!control_param.empty())
      {
        ...
        for (const std::vector<Real> * ptr : control_param.get())
          if (ptr->size() != n)
            mooseError("The std::vector<Real> parameters beging controlled must all be the same size:\n", control_param.dump());

        // There must be enough data to populate the controlled parameter
        if (value_position + n > _values.size())
          mooseError("The supplied vector of Real values is not sized correctly, the std::vector<Real> parameter '", param_name, " requires ", n, " values but only ", _values.size() - value_position, " are available in the supplied vector.");

        // Set the value
        std::vector<Real> value(_values.begin() + value_position, _values.begin() + value_position + n);
        value_position += n;
    ...
    mooseError("Unable to locate a Real or std::vector<Real> parameter with the name '", param_name, ".'");
  }

  // Error if there is un-used values
  if (value_position != _values.size())
    mooseError("The number of values supplied (", _values.size(), ") does not match the number of values consumed by setting parameter values (", value_position, ").");
}
