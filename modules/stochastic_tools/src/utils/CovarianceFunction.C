...
makeCovarianceKernel(const MooseEnum & kernel_type, const GaussianProcessTrainer * gp)
{
  ...
  if (kernel_type==2){
    ::mooseError("General Matern covariance function not yet implemented. Half-integer Matern implemented in MaternHalfInt if desired.");
    return nullptr;
  }
...
RealEigenMatrix
CovarianceKernel::compute_K(const RealEigenMatrix /*x*/, const RealEigenMatrix /*xp*/, const bool /*is_self_covariance*/) const
{
...
void CovarianceKernel::set_signal_variance(const Real sigma_f_squared)
{
  _sigma_f_squared = sigma_f_squared;
}

void CovarianceKernel::set_noise_variance(const Real sigma_n_squared)
{
...
void CovarianceKernel::set_p(const unsigned int /*p*/)
{
  ::mooseError("Attempting to set p hyper parameter for covariance function which does not utilize.");
}
...
RealEigenMatrix SquaredExponential::compute_K(const RealEigenMatrix x, const RealEigenMatrix xp, const bool is_self_covariance) const
{
...
RealEigenMatrix Exponential::compute_K(const RealEigenMatrix x, const RealEigenMatrix xp, const bool is_self_covariance) const
{
  unsigned int num_samples_x = x.rows();
  unsigned int num_samples_xp = xp.rows();
  unsigned int num_params_x = x.cols();
...
MaternHalfInt::MaternHalfInt()
  : CovarianceKernel()
{
}

RealEigenMatrix MaternHalfInt::compute_K(const RealEigenMatrix x, const RealEigenMatrix xp, const bool is_self_covariance) const
{
  unsigned int num_samples_x = x.rows();
  unsigned int num_samples_xp = xp.rows();
  unsigned int num_params_x = x.cols();
  ...
  for (unsigned int ii=0; ii<num_samples_x; ii++){
    for (unsigned int jj=0; jj<num_samples_xp; jj++){
        ...
        r_scaled = sqrt(r_scaled);
        Real summation=0;
        //tgamma(x+1) == x! when x is a natural number, which should always be the case for MaternHalfInt
        for (unsigned int tt=0; tt<_p+1; tt++){
          summation += (tgamma(_p + tt + 1)/(tgamma(tt+1) * tgamma(_p - tt + 1))) * pow(2* factor * r_scaled, _p-tt);
        }
        K(ii,jj) =  _sigma_f_squared * std::exp(- factor * r_scaled) * (tgamma(_p + 1)/(tgamma(2*_p + 1))) * summation;
      }
    if (is_self_covariance){
      K(ii,ii) += _sigma_n_squared;
    }
...
void MaternHalfInt::set_p(const unsigned int p)
{
  _p = p;
}
