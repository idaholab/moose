                                                        ...
                                                        RealEigenMatrix & k)
{
  int n1 = x1.rows();
  int n2 = x2.rows();
  k = RealEigenMatrix::Zero(n1, n2);
  Real theta = std::max(lengthscale(0, 0), 1e-12);
  ...
  for (int i = 0; i < n1; ++i)
  {
    for (int j = 0; j < n2; ++j)
    {
      ...
      Real dist2 = diff.squaredNorm();

      Real cov_val = scale * std::exp(-dist2 / theta);
      k(i, j) = cov_val;
    }
  }
...
// void
// TwoLayerGaussianProcess::logl(const RealEigenMatrix & out_vec, const RealEigenMatrix & x1, const
// RealEigenMatrix & x2, Real noise, const RealEigenMatrix & lengthscale,
//           LogLResult & result, bool outer, bool scale, Real outerscale) {
//   int n = out_vec.rows();
...
// }
void
TwoLayerGaussianProcess::logl(const RealEigenMatrix & out_vec,
                              ...
                              Real outerscale)
{
  const unsigned int n = out_vec.rows();
  RealEigenMatrix K(x1.rows(), x2.rows());
  ...
  if (quadterm <= 0.0 || !std::isfinite(quadterm))
  {
    result.logl = -INFINITY;
    result.scale = NAN;
    return;
  }
  Real logl_val;
  if (outer)
  {
    logl_val = -0.5 * n * std::log(quadterm) - 0.5 * ldet;
  }
  else
  {
    logl_val = -0.5 * ldet - 0.5 * quadterm;
  }
  Real scale_val = cal_scale ? (quadterm / static_cast<Real>(n)) : NAN;
  result.logl = logl_val;
  result.scale = scale_val;
}

void
TwoLayerGaussianProcess::sampleNoise(const RealEigenMatrix & out_vec,
                                     const RealEigenMatrix & x1,
                                     const RealEigenMatrix & x2,
                                     const Real noise_t,
                                     const RealEigenMatrix lengthscale,
                                     ...
                                     SampleNoiseResult & result)
{
  ...
  Real ll_new = ll_result.logl;

  const Real new_val = ll_new + std::log(Gamma::pdf(noise_star - 1.5e-8, alpha, beta));
...
void
TwoLayerGaussianProcess::sampleLengthscale(const RealEigenMatrix & out_vec,
                                           const RealEigenMatrix & x1,
                                           const RealEigenMatrix & x2,
                                           Real noise,
                                           const RealEigenMatrix & lengthscale_t,
                                           const unsigned int i,
                                           const Real alpha,
                                           const Real beta,
                                           const Real l,
                                           const Real u,
                                           SampleLengthscaleResult & result,
                                           Real ll_prev,
                                           const bool outer,
                                           const bool cal_scale)
{

  RealEigenMatrix lengthscale_star = lengthscale_t;
  const Real ru1 = MooseRandom::rand();
  const Real ru = MooseRandom::rand();
  lengthscale_star(0, i) =
      Uniform::quantile(ru1, l * lengthscale_t(0, i) / u, u * lengthscale_t(0, i) / l);
  ...
  const Real lpost_threshold = ll_prev + std::log(Gamma::pdf(lengthscale_t(0, i) - 1.5e-8, alpha, beta)) +
                         std::log(ru) - std::log(lengthscale_t(0, i)) +
                         std::log(lengthscale_star(0, i));


  LogLResult ll_result;
  logl(out_vec, x1, x2, noise, lengthscale_star, ll_result, outer, cal_scale);
  Real ll_new = ll_result.logl;
  Real scale_new = ll_result.scale;

  const Real new_val = ll_new + std::log(Gamma::pdf(lengthscale_star(0, i) - 1.5e-8, alpha, beta));
  if (new_val > lpost_threshold)
  {
...
void
TwoLayerGaussianProcess::multiVariateNormalSampling(const RealEigenMatrix & mean,
                                                    const RealEigenMatrix & cov,
                                                    const unsigned int n_dim,
                                                    const unsigned int n_draw,
                                                    RealEigenMatrix & final_sample_matrix)
{
  Eigen::LLT<RealEigenMatrix> cho_decomp = cov.llt();
  const RealEigenMatrix L = cho_decomp.matrixL();
...
void
TwoLayerGaussianProcess::sampleW(const RealEigenMatrix & out_vec,
                                 RealEigenMatrix & w_t,
                                 ...
                                 const RealEigenMatrix & prior_mean)
{
  ...
  for (unsigned int i = 0; i < x1.cols(); i++)
  {
    ...
    squared_exponential_covariance(x1, x2, 1, lengthscale_w_i, 0, K);
...
void
TwoLayerGaussianProcess::krig(const RealEigenMatrix & y,
                              ...
                              RealEigenMatrix & krig_sigma)
{
  ...
  squared_exponential_covariance(x, x, 1, lengthscale, noise, C);
  squared_exponential_covariance(x_new, x, 1, lengthscale, 0, C_cross);

  Eigen::LLT<RealEigenMatrix> llt(C);
  RealEigenMatrix C_inv = llt.solve(RealEigenMatrix::Identity(C.rows(), C.cols()));

  krig_mean = prior_mean_new + C_cross * C_inv * (y - prior_mean);

  if (cal_sigma)
  {
    RealEigenMatrix quad_term = C_cross * C_inv * C_cross.transpose();
    squared_exponential_covariance(x_new, x_new, 1, lengthscale, noise, C_new);
    krig_sigma = outerscale * (C_new - quad_term);
    // krig_sigma = (C_new - quad_term);
  }
}
...

{
  ...
  for (unsigned int j = 1; j < nmcmc; ++j)
  {
    ...
    //   sampleLengthscale(y, w[j-1], w[j-1], noise(j,0), lengthscale_y.row(j-1), i,
    //   settings.alpha.lengthscale_y, settings.beta.lengthscale_y, settings.l,
    ...
    sampleLengthscale(y,
                      ...
                      lengthscale_y.row(j - 1),
                      0,
                      settings.alpha.lengthscale_y,
                      settings.beta.lengthscale_y,
                      settings.l,
                      settings.u,
                      r_y,
                      ll,
    ...
    for (unsigned int i = 0; i < x.cols(); i++)
    {
      ...
      sampleLengthscale(w[j - 1].col(i),
                        x,
                        x,
                        noise,
                        lengthscale_w.row(j - 1),
                        i,
                        settings.alpha.lengthscale_w,
                        settings.beta.lengthscale_w,
                        settings.l,
                        settings.u,
                        sample_lengthscale_w_result,
                        ll,
                        false,
                        false);
