...
GaussianProcessTorched::GPOptimizerOptions::GPOptimizerOptions(const bool show_every_nth_iteration,
...
void
GaussianProcessTorched::setupCovarianceMatrix(const RealEigenMatrix & training_params,
...
void
GaussianProcessTorched::generateTuningMap(const std::vector<std::string> & params_to_tune,
...
void
GaussianProcessTorched::standardizeParameters(torch::Tensor & data, bool keep_moments)
{
  if (!keep_moments)
...
void
GaussianProcessTorched::standardizeData(torch::Tensor & data, bool keep_moments)
{
  if (!keep_moments)
...
void
GaussianProcessTorched::tuneHyperParamsAdam(const RealEigenMatrix & training_params,
                                            const RealEigenMatrix & training_data,
                                            const GPOptimizerOptions & opts)
{
...
void
GaussianProcessTorched::mapToVec(
...
void
GaussianProcessTorched::vecToMap(
    const std::unordered_map<std::string, std::tuple<unsigned int, unsigned int, Real, Real>> &
        tuning_data,
...
dataStore(std::ostream & stream,
          StochasticToolsTorched::GaussianProcessTorched & gp_utils,
          void * context)
{
