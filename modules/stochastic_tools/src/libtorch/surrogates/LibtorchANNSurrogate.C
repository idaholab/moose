...
LibtorchANNSurrogate::evaluate(const std::vector<Real> & x) const
{
  ...
  auto input_mean_accessor = input_mean.accessor<Real, 1>();
  auto input_std_accessor = input_std.accessor<Real, 1>();
  ...
  auto output_mean_accessor = output_mean.accessor<Real, 1>();
  auto output_std_accessor = output_std.accessor<Real, 1>();
  ...
  val = val * output_std_accessor[0] + output_mean_accessor[0];
