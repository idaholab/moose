...
#include "TwoLayerGaussianProcessData.h"

registerMooseObject("StochasticToolsApp", TwoLayerGaussianProcessData);

InputParameters
TwoLayerGaussianProcessData::validParams()
{
  ...
  params.addRequiredParam<UserObjectName>("tgp_name", "Name of TwoLayerGaussianProcess.");
  return params;
}

TwoLayerGaussianProcessData::TwoLayerGaussianProcessData(const InputParameters & parameters)
  : GeneralVectorPostprocessor(parameters),
    SurrogateModelInterface(this),
    _tgp_surrogate(getSurrogateModel<TwoLayerGaussianProcessSurrogate>("tgp_name"))
{
}

void
TwoLayerGaussianProcessData::initialize()
{
  const std::unordered_map<std::string, Real> & _hyperparam_map =
      _tgp_surrogate.getTGP().getHyperParamMap();
  const std::unordered_map<std::string, std::vector<Real>> & _hyperparam_vec_map =
      _tgp_surrogate.getTGP().getHyperParamVectorMap();
