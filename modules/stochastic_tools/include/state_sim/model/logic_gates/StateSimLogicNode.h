...
public:
   ...
   */
  void addGateChild(StateSimLogicNode & child) {_sub_gates[child.id()] = &child;};

  /**
   * removeGateChild - remove a child gate in this node
   * @param child - child to remove
   */
  void removeGateChild(const StateSimLogicNode & child) {_sub_gates.erase(child.id());};

  /**
   * addCompChild - add a child component to this node
   * @param child - child to add
   */
  void addCompChild(StateSimDiagramEval & child) {_comp_children[child.id()] = &child;};

  /**
   * removeCompChild - remove a child component in this node
   * @param child - child to remove
   */
  void removeCompChild(const StateSimDiagramEval & child) {_comp_children.erase(child.id());};
