...
def command_line_options():
    ...
    parser.add_argument(
        "--mpicmd", default="mpiexec", type=str, help="MPI command, default is mpiexec."
    )
    parser.add_argument(
        "--np",
        default=1,
        type=int,
        help="Number of processors to use for calculation, default is 1.",
    )
    parser.add_argument(
        "--runfom",
        default=1,
        type=int,
        help="Flag to run the full-order model for reference, default is True.",
    )
    parser.add_argument(
        "--runtrainer",
        default=1,
        type=int,
        help="Flag to run the trainer, default is True.",
    )
...
if __name__ == "__main__":
    ...
    fom_cmd = (
        ...
        + " -n "
        + str(opt.np)
        + " ../../../../stochastic_tools-opt -i full_order.i --allow-test-objects"
    )
    ...
    for no_modes in no_pod_modes:
        ...
        rel_diff = abs(
            fom_data["results:nodal_l2"].values - rom_data["rbpod:nodal_l2"].values
        ) / abs(fom_data["results:nodal_l2"].values)
