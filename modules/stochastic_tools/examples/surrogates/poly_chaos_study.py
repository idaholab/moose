...
def command_line_options():
    ...
    parser = argparse.ArgumentParser(
        description="Run case study for polynomial chaos with 1D heat conduction model."
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    train_mc_cmd = (
        ...
        + " -n "
        + str(opt.np)
        + " ../../stochastic_tools-opt -i poly_chaos_"
        + opt.dist
        + "_mc.i Outputs/csv=true"
    )
    train_quad_cmd = (
        opt.mpicmd
        + " -n "
        ...
        + " ../../stochastic_tools-opt -i poly_chaos_"
        + opt.dist
        + "_quad.i Outputs/csv=true"
    )
    uniform_cmd = (
        opt.mpicmd
        + " -n "
        + str(opt.np)
        + " ../../stochastic_tools-opt -i poly_chaos_uniform.i Samplers/active='' VectorPostprocessors/active='stats_avg stats_max'"
    )
    ...
    if opt.dist in ["uniform"]:
        ...
        t2 = 310
        a = 1.0 / 3.0
        mu_avg = a * np.log(k2 / k1) / (k2 - k1) * 0.5 * (q2**2 - q1**2) / (
            q2 - q1
        ) * 1.0 / 3.0 * (l2**3 - l1**3) / (l2 - l1) + 0.5 * (t2**2 - t1**2) / (
            t2 - t1
        )
        sig_avg = (
            a**2
            ...
            * (t2**2 - t1**2)
            / (t2 - t1)
            + 1.0 / 3.0 * (t2**3 - t1**3) / (t2 - t1)
            - mu_avg**2
        )
        sig_avg = np.sqrt(sig_avg)
        a = 0.5
        mu_max = a * np.log(k2 / k1) / (k2 - k1) * 0.5 * (q2**2 - q1**2) / (
            q2 - q1
        ) * 1.0 / 3.0 * (l2**3 - l1**3) / (l2 - l1) + 0.5 * (t2**2 - t1**2) / (
            t2 - t1
        )
        ...
        k2i = quad(
            normal_int, -np.inf, np.inf, args=((lambda k: 1.0 / k**2), km, ks)
        )[0]
        qi = 0
        q2i = 0
        li = 0
    ...
    for nrows in mc_rows:
        ...
        for i in range(nn):
            cmd = (
                train_mc_cmd
                + " Samplers/sample/num_rows="
                + str(nrows)
                + " Samplers/sample/seed="
                + str(i)
            )
    ...
    for ord in quad_order:
        ...
        for type in quad_type:
            ...
            cmd = (
                train_quad_cmd
                + " Samplers/sample/order="
                + str(ord)
                + " Samplers/sample/sparse_grid="
                + type
            )
            ...
            data = mooseutils.PostprocessorReader(
                "poly_chaos_training_samp_avg_0001.csv"
            )
    ...
    file_names = [
        "poly_chaos_avg_" + opt.dist + "_results.csv",
        "poly_chaos_max_" + opt.dist + "_results.csv",
    ]
    ...
    for ord in quad_order:
        ...
        for type in quad_type:
            ...
            cmd = (
                ...
                + " Samplers/sample/sparse_grid="
                + type
                + " Trainers/poly_chaos_avg/order="
                + str(ord)
                + " Trainers/poly_chaos_max/order="
                + str(ord)
            )
            print("Quad " + str(ord) + " order " + type + ": ")
            os.system(cmd + " > tmp.txt")
