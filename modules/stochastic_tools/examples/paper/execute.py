...
def get_args():
    parser = argparse.ArgumentParser(
        description="Utility for producing results, plots, and tables for STM paper"
    )
    parser.add_argument("--run", action="store_true", help="Perform simulations.")
    parser.add_argument(
        "--replicates", default=10, type=int, help="Number of replicates to perform."
    )
    parser.add_argument(
        "--base", default=128, type=int, help="The base number of samples to perform."
    )
    parser.add_argument(
        "--memory-levels",
        default=6,
        type=int,
        help="Number of levels to perform for memory/timing runs, n in [base*2^0, ..., base*2^n-1].",
    )
    parser.add_argument(
        "--memory-cores",
        default=32,
        type=int,
        help="Number of processors to use for memory/timing runs.",
    )
    parser.add_argument(
        "--weak-levels",
        default=7,
        type=int,
        help="Number of processor levels to perform for weak scaling, n in [2^0,...,2^n-1].",
    )
    parser.add_argument(
        "--write",
        default=1,
        type=int,
        help="Toggle writing to results directory when --run is used.",
    )
...
def plot(
    prefix, suffix, xname, yname, xlabel=None, ylabel=None, yerr=None, results=True
):
...
def table(prefix, write=True):
    ...
    for i, mode in enumerate(["normal", "batch-reset", "batch-restore"]):
        ...
        for idx, row in data.iterrows():
            ...
            times[key].append(
                (row["run_time"], row["run_time_min"], row["run_time_max"])
            )
    ...
    for key, value in times.items():
        ...
        out.append(
            r"{} & {} & {} & {} & {} \\".format(
                n_ranks, n_samples, normal, reset, restore
            )
        )
...
if __name__ == "__main__":
    ...
    if args.run:
        ...
        execute(
            input_file, prefix, "batch-reset", samples, mpi, args.replicates, args.write
        )
        execute(
            input_file,
            prefix,
    ...
    plot(
        "full_solve_memory_parallel",
        "time",
        xname="n_samples",
        xlabel="Number of Simulations",
        yname="run_time",
        ylabel="Time (sec.)",
        yerr=("run_time_min", "run_time_max"),
    )
