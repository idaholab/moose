...
def runner(infile, outfile, n_start, n_stop, mode, mpi):
    ...
    for n in range(n_start, n_stop + 1):
        ...
        exe_args = [
            "-i",
            infile,
            "Samplers/mc/num_rows={}".format(n_samples),
    ...
    if mpi:
        ...
        for mode in modes:
            p = multiprocessing.Process(
                target=runner, args=(infile, outfile, n_start, n_stop, mode, mpi)
            )
            p.start()
            jobs.append(p)
...
if __name__ == "__main__":
    ...
    execute(
        "transient.i", "transient_memory_serial", 0, 10, ["normal", "batch-restore"]
    )
    execute(
        "transient.i", "transient_memory_mpi", 5, 16, ["normal", "batch-restore"], 28
    )
