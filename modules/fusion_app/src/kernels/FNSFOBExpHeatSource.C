...
FNSFOBExpHeatSource::validParams()
{
  ...
  params.addClassDescription("Sets values using the functional form y = A*exp(-k*d) where d is the depth measured from the last closed flux surface in [m]. This IC is only intended to be used for the outboard blanket of the FNSF model.");
