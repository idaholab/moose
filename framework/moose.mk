...
$(moose_LIB): $(moose_objects) $(pcre_LIB) $(gtest_LIB) $(hit_LIB) $(pyhit_LIB) $(moose_revision_header)
	...
	bash -c '$(libmesh_LIBTOOL) --mode=install --quiet install -c $(moose_LIB) $(FRAMEWORK_DIR) 1> >(cat >&1) 2> >(grep -v "could not create compact unwind for" >&2)'
ifeq ($(MOOSE_HEADER_SYMLINKS),true)
...
$(exodiff_APP): $(exodiff_objects)
	...
	bash -c '$(libmesh_LIBTOOL) --tag=CXX $(LIBTOOLFLAGS) --mode=link --quiet \
	  $(libmesh_CXX) $(libmesh_CPPFLAGS) $(CXXFLAGS) $(libmesh_CXXFLAGS) $(libmesh_INCLUDE) $(exodiff_objects) -o $@ $(libmesh_LDFLAGS) $(libmesh_LIBS) $(EXTERNAL_FLAGS) > >(cat >&1) 2> >(grep -v "could not create compact unwind for" >&2)'
