...
user defined custom operators which may be raw a mfem::Operator optimised for a specific
purpose e.g. MHD with customised inputs that need thin layer access to the MOOSE multi-physics
system. As of yet only a single problem operator per MFEM problem is used, even if multiple
are defined. The ProblemOperatorBuilder classes are built within the MFEMProblem class however the
ProblemOperatos are built and owned by the MFEM executioners.
...
not need to be declared explicitly. The `ProblemOperatorBuilderSteady` class based upon 
systematic logic builds one of 3 ProblemOperators they are: `EquationSystemProblemOperator`
...
The custom problem operator example will follow the [unit-test](unit/src/MFEMCustomProblemOperator.C)
...
the MFEMSteady executioner (as in this example) then a the class inherits from 
`Moose::MFEM::ProblemOperator`. If the CustomProblemOperator class is transient i.e. called from
the MFEMTransient executioner then the class must inherit `Moose::MFEM::TimeDependentProblemOperator`.
For the case steady, an example class may look like this:
...
has been built e.g. the Solver, the lSolver, the FEspaces, the ParGridFunctions etc. A custom
operator builder function can be added in the case of non-linear problems where the operator is
...
jacobian, the self pointer would then be passed to the Non-linear solver via the 
`SetOperator(mfem::OperatorHandle &)` method. However as this case is linear the Operator is
simply built in the constructor. Firstly the class needs: the `Forms`, `Coefficients`
, BC `Arrays`, `Operators` and solution/problem `Vectors`.
...
Once the variables are declared, to build the `Forms` you need access to the FE-Spaces, and for the
post processors the `GridFunctions` are needed, to access them from MFEMProblem (Assuming that the
operator expects a specific naming convention:
...
data to the mfem `GridFunctions` so that the post-processors can be used. During construction as
the `Moose::MFEM::ProblemOperator` class has been constructed as well, the class inherits a
reference to the `MFEMProblem` namely `_problem_data` it can be used to acces `MFEMProblem`
members.
...
Once the `ProblemOperator` has been built the the `ProblemOperatorBuilder` class is needed
this class is constructed by `MFEMProblem` and called by the executioner. The builder class
must inherit from `ProblemOperatorBuilderBase` making is an `MFEMObject` and by proxy a 
...
be within the `Moose::MFEM` namespace for simplicity. An example minimal class may look
as follows:
...
these inputs can be filled in with the .i files in the ProblemOperatorBuilder block.
```
namespace Moose::MFEM
{
InputParameters ProblemOperatorBuilderCustomDummy::validParams()
{
  ...
  params.addParam<type>("name", "description", "Description of param");
  .
  .
  .
  return params;
...
options associated with it directly, but can also be used to retrieve the input params and store
them.
```
namespace Moose::MFEM
{
...
to the ProblemOperator that was defined earlier, and this is method called by the executioner,
