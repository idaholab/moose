/* PrismJS 1.29.0
https://prismjs.com/download.html#themes=prism-okaidia&languages=markup+css+clike+bash+c+cpp+diff+fortran+git+http+json+latex+makefile+markdown+python+yaml&plugins=line-numbers+normalize-whitespace+toolbar+copy-to-clipboard+diff-highlight */
...
Prism.languages.moose={comment:/(^|[^\\])#.*/,"moose-syntax":{pattern:/(?<=\[)(?:Adaptivity|Bounds|Mesh|MeshDivisions|MeshGenerators|MeshModifiers|Kernels|AuxKernels|ScalarKernels|AuxScalarKernels|Variables|AuxVariables|Materials|FunctorMaterials|Postprocessors|BCs|ICs|Executioner|Executors|Outputs|Problem|Physics|Debug|Preconditioning|UserObjects|Functions|GlobalParams|VectorPostprocessors|Dampers|DiracKernels|DGKernels|Constraints|NodalNormals|CoupledProblems|DeprecatedBlock|MultiApps|Transfers|InterfaceKernels|NodalKernels|Controls|Modules|Positions|Times|FluidProperties|SolidProperties|Closures|Components|ControlLogic|FVBCs|FVKernels|FVInterfaceKernels|Distributions|Likelihood|Samplers|Surrogates|VariableMappings|Reporters|OptimizationReporter|Splits|RayBCs|RayKernels)(?=\])/,alias:"keyword",lookbehind:!0},"moose-syntax-nested":{pattern:/(?<=\[\.\/)(?:TimeStepper|TimeSteppers|TimePeriods|Quadrature|Predictor|Adaptivity|Indicators|Markers|Periodic|InitialCondition|MortarInterfaces|Partitioner)(?=\])/,alias:"keyword",lookbehind:!0},"moose-object":{pattern:/((?:^|\s)\[)\w+(?=\])/,alias:"class-name",lookbehind:!0},"moose-type":{pattern:/\btype(?=\s*=\s*[^\s]+)/,alias:"keyword"},"moose-class":{pattern:/(?<=\btype\s*=\s*)[^\s]+\b/,alias:"function",greedy:!0},"moose-order":{pattern:/(\border\s*=\s*)CONSTANT|FIRST|SECOND|THIRD|FOURTH|FIFTH|SIXTH|SEVENTH|EIGHTH|NINTH/i,alias:"constant",lookbehind:!0},"moose-family":{pattern:/(\bfamily\s*=\s*)LAGRANGE|LAGRANGE_VEC|MONOMIAL|MONOMIAL_VEC|HERMITE|SCALAR|HIERARCHIC|SIDE_HIERARCHIC|CLOUGH|XYZ|SZABAB|BERNSTEIN|RATIONAL_BERNSTEIN|L2_LAGRANGE|L2_HIERARCHIC/i,alias:"constant",lookbehind:!0},"moose-elem-type":{pattern:/(\bfamily\s*=\s*)EDGE|EDGE2|EDGE3|EDGE4|QUAD|QUAD4|QUAD8|QUAD9|TRI3|TRI6|HEX8|HEX20|HEX27|HEX|TET4|TET10|PRISM6|PRISM15|PRISM18|PYRAMID5|PYRAMID13|PYRAMID14/,alias:"constant",lookbehind:!0},"moose-output-on":{pattern:/\b(?<=output_on\s*=\s*['"])(?:none|initial|linear|nonlinear|timestep_end|timestep_begin|final|failed|custom)(?:\s+(?:none|initial|linear|nonlinear|timestep_end|timestep_begin|final|failed|custom))*(?=['"])/i,alias:"constant",lookbehind:!0},"moose-execute-on":{pattern:/\b(?<=execute_on\s*=\s*['"])(?:none|initial|linear|nonlinear|timestep_end|timestep_begin|multiapp_fixed_point_begin|multiapp_fixed_point_begin|final|custom|always|forward|adjoint)(?:\s+(?:none|initial|linear|nonlinear|timestep_end|timestep_begin|multiapp_fixed_point_begin|multiapp_fixed_point_begin|final|custom|always|forward|adjoint))*(?=['"])/i,alias:"constant",lookbehind:!0},number:/\b0(?:b(?:_?[01])+|o(?:_?[0-7])+|x(?:_?[a-f0-9])+)\b|(?:\b\d+(?:_\d+)*(?:\.(?:\d+(?:_\d+)*)?)?|\B\.\d+(?:_\d+)*)(?:e[+-]?\d+(?:_\d+)*)?j?(?!\w)/i,boolean:/\b(?i:false|true)\b/,operator:/[\+\*\/\^%\-]/,builtin:/\b(?:abs|acos|acosh|arg|asin|atan|atan2|atanh|cbrt|ceil|conj|cos|cosh|cot|csc|exp|exp2|floor|hypot|if|imag|int|log|log10|log2|max|min|polar|pow|real|sec|sin|sinh|sqrt|tan|tanh|trunc|plog)\b/,punctuation:/[{}\(\)'"]/,property:/[a-zA-Z0-9_]+\b(?!\s*=)/};
