...
to roundoff. Setting `verify = level_operators` checks that after every linearization by applying each
...
nothing for the identity row to have to match. Setting `verify = operator_symmetry` measures the solver
...
`verify = operator_symmetry` for a transient system with Dirichlet-constrained degrees of freedom
