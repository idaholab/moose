...
When the user sets `default_block` in the `[Problem]` block, the specified block restrictions will be
automatically applied to all objects inheriting the BlockRestrictable interface. However, users can
still override this behavior by explicitly specifying the `block` parameter within each object.
If an explicitly specified `block` is not a subset of the `default_block`, MOOSE will throw a warning
 to notify the user of the inconsistency.
