...
is computed as follows. If the element centroid (computed as an average of the element vertex positions) or any of its nodes is inside the `proximity mesh` as reported by its point locator, then the distance is zero and the element is always deleted. Else, we use heuristics to determine the distance.
...
Then this KNN object provides the distance from the `proximity_mesh` to the element by computing the distance to
