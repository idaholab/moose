...
The id of the new external boundary produced by the trimming can be assigned using [!param](/Mesh/MeshLineCutter/new_boundary_id). As either method creates TRI3 elements if there are QUAD4 elements crossed by the cutting line, and as EXODUS II format does not allow TRI3 and QUAD4 elements sharing the same subdomain id and name. The newly formed TRI3 subdomains' ids and names can be customized using [!param](/Mesh/MeshLineCutter/tri_elem_subdomain_name_suffix) and [!param](/Mesh/MeshLineCutter/tri_elem_subdomain_shift).
...
The simple mesh shown above can be cut by `MeshLineCutter` with `CUT_ELEM` method using the following input block:
...
The complex mesh shown above can be cut by `MeshLineCutter` with `CUT_ELEM` method using the following input block:
