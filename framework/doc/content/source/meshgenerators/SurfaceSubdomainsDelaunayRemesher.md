...
The input of this mesh generator is defined by a surface mesh name ([!param](/Mesh/SurfaceSubdomainsDelaunayRemesher/input)) along with a list of subdomains names (with the [!param](/Mesh/SurfaceSubdomainsDelaunayRemesher/subdomain_names) parameter) within it. The generator will extract the specified subdomains from the input mesh and generate a 2D mesh through Delaunay triangulation within their (subdomain) boundary.
