...
Base class for MFEM DG kernels applied to the weak form being solved.
...
`MFEMDGKernel` is a purely virtual base class. Derived classes should override the `createBFIntegrator`
...
This base class provides default values for the DG parameters `sigma` and `kappa`, which are `-1.0` and
`(fe_order+1)^2` respectively.
