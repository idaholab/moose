...
`MFEMCoordinateCoefficients` is a base class. Derived classes should override `build()` to construct any required coefficients and `getBuiltinCoefficient(const std::string & name) const` to provide name based lookup of those coefficients. This allows coordinate depedent coefficients to be used through the normal MFEM scalar coefficient path, ie. `MFEMGenericFunctorMaterial` property definitions or MFEM kernels.
