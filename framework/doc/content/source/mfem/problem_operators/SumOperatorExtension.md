...
using the nonlinear forms' `GetDNFI()/GetBNFI()` methods.

We make the choice here to have the diagonal value on essential rows to be 1. To achieve this, we make sure
that the `DIAG_POLICY` for the gradient of the nonlinear form is 0, and for the constrained linear system operator
it's 1.
