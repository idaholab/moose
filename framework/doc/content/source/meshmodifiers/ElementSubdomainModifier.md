...
To apply different (re)initialization strategies, the user must set the `reinitialize_variables` parameter. This requires a one-to-one mapping with `reinitialization_strategy` (or a single global `reinitialization_strategy`) to determine the specific approach for each variable. If no strategy is explicitly defined, the default behavior is to reinitialize each variable to its initial condition as provided in the input file.

There are five available (re)initialization strategies:
- `NONE`: No (re)initialization is performed. This effectively sets newly activated DoFs to zero.
