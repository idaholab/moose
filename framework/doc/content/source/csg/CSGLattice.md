...
- Public Methods:
  ...
  - `getDimensions`: returns a map of the dimension attributes to their values
...
- Protected Methods:
  ...
  - `compareDimensions`: checks if the dimension attributes are identical to those of another lattice instance

### Defining a Valid Universe Layout

The rules of what makes a valid universe layout will varie depending on the lattice type.
...
The `getDimensions()` method returns a map of `std::any` data types (`std::unordered_map<std::string, std::any>`) to support flexibility in the type of data that might need to be defined.
...
These attributes are also what gets compared in the `compareDimensions()` method, which is used by the equality operators (`==` and `!=`) to check for lattice equality.
