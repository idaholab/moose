...
def createDOX(args):
    ...
    print(
        "Wrote to file:",
        args.application + "/" + args.application.split("/").pop() + ".dox",
    )


def _verifyARGs(args):
    if os.path.exists(args.application):
        args.file_object = open(
            args.application + "/" + args.application.split("/").pop() + ".dox", "w"
        )
        return args
    else:
...
def _parseARGs(args=None):
    parser = argparse.ArgumentParser(
        description="Build dox file for every input file. (Deletes DOX file if present)"
    )
