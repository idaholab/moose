...
struct IClientStream
{
  ...
  template <
      typename T,
      typename Chk1 =
          typename std::enable_if<std::is_same<decltype(T().data()), uint8_t *>::value>::type,
...
struct StdinClientStream : IClientStream
{
  ...
  size_t receive(void * target, size_t max) override { return fread(target, 1, max, stdin); }
  std::string receiveLine(bool /*asciiOnly = true*/, size_t /*max = -1*/) override
  {
    std::string res;
...
struct ICanRequestProtocolHandover
{
  ...
  virtual void acceptHandover(short & serverSock,
                              IClientStream & client,
                              std::unique_ptr<HttpRequest> srcRequest) = 0;
};
...
struct WebsockClientHandler
{
  ...
  void sendJson(const miniJson::Json & json) { sendText(json.serialize()); }
...
public:
  ...
  void requestProtocolHandover(ICanRequestProtocolHandover * newOwner) { mHandover = newOwner; }
...
struct HandlerBuilder
{
  ...
  virtual std::unique_ptr<HttpResponse> process(const HttpRequest & /*req*/) { return nullptr; }
};
...
class WebsockHandlerBuilder : public HandlerBuilder, public ICanRequestProtocolHandover
{
  ...
  {
    virtual WebsockClientHandler * makeInstance() { return new T(); }
  };

  std::unique_ptr<Factory> mFactory;

public:
  WebsockHandlerBuilder() : mFactory{new FactoryT<WebsockClientHandler>} {}
  ...
  void acceptHandover(short & serverSock,
                      IClientStream & client,
                      std::unique_ptr<HttpRequest> srcRequest) override;
};
#endif
...
public:
  ...
  {
    mHandlers.insert(
        std::pair<HttpRequestMethod, HandlerFunc>(HttpRequestMethod::POST, std::move(h)));
    return this;
  }
  ...
  {
    mHandlers.insert(
        std::pair<HttpRequestMethod, HandlerFunc>(HttpRequestMethod::GET, std::move(h)));
    return this;
  }
  ...
  template <typename T>
  HttpHandlerBuilder * posted(T x)
  {
    return posted(HandlerFunc(x));
  }

  template <typename T>
  HttpHandlerBuilder * requested(T x)
  {
    return requested(HandlerFunc(x));
  }
...
public:
  ...
  std::shared_ptr<WebsockHandlerBuilder> websocket(std::string path)
  {
    ...
    mHandlers.insert(mHandlers.begin(),
                     std::pair<std::string, std::shared_ptr<WebsockHandlerBuilder>>(path, h));
    return h;
  }
#endif
  ...
  {
    auto h = std::make_shared<HttpHandlerBuilder>();
    mReHandlers.push_back(
        std::pair<std::regex, std::shared_ptr<HttpHandlerBuilder>>(std::regex(path), h));
    return h;
  }
