...
public:
   ...
   */
  void addDistributionInContainer(const std::string & type,
                                  const std::string & name,
                                  MooseSharedPointer<BasicDistribution> dist);
  void addDistributionInContainerND(const std::string & type,
                                    const std::string & name,
                                    MooseSharedPointer<BasicDistributionND> dist);

  void seedRandom(unsigned int seed);

  bool isEmpty() { return _dist_by_name.empty() && _dist_nd_by_name.empty(); };
  ...
  double pdf(const std::string dist_alias, double x); // return pdf value of the distribution _type
                                                      // as function of the position x within [_xMin
                                                      // , xMax]
  double pdf(const char * dist_alias, std::vector<double> x);
  double pdf(const std::string dist_alias, std::vector<double> x); // return pdf value of the
                                                                   // distribution _type as function
                                                                   // of the position x within
                                                                   // [_xMin , xMax]
  ...
  double cdf(const std::string dist_alias, double x); // return cdf value of the distribution _type
                                                      // as function of the position x within [_xMin
                                                      // , xMax]
  double cdf(const char * dist_alias, std::vector<double> x);
  double cdf(const std::string dist_alias, std::vector<double> x); // return cdf value of the
                                                                   // distribution _type as function
                                                                   // of the position x within
                                                                   // [_xMin , xMax]

  /**
   * Function to get a random number distributed according to the distribution with a random number
   * calculated.
