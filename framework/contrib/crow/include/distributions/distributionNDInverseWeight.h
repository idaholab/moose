...
{
public:
  BasicMultiDimensionalInverseWeight(const char * data_filename, double p, bool cdf_provided)
    : _interpolator(data_filename, p)
  {
    _cdf_provided = cdf_provided;
    basicMultiDimensionalInverseWeightInit();
  };

  BasicMultiDimensionalInverseWeight(std::string data_filename, double p, bool cdf_provided)
    : _interpolator(data_filename, p)
  {
    _cdf_provided = cdf_provided;
    basicMultiDimensionalInverseWeightInit();
  ...
  void basicMultiDimensionalInverseWeightInit()
  {
    ...
    if (_cdf_provided)
    {
      ...
      if (LBcheck == false)
        throwError("BasicMultiDimensionalInverseWeight Distribution error: CDF values given as "
    ...
    { // PDF is provided
      ...
      // BasicMultiDimensionalCartesianSpline(std::string data_filename,std::vector<double> alpha,
      // std::vector<double> beta, bool cdf_provided)
      ...
      _cdf_spline =
          BasicMultiDimensionalCartesianSpline(discretizations, PDFvalues, alpha, beta, false);
    }
  }
  ...
  double cdf(std::vector<double> x)
  {
    ...
    if (value > 1.0)
      throwError("BasicMultiDimensionalInverseWeight Distribution error: CDF value calculated is "
                 "above 1.0");
  ...
  void updateRNGparameter(double tolerance, double initial_divisions)
  {
    ...
    // std::cout<<"Distribution updateRNGparameter" << _tolerance <<  _initial_divisions <<
    // std::endl;
  };
  ...
  double inverseMarginal(double f, int dimension)
  {
    ...
    if ((f < 1.0) and (f > 0.0))
    {
      if (_cdf_provided)
      {
        throwError("BasicMultiDimensionalInverseWeight Distribution error: inverseMarginal "
      ...
      throwError("BasicMultiDimensionalInverseWeight Distribution error: CDF value for inverse "
