...
*/
/*
 *      This class contains a set of multi-dimensional interpolation functions for both scattered
 * data and data lying on a cartesian grid
 *
 *      Sources:
 *      - General
 *        * Numerical Recipes in C++ 3rd edition
 *      - MD spline
 *        * Christian Habermann, Fabian Kindermann, "Multidimensional Spline Interpolation: Theory
 * and Applications", Computational Economics, Vol.30-2, pp 153-169 (2007)
...
protected:
  ...
  void refinedCellDivision(std::vector<std::vector<std::vector<double>>> & refined_cell,
                           std::vector<std::vector<double>> & cell,
                           int divisions);
  std::vector<int> arrayConverter(int one_d_coordinate, int divisions, int n_dimensions);
  std::vector<std::vector<double>> generateNewCell(std::vector<int> nd_coordinate,
                                                   std::vector<double> coordinate_of_point_zero,
                                                   std::vector<double> dxs,
                                                   int n_dimensions);
  std::vector<std::vector<double>>
  pickNewCell(std::vector<std::vector<std::vector<double>>> & cells_set, double g);
...
public:
  ...
  genericNDSpline(std::vector< std::vector<double> > & discretizations, std::vector<double> &
values, std::vector<double> alpha, std::vector<double> beta);
  void genericNDSpline_init(std::vector< std::vector<double> > & discretizations,
std::vector<double> & values, std::vector<double> alpha, std::vector<double> beta);
...
public:
  ...
  NDSpline(std::vector<std::vector<double>> & discretizations,
           std::vector<double> & values,
           std::vector<double> alpha,
           std::vector<double> beta);
  void ndSplineInit(std::vector<std::vector<double>> & discretizations,
                    std::vector<double> & values,
                    std::vector<double> alpha,
                    std::vector<double> beta);
...
private:
  ...
  std::vector<double> fillArrayCoefficient(int n_dimensions,
                                           std::vector<double> & data,
                                           std::vector<int> & loop_locator);

  void from2DTo1DRestructuring(std::vector<std::vector<double>> & two_d_data,
                               std::vector<double> & one_d_data);
  void from1DTo2DRestructuring(std::vector<std::vector<double>> & two_d_data,
                               std::vector<double> & one_d_data,
                               int spacing);
  ...
  void tridag(std::vector<double> & a,
              std::vector<double> & b,
              std::vector<double> & c,
              std::vector<double> & r,
              std::vector<double> & u);
  ...
  std::vector<std::vector<double>> tensorProductInterpolation(
      std::vector<std::vector<double>> step1, double h, double alpha, double beta);
