...
double
InverseDistanceWeighting::interpolateAt(std::vector<double> point)
{
  double value = 0;
  ...
  for (int i = 0; i < _number_of_points; i++)
  {
    ...
    {
      weights.at(i) =
          std::pow(1.0 / minkowskiDistance(point, _point_coordinates.at(i), _p), _dimensions + 1);
...
double
InverseDistanceWeighting::getGradientAt(std::vector<double> /* point */)
{
  // TO BE COMPLETED
  if (not _completed_init)
  {
...
void
InverseDistanceWeighting::fit(std::vector<std::vector<double>> coordinates,
                              std::vector<double> values)
{
