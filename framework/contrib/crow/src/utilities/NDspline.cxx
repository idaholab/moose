...
NDSpline::NDSpline(std::vector<std::vector<double>> & discretizations,
...
void
NDSpline::ndSplineInit(std::vector<std::vector<double>> & discretizations,
                       std::vector<double> & values,
                       std::vector<double> alpha,
                       std::vector<double> beta)
{
  ...
  */
  ...
  for (int nDim = 0; nDim < _dimensions; nDim++)
  {
    ...
    _hj.push_back((_discretizations.at(nDim).at(length - 1) - _discretizations.at(nDim).at(0)) /
                  (length - 1));
  ...
  for (int i = 0; i < _dimensions; i++)
  {
    ...
    _cell_dxs.push_back(_discretizations.at(i).at(_discretizations.at(i).size() - 1) -
                        _discretizations.at(i).at(0));
  }
...
double
NDSpline::interpolateAt(std::vector<double> point_coordinate)
{
  /**
  ...
  */
  ...
  if (outcome == true)
  {
    ...
    {
      throw("Error in interpolateAt: the class has not been completely initialized... you can not "
            "interpolate!!!!");
    }
  ...
  else
  {
    ...
    for (unsigned int i = 0; i < _values.size(); i++)
    {
      ...
      for (int nDim = 0; nDim < _dimensions; nDim++)
        distance =
            distance +
            pow((_discretizations.at(nDim).at(coordinates.at(nDim)) - point_coordinate.at(nDim)),
                2);
...
double
NDSpline::getGradientAt(std::vector<double> /* point_coordinate */)
{
  // TO BE COMPLETED
  double gradient = -1;
  if (not _completed_init)
  {
    throw("Error in getGradientAt: the class has not been completely initialized... you can not "
          "interpolate!!!!");
  }
  return gradient;
}
...
void
NDSpline::fit(std::vector<std::vector<double>> /* coordinates */, std::vector<double> /* values */)
{
...
double
NDSpline::splineCartesianInterpolation(std::vector<double> point_coordinate)
{
...
void
NDSpline::calculateCoefficients()
{
...
std::vector<double>
NDSpline::fillArrayCoefficient(int n_dimensions,
                               std::vector<double> & data,
                               std::vector<int> & loop_locator)
{
  ...
  for (unsigned int n = 0; n < _discretizations.at(n_dimensions - 1).size(); n++)
  {
    ...
    {                              // n=1
      ...
      tempCoefficients.push_back(getCoefficients(
          y, _hj.at(n_dimensions - 1), _alpha.at(n_dimensions - 1), _beta.at(n_dimensions - 1)));
      y.clear();
    }
  }
  std::vector<std::vector<double>> finalCoefficients =
      tensorProductInterpolation(tempCoefficients,
                                 _hj.at(n_dimensions - 1),
                                 _alpha.at(n_dimensions - 1),
                                 _beta.at(n_dimensions - 1));
...
std::vector<double>
NDSpline::coefficientRestructuring(std::vector<std::vector<double>> matrix)
{
...
NDSpline::tensorProductInterpolation(std::vector<std::vector<double>> step1,
                                     double h,
                                     double alpha,
                                     double beta)
{
...
std::vector<std::vector<double>>
NDSpline::matrixRestructuring(std::vector<std::vector<double>> step1)
{
  int rows = step1.size();
...
std::vector<double>
NDSpline::getValues(std::vector<int> & loop_locator)
{
...
void
NDSpline::from2DTo1DRestructuring(std::vector<std::vector<double>> & two_d_data,
...
double
NDSpline::uk(double x, std::vector<double> & discretizations, double k)
{
  // defined in Christian Habermann, Fabian Kindermann, "Multidimensional Spline Interpolation:
  // Theory and Applications", Computational Economics, Vol.30-2, pp 153-169 (2007)
  // [http://link.springer.com/article/10.1007%2Fs10614-007-9092-4]
  ...
  // double scaled_x = down +
  // (x-discretizations[(int)down])/(discretizations[(int)down+1]-discretizations[(int)down]);

  double scaled_x =
      (double)down +
      (x - discretizations.at(down)) / (discretizations.at(down + 1) - discretizations.at(down));
...
void
NDSpline::from1DTo2DRestructuring(std::vector<std::vector<double>> & two_d_data,
                                  std::vector<double> & one_d_data,
                                  int spacing)
{
  /**
...
void
NDSpline::tridag(std::vector<double> & a,
                 std::vector<double> & b,
                 std::vector<double> & c,
                 std::vector<double> & r,
                 std::vector<double> & u)
...
std::vector<double>
NDSpline::getCoefficients(std::vector<double> & y, double h, double alpha, double beta)
{
...
int
NDSpline::fromNDTo1DConverter(std::vector<int> coordinate)
{
  int coordinate1D = 0;
  int spacing;
...
std::vector<int>
NDSpline::from1DToNDConverter(int one_d_coordinate, std::vector<int> indexes)
{
...
double
NDSpline::retrieveCoefficient(std::vector<int> coefficient_coordinate)
{
...
bool
NDSpline::checkBoundaries(std::vector<double> point)
{
...
// derivative of uk
double
NDSpline::ukDeriv(double x, std::vector<double> & discretizations, double k)
{
  ...
  return phiDeriv((scaled_x - a) / h - (k - 2.0)) *
         (discretizations.at(down + 1) - discretizations.at(down));

  // return phiDeriv((scaled_x-a)/h - (k-2.0));

  // return phiDeriv((x-discretizations.at(0))/(discretizations.at(down+1)-discretizations.at(down))
  // - (k-2.0)) * (discretizations.at(down+1)-discretizations.at(down));
}

double
NDSpline::splineCartesianIntegration(std::vector<double> point_coordinate)
{
  ...
  for (int i = 0; i < numberOfIterations; i++)
  {
    ...
    {
      product *=
          ukDeriv(point_coordinate.at(nDim), _discretizations.at(nDim), coordinates.at(nDim) + 1);
    }
    interpolated_value += _spline_coefficients.at(i) * product;
  }
...
double
NDSpline::splineCartesianMarginalIntegration(double coordinate, int marginal_variable)
{
  double value = 0.0;
...
double
NDSpline::splineCartesianInverseMarginal(double cdf, int marginal_variable, double precision)
{
  ...
  double up =
      _discretizations.at(marginal_variable).at(_discretizations.at(marginal_variable).size() - 1);
  double down = _discretizations.at(marginal_variable).at(0) + 0.000001;
  cdf = splineCartesianMarginalIntegration(down, marginal_variable) +
        cdf * (splineCartesianMarginalIntegration(up, marginal_variable) -
               splineCartesianMarginalIntegration(down, marginal_variable));
  ...
  {
    if (x_np1 > x_n)
      derivative = (splineCartesianMarginalIntegration(x_np1, marginal_variable) -
                    splineCartesianMarginalIntegration(x_n, marginal_variable)) /
                   (x_np1 - x_n);
    else
      derivative = (splineCartesianMarginalIntegration(x_n, marginal_variable) -
                    splineCartesianMarginalIntegration(x_np1, marginal_variable)) /
                   (x_n - x_np1);
    double next =
        x_n - (splineCartesianMarginalIntegration(x_n, marginal_variable) - cdf) / derivative;
...
double
NDSpline::integralSpline(std::vector<double> point_coordinate)
{
...
double
NDSpline::val1(double /* t */)
{
  return 0.0;
}
double
NDSpline::val2(double t)
{
  double value = 8.0 * t + 6.0 * pow(t, 2) + 2.0 * pow(t, 3) + 0.25 * pow(t, 4);
  return value;
}
double
NDSpline::val3(double t)
{
  double value = 4.0 * t - 2.0 * pow(t, 3) - 0.75 * pow(t, 4);
  return value;
}
double
NDSpline::val4(double t)
{
  double value = 4.0 * t - 2.0 * pow(t, 3) + 0.75 * pow(t, 4);
  return value;
}
double
NDSpline::val5(double t)
{
  double value = 8.0 * t - 6.0 * pow(t, 2) + 2.0 * pow(t, 3) - 0.25 * pow(t, 4);
  return value;
}
double
NDSpline::val6(double /* t */)
{
  return 0.0;
}
...
NDSpline::phiDeriv(double t)
{
  ...
  if ((t + 2.0) < 0.00001)
    ...
    phiDeriv_value = val2(-1.0) - val2(-2.0) + val3(0.0) - val3(-1.0) + val4(1.0) - val4(0.0) +
                     (val5(t) - val5(1.0));
  else if (((t - 2.0) > 0.00001))
    phiDeriv_value = val2(-1.0) - val2(-2.0) + val3(0.0) - val3(-1.0) + val4(1.0) - val4(0.0) +
                     val5(2.0) - val5(1.0) + (val6(t) - val6(2.0));
