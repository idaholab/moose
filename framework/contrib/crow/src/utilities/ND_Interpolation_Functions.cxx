...
double
NDInterpolation::interpolateAt(std::vector<double> /* point_coordinate */)
{
...
double
NDInterpolation::getGradientAt(std::vector<double> /* point_coordinate */)
{
...
void
NDInterpolation::fit(std::vector<std::vector<double>> /* coordinates */,
...
void
NDInterpolation::updateRNGParameters(double tolerance, double initial_divisions)
{
...
bool
checkUpperBound(double upper_bound, std::vector<double> values)
{
  bool check = true;
...
bool
checkLowerBound(double lower_bound, std::vector<double> values)
{
  bool check = true;
...
bool
InverseDistanceWeighting::checkUB(double upper_bound)
{
  return checkUpperBound(upper_bound, _values);
}

bool
InverseDistanceWeighting::checkLB(double lower_bound)
{
  return checkLowerBound(lower_bound, _values);
}

bool
NDSpline::checkUB(double upper_bound)
{
  return checkUpperBound(upper_bound, _values);
}

bool
NDSpline::checkLB(double lower_bound)
{
  return checkLowerBound(lower_bound, _values);
}

double
NDInterpolation::minkowskiDistance(std::vector<double> point1, std::vector<double> point2, double p)
{
  double distance;
...
double
NDInterpolation::vectorNorm(std::vector<double> point, double p)
{
  double norm = 0;
...
// // iterative procedure of linear interpolation to determine a nextPoint between firstPoint and
// secondPoint until CDF(nextPoint) lies between f_min and f_max
//
...
//        for (int nDim=0; nDim<_dimensions; nDim++)
//            nextPoint[nDim] =
//            (firstPoint[nDim]-secondPoint[nDim])*(valueFirstPoint-valueSecondPoint)/(referenceCDF-valueSecondPoint)+secondPoint[nDim];
...
double
NDInterpolation::integralCellValue(std::vector<std::vector<double>> cell)
{
  /**
...
int
NDInterpolation::cdfWeightedPicking(std::vector<std::vector<std::vector<double>>> & vertices,
                                    double g)
{
  /**
   * This function randomly picks a cell weighted by the integral value of the ND function within
   * each cell
   */
...
bool
NDInterpolation::pivotCellCheck(std::vector<std::vector<double>> & cell, double f)
{
  // This function check if f is contained within cell
  // if the vertex outcome is the same for all vertices then f is not contained within the cell -->
  // outcome = True
...
int
NDInterpolation::vertexOutcome(std::vector<double> & vertex, double f)
{
  int outcome;
...
void
NDInterpolation::cellsFilter(std::vector<std::vector<std::vector<double>>> & vertices, double f)
{
...
void
NDInterpolation::refinedCellDivision(std::vector<std::vector<std::vector<double>>> & refined_cell,
                                     std::vector<std::vector<double>> & cell,
                                     int divisions)
{
  /**
   * This function partition the original cell into subcells (each dimensions is partitioned into
   * "divisions" intervals)
   */
  ...
  for (int n = 0; n < numberNewCells; n++)
  {
    ...
    std::vector<std::vector<double>> newCell =
        generateNewCell(nd_coordinate, cell.at(0), dxs, _dimensions);
    refined_cell.push_back(newCell);
  }
}

std::vector<int>
NDInterpolation::arrayConverter(int one_d_coordinate, int divisions, int n_dimensions)
{
...
NDInterpolation::generateNewCell(std::vector<int> nd_coordinate,
                                 std::vector<double> coordinate_of_point_zero,
                                 ...
                                 int n_dimensions)
{
  ...
  for (int i = 0; i < numberOfVerteces; i++)
  {
    ...
    for (int nDim = 0; nDim < n_dimensions; nDim++)
      vertexRealCoordinate.at(nDim) = coordinate_of_point_zero.at(nDim) +
                                      nd_coordinate.at(nDim) * dxs.at(nDim) +
                                      vertexIntCoordinate.at(nDim) * dxs.at(nDim);
...
std::vector<std::vector<double>>
NDInterpolation::pickNewCell(std::vector<std::vector<std::vector<double>>> & cells_set, double g)
{
  /**
...
std::vector<double>
NDInterpolation::getCellCenter(std::vector<std::vector<double>> & cell)
{
  /**
...
double
NDInterpolation::ndDerivative(std::vector<double> coordinate)
{
...
double
NDInterpolation::oneDDerivative(double fxph, double fx, double fxmh)
{
  double h = fxph - fx;
...
double
NDInterpolation::derivativeStep(std::vector<double> coordinate, int loop)
{
  double value;
  ...
  if (loop < _dimensions)
  {
    ...
    value = oneDDerivative(
        interpolateAt(coordinateU), interpolateAt(coordinateC), interpolateAt(coordinateD));

  return value;
}

std::vector<double>
intToBinary(int value, int size)
{
...
double
NDInterpolation::averageCellValue(std::vector<double> center, std::vector<double> dx)
{
  double value = 0.0;
...
std::vector<double>
NDInterpolation::ndInverseFunctionGrid(double f, double g)
{
