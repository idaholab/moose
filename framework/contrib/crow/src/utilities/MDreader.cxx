...
bool
checkIfdoubleIsInt(double value)
{
  double intPart;
...
void
readOrderedNDArray(std::string & filename,
                   ...
                   std::vector<double> & values)
{
  ...
  if (checkIfdoubleIsInt(data[startingPoint]))
    ...
    throwError("readOrderedNDArray: error in" << filename
                                              << "; number of dimensions must be integer");
  ...
  for (int i = 0; i < number_of_dimensions; i++)
  {
    if (checkIfdoubleIsInt(data[startingPoint]))
      ...
      throwError("readOrderedNDArray: error in" << filename
                                                << "; number of discretizaions must be integer");
...
void
readScatteredNDArray(std::string & filename,
                     int & number_of_dimensions,
                     ...
                     std::vector<double> & values)
{
  ...
  if (checkIfdoubleIsInt(data[0]))
    ...
    throwError("readScatteredNDArray: error in" << filename
                                                << "; number of dimensions must be integer");
  number_of_dimensions = (int)data[0];
  ...
  if (checkIfdoubleIsInt(data[1]))
    ...
    throwError("readScatteredNDArray: error in" << filename
                                                << "; number of points must be integer");
  ...
  if (values.size() != point_coordinates.size())
    throwError("Data contained in "
               << filename
               << " is not complete: point coordinates and values do not match.");
  if (number_of_points != point_coordinates.size())
    throwError(
        "Data contained in "
        << filename
        << " is not complete: expected number of points and point coordinates do not match.");

  // for (int n=0; n<number_of_points; n++){
  //       std::cout<< point_coordinates[n][0] << " ; " << point_coordinates[n][1] << " : " <<
  //       values[n] << std::endl;
  // }
}
...
unsigned int
readNumbers(const std::string & s, std::vector<double> & v)
{
...
void
importMatrixFromTxtFile(const std::string filename_x,
                        std::vector<double> & v,
                        unsigned int & rows,
                        unsigned int & cols)
{
...
std::vector<double>
read1DArray(std::string filename)
{
