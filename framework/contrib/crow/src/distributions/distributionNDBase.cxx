...
double
BasicDistributionND::cellIntegral(std::vector<double> center, std::vector<double> dx)
{
  /**
   * This function calculates the integral of the pdf in a cell region
   * In the 1D case a cell region is an interval [a,b], thus the integral of the pdf in such
   * interval is
   * calculated as CDF(b)-CDF(a). This functions perform a similar evolution but for a generic ND
   * cell
   */
...
std::vector<int>
BasicDistributionND::oneDtoNDconverter(int one_d_coordinate, std::vector<int> indexes)
{
  /**
   *  This function makes a conversion of a 1D array into an ND array.
   *  The objective it to determine the coordinates of an ND point from its coordinate in a 1D
   * vector.
...
void
DistributionUpdateVariable(BasicDistributionND & dist,
...
double
DistributionPdf(BasicDistributionND & dist, std::vector<double> & x)
{
  return dist.pdf(x);
}

double
DistributionCdf(BasicDistributionND & dist, std::vector<double> & x)
{
  return dist.cdf(x);
}

// std::vector<double> DistributionInverseCdf(BasicDistributionND & dist, double & min, double &
// max){
std::vector<double>
DistributionInverseCdf(BasicDistributionND & dist, double & f, double & g)
{
  return dist.inverseCdf(f, g);
}

double
returnUpperBound(BasicDistributionND & dist, int dimension)
{
  /**
...
double
returnLowerBound(BasicDistributionND & dist, int dimension)
{
  /**
