...
void
BasicMultivariateNormal::base10ToBaseN(int value_base10, int base, std::vector<int> & value_base_n)
{
  /**
...
// void BasicMultivariateNormal::basicMultivariateNormalInit(std::string data_filename,
// std::vector<double> mu){
void
BasicMultivariateNormal::basicMultivariateNormalInit(unsigned int & rows,
                                                     ...
                                                     std::vector<double> mu)
{
  ...
  // Creation BasicMultiDimensionalCartesianSpline(std::vector< std::vector<double> > &
  // discretizations, std::vector<double> & values, std::vector<double> alpha, std::vector<double>
  // beta, bool cdf_provided)
  ...
  _cartesian_distribution =
      BasicMultiDimensionalCartesianSpline(discretizations, values, alpha, beta, false);
}
...
BasicMultivariateNormal::BasicMultivariateNormal(std::vector<std::vector<double>> cov_matrix,
                                                 std::vector<double> mu)
{
  /**
   * This is the function that initializes the Multivariate normal distribution given:
...
BasicMultivariateNormal::BasicMultivariateNormal(std::vector<double> vec_cov_matrix,
                                                 std::vector<double> mu)
{
  /**
   * This is the function that initializes the Multivariate normal distribution given:
                                                 ...
                                                 int rank)
{
  /**
   * This is the function that initializes the Multivariate normal distribution given:
   * First, we will make sure the given covariance, i.e. vec_cov_matrix, is symmetric, function
   * 'computeNearestSymmetricMatrix' will be called
   * Second, we will compute the svd of the computed symmetric matrix
   * Third, we will make sure the reconstructed covariance matrix will be symmetric positive
   * semidefinite matrix, function resetSingularValues will be called
   * Reference for compute the nearest symmetric positive semidefinte matrix:
   * 1. Nicholas J. Higham, "Computing a Nearest Symmetric Positive Semidefinite Matrix," Linear
   * Algebra and Its Applications, vol. 103, pp. 103-118 (1988)
   * 2. Risto Vanhanen, "Computing Positive Semidefinite Multigroup Nuclear Data Covariances,"
   * Nuclear Science and Engineering, vol. 179, pp. 411-422 (2015)
   * Input Parameters
   * - vec_cov_matrix: covariance matrix stored in a vector<double>
   * - mu: the mean value vector
   * - rank: the reduced dimension
   * - type: the type of given covariance matrix (vec_cov_matrix), it can be 'abs' or 'rel', which
   * means absolute covariance matrix or relative convariance matrix respectively.
   */
  unsigned int rows, columns;
  std::vector<std::vector<double>> cov_matrix;
  ...
  if (_rank > _mu.size())
  {
    throwError("The  provided rank  is larger than the given problem's dimension, it should be "
               "less or equal!");
  }
  if (_rank == _mu.size())
  {
  ...
  resetSingularValues(
      _left_singular_vectors, _right_singular_vectors, _singular_values, _svd_transformed_matrix);
...
BasicMultivariateNormal::computeSVD()
{
   ...
   * and store the left singular vectors in _left_singular_vectors, right singular vectors in
   ...
   * @ Out, None
   */
  svdDecomposition(_cov_matrix,
                   _left_singular_vectors,
                   _right_singular_vectors,
                   _singular_values,
                   _svd_transformed_matrix);
}

void
BasicMultivariateNormal::computeSVD(int rank)
{
  /**
...
std::vector<double>
BasicMultivariateNormal::getTransformationMatrix(std::vector<int> index)
{
  /**
   * this function returns the transformation matrix
...
{
  /**
   * return the row and colum of the transformation matrix stored in returnVector.at(0) and
   * returnVector.at(1) respectively
   * @ In, None
   * @ Out, returnVector, std::vector<int>, row stored in returnVector.at(0), and column stored in
   * returnVector.at(1)
   */
...
std::vector<int>
BasicMultivariateNormal::getTransformationMatrixDimensions(std::vector<int> index)
{
  /**
...
std::vector<double>
BasicMultivariateNormal::getInverseTransformationMatrix(std::vector<int> index)
{
  /**
   * this function returns the transformation matrix
...
{
  /**
   * return the row and colum of the inverse transformation matrix stored in returnVector.at(0) and
   * returnVector.at(1) respectively
   * @ In, None
   * @ Out, returnVector, std::vector<int>, row stored in returnVector.at(0), and column stored in
   * returnVector.at(1)
...
std::vector<int>
BasicMultivariateNormal::getInverseTransformationMatrixDimensions(std::vector<int> index)
{
  /**
   * return the row and colum of the transformation matrix
   * @ In, index, std::vector<int>, the index of inverse transformation matrix
...
std::vector<double>
BasicMultivariateNormal::getLeftSingularVectors(std::vector<int> index)
{
  /**
...
std::vector<double>
BasicMultivariateNormal::getRightSingularVectors(std::vector<int> index)
{
  /**
...
std::vector<double>
BasicMultivariateNormal::getSingularValues(std::vector<int> index)
{
  /**
   * this function returns the singular values associated with the provided index
   * @ In, index, std::vector<int>, the  index of left singular vectors
   * @ Out, returnVector, std::vector<double>, the vector stores the singular values associated with
   * the provided inde
   */
...
{
  /**
   * return the row and column of left singular vectors stored in returnVector.at(0) and
   * returnVector.at(1) respectively
   * @ In, None
   * @ Out, returnVector, std::vector<int>, row stored in returnVector.at(0), and column stored in
   * returnVector.at(1)
...
std::vector<int>
BasicMultivariateNormal::getLeftSingularVectorsDimensions(std::vector<int> index)
{
  /**
   * return the row and column of left singular vectors stored in returnVector.at(0) and
   * returnVector.at(1) respectively
   * @ In, index, std::vector<int>, the index of left singular vectors
   * @ Out, returnVector, std::vector<int>, return the row and column of left singular vectors with
   * provided index  stored in returnVector.at(0) and returnVector.at(1) respectively
   */
  std::vector<int> returnVector;
  returnVector.push_back(_left_singular_vectors.size());
  returnVector.push_back(index.size());
...
{
  /**
   * return the row and column of right singular vectors stored in returnVector.at(0) and
   * returnVector.at(1) respectively
   * @ In, None
   * @ Out, returnVector, std::vector<int>, row stored in returnVector.at(0), and column stored in
   * returnVector.at(1)
...
std::vector<int>
BasicMultivariateNormal::getRightSingularVectorsDimensions(std::vector<int> index)
{
  /**
   * return the row and column of right singular vectors stored in returnVector.at(0) and
   * returnVector.at(1) respectively
   * @ In, index, std::vector<int>, the index of right singular vectors
   * @ Out, returnVector, std::vector<int>, return the row and column of right singular vectors with
   * provided index  stored in returnVector.at(0) and returnVector.at(1) respectively
   */
  std::vector<int> returnVector;
  returnVector.push_back(_right_singular_vectors.size());
...
int
BasicMultivariateNormal::getSingularValuesDimension(std::vector<int> index)
{
  /**
...
std::vector<double>
BasicMultivariateNormal::coordinateInTransformedSpace(int rank)
{
  /**
   ...
   * the random variable. In addition, thi function will be used in the input dimensionality
   * reduction
   * application. We will transform the correlated variables into uncorrelated variables, and using
   * this
   * function to draw the samples for the uncorrelated variables, and later transform the samples to
   * correlated
   * variables.
...
std::vector<double>
BasicMultivariateNormal::coordinateInverseTransformed(std::vector<double> & coordinate)
{
  /**
...
double
BasicMultivariateNormal::cellProbabilityWeight(std::vector<double> center, std::vector<double> dx)
{
  /**
   ...
   * cell
   * This function assumes all the input variables are uncorrelated, and follows univariate normal
   * distribution N(0,1)
   * @ In, center, std::vector<double>, a vector to store the grid coordinate, for ND grid sampler,
   * center represents the coordinate of given grid point
   * @ In, dx, std::vector<double>,  a vector to store the distance between given grid coordinate
   * and its connected points, for ND grid sampler, dx represents the distance between
   * grid_coordinate_plus_one - grid_coordinate_minus_one, where grid_coordinate_plus_one and
   * grid_coordinate_minus_one are the shift of "center"
...
double
BasicMultivariateNormal::inverseMarginalForPCA(double f)
{
  /**
   * This function calculates the inverse marginal distribution at f of a MVN distribution when
   * using pca decomposition
   * @ In, f, double, the value picked in the marginal cdf distribution
   * @ Out, normalDistribution->inverseCdf(f), double, the variable value corresponding to the
   * marginal cdf distribution at f
   */
  BasicNormalDistribution * normalDistribution = new BasicNormalDistribution(0, 1);
...
double
BasicMultivariateNormal::marginalCdfForPCA(double x)
{
  /**
   * This function calculates the marginal cdf at x of a MVN distribution when using pca
   * decomposition
   * If PCA method is used, the marginal cdf is assumed to be standard normal distribution, i.e.
...
double
BasicMultivariateNormal::getPdf(std::vector<double> x,
                                std::vector<double> mu,
                                std::vector<std::vector<double>> inverse_cov_matrix)
{
  /**
...
double
BasicMultivariateNormal::pdfInTransformedSpace(std::vector<double> x)
{
  /**
...
double
BasicMultivariateNormal::pdf(std::vector<double> x)
{
  /**
   * This function calculates the pdf values at x of a MVN distribution
   */
...
double
BasicMultivariateNormal::cdf(std::vector<double> x)
{
  /**
...
std::vector<double>
BasicMultivariateNormal::inverseCdf(double f, double g)
{
  /**
...
double
BasicMultivariateNormal::inverseMarginal(double f, int dimension)
{
  /**
   * This function calculates the inverse marginal distribution at f for a specific dimension of a
   * MVN distribution
   */
...
int
BasicMultivariateNormal::returnDimensionality()
{
  /**
...
void
BasicMultivariateNormal::updateRNGparameter(double tolerance, double initial_divisions)
{
  /**
...
double
BasicMultivariateNormal::marginal(double x, int dimension)
{
  /**
   * This function calculates the marginal distribution at x for a specific dimension of a MVN
   * distribution
   */
...
//
//  std::cout << "N " << N << " ; f: " << f.at(dimensions-1) << " ; delta: " << delta << " ; Intsum:
//  " << Intsum << " ; Varsum: " << Varsum << "; error: " << error << std::endl;
// }
//
// return Intsum;
//}
...
double
BasicMultivariateNormal::phi(double x)
{
...
double
BasicMultivariateNormal::phiInv(double x)
{
  normal s;
...
// double BasicMultivariateNormal::MVNDST(std::vector<double> a, std::vector<double> b, double
// alpha, double epsilon, int Nmax){
// int dimensions = _cov_matrix.size();
// double Intsum=0;
// double Varsum=0;
// int N = 0;
...
double *
BasicMultivariateNormal::cholesky(double * A, int n)
{
...
BasicMultivariateNormal::choleskyDecomposition(std::vector<std::vector<double>> matrix)
{
...
void
BasicMultivariateNormal::showMatrix(double * A, int n)
{
  for (int i = 0; i < n; i++)
  {
    for (int j = 0; j < n; j++)
...
//}
//
// void getInverse(std::vector<std::vector<double> > matrix, std::vector<std::vector<double> >
// inverse_matrix){
