...
DistributionContainer::~DistributionContainer() { delete _random; }

void
DistributionContainer::addDistributionInContainer(const std::string & /*type*/,
...
void
DistributionContainer::addDistributionInContainerND(const std::string & /*type*/,
                                                    const std::string & name,
...
bool
DistributionContainer::checkCdf(const std::string dist_alias, std::vector<double> value)
{
  bool result;
  if (cdf(std::string(dist_alias), value) >= getVariable(dist_alias, "ProbabilityThreshold"))
  {
...
// to be implemented
bool
DistributionContainer::checkCdf(const char * dist_alias, double value)
{
  return checkCdf(std::string(dist_alias), value);
}
bool
DistributionContainer::checkCdf(const char * dist_alias, std::vector<double> value)
{
  return checkCdf(std::string(dist_alias), value);
}
...
void
DistributionContainer::updateVariable(const char * dist_alias,
...
void
DistributionContainer::updateVariable(const std::string dist_alias,
                                      ...
                                      double new_value)
{
  ...
  {
    throwError("updateVariable: Distribution " + dist_alias +
               " was not found in distribution container.");
  }
}
...
DistributionContainer::getDistributionNames()
{
  ...
  for (std::map<std::string, MooseSharedPointer<BasicDistribution>>::iterator it =
           _dist_by_name.begin();
       it != _dist_by_name.end();
       it++)
  {
    distsNames.push_back(it->first);
  }
  for (std::map<std::string, MooseSharedPointer<BasicDistributionND>>::iterator it =
           _dist_nd_by_name.begin();
       it != _dist_nd_by_name.end();
       it++)
  {
    distsNames.push_back(it->first);
  }
...
DistributionContainer::getDistributionVariableNames(const std::string dist_alias)
{
  ...
  {
    throwError("getDistributionVariableNames: Distribution " + dist_alias +
               " was not found in distribution container.");
  }
}
...
DistributionContainer::inverseCdf(const std::string dist_alias, double rng)
{
  ...
  throwError("inverseCdf: Distribution " + dist_alias +
             " was not found in distribution container.");
  return -1.0;
}
...
DistributionContainer::getDistributionRandom(const std::string dist_alias)
{
  ...
  throwError("getDistributionRandom: Distribution " + dist_alias +
             " was not found in distribution container.");
  return -1.0;
}
...
DistributionContainer::inverseCdf(const std::string dist_alias, double f, double g)
{
  ...
  throwError("inverseCdf: Distribution ND" + dist_alias +
             " was not found in distribution container.");
...
std::string
DistributionContainer::lastDistributionTriggered()
{
...
bool
DistributionContainer::atLeastADistTriggered()
{
  return _at_least_a_dist_triggered;
}

int
DistributionContainer::returnDimensionality(const std::string dist_alias)
{
  if (_dist_by_name.find(dist_alias) != _dist_by_name.end())
  {
    MooseSharedPointer<BasicDistribution> dist = _dist_by_name.find(dist_alias)->second;
  ...
  throwError("returnDimensionality: Distribution " + dist_alias +
             " was not found in distribution container.");
  return -1.0;
}

int
DistributionContainer::returnDimensionality(const char * dist_alias)
{
  return returnDimensionality(std::string(dist_alias));
}

DistributionContainer &
DistributionContainer::instance()
{
  if (_instance == NULL)
  {
...
const char *
stringPToSt(const std::string * s)
{
  return s->c_str();
}

void
freeStringP(std::string * s)
{
  delete s;
}
