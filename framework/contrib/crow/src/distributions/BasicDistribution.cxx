...
BasicDistribution::getVariableNames()
{
  ...
  for (std::map<std::string, double>::iterator it = _dist_parameters.begin();
       it != _dist_parameters.end();
       it++)
  {
...
double
BasicDistribution::windowProcessing(double rng)
{
  double value;

  if (getVariable(std::string("PB_window_Low")) != 0.0 ||
      getVariable(std::string("PB_window_Up")) != 1.0)
  { // interval Pb window
    ...
    value = inverseCdf(pb);
    // std::cerr << " pbLOW " << pbLOW << " pbUP " << pbUP << " pb " << pb << " value " << value <<
    // std::endl;
  }
  else if (getVariable(std::string("V_window_Low")) != -std::numeric_limits<double>::max() &&
           getVariable(std::string("V_window_Up")) != std::numeric_limits<double>::max())
  { // interval V window
