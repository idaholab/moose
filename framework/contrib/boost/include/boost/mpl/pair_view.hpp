...
struct distance_impl<aux::pair_iter_tag>
{
  ...
  struct apply
  {
    ...
    typedef
        typename mpl::distance<typename first<Iter1>::type, typename first<Iter2>::type>::type type;
  };
};

template <typename BOOST_MPL_AUX_NA_PARAM(Sequence1), typename BOOST_MPL_AUX_NA_PARAM(Sequence2)>
struct pair_view
{
  ...
  typedef typename min<typename iterator_category<iter1_>::type,
