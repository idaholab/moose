...
#define BOOST_PP_SEQ_FOR_EACH_D(macro, data, seq)                                                  \
  BOOST_PP_SEQ_FOR_EACH_DETAIL_CHECK(macro, data, seq)
#endif
#
#define BOOST_PP_SEQ_FOR_EACH_DETAIL_CHECK_EXEC(macro, data, seq)                                  \
  BOOST_PP_FOR((macro, data, seq, BOOST_PP_SEQ_SIZE(seq)),                                         \
...
#else
#define BOOST_PP_SEQ_FOR_EACH_O(r, x)                                                              \
  BOOST_PP_SEQ_FOR_EACH_O_I(BOOST_PP_TUPLE_ELEM(4, 0, x),                                          \
...
#define BOOST_PP_SEQ_FOR_EACH_O_I_DEC(macro, data, seq, sz)                                        \
  (macro,                                                                                          \
   data,                                                                                           \
   BOOST_PP_IF(sz, BOOST_PP_SEQ_FOR_EACH_O_I_TAIL, BOOST_PP_SEQ_FOR_EACH_O_I_NIL)(seq),            \
   sz) \
/**/
...
#else
#define BOOST_PP_SEQ_FOR_EACH_M(r, x)                                                              \
  BOOST_PP_SEQ_FOR_EACH_M_I(r,                                                                     \
...
#
#if ~BOOST_PP_CONFIG_FLAGS() & BOOST_PP_CONFIG_EDG()
#define BOOST_PP_SEQ_FOR_EACH_R(r, macro, data, seq)                                               \
  BOOST_PP_SEQ_FOR_EACH_DETAIL_CHECK_R(r, macro, data, seq)
#else
#define BOOST_PP_SEQ_FOR_EACH_R(r, macro, data, seq) BOOST_PP_SEQ_FOR_EACH_R_I(r, macro, data, seq)
#define BOOST_PP_SEQ_FOR_EACH_R_I(r, macro, data, seq)                                             \
  BOOST_PP_SEQ_FOR_EACH_DETAIL_CHECK_R(r, macro, data, seq)
#endif
#
#define BOOST_PP_SEQ_FOR_EACH_DETAIL_CHECK_EXEC_R(r, macro, data, seq)                             \
  BOOST_PP_FOR_##r((macro, data, seq, BOOST_PP_SEQ_SIZE(seq)),                                     \
                   BOOST_PP_SEQ_FOR_EACH_P,                                                        \
                   BOOST_PP_SEQ_FOR_EACH_O,                                                        \
                   BOOST_PP_SEQ_FOR_EACH_M)
