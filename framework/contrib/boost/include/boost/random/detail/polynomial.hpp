...
public:
  ...
  static void
  multiply_simple(std::size_t size, const digit_t * lhs, const digit_t * rhs, digit_t * output)
  {
    std::size_t bits = std::numeric_limits<digit_t>::digits;
  ...
  static void multiply(const digit_t * lhs,
                       std::size_t lhs_size,
                       const digit_t * rhs,
                       std::size_t rhs_size,
                       digit_t * output)
  ...
  static void multiply_add(const digit_t * lhs,
                           std::size_t lhs_size,
                           const digit_t * rhs,
                           std::size_t rhs_size,
                           digit_t * output)
  ...
  struct sparse_mod
  {
    ...
    void operator()(digit_t * dividend, std::size_t dividend_bits)
    {
      ...
      while (dividend_bits > _remainder_bits)
      {
        ...
        add_shifted_inplace(
            block_size, &_partial_quotient[0], dividend + block_start / bits, block_start % bits);
  ...
  static void
  mod_pow_x(boost::uintmax_t exponent, const digit_t * mod, std::size_t mod_bits, digit_t * out)
  {
    const std::size_t bits = std::numeric_limits<digit_t>::digits;
...
public:
  ...
  {
  public:
    reference(digit_t & value, int idx) : _value(value), _idx(idx) {}
...
inline polynomial operator*(const polynomial & lhs, const polynomial & rhs)
{
  ...
  polynomial_ops::multiply(&lhs._storage[0],
                           lhs._storage.size(),
                           &rhs._storage[0],
                           rhs._storage.size(),
                           &result._storage[0]);
...
inline polynomial
mod_pow_x(boost::uintmax_t exponent, polynomial mod)
{
  polynomial result;
  mod.normalize();
