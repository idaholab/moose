...
struct seed_type
{
  typedef typename boost::mpl::if_<boost::is_integral<T>, T, boost::uint32_t>::type type;
};
...
void
generate_from_real(Engine & eng, Iter begin, Iter end)
{
  using std::fmod;
...
template <class Engine, class Iter>
void
generate_from_int(Engine & eng, Iter begin, Iter end)
{
...
template <class Engine, class Iter>
void
generate_impl(Engine & eng, Iter first, Iter last, boost::mpl::true_)
{
...
template <class Engine, class Iter>
void
generate_impl(Engine & eng, Iter first, Iter last, boost::mpl::false_)
{
...
template <class Engine, class Iter>
void
generate(Engine & eng, Iter first, Iter last)
{
  return detail::generate_impl(
      eng, first, last, boost::random::traits::is_integral<typename Engine::result_type>());
}

template <class IntType, IntType m, class SeedSeq>
IntType
seed_one_int(SeedSeq & seq)
{
...
IntType
get_one_int(Iter & first, Iter last)
{
  static const int log =
      ::boost::mpl::if_c<(m == 0),
                         ::boost::mpl::int_<(::std::numeric_limits<IntType>::digits)>,
...
void
seed_array_int_impl(SeedSeq & seq, UIntType (&x)[n])
{
...
inline void
seed_array_int_impl(SeedSeq & seq, IntType (&x)[n], boost::mpl::true_)
{
  BOOST_STATIC_ASSERT_MSG(boost::is_integral<IntType>::value, "Sorry but this routine has not been "
                                                              "ported to non built-in integers as "
                                                              "it relies on a reinterpret_cast.");
...
template <int w, std::size_t n, class SeedSeq, class IntType>
inline void
seed_array_int_impl(SeedSeq & seq, IntType (&x)[n], boost::mpl::false_)
{
...
template <int w, std::size_t n, class SeedSeq, class IntType>
inline void
seed_array_int(SeedSeq & seq, IntType (&x)[n])
{
...
void
fill_array_int_impl(Iter & first, Iter last, UIntType (&x)[n])
{
  for (std::size_t j = 0; j < n; j++)
  {
    UIntType val = 0;
...
inline void
fill_array_int_impl(Iter & first, Iter last, IntType (&x)[n], boost::mpl::true_)
{
  BOOST_STATIC_ASSERT_MSG(boost::is_integral<IntType>::value, "Sorry but this routine has not been "
                                                              "ported to non built-in integers as "
                                                              "it relies on a reinterpret_cast.");
...
template <int w, std::size_t n, class Iter, class IntType>
inline void
fill_array_int_impl(Iter & first, Iter last, IntType (&x)[n], boost::mpl::false_)
{
...
template <int w, std::size_t n, class Iter, class IntType>
inline void
fill_array_int(Iter & first, Iter last, IntType (&x)[n])
{
...
void
seed_array_real_impl(const boost::uint_least32_t * storage, RealType (&x)[n])
{
...
void
seed_array_real(SeedSeq & seq, RealType (&x)[n])
{
  using std::pow;
  boost::uint_least32_t storage[((w + 31) / 32) * n];
  seq.generate(&storage[0], &storage[0] + ((w + 31) / 32) * n);
...
void
fill_array_real(Iter & first, Iter last, RealType (&x)[n])
{
  boost::uint_least32_t mask = ~((~boost::uint_least32_t(0)) << (w % 32));
  RealType two32 = 4294967296.0;
  const RealType divisor = RealType(1) / detail::pow2<RealType>(w);
