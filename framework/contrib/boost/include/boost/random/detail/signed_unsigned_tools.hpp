...
template <class T,
          bool sgn = std::numeric_limits<T>::is_signed && std::numeric_limits<T>::is_bounded>
struct subtract
{
};
...
template <class T1,
          class T2,
          bool sgn = (std::numeric_limits<T2>::is_signed &&
...
struct add<
    T1,
    T2,
    /* signed or else T2 has more digits than T1 so the cast always works - needed when T2 is a multiprecision type and T1 is a native integer */
    false>
{
