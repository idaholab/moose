...
* built-in
* types this trait defaults to `boost::make_unsigned<T>::type`.  For user defined types it simply
* asserts that
...
* unbounded
* integer type.  This trait is used to generate types suitable for the calculation of a range: as a
* result
* if T is signed, then member `type` *should define a type with one more bit precision than T*.  For
* built-in
* types this trait defaults to `boost::make_unsigned<T>::type`.  For user defined types it simply
* asserts that
* the argument type T is either an unbounded integer, or an unsigned one (using
* std::numeric_limits).
* User defined specializations may be provided for other cases.
*/
template <class T>
...
template <class T>
struct is_signed
    : public mpl::bool_<boost::is_signed<T>::value ||
