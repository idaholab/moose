...
//  generated copy constructor, destructor are fine...
  ...
  T & operator[](std::ptrdiff_t i)
      const // never throws (but has a BOOST_ASSERT in it, so not marked with BOOST_NOEXCEPT)
  {
    BOOST_ASSERT(px != 0);
    BOOST_ASSERT(i >= 0);
    return px[i];
...
#include <boost/smart_ptr/detail/operator_bool.hpp>
  ...
  bool unique() const BOOST_NOEXCEPT { return pn.unique(); }
...
template <class T>
inline bool
operator==(shared_array<T> const & a, shared_array<T> const & b) BOOST_NOEXCEPT
{
  return a.get() == b.get();
}
...
template <class T>
inline bool
operator==(shared_array<T> const & p, boost::detail::sp_nullptr_t) BOOST_NOEXCEPT
{
  return p.get() == 0;
}
...
template <class T>
inline bool
operator==(boost::detail::sp_nullptr_t, shared_array<T> const & p) BOOST_NOEXCEPT
{
  return p.get() == 0;
}
...
template <class T>
void
swap(shared_array<T> & a, shared_array<T> & b) BOOST_NOEXCEPT
{
  a.swap(b);
}
...
D *
get_deleter(shared_array<T> const & p)
{
