...
inline char
wrap_narrow(const Facet & fac, Ch c, char deflt)
{
...
template <class Ch, class Facet>
inline bool
wrap_isdigit(const Facet & fac, Ch c)
{
...
Iter
wrap_scan_notdigit(const Facet & fac, Iter beg, Iter end)
{
  using namespace std;
...
Iter
str2int(const Iter & start, const Iter & last, Res & res, const Facet & fac)
{
  using namespace std;
  Iter it;
  res = 0;
...
template <class Iter, class Facet>
Iter
skip_asterisk(Iter start, Iter last, const Facet & fac)
{
  using namespace std;
  ++start;
...
inline void
maybe_throw_exception(unsigned char exceptions, std::size_t pos, std::size_t size)
{
...
bool
parse_printf_directive(Iter & start,
                       const Iter & last,
...
parse_precision:
  ...
  while (start != last &&
         (*start == const_or_not(fac).widen('l') || *start == const_or_not(fac).widen('L') ||
...
int
upper_bound_from_fstring(const String & buf,
...
basic_format<Ch, Tr, Alloc>::parse(const string_type & buf)
{
...
// has widen and narrow even on compilers without locale
  ...
  while ((i1 = buf.find(arg_mark, i1)) != string_type::npos)
  {
    ...
    it = buf.begin() + i1;
    bool parse_ok =
        io::detail::parse_printf_directive(it, buf.end(), &items_[cur_item], fac, i1, exceptions());
    i1 = it - buf.begin();
  ...
  if (!ordered_args)
  {
    ...
    { // dont mix positional with non-positionnal directives
      if (exceptions() & io::bad_format_string_bit)
        boost::throw_exception(io::bad_format_string(static_cast<std::size_t>(max_argN), 0));
