 ...
 * to the \f$i\f$-th element of the container. A storage type \c A can be specified which defaults
 * to \c unbounded_array.
 ...
 * \tparam A The type of the storage array of the vector. Default is \c unbounded_array<T>. \c
 * <bounded_array<T> and \c std::vector<T> can also be used
 */
...
public:
  ...
  BOOST_UBLAS_INLINE
  vector() : vector_container<self_type>(), data_() {}

  /// \brief Constructor of a vector with a predefined size
  /// By default, its elements are initialized to 0.
  /// \param size initial size of the vector
  explicit BOOST_UBLAS_INLINE vector(size_type size) : vector_container<self_type>(), data_(size) {}
  ...
  BOOST_UBLAS_INLINE
  vector(size_type /*size*/, const array_type & data) : vector_container<self_type>(), data_(data)
  {
  }
  ...
  /// \param data container of type \c A
  BOOST_UBLAS_INLINE
  vector(const array_type & data) : vector_container<self_type>(), data_(data) {}
  ...
  BOOST_UBLAS_INLINE
  vector(size_type size, const value_type & init) : vector_container<self_type>(), data_(size, init)
  {
  }
  ...
  BOOST_UBLAS_INLINE
  vector(const vector & v) : vector_container<self_type>(), data_(v.data_) {}
  ...
  BOOST_UBLAS_INLINE vector(const vector_expression<AE> & ae)
    : vector_container<self_type>(), data_(ae().size())
  {
  ...
  // XXX  this semantic is not the one expected by the name of this method
  BOOST_UBLAS_INLINE
  const_pointer find_element(size_type i) const { return &(data()[i]); }
  ...
  BOOST_UBLAS_INLINE
  reference insert_element(size_type i, const_reference t) { return (data()[i] = t); }

  /// \brief Set element \f$i\f$ to the \e zero value
  /// \param i index of the element
  BOOST_UBLAS_INLINE
  void erase_element(size_type i) { data()[i] = value_type /*zero*/ (); }
...
class fixed_vector : public vector_container<fixed_vector<T, N, A>>
{
...
public:
  ...
  BOOST_UBLAS_INLINE
  fixed_vector() : vector_container<self_type>(), data_() {}
  ...
  /// \param data container of type \c A
  BOOST_UBLAS_INLINE
  fixed_vector(const array_type & data) : vector_container<self_type>(), data_(data) {}

  /// \brief Constructor of a fixed_vector with a unique initial value
  /// \param init value to assign to each element of the vector
  BOOST_UBLAS_INLINE
  fixed_vector(const value_type & init) : vector_container<self_type>(), data_()
  {
    data_.fill(init);
  }
  ...
  BOOST_UBLAS_INLINE
  fixed_vector(const fixed_vector & v) : vector_container<self_type>(), data_(v.data_) {}
  ...
  /// \param ae the vector_expression which values will be duplicated into the vector
  template <class AE>
  BOOST_UBLAS_INLINE fixed_vector(const vector_expression<AE> & ae)
    : vector_container<self_type>(), data_()
  {
    vector_assign<scalar_assign>(*this, ae);
  }
...
/// fixed_vector<double, 3> v = { 1, 2, 3 } or fixed_vector<double,3> v( {1, 2, 3} ) or fixed_vector<double,3> v( 1, 2, 3 )
  ...
  fixed_vector(value_type v0, Types... vrest) : vector_container<self_type>(), data_{{v0, vrest...}}
  {
  }
#endif
  ...
  { // should have a const after C++14
    return data_.size();
  }
...
// Assignment
  ...
  /// \return a reference to a vector (i.e. the destination vector)
  template <class C> // Container assignment without temporary
  BOOST_UBLAS_INLINE fixed_vector & operator=(const vector_container<C> & v)
  {
    assign(v);
    return *this;
  }
  ...
  template <class AE>
  BOOST_UBLAS_INLINE fixed_vector & operator=(const vector_expression<AE> & ae)
  {
    self_type temporary(ae);
    return assign_temporary(temporary);
  }
  ...
  /// \return a reference to the resulting fixed_vector
  template <class AE>
  BOOST_UBLAS_INLINE fixed_vector & assign(const vector_expression<AE> & ae)
  {
    vector_assign<scalar_assign>(*this, ae);
    return *this;
  }
  ...
  /// \return a reference to the resulting vector
  template <class AE>
  BOOST_UBLAS_INLINE fixed_vector & plus_assign(const vector_expression<AE> & ae)
  {
    vector_assign<scalar_plus_assign>(*this, ae);
    return *this;
  }
  ...
  /// \return a reference to the resulting fixed_vector
  template <class AE>
  BOOST_UBLAS_INLINE fixed_vector & minus_assign(const vector_expression<AE> & ae)
  {
    vector_assign<scalar_minus_assign>(*this, ae);
    return *this;
  }
  ...
  /// \return a reference to the resulting fixed_vector
  template <class AT>
  BOOST_UBLAS_INLINE fixed_vector & operator*=(const AT & at)
  {
    vector_assign_scalar<scalar_multiplies_assign>(*this, at);
    return *this;
  }
  ...
  BOOST_UBLAS_INLINE
  friend void swap(fixed_vector & v1, fixed_vector & v2) { v1.swap(v2); }
...
#ifndef BOOST_UBLAS_USE_INDEXED_ITERATOR
  class const_iterator : public container_const_reference<fixed_vector>,
                         public random_access_iterator_base<dense_random_access_iterator_tag,
                                                            const_iterator,
                                                            value_type,
                                                            difference_type>
...
#ifndef BOOST_UBLAS_USE_INDEXED_ITERATOR
  class iterator : public container_reference<fixed_vector>,
                   public random_access_iterator_base<dense_random_access_iterator_tag,
                                                      iterator,
                                                      value_type,
                                                      difference_type>
...
public:
  ...
  BOOST_UBLAS_INLINE bounded_vector(const vector<T, A2> & v) : vector_type(v)
  {
  }
  template <class AE>
  BOOST_UBLAS_INLINE bounded_vector(const vector_expression<AE> & ae) : vector_type(ae)
  {
  }
  BOOST_UBLAS_INLINE
  ~bounded_vector() {}
...
public:
  ...
  // Construction and destruction
  BOOST_UBLAS_INLINE
  zero_vector() : vector_container<self_type>(), size_(0) {}
  explicit BOOST_UBLAS_INLINE zero_vector(size_type size)
    : vector_container<self_type>(), size_(size)
  {
  }
  BOOST_UBLAS_INLINE
  zero_vector(const zero_vector & v) : vector_container<self_type>(), size_(v.size_) {}
  ...
  // Resizing
  BOOST_UBLAS_INLINE
  void resize(size_type size, bool /*preserve*/ = true) { size_ = size; }

  // Element support
  BOOST_UBLAS_INLINE
  const_pointer find_element(size_type /*i*/) const { return &zero_; }

  // Element access
  BOOST_UBLAS_INLINE
  const_reference operator()(size_type /* i */) const { return zero_; }
  ...
  BOOST_UBLAS_INLINE
  friend void swap(zero_vector & v1, zero_vector & v2) { v1.swap(v2); }
...
public:
  ...
  // Element lookup
  BOOST_UBLAS_INLINE
  const_iterator find(size_type /*i*/) const { return const_iterator(*this); }
...
public:
  ...
  BOOST_UBLAS_INLINE
  unit_vector() : vector_container<self_type>(), size_(0), index_(0) {}
  ...
  explicit unit_vector(size_type size, size_type index = 0)
    : vector_container<self_type>(), size_(size), index_(index)
  {
  }
  ...
  unit_vector(const unit_vector & v)
    : vector_container<self_type>(), size_(v.size_), index_(v.index_)
  {
  }
  ...
  BOOST_UBLAS_INLINE
  friend void swap(unit_vector & v1, unit_vector & v2) { v1.swap(v2); }
...
public:
  ...
  // Element lookup
  BOOST_UBLAS_INLINE
  const_iterator find(size_type i) const { return const_iterator(*this, i <= index_); }
...
template <class T, class ALLOC>
typename unit_vector<T, ALLOC>::const_value_type
    unit_vector<T, ALLOC>::one_(1); // ISSUE: need 'one'-traits here
...
public:
  ...
  // Construction and destruction
  BOOST_UBLAS_INLINE
  scalar_vector() : vector_container<self_type>(), size_(0), value_() {}
  BOOST_UBLAS_INLINE
  explicit scalar_vector(size_type size, const value_type & value = value_type(1))
    : vector_container<self_type>(), size_(size), value_(value)
  {
  }
  BOOST_UBLAS_INLINE
  scalar_vector(const scalar_vector & v)
    : vector_container<self_type>(), size_(v.size_), value_(v.value_)
  {
  }
  ...
  // Element support
  BOOST_UBLAS_INLINE
  const_pointer find_element(size_type /*i*/) const { return &value_; }
  ...
  BOOST_UBLAS_INLINE
  friend void swap(scalar_vector & v1, scalar_vector & v2) { v1.swap(v2); }
...
public:
  ...
  const_iterator cbegin() const { return begin(); }
  BOOST_UBLAS_INLINE
  const_iterator end() const { return find(size_); }
  BOOST_UBLAS_INLINE
  const_iterator cend() const { return end(); }
...
public:
  ...
  explicit BOOST_UBLAS_INLINE c_vector(size_type size) : size_(size) /* , data_ () */
  {
    if (size_ > N)
      bad_size().raise();
  }
  ...
  BOOST_UBLAS_INLINE
  const_pointer find_element(size_type i) const { return &data_[i]; }
  ...
  // Zeroing
  BOOST_UBLAS_INLINE
  void clear() { std::fill(data_, data_ + size_, value_type /*zero*/ ()); }
...
// Assignment
  ...
  BOOST_UBLAS_INLINE
  friend void swap(c_vector & v1, c_vector & v2) { v1.swap(v2); }
...
public:
  ...
  class iterator
      : public container_reference<c_vector>,
        public random_access_iterator_base<dense_random_access_iterator_tag, iterator, value_type>
  {
  public:
    typedef typename c_vector::difference_type difference_type;
    typedef typename c_vector::value_type value_type;
  ...
  iterator begin() { return find(0); }
  BOOST_UBLAS_INLINE
  iterator end() { return find(size_); }
