...
public:
  ...
  BOOST_UBLAS_INLINE
  explicit permutation_matrix(const vector_type & init) : vector_type(init) {}
  BOOST_UBLAS_INLINE
  ~permutation_matrix() {}
...
lu_factorize(M & m)
{
  ...
  for (size_type i = 0; i < size; ++i)
  {
    ...
    project(m, range(i + 1, size1), range(i + 1, size2))
        .minus_assign(
            outer_prod(project(mci, range(i + 1, size1)), project(mri, range(i + 1, size2))));
  }
#if BOOST_UBLAS_TYPE_CHECK
  BOOST_UBLAS_CHECK(singular != 0 || detail::expression_type_check(
                                         prod(triangular_adaptor<matrix_type, unit_lower>(m),
...
axpy_lu_factorize(M & m, PM & pm)
{
  ...
  for (size_type i = 0; i < size; ++i)
  {
    ...
    project(v, range(i, size1))
        .assign(project(column(m, i), range(i, size1)) -
    ...
    if (v(i_norm_inf) != value_type /*zero*/ ())
    {
      if (i_norm_inf != i)
      {
        ...
        project(row(m, i_norm_inf), range(i + 1, size2))
            .swap(project(row(m, i), range(i + 1, size2)));
      }
      else
      {
        BOOST_UBLAS_CHECK(pm(i) == i_norm_inf, external_logic());
...
void
lu_substitute(const M & m, vector_expression<E> & e)
{
#if BOOST_UBLAS_TYPE_CHECK
...
#if BOOST_UBLAS_TYPE_CHECK
  BOOST_UBLAS_CHECK(detail::expression_type_check(
                        prod(triangular_adaptor<const_matrix_type, unit_lower>(m), e), cv1),
                    internal_logic());
  vector_type cv2(e);
#endif
...
template <class M, class E>
void
lu_substitute(const M & m, matrix_expression<E> & e)
{
#if BOOST_UBLAS_TYPE_CHECK
  typedef const M const_matrix_type;
...
#if BOOST_UBLAS_TYPE_CHECK
  BOOST_UBLAS_CHECK(detail::expression_type_check(
                        prod(triangular_adaptor<const_matrix_type, unit_lower>(m), e), cm1),
                    internal_logic());
  matrix_type cm2(e);
#endif
  inplace_solve(m, e, upper_tag());
...
void
lu_substitute(const M & m, const permutation_matrix<PMT, PMA> & pm, MV & mv)
{
  swap_rows(pm, mv);
  lu_substitute(m, mv);
}
...
void
lu_substitute(vector_expression<E> & e, const M & m)
{
#if BOOST_UBLAS_TYPE_CHECK
  typedef const M const_matrix_type;
  typedef vector<typename E::value_type> vector_type;
...
#if BOOST_UBLAS_TYPE_CHECK
  BOOST_UBLAS_CHECK(
      detail::expression_type_check(prod(e, triangular_adaptor<const_matrix_type, upper>(m)), cv1),
      internal_logic());
  vector_type cv2(e);
#endif
...
#if BOOST_UBLAS_TYPE_CHECK
  BOOST_UBLAS_CHECK(detail::expression_type_check(
                        prod(e, triangular_adaptor<const_matrix_type, unit_lower>(m)), cv2),
                    internal_logic());
#endif
}
template <class E, class M>
void
lu_substitute(matrix_expression<E> & e, const M & m)
{
#if BOOST_UBLAS_TYPE_CHECK
  typedef const M const_matrix_type;
  typedef matrix<typename E::value_type> matrix_type;
...
#if BOOST_UBLAS_TYPE_CHECK
  BOOST_UBLAS_CHECK(
      detail::expression_type_check(prod(e, triangular_adaptor<const_matrix_type, upper>(m)), cm1),
      internal_logic());
  matrix_type cm2(e);
#endif
  inplace_solve(e, m, unit_lower_tag());
#if BOOST_UBLAS_TYPE_CHECK
  BOOST_UBLAS_CHECK(detail::expression_type_check(
                        prod(e, triangular_adaptor<const_matrix_type, unit_lower>(m)), cm2),
                    internal_logic());
#endif
}
