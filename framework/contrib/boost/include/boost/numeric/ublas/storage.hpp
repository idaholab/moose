...
public:
  ...
  explicit BOOST_UBLAS_INLINE unbounded_array(const ALLOC & a = ALLOC()) : alloc_(a), size_(0)
  {
    data_ = 0;
  }
  explicit BOOST_UBLAS_INLINE unbounded_array(size_type size, const ALLOC & a = ALLOC())
    : alloc_(a), size_(size)
  {
    if (size_)
    {
  ...
  unbounded_array(const unbounded_array & c)
    : storage_array<unbounded_array<T, ALLOC>>(), alloc_(c.alloc_), size_(c.size_)
  {
    if (size_)
    {
      data_ = alloc_.allocate(size_);
...
public:
  BOOST_UBLAS_INLINE
  void resize(size_type size) { resize_internal(size, value_type(), false); }
  BOOST_UBLAS_INLINE
  ...
  BOOST_UBLAS_INLINE
  friend void swap(unbounded_array & a1, unbounded_array & a2) { a1.swap(a2); }
...
private:
  ...
  BOOST_UBLAS_INLINE
  static void iterator_destroy(iterator & i) { (&(*i))->~value_type(); }
  ALLOC alloc_;
  size_type size_;
  pointer data_;
};
...
public:
  ...
  explicit BOOST_UBLAS_INLINE bounded_array(size_type size) : size_(size) /*, data_ ()*/
  {
  ...
  {
    BOOST_UBLAS_CHECK(size_ <= N, bad_size());
    // ISSUE elements should be value constructed here, but we must fill instead as already default
    // constructed
  ...
  {
    // ISSUE elements should be copy constructed here, but we must copy instead as already default
    // constructed
  ...
  BOOST_UBLAS_INLINE
  friend void swap(bounded_array & a1, bounded_array & a2) { a1.swap(a2); }
...
private:
  size_type size_;
// MSVC does not like arrays of size 0 in base classes.  Hence, this conditionally changes the size
// to 1
#ifdef _MSC_VER
...
public:
  ...
  explicit BOOST_UBLAS_INLINE array_adaptor(size_type size)
  ...
  template <size_t N>
  BOOST_UBLAS_INLINE array_adaptor(T (&data)[N]) : size_(N), own_(false), data_(data)
  {
  }
  BOOST_UBLAS_INLINE
  array_adaptor(const array_adaptor & a)
    : storage_array<self_type>(), size_(a.size_), own_(true), data_(new value_type[a.size_])
  {
    *this = a;
  }
  BOOST_UBLAS_INLINE
...
public:
  ...
  void resize(size_type size, value_type init) { resize_internal(size, init, true); }
  BOOST_UBLAS_INLINE
  void resize(size_type size, pointer data) { resize_internal(size, data, value_type(), false); }
  BOOST_UBLAS_INLINE
  ...
  BOOST_UBLAS_INLINE
  friend void swap(array_adaptor & a1, array_adaptor & a2) { a1.swap(a2); }
...
public:
  ...
  explicit BOOST_UBLAS_INLINE shallow_array_adaptor(size_type size)
    : size_(size), own_(true), data_(new value_type[size])
  {
  }
  BOOST_UBLAS_INLINE
  ...
  shallow_array_adaptor(const shallow_array_adaptor & a)
    : storage_array<self_type>(), size_(a.size_), own_(a.own_), data_(a.data_)
  {
  }
...
public:
  ...
  BOOST_UBLAS_INLINE
  friend void swap(shallow_array_adaptor & a1, shallow_array_adaptor & a2) { a1.swap(a2); }
...
public:
  ...
  public:
    ...
    BOOST_UBLAS_INLINE
    difference_type operator-(const const_iterator & it) const { return it_ - it.it_; }
  ...
  const_iterator cbegin() const { return begin(); }
  BOOST_UBLAS_INLINE
  const_iterator end() const { return const_iterator(*this, start_ + size_); }
  BOOST_UBLAS_INLINE
  const_iterator cend() const { return end(); }
  ...
  static BOOST_UBLAS_INLINE const basic_range & all() { return all_; }
...
public:
  ...
  static BOOST_UBLAS_INLINE const basic_slice & all() { return all_; }
...
public:
  ...
  explicit BOOST_UBLAS_INLINE indirect_array(size_type size) : size_(size), data_(size) {}
  BOOST_UBLAS_INLINE
  ...
  size_type size() const { return size_; }
  BOOST_UBLAS_INLINE
  const_array_type data() const { return data_; }
  BOOST_UBLAS_INLINE
...
public:
  ...
  public:
    ...
    // Dereference
    BOOST_UBLAS_INLINE
    const_reference operator*() const { return (*this)()(it_); }
    ...
    // Index
    BOOST_UBLAS_INLINE
    size_type index() const { return it_; }
  ...
  static BOOST_UBLAS_INLINE const indirect_array & all() { return all_; }
...
public:
  ...
  { // For gcc 4.8 and c++11
    lhs.swap(rhs);
  }

  BOOST_UBLAS_INLINE
  bool equal(const self_type & rhs) const { return (v1_ == rhs.v1_); }
  BOOST_UBLAS_INLINE
  bool less(const self_type & rhs) const { return (v1_ < rhs.v1_); }
  BOOST_UBLAS_INLINE
  friend bool operator==(const self_type & lhs, const self_type & rhs) { return lhs.equal(rhs); }
  BOOST_UBLAS_INLINE
  friend bool operator!=(const self_type & lhs, const self_type & rhs) { return !lhs.equal(rhs); }
  BOOST_UBLAS_INLINE
  friend bool operator<(const self_type & lhs, const self_type & rhs) { return lhs.less(rhs); }
  BOOST_UBLAS_INLINE
  friend bool operator>=(const self_type & lhs, const self_type & rhs) { return !lhs.less(rhs); }
  BOOST_UBLAS_INLINE
  friend bool operator>(const self_type & lhs, const self_type & rhs) { return rhs.less(lhs); }
  BOOST_UBLAS_INLINE
  friend bool operator<=(const self_type & lhs, const self_type & rhs) { return !rhs.less(lhs); }
...
public:
  ...
  BOOST_UBLAS_INLINE
  bool equal(size_type i1, size_type i2) const { return data1_[i1] == data1_[i2]; }
  BOOST_UBLAS_INLINE
  bool less(size_type i1, size_type i2) const { return data1_[i1] < data1_[i2]; }
...
public:
  ...
  BOOST_UBLAS_INLINE
  bool equal(const self_type & rhs) const { return ((v1_ == rhs.v1_) && (v2_ == rhs.v2_)); }
  BOOST_UBLAS_INLINE
  bool less(const self_type & rhs) const
  {
    return ((v1_ < rhs.v1_) || (v1_ == rhs.v1_ && v2_ < rhs.v2_));
  }
  BOOST_UBLAS_INLINE
  friend bool operator==(const self_type & lhs, const self_type & rhs) { return lhs.equal(rhs); }
  BOOST_UBLAS_INLINE
...
public:
  ...
  {
    return ((data1_[i1] < data1_[i2]) || (data1_[i1] == data1_[i2] && data2_[i1] < data2_[i2]));
  }
