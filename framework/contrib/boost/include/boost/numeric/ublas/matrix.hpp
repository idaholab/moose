 ...
 *
 * However, be warned that using the ublas namespace and the std::vector at the same time can lead
 * to the compiler to confusion.
 * The solution is simply to prefix each ublas vector like \c boost::numeric::ublas::vector<T>. If
 * you think it's too long to
 * write, you can define a new namespace like \c namespace ublas = boost::numeric::ublas and then
 * just declare your vectors
 * with \c ublas::vector<T>. STL vectors will be declared as vector<T>. No need to prefix with \c
 * std::
 */
namespace ublas
{
...
BOOST_UBLAS_INLINE void
matrix_resize_preserve(M & m, M & temporary)
{
 ...
 *
 * For a \f$(m \times n)\f$-dimensional matrix and \f$ 0 \leq i < m, 0 \leq j < n\f$, every element
 * \f$ m_{i,j} \f$ is mapped to
 * the \f$(i.n + j)\f$-th element of the container for row major orientation or the \f$ (i + j.m)
 * \f$-th element of
 * the container for column major orientation. In a dense matrix all elements are represented in
 * memory in a
 * contiguous chunk of memory by definition.
 *
 * Orientation and storage can also be specified, otherwise a \c row_major and \c unbounded_array
 * are used. It is \b not
 * required by the storage to initialize elements of the matrix.
 *
 * \tparam T the type of object stored in the matrix (like double, float, complex, etc...)
 * \tparam L the storage organization. It can be either \c row_major or \c column_major. Default is
 * \c row_major
...
public:
  ...
  BOOST_UBLAS_INLINE
  matrix() : matrix_container<self_type>(), size1_(0), size2_(0), data_() {}
  ...
  matrix(size_type size1, size_type size2, const value_type & init)
    : matrix_container<self_type>(),
      size1_(size1),
      size2_(size2),
      data_(layout_type::storage_size(size1, size2), init)
  {
  }
  ...
  matrix(size_type size1, size_type size2, const array_type & data)
    : matrix_container<self_type>(), size1_(size1), size2_(size2), data_(data)
  {
  }
  ...
  matrix(const matrix & m)
    : matrix_container<self_type>(), size1_(m.size1_), size2_(m.size2_), data_(m.data_)
  {
  }
  ...
  BOOST_UBLAS_INLINE matrix(const matrix_expression<AE> & ae)
    : matrix_container<self_type>(),
      size1_(ae().size1()),
      size2_(ae().size2()),
      data_(layout_type::storage_size(size1_, size2_))
  {
   ...
   * You can also use the free size<>() function in operation/size.hpp as size<1>(m) where m is a
   * matrix
   */
  BOOST_UBLAS_INLINE
   ...
   * You can also use the free size<>() function in operation/size.hpp as size<2>(m) where m is a
   * matrix
   */
  BOOST_UBLAS_INLINE
   ...
   * If data are preserved, then if the size if bigger at least on one dimension, extra values are
   * filled with zeros.
   * If data are not preserved, then nothing has to be assumed regarding the content of the matrix
   * after resizing.
   * \param size1 the new number of rows
   * \param size2 the new number of colums
   * \param preserve a boolean to say if one wants the data to be preserved during the resizing.
   * Default is true.
   */
  BOOST_UBLAS_INLINE
   ...
   */
  BOOST_UBLAS_INLINE
  reference operator()(size_type i, size_type j) { return at_element(i, j); }
   ...
   * For user-defined types, it could be another value if you decided it. Your type in that case
   * must
   * contain a default null value.
   * \param i the first coordinate of the element. By default it's the row
   * \param j the second coordinate of the element. By default it's the column
   */
  void erase_element(size_type i, size_type j) { at_element(i, j) = value_type /*zero*/ (); }
...
// Assignment
  ...
  BOOST_UBLAS_INLINE
  friend void swap(matrix & m1, matrix & m2) { m1.swap(m2); }
...
public:
  ...
  BOOST_UBLAS_INLINE
  const_iterator1 end1() const { return find1(0, size1_, 0); }
  BOOST_UBLAS_INLINE
  ...
  class iterator1
      : public container_reference<matrix>,
        public random_access_iterator_base<dense_random_access_iterator_tag, iterator1, value_type>
  {
  public:
    typedef typename matrix::value_type value_type;
    typedef typename matrix::difference_type difference_type;
  ...
  BOOST_UBLAS_INLINE
  iterator1 end1() { return find1(0, size1_, 0); }
  ...
  BOOST_UBLAS_INLINE
  const_iterator2 end2() const { return find2(0, 0, size2_); }
  BOOST_UBLAS_INLINE
  ...
  class iterator2
      : public container_reference<matrix>,
        public random_access_iterator_base<dense_random_access_iterator_tag, iterator2, value_type>
  {
  public:
    typedef typename matrix::value_type value_type;
    typedef typename matrix::difference_type difference_type;
  ...
  BOOST_UBLAS_INLINE
  iterator2 end2() { return find2(0, 0, size2_); }
...
/** \brief A fixed size dense matrix of values of type \c T. Equivalent to a c-style 2 dimensional
 * array.
 *
 * For a \f$(m \times n)\f$-dimensional fixed_matrix and \f$ 0 \leq i < m, 0 \leq j < n\f$, every
 * element \f$ m_{i,j} \f$ is mapped to
 * the \f$(i.n + j)\f$-th element of the container for row major orientation or the \f$ (i + j.m)
 * \f$-th element of
 * the container for column major orientation. In a dense matrix all elements are represented in
 * memory in a
 * contiguous chunk of memory by definition.
 *
 * Orientation and storage can also be specified, otherwise \c row_major and \c std::array are used.
 * It is \b not
 * required by the storage container to initialize elements of the matrix.
 *
 * \tparam T the type of object stored in the matrix (like double, float, std::complex<double>,
 * etc...)
 * \tparam L the storage organization. It can be either \c row_major or \c column_major. Default is
 * \c row_major
...
class fixed_matrix : public matrix_container<fixed_matrix<T, M, N, L, A>>
{
...
public:
  ...
  BOOST_UBLAS_INLINE
  fixed_matrix() : matrix_container<self_type>(), data_() {}
  ...
  fixed_matrix(value_type v0, Types... vrest) : matrix_container<self_type>(), data_{{v0, vrest...}}
  {
  }
   ...
   * \param init initial value assigned to all elements
   */
  fixed_matrix(const value_type & init) : matrix_container<self_type>(), data_()
  {
    data_.fill(init);
  }
   ...
   */
  BOOST_UBLAS_INLINE
  fixed_matrix(const array_type & data) : matrix_container<self_type>(), data_(data) {}

  /** Copy-constructor of a dense fixed_matrix
   * \param m is a dense fixed_matrix
   */
  BOOST_UBLAS_INLINE
  fixed_matrix(const fixed_matrix & m) : matrix_container<self_type>(), data_(m.data_) {}

  /** Copy-constructor of a dense matrix from a matrix expression
   * \param ae is a matrix expression
   */
  template <class AE>
  BOOST_UBLAS_INLINE fixed_matrix(const matrix_expression<AE> & ae)
    : matrix_container<self_type>(), data_()
  {
    matrix_assign<scalar_assign>(*this, ae);
  }
   ...
   */
  BOOST_UBLAS_INLINE
  BOOST_CONSTEXPR size_type size1() const { return M; }
   ...
   */
  BOOST_UBLAS_INLINE
  BOOST_CONSTEXPR size_type size2() const { return N; }
  ...
  const_reference operator()(size_type i, size_type j) const
  {
    return data()[layout_type::element(
        i, M, j, N)]; // Fixme: add static lookup for element(...) i.e.: element<M, N>(i,j)
  }
...
// Assignment
...
#endif
  template <class C> // Container assignment without temporary
  BOOST_UBLAS_INLINE fixed_matrix & operator=(const matrix_container<C> & m)
  {
    resize(m().size1(), m().size2(), false);
    assign(m);
    return *this;
  ...
  template <class AE>
  BOOST_UBLAS_INLINE fixed_matrix & operator=(const matrix_expression<AE> & ae)
  {
    self_type temporary(ae);
    return assign_temporary(temporary);
  }
  template <class AE>
  BOOST_UBLAS_INLINE fixed_matrix & assign(const matrix_expression<AE> & ae)
  {
    matrix_assign<scalar_assign>(*this, ae);
    return *this;
  }
  ...
  template <class AE>
  BOOST_UBLAS_INLINE fixed_matrix & plus_assign(const matrix_expression<AE> & ae)
  {
    matrix_assign<scalar_plus_assign>(*this, ae);
    return *this;
  }
  ...
  template <class AE>
  BOOST_UBLAS_INLINE fixed_matrix & minus_assign(const matrix_expression<AE> & ae)
  {
    matrix_assign<scalar_minus_assign>(*this, ae);
    return *this;
  }
  template <class AT>
  BOOST_UBLAS_INLINE fixed_matrix & operator*=(const AT & at)
  {
    matrix_assign_scalar<scalar_multiplies_assign>(*this, at);
    return *this;
  }
  ...
  BOOST_UBLAS_INLINE
  friend void swap(fixed_matrix & m1, fixed_matrix & m2) { m1.swap(m2); }
...
public:
...
#ifndef BOOST_UBLAS_USE_INDEXED_ITERATOR
  class const_iterator1 : public container_const_reference<fixed_matrix>,
                          public random_access_iterator_base<dense_random_access_iterator_tag,
                                                             const_iterator1,
                                                             value_type>
  {
  ...
  const_iterator1 cbegin1() const { return begin1(); }
  BOOST_UBLAS_INLINE
  const_iterator1 end1() const { return find1(0, M, 0); }
  BOOST_UBLAS_INLINE
  const_iterator1 cend1() const { return end1(); }
  ...
  iterator1 begin1() { return find1(0, 0, 0); }
  BOOST_UBLAS_INLINE
  iterator1 end1() { return find1(0, M, 0); }

#ifndef BOOST_UBLAS_USE_INDEXED_ITERATOR
  class const_iterator2 : public container_const_reference<fixed_matrix>,
                          public random_access_iterator_base<dense_random_access_iterator_tag,
                                                             const_iterator2,
                                                             value_type>
  {
  ...
  const_iterator2 cbegin2() const { return begin2(); }
  BOOST_UBLAS_INLINE
  const_iterator2 end2() const { return find2(0, 0, N); }
  BOOST_UBLAS_INLINE
  const_iterator2 cend2() const { return end2(); }
  ...
  iterator2 begin2() { return find2(0, 0, 0); }
  BOOST_UBLAS_INLINE
  iterator2 end2() { return find2(0, 0, N); }
...
/** \brief A dense matrix of values of type \c T with a variable size bounded to a maximum of
 * \f$M\f$ by \f$N\f$.
 *
 * For a \f$(m \times n)\f$-dimensional matrix and \f$ 0 \leq i < m, 0 \leq j < n\f$, every element
 * \f$m_{i,j}\f$ is mapped
 * to the \f$(i.n + j)\f$-th element of the container for row major orientation or the \f$(i +
 * j.m)\f$-th element
 * of the container for column major orientation. Finally in a dense matrix all elements are
 * represented in memory
 * in a contiguous chunk of memory.
 *
 * Orientation can be specified. Default is \c row_major
 * The default constructor creates the matrix with size \f$M\f$ by \f$N\f$. Elements are constructed
 * by the storage
...
public:
  ...
  BOOST_UBLAS_INLINE bounded_matrix(const matrix<T, L, A2> & m) : matrix_type(m)
  {
  }
  template <class AE>
  BOOST_UBLAS_INLINE bounded_matrix(const matrix_expression<AE> & ae) : matrix_type(ae)
  {
  }
  BOOST_UBLAS_INLINE
  ~bounded_matrix() {}
...
*
* Rows or columns are not stored into contiguous chunks of memory but data inside rows (or columns)
* are.
* Orientation and storage can also be specified, otherwise a row major and unbounded arrays are
* used.
* The data is stored as a vector of vectors, meaning that rows or columns might not be stored into
* contiguous chunks
* of memory. Orientation and storage can also be specified, otherwise a row major and unbounded
* arrays are used.
* The storage type defaults to \c unbounded_array<unbounded_array<T>> and orientation is \c
* row_major. It is \b not
* required by the storage to initialize elements of the matrix. For a \f$(m \times n)\f$-dimensional
* matrix and
* \f$ 0 \leq i < m, 0 \leq j < n\f$, every element \f$m_{i,j}\f$ is mapped to the \f$(i.n + j)\f$-th
* element of the
* container for row major orientation or the \f$(i + j.m)\f$-th element of the container for column
* major orientation.
*
* \tparam T the type of object stored in the matrix (like double, float, complex, etc...)
* \tparam L the storage organization. It can be either \c row_major or \c column_major. By default
* it is \c row_major
...
public:
  ...
  // Construction and destruction
  BOOST_UBLAS_INLINE
  vector_of_vector() : matrix_container<self_type>(), size1_(0), size2_(0), data_(1) {}
  BOOST_UBLAS_INLINE
  vector_of_vector(size_type size1, size_type size2)
    : matrix_container<self_type>(), size1_(size1), size2_(size2), data_(1)
  {
  ...
  BOOST_UBLAS_INLINE vector_of_vector(const matrix_expression<AE> & ae)
    : matrix_container<self_type>(),
      size1_(ae().size1()),
      size2_(ae().size2()),
      data_(layout_type::size_M(size1_, size2_) + 1)
  {
  ...
  BOOST_UBLAS_INLINE
  friend void swap(vector_of_vector & m1, vector_of_vector & m2) { m1.swap(m2); }
...
public:
  ...
  const_iterator1 find1(int /*rank*/, size_type i, size_type j) const
  {
    ...
    return const_iterator1(*this, i, j);
#else
    return const_iterator1(
        *this, i, j, data()[layout_type::index_M(i, j)].begin() + layout_type::index_m(i, j));
#endif
  }
  BOOST_UBLAS_INLINE
  iterator1 find1(int /*rank*/, size_type i, size_type j)
  ...
  public:
    ...
    BOOST_UBLAS_INLINE
    size_type index2() const { return j_; }
...
public:
  ...
  // Construction and destruction
  BOOST_UBLAS_INLINE
  zero_matrix() : matrix_container<self_type>(), size1_(0), size2_(0) {}
  BOOST_UBLAS_INLINE
  zero_matrix(size_type size) : matrix_container<self_type>(), size1_(size), size2_(size) {}
  BOOST_UBLAS_INLINE
  zero_matrix(size_type size1, size_type size2)
    : matrix_container<self_type>(), size1_(size1), size2_(size2)
  {
  }
  BOOST_UBLAS_INLINE
  zero_matrix(const zero_matrix & m)
    : matrix_container<self_type>(), size1_(m.size1_), size2_(m.size2_)
  {
  }
  ...
  // Element access
  BOOST_UBLAS_INLINE
  const_reference operator()(size_type /* i */, size_type /* j */) const { return zero_; }
  ...
  BOOST_UBLAS_INLINE
  friend void swap(zero_matrix & m1, zero_matrix & m2) { m1.swap(m2); }
...
public:
  ...
  BOOST_UBLAS_INLINE
  const_iterator1 begin1() const { return const_iterator1(*this); }
  BOOST_UBLAS_INLINE
  const_iterator1 cbegin1() const { return begin1(); }
  BOOST_UBLAS_INLINE
 ...
 *
 * \tparam T the type of object stored in the matrix (like double, float, complex, etc...)
 * \tparam ALLOC an allocator for storing the zeros and one elements. By default, a standar
 * allocator is used.
 */
template <class T, class ALLOC>
...
public:
  ...
  // Construction and destruction
  BOOST_UBLAS_INLINE
  identity_matrix() : matrix_container<self_type>(), size1_(0), size2_(0), size_common_(0) {}
  BOOST_UBLAS_INLINE
  ...
  BOOST_UBLAS_INLINE
  friend void swap(identity_matrix & m1, identity_matrix & m2) { m1.swap(m2); }
...
public:
  ...
  public:
    ...
    // Dereference
    BOOST_UBLAS_INLINE
    const_reference operator*() const { return one_; }
    ...
    // Indices
    BOOST_UBLAS_INLINE
    size_type index1() const { return it_; }
    BOOST_UBLAS_INLINE
    size_type index2() const { return it_; }
  ...
  BOOST_UBLAS_INLINE
  const_iterator1 begin1() const { return const_iterator1(*this, 0); }
  BOOST_UBLAS_INLINE
  const_iterator1 cbegin1() const { return begin1(); }
  BOOST_UBLAS_INLINE
  const_iterator1 end1() const { return const_iterator1(*this, size_common_); }
  BOOST_UBLAS_INLINE
  const_iterator1 cend1() const { return end1(); }
  ...
  BOOST_UBLAS_INLINE
  const_iterator2 begin2() const { return const_iterator2(*this, 0); }
  BOOST_UBLAS_INLINE
  const_iterator2 cbegin2() const { return begin2(); }
  BOOST_UBLAS_INLINE
  const_iterator2 end2() const { return const_iterator2(*this, size_common_); }
  BOOST_UBLAS_INLINE
  const_iterator2 cend2() const { return end2(); }
...
const typename identity_matrix<T, ALLOC>::value_type
    identity_matrix<T, ALLOC>::one_(1); // ISSUE: need 'one'-traits here

/** \brief A matrix with all values of type \c T equal to the same value
 *
 * Changing one value has the effect of changing all the values. Assigning it to a normal matrix
 * will copy
 * the same value everywhere in this matrix. All accesses are constant time, due to the trivial
 * value.
 *
 * \tparam T the type of object stored in the matrix (like double, float, complex, etc...)
...
public:
  ...
  // Construction and destruction
  BOOST_UBLAS_INLINE
  scalar_matrix() : matrix_container<self_type>(), size1_(0), size2_(0), value_() {}
  BOOST_UBLAS_INLINE
  scalar_matrix(size_type size1, size_type size2, const value_type & value = value_type(1))
    : matrix_container<self_type>(), size1_(size1), size2_(size2), value_(value)
  {
  }
  BOOST_UBLAS_INLINE
  scalar_matrix(const scalar_matrix & m)
    : matrix_container<self_type>(), size1_(m.size1_), size2_(m.size2_), value_(m.value_)
  {
  }
  ...
  BOOST_UBLAS_INLINE
  friend void swap(scalar_matrix & m1, scalar_matrix & m2) { m1.swap(m2); }
...
public:
  ...
  public:
    ...
    BOOST_UBLAS_INLINE
    const_iterator1(const scalar_matrix & m,
    ...
    // Indices
    BOOST_UBLAS_INLINE
    size_type index1() const { return it1_; }
    BOOST_UBLAS_INLINE
    size_type index2() const { return it2_; }
  ...
  public:
    ...
    BOOST_UBLAS_INLINE
    const_iterator2(const scalar_matrix & m,
                    const const_subiterator_type & it1,
                    const const_subiterator_type & it2)
      : container_const_reference<scalar_matrix>(m), it1_(it1), it2_(it2)
    {
...
/** \brief An array based matrix class which size is defined at type specification or object
 * instanciation
 *
...
// Assignment
  ...
  BOOST_UBLAS_INLINE
  friend void swap(c_matrix & m1, c_matrix & m2) { m1.swap(m2); }
