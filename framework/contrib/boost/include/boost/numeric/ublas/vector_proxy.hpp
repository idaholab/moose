...
/** \brief A vector referencing a continuous subvector of elements of vector \c v containing all
 * elements specified by \c range.
 *
...
public:
  ...
  typedef typename boost::mpl::
      if_<boost::is_const<V>, typename V::const_reference, typename V::reference>::type reference;
  ...
  // Accessors
  BOOST_UBLAS_INLINE
  size_type start() const { return r_.start(); }
  BOOST_UBLAS_INLINE
  size_type size() const { return r_.size(); }

  // Storage accessors
  BOOST_UBLAS_INLINE
  const vector_closure_type & data() const { return data_; }
  BOOST_UBLAS_INLINE
  vector_closure_type & data() { return data_; }

// Element access
#ifndef BOOST_UBLAS_PROXY_CONST_MEMBER
  BOOST_UBLAS_INLINE
  const_reference operator()(size_type i) const { return data_(r_(i)); }
  BOOST_UBLAS_INLINE
  reference operator()(size_type i) { return data_(r_(i)); }
...
#else
  BOOST_UBLAS_INLINE
  reference operator()(size_type i) const { return data_(r_(i)); }
  ...
  // Closure comparison
  BOOST_UBLAS_INLINE
  bool same_closure(const vector_range & vr) const { return (*this).data_.same_closure(vr.data_); }
  ...
  BOOST_UBLAS_INLINE
  friend void swap(vector_range vr1, vector_range vr2) { vr1.swap(vr2); }
...
public:
  ...
  typedef indexed_iterator<vector_range<vector_type>, typename subiterator_type::iterator_category>
      iterator;
  ...
  public:
    ...
    // Index
    BOOST_UBLAS_INLINE
    size_type index() const { return it_.index() - (*this)().start(); }
 ...
 * Return a \c vector_range on a specified vector, a start and stop index. The resulting \c
 * vector_range can be manipulated like a normal vector.
 * If the specified range falls outside that of of the index range of the vector, then the resulting
 * \c vector_range is not a well formed
 * Vector Expression and access to an element outside of index range of the vector is \b undefined.
 */
template <class V>
BOOST_UBLAS_INLINE vector_range<V>
 ...
 * Return a \c const \c vector_range on a specified vector, a start and stop index. The resulting \c
 *const \c vector_range can be manipulated like a normal vector.
 *If the specified range falls outside that of of the index range of the vector, then the resulting
 *\c vector_range is not a well formed
 * Vector Expression and access to an element outside of index range of the vector is \b undefined.
 */
template <class V>
BOOST_UBLAS_INLINE vector_range<const V>
 ...
 * Return a \c const \c vector_range on a specified vector and \c range. The resulting \c
 * vector_range can be manipulated like a normal vector.
 * If the specified range falls outside that of of the index range of the vector, then the resulting
 * \c vector_range is not a well formed
 * Vector Expression and access to an element outside of index range of the vector is \b undefined.
 ...
 * Return a \c vector_range on a specified vector and \c range. The resulting \c vector_range can be
 * manipulated like a normal vector.
 * If the specified range falls outside that of of the index range of the vector, then the resulting
 * \c vector_range is not a well formed
 * Vector Expression and access to an element outside of index range of the vector is \b undefined.
 */
template <class V>
BOOST_UBLAS_INLINE const vector_range<const V>
 ...
 * Return a \c const \c vector_range on a specified vector and const \c range. The resulting \c
 * vector_range can be manipulated like a normal vector.
 * If the specified range falls outside that of of the index range of the vector, then the resulting
 * \c vector_range is not a well formed
 * Vector Expression and access to an element outside of index range of the vector is \b undefined.
 ...
 * Return a \c vector_range on a specified vector and const \c range. The resulting \c vector_range
 * can be manipulated like a normal vector.
 * If the specified range falls outside that of of the index range of the vector, then the resulting
 * \c vector_range is not a well formed
 * Vector Expression and access to an element outside of index range of the vector is \b undefined.
 */
template <class V>
BOOST_UBLAS_INLINE const vector_range<V>
...
template <class V>
struct vector_temporary_traits<vector_range<V>> : vector_temporary_traits<V>
{
};
template <class V>
struct vector_temporary_traits<const vector_range<V>> : vector_temporary_traits<V>
{
};

/** \brief A vector referencing a non continuous subvector of elements of vector v containing all
 * elements specified by \c slice.
 *
...
public:
    ...
    : data_(data), s_(s.preprocess(data.size()))
  {
    ...
    //                    s_.start () + s_.stride () * (s_.size () - (s_.size () > 0)) <= data_.size
    //                    (), bad_index ());
  }
  BOOST_UBLAS_INLINE
  ...
  // Accessors
  BOOST_UBLAS_INLINE
  size_type start() const { return s_.start(); }
  BOOST_UBLAS_INLINE
  difference_type stride() const { return s_.stride(); }
  BOOST_UBLAS_INLINE
...
// Element access
#ifndef BOOST_UBLAS_PROXY_CONST_MEMBER
  BOOST_UBLAS_INLINE
  const_reference operator()(size_type i) const { return data_(s_(i)); }
  BOOST_UBLAS_INLINE
  reference operator()(size_type i) { return data_(s_(i)); }
...
#else
  BOOST_UBLAS_INLINE
  reference operator()(size_type i) const { return data_(s_(i)); }
  ...
  // Closure comparison
  BOOST_UBLAS_INLINE
  bool same_closure(const vector_slice & vr) const { return (*this).data_.same_closure(vr.data_); }
  ...
  BOOST_UBLAS_INLINE
  friend void swap(vector_slice vs1, vector_slice vs2) { vs1.swap(vs2); }
...
public:
  ...
  class const_iterator
      : public container_const_reference<vector_slice>,
        public iterator_base_traits<typename V::const_iterator::iterator_category>::
            template iterator_base<const_iterator, value_type>::type
  {
  public:
  ...
  class iterator : public container_reference<vector_slice>,
                   public iterator_base_traits<typename V::iterator::iterator_category>::
                       template iterator_base<iterator, value_type>::type
  {
  public:
...
// Simple Projections
template <class V>
BOOST_UBLAS_INLINE vector_slice<V>
subslice(V & data,
...
template <class V>
BOOST_UBLAS_INLINE vector_slice<const V>
subslice(const V & data,
         typename V::size_type start,
         typename V::difference_type stride,
         typename V::size_type size)
{
...
template <class V>
BOOST_UBLAS_INLINE const vector_slice<const V>
...
template <class V>
BOOST_UBLAS_INLINE const vector_slice<V>
project(const vector_slice<V> & data, const typename vector_slice<V>::slice_type & s)
{
  return data.project(s);
}
// ISSUE in the following two functions it would be logical to use vector_slice<V>::range_type but
...
// Specialization of temporary_traits
template <class V>
struct vector_temporary_traits<vector_slice<V>> : vector_temporary_traits<V>
{
};
template <class V>
struct vector_temporary_traits<const vector_slice<V>> : vector_temporary_traits<V>
{
};
...
/** \brief A vector referencing a non continuous subvector of elements given another vector of
 * indices.
 *
 * It is the most general version of any subvectors because it uses another vector of indices to
 * reference
 * the subvector.
 *
 ...
 * - \c boost::numeric::ublas::vector<int> can work too (\c int can be replaced by another integer
 * type)
 * - etc...
 *
 * An indirect vector can be used as a normal vector in any expression. If the specified indirect
 * vector
 * falls outside that of the indices of the vector, then the \c vector_indirect is not a well formed
 * \i Vector \i Expression and access to an element outside of indices of the vector is \b
 * undefined.
 *
 * \tparam V the type of vector referenced (for example \c vector<double>)
...
public:
  ...
  // Accessors
  BOOST_UBLAS_INLINE
  size_type size() const { return ia_.size(); }
  BOOST_UBLAS_INLINE
  const_indirect_array_type & indirect() const { return ia_; }
  BOOST_UBLAS_INLINE
  indirect_array_type & indirect() { return ia_; }
...
// Element access
#ifndef BOOST_UBLAS_PROXY_CONST_MEMBER
  BOOST_UBLAS_INLINE
  const_reference operator()(size_type i) const { return data_(ia_(i)); }
  BOOST_UBLAS_INLINE
  reference operator()(size_type i) { return data_(ia_(i)); }
...
#else
  BOOST_UBLAS_INLINE
  reference operator()(size_type i) const { return data_(ia_(i)); }
  ...
  // Closure comparison
  BOOST_UBLAS_INLINE
  bool same_closure(const vector_indirect & /*vr*/) const { return true; }
  ...
  BOOST_UBLAS_INLINE
  friend void swap(vector_indirect vi1, vector_indirect vi2) { vi1.swap(vi2); }
...
BOOST_UBLAS_INLINE vector_indirect<V, indirect_array<A>>
project(V & data, const indirect_array<A> & ia)
{
  return vector_indirect<V, indirect_array<A>>(data, ia);
}
template <class V, class A>
BOOST_UBLAS_INLINE const vector_indirect<const V, indirect_array<A>>
...
template <class V, class IA>
BOOST_UBLAS_INLINE vector_indirect<V, IA>
project(vector_indirect<V, IA> & data, const typename vector_indirect<V, IA>::range_type & r)
{
  return data.project(r);
}
template <class V, class IA>
BOOST_UBLAS_INLINE const vector_indirect<V, IA>
...
template <class V, class A>
BOOST_UBLAS_INLINE const vector_indirect<V, indirect_array<A>>
...
// Specialization of temporary_traits
template <class V>
struct vector_temporary_traits<vector_indirect<V>> : vector_temporary_traits<V>
{
};
template <class V>
struct vector_temporary_traits<const vector_indirect<V>> : vector_temporary_traits<V>
{
};
}
}
