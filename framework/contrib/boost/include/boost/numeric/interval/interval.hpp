...
{
public:
  comparison_error() : std::runtime_error("boost::interval: uncertain comparison") {}
};
...
private:
  struct interval_holder
  {
    ...
    interval_holder(const interval<T, Policies2> & r) : low(r.lower()), up(r.upper())
    {
...
interval<T, Policies>::operator=(T const & v)
{
  if (checking::is_nan(v))
    set_empty();
  else
