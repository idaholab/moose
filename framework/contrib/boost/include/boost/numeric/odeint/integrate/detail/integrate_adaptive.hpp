...
size_t integrate_const(Stepper stepper,
                       System system,
                       State & start_state,
                       Time start_time,
                       Time end_time,
...
integrate_adaptive(Stepper stepper,
                   ...
                   stepper_tag)
{
  size_t steps = detail::integrate_const(
      stepper, system, start_state, start_time, end_time, dt, observer, stepper_tag());
                   ...
                   dense_output_stepper_tag)
{
  ...
  while (less_with_sign(st.current_time(), end_time, st.current_time_step()))
  {
    ...
    st.initialize(
        st.current_state(), st.current_time(), static_cast<Time>(end_time - st.current_time()));
  }
  obs(st.current_state(), st.current_time());
