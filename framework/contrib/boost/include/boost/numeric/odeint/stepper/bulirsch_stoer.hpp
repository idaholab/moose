...
public:
  ...
  bulirsch_stoer(value_type eps_abs = 1E-6,
  ...
  /**
   * \brief Second version to solve the forwarding problem, can be used with Boost.Range as
   * StateInOut.
   */
  template <class System, class StateInOut>
  ...
  try_step(System system, StateInOut & x, const DerivIn & dxdt, time_type & t, time_type & dt)
  {
    m_xnew_resizer.adjust_size(
        x,
        detail::bind(&controlled_error_bs_type::template resize_m_xnew<StateInOut>,
                     detail::ref(*this),
                     detail::_1));
                                  ...
                                  time_type & dt)
  {
    ...
    if (m_resizer.adjust_size(in,
                              detail::bind(&controlled_error_bs_type::template resize_impl<StateIn>,
                                           detail::ref(*this),
                                           detail::_1)))
    {
...
private:
  ...
  controlled_step_result try_step_v1(System system, StateInOut & x, time_type & t, time_type & dt)
  {
    ...
    m_dxdt_resizer.adjust_size(
        x,
        detail::bind(&controlled_error_bs_type::template resize_m_dxdt<StateInOut>,
                     detail::ref(*this),
                     detail::_1));
    sys(x, m_dxdt.m_v, t);
  ...
  void
  extrapolate(size_t k, state_table_type & table, const value_matrix & coeff, StateInOut & xest)
  ...
  /* calculates the optimal step size for a given error and stage number */
  {
    ...
    else
    {
      ...
      fac = max BOOST_PREVENT_MACRO_SUBSTITUTION(
          ...
          min BOOST_PREVENT_MACRO_SUBSTITUTION(static_cast<value_type>(1.0 / facmin), fac));
    }
    return h * fac;
  }

  controlled_step_result
  set_k_opt(size_t k, const inv_time_vector & work, const time_vector & h_opt, time_type & dt)
...
/**
 * \fn bulirsch_stoer::try_step( System system , const StateIn &in , const DerivIn &dxdt , time_type
 * &t , StateOut &out , time_type &dt )
 * \brief Tries to perform one step.
 *
 * This method tries to do one step with step size dt. If the error estimate
 * is to large, the step is rejected and the method returns fail and the
