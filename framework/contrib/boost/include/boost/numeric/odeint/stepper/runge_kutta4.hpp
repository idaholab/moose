...
{
  rk4_coefficients_a1(void) { (*this)[0] = static_cast<Value>(1) / static_cast<Value>(2); }
};
...
public:
  ...
  runge_kutta4(const algebra_type & algebra = algebra_type())
    : stepper_base_type(boost::fusion::make_vector(rk4_coefficients_a1<Value>(),
                        ...
                        rk4_coefficients_b<Value>(),
                        rk4_coefficients_c<Value>(),
                        algebra)
  {
  }
};
 ...
 * Furthermore, it derivs from explicit_generic_rk which is a generic Runge-Kutta algorithm. For
 * more details see
