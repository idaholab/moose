...
template <size_t StageCount,
          ...
          class Resizer>
#ifndef DOXYGEN_SKIP
class explicit_generic_rk : public explicit_stepper_base<explicit_generic_rk<StageCount,
                                                                             Order,
                                                                             State,
                                                                             Value,
                                                                             Deriv,
...
public:
#ifndef DOXYGEN_SKIP
  typedef explicit_stepper_base<explicit_generic_rk<StageCount,
                                                    Order,
                                                    State,
                                                    Value,
                                                    Deriv,
...
#ifndef DOXYGEN_SKIP
  typedef explicit_generic_rk<StageCount,
                              Order,
                              State,
                              Value,
                              Deriv,
...
public:
                    ...
                    time_type dt)
  {
    ...
    m_rk_algorithm.do_step(
        stepper_base_type::m_algebra, system, in, dxdt, t, out, dt, m_x_tmp.m_v, m_F);
  }
...
private:
  ...
  bool resize_impl(const StateIn & x)
  {
    ...
    {
      resized |=
          adjust_size_by_resizeability(m_F[i], x, typename is_resizeable<deriv_type>::type());
    }
    return resized;
  }
 ...
 *
 * This class implements the explicit Runge-Kutta algorithms without error estimation in a generic
 * way.
 * The Butcher tableau is passed to the stepper which constructs the stepper scheme with the help of
 * a
 ...
 * \brief Constructs the explicit_generic_rk class. See examples section for details on the
 * coefficients.
 ...
 * \brief This method performs one step. The derivative `dxdt` of `in` at the time `t` is passed to
 * the method.
