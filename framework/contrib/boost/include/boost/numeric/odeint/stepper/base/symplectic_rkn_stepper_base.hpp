...
public:
...
#ifndef DOXYGEN_SKIP
  typedef symplectic_nystroem_stepper_base<NumOfStages,
                                           Order,
                                           Coor,
                                           Momentum,
                                           Value,
  ...
  /**
   * \brief Same function as do_step( system , q , p , t , dt ). It differs only in a different
   * const specifier in order
   * to solve the forwarding problem, can be called with Boost.Range.
   */
  template <class System, class CoorInOut, class MomentumInOut>
  void
  do_step(System system, const CoorInOut & q, const MomentumInOut & p, time_type t, time_type dt)
  {
    do_step(system, std::make_pair(detail::ref(q), detail::ref(p)), t, dt);
  }
...
private:
  ...
  {
    typedef typename odeint::unwrap_reference<System>::type system_type;
    typedef typename odeint::unwrap_reference<typename system_type::first_type>::type
        coor_deriv_func_type;
    ...
    typedef
        typename odeint::unwrap_reference<typename state_out_type::first_type>::type coor_out_type;
    typedef typename odeint::unwrap_reference<typename state_out_type::second_type>::type
        momentum_out_type;
    ...
    m_dqdt_resizer.adjust_size(
        coor_in,
        detail::bind(&internal_stepper_base_type::template resize_dqdt<coor_in_type>,
                     detail::ref(*this),
                     detail::_1));
    m_dpdt_resizer.adjust_size(
        momentum_in,
        detail::bind(&internal_stepper_base_type::template resize_dpdt<momentum_in_type>,
                     detail::ref(*this),
                     detail::_1));
    ...
    for (size_t l = 0; l < num_of_stages; ++l)
    {
      if (l == 0)
      {
        ...
        this->m_algebra.for_each3(
            ...
            coor_in,
            m_dqdt.m_v,
            typename operations_type::template scale_sum2<value_type, time_type>(1.0,
                                                                                 m_coef_a[l] * dt));
                    ...
                    boost::mpl::false_)
  {
    ...
    // m_dqdt_resizer.adjust_size( coor_in , detail::bind( &internal_stepper_base_type::template
    // resize_dqdt< coor_in_type > , detail::ref( *this ) , detail::_1 ) );
    m_dpdt_resizer.adjust_size(
        momentum_in,
        detail::bind(&internal_stepper_base_type::template resize_dpdt<momentum_in_type>,
                     detail::ref(*this),
    ...
    // step 0
    this->m_algebra.for_each3(coor_out,
                              coor_in,
                              momentum_in,
                              typename operations_type::template scale_sum2<value_type, time_type>(
                                  1.0, m_coef_a[0] * dt));
    momentum_func(coor_out, m_dpdt.m_v);
    this->m_algebra.for_each3(momentum_out,
                              momentum_in,
                              m_dpdt.m_v,
                              typename operations_type::template scale_sum2<value_type, time_type>(
                                  1.0, m_coef_b[0] * dt));
 ...
 *
 * This class is the base class for the symplectic Runge-Kutta-Nystroem steppers. Symplectic
 * steppers are usually
 ...
 * in average. In detail this class of steppers can be used to solve separable Hamiltonian systems
 * which can be written
 * in the form H(q,p) = H1(p) + H2(q). q is usually called the coordinate, while p is the momentum.
 ...
 * estimation are not
 * provided for this class of solvers. It derives from algebra_stepper_base. Several `do_step`
 * variants are provided:
 *
 * - `do_step( sys , x , t , dt )` - The classical `do_step` method. The sys can be either a pair of
 * function objects
 *    for the coordinate or the momentum part or one function object for the momentum part. `x` is a
 * pair of coordinate
 *    and momentum. The state is updated in-place.
 * - `do_step( sys , q , p , t , dt )` - This method is similar to the method above with the
 * difference that the coordinate
 *    and the momentum are passed explicitly and not packed into a pair.
 * - `do_step( sys , x_in , t , x_out , dt )` - This method transforms the state out-of-place.
 * `x_in` and `x_out` are here pairs
 ...
 * \brief Constructs a symplectic_nystroem_stepper_base class. The parameters of the specific
 * Nystroem method and the
 ...
 * is updated in-place.
 *
 * \note boost::ref or std::ref can be used for the system as well as for the state. So, it is
 * correct
 ...
 *
 * \param system The system, can be represented as a pair of two function object or one function
 * object. See above.
 * \param state The state of the ODE. It is a pair of Coor and Momentum. The state is updated
 ...
 * \param system The system, can be represented as a pair of two function object or one function
 * object. See above.
 * \param q The coordinate of the ODE. It is updated in-place. Therefore, the new value of the
 * coordinate will be written
 * into this variable.
 * \param p The momentum of the ODE. It is updated in-place. Therefore, the new value of the
 * momentum will be written info
 * this variable.
 * \param t The time of the ODE. It is not advanced by this method.
 * \param dt The time step.
 */
