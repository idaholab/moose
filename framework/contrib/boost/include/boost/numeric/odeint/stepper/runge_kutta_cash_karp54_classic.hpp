                    ...
                    Err & xerr)
  {
    ...
    stepper_base_type::m_algebra.for_each6(
        ...
        typename operations_type::
            template scale_sum5<time_type, time_type, time_type, time_type, time_type>(
 ...
 * recurring
 * template pattern). This class implements the method directly, hence the generic Runge-Kutta
 ...
 * )
 * \brief Constructs the runge_kutta_cash_karp54_classic class. This constructor can be used as a
 * default
...
/**
 * \fn runge_kutta_cash_karp54_classic::do_step_impl( System system , const StateIn &in , const
 DerivIn &dxdt , time_type t , StateOut &out , time_type dt , Err &xerr )
...
/**
 * \fn runge_kutta_cash_karp54_classic::do_step_impl( System system , const StateIn &in , const
 * DerivIn &dxdt , time_type t , StateOut &out , time_type dt )
 * \brief This method performs one step. The derivative `dxdt` of `in` at the time `t` is passed to
 * the method.
 * The result is updated out-of-place, hence the input is in `in` and the output in `out`.
 * Access to this step functionality is provided by explicit_error_stepper_base and
