...
public:
  ...
  typedef adams_moulton<steps,
                        ...
                        resizer_type>
      adams_moulton_type;
  typedef adams_bashforth_moulton<steps,
                                  state_type,
                                  value_type,
                                  deriv_type,
                                  time_type,
  ...
  adams_bashforth_moulton(void)
    : m_adams_bashforth(), m_adams_moulton(m_adams_bashforth.algebra()), m_x(), m_resizer()
  {
  }

  adams_bashforth_moulton(const algebra_type & algebra)
    : m_adams_bashforth(algebra), m_adams_moulton(m_adams_bashforth.algebra()), m_x(), m_resizer()
  {
  }
  ...
  /**
   * \brief Second version to solve the forwarding problem, can be called with Boost.Range as
   * StateInOut.
   */
  template <class System, class StateInOut>
  ...
  void initialize(
      ExplicitStepper explicit_stepper, System system, StateIn & x, time_type & t, time_type dt)
  {
...
private:
  ...
  void do_step_impl1(System system, StateInOut & x, time_type t, time_type dt)
  {
    ...
    {
      m_resizer.adjust_size(x,
                            detail::bind(&stepper_type::template resize_impl<StateInOut>,
                                         detail::ref(*this),
                                         detail::_1));
 ...
 * \param x The state of the ODE which should be solved. After calling do_step the result is updated
 * in x.
...
/**
 * \fn adams_bashforth_moulton::do_step( System system , const StateIn &in , time_type t , const
...
/**
 * \fn adams_bashforth_moulton::initialize( ExplicitStepper explicit_stepper , System system ,
