...
public:
#ifndef DOXYGEN_SKIP
  typedef explicit_stepper_base<Stepper,
                                Order,
                                State,
                                Value,
                                Deriv,
 ...
 * Step size control and error estimation as well as dense output are not provided.
 * explicit_stepper_base
 * is used as the interface in a CRTP (currently recurring template pattern). In order to work
 * correctly the parent class needs to have a method `do_step_impl( system , in , dxdt_in , t , out
 * , dt )`.
 ...
 *  public:
 *
 *     typedef explicit_stepper_base< 1 , State , Value , Deriv , Time , Algebra , Operations ,
 * Resizer > base_type;
 *
 ...
 * \endcode
 *
 * For the Stepper concept only the `do_step( sys , x , t , dt )` needs to be implemented. But this
 * class
 * provides additional `do_step` variants since the stepper is explicit. These methods can be used
 * to increase
 * the performance in some situation, for example if one needs to analyze `dxdt` during each step.
 * In this case
 * one can use
 *
 * \code
 ...
 * derivative at the point `t` must be
 ...
 * appropriate `integrate` routines or `iterator`s.
 *
...
/**
 * \fn explicit_stepper_base::do_step( System system , StateInOut &x , const DerivIn &dxdt ,
...
/**
 * \fn void explicit_stepper_base::do_step( System system , const StateIn &in , time_type t ,
