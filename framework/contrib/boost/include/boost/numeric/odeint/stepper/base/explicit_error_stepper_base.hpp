...
public:
  ...
  typedef explicit_error_stepper_base<Stepper,
                                      Order,
                                      StepperOrder,
                                      ErrorOrder,
                                      State,
  ...
  typename boost::disable_if<boost::is_same<DerivIn, time_type>, void>::type do_step(
      System system, StateInOut & x, const DerivIn & dxdt, time_type t, time_type dt, Err & xerr)
  {
 ...
 *
 * This class serves as the base class for all explicit steppers with algebra and operations. In
 * contrast to
 * explicit_stepper_base it also estimates the error and can be used in a controlled stepper to
 * provide
 * step size control.
 *
 ...
 *
 * explicit_error_stepper_base provides several overloaded `do_step` methods, see the list below.
 * Only two of them
 * are needed to fulfill the Error Stepper concept. The other ones are for convenience and for
 * performance. Some
 * of them simply update the state out-of-place, while other expect that the first derivative at `t`
 * is passed to the
 * stepper.
 *
 ...
 * - `do_step( sys , x , dxdt , t , dt )` - This method updates the state in-place, but the
 ...
 * estimates the error
 *      during the step.
 * - `do_step( sys , in , dxdt , t , out , dt , xerr )` - This methods updates the state
 * out-of-place and estimates
...
/**
 * \fn explicit_error_stepper_base::do_step( System system , StateInOut &x , time_type t , time_type
 * dt )
 ...
 * method
 * the derivative of x is also passed to this method. It is supposed to be used in the following
 * way:
 *
 * \code
 * sys( x , dxdt , t );
...
/**
 * \fn explicit_error_stepper_base::do_step( System system , const StateIn &in , time_type t ,
 ...
 * \endcode
 *
 * The result is updated in place in x. This method is disabled if Time and DerivIn are of the same
 * type. In this
 * case the method could not be distinguished from other `do_step` versions.
 *
 * \note This method does not solve the forwarding problem.
