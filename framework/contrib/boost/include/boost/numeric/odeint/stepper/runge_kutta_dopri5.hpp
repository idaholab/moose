/*
 ...
 [begin_description]
 Implementation of the Dormand-Prince 5(4) method. This stepper can also be used with the
...
public:
                    ...
                    time_type dt)
  {
    ...
    m_k_x_tmp_resizer.adjust_size(in,
                                  detail::bind(&stepper_type::template resize_k_x_tmp_impl<StateIn>,
                                               detail::ref(*this),
                                               detail::_1));
    ...
    stepper_base_type::m_algebra.for_each7(
        ...
        m_k5.m_v,
        typename operations_type::
            template scale_sum6<value_type, time_type, time_type, time_type, time_type, time_type>(
                    ...
                    Err & xerr)
  {
    ...
    if (same_instance(dxdt_in, dxdt_out))
    {
      ...
      stepper_base_type::m_algebra.for_each7(
          ...
          dxdt_out,
          typename operations_type::
              template scale_sum6<time_type, time_type, time_type, time_type, time_type, time_type>(
                  ...
                  time_type t_new) const
  {
    ...
    const value_type A =
        theta_sq * (static_cast<value_type>(3) - static_cast<value_type>(2) * theta);
    ...
    stepper_base_type::m_algebra.for_each8(
        ...
        m_k6.m_v,
        deriv_new,
        typename operations_type::template scale_sum7<value_type,
                                                      time_type,
                                                      time_type,
                                                      time_type,
                                                      time_type,
 ...
 * applications.
 *
 * This class derives from explicit_error_stepper_fsal_base and inherits its interface via CRTP
 * (current recurring
 ...
 * \brief This method performs one step. The derivative `dxdt_in` of `in` at the time `t` is passed
 * to the
 * method. The result is updated out-of-place, hence the input is in `in` and the output in `out`.
 * Furthermore,
 * the derivative is update out-of-place, hence the input is assumed to be in `dxdt_in` and the
 * output in
 * `dxdt_out`.
...
/**
 * \fn runge_kutta_dopri5::calc_state( time_type t , StateOut &x , const StateIn1 &x_old , const
 * DerivIn1 &deriv_old , time_type t_old , const StateIn2 &  , const DerivIn2 &deriv_new , time_type
 * t_new ) const
 * \brief This method is used for continuous output and it calculates the state `x` at a time `t`
 * from the
 * knowledge of two states `old_state` and `current_state` at time points `t_old` and `t_new`. It
 * also uses
