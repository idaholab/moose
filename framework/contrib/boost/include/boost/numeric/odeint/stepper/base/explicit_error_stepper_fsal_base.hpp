...
public:
  ...
  typedef explicit_error_stepper_fsal_base<Stepper,
                                           Order,
                                           StepperOrder,
                                           ErrorOrder,
                                           State,
  ...
  template <class System, class StateInOut, class DerivInOut>
  void
  do_step_dxdt_impl(System system, StateInOut & x, DerivInOut & dxdt, time_type t, time_type dt)
  {
    m_first_call = true;
    this->stepper().do_step_impl(system, x, dxdt, t, x, dxdt, dt);
  }
  ...
  void
  do_step(System system, const StateIn & in, time_type t, StateOut & out, time_type dt, Err & xerr)
  {
    if (m_resizer.adjust_size(
            in,
            detail::bind(&internal_stepper_base_type::template resize_impl<StateIn>,
 ...
 *
 * This class serves as the base class for all explicit steppers with algebra and operations and
 * which fulfill the FSAL
 ...
 *
 * The FSAL property means that the derivative of the system at t+dt is already used in the current
 * step going from
 * t to t +dt. Therefore, some more do_steps method can be introduced and the controlled steppers
 * can explicitly make use
 * of this property.
 *
 * \note This stepper provides `do_step` methods with and without error estimation. It has therefore
 * three orders,
 * one for the order of a step if the error is not estimated. The other two orders are the orders of
 * the step and
 * the error step if the error estimation is performed.
 *
 * explicit_error_stepper_fsal_base  is used as the interface in a CRTP (currently recurring
 * template
 ...
 *
 * This class can have an intrinsic state depending on the explicit usage of the `do_step` method.
 * This means that some
 * `do_step` methods are expected to be called in order. For example the `do_step( sys , x , t , dt
 * , xerr )` will keep track
 * of the derivative of `x` which is the internal state. The first call of this method is recognized
 * such that one
 ...
 * \endcode
 *
 * But it is unsafe to call this method with different system functions after each other. Do do so,
 * one must initialize the
 * internal state with the `initialize` method or reset the internal state with the `reset` method.
 *
 * explicit_error_stepper_fsal_base provides several overloaded `do_step` methods, see the list
 * below. Only two of them are needed
 * to fulfill the Error Stepper concept. The other ones are for convenience and for better
 * performance. Some of them
 * simply update the state out-of-place, while other expect that the first derivative at `t` is
 * passed to the stepper.
 *
 * - `do_step( sys , x , t , dt )` - The classical `do_step` method needed to fulfill the Error
 * Stepper concept. The
 ...
 * result of the step
 *      is stored in `out`.
 * - `do_step( sys , in , dxdt_in , t , out , dxdt_out , dt )` - This method updates the state and
 * the derivative
 *     out-of-place. It expects that the derivative at the point `t` is explicitly passed in
 * `dxdt_in`.
 * - `do_step( sys , x , t , dt , xerr )` - This `do_step` method is needed to fulfill the Error
 * Stepper concept. The
 *     state is updated in-place and an error estimate is calculated. A type modelling a Boost.Range
 * can be used for x.
 * - `do_step( sys , x , dxdt , t , dt , xerr )` - This method updates the state and the derivative
 * in-place. It is assumed
 *      that the dxdt has the value of the derivative of x at time t. An error estimate is
 * calculated.
 * - `do_step( sys , in , t , out , dt , xerr )` - This method updates the state out-of-place and
 * estimates the error
 *      during the step.
 * - `do_step( sys , in , dxdt_in , t , out , dxdt_out , dt , xerr )` - This methods updates the
 * state and the derivative
 *      out-of-place and estimates the error during the step. It is assumed the dxdt_in is
 * derivative of in at time t.
 *
 * \note The system is always passed as value, which might result in poor performance if it contains
 * data. In this
 *      case it can be used with `boost::ref` or `std::ref`, for example `stepper.do_step(
 * boost::ref( sys ) , x , t , dt );`
 *
 * \note The time `t` is not advanced by the stepper. This has to done manually, or by the
 ...
 * \tparam StepperOrder The order of a step if the stepper is used with error estimation. Usually
 * Order and StepperOrder have
 * the same value.
 ...
 * double, or a multiprecision type. It must not necessary be the value_type of the State. For
 * example
 * the State can be a `vector< complex< double > >` in this case the Value must be double.
 * The default value is double.
 * \tparam Deriv The type representing time derivatives of the state type. It is usually the same
 * type as the
 * state type, only if used with Boost.Units both types differ.
 * \tparam Time The type representing the time. Usually the same type as the value type. When
 * Boost.Units is
...
/**
 * \fn explicit_error_stepper_fsal_base::explicit_error_stepper_fsal_base( const algebra_type
 * &algebra )
...
/**
 * \fn explicit_error_stepper_fsal_base::do_step( System system , StateInOut &x , time_type t ,
 * time_type dt )
 ...
 * \endcode
 *
 * \note This method does NOT use the initial state, since the first derivative is explicitly passed
 * to this method.
 *
 ...
 * \brief The method performs one step with the stepper passed by Stepper. The state of the ODE is
 * updated out-of-place.
 * Furthermore, the derivative of x at t is passed to the stepper and updated by the stepper to its
 * new value at
 * t+dt.
 *
 * \note This method does not solve the forwarding problem.
 ...
 * \note This method does NOT use the internal state of the stepper.
 *
 * The result is updated in place in x. This method is disabled if Time and Deriv are of the same
 * type. In this
 * case the method could not be distinguished from other `do_step` versions. This method is disabled
 * if StateInOut and
 ...
 * \param x The state of the ODE which should be solved. After calling do_step the result is updated
 * in x.
 * \param dxdt The derivative of x at t. After calling `do_step` this value is updated to the new
 * value at `t+dt`.
 * \param t The value of the time, at which the step should be performed.
 * \param dt The step size.
 ...
 * \param deriv The derivative of x. The next call of `do_step` expects that the derivative of `x`
 * passed to `do_step`
 ...
 * \brief Returns if the stepper is already initialized. If the stepper is not initialized, the
 * first
