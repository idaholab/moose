...
template <int N>
struct order_helper
    : mpl::max<typename mpl::eval_if<mpl::equal_to<mpl::modulus<int_<N>, int_<2>>, int_<0>>,
                                     int_<N>,
                                     int_<N + 1>>::type,
               int_<4>>::type
{
...
public:
      ...
      ExplicitStepper explicit_stepper, System system, StateIn & x, time_type & t, time_type dt)
  {
    ...
    for (size_t i = 0; i + 1 < steps; ++i)
    {
      ...
      stepper.do_step_dxdt_impl(system, x, m_step_storage[0].m_v, t, dt);
      t += dt;
    }
  ...
  bool is_initialized(void) const { return m_steps_initialized >= (steps - 1); }
...
private:
  ...
  void do_step_impl(System system, const StateIn & in, time_type t, StateOut & out, time_type dt)
  {
    ...
    if (m_resizer.adjust_size(in,
                              detail::bind(&stepper_type::template resize_impl<StateIn>,
                                           detail::ref(*this),
                                           detail::_1)))
    {
      m_steps_initialized = 0;
    ...
    if (m_steps_initialized + 1 < steps)
    {
      ...
      m_initializing_stepper.do_step_dxdt_impl(system, in, m_step_storage[0].m_v, t, out, dt);
      ++m_steps_initialized;
    }
    else
    {
      ...
      detail::adams_bashforth_call_algebra<steps, algebra_type, operations_type>()(
          this->m_algebra, in, out, m_step_storage, m_coefficients, dt);
    }
  }
...
/**
 * \fn void adams_bashforth::do_step( System system , const StateIn &in , time_type t , StateOut
 * &out , time_type dt )
...
/**
 * \fn void adams_bashforth::initialize( ExplicitStepper explicit_stepper , System system , StateIn
 * &x , time_type &t , time_type dt )
 ...
 * \brief Initialized the stepper. Does Steps-1 steps with an internal instance of
 * InitializingStepper to fill the buffer.
