...
public:
  ...
  bulirsch_stoer_dense_out(value_type eps_abs = 1E-6,
      ...
      KFAC2(0.9)
  {
    ...
    for (unsigned short i = 0; i < m_k_max + 1; i++)
    {
      ...
      for (size_t k = 0; k < i; ++k)
      {
        ...
        m_coeff[i][k] =
            1.0 / (r * r - static_cast<value_type>(1.0)); // coefficients for extrapolation
      }
      ...
      BOOST_PREVENT_MACRO_SUBSTITUTION( static_cast<int>( m_k_max-1 ) , static_cast<int>( logfact )
      ));
      */
    }
    int num = 1;
                                  ...
                                  time_type & dt)
  {
    ...
    // std::cout << "t=" << t <<", dt=" << dt << ", k_opt=" << m_current_k_opt << ", first: " <<
    // m_first << std::endl;
    ...
    for (size_t k = 0; k <= m_current_k_opt + 1; k++)
    {
      ...
      {
        m_midpoint.do_step(
            system, in, dxdt, t, m_table[k - 1].m_v, dt, m_mp_states[k].m_v, m_derivs[k]);
    ...
    if (!reject)
    {
      ...
      {
        reject = true;
        new_h = dt * pow BOOST_PREVENT_MACRO_SUBSTITUTION(
...
private:
  ...
  // polynomial extrapolation, see http://www.nr.com/webnotes/nr3web21.pdf
  {
    ...
    for (int j = k - 1; j > 0; --j)
    {
      m_algebra.for_each3(table[j - 1].m_v,
                          table[j].m_v,
                          table[j - 1].m_v,
  ...
  void extrapolate_dense_out(size_t k,
                             StateVector & table,
                             const value_matrix & coeff,
                             size_t order_start_index = 0)
  // polynomial extrapolation, see http://www.nr.com/webnotes/nr3web21.pdf
  {
    ...
    m_algebra.for_each3(table[0].m_v,
                        table[1].m_v,
                        table[0].m_v,
                        typename operations_type::template scale_sum2<value_type, value_type>(
  ...
  time_type calc_h_opt(time_type h, value_type error, size_t k) const
  {
    ...
    else
    {
      ...
      fac = max BOOST_PREVENT_MACRO_SUBSTITUTION(
          static_cast<value_type>(facmin / STEPFAC4),
          min BOOST_PREVENT_MACRO_SUBSTITUTION(
              static_cast<value_type>(static_cast<value_type>(1) / facmin), fac));
    }
    return h * fac;
  }
  ...
  value_type prepare_dense_output(int k,
                                  const StateIn1 & x_start,
                                  const DerivIn1 & dxdt_start,
                                  const StateIn2 & /* x_end */,
                                  const DerivIn2 & /*dxdt_end */,
                                  time_type dt)
  /* k is the order to which the result was approximated */
  {
     ...
     * the values are x(t+dt/2) and the derivatives dx/dt , ... d^(2k+2) x / dt^(2k+2) at the
     * midpoints
    ...
    for (int kappa = 0; kappa <= 2 * k + 1; kappa++)
    {
      ...
      // divide kappa-th derivative by kappa because we need these terms for dense output
      // interpolation
      m_algebra.for_each1(
          m_diffs[kappa][0].m_v,
          typename operations_type::template scale<time_type>(static_cast<time_type>(d)));
    ...
    // this is because we use only the midpoint theta=0 as support for the interpolation (remember
    // that theta = -1 .. 1)
  ...
  void calculate_finite_difference(size_t j, size_t kappa, value_type fac, const DerivIn & dxdt)
  {
    ...
    {
      m_algebra.for_each2(m_diffs[0][j].m_v,
                          m_derivs[j][m].m_v,
    ...
    else
    {
      ...
      m_algebra.for_each2(m_diffs[kappa][j_diffs].m_v,
                          m_derivs[j][m + kappa].m_v,
                          typename operations_type::template scale_sum1<value_type>(fac));
      value_type sign = -1.0;
      int c = 1;
      ...
      for (int i = m + static_cast<int>(kappa) - 2; i >= m - static_cast<int>(kappa); i -= 2)
      {
        ...
        {
          m_algebra.for_each3(m_diffs[kappa][j_diffs].m_v,
                              m_diffs[kappa][j_diffs].m_v,
                              dxdt,
                              typename operations_type::template scale_sum2<value_type, value_type>(
                                  1.0, sign * fac));
  ...
  void do_interpolation(time_type t, StateOut & out) const
  {
    ...
    // m_k_final is the number of order-iterations done for the last step - it governs the order of
  ...
  bool resize_impl(const StateIn & x)
  {
    ...
    for (size_t i = 0; i < m_k_max; ++i)
      resized |=
          adjust_size_by_resizeability(m_table[i], x, typename is_resizeable<state_type>::type());
  ...
  state_type & get_current_state(void) { return m_current_state_x1 ? m_x1.m_v : m_x2.m_v; }
  ...
  state_type & get_old_state(void) { return m_current_state_x1 ? m_x2.m_v : m_x1.m_v; }

  const state_type & get_old_state(void) const { return m_current_state_x1 ? m_x2.m_v : m_x1.m_v; }

  deriv_type & get_current_deriv(void) { return m_current_state_x1 ? m_dxdt1.m_v : m_dxdt2.m_v; }
  ...
  deriv_type & get_old_deriv(void) { return m_current_state_x1 ? m_dxdt2.m_v : m_dxdt1.m_v; }
  ...
  modified_midpoint_dense_out<state_type,
                              value_type,
                              deriv_type,
                              time_type,
                              algebra_type,
...
/**
 * \fn bulirsch_stoer_dense_out::initialize( const StateType &x0 , const time_type &t0 , const
 * time_type &dt0 )
