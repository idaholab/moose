...
public:
  ...
  velocity_verlet(const algebra_type & algebra = algebra_type())
    : algebra_stepper_base_type(algebra), m_first_call(true), m_a1(), m_a2(), m_current_a1(true)
  {
  }
               ...
               time_type dt)
  {
    ...
    algebra_stepper_base_type::m_algebra.for_each4(
        ...
        pin,
        ain,
        typename operations_type::template scale_sum3<value_type, time_type, time_square_type>(
            one, one * dt, one_half * dt * dt));
    ...
    algebra_stepper_base_type::m_algebra.for_each4(
        ...
        ain,
        aout,
        typename operations_type::template scale_sum3<value_type, time_type, time_type>(
            one, one_half * dt, one_half * dt));
  }
   ...
   * \param deriv The acceleration of x. The next call of `do_step` expects that the acceleration of
   * `x` passed to `do_step`
...
private:
  ...
  void do_step_v1(System system, StateInOut & x, time_type t, time_type dt)
  {
    ...
    typedef
        typename odeint::unwrap_reference<typename state_in_type::first_type>::type coor_in_type;
    typedef typename odeint::unwrap_reference<typename state_in_type::second_type>::type
        momentum_in_type;
  ...
  bool resize_impl(const StateIn & x)
  {
    bool resized = false;
    resized |=
        adjust_size_by_resizeability(m_a1, x, typename is_resizeable<acceleration_type>::type());
    resized |=
        adjust_size_by_resizeability(m_a2, x, typename is_resizeable<acceleration_type>::type());
    return resized;
  }

  acceleration_type & get_current_acc(void) { return m_current_a1 ? m_a1.m_v : m_a2.m_v; }
  ...
  acceleration_type & get_old_acc(void) { return m_current_a1 ? m_a2.m_v : m_a1.m_v; }

  const acceleration_type & get_old_acc(void) const { return m_current_a1 ? m_a2.m_v : m_a1.m_v; }

  void toggle_current_acc(void) { m_current_a1 = !m_current_a1; }
 ...
 *
 * <a href="http://en.wikipedia.org/wiki/Verlet_integration" >The Velocity-Verlet algorithm</a> is a
 * method for simulation of molecular dynamics systems. It solves the ODE
 * a=f(r,v',t)  where r are the coordinates, v are the velocities and a are the accelerations, hence
 * v = dr/dt, a=dv/dt.
 *
...
/**
 * \fn velocity_verlet::do_step( System system , CoorIn const & qin , VelocityIn const & pin ,
 * AccelerationIn const & ain , CoorOut & qout , VelocityOut & pout , AccelerationOut & aout ,
 * time_type t , time_type dt )
 * \brief This method performs one step. It transforms the result in-place. Additionally to the
 * other methods
 * the coordinates, velocities and accelerations are passed directly to do_step and they are
 * transformed out-of-place.
 *
 * It can be used like
 * \code
...
/**
 * \fn velocity_verlet::initialize( System system , const CoorIn &qin , const VelocityIn &pin ,
 * time_type t )
 * \brief Initializes the internal state of the stepper.
 *
