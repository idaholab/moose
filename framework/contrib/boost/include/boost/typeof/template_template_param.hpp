...
#define BOOST_TYPEOF_TEMPLATE_PARAM(Params) (TEMPLATE_PARAM)(Params)
...
#define BOOST_TYPEOF_TEMPLATE_PARAM_ENCODE(This, n)                                                \
  typedef typename boost::type_of::encode_template<                                                \
      BOOST_PP_CAT(V, n),                                                                          \
      BOOST_PP_CAT(P, n) < BOOST_TYPEOF_SEQ_ENUM(BOOST_TYPEOF_MAKE_OBJS(                           \
                                                     BOOST_TYPEOF_TEMPLATE_PARAM_GETPARAMS(This)), \
                                                 BOOST_TYPEOF_PLACEHOLDER)>>                       \
...
#define BOOST_TYPEOF_TEMPLATE_PARAM_PLACEHOLDER_TYPES(Param, n)                                    \
  BOOST_PP_CAT(T, n)<BOOST_TYPEOF_SEQ_ENUM_1(                                                      \
      BOOST_TYPEOF_MAKE_OBJS(BOOST_TYPEOF_TEMPLATE_PARAM_GETPARAMS(Param)),                        \
      BOOST_TYPEOF_PLACEHOLDER)>
...
struct encode_template : BOOST_TYPEOF_ENCODE_NS_QUALIFIER::encode_template_impl<V, T>
{
};

template <class Iter>
struct decode_template : BOOST_TYPEOF_ENCODE_NS_QUALIFIER::decode_template_impl<typename Iter::type,
                                                                                typename Iter::next>
{
};
}
...
#define BOOST_TYPEOF_REGISTER_TYPE_FOR_TEMPLATE_TEMPLATE(Name, Params, ID)                         \
  template <class V BOOST_TYPEOF_SEQ_ENUM_TRAILING(Params,                                         \
                                                   BOOST_TYPEOF_REGISTER_TEMPLATE_PARAM_PAIR)>     \
  struct encode_template_impl<V, Name<BOOST_PP_ENUM_PARAMS(BOOST_PP_SEQ_SIZE(Params), P)>>         \
...
#ifdef __BORLANDC__
#define BOOST_TYPEOF_DECODE_NESTED_TEMPLATE_HELPER_NAME                                            \
  BOOST_PP_CAT(                                                                                    \
      BOOST_PP_CAT(BOOST_PP_CAT(decode_nested_template_helper, BOOST_TYPEOF_REGISTRATION_GROUP),   \
                   0x10000),                                                                       \
      __LINE__)
#define BOOST_TYPEOF_REGISTER_DECODE_NESTED_TEMPLATE_HELPER_IMPL(Name, Params, ID)                 \
  ...
  {                                                                                                \
    template <BOOST_TYPEOF_SEQ_ENUM(Params,                                                        \
                                    BOOST_TYPEOF_REGISTER_DECLARE_DECODER_TYPE_PARAM_PAIR)>        \
...
#define BOOST_TYPEOF_TYPEDEF_DECODED_TEMPLATE_TEMPLATE_TYPE(Name, Params)                          \
  typedef typename BOOST_TYPEOF_DECODE_NESTED_TEMPLATE_HELPER_NAME::decode_params<                 \
      BOOST_PP_ENUM_PARAMS(BOOST_PP_SEQ_SIZE(Params), P)>::type type;
...
#define BOOST_TYPEOF_REGISTER_TEMPLATE_TEMPLATE_IMPL(Name, Params, ID)                             \
  BOOST_PP_IF(BOOST_TYPEOF_HAS_TEMPLATES(Params),                                                  \
              ...
              BOOST_TYPEOF_REGISTER_TYPE_FOR_TEMPLATE_TEMPLATE)                                    \
  (Name, Params, ID)
