...
#else
template <class V, class T>
typename enable_if<typename is_function<T>::type, sizer<typename encode_type<V, T>::type>>::type
encode(T &);

template <class V, class T>
typename disable_if<typename is_function<T>::type, sizer<typename encode_type<V, T>::type>>::type
encode(const T &);
#endif
}
}
...
#define BOOST_TYPEOF_ENCODED_VECTOR(Expr)                                                          \
  BOOST_TYPEOF_VECTOR(BOOST_TYPEOF_LIMIT_SIZE)<BOOST_PP_ENUM(                                      \
      BOOST_TYPEOF_LIMIT_SIZE, BOOST_TYPEOF_TYPEITEM, Expr)>
...
#define BOOST_TYPEOF_NESTED_TYPEDEF_IMPL(expr)                                                     \
  ...
  {                                                                                                \
    ...
    typedef boost::type_of::offset_vector<BOOST_TYPEOF_VECTOR(0) <>,                               \
        boost::mpl::size_t<self_t::_typeof_encode_offset>> _typeof_start_vector;                   \
...
#define BOOST_TYPEOF_NESTED_TYPEDEF_IMPL(expr)                                                     \
  ...
  {                                                                                                \
    ...
    typedef typename boost::type_of::v_iter<fraction_type, boost::mpl::int_<self_t::where>>::type  \
        type;                                                                                      \
...
struct BOOST_PP_CAT(_typeof_template_, name)                                                       \
  {                                                                                                \
    ...
    typedef                                                                                        \
        typename boost::type_of::decode_type<_typeof_fraction_iter<boost::mpl::size_t<0>>>::type   \
            type;                                                                                  \
  \
};                                                                                                 \
  \
typedef BOOST_PP_CAT(_typeof_template_, name)<int>                                                 \
      name;
