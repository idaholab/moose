...
#define BOOST_TYPEOF_MAKE_OBJ(elem)                                                                \
  BOOST_PP_CAT(BOOST_TYPEOF_MAKE_OBJ, BOOST_PP_SEQ_CAT((_)BOOST_TYPEOF_TO_SEQ(elem)))
...
#define BOOST_TYPEOF_REGISTER_TEMPLATE_EXPLICIT_ID(Name, Params, Id)                               \
  BOOST_TYPEOF_REGISTER_TEMPLATE_IMPL(Name,                                                        \
...
#define BOOST_TYPEOF_TOSEQ(x)                                                                      \
  BOOST_PP_IIF(BOOST_PP_IS_UNARY(x), x BOOST_PP_TUPLE_EAT(3), BOOST_PP_REPEAT)                     \
...
// Since we are creating an internal decode struct, we need to use different template names, T
// instead of P.
...
#define BOOST_TYPEOF_TYPEDEF_DECODED_TYPE(Name, Params)                                            \
  BOOST_PP_IF(BOOST_TYPEOF_HAS_TEMPLATES(Params),                                                  \
              ...
              BOOST_TYPEOF_TYPEDEF_DECODED_TEMPLATE_TYPE)                                          \
  (Name, Params)
