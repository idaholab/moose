...
template <class T>
inline T
asymptotic_bessel_amplitude(T v, T x)
{
...
template <class T>
T
asymptotic_bessel_phase_mx(T v, T x)
{
  //
...
template <class T>
inline T
asymptotic_bessel_y_large_x_2(T v, T x)
{
  // See A&S 9.2.19.
  BOOST_MATH_STD_USING
...
template <class T>
inline T
asymptotic_bessel_j_large_x_2(T v, T x)
{
  // See A&S 9.2.19.
  BOOST_MATH_STD_USING
  // Get the phase and amplitude:
...
template <class T>
inline bool
asymptotic_bessel_large_x_limit(int v, const T & x)
{
  BOOST_MATH_STD_USING
  //
...
template <class T>
inline bool
asymptotic_bessel_large_x_limit(const T & v, const T & x)
{
  BOOST_MATH_STD_USING
  //
  // Determines if x is large enough compared to v to take the asymptotic
...
void
temme_asyptotic_y_small_x(T v, T x, T * Y, T * Y1, const Policy & pol)
{
  T c = 1;
...
template <class T, class Policy>
T
asymptotic_bessel_i_large_x(T v, T x, const Policy & pol)
{
  ...
  return (boost::math::isfinite)(s)
             ...
             : policies::raise_overflow_error<T>(
                   "boost::math::asymptotic_bessel_i_large_x<%1%>(%1%,%1%)", 0, pol);
}
}
}
} // namespaces
