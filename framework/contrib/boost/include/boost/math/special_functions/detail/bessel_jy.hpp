...
bool
hankel_PQ(T v, T x, T * p, T * q, const Policy &)
{
...
int
temme_jy(T v, T x, T * Y, T * Y1, const Policy & pol)
{
  ...
  e = abs(v) < tools::epsilon<T>() ? T(v * pi<T>() * pi<T>() / 2) : T(2 * spv2 * spv2 / v);
...
template <typename T, typename Policy>
int
CF1_jy(T v, T x, T * fv, int * sign, const Policy & pol)
{
...
template <typename T, typename Policy>
int
CF2_jy(T v, T x, T * p, T * q, const Policy & pol)
{
  BOOST_MATH_STD_USING
...
template <typename T, typename Policy>
int
bessel_jy(T v, T x, T * J, T * Y, int kind, const Policy & pol)
{
  BOOST_ASSERT(x >= 0);
  ...
  if (v > static_cast<T>((std::numeric_limits<int>::max)()))
  {
    *J = *Y = policies::raise_evaluation_error<T>(
        function, "Order of Bessel function is too large to evaluate: got %1%", v, pol);
    return 1;
  }
  n = iround(v, pol);
  u = v - n; // -1/2 <= u < 1/2
  ...
  if (x == 0)
  {
    if (v == 0)
      ...
      *J = policies::raise_domain_error<T>(function,
                                           "Value of Bessel J_v(x) is complex-infinity at %1%",
                                           x,
                                           pol); // complex infinity
    else
    ...
    if ((kind & need_y) == 0)
      ...
      *Y = policies::raise_domain_error<T>(function,
                                           "Value of Bessel Y_v(x) is complex-infinity at %1%",
                                           x,
                                           pol); // complex infinity
    return 1;
  }
  ...
  if (((kind & need_y) == 0) && ((x < 1) || (v > x * x / 4) || (x < 5)))
  {
    ...
    Yv = std::numeric_limits<T>::quiet_NaN();
           ...
           (log(policies::get_epsilon<T, Policy>() / 2) > v * log((x / 2) * (x / 2) / v)))
  {
  ...
  if (reflect)
  {
    if ((sp != 0) && (tools::max_value<T>() * fabs(Yv_scale) < fabs(sp * Yv)))
      *J = org_kind & need_j ? T(-sign(sp) * sign(Yv) * sign(Yv_scale) *
    ...
    if ((cp != 0) && (tools::max_value<T>() * fabs(Yv_scale) < fabs(cp * Yv)))
      *Y = org_kind & need_y ? T(-sign(cp) * sign(Yv) * sign(Yv_scale) *
                                 policies::raise_overflow_error<T>(function, 0, pol))
                             : T(0);
    else
  ...
  else
  {
    *J = Jv;
    if (tools::max_value<T>() * fabs(Yv_scale) < fabs(Yv))
      *Y = org_kind & need_y
               ? T(sign(Yv) * sign(Yv_scale) * policies::raise_overflow_error<T>(function, 0, pol))
               : T(0);
    else
      *Y = Yv / Yv_scale;
  }
