...
T
erf_inv_imp(const T & p, const T & q, const Policy &, const boost::mpl::int_<64> *)
{
  ...
  if (p <= 0.5)
  {
    ...
    static const T P[] = {BOOST_MATH_BIG_CONSTANT(T, 64, -0.000508781949658280665617),
    ...
    static const T Q[] = {BOOST_MATH_BIG_CONSTANT(T, 64, 1.0),
  ...
  else if (q >= 0.25)
  {
    ...
    static const T P[] = {BOOST_MATH_BIG_CONSTANT(T, 64, -0.202433508355938759655),
  ...
  else
  {
    ...
    if (x < 3)
    {
      ...
      static const T P[] = {BOOST_MATH_BIG_CONSTANT(T, 64, -0.131102781679951906451),
    ...
    else if (x < 6)
    {
      ...
      static const T P[] = {BOOST_MATH_BIG_CONSTANT(T, 64, -0.0350353787183177984712),
    ...
    else if (x < 18)
    {
      ...
      static const T P[] = {BOOST_MATH_BIG_CONSTANT(T, 64, -0.0167431005076633737133),
    ...
    else if (x < 44)
    {
      ...
      static const T P[] = {BOOST_MATH_BIG_CONSTANT(T, 64, -0.0024978212791898131227),
    ...
    else
    {
      ...
      static const T P[] = {BOOST_MATH_BIG_CONSTANT(T, 64, -0.000539042911019078575891),
...
struct erf_roots
{
  boost::math::tuple<T, T, T> operator()(const T & guess)
  {
    ...
    return boost::math::make_tuple(
        ((sign > 0) ? static_cast<T>(boost::math::erf(guess, Policy()) - target)
                    : static_cast<T>(boost::math::erfc(guess, Policy())) - target),
        derivative,
        derivative2);
  }
...
template <class T, class Policy>
T
erf_inv_imp(const T & p, const T & q, const Policy & pol, const boost::mpl::int_<0> *)
{
  //
  ...
  if (policies::digits<T, Policy>() > 64)
  {
    ...
    if (p <= 0.5)
    {
      result = tools::halley_iterate(detail::erf_roots<typename remove_cv<T>::type, Policy>(p, 1),
                                     guess,
                                     static_cast<T>(0),
                                     tools::max_value<T>(),
                                     (policies::digits<T, Policy>() * 2) / 3,
                                     max_iter);
    }
    else
    {
      result = tools::halley_iterate(detail::erf_roots<typename remove_cv<T>::type, Policy>(q, -1),
                                     guess,
                                     static_cast<T>(0),
                                     tools::max_value<T>(),
                                     (policies::digits<T, Policy>() * 2) / 3,
...
template <class T, class Policy>
bool
erf_inv_initializer<T, Policy>::init::is_value_non_zero(T v)
{
...
erfc_inv(T z, const Policy & pol)
{
  ...
  if ((z < 0) || (z > 2))
    return policies::raise_domain_error<result_type>(
        function, "Argument outside range [0,2] in inverse erfc function (got p=%1%).", z, pol);
  if (z == 0)
  ...
  return s * policies::checked_narrowing_cast<result_type, forwarding_policy>(
                 detail::erf_inv_imp(static_cast<eval_type>(p),
                                     static_cast<eval_type>(q),
                                     forwarding_policy(),
...
erf_inv(T z, const Policy & pol)
{
  ...
  if ((z < -1) || (z > 1))
    return policies::raise_domain_error<result_type>(
        function, "Argument outside range [-1, 1] in inverse erf function (got p=%1%).", z, pol);
  if (z == 1)
    return policies::raise_overflow_error<result_type>(function, 0, pol);
  if (z == -1)
    return -policies::raise_overflow_error<result_type>(function, 0, pol);
