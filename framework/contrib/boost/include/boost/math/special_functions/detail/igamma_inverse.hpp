...
find_inverse_s(T p, T q)
{
  ...
  static const double a[4] = {
      3.31125922108741, 11.6616720288968, 4.28342155967104, 0.213623493715853};
  static const double b[5] = {
      1, 6.61053765625462, 6.40691597760039, 1.27364489782223, 0.3611708101884203e-1};
...
template <class T>
T
didonato_SN(T a, T x, unsigned N, T tolerance = 0)
{
  //
  // Computation of the Incomplete Gamma Function Ratios and their Inverse
  // ARMIDO R. DIDONATO and ALFRED H. MORRIS, JR.
...
inline T
didonato_FN(T p, T a, T x, unsigned N, T tolerance, const Policy & pol)
{
  //
  // Computation of the Incomplete Gamma Function Ratios and their Inverse
  // ARMIDO R. DIDONATO and ALFRED H. MORRIS, JR.
...
template <class T, class Policy>
T
find_inverse_gamma(T a, T p, T q, const Policy & pol, bool * p_has_10_digits)
{
  //
  ...
  *p_has_10_digits = false;
  ...
  else if (a < 1)
  {
    ...
    else
    {
      ...
      T c4 = (a - 1) * ((c1_3 / 3) - (3 * a - 5) * c1_2 / 2 + (a_2 - 6 * a + 7) * c1 +
                        (11 * a_2 - 46 * a + 47) / 6);
      T c5 = (a - 1) * (-(c1_4 / 4) + (11 * a - 17) * c1_3 / 6 + (-3 * a_2 + 13 * a - 13) * c1_2 +
...
struct gamma_p_inverse_func
{
  ...
  boost::math::tuple<T, T, T> operator()(const T & x) const
  {
    ...
    BOOST_MATH_STD_USING // For ADL of std functions.
        ...
        f1;
    value_type ft;
    f = static_cast<T>(boost::math::detail::gamma_incomplete_imp(static_cast<value_type>(a),
...
template <class T, class Policy>
T
gamma_p_inv_imp(T a, T p, const Policy & pol)
{
  ...
  if (a <= 0)
    return policies::raise_domain_error<T>(
        function,
        "Argument a in the incomplete gamma function inverse must be >= 0 (got a=%1%).",
        a,
        pol);
  if ((p < 0) || (p > 1))
    return policies::raise_domain_error<T>(function,
                                           "Probabilty must be in the range [0,1] in the "
                                           "incomplete gamma function inverse (got p=%1%).",
                                           p,
                                           pol);
  if (p == 1)
  ...
  guess = tools::halley_iterate(detail::gamma_p_inverse_func<T, Policy>(a, p, false),
                                guess,
                                lower,
                                tools::max_value<T>(),
                                digits,
  ...
  BOOST_MATH_INSTRUMENT_VARIABLE(guess);
  if (guess == lower)
    guess = policies::raise_underflow_error<T>(
        function,
        "Expected result known to be non-zero, but is smaller than the smallest available number.",
        pol);
  return guess;
}
...
template <class T, class Policy>
T
gamma_q_inv_imp(T a, T q, const Policy & pol)
{
  BOOST_MATH_STD_USING // ADL of std functions.
  ...
  if ((q < 0) || (q > 1))
    return policies::raise_domain_error<T>(function,
                                           "Probabilty must be in the range [0,1] in the "
                                           "incomplete gamma function inverse (got q=%1%).",
                                           q,
                                           pol);
  if (q == 0)
    return policies::raise_overflow_error<T>(function, 0, Policy());
  ...
  //
  boost::uintmax_t max_iter = policies::get_max_root_iterations<Policy>();
  guess = tools::halley_iterate(detail::gamma_p_inverse_func<T, Policy>(a, q, true),
                                guess,
                                lower,
                                tools::max_value<T>(),
                                digits,
...
gamma_p_inv(T1 a, T2 p, const Policy & pol)
{
  ...
  return detail::gamma_p_inv_imp(static_cast<result_type>(a), static_cast<result_type>(p), pol);
}
...
{
  typedef typename tools::promote_args<T1, T2>::type result_type;
  return detail::gamma_q_inv_imp(static_cast<result_type>(a), static_cast<result_type>(p), pol);
}
