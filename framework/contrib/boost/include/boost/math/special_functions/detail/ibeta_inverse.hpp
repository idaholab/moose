...
T
temme_method_1_ibeta_inverse(T a, T b, T z, const Policy & pol)
{
  BOOST_MATH_STD_USING // ADL of std names
...
template <class T, class Policy>
T
temme_method_2_ibeta_inverse(T /*a*/, T /*b*/, T z, T r, T theta, const Policy & pol)
{
  BOOST_MATH_STD_USING // ADL of std names
  ...
  static const BOOST_MATH_INT_TABLE_TYPE(T, int)
...
template <class T, class Policy>
T
temme_method_3_ibeta_inverse(T a, T b, T p, T q, const Policy & pol)
{
  BOOST_MATH_STD_USING // ADL of std names
  ...
  e1 -= (75 * w_6 + 202 * w_5 + 188 * w_4 - 888 * w_3 - 1345 * w_2 + 118 * w + 138) * d_4 /
        (272160 * w1_4 * w_5);

  T e2 = (28 * w_4 + 131 * w_3 + 402 * w_2 + 581 * w + 208) * (w - 1) / (1620 * w1 * w_3);
  e2 -= (35 * w_6 - 154 * w_5 - 623 * w_4 - 1636 * w_3 - 3983 * w_2 - 3514 * w - 925) * d /
        (12960 * w1_2 * w_4);
  e2 -= (2132 * w_7 + 7915 * w_6 + 16821 * w_5 + 35066 * w_4 + 87490 * w_3 + 141183 * w_2 +
         95993 * w + 21640) *
        d_2 / (816480 * w_5 * w1_3);
  e2 -= (11053 * w_8 + 53308 * w_7 + 117010 * w_6 + 163924 * w_5 + 116188 * w_4 - 258428 * w_3 -
         677042 * w_2 - 481940 * w - 105497) *
        d_3 / (14696640 * w1_4 * w_6);

  T e3 = -((3592 * w_7 + 8375 * w_6 - 1323 * w_5 - 29198 * w_4 - 89578 * w_3 - 154413 * w_2 -
            116063 * w - 29632) *
           (w - 1)) /
         (816480 * w_5 * w1_2);
  e3 -= (442043 * w_9 + 2054169 * w_8 + 3803094 * w_7 + 3470754 * w_6 + 2141568 * w_5 -
         2393568 * w_4 - 19904934 * w_3 - 34714674 * w_2 - 23128299 * w - 5253353) *
        d / (146966400 * w_6 * w1_3);
  e3 -=
      (116932 * w_10 + 819281 * w_9 + 2378172 * w_8 + 4341330 * w_7 + 6806004 * w_6 +
       10622748 * w_5 + 18739500 * w_4 + 30651894 * w_3 + 30869976 * w_2 + 15431867 * w + 2919016) *
      d_2 / (146966400 * w1_4 * w_7);
  //
...
struct ibeta_roots
{
  ibeta_roots(T _a, T _b, T t, bool inv = false) : a(_a), b(_b), target(t), invert(inv) {}
...
template <class T, class Policy>
T
ibeta_inv_imp(T a, T b, T p, T q, const Policy & pol, T * py)
{
  ...
  else if ((a > 1) && (b > 1))
  {
    ...
    if ((b < a) && (x < 0.2))
    {
      ...
      ap1 *= (a + 1);
      terms[4] = bm1 * (33 * a * b_2 + 31 * b_2 + 8 * a_2 * b_2 - 30 * a * b - 47 * b +
  ...
  x = boost::math::tools::halley_iterate(
      boost::math::detail::ibeta_roots<T, Policy>(a, b, (p < q ? p : q), (p < q ? false : true)),
      x,
      lower,
      upper,
      digits,
  ...
  // BOOST_ASSERT((x != lower) || (x == boost::math::tools::min_value<T>()) || (x ==
...
ibeta_inv(T1 a, T2 b, T3 p, T4 * py, const Policy & pol)
{
  ...
  if (a <= 0)
    return policies::raise_domain_error<result_type>(function,
                                                     "The argument a to the incomplete beta "
                                                     "function inverse must be greater than zero "
                                                     "(got a=%1%).",
                                                     a,
                                                     pol);
  if (b <= 0)
    return policies::raise_domain_error<result_type>(function,
                                                     "The argument b to the incomplete beta "
                                                     "function inverse must be greater than zero "
                                                     "(got b=%1%).",
                                                     b,
                                                     pol);
  if ((p < 0) || (p > 1))
    return policies::raise_domain_error<result_type>(
        function,
        "Argument p outside the range [0,1] in the incomplete beta function inverse (got p=%1%).",
        p,
        pol);

  value_type rx, ry;

  rx = detail::ibeta_inv_imp(static_cast<value_type>(a),
...
ibetac_inv(T1 a, T2 b, T3 q, T4 * py, const Policy & pol)
{
  ...
  if ((q < 0) || (q > 1))
    return policies::raise_domain_error<result_type>(
        function,
        "Argument q outside the range [0,1] in the incomplete beta function inverse (got q=%1%).",
        q,
        pol);
