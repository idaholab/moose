...
struct bessel_k0_initializer
{
  ...
  {
    init() { do_init(); }
    static void do_init() { bessel_k0(T(1), Policy()); }
...
template <typename T, typename Policy>
T
bessel_k0(T x, const Policy & pol)
{
  BOOST_MATH_INSTRUMENT_CODE(x);

  bessel_k0_initializer<T, Policy>::force_instantiate();

  static const T P1[] = {static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 2.4708152720399552679e+03)),
  ...
  static const T Q1[] = {static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 2.1312714303849120380e+04)),
  ...
  static const T Q2[] = {static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, -1.6128136304458193998e+06)),
                         ...
                         static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1.0))};
  static const T P3[] = {static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1.1600249425076035558e+02)),
  ...
  static const T Q3[] = {static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 9.2556599177304839811e+01)),
  ...
  if (x < 0)
  {
    return policies::raise_domain_error<T>(
        function,
        "Got x = %1%, but argument x must be non-negative, complex number result not supported",
        x,
        pol);
  }
  if (x == 0)
