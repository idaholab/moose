...
T
inverse_students_t_hill(T ndf, T u, const Policy & pol)
{
  ...
  {
    y = static_cast<T>(
        ((1 / (((ndf + 6) / (ndf * y) - 0.089f * d - 0.822f) * (ndf + 2) * 3) + 0.5 / (ndf + 4)) *
             y -
         1) *
            (ndf + 1) / (ndf + 2) +
        1 / y);
...
template <class T, class Policy>
T
inverse_students_t_tail_series(T df, T v, const Policy & pol)
{
  BOOST_MATH_STD_USING
  ...
  np6 *= (df + 6);
  d[6] = -df * (df + 1) * (df + 11) *
         ((((((((((((945 * df) + 31506) * df + 425858) * df + 2980236) * df + 11266745) * df +
                20675018) *
                   df +
               7747124) *
                  df -
...
template <class T, class Policy>
T
inverse_students_t_body_series(T df, T u, const Policy & pol)
{
  BOOST_MATH_STD_USING
  //
  ...
  c[3] = static_cast<T>((0.0083333333333333333333 * in + 0.066666666666666666667) * in +
                        0.058333333333333333333);
  c[4] = static_cast<T>(
      ((0.00019841269841269841270 * in + 0.0017857142857142857143) * in + 0.026785714285714285714) *
          in +
      0.025198412698412698413);
  c[5] = static_cast<T>((((2.7557319223985890653e-6 * in + 0.00037477954144620811287) * in -
                          0.0011078042328042328042) *
                             in +
                         0.010559964726631393298) *
                            in +
                        0.012039792768959435626);
  c[6] = static_cast<T>(((((2.5052108385441718775e-8 * in - 0.000062705427288760622094) * in +
  ...
  c[7] = static_cast<T>((((((1.6059043836821614599e-10 * in + 0.000015401265401265401265) * in -
  ...
  c[8] = static_cast<T>(((((((7.6471637318198164759e-13 * in - 3.9851014346715404916e-6) * in +
  ...
  c[9] = static_cast<T>((((((((2.8114572543455207632e-15 * in + 1.0914179173496789432e-6) * in -
  ...
  c[10] = static_cast<T>(((((((((8.2206352466243297170e-18 * in - 3.1239569599829868045e-7) * in +
...
template <class T, class Policy>
T
inverse_students_t(T df, T u, T v, const Policy & pol, bool * pexact = 0)
{
  //
  ...
  calculate_real:
    ...
    else
    {
      ...
      T crossover = ldexp(
          ...
          iround(T(df / -0.654f),
                 typename policies::
                     normalise<Policy, policies::rounding_error<policies::ignore_error>>::type()));
      if (u > crossover)
      {
...
template <class T, class Policy>
inline T
find_ibeta_inv_from_t_dist(T a, T p, T /*q*/, T * py, const Policy & pol)
{
  T u = p / 2;
  T v = 1 - u;
  T df = a * 2;
...
template <class T, class Policy>
inline T
fast_students_t_quantile_imp(T df, T p, const Policy & pol, const mpl::false_ *)
{
  BOOST_MATH_STD_USING
  //
...
template <class T, class Policy>
T
fast_students_t_quantile_imp(T df, T p, const Policy & pol, const mpl::true_ *)
{
  BOOST_MATH_STD_USING
  bool invert = false;
  if ((df < 2) && (floor(df) != df))
    return boost::math::detail::fast_students_t_quantile_imp(
        df, p, pol, static_cast<mpl::false_ *>(0));
  if (p > 0.5)
  {
    p = 1 - p;
    invert = true;
...
template <class T, class Policy>
inline T
fast_students_t_quantile(T df, T p, const Policy & pol)
{
  ...
  typedef mpl::bool_<(std::numeric_limits<T>::digits <= 53) &&
                     (std::numeric_limits<T>::is_specialized) &&
                     (std::numeric_limits<T>::radix == 2)>
      tag_type;
  return policies::checked_narrowing_cast<T, forwarding_policy>(
      fast_students_t_quantile_imp(
          static_cast<value_type>(df), static_cast<value_type>(p), pol, static_cast<tag_type *>(0)),
      "boost::math::students_t_quantile<%1%>(%1%,%1%,%1%)");
}
}
}
} // namespaces
