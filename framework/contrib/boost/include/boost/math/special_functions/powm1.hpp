...
inline T
powm1_imp(const T x, const T y, const Policy & pol)
{
  ...
  else
  {
    ...
    if (boost::math::trunc(y) != y)
      return boost::math::policies::raise_domain_error<T>(
          function, "For non-integral exponent, expected base > 0 but got %1%", x, pol);
...
powm1(const T1 a, const T2 z)
{
  ...
  return detail::powm1_imp(
      static_cast<result_type>(a), static_cast<result_type>(z), policies::policy<>());
}
