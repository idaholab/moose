...
template <class T>
inline bool
is_odd(T v, const boost::true_type &)
{
...
template <class T>
inline bool
is_odd(T v, const boost::false_type &)
{
...
T
gamma_imp(T z, const Policy & pol, const Lanczos & l)
{
  BOOST_MATH_STD_USING
  ...
  if (z <= 0)
  {
    if (floor(z) == z)
      return policies::raise_pole_error<T>(
          function, "Evaluation of tgamma at a negative integer %1%.", z, pol);
    if (z <= -20)
    {
      result = gamma_imp(T(-z), pol, l) * sinpx(z);
      BOOST_MATH_INSTRUMENT_VARIABLE(result);
      if ((fabs(result) < 1) &&
          (tools::max_value<T>() * fabs(result) < boost::math::constants::pi<T>()))
...
template <class T, class Policy, class Lanczos>
T
lgamma_imp(T z, const Policy & pol, const Lanczos & l, int * sign = 0)
{
#ifdef BOOST_MATH_INSTRUMENT
  static bool b = false;
  if (!b)
  ...
  if (z <= -tools::root_epsilon<T>())
  {
    ...
    if (floor(z) == z)
      return policies::raise_pole_error<T>(
          function, "Evaluation of lgamma at a negative integer %1%.", z, pol);
  ...
  else if (z < 15)
  {
    ...
    typedef typename mpl::if_<
        ...
        mpl::int_<64>,
        typename mpl::if_<mpl::and_<mpl::less_equal<precision_type, mpl::int_<113>>,
                                    mpl::greater<precision_type, mpl::int_<0>>>,
                          mpl::int_<113>,
                          mpl::int_<0>>::type>::type tag_type;
...
template <class T>
inline T
upper_gamma_fraction(T a, T z, T eps)
{
...
inline T
lower_gamma_series(T a, T z, const Policy & pol, T init_value = 0)
{
...
template <class T, class Policy>
T
gamma_imp(T z, const Policy & pol, const lanczos::undefined_lanczos &)
{
  BOOST_MATH_STD_USING
  ...
  if (zz < tools::cbrt_epsilon<T>())
  {
    ...
    const T six_euler_squared(
        (boost::math::constants::euler<T>() * boost::math::constants::euler<T>()) * 6);
  ...
  if (b_neg)
  {
    ...
    if ((boost::math::fpclassify)(gamma_value) == static_cast<int>(FP_SUBNORMAL))
      return policies::raise_denorm_error<T>(
          function, "Result of tgamma is denormalized.", gamma_value, pol);
  }
...
template <class T, class Policy>
T
lgamma_imp(T z, const Policy & pol, const lanczos::undefined_lanczos &, int * sign)
{
  BOOST_MATH_STD_USING
  ...
  else
  {
    ...
    for (std::size_t n = 2U; n < number_of_bernoullis_b2n; ++n)
    {
      ...
      const T term =
          (boost::math::bernoulli_b2n<T>(static_cast<int>(n)) * one_over_x_pow_two_n_minus_one) /
          (n2 * (n2 - 1U));
    ...
    log_gamma_value =
        ((((zz - boost::math::constants::half<T>()) * log(zz)) - zz) + half_ln_two_pi) + sum;
  }
...
template <class T, class Policy, class Lanczos>
T
tgammap1m1_imp(T dz, Policy const & pol, const Lanczos & l)
{
  BOOST_MATH_STD_USING
  ...
  typedef typename mpl::if_<mpl::or_<mpl::less_equal<precision_type, mpl::int_<0>>,
                            ...
                            typename mpl::if_<is_same<Lanczos, lanczos::lanczos24m113>,
                                              mpl::int_<113>,
                                              mpl::int_<0>>::type,
                            typename mpl::if_<mpl::less_equal<precision_type, mpl::int_<64>>,
                                              mpl::int_<64>,
                                              mpl::int_<113>>::type>::type tag_type;
...
template <class T, class Policy>
inline T
tgammap1m1_imp(T dz, Policy const & pol, const ::boost::math::lanczos::undefined_lanczos & l)
{
  ...
  BOOST_MATH_INSTRUMENT_CODE(
      (ldexp(1.0, boost::math::policies::digits<T, Policy>()) * fabs(result) < 1e34));
  if ((dz > -0.5) && (dz < 2) &&
      (ldexp(1.0, boost::math::policies::digits<T, Policy>()) * fabs(result) < 1e34))
  {
...
template <class T, class Policy>
T
full_igamma_prefix(T a, T z, const Policy & pol)
{
  BOOST_MATH_STD_USING
...
template <class T, class Policy, class Lanczos>
T
regularised_gamma_prefix(T a, T z, const Policy & pol, const Lanczos & l)
{
  BOOST_MATH_STD_USING
        ...
        ((std::max)(alz, amz) >= tools::log_max_value<T>()))
    {
          ...
          ((std::max)(alz, amz) / 2 < tools::log_max_value<T>()))
      {
        ...
        prefix = sq * sq;
               ...
               ((std::max)(alz, amz) / 4 < tools::log_max_value<T>()) && (z > a))
      {
...
template <class T, class Policy>
T
regularised_gamma_prefix(T a, T z, const Policy & pol, const lanczos::undefined_lanczos &)
{
  BOOST_MATH_STD_USING
...
tgamma_small_upper_part(
    T a, T x, const Policy & pol, T * pgam = 0, bool invert = false, T * pderivative = 0)
{
...
template <class T, class Policy>
inline T
finite_gamma_q(T a, T x, Policy const & pol, T * pderivative = 0)
{
  //
...
template <class T, class Policy>
T
finite_half_gamma_q(T a, T x, T * p_derivative, const Policy & pol)
{
  //
...
gamma_incomplete_imp(T a, T x, bool normalised, bool invert, const Policy & pol, T * p_derivative)
{
  ...
  if (x < 0)
    return policies::raise_domain_error<T>(
        function, "Argument x to the incomplete gamma function must be >= 0 (got x=%1%).", x, pol);
  ...
  T result =
      0; // Just to avoid warning C4701: potentially uninitialized local variable 'result' used
    ...
    else
    {
      ...
      if (result == 0)
      {
        if (invert)
        {
          ...
          result =
              log(result) - a + (a - 0.5f) * log(a) + log(boost::math::constants::root_two_pi<T>());
          if (p_derivative)
  ...
  switch (eval_method)
  {
    ...
    {
      // Compute P:
      result = normalised ? regularised_gamma_prefix(a, x, pol, lanczos_type())
    ...
    case 5:
    {
      ...
      typedef typename mpl::if_<
          mpl::or_<mpl::equal_to<precision_type, mpl::int_<0>>,
                   mpl::greater<precision_type, mpl::int_<113>>>,
          mpl::int_<0>,
          typename mpl::if_<mpl::less_equal<precision_type, mpl::int_<53>>,
                            mpl::int_<53>,
                            typename mpl::if_<mpl::less_equal<precision_type, mpl::int_<64>>,
                                              mpl::int_<64>,
...
template <class T, class Policy, class Lanczos>
T
tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy & pol, const Lanczos & l)
{
  BOOST_MATH_STD_USING
...
template <class T, class Policy>
T
tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy & pol, const lanczos::undefined_lanczos &)
{
  BOOST_MATH_STD_USING
  //
  ...
  if (fabs(tools::max_value<T>() / prefix) < fabs(sum))
    return policies::raise_overflow_error<T>("boost::math::tgamma_delta_ratio<%1%>(%1%, %1%)",
...
template <class T, class Policy>
T
tgamma_delta_ratio_imp(T z, T delta, const Policy & pol)
{
  BOOST_MATH_STD_USING
    ...
    if (floor(z) == z)
    {
      ...
      if ((z <= max_factorial<T>::value) && (z + delta <= max_factorial<T>::value))
      {
        return unchecked_factorial<T>((unsigned)itrunc(z, pol) - 1) /
               unchecked_factorial<T>((unsigned)itrunc(T(z + delta), pol) - 1);
      }
    }
    if (fabs(delta) < 20)
    {
...
template <class T, class Policy>
T
tgamma_ratio_imp(T x, T y, const Policy & pol)
{
  BOOST_MATH_STD_USING
  ...
  if ((x <= 0) || (boost::math::isinf)(x))
    return policies::raise_domain_error<T>(
        ...
        "Gamma function ratios only implemented for positive arguments (got a=%1%).",
        x,
        pol);
  if ((y <= 0) || (boost::math::isinf)(y))
    return policies::raise_domain_error<T>(
        "boost::math::tgamma_ratio<%1%>(%1%, %1%)",
        "Gamma function ratios only implemented for positive arguments (got b=%1%).",
        y,
        pol);
...
template <class T, class Policy>
T
gamma_p_derivative_imp(T a, T x, const Policy & pol)
{
  BOOST_MATH_STD_USING
  //
  ...
  {
    return (a > 1) ? 0
                   : (a == 1) ? 1 : policies::raise_overflow_error<T>(
...
tgamma(T z, const Policy & /* pol */, const mpl::true_)
{
  ...
  return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::gamma_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type()),
...
lgamma(T z, int * sign, const Policy &)
{
  ...
  return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::lgamma_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type(), sign),
...
tgamma1pm1(T z, const Policy & /* pol */)
{
  ...
  return policies::checked_narrowing_cast<typename remove_cv<result_type>::type, forwarding_policy>(
      detail::tgammap1m1_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type()),
...
tgamma_delta_ratio(T1 z, T2 delta, const Policy & /* pol */)
{
  ...
  return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::tgamma_delta_ratio_imp(
          static_cast<value_type>(z), static_cast<value_type>(delta), forwarding_policy()),
...
tgamma_ratio(T1 a, T2 b, const Policy &)
{
  ...
  return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::tgamma_ratio_imp(
          static_cast<value_type>(a), static_cast<value_type>(b), forwarding_policy()),
      "boost::math::tgamma_delta_ratio<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type
...
gamma_p_derivative(T1 a, T2 x, const Policy &)
{
  ...
  return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::gamma_p_derivative_imp(
          static_cast<value_type>(a), static_cast<value_type>(x), forwarding_policy()),
      "boost::math::gamma_p_derivative<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type
