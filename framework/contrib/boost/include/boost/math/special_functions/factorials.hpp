...
inline T
factorial(unsigned i, const Policy & pol)
{
...
template <class T, class Policy>
T
double_factorial(unsigned i, const Policy & pol)
{
  BOOST_STATIC_ASSERT(!boost::is_integral<T>::value);
      ...
      if (i & 1)
  {
    ...
    {
      unsigned n = (i - 1) / 2;
      return ceil(unchecked_factorial<T>(i) / (ldexp(T(1), (int)n) * unchecked_factorial<T>(n)) -
                  0.5f);
    }
    //
...
template <class T, class Policy>
T
rising_factorial_imp(T x, int n, const Policy & pol)
{
  BOOST_STATIC_ASSERT(!boost::is_integral<T>::value);
  if (x < 0)
  {
...
template <class T, class Policy>
inline T
falling_factorial_imp(T x, unsigned n, const Policy & pol)
{
  BOOST_STATIC_ASSERT(!boost::is_integral<T>::value);
  ...
  if (x < 0.5f)
  {
    ...
    if (n > max_factorial<T>::value - 2)
    {
      ...
      T t2 = boost::math::falling_factorial(x - max_factorial<T>::value + 1,
                                            n - max_factorial<T>::value + 1);
      if (tools::max_value<T>() / fabs(t1) < fabs(t2))
        return boost::math::sign(t1) * boost::math::sign(t2) *
               policies::raise_overflow_error<T>("boost::math::falling_factorial<%1%>", 0, pol);
      return t1 * t2;
    }
...
falling_factorial(RT x, unsigned n)
{
  ...
  return detail::falling_factorial_imp(static_cast<result_type>(x), n, policies::policy<>());
}
...
{
  typedef typename tools::promote_args<RT>::type result_type;
  return detail::rising_factorial_imp(static_cast<result_type>(x), n, policies::policy<>());
}
