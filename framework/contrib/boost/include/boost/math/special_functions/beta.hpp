...
T
beta_imp(T a, T b, const Lanczos &, const Policy & pol)
{
...
T
beta_imp(T a, T b, const lanczos::undefined_lanczos & /* l */, const Policy & pol)
{
                  ...
                  const char * function = "boost::math::ibeta<%1%>(%1%, %1%, %1%)")
{
  ...
  else
  {
    ...
    if ((l1 >= tools::log_max_value<T>()) || (l1 <= tools::log_min_value<T>()) ||
        (l2 >= tools::log_max_value<T>()) || (l2 <= tools::log_min_value<T>()))
    {
      ...
      if (a < b)
      {
        ...
        if ((l3 < tools::log_max_value<T>()) && (l3 > tools::log_min_value<T>()))
        {
          result *= pow(p1 * b1, a);
        }
        else
                  ...
                  const char * = "boost::math::ibeta<%1%>(%1%, %1%, %1%)")
{
  ...
  if ((lb1 >= tools::log_max_value<T>()) || (lb1 <= tools::log_min_value<T>()) ||
      (lb2 >= tools::log_max_value<T>()) || (lb2 <= tools::log_min_value<T>()) ||
             ...
             const Policy & pol)
{
  ...
  if (normalised)
  {
    ...
    //
    if ((l1 > tools::log_min_value<T>()) && (l1 < tools::log_max_value<T>()) &&
        (l2 > tools::log_min_value<T>()) && (l2 < tools::log_max_value<T>()))
    {
      if (a * b < bgh * 10)
  ...
  policies::check_series_iterations<T>(
      "boost::math::ibeta<%1%>(%1%, %1%, %1%) in ibeta_series (with lanczos)", max_iter, pol);
  return result;
}
//
             ...
             const Policy & pol)
{
  ...
  result = boost::math::tools::sum_series(
      s, boost::math::policies::get_epsilon<T, Policy>(), max_iter, s0);
  policies::check_series_iterations<T>(
      "boost::math::ibeta<%1%>(%1%, %1%, %1%) in ibeta_series (without lanczos)", max_iter, pol);
  return result;
}
...
template <class T, class Policy>
inline T
ibeta_fraction2(T a, T b, T x, T y, const Policy & pol, bool normalised, T * p_derivative)
{
  ...
  T fract =
      boost::math::tools::continued_fraction_b(f, boost::math::policies::get_epsilon<T, Policy>());
...
template <class T, class Policy>
T
ibeta_a_step(T a, T b, T x, T y, int k, const Policy & pol, bool normalised, T * p_derivative)
{
  typedef typename lanczos::lanczos<T, Policy>::type lanczos_type;
...
template <class T>
inline T
rising_factorial_ratio(T a, T b, int k)
{
  // calculate:
  // (a)(a+1)(a+2)...(a+k-1)
...
template <class T, class Policy>
T
beta_small_b_large_a_series(T a, T b, T x, T y, T s0, T mult, const Policy & pol, bool normalised)
{
  typedef typename lanczos::lanczos<T, Policy>::type lanczos_type;
  BOOST_MATH_STD_USING
  //
  ...
  return sum;
} // template <class T, class Lanczos>T beta_small_b_large_a_series(T a, T b, T x, T y, T s0, T
  // mult, const Lanczos& l, bool normalised)
...
template <class T, class Policy>
T
ibeta_imp(T a, T b, T x, const Policy & pol, bool inv, bool normalised, T * p_derivative)
{
    ...
    if (a == 0)
    {
      if (b == 0)
        return policies::raise_domain_error<T>(function,
                                               "The arguments a and b to the incomplete beta "
                                               "function cannot both be zero, with x=%1%.",
                                               x,
                                               pol);
      if (b > 0)
  ...
  if (x == 0)
  {
    ...
    {
      *p_derivative =
          (a == 1) ? (T)1 : (a < 1) ? T(tools::max_value<T>() / 2) : T(tools::min_value<T>() * 2);
    }
  ...
  if (x == 1)
  {
    ...
    {
      *p_derivative =
          (b == 1) ? T(1) : (b < 1) ? T(tools::max_value<T>() / 2) : T(tools::min_value<T>() * 2);
    }
  ...
  if ((a == 0.5f) && (b == 0.5f))
  {
    ...
    T p =
        invert ? asin(sqrt(y)) / constants::half_pi<T>() : asin(sqrt(x)) / constants::half_pi<T>();
    if (!normalised)
      p *= constants::pi<T>();
    return p;
  }
  ...
  if (b == 1)
  {
    ...
    T p;
    if (y < 0.5)
      p = invert ? T(-boost::math::expm1(a * boost::math::log1p(-y, pol), pol))
      ...
      else
      {
        ...
        else
        {
          ...
          else
          {
            ...
            invert = false;
            fract =
                -beta_small_b_large_a_series(T(a + 20), b, x, y, fract, prefix, pol, normalised);
            BOOST_MATH_INSTRUMENT_VARIABLE(fract);
          }
        }
      }
  ...
  else
  {
    ...
    if (b < 40)
    {
      ...
      else if (normalised)
      {
        ...
        if (invert)
          fract -= 1; // Note this line would need changing if we ever enable this branch in
                      // non-normalized case
...
} // template <class T, class Lanczos>T ibeta_imp(T a, T b, T x, const Lanczos& l, bool inv, bool
  // normalised)

template <class T, class Policy>
inline T
ibeta_imp(T a, T b, T x, const Policy & pol, bool inv, bool normalised)
{
...
template <class T, class Policy>
T
ibeta_derivative_imp(T a, T b, T x, const Policy & pol)
{
  ...
  {
    return (a > 1) ? 0
                   : (a == 1) ? 1 / boost::math::beta(a, b, pol)
                              : policies::raise_overflow_error<T>(function, 0, pol);
  }
  else if (x == 1)
  {
    return (b > 1) ? 0
                   : (b == 1) ? 1 / boost::math::beta(a, b, pol)
                              : policies::raise_overflow_error<T>(function, 0, pol);
  }
  //
...
beta(RT1 a, RT2 b, const Policy &, const mpl::true_ *)
{
  ...
  return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::beta_imp(static_cast<value_type>(a),
...
beta(RT1 a, RT2 b, RT3 x, const Policy &)
{
  ...
  return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::ibeta_imp(static_cast<value_type>(a),
                        static_cast<value_type>(b),
...
ibeta_derivative(RT1 a, RT2 b, RT3 x, const Policy &)
{
  ...
  return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::ibeta_derivative_imp(static_cast<value_type>(a),
                                   static_cast<value_type>(b),
                                   static_cast<value_type>(x),
                                   forwarding_policy()),
