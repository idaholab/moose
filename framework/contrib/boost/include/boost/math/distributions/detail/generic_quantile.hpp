...
struct generic_quantile_finder
{
  ...
  generic_quantile_finder(const Dist & d, value_type t, bool c) : dist(d), target(t), comp(c) {}
...
inline T
check_range_result(const T & x, const Policy & pol, const char * function)
{
...
generic_quantile(const Dist & dist,
                 ...
                 const char * function)
{
  typedef typename Dist::value_type value_type;
  typedef typename Dist::policy_type policy_type;
  typedef typename policies::normalise<policy_type,
  ...
  if (p == 0)
  {
    return comp ? check_range_result(range(dist).second, forwarding_policy(), function)
