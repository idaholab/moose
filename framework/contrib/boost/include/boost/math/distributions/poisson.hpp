...
inline bool
check_mean(const char * function, const RealType & mean, RealType * result, const Policy & pol)
{
  if (!(boost::math::isfinite)(mean) || (mean < 0))
  {
    *result = policies::raise_domain_error<RealType>(
        function, "Mean argument is %1%, but must be >= 0 !", mean, pol);
    return false;
  }
  return true;
} // bool check_mean
...
template <class RealType, class Policy>
inline bool
check_mean_NZ(const char * function, const RealType & mean, RealType * result, const Policy & pol)
...
template <class RealType, class Policy>
inline bool
check_dist(const char * function, const RealType & mean, RealType * result, const Policy & pol)
...
template <class RealType, class Policy>
inline bool
check_k(const char * function, const RealType & k, RealType * result, const Policy & pol)
{
  if ((k < 0) || !(boost::math::isfinite)(k))
  {
    *result = policies::raise_domain_error<RealType>(
        function, "Number of events k argument is %1%, but must be >= 0 !", k, pol);
    return false;
  }
  return true;
} // bool check_k
...
inline bool
check_dist_and_k(
    const char * function, RealType mean, RealType k, RealType * result, const Policy & pol)
{
...
inline bool
check_dist_and_prob(
    const char * function, RealType mean, RealType p, RealType * result, const Policy & pol)
{
  if ((check_dist(function, mean, result, pol) == false) ||
...
public:
  ...
  { // Expected mean number of events that occur during the given interval.
    ...
    poisson_detail::check_dist(
        "boost::math::poisson_distribution<%1%>::poisson_distribution", m_l, &r, Policy());
...
range(const poisson_distribution<RealType, Policy> & /* dist */)
...
template <class RealType, class Policy>
inline const std::pair<RealType, RealType>
support(const poisson_distribution<RealType, Policy> & /* dist */)
...
template <class RealType, class Policy>
inline RealType
mean(const poisson_distribution<RealType, Policy> & dist)
...
template <class RealType, class Policy>
inline RealType
mode(const poisson_distribution<RealType, Policy> & dist)
{                      // mode.
...
template <class RealType, class Policy>
inline RealType
variance(const poisson_distribution<RealType, Policy> & dist)
{ // variance.
  return dist.mean();
}
...
template <class RealType, class Policy>
inline RealType
skewness(const poisson_distribution<RealType, Policy> & dist)
{                      // skewness = sqrt(l).
  BOOST_MATH_STD_USING // ADL of std functions.
      return 1 /
      sqrt(dist.mean());
...
template <class RealType, class Policy>
inline RealType
kurtosis_excess(const poisson_distribution<RealType, Policy> & dist)
{                         // skewness = sqrt(l).
...
template <class RealType, class Policy>
inline RealType
kurtosis(const poisson_distribution<RealType, Policy> & dist)
...
template <class RealType, class Policy>
RealType
pdf(const poisson_distribution<RealType, Policy> & dist, const RealType & k)
{ // Probability Density/Mass Function.
  ...
  RealType result = 0;
  if (false ==
      poisson_detail::check_dist_and_k(
          "boost::math::pdf(const poisson_distribution<%1%>&, %1%)", mean, k, &result, Policy()))
  {
    return result;
  }
...
template <class RealType, class Policy>
RealType
cdf(const poisson_distribution<RealType, Policy> & dist, const RealType & k)
{ // Cumulative Distribution Function Poisson.
  ...
  if (false ==
      poisson_detail::check_dist_and_k(
          "boost::math::cdf(const poisson_distribution<%1%>&, %1%)", mean, k, &result, Policy()))
  {
    return result;
  }
  // Special cases:
...
template <class RealType, class Policy>
RealType
cdf(const complemented2_type<poisson_distribution<RealType, Policy>, RealType> & c)
...
template <class RealType, class Policy>
inline RealType
quantile(const poisson_distribution<RealType, Policy> & dist, const RealType & p)
{ // Quantile (or Percent Point) Poisson function.
  ...
  if (false == poisson_detail::check_prob(function, p, &result, Policy()))
  {
    return result;
  }
  // Special case:
  ...
  { // if mean = 0 then p = 0, so k can be anything?
    if (false == poisson_detail::check_mean_NZ(function, dist.mean(), &result, Policy()))
    {
      return result;
    }
  }
...
template <class RealType, class Policy>
inline RealType
quantile(const complemented2_type<poisson_distribution<RealType, Policy>, RealType> & c)
{ // Quantile (or Percent Point) of Poisson function.
  ...
  static const char * function =
      "boost::math::quantile(complement(const poisson_distribution<%1%>&, %1%))";
  RealType q = c.param;
  const poisson_distribution<RealType, Policy> & dist = c.dist;
  RealType result = 0; // of argument checks.
  if (false == poisson_detail::check_prob(function, q, &result, Policy()))
  {
    return result;
  }
  // Special case:
