...
inline bool
check_skew_normal_shape(const char * function,
                        RealType shape,
                        RealType * result,
                        const Policy & pol)
{
  if (!(boost::math::isfinite)(shape))
  {
    *result = policies::raise_domain_error<RealType>(
        function, "Shape parameter is %1%, but must be finite!", shape, pol);
    return false;
  }
  return true;
}
...
public:
  ...
  { // Default is a 'standard' normal distribution N01. (shape=0 results in the normal distribution
    // with no skew)
    static const char * function =
        "boost::math::skew_normal_distribution<%1%>::skew_normal_distribution";
  ...
  RealType shape() const { return shape_; }
...
range(const skew_normal_distribution<RealType, Policy> & /*dist*/)
{ // Range of permissible values for random variable x.
  using boost::math::tools::max_value;
  return std::pair<RealType, RealType>(
      std::numeric_limits<RealType>::has_infinity ? -std::numeric_limits<RealType>::infinity()
                                                  : -max_value<RealType>(),
      std::numeric_limits<RealType>::has_infinity ? std::numeric_limits<RealType>::infinity()
...
template <class RealType, class Policy>
inline const std::pair<RealType, RealType>
support(const skew_normal_distribution<RealType, Policy> & /*dist*/)
...
pdf(const skew_normal_distribution<RealType, Policy> & dist, const RealType & x)
{
  ...
  // if(std::numeric_limits<RealType>::has_infinity && abs(x) ==
...
cdf(const skew_normal_distribution<RealType, Policy> & dist, const RealType & x)
{
  ...
  result =
      cdf(std_normal, transformed_x) - owens_t(transformed_x, shape) * static_cast<RealType>(2);
...
template <class RealType, class Policy>
inline RealType
cdf(const complemented2_type<skew_normal_distribution<RealType, Policy>, RealType> & c)
{
  ...
  static const char * function =
      "boost::math::cdf(const complement(skew_normal_distribution<%1%>&), %1%)";
  ...
  result = cdf(complement(std_normal, transformed_x)) +
           owens_t(transformed_x, shape) * static_cast<RealType>(2);
  return result;
} // cdf complement

template <class RealType, class Policy>
inline RealType
location(const skew_normal_distribution<RealType, Policy> & dist)
{
...
template <class RealType, class Policy>
inline RealType
scale(const skew_normal_distribution<RealType, Policy> & dist)
{
  return dist.scale();
}
...
template <class RealType, class Policy>
inline RealType
shape(const skew_normal_distribution<RealType, Policy> & dist)
{
  return dist.shape();
}
...
mean(const skew_normal_distribution<RealType, Policy> & dist)
{
  ...
  return dist.location() +
         dist.scale() * dist.shape() /
             sqrt(pi<RealType>() + pi<RealType>() * dist.shape() * dist.shape()) *
             root_two<RealType>();
}
...
variance(const skew_normal_distribution<RealType, Policy> & dist)
{
  ...
  const RealType delta2 =
      ...
      (static_cast<RealType>(1) + static_cast<RealType>(1) / (dist.shape() * dist.shape()));
  // const RealType inv_delta2 =
  // static_cast<RealType>(1)+static_cast<RealType>(1)/(dist.shape()*dist.shape());

  RealType variance =
      dist.scale() * dist.scale() * (static_cast<RealType>(1) - two_div_pi<RealType>() * delta2);
  // RealType variance =
  // dist.scale()*dist.scale()*(static_cast<RealType>(1)-two_div_pi<RealType>()/inv_delta2);
...
template <class RealType, class Policy>
inline RealType
mode_fallback(const skew_normal_distribution<RealType, Policy> & dist)
{ // mode.
  ...
  // 21 elements
  static const RealType shapes[] = {0.0,
  ...
  // 21 elements
  static const RealType guess[] = {0.0,
  ...
  if (d < static_cast<diff_type>(21)) // shape smaller 100
  {
    result = guess[d - static_cast<diff_type>(1)] +
             (guess[d] - guess[d - static_cast<diff_type>(1)]) /
                 (shapes[d] - shapes[d - static_cast<diff_type>(1)]) *
...
struct skew_normal_mode_functor
{
  ...
  boost::math::tuple<RealType, RealType> operator()(RealType const & x)
  {
    ...
    RealType dx = static_cast<RealType>(2) * shape * x * normpdf_x * normpdf_ax *
                      (static_cast<RealType>(1) + shape * shape) +
                  pdf_x + x * fx;
...
template <class RealType, class Policy>
inline RealType
mode(const skew_normal_distribution<RealType, Policy> & dist)
{
  const RealType scale = dist.scale();
  const RealType location = dist.location();
  const RealType shape = dist.shape();
  ...
  {
    result = 1e-4f;
    search_max = guess[19]; // set 19 instead of 20 to have a safety margin because the table may
                            // not be exact @ shape=100
  }
  ...
  result = tools::newton_raphson_iterate(detail::skew_normal_mode_functor<RealType, Policy>(helper),
                                         result,
                                         search_min,
                                         search_max,
                                         get_digits,
...
skewness(const skew_normal_distribution<RealType, Policy> & dist)
{
  ...
  const RealType delta =
      dist.shape() / sqrt(static_cast<RealType>(1) + dist.shape() * dist.shape());

  return factor * pow(root_two_div_pi<RealType>() * delta, 3) /
         pow(static_cast<RealType>(1) - two_div_pi<RealType>() * delta * delta,
...
{
  skew_normal_quantile_functor(const boost::math::skew_normal_distribution<RealType, Policy> dist,
                               RealType const & p)
...
template <class RealType, class Policy>
inline RealType
quantile(const skew_normal_distribution<RealType, Policy> & dist, const RealType & p)
{
  const RealType scale = dist.scale();
  const RealType location = dist.location();
  const RealType shape = dist.shape();
  ...
  if (shape != 0)
  {
    ...
    x = x + (x * x - static_cast<RealType>(1)) * skew / static_cast<RealType>(6) +
        ...
        x * (static_cast<RealType>(2) * x * x - static_cast<RealType>(5)) * skew * skew /
            static_cast<RealType>(36);
  } // if(shape != 0)
  ...
  result =
      tools::newton_raphson_iterate(detail::skew_normal_quantile_functor<RealType, Policy>(dist, p),
                                    result,
                                    search_min,
                                    search_max,
                                    get_digits,
...
template <class RealType, class Policy>
inline RealType
quantile(const complemented2_type<skew_normal_distribution<RealType, Policy>, RealType> & c)
{
  const RealType scale = c.dist.scale();
  const RealType location = c.dist.location();
  const RealType shape = c.dist.shape();
  ...
  static const char * function =
      "boost::math::quantile(const complement(skew_normal_distribution<%1%>&), %1%)";
  RealType result = 0;
  if (false == detail::check_scale(function, scale, &result, Policy()))
    return result;
  if (false == detail::check_location(function, location, &result, Policy()))
