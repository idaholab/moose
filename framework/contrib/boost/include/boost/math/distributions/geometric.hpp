...
template <class RealType, class Policy>
inline bool
check_dist(const char * function, const RealType & p, RealType * result, const Policy & pol)
{
...
inline bool
check_dist_and_k(
    const char * function, const RealType & p, RealType k, RealType * result, const Policy & pol)
{
...
inline bool
check_dist_and_prob(
    const char * function, RealType p, RealType prob, RealType * result, const Policy & pol)
{
  if ((check_dist(function, p, result, pol) &&
...
public:
  ...
  { // Constructor stores success_fraction p.
    RealType result;
    geometric_detail::check_dist("geometric_distribution<%1%>::geometric_distribution",
                        ...
                        RealType alpha) // alpha 0.05 equivalent to 95% for one-sided test.
  {
    ...
    if (false == detail::check_probability(function, alpha, &result, Policy()) &&
        geometric_detail::check_dist_and_k(function, RealType(0), failures, &result, Policy()))
    {
      return result;
    }
                                ...
                                RealType alpha) // risk level threshold 0 <= alpha <= 1.
  {
    ...
    // Error checks:
    RealType result = 0;
    if (false == geometric_detail::check_dist_and_k(function, p, k, &result, Policy()) &&
        detail::check_probability(function, alpha, &result, Policy()))
    {
      return result;
    }
...
range(const geometric_distribution<RealType, Policy> & /* dist */)
...
template <class RealType, class Policy>
inline const std::pair<RealType, RealType>
support(const geometric_distribution<RealType, Policy> & /* dist */)
...
template <class RealType, class Policy>
inline RealType
mode(const geometric_distribution<RealType, Policy> &)
...
template <class RealType, class Policy>
inline RealType
pdf(const geometric_distribution<RealType, Policy> & dist, const RealType & k)
...
template <class RealType, class Policy>
inline RealType
cdf(const geometric_distribution<RealType, Policy> & dist, const RealType & k)
...
template <class RealType, class Policy>
inline RealType
cdf(const complemented2_type<geometric_distribution<RealType, Policy>, RealType> & c)
...
{ // Quantile, percentile/100 or Percent Point geometric function.
  ...
  // Inverse cumulative Distribution Function or Quantile (percentile / 100) of geometric
  // Probability.
  // k argument may be integral, signed, or unsigned, or floating point.
  ...
  RealType result = 0;
  if (false ==
      geometric_detail::check_dist_and_prob(function, success_fraction, x, &result, Policy()))
  {
    return result;
  }
...
template <class RealType, class Policy>
inline RealType
quantile(const complemented2_type<geometric_distribution<RealType, Policy>, RealType> & c)
