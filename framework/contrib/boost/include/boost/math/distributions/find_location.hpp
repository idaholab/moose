...
find_location( // For example, normal mean.
    typename Dist::value_type
        z, // location of random variable z to give probability, P(X > z) == p.
    ...
    complemented3_type<Real1, Real2, Real3> const & c)
{
  ...
  if (!(boost::math::isfinite)(p) || (p < 0) || (p > 1))
  {
    return policies::raise_domain_error<typename Dist::value_type>(
        function,
        "Probability parameter was %1%, but must be >= 0 and <= 1!",
        p,
        policies::policy<>());
  }
  ...
  // cout << "z " << c.dist << ", quantile (Dist(), " << c.param1 << ") * scale " << c.param2 <<
  // endl;
  return z - quantile(Dist(), p) * scale;
