...
T
non_central_t2_p(T v, T delta, T x, T y, const Policy & pol, T init_val)
{
  ...
  pois = gamma_p_derivative(T(k + 1), d2, pol) * tgamma_delta_ratio(T(k + 1), T(0.5f)) * delta /
         constants::root_two<T>();
  if (pois == 0)
    return init_val;
  T xterm, beta;
  // Recurrance & starting beta terms:
  beta = x < y ? detail::ibeta_imp(T(k + 1), T(v / 2), x, pol, false, true, &xterm)
  ...
  for (int i = k + 1;; ++i)
  {
    ...
    if (count > max_iter)
    {
      return policies::raise_evaluation_error("cdf(non_central_t_distribution<%1%>, %1%)",
...
template <class T, class Policy>
T
non_central_t2_q(T v, T delta, T x, T y, const Policy & pol, T init_val)
{
  BOOST_MATH_STD_USING
  //
  // Variables come first:
  ...
  // Starting Poisson weight:
  T pois;
  if ((k < (int)(max_factorial<T>::value)) && (d2 < tools::log_max_value<T>()) &&
      (log(d2) * k < tools::log_max_value<T>()))
  {
    //
  ...
  if (k != 0)
  {
    beta = x < y ? detail::ibeta_imp(T(k + 1), T(v / 2), x, pol, true, true, &xterm)
...
template <class T, class Policy>
T
non_central_t_cdf(T v, T delta, T t, bool invert, const Policy & pol)
{
  BOOST_MATH_STD_USING
...
template <class T, class Policy>
T
non_central_t_quantile(const char * function, T v, T delta, T p, T q, const Policy &)
{
  BOOST_MATH_STD_USING
  ...
  T r;
  if (!detail::check_df_gt0_to_inf(function, v, &r, Policy()) ||
      !detail::check_finite(function, delta, &r, Policy()) ||
      !detail::check_probability(function, p, &r, Policy()))
    return r;
  ...
  { // Use normal distribution to calculate guess.
    value_type mean = (v > 1 / policies::get_epsilon<T, Policy>())
                          ? delta
                          : delta * sqrt(v / 2) * tgamma_delta_ratio((v - 1) * 0.5f, T(0.5f));
    value_type var = (v > 1 / policies::get_epsilon<T, Policy>())
                         ? value_type(1)
    ...
    if (p < q)
      ...
      guess =
          quantile(complement(normal_distribution<value_type, forwarding_policy>(mean, var), q));
  }
  //
  // We *must* get the sign of the initial guess correct,
  // or our root-finder will fail, so double check it now:
  //
  value_type pzero = non_central_t_cdf(static_cast<value_type>(v),
  ...
  value_type result =
      detail::generic_quantile(non_central_t_distribution<value_type, forwarding_policy>(v, delta),
                               (p < q ? p : q),
                               guess,
                               (p >= q),
                               function);
  return policies::checked_narrowing_cast<T, forwarding_policy>(result, function);
}

template <class T, class Policy>
T
non_central_t2_pdf(T n, T delta, T x, T y, const Policy & pol, T init_val)
{
  BOOST_MATH_STD_USING
  //
  // Variables come first:
  ...
  // Starting beta term:
  xterm =
      x < y ? ibeta_derivative(T(k + 1), n / 2, x, pol) : ibeta_derivative(n / 2, T(k + 1), y, pol);
  ...
  for (int i = k; i >= 0; --i)
  {
    ...
    if (count > max_iter)
    {
      return policies::raise_evaluation_error("pdf(non_central_t_distribution<%1%>, %1%)",
                                              "Series did not converge, closest value was %1%",
                                              sum,
                                              pol);
    }
...
template <class T, class Policy>
T
non_central_t_pdf(T n, T delta, T t, const Policy & pol)
{
  BOOST_MATH_STD_USING
  ...
  if (t == 0)
  {
    ...
    // 1F1(a,b,0) = 1.
    //
    return tgamma_delta_ratio(n / 2 + 0.5f, T(0.5f)) * sqrt(n / constants::pi<T>()) *
           exp(-delta * delta / 2) / 2;
  }
...
template <class T, class Policy>
T
mean(T v, T delta, const Policy & pol)
{
  if ((boost::math::isinf)(v))
  {
    return delta;
...
template <class T, class Policy>
T
variance(T v, T delta, const Policy & pol)
{
  if ((boost::math::isinf)(v))
  {
    return 1;
...
template <class T, class Policy>
T
skewness(T v, T delta, const Policy & pol)
{
  BOOST_MATH_STD_USING
  if ((boost::math::isinf)(v))
  {
...
template <class T, class Policy>
T
kurtosis_excess(T v, T delta, const Policy & pol)
{
  BOOST_MATH_STD_USING
  if ((boost::math::isinf)(v))
  {
...
public:
  ...
  {
    const char * function =
        "boost::math::non_central_t_distribution<%1%>::non_central_t_distribution(%1%,%1%)";
    RealType r;
    detail::check_df_gt0_to_inf(function, v, &r, Policy());
    detail::check_finite(function, lambda, &r, Policy());
...
range(const non_central_t_distribution<RealType, Policy> & /* dist */)
...
template <class RealType, class Policy>
inline const std::pair<RealType, RealType>
support(const non_central_t_distribution<RealType, Policy> & /* dist */)
...
template <class RealType, class Policy>
inline RealType
mode(const non_central_t_distribution<RealType, Policy> & dist)
{ // mode.
  ...
  RealType r;
  if (!detail::check_df_gt0_to_inf(function, v, &r, Policy()) ||
      !detail::check_finite(function, l, &r, Policy()))
    return (RealType)r;
  ...
  return detail::generic_find_mode(dist, m, function, sqrt(var));
}

template <class RealType, class Policy>
inline RealType
mean(const non_central_t_distribution<RealType, Policy> & dist)
{
  BOOST_MATH_STD_USING
  ...
  if (v <= 1)
    return policies::raise_domain_error<RealType>(function,
                                                  "The non-central t distribution has no defined "
                                                  "mean for degrees of freedom <= 1: got v=%1%.",
                                                  v,
                                                  Policy());
  // return l * sqrt(v / 2) * tgamma_delta_ratio((v - 1) * 0.5f, RealType(0.5f));
  return policies::checked_narrowing_cast<RealType, forwarding_policy>(
      detail::mean(static_cast<value_type>(v), static_cast<value_type>(l), forwarding_policy()),
      function);
...
template <class RealType, class Policy>
inline RealType
variance(const non_central_t_distribution<RealType, Policy> & dist)
{ // variance.
  ...
  if (v <= 2)
    return policies::raise_domain_error<RealType>(function,
                                                  "The non-central t distribution has no defined "
                                                  "variance for degrees of freedom <= 2: got "
                                                  "v=%1%.",
                                                  v,
                                                  Policy());
  return policies::checked_narrowing_cast<RealType, forwarding_policy>(
      detail::variance(static_cast<value_type>(v), static_cast<value_type>(l), forwarding_policy()),
      function);
}
...
template <class RealType, class Policy>
inline RealType
skewness(const non_central_t_distribution<RealType, Policy> & dist)
{ // skewness = sqrt(l).
  ...
  if (v <= 3)
    return policies::raise_domain_error<RealType>(function,
                                                  "The non-central t distribution has no defined "
                                                  "skewness for degrees of freedom <= 3: got "
                                                  "v=%1%.",
                                                  v,
                                                  Policy());
  ;
  return policies::checked_narrowing_cast<RealType, forwarding_policy>(
      detail::skewness(static_cast<value_type>(v), static_cast<value_type>(l), forwarding_policy()),
      function);
}

template <class RealType, class Policy>
inline RealType
kurtosis_excess(const non_central_t_distribution<RealType, Policy> & dist)
{
  ...
  if (v <= 4)
    return policies::raise_domain_error<RealType>(function,
                                                  "The non-central t distribution has no defined "
                                                  "kurtosis for degrees of freedom <= 4: got "
                                                  "v=%1%.",
                                                  v,
                                                  Policy());
  ;
  return policies::checked_narrowing_cast<RealType, forwarding_policy>(
      detail::kurtosis_excess(
          static_cast<value_type>(v), static_cast<value_type>(l), forwarding_policy()),
      function);
} // kurtosis_excess

template <class RealType, class Policy>
inline RealType
kurtosis(const non_central_t_distribution<RealType, Policy> & dist)
{
...
template <class RealType, class Policy>
inline RealType
pdf(const non_central_t_distribution<RealType, Policy> & dist, const RealType & t)
{ // Probability Density/Mass Function.
  ...
  if (!detail::check_df_gt0_to_inf(function, v, &r, Policy()) ||
      !detail::check_finite(function, l, &r, Policy()) ||
      !detail::check_x(function, t, &r, Policy()))
    return (RealType)r;
  return policies::checked_narrowing_cast<RealType, forwarding_policy>(
...
template <class RealType, class Policy>
RealType
cdf(const non_central_t_distribution<RealType, Policy> & dist, const RealType & x)
{
  ...
  if (!detail::check_df_gt0_to_inf(function, v, &r, Policy()) ||
      !detail::check_finite(function, l, &r, Policy()) ||
      !detail::check_x(function, x, &r, Policy()))
    return (RealType)r;
  if ((boost::math::isinf)(v))
  { // Infinite degrees of freedom, so use normal distribution located at delta.
  ...
  return policies::checked_narrowing_cast<RealType, forwarding_policy>(
      detail::non_central_t_cdf(static_cast<value_type>(v),
                                static_cast<value_type>(l),
                                static_cast<value_type>(x),
                                false,
                                Policy()),
...
template <class RealType, class Policy>
RealType
cdf(const complemented2_type<non_central_t_distribution<RealType, Policy>, RealType> & c)
{ // Complemented Cumulative Distribution Function
  // was       const char* function = "boost::math::non_central_t_distribution<%1%>::cdf(%1%)";
  const char * function =
      "boost::math::cdf(const complement(non_central_t_distribution<%1%>&), %1%)";
  typedef typename policies::evaluation<RealType, Policy>::type value_type;
  typedef typename policies::normalise<Policy,
                                       policies::promote_float<false>,
                                       policies::promote_double<false>,
...
template <class RealType, class Policy>
inline RealType
quantile(const non_central_t_distribution<RealType, Policy> & dist, const RealType & p)
...
template <class RealType, class Policy>
inline RealType
quantile(const complemented2_type<non_central_t_distribution<RealType, Policy>, RealType> & c)
{ // Quantile (or Percent Point) function.
