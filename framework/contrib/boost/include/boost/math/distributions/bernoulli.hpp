                       ...
                       const Policy & /* pol */)
{
  if (!(boost::math::isfinite)(p) || (p < 0) || (p > 1))
  {
    *result = policies::raise_domain_error<RealType>(
        function, "Success fraction argument is %1%, but must be >= 0 and <= 1 !", p, Policy());
    return false;
  }
  return true;
}
...
inline bool
check_dist(
    const char *, const RealType &, RealType *, const Policy & /* pol */, const mpl::false_ &)
{
  return true;
}
template <class RealType, class Policy>
...
{
  return check_dist(
      function, p, result, Policy(), typename policies::constructor_error_check<Policy>::type());
}
...
{
  if (check_dist(
          function, p, result, Policy(), typename policies::method_error_check<Policy>::type()) ==
      false)
  {
    return false;
  }
  ...
  {
    *result = policies::raise_domain_error<RealType>(
        function, "Number of successes argument is %1%, but must be 0 or 1 !", k, pol);
    return false;
  }
  return true;
}
    ...
    const char * function, RealType p, RealType prob, RealType * result, const Policy & /* pol */)
{
  if ((check_dist(
           function, p, result, Policy(), typename policies::method_error_check<Policy>::type()) &&
       detail::check_probability(function, prob, result, Policy())) == false)
  {
    return false;
  }
  return true;
...
public:
  ...
  { // Default probability = half suits 'fair' coin tossing
    ...
    bernoulli_detail::check_dist(
        "boost::math::bernoulli_distribution<%1%>::bernoulli_distribution", m_p, &result, Policy());
...
range(const bernoulli_distribution<RealType, Policy> & /* dist */)
...
template <class RealType, class Policy>
inline const std::pair<RealType, RealType>
support(const bernoulli_distribution<RealType, Policy> & /* dist */)
...
template <class RealType, class Policy>
inline RealType
mean(const bernoulli_distribution<RealType, Policy> & dist)
...
template <class RealType, class Policy>
inline RealType
variance(const bernoulli_distribution<RealType, Policy> & dist)
...
template <class RealType, class Policy>
RealType
pdf(const bernoulli_distribution<RealType, Policy> & dist, const RealType & k)
{ // Probability Density/Mass Function.
  BOOST_FPU_EXCEPTION_GUARD
  // Error check:
  RealType result = 0; // of checks.
  if (false ==
      bernoulli_detail::check_dist_and_k("boost::math::pdf(bernoulli_distribution<%1%>, %1%)",
...
template <class RealType, class Policy>
inline RealType
cdf(const bernoulli_distribution<RealType, Policy> & dist, const RealType & k)
{ // Cumulative Distribution Function Bernoulli.
  RealType p = dist.success_fraction();
  // Error check:
  RealType result = 0;
  if (false == bernoulli_detail::check_dist_and_k(
                   "boost::math::cdf(bernoulli_distribution<%1%>, %1%)", p, k, &result, Policy()))
  {
    return result;
  }
  if (k == 0)
...
template <class RealType, class Policy>
inline RealType
cdf(const complemented2_type<bernoulli_distribution<RealType, Policy>, RealType> & c)
...
template <class RealType, class Policy>
inline RealType
quantile(const bernoulli_distribution<RealType, Policy> & dist, const RealType & p)
...
template <class RealType, class Policy>
inline RealType
quantile(const complemented2_type<bernoulli_distribution<RealType, Policy>, RealType> & c)
{ // Quantile or Percent Point bernoulli function.
...
template <class RealType, class Policy>
inline RealType
mode(const bernoulli_distribution<RealType, Policy> & dist)
{
...
template <class RealType, class Policy>
inline RealType
skewness(const bernoulli_distribution<RealType, Policy> & dist)
{
...
template <class RealType, class Policy>
inline RealType
kurtosis_excess(const bernoulli_distribution<RealType, Policy> & dist)
{
  RealType p = dist.success_fraction();
...
template <class RealType, class Policy>
inline RealType
kurtosis(const bernoulli_distribution<RealType, Policy> & dist)
{
  RealType p = dist.success_fraction();
