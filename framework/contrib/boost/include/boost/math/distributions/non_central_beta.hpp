...
T
non_central_beta_p(T a, T b, T lam, T x, T y, const Policy & pol, T init_val = 0)
{
  ...
  T xterm;
  // Starting beta term:
  T beta = x < y ? detail::ibeta_imp(T(a + k), b, x, pol, false, true, &xterm)
  ...
  for (int i = k + 1;; ++i)
  {
    ...
    {
      return policies::raise_evaluation_error("cdf(non_central_beta_distribution<%1%>, %1%)",
...
template <class T, class Policy>
T
non_central_beta_q(T a, T b, T lam, T x, T y, const Policy & pol, T init_val = 0)
{
  BOOST_MATH_STD_USING
  using namespace boost::math;
  //
  ...
  T xterm;
  // Starting beta term:
  T beta = x < y ? detail::ibeta_imp(T(a + k), b, x, pol, true, true, &xterm)
...
non_central_beta_cdf(
    RealType x, RealType y, RealType a, RealType b, RealType l, bool invert, const Policy &)
{
  ...
  else if (x > cross)
  {
    ...
    result = detail::non_central_beta_q(static_cast<value_type>(a),
  ...
  {
    result = detail::non_central_beta_p(static_cast<value_type>(a),
                                        static_cast<value_type>(b),
                                        static_cast<value_type>(l),
                                        static_cast<value_type>(x),
                                        static_cast<value_type>(y),
  ...
  return policies::checked_narrowing_cast<RealType, forwarding_policy>(
      result, "boost::math::non_central_beta_cdf<%1%>(%1%, %1%, %1%)");
}
...
struct nc_beta_quantile_functor
{
  ...
  {
    return comp ? T(target - cdf(complement(dist, x))) : T(cdf(dist, x) - target);
  }
                          ...
                          const Policy & pol)
{
  ...
  if ((fa < 0) == (guess < 0 ? !rising : rising))
  {
    ...
    while ((boost::math::sign)(fb) == (boost::math::sign)(fa))
    {
      if (count == 0)
      {
        b = policies::raise_evaluation_error(
      ...
      BOOST_MATH_INSTRUMENT_CODE(
          "a = " << a << " b = " << b << " fa = " << fa << " fb = " << fb << " count = " << count);
    }
  }
  else
  {
    ...
    while ((boost::math::sign)(fb) == (boost::math::sign)(fa))
    {
      ...
      if (count == 0)
      {
        a = policies::raise_evaluation_error(
  ...
  std::pair<T, T> r = toms748_solve(
      f, (a < 0 ? b : a), (a < 0 ? a : b), (a < 0 ? fb : fa), (a < 0 ? fa : fb), tol, count, pol);
  max_iter += count;
...
template <class RealType, class Policy>
RealType
nc_beta_quantile(const non_central_beta_distribution<RealType, Policy> & dist,
                 const RealType & p,
                 bool comp)
{
  ...
  value_type r;
  if (!beta_detail::check_alpha(function, a, &r, Policy()) ||
      !beta_detail::check_beta(function, b, &r, Policy()) ||
  ...
  value_type guess = comp
     ? quantile(complement(normal_distribution<RealType, Policy>(static_cast<RealType>(mean),
  static_cast<RealType>(sd)), p))
     : quantile(normal_distribution<RealType, Policy>(static_cast<RealType>(mean),
  static_cast<RealType>(sd)), p);
  ...
  */
  ...
  detail::nc_beta_quantile_functor<value_type, Policy> f(
      non_central_beta_distribution<value_type, Policy>(a, b, l), p, comp);
  tools::eps_tolerance<value_type> tol(policies::digits<RealType, Policy>());
  boost::uintmax_t max_iter = policies::get_max_root_iterations<Policy>();

  std::pair<value_type, value_type> ir =
      bracket_and_solve_root_01(f, guess, value_type(2.5), true, tol, max_iter, Policy());
...
template <class T, class Policy>
T
non_central_beta_pdf(T a, T b, T lam, T x, T y, const Policy & pol)
{
  BOOST_MATH_STD_USING
  //
  // Special cases:
  ...
  T pois = gamma_p_derivative(T(k + 1), l2, pol);
  // Starting beta term:
  T beta = x < y ? ibeta_derivative(a + k, b, x, pol) : ibeta_derivative(b, a + k, y, pol);
  T sum = 0;
  T poisf(pois);
  T betaf(beta);
  ...
  for (int i = k + 1;; ++i)
  {
    ...
    if (static_cast<boost::uintmax_t>(count + i - k) > max_iter)
    {
      return policies::raise_evaluation_error("pdf(non_central_beta_distribution<%1%>, %1%)",
                                              "Series did not converge, closest value was %1%",
                                              sum,
                                              pol);
    }
...
template <class RealType, class Policy>
RealType
nc_beta_pdf(const non_central_beta_distribution<RealType, Policy> & dist, const RealType & x)
{
  BOOST_MATH_STD_USING
  ...
  if (!beta_detail::check_alpha(function, a, &r, Policy()) ||
      !beta_detail::check_beta(function, b, &r, Policy()) ||
      !detail::check_non_centrality(function, l, &r, Policy()) ||
      !beta_detail::check_x(function, static_cast<value_type>(x), &r, Policy()))
    return (RealType)r;

  if (l == 0)
    return pdf(boost::math::beta_distribution<RealType, Policy>(dist.alpha(), dist.beta()), x);
  return policies::checked_narrowing_cast<RealType, forwarding_policy>(
      non_central_beta_pdf(a,
                           b,
                           l,
                           static_cast<value_type>(x),
                           value_type(1 - static_cast<value_type>(x)),
                           forwarding_policy()),
      "function");
}
...
struct hypergeometric_2F2_sum
{
  ...
  hypergeometric_2F2_sum(T a1_, T a2_, T b1_, T b2_, T z_)
    : a1(a1_), a2(a2_), b1(b1_), b2(b2_), z(z_), term(1), k(0)
  {
  }
  T operator()()
  {
...
template <class T, class Policy>
T
hypergeometric_2F2(T a1, T a2, T b1, T b2, T z, const Policy & pol)
{
  ...
  value_type zero = 0;
  value_type result = boost::math::tools::sum_series(
      s, boost::math::policies::get_epsilon<value_type, Policy>(), max_iter, zero);
#else
  value_type result = boost::math::tools::sum_series(
      s, boost::math::policies::get_epsilon<value_type, Policy>(), max_iter);
#endif
...
public:
  ...
  non_central_beta_distribution(RealType a_, RealType b_, RealType lambda)
    : a(a_), b(b_), ncp(lambda)
  {
    const char * function =
        "boost::math::non_central_beta_distribution<%1%>::non_central_beta_distribution(%1%,%1%)";
    RealType r;
    beta_detail::check_alpha(function, a, &r, Policy());
    beta_detail::check_beta(function, b, &r, Policy());
    detail::check_non_centrality(function, lambda, &r, Policy());
...
range(const non_central_beta_distribution<RealType, Policy> & /* dist */)
...
template <class RealType, class Policy>
inline const std::pair<RealType, RealType>
support(const non_central_beta_distribution<RealType, Policy> & /* dist */)
...
template <class RealType, class Policy>
inline RealType
mode(const non_central_beta_distribution<RealType, Policy> & dist)
{ // mode.
  ...
  return detail::generic_find_mode_01(dist, mean, function);
}
...
template <class RealType, class Policy>
inline RealType
mean(const non_central_beta_distribution<RealType, Policy> & dist)
{
  BOOST_MATH_STD_USING
  RealType a = dist.alpha();
  RealType b = dist.beta();
  ...
  RealType apb = a + b;
  return exp(-d / 2) * a *
         detail::hypergeometric_2F2<RealType, Policy>(1 + a, apb, a, 1 + apb, d / 2, Policy()) /
         apb;
} // mean

template <class RealType, class Policy>
inline RealType
variance(const non_central_beta_distribution<RealType, Policy> & dist)
{
  //
  ...
  RealType d = dist.non_centrality();
  RealType apb = a + b;
  RealType result = detail::hypergeometric_2F2(
      RealType(1 + a), apb, a, RealType(1 + apb), RealType(d / 2), Policy());
  result *= result * -exp(-d) * a * a / (apb * apb);
  result += exp(-d / 2) * a * (1 + a) *
            detail::hypergeometric_2F2(
                RealType(2 + a), apb, a, RealType(2 + apb), RealType(d / 2), Policy()) /
            (apb * (1 + apb));
  return result;
}
...
template <class RealType, class Policy>
inline RealType
skewness(const non_central_beta_distribution<RealType, Policy> & /*dist*/)
{ // skewness = sqrt(l).
...
template <class RealType, class Policy>
inline RealType
kurtosis_excess(const non_central_beta_distribution<RealType, Policy> & /*dist*/)
{
...
template <class RealType, class Policy>
inline RealType
kurtosis(const non_central_beta_distribution<RealType, Policy> & dist)
{
...
template <class RealType, class Policy>
inline RealType
pdf(const non_central_beta_distribution<RealType, Policy> & dist, const RealType & x)
...
template <class RealType, class Policy>
RealType
cdf(const non_central_beta_distribution<RealType, Policy> & dist, const RealType & x)
{
  ...
  if (!beta_detail::check_alpha(function, a, &r, Policy()) ||
      !beta_detail::check_beta(function, b, &r, Policy()) ||
      !detail::check_non_centrality(function, l, &r, Policy()) ||
      !beta_detail::check_x(function, x, &r, Policy()))
    return (RealType)r;
...
template <class RealType, class Policy>
RealType
cdf(const complemented2_type<non_central_beta_distribution<RealType, Policy>, RealType> & c)
...
template <class RealType, class Policy>
inline RealType
quantile(const non_central_beta_distribution<RealType, Policy> & dist, const RealType & p)
...
template <class RealType, class Policy>
inline RealType
quantile(const complemented2_type<non_central_beta_distribution<RealType, Policy>, RealType> & c)
{ // Quantile (or Percent Point) function.
