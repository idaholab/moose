...
public:
  ...
  { // Default is a 1,1 inverse_gaussian distribution.
    static const char * function =
        "boost::math::inverse_gaussian_distribution<%1%>::inverse_gaussian_distribution";
...
range(const inverse_gaussian_distribution<RealType, Policy> & /*dist*/)
...
template <class RealType, class Policy>
inline const std::pair<RealType, RealType>
support(const inverse_gaussian_distribution<RealType, Policy> & /*dist*/)
...
template <class RealType, class Policy>
inline RealType
pdf(const inverse_gaussian_distribution<RealType, Policy> & dist, const RealType & x)
{                      // Probability Density Function
  ...
  result = sqrt(scale / (constants::two_pi<RealType>() * x * x * x)) *
...
template <class RealType, class Policy>
inline RealType
cdf(const inverse_gaussian_distribution<RealType, Policy> & dist, const RealType & x)
{                      // Cumulative Density Function.
  ...
  // result = 0.5 * (erf(sqrt(scale / x) * ((x / mean) - 1) / constants::root_two<RealType>(),
  // Policy()) + 1)
  //  + exp(2 * scale / mean) / 2
...
struct inverse_gaussian_quantile_functor
{
  ...
  inverse_gaussian_quantile_functor(
      const boost::math::inverse_gaussian_distribution<RealType, Policy> dist, RealType const & p)
...
{
  inverse_gaussian_quantile_complement_functor(
      const boost::math::inverse_gaussian_distribution<RealType, Policy> dist, RealType const & p)
    : distribution(dist), prob(p)
  {
  }
...
inline RealType
guess_ig(RealType p, RealType mu = 1, RealType lambda = 1)
{ // guess at random variate value x for inverse gaussian quantile.
  ...
  typedef policy<overflow_error<ignore_error> // Ignore overflow (return infinity)
                 >
      no_overthrow_policy;
...
template <class RealType, class Policy>
inline RealType
quantile(const inverse_gaussian_distribution<RealType, Policy> & dist, const RealType & p)
{
  BOOST_MATH_STD_USING // for ADL of std functions.
  ...
  RealType scale = dist.scale();
  static const char * function =
      "boost::math::quantile(const inverse_gaussian_distribution<%1%>&, %1%)";
  ...
  // int digits = std::numeric_limits<RealType>::digits; // Maximum possible binary digits accuracy
  // for type T.
  ...
  using boost::math::tools::newton_raphson_iterate;
  result = newton_raphson_iterate(
      inverse_gaussian_quantile_functor<RealType, Policy>(dist, p), guess, min, max, get_digits, m);
  return result;
} // quantile

template <class RealType, class Policy>
inline RealType
cdf(const complemented2_type<inverse_gaussian_distribution<RealType, Policy>, RealType> & c)
{
  BOOST_MATH_STD_USING // for ADL of std functions.
  ...
  RealType x = c.param;
  static const char * function =
      "boost::math::cdf(const complement(inverse_gaussian_distribution<%1%>&), %1%)";
...
template <class RealType, class Policy>
inline RealType
quantile(const complemented2_type<inverse_gaussian_distribution<RealType, Policy>, RealType> & c)
{
  BOOST_MATH_STD_USING // for ADL of std functions
  ...
  RealType mean = c.dist.mean();
  static const char * function =
      "boost::math::quantile(const complement(inverse_gaussian_distribution<%1%>&), %1%)";
  RealType result = 0;
  if (false == detail::check_scale(function, scale, &result, Policy()))
    return result;
  if (false == detail::check_location(function, mean, &result, Policy()))
  ...
  using boost::math::tools::newton_raphson_iterate;
  result = newton_raphson_iterate(
      inverse_gaussian_quantile_complement_functor<RealType, Policy>(c.dist, q),
      guess,
      min,
      max,
      get_digits,
...
template <class RealType, class Policy>
inline RealType
mean(const inverse_gaussian_distribution<RealType, Policy> & dist)
{ // aka mu
  return dist.mean();
}
...
template <class RealType, class Policy>
inline RealType
scale(const inverse_gaussian_distribution<RealType, Policy> & dist)
{ // aka lambda
  return dist.scale();
}
...
template <class RealType, class Policy>
inline RealType
shape(const inverse_gaussian_distribution<RealType, Policy> & dist)
{ // aka phi
  return dist.shape();
}
...
template <class RealType, class Policy>
inline RealType
standard_deviation(const inverse_gaussian_distribution<RealType, Policy> & dist)
{
  BOOST_MATH_STD_USING
  RealType scale = dist.scale();
  RealType mean = dist.mean();
...
template <class RealType, class Policy>
inline RealType
mode(const inverse_gaussian_distribution<RealType, Policy> & dist)
{
  BOOST_MATH_STD_USING
  RealType scale = dist.scale();
  RealType mean = dist.mean();
  RealType result =
      mean * (sqrt(1 + (9 * mean * mean) / (4 * scale * scale)) - 3 * mean / (2 * scale));
  return result;
}

template <class RealType, class Policy>
inline RealType
skewness(const inverse_gaussian_distribution<RealType, Policy> & dist)
{
  BOOST_MATH_STD_USING
  RealType scale = dist.scale();
  RealType mean = dist.mean();
...
template <class RealType, class Policy>
inline RealType
kurtosis(const inverse_gaussian_distribution<RealType, Policy> & dist)
{
  RealType scale = dist.scale();
  RealType mean = dist.mean();
...
template <class RealType, class Policy>
inline RealType
kurtosis_excess(const inverse_gaussian_distribution<RealType, Policy> & dist)
{
  RealType scale = dist.scale();
  RealType mean = dist.mean();
