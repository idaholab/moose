...
find_scale( // For example, normal mean.
    ...
    typename Dist::value_type
        location, // location parameter, for example, normal distribution mean.
    const Policy & pol)
{
...
inline typename Dist::value_type
find_scale(complemented4_type<Real1, Real2, Real3, Policy> const & c)
{
...
//  << endl;
  ...
  static const char * function =
      "boost::math::find_scale<Dist, Policy>(complement(%1%, %1%, %1%, Policy))";
  ...
  return result;
} // template <class Dist, class Policy, class Real1, class Real2, class Real3> typename
  // Dist::value_type find_scale
...
inline typename Dist::value_type
find_scale(complemented3_type<Real1, Real2, Real3> const & c)
{
// cout << "cparam1 q " << c.param1 // q
//  << ", c.dist z " << c.dist // z
//  << ", c.param2 l " << c.param2 // l
...
//  << endl;
  ...
  if (!(boost::math::isfinite)(location))
  {
    return policies::raise_domain_error<typename Dist::value_type>(
        function,
        "find_scale location parameter was %1%, but must be finite!",
        location,
        policies::policy<>());
  }
  ...
  if (result <= 0)
  { // If policy isn't to throw, return the scale <= 0.
    policies::raise_evaluation_error<typename Dist::value_type>(
        function,
        "Computed scale (%1%) is <= 0!"
        " Was the complement intended?",
        result,
        policies::policy<>()); // This is only the default policy - also Want a version with Policy
                               // here.
  }
  return result;
} // template <class Dist, class Real1, class Real2, class Real3> typename Dist::value_type
  // find_scale
