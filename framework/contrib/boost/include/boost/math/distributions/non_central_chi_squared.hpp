...
T
non_central_chi_square_q(T x, T f, T theta, const Policy & pol, T init_sum = 0)
{
  //
  ...
  if (static_cast<boost::uintmax_t>(i - k) >= max_iter)
    return policies::raise_evaluation_error("cdf(non_central_chi_squared_distribution<%1%>, %1%)",
                                            "Series did not converge, closest value was %1%",
                                            sum,
                                            pol);
  //
...
template <class T, class Policy>
T
non_central_chi_square_p_ding(T x, T f, T theta, const Policy & pol, T init_sum = 0)
{
  //
...
template <class T, class Policy>
T
non_central_chi_square_p(T y, T n, T lambda, const Policy & pol, T init_sum)
{
  //
...
template <class T, class Policy>
T
non_central_chi_square_pdf(T x, T n, T lambda, const Policy & pol)
{
  //
  ...
  T sum = 0;
  int k = itrunc(l2);
  T pois = gamma_p_derivative(static_cast<T>(k + 1), l2, pol) *
           gamma_p_derivative(static_cast<T>(n2 + k), x2);
  if (pois == 0)
    return 0;
  T poisb = pois;
  for (int i = k;; ++i)
  {
    ...
    if (pois / sum < errtol)
      break;
    if (static_cast<boost::uintmax_t>(i - k) >= max_iter)
      return policies::raise_evaluation_error("pdf(non_central_chi_squared_distribution<%1%>, %1%)",
                                              "Series did not converge, closest value was %1%",
                                              sum,
                                              pol);
    pois *= l2 * x2 / ((i + 1) * (n2 + i));
...
template <class RealType, class Policy>
inline RealType
non_central_chi_squared_cdf(RealType x, RealType k, RealType l, bool invert, const Policy &)
{
  ...
  if (l == 0)
    return invert == false
               ? cdf(boost::math::chi_squared_distribution<RealType, Policy>(k), x)
               : cdf(complement(boost::math::chi_squared_distribution<RealType, Policy>(k), x));
  else if (x > k + l)
  {
    // Complement is the smaller of the two:
    result = detail::non_central_chi_square_q(static_cast<value_type>(x),
  ...
  else if (l < 200)
  {
    ...
    result = detail::non_central_chi_square_p_ding(static_cast<value_type>(x),
                                                   static_cast<value_type>(k),
                                                   static_cast<value_type>(l),
                                                   forwarding_policy(),
  ...
  else
  {
    ...
    result = detail::non_central_chi_square_p(static_cast<value_type>(x),
                                              static_cast<value_type>(k),
                                              static_cast<value_type>(l),
                                              forwarding_policy(),
                                              static_cast<value_type>(invert ? -1 : 0));
  ...
  return policies::checked_narrowing_cast<RealType, forwarding_policy>(
      result, "boost::math::non_central_chi_squared_cdf<%1%>(%1%, %1%, %1%)");
}
...
template <class RealType, class Policy>
RealType
nccs_quantile(const non_central_chi_squared_distribution<RealType, Policy> & dist,
              const RealType & p,
              bool comp)
{
  BOOST_MATH_STD_USING
  ...
  value_type r;
  if (!detail::check_df(function, k, &r, Policy()) ||
      !detail::check_non_centrality(function, l, &r, Policy()) ||
      !detail::check_probability(function, static_cast<value_type>(p), &r, Policy()))
    return (RealType)r;
  //
  ...
  // Hardeo Sahai and Mario Miguel Ojeda, Revista de Matematica: Teoria y Aplicaciones 2003 10(1-2)
  // : 57-76.
  // Note that the latter reference refers to an approximation of the CDF, when they really mean the
  // quantile.
  //
  ...
  if (comp)
  {
    guess =
        b +
        c * quantile(complement(chi_squared_distribution<value_type, forwarding_policy>(ff), p));
  }
  else
  {
  ...
  if (guess < 0.005)
  {
    ...
    guess = pow(pow(value_type(2), (k / 2 - 1)) * exp(l / 2) * pp * k *
                    boost::math::tgamma(k / 2, forwarding_policy()),
                (2 / k));
    if (guess == 0)
  ...
  return policies::checked_narrowing_cast<RealType, forwarding_policy>(result, function);
}

template <class RealType, class Policy>
RealType
nccs_pdf(const non_central_chi_squared_distribution<RealType, Policy> & dist, const RealType & x)
{
  BOOST_MATH_STD_USING
  ...
  if (!detail::check_df(function, k, &r, Policy()) ||
      !detail::check_non_centrality(function, l, &r, Policy()) ||
      !detail::check_positive_x(function, (value_type)x, &r, Policy()))
    return (RealType)r;

  if (l == 0)
    return pdf(boost::math::chi_squared_distribution<RealType, forwarding_policy>(
  ...
  return policies::checked_narrowing_cast<RealType, forwarding_policy>(r, function);
}
...
struct degrees_of_freedom_finder
{
  ...
  RealType operator()(const RealType & v)
  {
    ...
    return comp ? RealType(p - cdf(complement(d, x))) : RealType(cdf(d, x) - p);
  }
...
template <class RealType, class Policy>
inline RealType
find_degrees_of_freedom(RealType lam, RealType x, RealType p, RealType q, const Policy & pol)
{
  ...
  if ((p == 0) || (q == 0))
  {
    ...
    return policies::raise_evaluation_error<RealType>(
        ...
        RealType(std::numeric_limits<RealType>::quiet_NaN()),
        Policy());
  }
  ...
  if (guess < 1)
    guess = 1;
  std::pair<RealType, RealType> ir =
      tools::bracket_and_solve_root(f, guess, RealType(2), false, tol, max_iter, pol);
...
template <class RealType, class Policy>
inline RealType
find_non_centrality(RealType v, RealType x, RealType p, RealType q, const Policy & pol)
{
  ...
  if ((p == 0) || (q == 0))
  {
    ...
    return policies::raise_evaluation_error<RealType>(
        function,
        "Can't find non centrality parameter when the probability is 0 or 1, only possible answer "
        "is %1%",
        RealType(std::numeric_limits<RealType>::quiet_NaN()),
        Policy());
  }
...
public:
  ...
  {
    const char * function = "boost::math::non_central_chi_squared_distribution<%1%>::non_central_"
                            "chi_squared_distribution(%1%,%1%)";
    RealType r;
    detail::check_df(function, df, &r, Policy());
    detail::check_non_centrality(function, ncp, &r, Policy());
  ...
  static RealType find_degrees_of_freedom(RealType lam, RealType x, RealType p)
  {
    ...
    typedef typename policies::normalise<Policy,
                                         ...
                                         policies::discrete_quantile<>,
                                         policies::assert_undefined<>>::type forwarding_policy;
    eval_type result = detail::find_degrees_of_freedom(static_cast<eval_type>(lam),
  ...
  static RealType find_degrees_of_freedom(const complemented3_type<A, B, C> & c)
  {
    ...
    typedef typename policies::normalise<Policy,
                                         ...
                                         policies::discrete_quantile<>,
                                         policies::assert_undefined<>>::type forwarding_policy;
    eval_type result = detail::find_degrees_of_freedom(static_cast<eval_type>(c.dist),
  ...
  static RealType find_non_centrality(RealType v, RealType x, RealType p)
  {
    ...
    typedef typename policies::normalise<Policy,
                                         ...
                                         policies::discrete_quantile<>,
                                         policies::assert_undefined<>>::type forwarding_policy;
    eval_type result = detail::find_non_centrality(static_cast<eval_type>(v),
                                                   static_cast<eval_type>(x),
                                                   static_cast<eval_type>(p),
                                                   static_cast<eval_type>(1 - p),
                                                   forwarding_policy());
  ...
  static RealType find_non_centrality(const complemented3_type<A, B, C> & c)
  {
    ...
    typedef typename policies::normalise<Policy,
                                         ...
                                         policies::discrete_quantile<>,
                                         policies::assert_undefined<>>::type forwarding_policy;
    eval_type result = detail::find_non_centrality(static_cast<eval_type>(c.dist),
                                                   static_cast<eval_type>(c.param1),
                                                   static_cast<eval_type>(1 - c.param2),
                                                   static_cast<eval_type>(c.param2),
                                                   forwarding_policy());
...
typedef non_central_chi_squared_distribution<double>
    non_central_chi_squared; // Reserved name of type double.

// Non-member functions to give properties of the distribution.

template <class RealType, class Policy>
inline const std::pair<RealType, RealType>
range(const non_central_chi_squared_distribution<RealType, Policy> & /* dist */)
{ // Range of permissible values for random variable k.
  using boost::math::tools::max_value;
  return std::pair<RealType, RealType>(static_cast<RealType>(0),
                                       max_value<RealType>()); // Max integer?
}

template <class RealType, class Policy>
inline const std::pair<RealType, RealType>
support(const non_central_chi_squared_distribution<RealType, Policy> & /* dist */)
...
template <class RealType, class Policy>
inline RealType
mean(const non_central_chi_squared_distribution<RealType, Policy> & dist)
...
template <class RealType, class Policy>
inline RealType
mode(const non_central_chi_squared_distribution<RealType, Policy> & dist)
{ // mode.
...
template <class RealType, class Policy>
inline RealType
variance(const non_central_chi_squared_distribution<RealType, Policy> & dist)
{ // variance.
...
template <class RealType, class Policy>
inline RealType
skewness(const non_central_chi_squared_distribution<RealType, Policy> & dist)
{ // skewness = sqrt(l).
...
template <class RealType, class Policy>
inline RealType
kurtosis_excess(const non_central_chi_squared_distribution<RealType, Policy> & dist)
{
  const char * function =
      "boost::math::non_central_chi_squared_distribution<%1%>::kurtosis_excess()";
  RealType k = dist.degrees_of_freedom();
  RealType l = dist.non_centrality();
  RealType r;
  if (!detail::check_df(function, k, &r, Policy()) ||
...
template <class RealType, class Policy>
inline RealType
kurtosis(const non_central_chi_squared_distribution<RealType, Policy> & dist)
{
...
template <class RealType, class Policy>
inline RealType
pdf(const non_central_chi_squared_distribution<RealType, Policy> & dist, const RealType & x)
...
template <class RealType, class Policy>
RealType
cdf(const non_central_chi_squared_distribution<RealType, Policy> & dist, const RealType & x)
{
  ...
  if (!detail::check_df(function, k, &r, Policy()) ||
      !detail::check_non_centrality(function, l, &r, Policy()) ||
      !detail::check_positive_x(function, x, &r, Policy()))
    return r;
...
template <class RealType, class Policy>
RealType
cdf(const complemented2_type<non_central_chi_squared_distribution<RealType, Policy>, RealType> & c)
...
template <class RealType, class Policy>
inline RealType
quantile(const non_central_chi_squared_distribution<RealType, Policy> & dist, const RealType & p)
...
inline RealType
quantile(
    const complemented2_type<non_central_chi_squared_distribution<RealType, Policy>, RealType> & c)
{ // Quantile (or Percent Point) function.
