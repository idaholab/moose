...
inline bool
check_alpha(const char * function, const RealType & alpha, RealType * result, const Policy & pol)
{
  if (!(boost::math::isfinite)(alpha) || (alpha <= 0))
  {
    *result = policies::raise_domain_error<RealType>(
        function, "Alpha argument is %1%, but must be > 0 !", alpha, pol);
    return false;
  }
  return true;
} // bool check_alpha
...
template <class RealType, class Policy>
inline bool
check_beta(const char * function, const RealType & beta, RealType * result, const Policy & pol)
{
  if (!(boost::math::isfinite)(beta) || (beta <= 0))
  {
    *result = policies::raise_domain_error<RealType>(
        function, "Beta argument is %1%, but must be > 0 !", beta, pol);
    return false;
  }
  return true;
} // bool check_beta
...
template <class RealType, class Policy>
inline bool
check_x(const char * function, const RealType & x, RealType * result, const Policy & pol)
{
  if (!(boost::math::isfinite)(x) || (x < 0) || (x > 1))
  {
    *result = policies::raise_domain_error<RealType>(
        function, "x argument is %1%, but must be >= 0 and <= 1 !", x, pol);
    return false;
  }
  return true;
} // bool check_x
...
template <class RealType, class Policy>
inline bool
check_variance(const char * function,
               ...
               const Policy & pol)
{
  ...
  {
    *result = policies::raise_domain_error<RealType>(
        function, "variance argument is %1%, but must be > 0 !", variance, pol);
    return false;
  }
  return true;
} // bool check_variance
...
public:
  ...
  {
    RealType result;
    beta_detail::check_dist("boost::math::beta_distribution<%1%>::beta_distribution",
                            m_alpha,
                            m_beta,
                            &result,
                            Policy());
                             ...
                             RealType variance) // Expected value of variance.
  {
    ...
    if (false == (beta_detail::check_mean(function, mean, &result, Policy()) &&
                             ...
                             RealType probability) // cdf
  {
    static const char * function = "boost::math::beta_distribution<%1%>::find_alpha";
    RealType result = 0; // of error checks.
    if (false == (beta_detail::check_prob(function, probability, &result, Policy()) &&
...
range(const beta_distribution<RealType, Policy> & /* dist */)
...
template <class RealType, class Policy>
inline const std::pair<RealType, RealType>
support(const beta_distribution<RealType, Policy> & /* dist */)
...
template <class RealType, class Policy>
inline RealType
mean(const beta_distribution<RealType, Policy> & dist)
...
template <class RealType, class Policy>
inline RealType
variance(const beta_distribution<RealType, Policy> & dist)
...
template <class RealType, class Policy>
inline RealType
mode(const beta_distribution<RealType, Policy> & dist)
{
  ...
  {
    result = policies::raise_domain_error<RealType>(
        function, "mode undefined for alpha = %1%, must be > 1!", dist.alpha(), Policy());
    return result;
  }
  ...
  {
    result = policies::raise_domain_error<RealType>(
        function, "mode undefined for beta = %1%, must be > 1!", dist.beta(), Policy());
    return result;
  }
  RealType a = dist.alpha();
  RealType b = dist.beta();
...
//  return tools::domain_error<RealType>(function, "Median is not implemented, result is %1%!",
...
template <class RealType, class Policy>
inline RealType
skewness(const beta_distribution<RealType, Policy> & dist)
{
...
template <class RealType, class Policy>
inline RealType
kurtosis_excess(const beta_distribution<RealType, Policy> & dist)
{
  RealType a = dist.alpha();
  RealType b = dist.beta();
  RealType a_2 = a * a;
...
template <class RealType, class Policy>
inline RealType
kurtosis(const beta_distribution<RealType, Policy> & dist)
{
...
template <class RealType, class Policy>
inline RealType
pdf(const beta_distribution<RealType, Policy> & dist, const RealType & x)
{ // Probability Density/Mass Function.
  ...
  // Argument checks:
  RealType result = 0;
  if (false == beta_detail::check_dist_and_x(function, a, b, x, &result, Policy()))
  {
    return result;
  }
...
template <class RealType, class Policy>
inline RealType
cdf(const beta_distribution<RealType, Policy> & dist, const RealType & x)
...
template <class RealType, class Policy>
inline RealType
cdf(const complemented2_type<beta_distribution<RealType, Policy>, RealType> & c)
...
template <class RealType, class Policy>
inline RealType
quantile(const beta_distribution<RealType, Policy> & dist, const RealType & p)
{ // Quantile or Percent Point beta function or
  ...
  RealType a = dist.alpha();
  RealType b = dist.beta();
  if (false == beta_detail::check_dist_and_prob(function, a, b, p, &result, Policy()))
  {
    return result;
  }
  // Special cases:
...
template <class RealType, class Policy>
inline RealType
quantile(const complemented2_type<beta_distribution<RealType, Policy>, RealType> & c)
{ // Complement Quantile or Percent Point beta function .
  ...
  RealType a = dist.alpha();
  RealType b = dist.beta();
  if (false == beta_detail::check_dist_and_prob(function, a, b, q, &result, Policy()))
  {
    return result;
  }
  // Special cases:
