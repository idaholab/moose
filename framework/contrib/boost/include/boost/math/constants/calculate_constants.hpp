...
// template<int N>
// inline T
// constant_pow23_four_minus_pi<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
//{
//   BOOST_MATH_STD_USING
...
template <int N>
inline T
    constant_one_div_root_two_pi<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{
...
template <int N>
inline T
    constant_three_quarters_pi<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{
  BOOST_MATH_STD_USING
...
{
  BOOST_MATH_STD_USING
  return pow(pi<T, policies::policy<policies::digits2<N>>>(),
             e<T, policies::policy<policies::digits2<N>>>()); //
}
...
inline T constant_pi_sqr_div_six<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{
  BOOST_MATH_STD_USING
  return pi<T, policies::policy<policies::digits2<N>>>() *
         pi<T, policies::policy<policies::digits2<N>>>() / static_cast<T>(6); //
}
...
{
  BOOST_MATH_STD_USING
  return static_cast<T>(1) / pow(pi<T, policies::policy<policies::digits2<N>>>(),
                                 static_cast<T>(1) / static_cast<T>(3));
}
...
{
  BOOST_MATH_STD_USING
  return pow(e<T, policies::policy<policies::digits2<N>>>(),
             pi<T, policies::policy<policies::digits2<N>>>()); //
}
...
{
  BOOST_MATH_STD_USING
  return static_cast<T>(1) / log10(e<T, policies::policy<policies::digits2<N>>>());
}
...
{
  BOOST_MATH_STD_USING
  return static_cast<T>(1) / log((static_cast<T>(1) + sqrt(static_cast<T>(5))) / static_cast<T>(2));
}
...
inline T constant_zeta_three<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{
  ...
  //"1.202056903159594285399738161511449990,
  //76498629234049888179227155534183820578631309018645587360933525814619915"  A002117
  // 1.202056903159594285399738161511449990,
  // 76498629234049888179227155534183820578631309018645587360933525814619915780, +00);
  //"1.2020569031595942 double
  // http://www.spaennare.se/SSPROG/ssnum.pdf  // section 11, Algorithm for Apery's constant
  // zeta(3).
  ...
  { // three to five decimal digits per term, so 40 should be plenty for 100 decimal digits.
    // cout << "n = " << n << endl;
    n_fact *= n; // n!
    T n_fact_p10 = n_fact * n_fact * n_fact * n_fact * n_fact * n_fact * n_fact * n_fact * n_fact *
                   n_fact;                                 // (n!)^10
...
{ // http://oeis.org/A006752/constant
  ...
  // 9.159655941772190150546035149323841107,
  // 74149374281672134266498119621763019776254769479356512926115106248574422619, -01);
  ...
  return boost::math::constants::pi<T, boost::math::policies::policy<>>() *
             log(2 + boost::math::constants::root_three<T, boost::math::policies::policy<>>()) / 8 +
         3 * sum / 8;
}
...
template <class T>
T
zeta_polynomial_series(T s, T sc, int digits)
{
  BOOST_MATH_STD_USING
  //
...
template <class T>
template <int N>
inline T constant_extreme_value_skewness<T>::compute(
...
template <class T>
T
zeta_series_derivative_2(unsigned digits)
{
...
inline T constant_glaisher<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{
  ...
  v *= 6;
  v /= boost::math::constants::pi<T, forwarding_policy>() *
       boost::math::constants::pi<T, forwarding_policy>();
...
template <int N>
inline T
    constant_rayleigh_skewness<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{ // From e_float
...
template <class T>
template <int N>
inline T constant_rayleigh_kurtosis_excess<T>::compute(
    BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{ // - (6 Pi^2 - 24 Pi + 16)/((Pi - 4)^2)
  ...
  return -(((static_cast<T>(6) * pi<T, policies::policy<policies::digits2<N>>>() *
             pi<T, policies::policy<policies::digits2<N>>>()) -
            (static_cast<T>(24) * pi<T, policies::policy<policies::digits2<N>>>()) +
            static_cast<T>(16)) /
...
template <int N>
inline T
    constant_rayleigh_kurtosis<T>::compute(BOOST_MATH_EXPLICIT_TEMPLATE_TYPE_SPEC(mpl::int_<N>))
{ // 3 - (6 Pi^2 - 24 Pi + 16)/((Pi - 4)^2)
  // Might provide and calculate this using pi_minus_four.
  BOOST_MATH_STD_USING
  return static_cast<T>(3) -
         (((static_cast<T>(6) * pi<T, policies::policy<policies::digits2<N>>>() *
            pi<T, policies::policy<policies::digits2<N>>>()) -
           (static_cast<T>(24) * pi<T, policies::policy<policies::digits2<N>>>()) +
           static_cast<T>(16)) /
          ((pi<T, policies::policy<policies::digits2<N>>>() - static_cast<T>(4)) *
