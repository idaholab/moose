...
{
  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED explicit segmented_fold_fun(Fun const & f) : fun(f) {}
  ...
  struct apply
  {
    ...
    call(Sequence & seq, State const & state, Context const &, segmented_fold_fun const & fun)
    {
...
{
  typedef
      typename result_of::segmented_fold_until<Sequence, State, segmented_fold_fun<Fun>>::type type;
