...
#pragma wave option(preserve : 2,                                                                  \
                    line : 0,                                                                      \
                    output : "detail/preprocessed/zip" FUSION_MAX_ZIP_SEQUENCES_STR ".hpp")
#endif
...
struct zip<BOOST_PP_ENUM_PARAMS(ZIP_ITERATION, T) BOOST_PP_REPEAT_FROM_TO(
    BOOST_PP_DEC(ZIP_ITERATION), FUSION_MAX_ZIP_SEQUENCES, FUSION_TEXT, void_)>
{
    ...
    zip(BOOST_PP_ENUM_BINARY_PARAMS(ZIP_ITERATION, T, const & t))
{
  ...
  return typename result_of::zip<BOOST_PP_ENUM_PARAMS(ZIP_ITERATION, const T)>::type(seqs);
}
