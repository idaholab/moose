...
struct transform_view : sequence_base<transform_view<Sequence1, Sequence2, F>>
{
  ...
  typedef
      typename detail::strictest_traversal<fusion::vector2<Sequence1, Sequence2>>::type category;
  ...
  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED last1_type last1() const { return fusion::end(seq1); }
  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED last2_type last2() const { return fusion::end(seq2); }
...
#endif
{
  ...
  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED transform_view(Sequence & in_seq, F const & in_f)
    : seq(in_seq), f(in_f)
  {
  }
