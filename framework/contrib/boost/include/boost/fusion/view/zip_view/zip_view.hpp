...
struct all_references
    : fusion::result_of::equal_to<
          typename fusion::result_of::find_if<Sequences, mpl::not_<is_reference<mpl::_>>>::type,
...
struct seq_ref_size
{
  ...
  {
    static int const high_int = static_cast<int>((static_cast<unsigned>(~0) >> 1) - 1);
...
// never called, but needed for decltype-based result_of (C++0x)
  ...
  template <typename Seq>
  BOOST_FUSION_GPU_ENABLED typename result<seq_ref_size(Seq)>::type operator()(Seq &&) const;
#endif
};
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
  template <typename Lhs, typename Rhs>
  BOOST_FUSION_GPU_ENABLED typename result<poly_min(Lhs, Rhs)>::type operator()(Lhs &&,
                                                                                Rhs &&) const;
#endif
};
...
struct min_size
{
  ...
  typedef typename result_of::
      fold<sizes, typename result_of::front<sizes>::type, detail::poly_min>::type type;
};
}
...
struct zip_view : sequence_base<zip_view<Sequences>>
{
  ...
  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED zip_view(const Sequences & seqs) : sequences_(seqs){};
