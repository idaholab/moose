...
struct endpoints
{
  ...
  struct result<endpoints<M1>(SeqRef)> : mpl::eval_if<is_same<SeqRef, unused_type const &>,
  ...
  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED typename result<endpoints(Seq &)>::type
  ...
  template <typename Seq>
  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED typename result<endpoints(Seq const &)>::type
  operator()(Seq const & seq) const
  {
    return fusion::advance<M>(fusion::begin(seq));
  }

  BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED unused_type operator()(unused_type const &) const
  {
    return unused_type();
  }
};
...
struct end_impl<zip_view_tag>
{
  ...
  struct apply
  {
    ...
    BOOST_CONSTEXPR BOOST_FUSION_GPU_ENABLED static type call(Sequence & sequence)
    {
      return type(
