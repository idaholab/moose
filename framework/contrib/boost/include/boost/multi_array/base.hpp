...
struct mutable_iterator_tag : boost::random_access_traversal_tag, std::input_iterator_tag
{
...
////////////////////////////////////////////////////////////////////////
template <typename T, std::size_t NumDims>
class multi_array_impl_base : public value_accessor_generator<T, mpl::size_t<NumDims>>::type
{
...
//
  ...
  typedef array_iterator<T, T *, mpl::size_t<NumDims>, reference, mutable_iterator_tag> iterator;
#else
...
protected:
  ...
  void compute_strides(StrideList & stride_list,
  ...
  {
    return calculate_descending_dimension_offset(stride_list, extent_list, storage) +
  ...
  template <typename StrideList, typename ExtentList>
  index calculate_descending_dimension_offset(const StrideList & stride_list,
                                              const ExtentList & extent_list,
                                              const general_storage_order<NumDims> & storage)
  {
    index offset = 0;
                      ...
                      TPtr base) const
  {
    ...
    for (size_type n = 0; n != NumDims; ++n)
    {
      ...
      BOOST_ASSERT(index_bases[n] <= start && ((start <= index_bases[n] + index(extents[n])) ||
