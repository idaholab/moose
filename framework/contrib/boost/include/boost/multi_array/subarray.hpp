...
public:
  ...
  const_sub_array(const const_sub_array<T, NumDims, OPtr> & rhs)
    : base_(rhs.base_), extents_(rhs.extents_), strides_(rhs.strides_), index_base_(rhs.index_base_)
  {
  }
  ...
  const_reference operator[](index idx) const
  {
    return super_type::access(
        boost::type<const_reference>(), idx, base_, shape(), strides(), index_bases());
  }
  ...
  operator[](const boost::detail::multi_array::index_gen<NumDims, NDims> & indices) const
  {
    ...
    return super_type::generate_array_view(
        boost::type<return_type>(), indices, shape(), strides(), index_bases(), base_);
  }
  ...
  size_type num_elements() const
  {
    return std::accumulate(
        shape(), shape() + num_dimensions(), size_type(1), std::multiplies<size_type>());
  }
...
public: // Should be protected
  ...
  const_sub_array(TPtr base,
    ...
    : base_(base), extents_(extents), strides_(strides), index_base_(index_base)
  {
  }
...
public:
  ...
  {
    function_requires<
        boost::multi_array_concepts::ConstMultiArrayConcept<ConstMultiArray, NumDims>>();
  ...
  operator[](const boost::detail::multi_array::index_gen<NumDims, NDims> & indices)
  {
  ...
  const_reference operator[](index idx) const { return super_type::operator[](idx); }
...
public: // should be private
  ...
  sub_array(T * base, const size_type * extents, const index * strides, const index * index_base)
