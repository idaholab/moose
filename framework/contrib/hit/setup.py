...
class BuildWithWasp(build_ext):
    def run(self):
        ...
        # Build hit executable and copy src, include, and executable to package distribution
        self._build_hit()
    ...
    def _build_wasp(self) -> tuple[str, str, list[str]]:
        ...
        env["WASP_PREFIX"] = self.wasp_install_dir
        ...
        incdir = os.path.join(self.wasp_install_dir, "include")
        libdir = os.path.join(self.wasp_install_dir, "lib")
    ...
    def _build_hit(self):
        # Build executable
        env = os.environ.copy()
        env["WASP_DIR"] = self.wasp_install_dir
        subprocess.run(["make", "hit"], env=env, check=True, cwd=THIS_DIR)

        # Move executable
        os.makedirs(self.hit_install_dir, exist_ok=True)
        shutil.copy2(os.path.join(THIS_DIR, "hit"), os.path.join(self.hit_install_dir, "hit"))

        # Copy src and include directories
        for dir in ["src", "include"]:
            shutil.copytree(
                os.path.join(THIS_DIR, dir),
                os.path.join(self.hit_install_dir, dir),
                dirs_exist_ok=True,
            )
...
setup(
    name="moose-hit",
    ext_modules=[ext],
    cmdclass={"build_ext": BuildWithWasp},
)
