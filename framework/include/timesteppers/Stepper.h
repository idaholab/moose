...
/// Holds all information used by Steppers to calculate dt via the "advance" function.
struct StepperInfo {
  ...
  double prev_dt;
  /// dt used between solves for the second most and third most recent time steps.
  double prev_prev_dt;
  /// dt used between solves for the third most and fourth most recent time steps.
  ...
  /// If no predictor was used, this is a zero vector with the same length as soln_nonlin.
  std::unique_ptr<NumericVector<Number> > soln_predicted;
};

/// A base class for time stepping algorithms for use in determining dt between time steps of a transient problem solution.  Implementations should strive to be immutable - facilitating easier restart/recovery and testing.
class Stepper
{
public:
  ...
  /// Returns a value for dt to calculate the next time step.  Implementations can assume si is not NULL.  Implementations of advance should strive to be idempotent.
...
/// use the dt_store pointer.  This stepper enables steppers at one layer of nesting to base their dt calculations on dt values computed at a different layer of nesting.
...
/// previous dt was not what the underlying stepper returned on the prior call to
/// advance, this stepper returns/retries that dt value.
class RetryUnusedStepper : public Stepper
{
public:
  /// If prev_prev is true, on retry cases, "advance" will return the last returned dt that was used (i.e. prev_prev_dt) instead of the last returned dt that was not used.
...
/// Uses user-specified (time,dt) points to do a piece-wise linear interpolation to calculate dt.
class PiecewiseStepper : public Stepper
{
public:
  /// If interpolate is false, then this finds the pair of values in the times vector that the current simulation time resides between and returns the dt at the index of the lower bound.
...
/// MinOfStepper returns the smaller of two dt's from two underlying steppers (one of them preferred).
/// It returns the preferred stepper's dt if "dt_preferred - tolerance < dt_alternate".
...
/// Computes dt adaptively based on the number of linear and non-linear iterations that were required to converge on the most recent solve.  Too many iterations results in dt contraction, too few iterations results in dt growth.
/// For algorithm details, read the code.
class AdaptiveStepper : public Stepper
{
public:
  /// shrink_factor must be between 0 and 1.0.  growth_factor must be greater than or equal to 1.0.
  AdaptiveStepper(
...
/// Adjusts dt in order to minimize the quantity "solve_time / prev_dt" by multiplying dt by "1 +/- frac_change" if the ratio increases on consecutive time steps.
class SolveTimeAdaptiveStepper : public Stepper
{
public:
  /// initial_direc must be either +1.0 or -1.0 indicating whether initial adjustments to dt should be an increase or decrease.  frac_change should generally be between 0.0 and 1.0.
...
/// Returns an initial, user-specified dt until the si.step_count exceeds a specified number (not a number of calls to "advance") after which it returns the dt calculated by an underlying stepper.
class StartupStepper : public Stepper
{
public:
  /// n_steps is the number of steps to return initial_dt before defaulting to returning dt from s.
...
/// Calls one underlying stepper to calculate dt if the last solve converged, and another otherwise.
...
/// If the solve converged, multiply the previous dt by a growth factor, otherwise multiply it by a shrink factor.
class GrowShrinkStepper : public Stepper
{
public:
  /// shrink_factor must be between 0.0 and 1.0.  growth_factor must be greater than or equal to 1.0.
  /// If source_dt != nullptr, the dt returned by source_dt is used to grow or shrink
...
/// Uses the error between a predictor's solution and the actual last solution to compute adjustments to dt.
/// In order to use this stepper, an appropriate predictor must have been added to the simulation problem.
class PredictorCorrectorStepper : public Stepper
{
public:
  /// start_adapting is the first si.step_count to start doing dt corrector calculations for - otherwise, prev_dt is returned by "advance".  For details on e_tol and scaling_param usage - divine it from the code.
