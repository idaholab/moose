 ...
 * two overlapping blocks. It works by allowing you to modify the residual
 * and jacobian entries on "this" side (the node side, also referred to as
 * the slave side) and the "other" side (the elem side, also referred to as
 * the master side)
 */
...
public:
   ...
   *
   * Get's called once per slave node per variable.
   */
  virtual bool shouldApply() { return true; }

  /**
   * Compute contact force of the constraint depending on the formulation
   */
