...
public:
   ...
   */
  Point getIntersection(Point & p1, Point & p2, Point & q1, Point & q2, Real & s);

  /**
   * Check that a point is inside the secondary polygon (for verification only)
   */
  bool isInsideSecondary(Point & pt);

  /**
   * Checks whether polygons are disjoint for an easy out
   */
  bool isDisjoint(std::vector<Point> & poly);
   ...
   */
  void triangulatePoly(std::vector<Point> & nodes, std::vector<std::array<int, 3>> & tri_map);

  /**
   * Get mortar segments generated by a secondary and primary node pair
   */
  void getMortarSegments(const Elem * primary_elem_ptr,
   ...
   */
  Real polyArea(std::vector<Point> & nodes);

  /**
   * Get area fraction remaining after clipping against primary elements
   */
  Real getRemainder() { return _remaining_area_fraction; }
