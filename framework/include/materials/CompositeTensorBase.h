...
CompositeTensorBase<T, U>::computeQpTensorProperties(MaterialProperty<T> & M, Real derivative_prefactor)
{
  ...
  for (unsigned int i = 0; i < _num_comp; ++i)
  {
    ...
    for (unsigned int j = 0; j < _num_args; ++j)
    {
      ...
      (*_dM[j])[qp] += derivative_prefactor * ((*_tensors[i])[qp] * (*_dweights[i][j])[qp] + (*_dtensors[i][j])[qp] * (*_weights[i])[qp]);
      ...
      for (unsigned int k = 0; k <= j; ++k)
      {
        ...
        (*_d2M[j][k])[qp] += derivative_prefactor * (2.0 * (*_dtensors[i][j])[qp] * (*_dweights[i][j])[qp] + (*_tensors[i])[qp] * (*_d2weights[i][j][k])[qp] + (*_d2tensors[i][j][k])[qp] * (*_weights[i])[qp]);
      }
    }
  }
}
