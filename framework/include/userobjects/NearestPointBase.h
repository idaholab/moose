...
NearestPointBase<UserObjectType, BaseType>::validParams()
{
  ...
  params.addParam<PositionsName>("positions_object",
                                 "The name of a Positions object that will contain "
                                 "the locations. This and 'points(_file)' cannot be both supplied");
...
NearestPointBase<UserObjectType, BaseType>::fillPoints()
{
  ...
  else if (isParamValid("positions_object"))
  {
    ...
    _points = problem->getPositionsObject(positions_name).getPositions(true);
  }
  else
    mooseError(
        name(),
