...
public:
  ...
  /**
   * @returns Whether or not the MeshGeneartor with the name \p name is a parent of this
   * MeshGenerator.
   */
...
protected:
   ...
   *
   * You _must_ delcare all such MeshGenerators that are passed by parameter to this
  ...
  std::unique_ptr<MeshBase> & addMeshSubgenerator(const std::string & type,
   ...
   * @param type the type of of the subgenerator
...
MeshGenerator::setMeshProperty(const std::string & data_name, Args &&... args)
{
  ...
  RestartableData<T> * T_value = dynamic_cast<RestartableData<T> *>(value);
  if (!T_value)
    mooseError("While retreiving mesh property '",
               data_name,
               "' with type '",
               MooseUtils::prettyCppType<T>(),
                                   ...
                                   Ts... extra_input_parameters)
{
  ...
  return this->addMeshSubgenerator(type, name, subgenerator_params);
}
