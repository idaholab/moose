...
public:
   ...
   */
  RestartableDataValue(const std::string & name, const Moose::AnyPointer context);
   ...
   */
  Moose::AnyPointer context() { return _context; }

  /**
   * @return Whether or not the data has context set.
   */
  bool hasContext() const { return _context.hasValue(); }
...
protected:
  ...
  const Moose::AnyPointer _context;
...
public:
  ...
  RestartableData(const std::string & name, const Moose::AnyPointer context, Params &&... args)
