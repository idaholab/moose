...
protected:
  ...
  ///@{
  /**
   * Return the ControllableParameter object. Generally, these methods should be avoided if
   * possible.
   *
   * However, in some cases (like when the type is not known) to interrogate the actual object.
   */
  template <typename T>
  ControllableParameter<T> getControllableParameter(const std::string & name,
                                                    bool warn_when_values_differ = false);

  template <typename T>
  ControllableParameter<T> getControllableParameterByName(const std::string & name,
                                                          bool warn_when_values_differ = false);

  template <typename T>
  ControllableParameter<T> getControllableParameterByName(const std::string & object_name,
                                                          const std::string & param_name,
                                                          bool warn_when_values_differ = false);

  template <typename T>
  ControllableParameter<T> getControllableParameterByName(const MooseObjectName & object_name,
                                                          const std::string & param_name,
                                                          bool warn_when_values_differ = false);

  template <typename T>
  ControllableParameter<T> getControllableParameterByName(const std::string & tag,
                                                          const std::string & object_name,
                                                          const std::string & param_name,
                                                          bool warn_when_values_differ = false);

  template <typename T>
  ControllableParameter<T> getControllableParameterByName(const MooseObjectParameterName & name,
                                                          bool warn_when_values_differ = false);
  ///@}
...
{
  MooseObjectParameterName desired(name);
  return getControllableParameterHelper<T>(
      desired, warn_when_values_differ, /*mark_as_set=*/true, /*error_on_empty=*/false);
}

template <typename T>
ControllableParameter<T>
Control::getControllableParameterByName(const std::string & object_name,
                                        const std::string & param_name,
                                        bool warn_when_values_differ /*=false*/)
{
  MooseObjectParameterName desired(MooseObjectName(object_name), param_name);
...
template <typename T>
ControllableParameter<T>
Control::getControllableParameterByName(const MooseObjectParameterName & desired,
                                        bool warn_when_values_differ /*=false*/)
{
  return getControllableParameterHelper<T>(
      desired, warn_when_values_differ, /*mark_as_set=*/true, /*error_on_empty=*/false);
