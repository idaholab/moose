...
public:
  ...
  GPUMaterial(const InputParameters & parameters)
    : GPUMaterialBase(parameters),
      ...
      _default_init(&Material::initQpStatefulProperties == &GPUMaterial::initQpStatefulProperties),
...
protected:
  ...
  getGPUGenericMaterialPropertyByName(const std::string & prop_name_in)
  {
    ...
    auto prop = MaterialPropertyInterface::getGPUGenericMaterialPropertyByName<T, dimension, state>(
        prop_name);
