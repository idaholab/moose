...
public:
   ...
   */
  MemoryPool(size_t size) : _pool(_space, size) {}
   ...
   */
  template <typename T>
  KOKKOS_FUNCTION MemoryChunk<T> allocate(size_t size) const
  {
    return MemoryChunk<T>(_pool, size * sizeof(T));
  }

private:
  /**
   * Kokkos memory pool space
   */
  ::Kokkos::DefaultExecutionSpace::memory_space _space;
  /**
   * Kokkos memory pool
   */
  ::Kokkos::MemoryPool<MemSpace> _pool;
};
