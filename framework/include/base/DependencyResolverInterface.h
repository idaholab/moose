...
public:
  ...
  /**
   * A method for sorting object based on the items requested or supplied
   */
    ...
    for (Iter iter = start; iter != end ; ++iter)
    {
      ...
      for (Iter iter2 = start; iter2 != end; ++iter2)
      {
        ...
        if (debug)
        {
          ...
          Moose::out << (*iter)->name() << " -> " << (*iter2)->name() << std::endl;
          Moose::out << (*iter)->name() << '\n';
        ...
        if (!intersect.empty())
        {
          ...
          // There is an allowable dependency if the intersection contains items
          if (!iterative_intersect.empty())
          {
            ...
            // Continue to the next set of items
            continue;
          }
