...
#pragma once
/**
 * A node in the SolutionInvalidity.  Stores the warning message for a particular section of code.
 *
...
public:
  /**
   * Create a SolutionNode with the given ID
   */
  SolutionNode(const PerfID id) : _id(id), _total_time(0), _num_calls(0), _total_memory(0) {}
   ...
   */
  SolutionNode * getChild(const PerfID id)
  {
    // RHS insertion on purpose
    auto & child_node = _children[id];

    if (!child_node)
      child_node.reset(new SolutionNode(id));
   ...
   * Get the children
   */
  const std::map<PerfID, std::unique_ptr<SolutionNode>> & children() const { return _children; }
...
private:
  ...
  std::map<PerfID, std::unique_ptr<SolutionNode>> _children;

  friend void dataStore(std::ostream &, const std::unique_ptr<SolutionNode> &, void *);
  friend void dataLoad(std::istream &, const std::unique_ptr<SolutionNode> &, void *);
};

void dataStore(std::ostream & stream, const std::unique_ptr<SolutionNode> & node, void * context);
void dataLoad(std::istream & stream, const std::unique_ptr<SolutionNode> & node, void * context);
