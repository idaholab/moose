...
template<>
void mooseSetToZero<RankFourTensorTempl<DualReal>>(
		RankFourTensorTempl<DualReal> &v);
}
...
{
	return MooseEnum(
			"antisymmetric symmetric9 symmetric21 general_isotropic symmetric_isotropic "
...
auto RankFourTensorTempl<T>::operator+(
		const RankFourTensorTempl<T2> &b) const ->
	...
	for (unsigned int i = 0; i < N; ++i)
		for (unsigned int j = 0; j < N; ++j)
			for (unsigned int k = 0; k < N; ++k)
				...
				{
					if (i == j)
						mat[k == l ? i * ntens + k : i * ntens + k + nskip + l] +=
								_vals[index];
					else
						// i!=j
						mat[k == l ?
								(nskip + i + j) * ntens + k :
								(nskip + i + j) * ntens + k + nskip + l] +=
			...
			for (unsigned int k = 0; k < N; ++k)
				for (unsigned int l = 0; l < N; ++l)
				{
					if (i == j)
						...
						result._vals[index] =
								k == l ?
										mat[(nskip + i + j) * ntens + k] :
										mat[(nskip + i + j) * ntens + k + nskip
												+ l] / 2.0;
					index++;
				}
...
template<typename T>
void RankFourTensorTempl<T>::print(std::ostream &stm) const
{
	for (unsigned int i = 0; i < N; ++i)
		for (unsigned int j = 0; j < N; ++j)
		{
		...
		const std::vector<T> &input)
{
	...
	else if (input.size() == 2)
	{
		...
		mooseError(
				"Please provide correct number of inputs for surface RankFourTensorTempl<T> "
						"initialization.");
}
		...
		FillMethod fill_method)
{
	...
	case orthotropic:
		...
		break;
	default:
		mooseError("fillFromInputVector called with unknown fill_method of ",
				fill_method);
	}
}
...
template<typename T>
void RankFourTensorTempl<T>::fillAntisymmetricFromInputVector(
		const std::vector<T> &input)
{
	if (input.size() != 6)
		mooseError(
...
template<typename T>
void RankFourTensorTempl<T>::fillGeneralIsotropicFromInputVector(
		const std::vector<T> &input)
{
	if (input.size() != 3)
		mooseError(
				"To use fillGeneralIsotropicFromInputVector, your input must have size 3.  Yours "
						"has size ", input.size());
...
template<typename T>
void RankFourTensorTempl<T>::fillGeneralIsotropic(T i0, T i1, T i2)
{
	for (unsigned int i = 0; i < N; ++i)
		for (unsigned int j = 0; j < N; ++j)
			for (unsigned int k = 0; k < N; ++k)
				for (unsigned int l = 0; l < N; ++l)
				{
					(*this)(i, j, k, l) = i0 * Real(i == j) * Real(k == l)
							+ i1 * Real(i == k) * Real(j == l)
							+ i1 * Real(i == l) * Real(j == k);
					for (unsigned int m = 0; m < N; ++m)
						(*this)(i, j, k, l) += i2
								* Real(PermutationTensor::eps(i, j, m))
								* Real(PermutationTensor::eps(k, l, m));
				}
}

template<typename T>
void RankFourTensorTempl<T>::fillAntisymmetricIsotropicFromInputVector(
		const std::vector<T> &input)
{
	if (input.size() != 1)
		mooseError(
				"To use fillAntisymmetricIsotropicFromInputVector, your input must have size 1. "
						"Yours has size ", input.size());

	fillGeneralIsotropic(0.0, 0.0, input[0]);
}

template<typename T>
void RankFourTensorTempl<T>::fillAntisymmetricIsotropic(T i0)
{
	fillGeneralIsotropic(0.0, 0.0, i0);
}

template<typename T>
void RankFourTensorTempl<T>::fillSymmetricIsotropicFromInputVector(
		const std::vector<T> &input)
{
...
template<typename T>
void RankFourTensorTempl<T>::fillSymmetricIsotropic(T lambda, T G)
{
	fillSymmetric9FromInputVector(
			{ lambda + 2.0 * G, lambda, lambda, lambda + 2.0 * G, lambda, lambda
					+ 2.0 * G, G, G, G });
}

template<typename T>
void RankFourTensorTempl<T>::fillSymmetricIsotropicEandNuFromInputVector(
		const std::vector<T> &input)
{
	if (input.size() != 2)
		mooseError(
...
template<typename T>
void RankFourTensorTempl<T>::fillSymmetricIsotropicEandNu(T E, T nu)
{
...
template<typename T>
void RankFourTensorTempl<T>::fillAxisymmetricRZFromInputVector(
		const std::vector<T> &input)
{
	...
	fillSymmetric9FromInputVector(
	{ input[0], input[1], input[2], input[0], input[2],
	// C3333     C2323     C3131=C2323   C1212
			input[3], input[4], input[4], (input[0] - input[1]) * 0.5 });
}

template<typename T>
void RankFourTensorTempl<T>::fillGeneralFromInputVector(
		const std::vector<T> &input)
{
	if (input.size() != 81)
		mooseError(
...
template<typename T>
void RankFourTensorTempl<T>::fillPrincipalFromInputVector(
		const std::vector<T> &input)
{
	if (input.size() != 9)
		mooseError(
...
template<typename T>
void RankFourTensorTempl<T>::fillGeneralOrthotropicFromInputVector(
		const std::vector<T> &input)
{
	if (input.size() != 12)
		mooseError(
				"To use fillGeneralOrhotropicFromInputVector, your input must have size 12. Yours has size ",
				input.size());
	...
	bool preserve_symmetry = MooseUtils::absoluteFuzzyEqual(nuab * Eb,
			nuba * Ea) && MooseUtils::absoluteFuzzyEqual(nuca * Ea, nuac * Ec)
			&& MooseUtils::absoluteFuzzyEqual(nubc * Ec, nucb * Eb);

	if (!preserve_symmetry)
		mooseError(
				"Orthotropic elasticity tensor input is not consistent with symmetry requirements. Check for input for accuracy");
	...
	T k = 1 - nuab * nuba - nubc * nucb - nuca * nuac - nuab * nubc * nuca
			- nuba * nucb * nuac;

	bool is_positive_definite = (k > 0) && (1 - nubc * nucb) > 0
			&& (1 - nuac * nuca) > 0 && (1 - nuab * nuba) > 0;
	if (!is_positive_definite)
		mooseError(
				"Orthotropic elasticity tensor input is not positive definite. Check for input for accuracy");
...
T RankFourTensorTempl<T>::sum3x3() const
{
	...
	// correction
	return (*this)(0, 0, 0, 0) + (*this)(0, 0, 1, 1) + (*this)(0, 0, 2, 2)
			+ (*this)(1, 1, 0, 0) + (*this)(1, 1, 1, 1) + (*this)(1, 1, 2, 2)
			+ (*this)(2, 2, 0, 0) + (*this)(2, 2, 1, 1) + (*this)(2, 2, 2, 2);
}
...
bool RankFourTensorTempl<T>::isIsotropic() const
{
	...
	if ((*this)(1, 2, 1, 2) != mu || (*this)(2, 0, 2, 0) != mu)
		return false;
	// ...off-diagonal
	if ((*this)(2, 0, 1, 2) != 0.0 || (*this)(0, 1, 1, 2) != 0.0
			|| (*this)(0, 1, 2, 0) != 0.0)
		return false;
	...
	if (!MooseUtils::relativeFuzzyEqual(K1 - 4.0 * mu / 3.0,
			K2 + 2.0 * mu / 3.0))
		return false;
