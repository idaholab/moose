 ...
 *
 * This class assumes that all data is a string. If a Communicator is provided
 * then it will only read on processor 0 and broadcast the data to all
 * processors. If not provided it will read on all processors.
 */
class DelimitedFileReaderOfString
{
public:
   ...
   */
  const std::vector<std::string> & getData(const std::string & name) const;
  const std::vector<std::string> & getData(std::size_t index) const;
  ///@}
...
private:
   ...
   */
  void readColumnData(std::ifstream & stream_data, std::vector<std::string> & output);
  void readRowData(std::ifstream & stream_data, std::vector<std::string> & output);
  ///@}
