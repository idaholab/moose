...
public:
   ...
   * Default constructor
   */
  Map() : _map_host(std::make_shared<std::unordered_map<T1, T2>>()) {}
...
private:
   ...
   */
  std::shared_ptr<std::unordered_map<T1, T2>> _map_host;
  /**
   * Keys on device
   */
  Array<T1> _keys;
...
dataStore(std::ostream & stream, Map<T1, T2> & map, void * context)
{
  ...
  dataStore(stream, *map._map_host, context);
...
dataLoad(std::istream & stream, Map<T1, T2> & map, void * context)
{
  ...
  dataLoad(stream, *map._map_host, context);
