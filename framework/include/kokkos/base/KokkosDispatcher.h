...
public:
  ...
  virtual std::shared_ptr<DispatcherBase> build(const void * object) = 0;
};
...
{
public:
  virtual std::shared_ptr<DispatcherBase> build(const void * object)
  {
    return std::make_shared<Dispatcher<Operation, Object>>(object);
  }
};
...
public:
  ...
  {
    getRegistry()._dispatchers[std::make_pair(typeid(Operation).name(), name)] =
        std::make_unique<DispatcherRegistryEntry<Operation, Object>>();
  }

  template <typename Operation>
  static void hasUserMethod(const std::string & name, bool overriden)
  {
    getRegistry()._user_override[std::make_pair(typeid(Operation).name(), name)] = overriden;
  }

  template <typename Operation>
  static bool hasUserMethod(const std::string & name)
  {
    auto it = getRegistry()._user_override.find(std::make_pair(typeid(Operation).name(), name));

    return it == getRegistry()._user_override.end() ? false : it->second;
  }

  template <typename Operation>
  static std::shared_ptr<DispatcherBase> build(const void * object, const std::string & name)
  {
    auto it = getRegistry()._dispatchers.find(std::make_pair(typeid(Operation).name(), name));
...
private:
  std::map<std::pair<std::string, std::string>, std::unique_ptr<DispatcherRegistryEntryBase>>
      _dispatchers;
  std::map<std::pair<std::string, std::string>, bool> _user_override;
};
