...
public:
  ...
  /**
   * The Kokkos matrix-free Jacobian-vector product is not supported for nodal kernels; this
   * exists only to satisfy the shared dispatcher registration macro's compile-time requirements.
   */
  template <typename Derived>
  KOKKOS_FUNCTION void operator()(JacobianVectorProductLoop, const ThreadID, const Derived &) const
  {
    ::Kokkos::abort("The Kokkos matrix-free Jacobian-vector product is not supported for nodal "
                    "kernels.");
  }
  /**
   * The Kokkos matrix-free Jacobian diagonal is not supported for nodal kernels; this exists
  ...
  {
    ::Kokkos::abort("The Kokkos matrix-free Jacobian diagonal is not supported for nodal "
                    "kernels.");
  }
  ///@}
