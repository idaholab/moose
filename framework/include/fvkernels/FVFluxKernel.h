...
{
public:
  /// This codifies a set of available ways to interpolate with left+right
...
protected:
  ...
  /// face value needs to be computed using left and right information (e.g. a
  /// material property, solution value, etc.).  left and right represent the
  /// property/value to compute the face value for.
  template <typename T>
  void interpolate(InterpMethod m, T & result, const T & left, const T & right)
  {
    switch (m)
    {
  ...
  /// u_left and u_right.  The interpolated value is stored in result.  left
  /// and right represent the property/value being advected in the left and
  ...
  template <typename T>
  void interpolate(
      InterpMethod m, T & result, const T & left, const T & right, ADRealVectorValue advector)
  {
    switch (m)
    {
      case InterpMethod::Average:
