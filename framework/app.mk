...
csrcfiles   := $(filter %.c, $(other_src_files))
fsrcfiles   := $(filter %.f, $(other_src_files))
f90srcfiles := $(filter %.f90, $(other_src_files))
...
all_test_files   := $(shell find $(TEST_SRC_DIRS) -type f 2>/dev/null)
...
# plugin files
all_plugins := $(shell find $(PLUGIN_DIR) -type f 2>/dev/null)
plugfiles   := $(filter-out "#%.C ~%.C", $(filter %.C, $(all_test_files)))
