...
def parse_gcov(fname):
    ...
    branch_tuple = (
        None  # (num_executions, file, line, code, percentage) of the current bracnh
    )
    ...
    for line in lines:
        if branch:
            ...
            if words[0] != "call" and words[0] != "branch":
                branch = False
            if (
                words[0] == "branch" and words[-1] != "executed"
            ):  # it could be "branch # never executed"
...
def print_results(results):
    ...
    cutoff = min(
        len(results),
        options.min if options.min > 0 else int(options.perc / 100.0 * len(results)),
    )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="filter",
        default=False,
        help="Show ifs that are already marked likely or unlikely [default=FALSE]",
    )
    parser.add_option(
        "-p",
        "--perc-branches",
        ...
        dest="perc",
        default=10,
        help="Only show the top percentage of branches [default=10%]",
    )
    parser.add_option(
        "-m",
        "--min-branches",
        ...
        dest="min",
        default=-1,
        help="Only show ifs that have been executed more than num times [default use percentage]",
    )
    parser.add_option(
        "-u",
        "--use-existing",
        ...
        dest="gcov",
        default=True,
        help="Don't run gcov, just parse all .gcov files already there",
    )
    parser.add_option(
        "-e",
        "--expanded-search",
        ...
        dest="expand",
        default=False,
        help="By default it will only look at .gcov files from .C or .gcno files, with this option look in *all* the generated .gcov files (ie included headers)",
    )
    parser.add_option(
        "-f",
        "--output-file",
        action="store",
        dest="output",
        default="likely_branches.out",
        metavar="FILE",
        help="outputs a list of all the branches ordered by execution counts",
    )
