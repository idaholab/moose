...
def shellCommand(command, cwd=None):
    ...
    with open(os.devnull, "w") as devnull:
        ...
        if retcode != 0:
            raise Exception(
                "Exception raised while running the command: "
                + command
                + " in directory: "
                + cwd
            )
...
class VersionInfo:
    ...
    def gitSHA1(self, cwd=None):
        ...
        if self.SHA1 == None:
            try:
                ...
                self.SHA1 = shellCommand(
                    "git show " + self.hideSignature() + " -s --format=%h", cwd
                ).strip()
    ...
    def gitDate(self, cwd=None):
        try:
            ...
            return shellCommand(
                "git show " + self.hideSignature() + " -s --format=%ci", cwd
            ).split()[0]
        except:  # subprocess.CalledProcessError:
            return None
    ...
    def gitVersionString(self, cwd=None):
        ...
        try:
            description = shellCommand(
                'git describe --tags --long --match "v[0-9]*"', cwd
            ).rsplit("-", 2)
            tag = description[0] + ", "
    ...
    def gitSvnVersionString(self, cwd=None):
        try:
            ...
            revision = shellCommand(
                "git svn find-rev $(git log --max-count 1 --pretty=format:%H)", cwd
            ).strip()
        ...
        if revision_changed:
            ...
            for touch_file in timestamps_to_modify:
                if os.path.isfile(os.path.join(self.repo_location, touch_file)):
                    shellCommand(
                        "touch -r " + self.header_file + " " + touch_file,
                        self.repo_location,
                    )
