...
def get_registered_apps(parts_list):
    ...
    """
    parts = [f"{x}APP" for x in re.findall(r"-D(\w+)_ENABLED", "".join(parts_list))]
    app_name = re.sub("_|-", "", f"{os.path.basename(parts_list[0])}")
    app_and_test = set(
        [f"{app_name.upper()}APP", f'{app_name.upper().replace("TEST", "")}TESTAPP']
    )
...
def get_parts(parts_list):
    ...
    """
    parse_methods = ["get_registered_apps", "parse_keyvalues"]
    growing = {}
