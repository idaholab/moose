...
def launchCEI(options):
    ...
    if cei != None:
        cei_command = [
            ...
            "-v",
            "-child",
            "listen://\?port=" + str(options.cei_port) + "\&timeout=-1",
        ]
        if options.debug:
            print("CEI Local lauch command:", cei_command)
        cei_process = subprocess.Popen(
            cei_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        ...
        if head_node != None:
            ...
            sys.exit(1)
        ensight_process = launchEnsight(
            options, head_node, job_id, cei_process, sos_process
        )
    else:
...
def launchNodes(options):
    ...
    if options.save_output:
        ...
        pbs_template = pbs_template.replace(
            "<OUTPUT>", "#PBS -o /dev/null\n#PBS -e /dev/null"
        )
    sos_command = [
        "mpiexec",
        "ceishell30",
        "-v",
        "-parent connect://$HOSTNAME\?port=" + str(options.sos_port),
        "-role",
        "SOS_SERVERS",
    ]
    ...
    qsub_process = subprocess.Popen(
        qsub_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def launchSOS(options, head_node):
    sos_command = [
        ...
        '"ceishell30',
        "-parent",
        "connect://" + socket.gethostname() + "\?port=" + str(options.cei_port),
        "-child",
        "listen://\?nconnections="
        + str(options.select * options.procs)
        + "\&timeout=-1\&port="
        + str(options.sos_port),
        "-role",
        'SOS"',
    ]
    if options.debug:
...
def launchEnsight(options, head_node, job_id, cei_process, sos_process):
    ...
    ensight_process = subprocess.Popen(
        ensight_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def getHeadnode(options, job_id):
    jobid_command = [
        "ssh",
        options.cluster,
        'bash --login -c "source /etc/profile && ' + "qstat -f " + str(job_id) + '"',
    ]
    qstat_process = subprocess.Popen(
        jobid_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def parseArguments(args=None):
    ...
    options = parser.add_argument_group(
        "Options", "The following options control how Ensight will be launched"
    )
    options.add_argument(
        ...
        metavar="str",
        default=["flogin2"],
        help="Specify the cluster to use (default: flogin2)\n ",
    )
    options.add_argument(
        "--select",
        nargs=1,
        metavar="int",
        default=[8],
        help="Specify how many nodes to utilize (default: 8)\n ",
    )
    options.add_argument(
        "--procs",
        nargs=1,
        metavar="int",
        default=[2],
        help="Specify how many processors to utilize (default: 2)\n ",
    )
    options.add_argument(
        "--walltime",
        metavar="str",
        ...
        type=str,
        default=["24:00:00"],
        help="Specify the wall time of the job (default: 24:00:00)\n ",
    )
    options.add_argument(
        "--queue",
        metavar="str",
        ...
        type=str,
        default=["general"],
        help="Specify the job queue (default: general)\n ",
    )
    options.add_argument(
        "--save-output",
        action="store_const",
        const=True,
        default=False,
        help="Save the job output. (default: False)\n ",
    )

    ensight_options = parser.add_argument_group(
        "Ensight Options", "The following options are specific to CEI and SOS"
    )
    ensight_options.add_argument(
        "--cei-port",
        nargs=1,
        metavar="int",
        type=int,
        default=[12350],
        help="Specify the CEI port (default: 12350)\n ",
    )
    ensight_options.add_argument(
        "--sos-port",
        nargs=1,
        ...
        type=int,
        default=[34568],
        help="Specify the SOS port (default: 34568)\n ",
    )

    script_options = parser.add_argument_group(
        "EnsightDR Launcher Options",
        "The following options are specific to this launcher script",
    )
    script_options.add_argument(
        ...
        const=True,
        default=False,
        help="Print any extras as we run\n ",
    )
