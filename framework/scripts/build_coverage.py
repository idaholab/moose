...
def _verifyOptions(options):
  ...
  if options.mode == 'sync':
    if options.sync_location is None:
      error_list.append('--mode sync requires --sync-location to be set')
    if options.html_location is None:
      error_list.append('--mode sync requires --html-location to be set')
    if options.generate_html:
      error_list.append('--mode sync does not permit generating HTML content')
...
def _parseARGs(args=None):
  ...
  parser.add_argument('--title', nargs='?', help='Title name of code coverage generated HTML \noutput\n ')
  parser.add_argument('--outfile', nargs='?', help='Output tracefile\n ')
  ...
  parser.add_argument('--sync-location', nargs='?', help='location to rsync the data to:\nuserid@server:/some/location\n ')
