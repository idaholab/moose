...
ParsedMaterialBase::validateVectorNames(const std::set<std::string> reserved_names)
{
  ...
  auto throwParamErr = [this](std::string param_name, std::string msg)
  {
  ...
  if (hasDuplicates(_constant_names))
    throwParamErr("constant_names", "In the constants duplicate names are not permitted.");
  for (const auto & name : _constant_names)
  {
    if (isReservedName(name))
      throwParamErr("constant_names", "In constants, the name '" + name + "' is not permitted.");
  }

  // check tolerance vectors
  if (hasDuplicates(_tol_names))
    throwParamErr("tol_names", "In the tolerances duplicate names are not permitted.");

  // check functor vectors
  if (_functor_symbols.empty())
  {
    if (!_functor_names.empty())
      throwParamErr("functor_names", "functor_symbols must be the same length as functor_names.");
  }
  else
  {
    ...
    if (hasDuplicates(names))
      throwParamErr(_functor_symbols.empty() ? "functor_names" : "functor_symbols",
