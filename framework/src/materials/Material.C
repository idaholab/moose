...
Material::Material(const std::string & name, InputParameters parameters) :
    ...
    _current_elem(parameters.get<bool>("_neighbor") ? _assembly.neighbor() : _assembly.elem()),
    _current_side(parameters.get<bool>("_neighbor") ? _assembly.neighborSide() : _assembly.side()),
