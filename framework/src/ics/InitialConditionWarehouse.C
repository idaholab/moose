                                     ...
                                     bool recurse)
{
  ...
  if (object->boundaryRestricted())
  {
    ...
    if (iter != _boundary_ics[tid].end() && object->hasBoundary(iter->second))
      mooseError("The initial condition '",
                 object->name(),
                 "' is being defined on a boundary that already has an initial condition defined (for the variable and state combo).");
    else
      _boundary_ics[tid][ic_key].insert(object->boundaryIDs().begin(),
  ...
  else if (object->blockRestricted())
  {
    ...
    if (iter != _block_ics[tid].end() &&
      ...
      mooseError("The initial condition '",
                 object->name(),
                 "' is being defined on a block that already has an initial condition defined (for the variable and state combo).");
    else
