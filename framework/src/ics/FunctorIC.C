    ...
    _pl(_fe_problem.mesh().getPointLocator())
{
  ...
  else if (_fe_problem.hasVariable(functor_name))
    paramError("functor",
  ...
  // Functor materials: fairly high risk since they could depend on variables
    ...
    paramInfo("functor", "Functor materials should not depend on variables if used in a FunctorIC");
}
