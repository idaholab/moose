...
AddOutputAction::act()
{
  ...
  if (!_moose_object_pars.get<bool>("_built_by_moose") && output_warehouse.isReservedName(_short_name))
    mooseError("The name '" << _short_name << "' is a reserved name for output objects");
  ...
  if (output_warehouse.hasOutput(_short_name))
    mooseError("An output object named '" << _short_name << "' already exists");
  ...
  if (_short_name == "auto_recovery_checkpoint")
    _moose_object_pars.set<std::string>("suffix") = "auto_recovery";

  // Create the object and add it to the warehouse
  MooseSharedPointer<Output> output = MooseSharedNamespace::static_pointer_cast<Output>(_factory.create(_type, _short_name, _moose_object_pars));
