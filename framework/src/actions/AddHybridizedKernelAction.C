//* This file is part of the MOOSE framework
//* https://www.mooseframework.org
//*
//* All rights reserved, see COPYRIGHT for full restrictions
//* https://github.com/idaholab/moose/blob/master/COPYRIGHT
//*
//* Licensed under LGPL 2.1, please see LICENSE for details
//* https://www.gnu.org/licenses/lgpl-2.1.html

#include "AddHybridizedKernelAction.h"
#include "FEProblem.h"

registerMooseAction("MooseApp", AddHybridizedKernelAction, "add_kernel");

InputParameters
AddHybridizedKernelAction::validParams()
{
  InputParameters params = MooseObjectAction::validParams();
  params.addClassDescription("Add a hybridized kernel object to the simulation.");
  return params;
}

AddHybridizedKernelAction::AddHybridizedKernelAction(const InputParameters & params)
  : MooseObjectAction(params)
{
}

void
AddHybridizedKernelAction::act()
{
  _problem->addHybridizedKernel(_type, _name, _moose_object_pars);
}
