...
InputParameters validParams<AdaptivityAction>()
{
  ...
  params.addParam<bool>("use_refined_mesh", false, "Recalculate indicators and markers on refined mesh");
  return params;
}
...
AdaptivityAction::act()
{
  ...
  adapt.setParam("use_refined_mesh", getParam<bool>("use_refined_mesh"));
