...
CreateExecutionerAction::act()
{
  ...
  if (fe_problem)
    _moose_object_pars.addPrivateParam<FEProblem *>("_fe_problem", fe_problem.get());

  std::shared_ptr<EigenProblem> eigen_problem = std::dynamic_pointer_cast<EigenProblem>(_problem);
  if (eigen_problem)
    _moose_object_pars.addPrivateParam<EigenProblem *>("_eigen_problem", eigen_problem.get());
