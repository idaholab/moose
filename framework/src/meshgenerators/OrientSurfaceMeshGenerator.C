...
OrientSurfaceMeshGenerator::validParams()
{
  ...
  params.renameParam(
      "normal", "normal_to_align_with", "Direction vector that element normals should align with");
  ...
  // Flooding / spreading algorithm parameters
  params.suppressParameter<std::vector<Real>>("max_subdomain_size_centroids");
  ...
  params.addClassDescription("Change the orientation of the surface mesh.");
  return params;
}
...
OrientSurfaceMeshGenerator::generate()
{
  ...
  if (!mesh->is_replicated())
    mooseError("OrientSurfaceMeshGenerator is not implemented for distributed meshes");
  setup(*mesh);
  ...
  if (num_flipped)
    _console << "Flipped the normal for " << num_flipped << " surface elements." << std::endl;
