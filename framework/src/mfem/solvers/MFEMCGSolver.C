...
MFEMCGSolver::updateSolver(mfem::ParBilinearForm & a, mfem::Array<int> & tdofs)
{
  ...
  else if (_lor)
  {
    if (!checkSpectralEquivalence(a))
      mooseError("Low-Order-Refined solver requires the FESpace closed_basis to be GaussLobatto and the open-basis to be IntegratedGLL for ND and RT elements.");
