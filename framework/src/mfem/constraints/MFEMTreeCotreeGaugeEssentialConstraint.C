                                                        ...
                                                        mfem::Array<int> & ess_tdof_list)
{
  ...
  // this variable. When no boundary is listed there is no such condition to
  // stay compatible with, so the marker is left all-zero.
  mfem::Array<int> ess_bdr(pmesh.bdr_attributes.Size() ? pmesh.bdr_attributes.Max() : 0);
  ess_bdr = 0;
  if (isBoundaryRestricted())
    ess_bdr = getBoundaryMarkers();
  ...
  // Edges of subdomains that are not gauged (empty 'block' -> none).
  mfem::Array<int> edge_excluded = excludedEdgeMarker(pmesh, getSubdomainAttributes());

  // Interior spanning-tree edge dofs that must additionally be fixed to zero.
  mfem::Array<int> tree_tdofs = BuildTreeCotreeGaugeTDofs(pfes, ess_bdr, pec_tdofs, edge_excluded);
