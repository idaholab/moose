...
registerMooseMFEMObject("MooseApp", CrossProductAux);

namespace Moose::MFEM
{
InputParameters
CrossProductAux::validParams()
{
...
CrossProductAux::CrossProductAux(const InputParameters & parameters)
    ...
    _scaled_cross(getScalarCoefficient("coefficient"), _cross)
{
  ...
  if (!dynamic_cast<const mfem::L2_FECollection *>(fes->FEColl()) || fes->GetVDim() != 3)
    mooseError("Moose::MFEM::CrossProductAux requires the target variable to be vector [L2]^3.");

  // Must have no shared/constrained DOFs (pure interior DOFs)
  if (fes->GetTrueVSize() != fes->GetVSize())
    mooseError("Moose::MFEM::CrossProductAux currently supports only L2 spaces with interior DOFs "
               "(no shared/constrained DOFs).");
}

void
CrossProductAux::execute()
{
