...
void
MFEMNodalProjector::extractNodePositions(const mfem::ParFiniteElementSpace & fespace, mfem::Vector & node_positions, mfem::Ordering::Type & node_ordering)
{
  ...
  for (int i = 0; i < NE; i++)
  {
      ...
      mfem::Vector rowx(node_positions.GetData() + i*nsp, nsp),
            rowy(node_positions.GetData() + i*nsp + NE*nsp, nsp),
            rowz;
      if (dim == 3)
...
void
MFEMNodalProjector::projectNodalValues(const mfem::Vector & nodal_vals, const mfem::Ordering::Type & nodal_val_ordering, mfem::ParGridFunction & gridfunction)
{
    ...
    for (int i = 0; i < NE; i++)
    {
      ...
      for (int j = 0; j < nsp; j++)
      {
        for (int d = 0; d < gf_ncomp; d++)
        {
          int idx;
          if (gf_ncomp>1)
            idx = nodal_val_ordering == mfem::Ordering::Type::byNODES ?
  ...
  else // H(div) or H(curl)
  {
    ...
    for (int i = 0; i < NE; i++)
    {
      ...
      fespace.GetFE(i)->ProjectFromNodes(elem_dof_vals,
