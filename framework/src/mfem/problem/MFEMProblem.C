                                  ...
                                  InputParameters & parameters)
{
  ...
  else if (dynamic_cast<const MFEMDGBoundaryCondition *>(mfem_bc_uo))
  {
    auto object_ptr = getUserObject<MFEMDGBoundaryCondition>(name).getSharedPtr();
    auto bc = std::dynamic_pointer_cast<MFEMDGBoundaryCondition>(object_ptr);
    auto eqsys =
        std::dynamic_pointer_cast<Moose::MFEM::EquationSystem>(getProblemData().eqn_system);
    if (eqsys)
    {
      eqsys->AddDGBC(std::move(bc));
    }
    else
      mooseError("Cannot add boundary condition with name '" + name +
                 "' because there is no corresponding equation system.");
