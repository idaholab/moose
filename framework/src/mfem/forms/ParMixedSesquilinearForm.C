                                      ...
                                      mfem::Vector &X, mfem::Vector &B)
{
   ...
   x.Read();
   mfem::Vector x_r; x_r.MakeRef(x, 0, vsize_trial);
   mfem::Vector x_i; x_i.MakeRef(x, vsize_trial, vsize_trial);

   MFEM_ASSERT(b.Size() == 2 * vsize_test, "Input LinearForm of incorrect size!");
   b.Read();
   mfem::Vector b_r; b_r.MakeRef(b, 0, vsize_test);
   mfem::Vector b_i; b_i.MakeRef(b, vsize_test, vsize_test);
   ...
   mfem::Vector X_r; X_r.MakeRef(X, 0, tvsize_trial);
   mfem::Vector X_i; X_i.MakeRef(X, tvsize_trial, tvsize_trial);
   mfem::Vector B_r; B_r.MakeRef(B, 0, tvsize_test);
   mfem::Vector B_i; B_i.MakeRef(B, tvsize_test, tvsize_test);
   ...
   {
      b_0 = b_r;
      _pmblfr->FormRectangularLinearSystem(ess_trial_tdof_list, ess_test_tdof_list, x_r, b_0, A_r, X_0, B_0);
      X_r = X_0; B_r = B_0;

      b_0 = b_i;
      _pmblfr->FormRectangularLinearSystem(ess_trial_tdof_list, ess_test_tdof_list, x_i, b_0, A_r, X_0, B_0);
      X_i = X_0; B_i = B_0;

      if (ImagInteg())
      {
         b_0 = 0.0;
         _pmblfi->FormRectangularLinearSystem(ess_trial_tdof_list, ess_test_tdof_list, x_i, b_0, A_i, X_0, B_0);
         B_r -= B_0;

         b_0 = 0.0;
         _pmblfi->FormRectangularLinearSystem(ess_trial_tdof_list, ess_test_tdof_list, x_r, b_0, A_i, X_0, B_0);
         B_i += B_0;
      }
   }
   else if (ImagInteg())
   ...
   if (RealInteg() && ImagInteg())
   {
      ...
      // If A_i is a RectangularConstrainedOperator, this is handled automatically in the Mult() method
