...
MFEMRWTE10IntegratedBC::validParams()
{
  ...
  params.addClassDescription("Adds the Robin boundary conditions for an electromagnetic problem with transverse waves in a rectangular waveguide.");
  params.addParam<RealVectorValue>(
      "port_length_vector", "Vector along the x-axis of the port, where its magnitude is the port length in meters.");
  params.addParam<RealVectorValue>(
      "port_width_vector", "Vector along the y-axis of the port, where its magnitude is the port width in meters.");
  params.addParam<Real>(
      "frequency", 1.0, "Mode frequency in Hz.");
  params.addParam<Real>(
  ...
  params.addParam<bool>(
      "input_port", false, "Whether the boundary attribute passed to this BC corresponds to the input port of the waveguide.");
