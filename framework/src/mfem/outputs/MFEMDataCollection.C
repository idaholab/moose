...
MFEMDataCollection::validParams()
{
  ...
  params.addParam<std::string>("submesh", "", "Submesh to output variables on. Leave blank to use base mesh.");
  return params;
}
...
MFEMDataCollection::registerFields()
{
  ...
  for (auto const & [gf_name, gf_ptr] : _problem_data.gridfunctions)
  {
    ...
    {
      mooseWarning("The variable ", gf_name, " is not defined on the same mesh as the output DataCollection.");
    }
  }
}
