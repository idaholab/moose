...
MFEMVectorFEDomainLFKernel::validParams()
{
  ...
  params.addParam<MFEMVectorCoefficientName>(
      "vector_coefficient_imag", "The name of the imaginary part of the vector coefficient f");
...
{
  return std::make_pair(new mfem::VectorFEDomainLFIntegrator(_vec_coef),
                        isParamValid("vector_coefficient_imag")
                            ? new mfem::VectorFEDomainLFIntegrator(_vec_coef_imag)
                            : nullptr);
}
