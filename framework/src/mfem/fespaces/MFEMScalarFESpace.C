...
MFEMScalarFESpace::validParams()
{
  ...
  params.addParam<std::string>("basis", "GaussLobatto", "Specifies the quadrature basis used for scalar elements.");
...
MFEMScalarFESpace::getFECName() const
{
  ...
  return _fec_type + basis + "_" + std::to_string(getProblemDim()) + "D_P" + std::to_string(_fec_order);
}
