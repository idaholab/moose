...
validParams<LeastSquaresFit>()
{
  ...
  params.addParam<Real>("x_scale", 1.0, "Value used to scale x values (scaling is done after shifting)");
  params.addParam<Real>("x_shift", 0.0, "Value used to shift x values (shifting is done before scaling)");
  params.addParam<Real>("y_scale", 1.0, "Value used to scale y values (scaling is done after shifting)");
