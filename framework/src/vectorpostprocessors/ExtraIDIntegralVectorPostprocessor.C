...
ExtraIDIntegralVectorPostprocessor::validParams()
{
  ...
  params.addParam<std::string>(
      "spatial_value_name",
      "To specify which variable or material property is to be used for evaluating spatial values");
    ...
    _spatial_evaluation_index(std::numeric_limits<unsigned int>::max())
{
  ...
  {
    auto name = getParam<std::string>("spatial_value_name");
    _spatial_evaluation_index = vector_names.size();
    for (const auto i : make_range(vector_names.size()))
