...
#include "ElementMaterialPropertySampler.h"
#include "Material.h"
...
registerMooseObject("MooseApp", ElementMaterialPropertySampler);
registerMooseObjectRenamed("MooseApp",
                           MaterialVectorPostprocessor,
                           "06/30/2025 24:00",
                           ElementMaterialPropertySampler);

InputParameters
ElementMaterialPropertySampler::validParams()
{
...
ElementMaterialPropertySampler::ElementMaterialPropertySampler(const InputParameters & parameters)
  ...
  for (auto & prop : prop_names)
  {
    ...
    else
    {
      mooseWarning("property " + prop +
                   " is of unsupported type and skipped by ElementMaterialPropertySampler");
      continue;
    }
...
void
ElementMaterialPropertySampler::initialize()
{
...
void
ElementMaterialPropertySampler::execute()
{
...
void
ElementMaterialPropertySampler::finalize()
{
...
void
ElementMaterialPropertySampler::threadJoin(const UserObject & y)
{
  const auto & vpp = static_cast<const ElementMaterialPropertySampler &>(y);
...
void
ElementMaterialPropertySampler::sortVecs()
{
