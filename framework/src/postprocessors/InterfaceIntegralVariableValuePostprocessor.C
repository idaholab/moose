...
InterfaceIntegralVariableValuePostprocessor::computeQpIntegral()
{
  if (_has_fv_vars)
  {
    ...
    {
      u = MetaPhysicL::raw_value(
          _fv_variable->getBoundaryFaceValue(*_fi, Moose::currentTimeFunctorArg()));
      u_neighbor = MetaPhysicL::raw_value(
          _neighbor_fv_variable->getBoundaryFaceValue(*_fi, Moose::currentTimeFunctorArg()));
    }
