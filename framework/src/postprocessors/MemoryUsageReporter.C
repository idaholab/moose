...
MemoryUsageReporter::MemoryUsageReporter(const MooseObject * moose_object)
{
  ...
  std::vector<unsigned long long> memory_totals;
  moose_object->comm().gather(0, _memory_total, memory_totals);
  ...
  moose_object->comm().gather(0, processor_name, processor_names);

  // assign a unique numerical id to them on processor zero
  unsigned int id = 0;
  if (moose_object->comm().rank() == 0)
  {
    mooseAssert(processor_names.size() == memory_totals.size(), "Inconsistent per rank MPI data!");
    ...
    for (std::size_t i = 0; i < nproc; ++i)
    {
      ...
      {
        _hardware_memory_total.resize(id);
        _hardware_memory_total[id - 1] = memory_totals[i];
      }
      else if (_hardware_memory_total[id - 1] != memory_totals[i])
