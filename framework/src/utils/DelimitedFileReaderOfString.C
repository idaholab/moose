...
void
DelimitedFileReaderOfString::read()
{
  // Number of columns
  std::size_t n_cols;
  ...
  if (_format_flag == FormatFlag::ROWS)
  {
    std::vector<std::string>::iterator start = raw.begin();
    for (std::size_t j = 0; j < n_cols; ++j)
    {
      _data[j] = std::vector<std::string>(start, start + _row_offsets[j]);
...
std::size_t
DelimitedFileReaderOfString::numEntries() const
{
...
DelimitedFileReaderOfString::getNames() const
{
  return _names;
}

const std::vector<std::vector<std::string>> &
DelimitedFileReaderOfString::getData() const
{
  return _data;
}

const std::vector<std::string> &
DelimitedFileReaderOfString::getData(const std::string & name) const
{
...
const std::vector<std::string> &
DelimitedFileReaderOfString::getData(std::size_t index) const
{
  if (index >= _data.size())
...
bool
DelimitedFileReaderOfString::preprocessLine(std::string & line, const unsigned int & num)
{
  // Handle row comments
                                 ...
                                 const unsigned int & num)
{
  ...
  if (!status)
    mooseError("Failed to convert a delimited data when reading line ",
               num,
               " in file ",
               _filename,
               ".\n  LINE ",
...
const std::string &
DelimitedFileReaderOfString::delimiter(const std::string & line)
{
  if (_delimiter.empty())
  {
...
bool
DelimitedFileReaderOfString::header(const std::string & line)
{
  switch (_header_flag)
  {
    case HeaderFlag::OFF:
