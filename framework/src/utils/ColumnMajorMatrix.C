...
#if !defined(LIBMESH_HAVE_PETSC)
  FORTRAN_CALL(dgeev)
  (&jobvl, &jobvr, &n, a_data, &n, eval_r, eval_i, evec_le, &n, evec_ri, &n, &buffer[0], &buffer_size, &return_value);
#else
  LAPACKgeev_(&jobvl, &jobvr, &n, a_data, &n, eval_r, eval_i, evec_le, &n, evec_ri, &n, &buffer[0], &buffer_size, &return_value);
#endif
...
#if !defined(LIBMESH_HAVE_PETSC)
  FORTRAN_CALL(dgetrf)
  (&n, &n, invA_data, &n, &ipiv[0], &return_value);
#else
...
#if !defined(LIBMESH_HAVE_PETSC) || PETSC_VERSION_LESS_THAN(3, 5, 0)
  FORTRAN_CALL(dgetri)
  (&n, invA_data, &n, &ipiv[0], &buffer[0], &buffer_size, &return_value);
#else
