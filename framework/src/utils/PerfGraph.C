...
PerfGraph::PerfGraph(const std::string & root_name,
  ...
  : ConsoleStreamInterface(app),
    ...
    // Tell the root node not to acrue self time if it's not the Main app in the multiapp system
    _root_node(std::make_unique<PerfNode>(_root_node_id, /*no_self = */ app.multiAppLevel())),
    _current_position(-1),
    _stack(),
