...
RankTwoTensor &
    RankTwoTensor::
    operator=(const RankTwoTensor & a)
{
  for (unsigned int i = 0; i < N; ++i)
    for (unsigned int j = 0; j < N; ++j)
      _vals[i][j] = a(i, j);
...
RankTwoTensor
    RankTwoTensor::
    operator+(const RankTwoTensor & b) const
{
  RankTwoTensor result;
  const RankTwoTensor & a = *this;
...
RankTwoTensor
    RankTwoTensor::
    operator*(const TypeTensor<Real> & b) const
{
  RankTwoTensor result;
  const RankTwoTensor & a = *this;
...
bool
    RankTwoTensor::
    operator==(const RankTwoTensor & a) const
{
  for (unsigned int i = 0; i < N; ++i)
    for (unsigned int j = 0; j < N; ++j)
...
{
  const RankTwoTensor & a = *this;
  Real result = a(0, 0) * a(1, 1) + a(0, 0) * a(2, 2) + a(1, 1) * a(2, 2) - a(0, 1) * a(1, 0) - a(0, 2) * a(2, 0) - a(1, 2) * a(2, 1);
  return result;
}
      ...
      for (unsigned int i = 0; i < N; ++i)
        for (unsigned int j = 0; j < N; ++j)
          for (unsigned int k = 0; k < N; ++k)
            for (unsigned int l = 0; l < N; ++l)
            {
              deriv[alpha](i, j, k, l) += 0.5 * (ev[beta][i] * ev[alpha][j] + ev[alpha][i] * ev[beta][j]) * (ev[beta][k] * ev[alpha][l] + ev[beta][l] * ev[alpha][k]) / (eigvals[alpha] - eigvals[beta]);
            }
    }
}
