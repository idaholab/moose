...
PolynomialFit::doLeastSquares()
{
  ...
  FORTRAN_CALL(dgels)
  (&mode, &num_rows, &num_coeff, &num_rhs, &_matrix[0], &num_rows, &rhs[0], &num_rows, &opt_buffer_size, &buffer_size, &return_value);
  if (return_value)
  ...
  FORTRAN_CALL(dgels)
  (&mode, &num_rows, &num_coeff, &num_rhs, &_matrix[0], &num_rows, &rhs[0], &num_rows, buffer, &buffer_size, &return_value);
  delete[] buffer;
