...
SolutionInvalidity::SolutionInvalidity(const std::string & root_name, MooseApp & app)
  : ConsoleStreamInterface(app),
    _moose_app(app),
    _solution_invalidity_registry(moose::internal::getSolutionInvalidityRegistry()),
    _pid(app.comm().rank()),
    _root_name(root_name),
    _root_node_id(_solution_invalidity_registry.registerSection(root_name, 0)),
    _root_node(std::make_unique<SolutionNode>(_root_node_id)),
    _current_position(-1),
    _stack(),
...
Real
SolutionInvalidity::sectionData(const DataType type,
                                ...
                                const bool must_exist /* = true */)
{
  ...
  if (section_it == _cumulative_section_info.end())
  {
    if (!must_exist || // isn't required to exist
        _solution_invalidity_registry.sectionExists(
            section_name) // or, is required to exist and it does
    )
      return 0;

    mooseError(
        "Unknown SolutionInvalidity section name \"",
        section_name,
        "\" in SolutionInvalidity::sectionData().\nIf you are attempting to retrieve the root use "
        "\"Root\".");
  }
...
void
SolutionInvalidity::addToExecutionList(
    const PerfID id,
...
void
SolutionInvalidity::push(const PerfID id)
{
  if (!_active)
    return;

  SolutionNode * new_node = nullptr;
  ...
  if (_current_position >= MAX_STACK_SIZE)
    mooseError("SolutionInvalidity is out of stack space!");

  _stack[_current_position] = new_node;
}

void
SolutionInvalidity::pop()
{
  if (!_active)
    return;
...
SolutionInvalidity::update()
{
  ...
  _cumulative_section_info_ptrs.resize(_solution_invalidity_registry.numSections());

  for (auto & section_time_it : _cumulative_section_info)
  {
    auto id = _solution_invalidity_registry.sectionID(section_time_it.first);
...
void
SolutionInvalidity::recursivelyUpdate(const SolutionNode & current_node)
{
  const auto & section_name =
      _solution_invalidity_registry.readSectionInfo(current_node.id())._name;
...
SolutionInvalidity::treeTable(const unsigned int level, const bool heaviest /* = false */)
{
  update();
  ...
  auto act = [this, &vtable](const SolutionNode & node,
                             const moose::internal::SolutionInvaliditySectionInfo & section_info,
...
void
SolutionInvalidity::print(const ConsoleStream & console, unsigned int level)
{
  console << "\nPerformance Graph:\n";
...
void
SolutionInvalidity::printHeaviestBranch(const ConsoleStream & console)
{
  console << "\nHeaviest Branch:\n";
...
void
SolutionInvalidity::printHeaviestSections(const ConsoleStream & console,
...
void
dataStore(std::ostream & stream, SolutionInvalidity & solution_invalidity, void *)
{
  ...
  // dataStore(stream, solution_invalidity._solution_invalidity_registry._id_to_section_info,
  // nullptr);

  // Update before serializing the nodes so that the time/memory/calls are correct
  solution_invalidity.update();

  // Recursively serialize all of the nodes
  dataStore(stream, solution_invalidity._root_node, nullptr);
}

void
dataLoad(std::istream & stream, SolutionInvalidity & solution_invalidity, void *)
{
  // Load in all of the recovered sections and register those that do not exist yet
  std::vector<moose::internal::SolutionInvaliditySectionInfo> recovered_section_info;
  // dataLoad(stream, recovered_section_info, nullptr);
  // for (const auto & info : recovered_section_info)
  //{
  // if (info._live_message.size())
  //   solution_invalidity._solution_invalidity_registry.registerSection(
  //       info._name, info._level, info._live_message, info._print_dots);
  // else
  //   solution_invalidity._solution_invalidity_registry.registerSection(info._name, info._level);
  //}
  ...
  dataLoad(stream, solution_invalidity._root_node, &solution_invalidity);
}
