...
MultiPostprocessorConvergence::validParams()
{
  ...
  params.addRequiredParam<bool>("require_one_iteration",
                                "If set to true, at least one iteration is required. This is "
                                "recommended to be used when step criteria is included.");
...
MultiPostprocessorConvergence::MultiPostprocessorConvergence(const InputParameters & parameters)
  : Convergence(parameters),
    ...
    _require_one_iteration(getParam<bool>("require_one_iteration"))
{
...
MultiPostprocessorConvergence::checkConvergence(unsigned int iter)
{
  ...
  if (_require_one_iteration && iter == 0)
  {
