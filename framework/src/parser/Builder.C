  ...
  if (messages.size())
  {
    ...
    if (err_unused)
    {
      if (_parser.getThrowOnError())
        Moose::ParseUtils::parseError(_root, messages, true);
      else
        mooseError(
            message +
...
{
  const auto info = ParameterExtraction::extract(_root, section_node, p);
...
{
  const auto info = ParameterExtraction::extract(_root, prefix, p);
  appendExtractionInfo(info);
}
