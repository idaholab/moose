...
{
ExtractionInfo
extract(const hit::Node & root, const hit::Node * const section_node, InputParameters & params)
{
    ...
    for (const auto & param_name : params.paramAliases(name))
    {
      ...
      if (param_node)
      {
        ...
        if (params.isCommandLineParameter(param_name))
          params.commandLineParamSet(param_name, {});
...
ExtractionInfo
extract(const hit::Node & root, const std::string & prefix, InputParameters & p)
{
  const auto node = root.find(prefix);
  return extract(root, (node && node->type() == hit::NodeType::Section) ? node : nullptr, p);
}
}
