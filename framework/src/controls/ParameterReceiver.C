...
registerMooseObject("MooseApp", ParameterReceiver);

InputParameters
ParameterReceiver::validParams()
{
  InputParameters params = Control::validParams();
  params.addClassDescription("Control for receiving parameters from a Transfer.");
  params.set<ExecFlagEnum>("execute_on") = EXEC_TIMESTEP_BEGIN;
  return params;
}

ParameterReceiver::ParameterReceiver(const InputParameters & parameters) : Control(parameters) {}

void
ParameterReceiver::execute()
{
...
void
ParameterReceiver::transfer(const std::vector<std::string> & names,
