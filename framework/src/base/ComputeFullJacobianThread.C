...
{
  const KernelWarehouse * warehouse;
  switch (_kernel_type)
  {
    case Moose::KT_ALL:
  ...
  for (const auto & it : ce)
  {
    ...
    if (ivariable.activeOnSubdomain(_subdomain) && jvariable.activeOnSubdomain(_subdomain) &&
        warehouse->hasActiveVariableBlockObjects(ivar, _subdomain, _tid))
    {
      ...
      const std::vector<std::shared_ptr<KernelBase>> & kernels =
          warehouse->getActiveVariableBlockObjects(ivar, _subdomain, _tid);
  ...
  if (scalar_vars.size() > 0)
  {
    ...
    for (const auto & ivariable : vars)
      if (ivariable->activeOnSubdomain(_subdomain) > 0 &&
          warehouse->hasActiveVariableBlockObjects(ivariable->number(), _subdomain, _tid))
      {
        // for each variable get the list of active kernels
        const std::vector<std::shared_ptr<KernelBase>> & kernels =
            warehouse->getActiveVariableBlockObjects(ivariable->number(), _subdomain, _tid);
        for (const auto & kernel : kernels)
