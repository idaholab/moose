...
InputParameters validParams<FEProblem>()
{
  ...
  params.addParam<bool>("error_on_jacobian_nonzero_reallocation", true, "This causes PETSc to error if it had to reallocate memory in the Jacobian matrix due to not having enough nonzeros");
  return params;
}
