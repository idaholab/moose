...
AssemblySystem::AssemblySystem(FEProblem & fe_problem, const std::string & name) :
...
void
AssemblySystem::restoreSolutions()
{
  // call parent
...
void
AssemblySystem::setupFiniteDifferencedPreconditioner()
{
#ifdef LIBMESH_HAVE_PETSC
...
void
AssemblySystem::addTimeIntegrator(const std::string & type, const std::string & name, InputParameters parameters)
{
...
void
AssemblySystem::addBoundaryCondition(const std::string & bc_name, const std::string & name, InputParameters parameters)
{
  // ThreadID
  THREAD_ID tid = 0;
...
void
AssemblySystem::addInterfaceKernel(std::string interface_kernel_name, const std::string & name, InputParameters parameters)
{
  for (THREAD_ID tid = 0; tid < libMesh::n_threads(); ++tid)
  {
...
void
AssemblySystem::computeResidual(NumericVector<Number> & residual, Moose::KernelType type)
{
...
void
AssemblySystem::setInitialSolution()
{
...
AssemblySystem::solutionUDot()
{
  return _u_dot;
}

NumericVector<Number> &
AssemblySystem::residualVector(Moose::KernelType type)
{
  switch (type)
  {
...
void
AssemblySystem::setConstraintSlaveValues(NumericVector<Number> & solution, bool displaced)
{
...
void
AssemblySystem::constraintResiduals(NumericVector<Number> & residual, bool displaced)
{
...
void
AssemblySystem::computeResidualInternal(Moose::KernelType type)
{
  for (THREAD_ID tid = 0; tid < libMesh::n_threads(); tid++)
  {
...
void
AssemblySystem::computeNodalBCs(NumericVector<Number> & residual)
{
...
void
AssemblySystem::getNodeDofs(unsigned int node_id, std::vector<dof_id_type> & dofs)
{
...
void
AssemblySystem::findImplicitGeometricCouplingEntries(GeometricSearchData & geom_search_data, std::map<dof_id_type, std::vector<dof_id_type> > & graph)
{
...
void
AssemblySystem::addImplicitGeometricCouplingEntries(SparseMatrix<Number> & jacobian, GeometricSearchData & geom_search_data)
{
...
void
AssemblySystem::constraintJacobians(SparseMatrix<Number> & jacobian, bool displaced)
{
#if PETSC_VERSION_LESS_THAN(3,3,0)
#else
...
void
AssemblySystem::computeJacobianInternal(SparseMatrix<Number> &  jacobian)
{
#ifdef LIBMESH_HAVE_PETSC
  //Necessary for speed
#if PETSC_VERSION_LESS_THAN(3,0,0)
...
void
AssemblySystem::setVariableGlobalDoFs(const std::string & var_name)
{
...
void
AssemblySystem::computeJacobian(SparseMatrix<Number> & jacobian)
{
...
void
AssemblySystem::computeJacobianBlocks(std::vector<JacobianBlock *> & blocks)
{
...
Real
AssemblySystem::computeDamping(const NumericVector<Number> & solution,
...
void
AssemblySystem::computeDiracContributions(SparseMatrix<Number> * jacobian)
{
...
NumericVector<Number> &
AssemblySystem::residualGhosted()
{
...
void
AssemblySystem::augmentSparsity(SparsityPattern::Graph & sparsity,
...
void
AssemblySystem::serializeSolution()
{
  if (_need_serialized_solution)
  {
...
void
AssemblySystem::setSolution(const NumericVector<Number> & soln)
{
...
void
AssemblySystem::setSolutionUDot(const NumericVector<Number> & udot)
{
  _u_dot = udot;
}

NumericVector<Number> &
AssemblySystem::serializedSolution()
{
...
void
AssemblySystem::checkKernelCoverage(const std::set<SubdomainID> & mesh_subdomains) const
{
...
Real
AssemblySystem::relativeSolutionDifferenceNorm()
{
