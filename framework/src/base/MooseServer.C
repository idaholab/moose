                                 ...
                                 const std::set<std::string> & obj_act_tasks)
{
  ...
  // use object type to further gather valid object parameters from factory
                                 ...
                                 int request_char)
{
  ...
  for (const auto & valid_params_iter : valid_params)
  {
    ...
    // use parameter information to build input default value in completion
                                ...
                                int request_char)
{
  ...
  if (_syntax_to_subblocks.empty())
  {
    for (const auto & syntax_path_iter : syntax.getAssociatedActions())
    {
      ...
      for (size_t last_slash; (last_slash = syntax_path.find_last_of("/")) != std::string::npos;)
      {
        std::string subblock_name = syntax_path.substr(last_slash + 1);

        syntax_path = syntax_path.substr(0, last_slash);
  ...
  // add subblock names of found syntax or document root to completion list
                             ...
                             int replace_char_end)
{
  ...
  else
  {
    ...
    if (input_path_iter != _type_to_input_paths.end())
    {
      ...
      for (const auto & input_path : input_path_iter->second)
      {
        ...
        size_t count = selector.evaluate(view_root, results);
  ...
  // walk over gathered options and descriptions and add to completion list
