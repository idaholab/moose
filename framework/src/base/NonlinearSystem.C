...
NonlinearSystem::solve()
{
  ...
  // Set up splits before the DM, so that the DM knows how to build up the decomposition
  // Should setupDecomposition() be moved into petscSetupDM() to avoid the fragility of
  // this dependence?
  if(_have_decomposition)
    setupDecomposition();

  Moose::PetscSupport::petscSetupDM(*this);
