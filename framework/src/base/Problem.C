...
Problem::Problem(Moose::Mesh &mesh) :
    _mesh(mesh),
    _eq(_mesh),
    _transient(false),
...
Problem::update()
{
  for (std::vector<SubProblem *>::iterator it = _sys.begin(); it != _sys.end(); ++it)
    (*it)->update();
}
...
bool
Problem::hasVariable(const std::string & var_name)
{
...
Moose::Variable &
Problem::getVariable(THREAD_ID tid, const std::string & var_name)
{
  for (unsigned int i = 0; i < _sys.size(); i++)
    if (_sys[i]->hasVariable(var_name))
...
void
Problem::addInitialCondition(const std::string & ic_name, const std::string & name, InputParameters parameters, std::string var_name)
{
...
void
Problem::addInitialCondition(const std::string & var_name, Real value)
{
...
Number
Problem::initialValue (const Point& p,
                       const Parameters& /*parameters*/,
                       const std::string& /*sys_name*/,
                       const std::string& var_name)
{
...
Gradient
Problem::initialGradient (const Point& p,
                          const Parameters& /*parameters*/,
                          const std::string& /*sys_name*/,
                          const std::string& var_name)
{
...
void
Problem::initialCondition(EquationSystems& es, const std::string& system_name)
{
