...
LinearFVAdvectionDiffusionFunctorDirichletBC::validParams()
{
  ...
  params.addParam<MooseEnum>(
      ...
      "multiplication occurs if 'none' is selected). Note that this parameter probably only makes "
      "sense when applying Dirichlet conditions for vector fields. And in that case ensure that "
      "the provided Dirichlet value is positive if the dot product of the vector Dirichlet "
      "condition and the normal vector is positive, e.g. outflow, and negative for inflow.");
  return params;
}
