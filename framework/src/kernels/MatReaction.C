...
MatReaction::validParams()
{
  ...
  params.addDeprecatedParam<MaterialPropertyName>(
      "mob_name", "The reaction rate used with the kernel", "Use the parameter 'reaction_rate'");
...
MatReaction::MatReaction(const InputParameters & parameters)
  : DerivativeMaterialInterface<JvarMapKernelInterface<Kernel>>(parameters),
    ...
    _L(isParamValid("mob_name") ? getMaterialProperty<Real>("mob_name")
                                : getMaterialProperty<Real>("reaction_rate")),
    _eta_name(_var.name()),
    _dLdop(isParamValid("mob_name")
               ? getMaterialPropertyDerivative<Real>("mob_name", _eta_name)
               : getMaterialPropertyDerivative<Real>("reaction_rate", _eta_name)),
    _dLdv(isParamValid("mob_name") ? getMaterialPropertyDerivative<Real>("mob_name", _v_name)
                                   : getMaterialPropertyDerivative<Real>("reaction_rate", _v_name)),
    _dLdarg(_n_args)
{
  // Get reaction rate derivatives
  for (unsigned int i = 0; i < _n_args; ++i)
    _dLdarg[i] = isParamValid("mob_name")
                     ? &getMaterialPropertyDerivative<Real>("mob_name", i)
                     : &getMaterialPropertyDerivative<Real>("reaction_rate", i);
}

void
MatReaction::initialSetup()
{
  if (isParamValid("mob_name"))
