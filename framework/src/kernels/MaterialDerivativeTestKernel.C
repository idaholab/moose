...
InputParameters validParams<MaterialDerivativeTestKernel>()
{
  ...
  params.addRequiredParam<MaterialPropertyName>("material_property", "Name of material property for which derivatives are to be tested.");
    ...
    _p_derivatives(_n_vars)
{
  for (unsigned int m = 0; m < _n_vars; ++m)
    _p_derivatives[m] = & getMaterialPropertyDerivative<Real>("material_property", _coupled_moose_vars[m]->name());
}
