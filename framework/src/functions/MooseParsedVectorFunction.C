...
MooseParsedVectorFunction::initialSetup()
{
  ...
  if (!_function_ptr)
    _function_ptr =
        libmesh_make_unique<MooseParsedFunctionWrapper>(this, _vector_value, _vars, _vals, tid);

  if (!_curl_function_ptr)
    _curl_function_ptr =
        libmesh_make_unique<MooseParsedFunctionWrapper>(this, _curl_value, _vars, _vals, tid);
}
