...
IterationAdaptiveDT::computeAdaptiveDT(Real & dt, bool allowToGrow, bool allowToShrink)
{
  ...
  if (allowToGrow && (_solver_its < growth_nl_its && _l_its < growth_l_its))
  {
    // Grow the timestep
    dt *= _growth_factor;

    if (_verbose)
      _console << "Growing dt: " + ite_type + " its = " << _solver_its << " < " << growth_nl_its
               << " && lin its = " << _l_its << " < " << growth_l_its << " old dt: " << std::setw(9)
               << _dt_old << " new dt: " << std::setw(9) << dt << '\n';
  }
  else if (allowToShrink && (_solver_its > shrink_nl_its || _l_its > shrink_l_its))
  {
    // Shrink the timestep
    dt *= _cutback_factor;

    if (_verbose)
      _console << "Shrinking dt: " + ite_type + " its = " << _solver_its << " > " << shrink_nl_its
...
IterationAdaptiveDT::acceptStep()
{
  ...
  for (const auto i : make_range(_fe_problem.numNonlinearSystems()))
    _solver_its += _fe_problem.getNonlinearSystemBase(/*nl_sys=*/i).nNonlinearIterations();
  ...
  for (const auto i : make_range(_fe_problem.numLinearSystems()))
  {
    ...
    _solver_its += _fe_problem.getLinearSystem(/*nl_sys=*/i).nLinearIterations();
  }
