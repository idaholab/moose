...
// MOOSE includes
#include "IterationAdaptivePostprocessorDT.h"
#include "Function.h"
#include "Piecewise.h"
#include "Transient.h"
...
IterationAdaptivePostprocessorDT::IterationAdaptivePostprocessorDT(const InputParameters & parameters) :
...
IterationAdaptivePostprocessorDT::~IterationAdaptivePostprocessorDT()
{
}


void
IterationAdaptivePostprocessorDT::init()
{
  if (isParamValid("timestep_limiting_function"))
  {
...
void
IterationAdaptivePostprocessorDT::preExecute()
{
...
Real
IterationAdaptivePostprocessorDT::computeInitialDT()
{
  Real dt = _input_dt;
  return dt;
}
...
Real
IterationAdaptivePostprocessorDT::computeDT()
{
  Real dt(_dt_old);
...
bool
IterationAdaptivePostprocessorDT::constrainStep(Real &dt)
{
  bool at_sync_point = TimeStepper::constrainStep(dt);

  std::ostringstream diag;

  // Limit the timestep according to postprocessor value
  limitDTByPostprocessor(dt);
...
Real
IterationAdaptivePostprocessorDT::computeFailedDT()
{
...
void
IterationAdaptivePostprocessorDT::limitDTByFunction(Real & limitedDT)
{
  std::ostringstream diag;
...
void
IterationAdaptivePostprocessorDT::limitDTByPostprocessor(Real & limitedDT)
{
  std::ostringstream diag;

  if( _pps_value > 0. && limitedDT > _pps_value)
  {
    limitedDT = _pps_value;

    diag << "Limiting dt to postprocessor value. dt: "
         << std::setw(9)
         << std::setprecision(6)
         << std::setfill('0')
         << std::showpoint
...
void
IterationAdaptivePostprocessorDT::computeAdaptiveDT(Real &dt, bool allowToGrow, bool allowToShrink)
{
...
void
IterationAdaptivePostprocessorDT::computeInterpolationDT(Real & dt)
{
  std::ostringstream diag;
  dt = _time_ipol.sample(_time_old);
  if (dt > _dt_old * _growth_factor)
...
void
IterationAdaptivePostprocessorDT::rejectStep()
{
...
void
IterationAdaptivePostprocessorDT::acceptStep()
{
