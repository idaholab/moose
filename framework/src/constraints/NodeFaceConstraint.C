...
NodeFaceConstraint::computeResidual()
{
  ...
  for (_i = 0; _i < _test_master.size(); _i++)
    _local_re(_i) += computeQpResidual(Moose::Master);
  ...
  _i = 0;
  _local_re(0) = computeQpResidual(Moose::Slave);
...
NodeFaceConstraint::computeJacobian()
{
  ...
  if (_local_ke.m() && _local_ke.n())
    for (_i = 0; _i < _test_slave.size(); _i++)
      for (_j = 0; _j < _phi_master.size(); _j++)
        _local_ke(_i, _j) += computeQpJacobian(Moose::SlaveMaster);
  ...
  if (_local_ke.m() && _local_ke.n())
    for (_i = 0; _i < _test_master.size(); _i++)
      for (_j = 0; _j < _phi_master.size(); _j++)
        _local_ke(_i, _j) += computeQpJacobian(Moose::MasterMaster);
...
NodeFaceConstraint::computeOffDiagJacobian(unsigned int jvar)
{
  ...
  for (_i = 0; _i < _test_slave.size(); _i++)
    for (_j = 0; _j < _phi_master.size(); _j++)
      _local_ke(_i, _j) += computeQpOffDiagJacobian(Moose::SlaveMaster, jvar);
  ...
  for (_i = 0; _i < _test_master.size(); _i++)
    for (_j = 0; _j < _phi_master.size(); _j++)
      _local_ke(_i, _j) += computeQpOffDiagJacobian(Moose::MasterMaster, jvar);
