...
Restartable::Restartable(const InputParameters & parameters, std::string system_name, SubProblem * subproblem) :
    ...
    parameters.get<SubProblem *>("_subproblem") : (parameters.isParamValid("_fe_problem_base") ?
    parameters.get<FEProblemBase *>("_fe_problem_base") : (parameters.isParamValid("_fe_problem") ?
    parameters.get<FEProblem *>("_fe_problem") : subproblem)) )
{
}
