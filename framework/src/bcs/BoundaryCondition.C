...
BoundaryCondition::BoundaryCondition(std::string name, MooseSystem & moose_system, InputParameters parameters) :
   ...
   _side_elem(NULL),
   _JxW_face(*_face_data._JxW_face[_tid][_fe_type]),
   _phi_face(*_face_data._phi_face[_tid][_fe_type]),
   _dphi_face(*_face_data._dphi_face[_tid][_fe_type]),
   _d2phi_face(*_face_data._d2phi_face[_tid][_fe_type]),
   _normals_face(*_face_data._normals_face[_tid][_fe_type]),
   _qface(_face_data._qface[_tid]),
   _q_point_face(*_face_data._q_point_face[_tid][_fe_type]),
   ...
   _current_side(_face_data._current_side[_tid]),
   _current_node(_face_data._current_node[_tid]),
   _current_residual(_face_data._current_residual[_tid]),
   _u_face(_integrated ? _face_data._var_vals_face[_tid][_var_num] : _face_data._var_vals_face_nodal[_tid][_var_num]),
   _grad_u_face(_integrated ? _face_data._var_grads_face[_tid][_var_num] : moose_system._grad_zero[_tid]),
   _second_u_face(_integrated ? _face_data._var_seconds_face[_tid][_var_num] : moose_system._second_zero[_tid])
{
  ...
  if(_integrated)
    addVarNums(_face_data._boundary_to_var_nums[_boundary_id]);
  else
    addVarNums(_face_data._boundary_to_var_nums_nodal[_boundary_id]);
}
...
{
  _qp = 0;
  _current_residual->set(_face_data._nodal_bc_var_dofs[_tid][_var_num], _element_data._scaling_factor[_var_num]*computeQpResidual());
}
...
BoundaryCondition::coupledValFace(std::string name)
{
  ...
  if(_integrated)
    return _face_data._var_vals_face[_tid][_coupled_as_to_var_num[name]];

  return _face_data._var_vals_face_nodal[_tid][_coupled_as_to_var_num[name]];
}
...
BoundaryCondition::coupledGradFace(std::string name)
{
  ...
  if(_integrated)
    return _face_data._var_grads_face[_tid][_coupled_as_to_var_num[name]];
