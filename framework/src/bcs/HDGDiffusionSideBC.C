...
registerMooseObject("MooseApp", HDGDiffusionSideBC);

InputParameters
HDGDiffusionSideBC::validParams()
{
...
HDGDiffusionSideBC::HDGDiffusionSideBC(const InputParameters & parameters)
...
ADReal
HDGDiffusionSideBC::computeQpResidual()
{
  const Real h_elem = _current_elem_volume / _current_side_volume;
  return _alpha / h_elem * _diff[_qp] * (_u[_qp] - _func.value(_t, _q_point[_qp])) * _test[_i][_qp];
}
