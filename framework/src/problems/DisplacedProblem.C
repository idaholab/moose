...
DisplacedProblem::updateMesh(bool mesh_changing)
{
  ...
  Threads::parallel_reduce(node_range, udmt, this->numThreads());
...
DisplacedProblem::undisplaceMesh()
{
  ...
  Threads::parallel_reduce(node_range, rdmt, this->numThreads());
}
...
DisplacedProblem::UpdateDisplacedMeshThread::init()
{
  ...
  for (auto & [sys_num, var_num_and_direction] : _sys_to_var_num_and_direction)
  {
    ...
    Threads::parallel_reduce(node_range, send_list, this->_fe_problem.numThreads());
    send_list.unique();
