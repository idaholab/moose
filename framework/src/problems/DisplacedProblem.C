...
void
DisplacedProblem::swapBackMaterials(THREAD_ID tid)
{
  _mproblem.swapBackMaterials(tid);
}

void
DisplacedProblem::swapBackMaterialsFace(THREAD_ID tid)
{
  auto && elem = _assembly[tid]->elem();
  unsigned int side = _assembly[tid]->side();

  _mproblem.swapBackMaterialsFace(_ref_mesh.elemPtr(elem->id()), side, tid);
}

void
DisplacedProblem::swapBackMaterialsNeighbor(THREAD_ID tid)
{
  ...
  _mproblem.swapBackMaterialsNeighbor(_ref_mesh.elemPtr(neighbor->id()), neighbor_side, tid);
}

void
DisplacedProblem::reinitMaterialsFace(SubdomainID blk_id, THREAD_ID tid, bool swap_stateful)
{
  if (hasActiveMaterialProperties(tid))
    _mproblem.reinitMaterialsFace(_ref_mesh.elemPtr(_assembly[tid]->elem()->id()),
                                  _assembly[tid]->side(),
...
void
DisplacedProblem::reinitMaterialsNeighbor(SubdomainID blk_id, THREAD_ID tid, bool swap_stateful)
{
  if (hasActiveMaterialProperties(tid))
  {
    // NOTE: this will not work with h-adaptivity
    ...
    _mproblem.reinitMaterialsNeighbor(
        _ref_mesh.elemPtr(neighbor->id()), neighbor_side, n_points, blk_id, tid, swap_stateful);
  }
}
