...
validParams<PicardSolve>()
{
  ...
  params.addParam<unsigned int>(
      "picard_max_its",
      1,
      "Maximum number of times each timestep will be solved.  Mainly used when wanting to do "
      "Picard iterations with MultiApps that are set to execute_on timestep_end or timestep_begin. "
  ...
  params.addParam<Real>(
      "picard_rel_tol",
      1e-8,
      "The relative nonlinear residual drop to shoot for during Picard iterations.  This check is "
      "performed based on the Master app's nonlinear residual.");
  params.addParam<Real>("picard_abs_tol",
                        1e-50,
                        "The absolute nonlinear residual to shoot for during Picard iterations. "
  ...
  params.addParamNamesToGroup(
      "picard_max_its accept_on_max_picard_iteration disable_picard_residual_norm_check "
      "picard_rel_tol picard_abs_tol picard_force_norms relaxation_factor relaxed_variables",
      "Picard");
