...
Transient::incrementStepOrReject()
{
  if (lastSolveConverged())
  {
    ...
    else
    {
       ...
       */
      if (_fe_problem.solverParams()._has_picard_its)
      {
...
Transient::endStep(Real input_time)
{
  ...
  if (lastSolveConverged() && !_problem.XFEMRepeatStep())
  {
...
Transient::keepGoing()
{
  ...
  if (lastSolveConverged() && !_problem.XFEMRepeatStep() && _steady_state_detection == true &&
