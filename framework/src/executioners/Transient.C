...
Transient::execute()
{
  ...
  if (lastSolveConverged())
  {
     ...
     */
    if (!_iterative_multiapp_solve->autoAdvance())
    {
...
Transient::takeStep(Real input_dt)
{
  ...
  {
    _time_stepper->step();
    _xfem_repeat_step = _iterative_multiapp_solve->XFEMRepeatStep();
  ...
  if (!(_problem.haveXFEM() && _iterative_multiapp_solve->XFEMRepeatStep()))
  {
    if (lastSolveConverged())
