...
// App includes
#include "GhostAllPointNeighbors.h"
...
GhostAllPointNeighbors::GhostAllPointNeighbors(const InputParameters & params)
  : RelationshipManager(params)
{
}

GhostAllPointNeighbors::GhostAllPointNeighbors(const GhostAllPointNeighbors & other)
  : RelationshipManager(other)
{
}

std::string
GhostAllPointNeighbors::getInfo() const
{
  std::ostringstream oss;
  oss << "GhostAllPointNeighbors";
  return oss.str();
}

void
GhostAllPointNeighbors::operator()(const MeshBase::const_element_iterator & range_begin,
                                   ...
                                   map_type & coupled_elements)
{
  ...
  for (const Elem * const elem : as_range(range_begin, range_end))
    ...
    {
      // We want to ghost the point neighbors if the node hasn't been partitioned yet
      ...
      if (node_pid == p || node_pid == DofObject::invalid_processor_id)
      {
        ...
        for (const auto elem_id : elem_node_neighbors)
        {
          ...
          if (elem_node_neighbor->processor_id() != p)
...
bool
GhostAllPointNeighbors::operator>=(const RelationshipManager & other) const
{
  return dynamic_cast<const GhostAllPointNeighbors *>(&other) ||
