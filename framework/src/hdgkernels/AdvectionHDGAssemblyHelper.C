...
AdvectionHDGAssemblyHelper::validParams()
{
  ...
  params.addRequiredParam<MaterialPropertyName>("velocity", "Cell velocity vector");
...
AdvectionHDGAssemblyHelper::scalarVolume()
{
  for (const auto qp : make_range(_ip_qrule->n_points()))
  {
    ...
    const auto qp_term = _ip_JxW[qp] * _velocity[qp] * advected_quantity;
...
{
  const auto vdotn = faceVelocity(qp) * _ip_normals[qp];
