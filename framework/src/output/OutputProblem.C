...
OutputProblem::OutputProblem(const std::string & name, InputParameters parameters):
    ...
    _mesh(*meshInitHelper(_mproblem, parameters)),
    _eq(_mesh),
    _out(_mproblem, _eq)
{
...
// Helper function for setting up mesh reference
MooseMesh *
OutputProblem::meshInitHelper(FEProblem & fe_problem, InputParameters & params)
{
  MooseMesh *mesh;
