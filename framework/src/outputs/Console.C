...
InputParameters validParams<Console>()
{
  ...
  params.addParam<bool>("Newton_postprocessors", false, "Output postprocessor tables to the screen on nonlinear Newton steps");
...
Console::Console(const std::string & name, InputParameters parameters) :
    ...
    _newton_pps(getParam<bool>("Newton_postprocessors")),
...
Console::output()
{
  ...
  if (onNonlinearResidual())
  {
    ...
    if (hasPostprocessorOutput() && _newton_pps && _nonlinear_iter>0)
    {
      // FIXME: need to set the time correctly here: suggested formula _time + _dt*_nonlinear_iter*0.001
...
void
Console::outputPostprocessorOnNewton()
{
  _newton_pps = true;
}

void
Console::noOutputPostprocessorOnNewton()
{
  _newton_pps = false;
}

bool
Console::postprocessorOnNewtonAllowed() const
{
  return _newton_pps;
}
