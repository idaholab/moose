...
MaterialPropertyDebugOutput::printMaterialMap() const
{
  ...
  // Write the stored strings to the SimulationInformation output objects
  _console << "Materials:\n";
  _console << std::setw(SimulationInformation::_field_width) << "  Active Materials on Subdomain:\n";
  _console << std::setw(SimulationInformation::_field_width) << active_block.str() << '\n';

  _console << std::setw(SimulationInformation::_field_width) << "  Active Face Materials on Subdomain:\n";
  _console << std::setw(SimulationInformation::_field_width) << active_face.str() << '\n';

  _console << std::setw(SimulationInformation::_field_width) << "  Active Neighboring Materials on Subdomain:\n";
  _console << std::setw(SimulationInformation::_field_width) << active_neighbor.str() << '\n';

  _console << std::setw(SimulationInformation::_field_width) << "  Active Materials on Boundaries:\n";
  _console << std::setw(SimulationInformation::_field_width) << active_boundary.str() << '\n';
}
...
MaterialPropertyDebugOutput::printMaterialProperties(std::stringstream & output, const std::vector<Material * > & materials) const
{
  ...
  for (std::vector<Material *>::const_iterator jt = materials.begin(); jt != materials.end(); ++jt)
  {
    ...
    output << std::left << std::setw(SimulationInformation::_field_width) << "      Material Name: " << (*jt)->name() << '\n';

    // Build stream for properties using the SimulationInformation helper functions to wrap the names if there are too many for one line
    std::streampos begin_string_pos = output.tellp();
    std::streampos curr_string_pos = begin_string_pos;
    output << std::left << std::setw(SimulationInformation::_field_width) << "      Property Names: ";
    for (std::set<std::string>::const_iterator prop_it = props.begin(); prop_it != props.end(); ++prop_it)
    {
      output << "\"" << (*prop_it) << "\" ";
      curr_string_pos = output.tellp();
      SimulationInformation::insertNewline(output, begin_string_pos, curr_string_pos);
    }
    output << '\n';
  }
}
