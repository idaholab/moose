...
SampledOutput::initSample()
{
  ...
  for (const auto sys_num : make_range(num_systems))
  {
    ...
    if (num_vars > 0)
    {
      ...
      // else
      //   // not serialized
      //   _serialized_solution->init(source_sys.solution->size(), false,
      //   source_sys.solution->type());
...
SampledOutput::cloneMesh()
{
  ...
  // Partition the same way as the initial mesh
  // auto pl = _mesh_ptr->getPointLocator();
  // pl->enable_out_of_mesh_mode();
  // for (const auto & elem : _cloned_mesh_ptr->getMesh().active_element_ptr_range())
  // {
  //   const auto & elem_found = (*pl)(elem->vertex_average());
  //   if (elem_found)
  //     elem->processor_id() = elem_found->processor_id();
  //   else
  //     elem->processor_id() =
  // }
