...
MultiAppDTKUserObjectEvaluator::MultiAppDTKUserObjectEvaluator(MultiApp & multi_app,
  ...
  : _multi_app(multi_app), _user_object_name(user_object_name)
{
}
...
MultiAppDTKUserObjectEvaluator::evaluate(const Teuchos::ArrayRCP<GlobalOrdinal> & bids,
                                         const Teuchos::ArrayRCP<double> & coords)
{
  Teuchos::RCP<const Teuchos::Comm<int>> comm = Teuchos::rcp(new Teuchos::MpiComm<int>(
      Teuchos::rcp(new Teuchos::OpaqueWrapper<MPI_Comm>(_multi_app.comm()))));
  ...
  for (GlobalOrdinal i = 0; i < static_cast<GlobalOrdinal>(num_values); i++)
  {
    ...
    if (std::binary_search(_box_ids.begin(), _box_ids.end(), bids[i]))
    {
      ...
      evaluated_data[i] = _multi_app.appUserObjectBase(app, _user_object_name)
                              .spatialValue(p - _multi_app.position(app));
    }
    else
...
MultiAppDTKUserObjectEvaluator::createSourceGeometry(
    const Teuchos::RCP<const Teuchos::Comm<int>> & comm)
{
  ...
  return Teuchos::rcp(new DataTransferKit::GeometryManager<DataTransferKit::Box, GlobalOrdinal>(
      _boxes, _box_ids, comm, 3));
}
