...
FileMesh::FileMesh(const InputParameters & parameters)
  : MooseMesh(parameters), _file_name(getParam<MeshFileName>("file")), _exreader(nullptr)
{
  getMesh().set_mesh_dimension(getParam<MooseEnum>("dim"));
}

FileMesh::FileMesh(const FileMesh & other_mesh)
  : MooseMesh(other_mesh), _file_name(other_mesh._file_name), _exreader(nullptr)
{
}
...
FileMesh::buildMesh()
{
  ...
  bool is_nemesis = _pre_split && (_file_name.rfind(".exd") < _file_name.size() ||
