...
bool
MooseMesh::hasSubdomainID(const SubdomainID & id)
{
  std::set<SubdomainID> ids;
  ids.insert(id);
  return hasSubdomainID(ids);
}

bool MooseMesh::hasSubdomainID(const std::set<SubdomainID> & ids)
{
  // ANY_BLOCK_ID in supplied set always produces a true
  if (ids.find(Moose::ANY_BLOCK_ID) != ids.end())
    return true;
  else
  {
    const std::set<SubdomainID> & valid_ids = meshSubdomains();
    return std::includes(valid_ids.begin(), valid_ids.end(), ids.begin(), ids.end());
  }
}

bool
MooseMesh::hasBoundaryID(const BoundaryID & id)
{
  std::set<BoundaryID> ids;
  ids.insert(id);
  return hasBoundaryID(ids);
}

bool MooseMesh::hasBoundaryID(const std::set<BoundaryID> & ids)
{
  // ANY_BOUNDARY_ID in supplied set always produces a true
  if (ids.find(Moose::ANY_BOUNDARY_ID) != ids.end())
    return true;
  else
  {
    const std::set<BoundaryID> & valid_ids = meshBoundaryIds();
    return std::includes(valid_ids.begin(), valid_ids.end(), ids.begin(), ids.end());
  }
}
