...
SlaveNeighborhoodThread::SlaveNeighborhoodThread(SlaveNeighborhoodThread & x,
  ...
  : _kd_tree(x._kd_tree),
    ...
    _patch_update_strategy(x._patch_update_strategy)
{
}
...
SlaveNeighborhoodThread::operator()(const NodeIdRange & range)
{
  ...
  for (const auto & node_id : range)
  {
    ...
    if (_patch_update_strategy == 3) // nonlinear_iter patch update strategy
      // Set the slave node's neighbors irrespective of which processor they belong to because the
      // full master boundary is ghosted
      _neighbor_nodes[node_id] = neighbor_nodes;
    else // all other patch update strategies
    {
