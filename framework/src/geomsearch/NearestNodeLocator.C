...
NearestNodeLocator::findNodes()
{
  ...
  if (_first || (_reinit_iteration && _patch_update_strategy == Moose::Iteration))
  {
    ...
    Threads::parallel_reduce(trial_secondary_node_range, snt, _subproblem.numThreads());
    ...
    if (_patch_update_strategy == Moose::Iteration)
    {
      ...
      Threads::parallel_reduce(trial_secondary_node_range, snt_ghosting, _subproblem.numThreads());
  ...
  Threads::parallel_reduce(*_secondary_node_range, nnt, _subproblem.numThreads());
...
NearestNodeLocator::updatePatch(std::vector<dof_id_type> & secondary_nodes)
{
  ...
  Threads::parallel_reduce(secondary_node_range, snt, _subproblem.numThreads());

  // Calculate new ghosting patch for the secondary_node_range
  SecondaryNeighborhoodThread snt_ghosting(
      _mesh, trial_primary_nodes, node_to_elem_map, _mesh.getGhostingPatchSize(), kd_tree);

  Threads::parallel_reduce(secondary_node_range, snt_ghosting, _subproblem.numThreads());
  ...
  Threads::parallel_reduce(tracked_secondary_node_range, nnt, _subproblem.numThreads());
