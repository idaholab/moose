...
ComputeMortarFunctor::operator()()
{
  ...
  // Loop through secondary elements, accumulating quadrature points for all corresponding mortar segments
  for (const auto & elem_to_msm : secondary_elems_to_mortar_segments)
  {
    ...
    for (const auto msm_elem : msm_elems)
    {
      ...
      mooseAssert(secondary_xi_pts.size() == expected_length, "Fewer than expected secondary quadrature points");
      mooseAssert(primary_xi_pts.size() == expected_length, "Fewer than expected primary quadrature points");
    ...
    for (const auto msm_elem : msm_elems)
    {
      ...
      // Extract previously computed mapped quadrature points for secondary and primary face elements
      const unsigned int start = (n_segment - 1) * _qrule_msm->n_points();
      const unsigned int end = n_segment * _qrule_msm->n_points();
      xi1_pts.insert(xi1_pts.begin(), secondary_xi_pts.begin() + start, secondary_xi_pts.begin() + end);
                                       ...
                                       std::vector<Point> & q_pts)
{
  ...
  for (auto qp : make_range(_qrule_msm->n_points()))
  {
    ...
    for (MooseIndex(msm_elem->n_nodes()) n = 0; n < msm_elem->n_nodes(); ++n)
      x0 += Moose::fe_lagrange_2D_shape(msm_type, msm_order, n, static_cast<const TypeVector<Real> &>(_qrule_msm->qp(qp))) *
            msm_elem->point(n);
    ...
    if (current_iterate < max_iterates)
    {
      ...
      {
        if (q_pts.back()(0) < -1 || q_pts.back()(0) > 1 || q_pts.back()(1) < -1 || q_pts.back()(1) > 1)
        {
          mooseError("Quadrature point: ", q_pts.back(), " out of bounds, truncating");
        }
      }
