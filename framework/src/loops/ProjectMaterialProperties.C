  ...
  if (_refine)
  {
    ...
    _material_props.prolongStatefulProps(
        ...
        *elem,
        ...
        -1,
        -1,
        _mesh.doingPRefinement()); // Gets us volume projection
  }
  else
  {
    ...
    if (_refine)
    {
      ...
      _bnd_material_props.prolongStatefulProps(
          ...
          *elem,
          ...
          -1,
          side,
          _mesh.doingPRefinement()); // Gets us side to side projection
    }
    else
    {
      const std::vector<std::pair<unsigned int, QpMap>> & coarsening_map =
        ...
        if (!elem->is_child_on_side(child, side)) // Otherwise we already projected it
        {
          ...
          _bnd_material_props.prolongStatefulProps(
              ...
              *elem,
              ...
              child,
              side,
              _mesh.doingPRefinement()); // Gets us volume to side projection
        }
      }
    }
  }
