...
setenv          PETSC_DIR       $base_path/petsc/petsc-!include docs/content/install/petsc_default_version.md/gcc-opt
```
...
!!! note
    Verify that the environment variable 'PETSC_DIR' is available and returning $PACKAGES_DIR/petsc/petsc-!include docs/content/install/petsc_default_version.md. If not, something went wrong with creating the moose-dev-gcc module above.

* Download and extract PETSc:
```bash
curl -L -O http://ftp.mcs.anl.gov/pub/petsc/release-snapshots/petsc-!include docs/content/install/petsc_default_version.md.tar.gz
tar -xf petsc-!include docs/content/install/petsc_default_version.md.tar.gz
cd petsc-!include docs/content/install/petsc_default_version.md
```
* Configure PETSc using the following options (see Info admonition below):

!include docs/content/install/petsc_default.md
