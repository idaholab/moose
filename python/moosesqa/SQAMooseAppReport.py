...
class SQAMooseAppReport(SQAReport):
    ...
    def __init__(self, **kwargs):
        ...
        self.content_directory = kwargs.pop(
            "content_directory", os.path.join(self.exe_directory, "doc", "content")
        )
    ...
    def execute(self, **kwargs):
        ...
        if not os.path.isdir(content_dir):
            raise NotADirectoryError(
                "'content_directory' input is not a directory: {}".format(content_dir)
            )
        ...
        exe = mooseutils.find_moose_executable(
            exe_dir, name=self.exe_name, show_error=False
        )
        if exe is None:
            raise OSError(
        ...
        if self.app_syntax is None:
            ...
            self.app_syntax = moosesyntax.get_moose_syntax_tree(
                exe, remove=remove, alias=alias, unregister=unregister
            )
        ...
        if filenames is not None:
            for fname in filenames:
                ...
                if not os.path.isfile(yml_file):
                    raise NotADirectoryError(
                        "YAML file is not a directory: {}".format(fname)
                    )
