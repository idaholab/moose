...
class TestCheckSyntax(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if self._testMethodName == "testMissing":
            setattr(
                sys.modules["moosesqa.check_syntax"], "find_md_file", lambda *args: None
            )
        else:
            setattr(
                sys.modules["moosesqa.check_syntax"],
                "find_md_file",
                TestCheckSyntax.find_md_file,
            )

        if self._testMethodName in ("testIsStub",):
            setattr(
                sys.modules["moosesqa.check_syntax"], "file_is_stub", lambda *args: True
            )
        else:
            setattr(
                sys.modules["moosesqa.check_syntax"],
                "file_is_stub",
                TestCheckSyntax.file_is_stub,
            )
        ...
        if self._testMethodName in ("testGiantIfStatement",):
            setattr(
                sys.modules["moosesqa.check_syntax"],
                "_check_node",
                self._patch_check_node,
            )
        else:
            setattr(
                sys.modules["moosesqa.check_syntax"],
                "_check_node",
                TestCheckSyntax._check_node,
            )
    ...
    def testDuplicateFiles(self):
        ...
        self.assertIn(
            "Located multiple files for the desired markdown: Kernels/index.md",
            cm.output[0],
        )
