...
class TestContentOutline(MooseDocsTestCase):
    # Eventually, MooseDocs/test/content should serve as a fast and simplified version of the main
    # website for experimentation purposes. The setup for MooseDocsTestCases, which use those
    # MooseDown pages for content*, therefore, need to be carefully configured to avoid failures
    # caused by future modifications.
    #
    # The logic for setting up MooseDocs content to avoid unnecssary tokenize errors is as follows:
    #
    # EXTENSIONS: All available MooseDocs extensions are added so that commands are recognized
    #             during tokenization of actived pages in content/extensions/.
    # ACTIVE: Only those extensions necessary to render an outline are set as 'active'. If a page
    #         invokes an inactive extension, it will be ignored and won't produce a tokenize error.
    # PAGES: All pages in content/extensions/ are added, but only a few are set as 'active'.
    #        Otherwise, every page would be tokenized and there would be too many headings
    #        generated for testing of extension commands. Any cross-linking between active and/or
    #        inactive MooseDown pages will be recognized and won't produce a tokenize error.
    # TEXT: Raw syntax for invoking !content outline command. Two cases: first is a test for
    #       compiling all headings from all (active) pages in a directory specified by 'location'.
    #       Only level 1 & 2 headeres are rendered, since `max_level=2`, and any heading with
    #       `id=outline-directory` will not be rendered. Second is a test for compiling only
    #       all headings from those pages specified by 'pages'. Headings should also be rendered
    #       in order of input. `max_level=6`, so all headings, except any with 'id=outline-pages',
    #       should be rendered.
    # HEADINGS: A list of dictionaries of dictionaries corresponding to the two test cases
    #           invoked by TEXT. The main keys of the dictionaries are the names of the headings
    #           in raw text format. Each item includes attributes to identify page, id, and level.
    #
    # If the HEADINGS generated by PAGES are rearranged, deleted, or if more are added in between,
    # this test will fail.
    #
    # *TODO: create specialized directories with MooseDown pages used exclusively for test content
    #

    EXTENSIONS = list(extensions.__dict__.values())[9:]
    ACTIVE = ['core', 'command', 'heading', 'content']
    PAGES = ['core.md', 'materialicon.md', 'content.md', 'config.md']
    ...
    def setupContent(self):
        ...
        for page in content[1:]:
            page['active'] = False if page._name not in self.PAGES else True
        return content

    def setupExtension(self, ext):
        if ext.__name__.split('.')[-1] not in self.ACTIVE:
            return dict(active=False)

    def testAST(self):
        for case in range(len(self.TEXT)):
            ast = self.tokenize(self.TEXT[case])
            self.assertSize(ast, 1)
            if case == 0:
                self.assertToken(ast(0), 'ContentOutline',
                                 location='extensions',
                                 pages=[],
                                 max_level=2, hide=['outline-directory'])
    ...
    # Latex test might be needed in the future
    def testLatex(self):
        return None
