...
class TestTaggingCommand(MooseDocsTestCase):
    ...
    def setupExtension(self, ext):
        if ext == tagging:
            return dict(
                active=True, allowed_keys=["application", "foo"], js_file="tagging.js"
            )

    def setupContent(self):
        config = [
            dict(root_dir="python/MooseDocs/test/content", content=["js/tagging.js"])
        ]
    ...
    def testTaggingDuplicateNamesWarning(self):
        ...
        self.assertEqual(
            ast2(0)["attr_name"], ""
        )  # Not added to global attributes, so name attribute is empty
