...
class TestStatements(MooseDocsTestCase):
    ...
    def testIfElif(self):
        ...
        # False/True
        ast = self.tokenize(
            "!if function=testFunction(False)\ncontent\n\n!elif function=testFunction(True)\nelif"
        )
        self.assertToken(ast(0), "Statement", size=2)
        self.assertToken(ast(0, 0), "Condition", size=0, command="if")
    ...
    def testIfCommandBaseErrors_evaluateFunction(self):
        ...
        self.assertToken(
            ast(1),
            "ErrorToken",
            message="The return value from the function 'testFunction' must be a 'bool' type, but '<class 'int'>' returned.",
        )
