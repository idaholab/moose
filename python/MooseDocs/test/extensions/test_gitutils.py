...
class TestGitUtilsCommit(MooseDocsTestCase):
    ...
    def testContentException(self):
        ...
        self.assertIn(
            "Content is not supported for the 'git commit' command.", ast(0)["message"]
        )
        ...
        self.assertToken(ast(0), "ErrorToken")
        self.assertIn(
            "The current working directory is not a git repository.", ast(0)["message"]
        )
...
class TestGitUtilsCommit(MooseDocsTestCase):
    ...
    def testJoin(self):
        ast = self.tokenize(
            "[!git!submodule-hash url=https://github.com/petsc/petsc/commit/](petsc)"
        )
        link = ast(0, 0)
