...
from MooseDocs.common.get_content import (
    _build_regex,
    _find_files,
    _doc_import,
    get_content,
...
class TestBuildRegex(unittest.TestCase):
    def testBasic(self):
        ...
        path = "*/three/*/nine/*"
        self.assertEqual(
            _build_regex(path), r"^(?:[^/]*?)/three/(?:[^/]*?)/nine/(?:[^/]*?)$"
        )
        ...
        self.assertEqual(
            _build_regex(path), r"^(?:.*?)/three/(?:[^/]*?)/nine/(?:.*?)/foo\.md$"
        )
...
class TestDocTree(unittest.TestCase):
    def testBasic(self):
        ...
        items = [
            dict(
                root_dir=os.path.join(ROOT_DIR, "framework/doc/content"),
