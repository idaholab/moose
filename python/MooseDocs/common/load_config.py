    ...
    for file in filenames:
        ...
        if current is not None:
            if destined and current != destination:
                msg = (
                    "The translator destination '{}' was specified by {}, but another "
...
def _get_module(ext):
    ...
    if isinstance(ext, types.ModuleType):
        ...
        msg = (
            "The supplied module ({}) must be a module type or a string, but a {} object "
            "was provided."
        )
...
def _yaml_load_extensions(config):
    ...
    for ext_type, settings in options.items():
        if (settings is not None) and ("type" in settings):
            msg = (
                "Using 'type' for the extensions is deprecated, the type should be supplied "
        ...
        if isinstance(settings, dict):
            ...
            msg = (
                "The supplied settings for the '%s' extension must be dict() or the 'default' "
