...
DEFINITION_RE = re.compile(
    r"(register(|AD)MooseObject(|Aliased)|registerKokkos\w+)\(\"\w+App\"(,\s+|,\s+\w+,\s+\")(?P<class>\w+)\"?\);"
)
...
def _locate_filenames(directories, ext):
    ...
    for location in directories:
        for filename in mooseutils.git_ls_files(
            os.path.join(MooseDocs.ROOT_DIR, location)
        ):
...
def _match_input(objects, filename, match):
    ...
    if key.replace("AD", "") in objects and (
        key not in objects
        or (objects[key.replace("AD", "")].source == objects[key].source)
    ):
