...
class LocalListingCommand(command.CommandComponent):
    ...
    def createToken(self, parent, info, page, settings):
        flt = floats.create_float(
            parent, self.extension, self.reader, page, settings, token_type=Listing
        )
    ...
    def appendMooseSyntax(self, content: str, page: pages.Page) -> str:
        ...
        for node in moosetree.iterate(root):
            ...
            if isinstance(moose_node, SyntaxNode):
                parameters = [
                    param
                    for action in moose_node.actions()
            ...
            for param, val in node.params():
                ...
                content_lines[ci] = (
                    line[:ind] + "<<<" + json.dumps(attr) + ">>>" + line[ind:]
                )
...
class FileListingCommand(LocalListingCommand):
    ...
    def createToken(self, parent, info, page, settings):
        ...
        code = core.Code(
            flt, content=content, language=lang, max_height=settings["max-height"]
        )
    ...
    def codeDiff(before: str, after: str) -> str:
        ...
        diff = difflib.unified_diff(
            after_split, before_split, n=(len(before_split) + len(after_split))
        )
