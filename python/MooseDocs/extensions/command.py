        ...
        for cmd in commands:
            for sub in subcommands:
                ...
                if pair in CommandExtension.EXTENSION_COMMANDS[uid]:
                    msg = (
                        "A CommandComponent object exists with the command '{}' and "
                        "subcommand '{}'."
                    )
...
class CommandBase(components.ReaderComponent):
    ...
    def createToken(self, parent, info, page, _):
        ...
        if info["subcommand"]:
            ...
            if match:
                ...
                cmd = (
                    info["command"],
                    "hit",
                )  # consider 'tests' as input files for hit formatting
        ...
        try:
            ...
            try:
                obj = CommandExtension.EXTENSION_COMMANDS[page.translator.uid][
                    (cmd[0], "*")
                ]
            except KeyError:
