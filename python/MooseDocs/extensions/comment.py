...
class CommentInline(components.ReaderComponent):
    ...
    RE = re.compile(
        r"(?<!!)!{2}(?!!)(?P<content>.*?)$", flags=re.UNICODE | re.MULTILINE
    )
...
class HTMLCommentBlock(components.ReaderComponent):
    ...
    RE = re.compile(
        r"<!--(?P<content>.*?)-->", flags=re.UNICODE | re.MULTILINE | re.DOTALL
    )

    def createToken(self, parent, info, page, settings):
        raise RuntimeError(
            "Use of HTML-style comments are not allowed. Please use '!!' for inline and '!!! .... !!!' for block syntax!"
        )
