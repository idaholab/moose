...
Tag = tokens.newToken(
    "Tag", attr_name="", path="", description="", image="", key_vals=[]
)
...
class TaggingExtension(command.CommandExtension):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self["js_file"] is None:
            msg = (
                "No javascript file identified. The tagging extension will be disabled."
            )
            LOG.warning(msg)
        ...
        if self["active"] is True:
            LOG.warning(
                "The tagging extension is experimental! If documentation tagging features "
                ...
                for entry in key_list_regex:
                    ...
                    if entry == "path":
                        ...
                        if len(target_page) != 1:
                            LOG.error(
                                str(len(target_page))
                                + " pages found after truncating address when "
                                "tagging for page initially at address: " + path_value
                            )
                        if len(filter_page) != 1:
                            LOG.warning(
                                str(len(filter_page))
                                + " pages have been found for the filter page "
...
class TaggingCommand(command.CommandComponent):
    ...
    def createToken(self, parent, info, page, settings):
        ...
        if settings["name"] is None:
            msg = (
                "%s: No 'name' provided for page and associated tags; check markdown file. "
        ...
        for pair in entry_key_values:
            if (
                pair[0] not in self.extension.allowed_keys
                and len(self.extension.allowed_keys) > 0
            ):
                msg = (
                    "%s: Provided 'key' not in allowed_keys (see config.yml); not adding the "
        ...
        if len(name) != 0:  # Only add to tag database if 'name' is provided
            page_data = {
                ...
                "image": image,
                "key_vals": dict(good_keys),
            }

            tag_id_name = ""
            if self.extension.get_tag_data("tag_" + name):
                msg = (
                    "%s: Tag page identifier already exists; not adding the following 'name' to "
                    "dictionary: %s"
                )
