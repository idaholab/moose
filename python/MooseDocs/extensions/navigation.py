...
class NavigationExtension(Extension):
    ...
    def defaultConfig():
        ...
        config["menu"] = (
            dict(),
            "Navigation items, this can either be a *.menu.md file or dict. "
            "The former creates a 'mega men' and the later uses dropdowns.",
        )
        config["google-cse"] = (
            None,
            "Enable Google search by supplying the Google 'search engine ID'.",
        )
        ...
        config["long-name"] = (
            None,
            "A long version of the page name that is used in the title, 'name' by default.",
        )
        config["breadcrumbs"] = (
            True,
    ...
    def extend(self, reader, renderer):
        ...
        if (not menu) and (renderer.get("navigation", None) is not None):
            msg = (
                "The navigation setting in the MaterializeRenderer is deprecated, "
    ...
    def postRender(self, page, result):
        ...
        root = result.root
        header = tree.find(root, lambda n: n.name == "header")
        nav = html.Tag(html.Tag(header, "nav"), "div", class_="nav-wrapper container")
        container = tree.find(root, lambda n: n.name == "main").children[0]
        ...
        if (self.getConfig(page, "google-cse") is not None) or self.getConfig(
            page, "search"
        ):
    ...
    def postTokenize(self, page, ast):
        ...
        index = []
        title = None
        for head in tree.findall(ast, lambda n: n.name == "Heading"):
    ...
    def postExecute(self):
        ...
        if home is None:
            LOG.warning(
                "The use of a URL in the 'home' option is deprecated. By default the home "
    ...
    def _addName(self, nav, page):
        ...
        if name is not None:
            a = html.Tag(
                None,
                "a",
                class_="left moose-logo hide-on-med-and-down",
                href=href,
                string=str(name),
                id_="home-button",
            )
    ...
    def _addRepo(self, nav, page):
        ...
        if "github" in repo:
            ...
            html.Tag(
                a,
                "img",
                src=img0.relativeDestination(page),
                alt="GitHub wordmark",
                class_="github-mark",
            )
            html.Tag(
                a,
                "img",
                src=img1.relativeDestination(page),
                alt="GitHub logo",
                class_="github-logo",
            )
            ...
            html.Tag(
                a,
                "img",
                src=img.relativeDestination(page),
                alt="GitLab logo",
                class_="gitlab-logo",
            )
    ...
    def _addSearch(self, parent, page):
        ...
        # Search modal
        div = html.Tag(
            tree.find(parent.root, lambda n: n.name == "header"),
            "div",
            id_="moose-search",
            class_="modal modal-fixed-footer moose-search-modal",
        )
        container = html.Tag(
            div, "div", class_="modal-content container moose-search-modal-content"
        )
        ...
        if cx is not None:
            ...
            html.Tag(
                result_wrapper, "div", id_="moose-search-results", class_="col s12"
            )
    ...
    def _addSideNavigation(self, parent, page):
        ...
        a["data-target"] = id_
        html.Tag(a, "i", class_="material-icons", string="menu")
    ...
    def _addBreadcrumbs(self, container, page):
        ...
        for i in range(1, len(parts)):
            current = self.translator.findPage(
                lambda p: p.local == os.path.join(*parts[:i])
            )

            if isinstance(current, pages.Directory):
                idx = self.translator.findPages(
                    os.path.join(current.local, "index.md"), exact=True
                )
                if idx:
                    url = os.path.relpath(
                        current.local, os.path.dirname(page.local)
                    ).replace(".md", ".html")
                    a = html.Tag(
    ...
    def _addSections(self, container, page):
        ...
        for node in tree.iterate(container, lambda n: n.name == "section"):
    ...
    def _addContents(self, toc, content, page):
        ...
        for node in tree.iterate(content):
    ...
    def _createNavigation(self, ul, page, mega=True):
        ...
        for key, value in self.get("menu", dict()).items():
            ...
            if isinstance(value, str) and value.endswith("menu.md") and mega:
                ...
                html.Tag(
                    a, "i", class_="material-icons right", string="arrow_drop_down"
                )
                ...
                id_ = uuid.uuid4()
                a = html.Tag(
                    li, "a", class_="dropdown-trigger", href="#!", string=str(key)
                )
                a["data-target"] = id_
    ...
    def _addMegaMenu(self, parent, filename, page):
        ...
        id_ = uuid.uuid4()
        header = tree.find(parent.root, lambda n: n.name == "header")
        ...
        key = (
            ...
            node.local,
        )  # cache on current directory and menu name
