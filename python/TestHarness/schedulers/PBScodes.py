...
PBS_User_EXITCODES = {
    ...
    -5: (
        "JOB_EXEC_INITRST",
        "Job aborted on MoM initialization, checkpoint, no migrate",
    ),
    -6: (
        "JOB_EXEC_INITRMG",
        "Job aborted on MoM initialization, checkpoint, ok migrate",
    ),
    ...
    -14: (
        ...
        "Job was requeued (if rerunnable) or deleted (if not) due to a communication failure between the primary execution host MoM and a Sister",
    ),
    -15: ("JOB_EXEC_QUERST", "Requeue job for restart from checkpoint"),
    -16: (
        "JOB_EXEC_FAILHOOK_RERUN",
        "Job execution failed due to hook rejection; requeue for later retry",
    ),
    -17: (
        "JOB_EXEC_FAILHOOK_DELETE",
        "Job execution failed due to hook rejection; delete the job at end",
    ),
    ...
    -20: (
        "JOB_EXEC_RERUN_MS_FAIL",
        "Job requeued because server could not contact the primary execution host MoM",
    ),
    -21: ("JOB_EXEC_FAIL_SECURITY", "Security breach in PBS directory"),
    -22: (
        "JOB_EXEC_HOOKERROR",
        "Job exec failed due to unexpected exception or hook execution timed out",
    ),
    -23: ("JOB_EXEC_FAIL_KRB5", "Error no kerberos credentials supplied"),
    -24: (
        "JOB_EXEC_KILL_NCPUS_BURST",
        "Job exec failed due to exceeding ncpus (burst)",
    ),
...
PBS_Server_EXITCODES = {
    "1": " PBSE_UNKJOBID : Unknown Job Identifier",
