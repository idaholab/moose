...
import json
import datetime
from TestHarness.schedulers import RunHPC
    ...
    def updateHPCJobs(self, hpc_jobs):
        ...
        for hpc_job in hpc_jobs:
            ...
            with hpc_job.getLock():
                ...
                if state == "H" and (
                    job_result.get("Hold_Types") != "u" or self.options.hpc_no_hold
                ):
                ...
                if exit_code is not None:
                    if exit_code < 0:
                        name, reason = PBS_User_EXITCODES.get(
                            exit_code, ("TERMINATED", "Unknown reason")
                        )
                        ...
                        # Everything else should be an error
                            ...
                            self.setHPCJobError(
                                hpc_job,
                                f"PBS ERROR: {name}",
                                f"was terminated with reason: {reason}",
                            )
                    # Job was killed with a signal
                    elif exit_code >= 128:
                        self.setHPCJobError(
                            hpc_job, "PBS JOB KILLED", "was killed by a signal"
                        )
                    ...
                    if stime and resources_used:
                        ...
                        if walltime:
                            ...
                            if search:
                                walltime_sec = datetime.timedelta(
                                    hours=int(search.group(1)),
                                    minutes=int(search.group(2)),
                                    seconds=int(search.group(3)),
                                ).total_seconds()
                                if walltime_sec != 0:
