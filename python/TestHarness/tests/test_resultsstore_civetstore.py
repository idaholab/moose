...
class TestCIVETStore(TestHarnessTestCase):
    ...
    @unittest.skipUnless(
        os.environ.get("CIVET_EVENT_CAUSE") == "Pull request",
    ...
    @unittest.skipUnless(
        os.environ.get("CIVET_EVENT_CAUSE", "").startswith("Push"),
    ...
    def checkResult(
        ...
        result: dict,
        stored_results: dict,
        stored_tests: Optional[dict[TestName, dict]],
        build_kwargs: dict = {},
    ):
        """
        ...
        for test in results_test_iterator(result):
            ...
            if (
                build_kwargs.get("ignore_skipped")
                and test_entry["status"]["status"] == "SKIP"
            ):
    ...
    def runTestBuild(
        self,
        run_tests_args: list[str] = DEFAULT_TESTHARNESS_ARGS,
    ...
    def testBuildOversizedTest(self):
        ...
        base_sha, env = build_civet_env()
        run_tests_result = self.runTestsCached(
            *DEFAULT_TESTHARNESS_ARGS, **DEFAULT_TESTHARNESS_KWARGS
        )
        results = run_tests_result.results
    ...
    def testHasAuth(self):
        """
        Check if auth is available or not.

        This lets us within CIVET tests assert whether or not
        the authentication is available when we expect it to be
        so (or not).
        """
    ...
    def runTestStore(self, separate_tests: bool = False, **kwargs):
        """Run a test foro the store() method."""
        if separate_tests:
        ...
        self.checkResult(
            base_sha, env, results, stored_result, stored_tests, build_kwargs=kwargs
        )

    @unittest.skipUnless(HAS_AUTH, "Skipping because authentication is not available")
    def testStore(self):
        """Test store() with no additional arguments and contained tests."""
        self.runTestStore()

    @unittest.skipUnless(HAS_AUTH, "Skipping because authentication is not available")
    def testStoreSeparateTests(self):
        """Test store() with no additional arguments and separate tests."""
