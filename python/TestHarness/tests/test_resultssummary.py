...
class TestResultsSummary(TestHarnessTestCase):
    ...
    def getResult(self, **kwargs) -> StoredResult:
        ...
        converted = TestResultsStoredResults.convertTestHarnessResults(
            run_test_result.results, **kwargs
        )
    ...
    def testPRTestNamesNoChange(
        self, mock_get_commit_results, mock_get_event_results, mock_init_reader
    ):
        """
        ...
        with tempfile.NamedTemporaryFile() as out_file:
            ...
            base_results, head_results, base_test_names, head_test_names = (
    ...
    @patch.object(TestHarnessResultsSummary, "get_event_results")
    @patch.object(TestHarnessResultsSummary, "get_commit_results")
    def testPRTestNamesNoBaseResults(
        self, mock_get_commit_results, mock_get_event_results, mock_init_reader
    ):
        """
    ...
    def testBuildDiffTableRemovedTest(self, mock_get_commit_results, mock_init_reader):
        ...
        removed_table = summary._build_diff_table(
            base_test_names, base_result_with_tests
        )
    ...
    @patch.object(TestHarnessResultsSummary, "init_reader")
    @patch.object(TestHarnessResultsSummary, "get_event_results")
    def testBuildDiffTableAddedTestNoRunTime(
        self, mock_get_event_results, mock_init_reader
    ):
        """
    ...
    @patch.object(TestHarnessResultsSummary, "init_reader")
    @patch.object(TestHarnessResultsSummary, "get_event_results")
    def testBuildDiffTableStatusValueSKIP(
        self, mock_get_event_results, mock_init_reader
    ):
        """
    ...
    def testBuildSameTable(
        ...
        self.assertGreater(
            abs(float(same_table[0][3].strip("%"))), fake_run_time_rate_floor * 100
        )

    @patch.object(TestHarnessResultsSummary, "init_reader")
    @patch.object(TestHarnessResultsSummary, "get_event_results")
    @patch.object(TestHarnessResultsSummary, "get_commit_results")
    def testBuildSameTableBaseTimeZero(
        self, mock_get_commit_results, mock_get_event_results, mock_init_reader
    ):
        """
        Tests _build_same_table() when same test name exit in both base and head, where:
    ...
    @patch.object(TestHarnessResultsSummary, "get_event_results")
    @patch.object(TestHarnessResultsSummary, "get_commit_results")
    def testBuildSameTableNoHeadRunTime(
        self, mock_get_commit_results, mock_get_event_results, mock_init_reader
    ):
        """
    ...
    @patch.object(TestHarnessResultsSummary, "get_event_results")
    @patch.object(TestHarnessResultsSummary, "get_commit_results")
    def testBuildSameTableNoBaseRunTime(
        self, mock_get_commit_results, mock_get_event_results, mock_init_reader
    ):
        """
        Tests _build_same_table() when same test name exit in both base and head but
    ...
    @patch.object(TestHarnessResultsSummary, "get_event_results")
    @patch.object(TestHarnessResultsSummary, "get_commit_results")
    def testBuildSameTableLowBaseRunTime(
        self, mock_get_commit_results, mock_get_event_results, mock_init_reader
    ):
        """
        Tests _build_same_table() when same test name exit in both base and head but
    ...
    @patch.object(TestHarnessResultsSummary, "get_event_results")
    @patch.object(TestHarnessResultsSummary, "get_commit_results")
    def testBuildSameTableLowHeadRunTime(
        self, mock_get_commit_results, mock_get_event_results, mock_init_reader
    ):
        """
        Tests _build_same_table() when same test name exit in both base and head but
    ...
    @patch.object(TestHarnessResultsSummary, "get_event_results")
    @patch.object(TestHarnessResultsSummary, "get_commit_results")
    def testBuildSameTableLowRelativeRunTime(
        self, mock_get_commit_results, mock_get_event_results, mock_init_reader
    ):
        """
    ...
    @patch.object(TestHarnessResultsSummary, "get_event_results")
    @patch.object(TestHarnessResultsSummary, "get_commit_results")
    def testDiffTableSameTestDisableRunTimeOption(
        self, mock_get_commit_results, mock_get_event_results, mock_init_reader
    ):
        """
    ...
    def testBuildSummaryHasTests(self, mock_init_reader):
        ...
        summary = TestHarnessResultsSummary(None)
        has_test_build_summary = summary.build_summary(
            removed_table, added_table, same_table
        )
    ...
    def testPRNoEvent(self, mock_get_event_results, mock_init_reader):
        ...
        with tempfile.NamedTemporaryFile() as out_file:
            ...
            with redirect_stdout(stdout), self.assertRaisesRegex(
                SystemExit, "Results do not exist for event"
            ):
                summary.pr(event_id=EVENT_ID, out_file=out_file.name)
    ...
    @unittest.skipUnless(
        ...
        "Skipping because TEST_RESULTSSUMMARY not set",
    )
    @unittest.skipUnless(
        os.environ.get("CIVET_EVENT_CAUSE", "").startswith("Pull"),
        "Skipping because not on a pull request",
    )
    def testPRLive(self):
        """
