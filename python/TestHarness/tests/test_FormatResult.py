...
class TestFormatResult(unittest.TestCase):
    ...
    def testNameOnly(self):
        ...
        long_term_cols = 200
        default_longer = self.formatResult(
            options={"colored": False, "term_cols": long_term_cols}
        )
    ...
    def testNameAndStatus(self):
        ...
        joint_status = JointStatus(
            status=status, color=status_color, message=status_message
        )
        ...
        civet_no_color = self.formatResult(
            entry=entry, options={"term_format": CIVET_FORMAT, "colored": False}
        )
        ...
        civet_same_message = self.formatResult(
            entry=entry_same, options={"term_format": CIVET_FORMAT}
        )
    ...
    def testLongCaveats(self):
        ...
        result = self.formatResult(
            entry={"caveats": caveats}, options={"term_format": "cn", "term_cols": 40}
        )
        c, n = result.split(" ")
        ...
        result = self.formatResult(
            ...
            options={"term_format": "cn", "extra_info": True},
        )
        c, n = result.split(" ")
        self.assertEqual(c, caveats_joined)
        self.assertEqual(n, TEST_NAME)
        ...
        result = self.formatResult(
            entry={"caveats": caveats}, options={"term_format": "nc"}
        )
        n, c = result.split(" ")
        self.assertEqual(n, TEST_NAME)
        self.assertEqual(c, caveats_joined)
    ...
    def testOrder(self):
        ...
        result = self.formatResult(
            ...
            options={"colored": False, "term_format": "pnjcst", "timing": True},
        )
    ...
    def testTimingPrecision(self):
        def test(timing: float, formatted: str):
            result = self.formatResult(
                entry={"timing": timing}, options={"term_format": "t", "timing": True}
            )
