    ...
    def testPBSBadLaunch(self):
        ...
        with self.harnessDAG() as harness_dag:
            ...
            for job_id, job in self.yieldJobs(job_dag):
                ...
                # We only launched one job, so the results of the second job will be included
                if job_id == 0:
                    self.assertRegexpMatches(result_list[0], 'FAILED \(QSUB FAILURE\)')
                    self.assertRegexpMatches(result_list[1], 'skipped \(skipped dependency\)')
    ...
    def testPBSBadQstat(self):
        ...
        with self.harnessDAG() as harness_dag:
            ...
            for job_id, job in self.yieldJobs(job_dag):
                ...
                # We only launched one job, so the results of the second job will be included
                if job_id == 0:
                    self.assertRegexpMatches(result_list[0], 'FAILED \(INVALID QSTAT RESULTS\)')
                    self.assertRegexpMatches(result_list[1], 'skipped \(skipped dependency\)')
    ...
    def testPBSUnknownQstat(self):
        ...
        with self.harnessDAG() as harness_dag:
            ...
            for job_id, job in self.yieldJobs(job_dag):
                ...
                # We only launched one job, so the results of the second job will be included
                if job_id == 0:
                    self.assertRegexpMatches(result_list[0], 'FAILED \(UNKNOWN PBS STATUS\)')
                    self.assertRegexpMatches(result_list[1], 'skipped \(skipped dependency\)')
    ...
    def testPBSProcessResults(self):
        ...
        with self.harnessDAG() as harness_dag:
            ...
            for job_id, job in self.yieldJobs(job_dag):
                ...
                # We only launched one job, so the results of the second job will be included
                if job_id == 0:
                    self.assertRegexpMatches(result_list[0], 'FAILED \(NO STDOUT FILE\)')
                    self.assertRegexpMatches(result_list[1], 'skipped \(skipped dependency\)')
