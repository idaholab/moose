...
class TestValidationCase(unittest.TestCase):
    ...
    def testCheckBounds(self):
        ...
        # Success
        status, message = ValidationCase.checkBounds(
            value, min_value, max_value, "coolunits"
        )
    ...
    def testCheckRelativeError(self):
        ...
        # Success
        status, message = ValidationCase.checkRelativeError(
            value, nominal, rel_err, units
        )
        self.assertEqual(status, ValidationCase.Status.OK)
        self.assertRegex(
            message, r"\d+.\d+E[-+]\d+ someunit relative error \d+.\d+E[-+]\d+ <"
        )

        # Success without units
        status, message = ValidationCase.checkRelativeError(
            value, nominal, rel_err, None
        )
        self.assertEqual(status, ValidationCase.Status.OK)
        ...
        self.assertEqual(status, ValidationCase.Status.OK)
        self.assertRegex(
            message, r"\d+.\d+E[-+]\d+ relative error skipped due to absolute zero"
        )

        # Success due to absolute zero with override
        status, message = ValidationCase.checkRelativeError(
            0, 0.01, rel_err, None, abs_zero=0.1
        )
        self.assertEqual(status, ValidationCase.Status.OK)
        self.assertRegex(
            message, r"\d+.\d+E[-+]\d+ relative error skipped due to absolute zero"
        ...
        status, message = ValidationCase.checkRelativeError(
            0.01, 0, rel_err, None, abs_zero=0.1
        )
        self.assertEqual(status, ValidationCase.Status.OK)
        self.assertRegex(
            message, r"\d+.\d+E[-+]\d+ relative error skipped due to absolute zero"
    ...
    def testToFloat(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "some context: could not convert string to float"
        ):
    ...
    def testAddScalarDataChecks(self):
        ...
        # Non-numeric value
        with self.assertRaisesRegex(
            ValueError, "value: could not convert string to float"
        ):
        ...
        with self.assertRaisesRegex(TypeError, "bounds: not of length 2"):
            ValidationCase().addScalarData(
                "k", 1, "unused", None, bounds=(None, None, None)
            )

        # Bad min bounds type
        with self.assertRaisesRegex(
            TypeError,
            "bounds min: float\\(\\) argument must be a string or a real number",
        ):
            ValidationCase().addScalarData("k", 1, "unused", None, bounds=(None, None))

        # Bad max bounds type
        with self.assertRaisesRegex(
            ValueError, "bounds max: could not convert string to float"
        ):
        ...
        # Non-numeric nominal
        with self.assertRaisesRegex(
            ValueError, "nominal: could not convert string to float"
        ):
    ...
    def testAddScalarRelativeErrorCheck(self):
        ...
        """
        cases = {
            "pass": {"nominal": 1.1, "rel_err": 1e-1},
            "pass_abs_zero": {"nominal": 0, "rel_err": 1e-12},
            "pass_abs_zero_override": {
                "nominal": 0,
                "rel_err": 1e-12,
                "abs_zero": 1e-2,
            },
        ...
        class Test(ValidationCase):
            ...
            def test_pass_abs_zero(self):
                self.addScalarData(
                    "pass_abs_zero", 0, "foo", None, **cases["pass_abs_zero"]
                )
    ...
    def testToListFloat(self):
        ...
        values = [None]
        with self.assertRaisesRegex(
            ValueError, "value\\(s\\) at indices \\[0\\] are nan"
        ):
            ValidationCase.toListFloat(values)
        with self.assertRaisesRegex(
            ValueError, "foobar: value\\(s\\) at indices \\[0\\] are nan"
        ):
            ValidationCase.toListFloat(values, "foobar:")
    ...
    def testAddVectorDataChecks(self):
        ...
        """
        good_args = {
            "x": ([0.0, 1.0], "description_x", "units_x"),
        ...
        for key in ["x", "value"]:
            ...
            with self.assertRaisesRegex(
                TypeError, f"{key}: first entry \\(values\\) not one-dimensional"
            ):
                ValidationCase().addVectorData("k", *args.values())
            # To-array failed
            args = dict(good_args)
            args[key] = ([1.0, "abcd"], "unused", None)
            with self.assertRaisesRegex(
                TypeError, f"{key}: first entry \\(values\\) array conversion failed"
            ):
                ValidationCase().addVectorData("k", *args.values())
            # Cast int list values to floats
            args = dict(good_args)
            args[key] = ([1.0, 1], "description", "units")
            case = ValidationCase()
            ...
            args[key] = ([], None, None)
            with self.assertRaisesRegex(
                TypeError, f"{key}: second entry \\(description\\) not of type str"
            ):
                ValidationCase().addVectorData("k", *args.values())
            ...
            args[key] = ([], "desc", 1)
            with self.assertRaisesRegex(
                TypeError, f"{key}: third entry \\(units\\) is not of type str or None"
            ):
                ValidationCase().addVectorData("k", *args.values())
            ...
            with self.assertRaisesRegex(
                ValueError, "Length of x and value values not the same"
            ):
                ValidationCase().addVectorData("k", *args.values())
    ...
    def testAddVectorDataBoundedCheck(self):
        ...
        def check_result(test, index, status):
            results = [
                ...
                if (r.test.endswith(test) and f"(index {index})" in r.message)
            ]
            self.assertEqual(len(results), 1)
            result = results[0]
            data = case.data[test]
            ...
            self.assertEqual(result.status, status)
            self.assertTrue(
                result.message.startswith(f"x = {x:{ValidationCase.number_format}}")
            )
