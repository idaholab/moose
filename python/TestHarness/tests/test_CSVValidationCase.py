...
class TestCSVValidationCase(unittest.TestCase):
    ...
    def compareGold(self, case: CSVValidationCase, name: str, rewrite: bool = False):
        ...
        data = {
            "results": [asdict(v) for v in case.results],
    ...
    def runValidationCase(
        self, gold_data, data, case_type, action=None, set_params={}, skip_write=False
    ) -> CSVValidationCase:
        gold_file = "gold.csv"
    ...
    def testInit(self):
        ...
        with self.assertRaisesRegex(
            ValueError, f"validation_csv={out_of_dir_csv} is not in this directory"
        ):
    ...
    def testGetScalarCSV(self):
        ...
        """
        key = "data_key"
        gold_data = {"time": [0, 1], key: [1.0, 2.0]}
        data = {"time": [0, 1], key: [3.0, 4.0]}
        ...
        for index in [*range(len(data[key]))] + [-1]:
            for gold in [True, False]:
                self.runValidationCase(
                    gold_data,
                    data,
                    CSVValidationCase,
                    action=lambda case: get_value(case, gold, index),
                )
        ...
        def out_of_range(case, gold):
            ...
            with self.assertRaisesRegex(
                IndexError, f"Index {index} out of range in {file}"
            ):
        ...
        for gold in [True, False]:
            self.runValidationCase(
                ...
                data,
                CSVValidationCase,
                action=lambda case: out_of_range(case, gold),
            )
        ...
        def missing_ley(case, gold):
            ...
            file = case._gold_csv if gold else case._csv
            with self.assertRaisesRegex(
                KeyError, f"Column {missing_key} does not exist in {file}"
            ):
        ...
        for gold in [True, False]:
            self.runValidationCase(
                ...
                data,
                CSVValidationCase,
                action=lambda case: missing_ley(case, gold),
            )
        ...
        with self.assertRaisesRegex(
            Exception,
            r"CSVValidationCase.initialize\(\) was not called in BadInitialize",
        ):
            self.runValidationCase(
                gold_data, data, BadInitialize, action=bad_initialize
            )
    ...
    def testAddScalarCSVDataFailRelErr(self):
        ...
        """
        key, description, units = "data", "data_description", "data_units"
        gold_data = {"time": [0, 1], key: [0.0, 1.0]}
        ...
        rel_err = 1.0e-4
        case = self.runValidationCase(
            gold_data, data, TestCase, set_params={"validation_rel_err": rel_err}
        )
    ...
    def testAddScalarCSVDataStoreKey(self):
        ...
        class TestCase(CSVValidationCase):
            def test(self):
                self.addScalarCSVData(
                    key, -1, "unused_description", None, store_key=store_key
                )
    ...
    def testAddScalarCSVDataAbsZero(self):
        ...
        abs_zero = 0.1
        pass_case = self.runValidationCase(
            gold_data, data, TestCase, set_params={"validation_abs_zero": abs_zero}
        )
