...
class TestHarnessTester(TestHarnessTestCase):
    def testSkippedAllocations(self):
        ...
        output = self.runTests(
            "--no-color", "-i", "always_ok", "-p", "2", "-j", "1"
        ).output
        self.assertRegex(
            output, "tests/test_harness.always_ok.*? \[INSUFFICIENT SLOTS\] SKIP"
        )

        # Subject a normally passing test to impossible thread allocations
        output = self.runTests(
            "--no-color", "-i", "always_ok", "--n-threads", "2", "-j", "1"
        ).output
        self.assertRegex(
            output, "tests/test_harness.always_ok.*? \[INSUFFICIENT SLOTS\] SKIP"
        )
        ...
        self.assertRegex(
            output, "tests/test_harness.allocation_test.*? \[INSUFFICIENT SLOTS\] SKIP"
        )
    ...
    def testOversizedCaveat(self):
        ...
        output = self.runTests("-i", "always_ok", "-p", "2").output
        ...
        output = self.runTests("-i", "always_ok", "--n-threads", "2").output
    ...
    def testCpuCaveats(self):
        ...
        output = self.runTests("-i", "allocation_test", "--n-threads", "2").output
        ...
        output = self.runTests(
            "-i", "allocation_test", "-p", "4", "--n-threads", "2"
        ).output
        self.assertNotIn("MIN_THREADS", output)
    ...
    def testThreadCaveats(self):
        ...
        output = self.runTests(
            "-i", "allocation_test", "-j", "4", "-p", "2", "--n-threads", "1"
        ).output
        self.assertNotIn("CPUS", output)
        ...
        output = self.runTests(
            "-i", "allocation_test", "-j", "6", "-p", "2", "--n-threads", "4"
        ).output
        self.assertNotIn("CPUS", output)
        self.assertNotIn("OVERSIZED", output)
    ...
    def testPerfectAllocation(self):
        ...
        output = self.runTests(
            "-i", "allocation_test", "-j", "4", "-p", "2", "--n-threads", "2"
        ).output
        self.assertNotIn("MIN_THREADS", output)
