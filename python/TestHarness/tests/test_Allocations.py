...
class TestHarnessTester(TestHarnessTestCase):
    ...
    def testCpuCaveats(self):
        ...
        output = self.runTests('-i', 'allocation_test', '-p', '4', '--n-threads', '2').output
        self.assertNotIn('MIN_THREADS', output)
    ...
    def testThreadCaveats(self):
        ...
        output = self.runTests('-i', 'allocation_test', '-j', '4', '-p', '2', '--n-threads', '1').output
        self.assertNotIn('CPUS', output)
        ...
        output = self.runTests('-i', 'allocation_test', '-j', '6', '-p', '2', '--n-threads', '4').output
        self.assertNotIn('CPUS', output)
        self.assertNotIn('OVERSIZED', output)
    ...
    def testPerfectAllocation(self):
        ...
        output = self.runTests('-i', 'allocation_test', '-j', '4', '-p', '2', '--n-threads', '2').output
        self.assertNotIn('MIN_THREADS', output)
