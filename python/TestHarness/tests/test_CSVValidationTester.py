...
class TestHarnessTester(TestHarnessTestCase):
    ...
    def testCSVValidationTester(self):
        ...
        """
        out = self.runTests('-i', 'csv_validation_tester', '--no-color', exit_code=129).output
        self.assertRegex(out, r'test_harness\.csv_validation_tester_01.*?OK')
        self.assertRegex(out, r'test_harness\.csv_validation_tester_02.*?FAILED \(DIFF\)')
    ...
    def testCSVValidationTesterVerbose(self):
        ...
        """
        out = self.runTests('-i', 'csv_validation_tester', '--verbose', '--no-color', exit_code=129).output
        self.assertRegex(out, 'csv_validation_tester_01.csv                        | 0.00 \xb1 0.01          | 0.01 \xb1 0.01')
        self.assertRegex(out, 'csv_validation_tester_02.csv                        | 0.00 \xb1 0.01          | 0.01 \xb1 0.00')
