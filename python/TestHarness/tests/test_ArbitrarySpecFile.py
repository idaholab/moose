...
class TestHarnessTester(TestHarnessTestCase):
    def testArbitrarySpecFile(self):
        ...
        output = self.runTests('--spec-file', 'tests/test_harness/arbitrary_test').output
        ...
        output = self.runTests('--spec-file', 'tests/test_harness', '-i', 'arbitrary_test').output
        self.assertIn('tests/test_harness.always_ok', output)
        ...
        output = self.runTests('--spec-file', 'tests/test_harness/arbitrary_directory', '-i', 'arbitrary_test').output
        self.assertIn('tests/test_harness/arbitrary_directory.always_ok', output)
