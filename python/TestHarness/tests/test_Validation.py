...
class TestValidation(TestHarnessTestCase):
    ...
    def test(self):
        ...
        out = results.results
        self.assertEqual(
            out["testharness"]["validation_version"], results.harness.VALIDATION_VERSION
        )
        ...
        self.assertIn(
            "Acquired 3 data value(s), 3 result(s): 3 ok, 0 fail, 0 skip",
            validation_output,
        )
    ...
    def testCSV(self):
        ...
        csv = os.path.join(
            job.getTestDir(), job.validation_cases[0].getParam("validation_csv")
        )
    ...
    def testFail(self):
        ...
        self.assertIn("Running validation case", validation_output)
        self.assertIn(
            "Acquired 1 data value(s), 1 result(s): 0 ok, 1 fail, 0 skip",
            validation_output,
        )
    ...
    def testException(self):
        ...
        """
        out = self.runTests(
            "-i", "validation", "--re", "exception", exit_code=132
        ).results
    ...
    def testDuplicateParam(self):
        ...
        self.assertIn(
            'validation_duplicate_param:2: Failed to create Tester: Duplicate parameter "type" from validation test',
            out,
        )
