...
def results_has_test(results: dict, name: TestName) -> bool:
    ...
    """
    return results_has_folder(results, name.folder) and \
        name.name in results_folder_entry(results, name.folder)['tests']

def results_set_test_value(results: dict, name: TestName, value: Any):
    """
    Sets the test value for a given test in TestHarness JSON results.
    """
...
class ResultsTestIterator:
    """
    Iterator that represents a single test entry
    in TestHarness results JSON output.

    Enables modification while iterating through
    the tests entry in the results.
    """
...
class ResultsFolderIterator:
    """
    Iterator that represents a single test folder entry
    in TestHarness results JSON output.
...
def results_folder_iterator(results: dict) -> Iterator[ResultsFolderIterator]:
    """
    Iterates through test folder entries in TestHarness results JSON output.
...
def results_test_iterator(results: dict) -> Iterator[ResultsTestIterator]:
    """
    Iterates through test entries in TestHarness results JSON output.
