...
# Splitting a string by spaces while ignoring quotations
def split_string_with_quotes(input_str):

    # For multiline strings, find all newlines followed by any number of spaces,
    ...
    # Splitting a command indicates that we are submitting test with shell=False.
    # Thus, remove any escape characters.
    # Have to escape "\" itself, so the find pattern ends up looking like "\\"
    input_str = input_str.replace("\\", "")

    # Determine style of quote that is used (single or double)
    # Single quote
    if input_str.find("'") != -1:
        quote_char = "'"
    # Double quote
        ...
        quote_char = '"'
    # No quotes present, use simple splitter
    else:
        quote_char = None
    ...
    if quote_char is not None:
        ...
        if len(quote_locations) % 2 != 0:
            raise ValueError("Odd number of quotes detected.")

        split_str = []
        last_quote_loc = -1
        for index, quote_loc in enumerate(quote_locations):
            if index % 2 == 0:
                # Split string up until quote as normal
                split_section = input_str[last_quote_loc + 1 : quote_loc].split(" ")

                # Throw away leading space
                if split_section[0] == '':
                    split_section = split_section[1:]

                split_str += split_section

                quote = input_str[quote_loc : quote_locations[index + 1] + 1]
                # Decide how to add on quoted part
                # Last character of substring was space, replace with quote
                if split_str[-1] == '':
                    split_str[-1] = quote
                # Append quote to last entry since there was no space at the end
                else:
                    split_str[-1] += quote

            last_quote_loc = quote_loc

        # Split substring after last quote as ususal
        split_section = input_str[quote_locations[-1] + 1 :].split(" ")
        if split_section[0] == '':
            split_section = split_section[1:]
    ...
    if split_str[0] == '/bin/sh':
        if split_str[1] != '-c':
            raise ValueError("Expected '-c' as an option to 'bin/sh'.")
        # Take all items after '-c' and combine them into a single item
        cmd = ' '.join(split_str[2:])
        split_str = split_str[:2]
        split_str.append(cmd)

    # Perform a final check to filter out blank string entries
    split_str = [item for item in split_str if not item == '']
