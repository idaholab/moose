...
class Tester(MooseObject, OutputInterface):
    ...
    def validParams():
        ...
        params.addParam(
            "exodus_version",
            ["ALL"],
            "A list of Exodus versions for which this test will run on, supports normal comparison operators ('<', '>', etc...)",
        )
        params.addParam(
            "vtk_version",
            ["ALL"],
            "A list of VTK versions for which this test will run on, supports normal comparison operators ('<', '>', etc...)",
        )
        params.addParam(
            "mesh_mode",
            ["ALL"],
        ...
        params.addParam(
            "min_ad_size", None, "A minimum AD size for which this test will run"
        )
        params.addParam(
            "max_ad_size", None, "A maximum AD size for which this test will run"
        )
        params.addParam(
            "method",
            ["ALL"],
        ...
        params.addParam(
            "libtorch",
            ["ALL"],
            "A test that runs only if libtorch is available ('ALL', 'TRUE', 'FALSE')",
        )
        params.addParam(
            "libtorch_version",
            ["ALL"],
            "A list of libtorch versions for which this test will run on, supports normal comparison operators ('<', '>', etc...)",
        )
        params.addParam(
            "installation_type",
            ["ALL"],
    ...
    def getResults(self, options) -> dict:
        ...
        json_metadata = {
            k: os.path.join(self.getTestDir(), v.path) if v else None
            for k, v in self.json_metadata.items()
        }
        return {
    ...
    def checkRunnableBase(self, options):
        ...
        if not None in [min_ad_size, max_ad_size]:
            ...
            if min_ad_size is not None and int(min_ad_size) > ad_size:
                reasons["min_ad_size"] = (
                    "Minimum AD size %d needed, but MOOSE is configured with %d"
                    % (int(min_ad_size), ad_size)
                )
            if max_ad_size is not None and int(max_ad_size) < ad_size:
                reasons["max_ad_size"] = (
                    "Maximum AD size %d needed, but MOOSE is configured with %d"
                    % (int(max_ad_size), ad_size)
                )

        # Check for PETSc versions
        (petsc_status, petsc_version) = util.checkPetscVersion(checks, self.specs)
        if not petsc_status:
            reasons["petsc_version"] = (
                "using PETSc " + str(checks["petsc_version"]) + " REQ: " + petsc_version
            )
        ...
        if not exodus_status:
            if checks["exodus_version"] != None:
                ...
                reasons["exodus_version"] = (
                    "Exodus version not detected. REQ: " + exodus_version
                )
        ...
        (libtorch_status, libtorch_version) = util.checkLibtorchVersion(
            checks, self.specs
        )
        if not libtorch_status:
            reasons["libtorch_version"] = (
                "using libtorch "
                + str(checks["libtorch_version"])
                + " REQ: "
                + libtorch_version
            )
