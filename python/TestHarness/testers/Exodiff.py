...
def findExodiff(moose_dir):
    ...
    if not os.path.exists(exodiff_bin):
        ...
        exodiff_bin = os.path.join(
            moose_dir, "framework", "contrib", "exodiff", "exodiff"
        )
    return exodiff_bin
    ...
    def processResultsCommand(self, moose_dir, options):
        ...
        for file in self.specs["exodiff"]:
            ...
            if self.specs.isValid("custom_cmp"):
                custom_cmp = " -f " + os.path.join(
                    self.getTestDir(), self.specs["custom_cmp"]
                )
            ...
            commands.append(
                findExodiff(moose_dir)
                + map_option
                + partial_option
                + custom_cmp
                + " -F"
                + " "
                + str(self.specs["abs_zero"])
                + old_floor
                + " -t "
                + str(self.specs["rel_err"])
                + " "
                + " ".join(self.specs["exodiff_opts"])
                + " "
