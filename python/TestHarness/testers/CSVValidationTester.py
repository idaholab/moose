...
def diff_files(gold_file, out_file, err_type="relative"):
    ...
    for key in keys1:
        ...
        for val1, val2 in zip(table1[key], table2[key]):
            ...
            if math.isnan(val1):
                csv.addError(
                    gold_file, 'The values in column "' + key.strip() + '" contain NaN'
                )

            # disallow inf in the gold file
            if math.isinf(val1):
                csv.addError(
                    gold_file, 'The values in column "' + key.strip() + '" contain Inf'
                )
...
class CSVValidationTester(FileTester):
    ...
    def validParams():
        ...
        params.addParam(
            "err_type", "relative", "Type of error ('relative' or 'absolute')."
        )
    ...
    def processResults(self, moose_dir, options, exit_code, runner_output):
        ...
        if not self.isFail():
            ...
            for file in self.specs["csvdiff"]:
                ...
                requested = "{:.2f} \u00b1 {:.2f}".format(
                    self.specs["mean_limit"], self.specs["std_limit"]
                )
