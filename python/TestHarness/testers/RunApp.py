...
class RunApp(Tester):
    ...
    def getCommand(self, options):
        ...
        if self.force_mpi or options.parallel or ncpus > 1 or nthreads > 1:
            ...
            # Open our pbs_template.i
            template_script = open(os.path.join(specs['moose_dir'], 'python', 'TestHarness', 'pbs_template.i'), 'r')
            content = template_script.read()
            template_script.close()

            # Add threads to cli_args when using PBS
            if options.parallel or ncpus > 1 or nthreads > 1:
                specs['cli_args'].append(' --n-threads=' + str(nthreads))
            ...
            specs['job_name'] = specs['input'][:6] + '_' + str(options.test_serial_number).zfill(2)
            specs['job_name'] = specs['job_name'].replace('.', '')
            specs['job_name'] = specs['job_name'].replace('-', '')

            # Convert TEST_NAME to input tests file name (normally just 'tests')
            specs['no_copy'] = options.input_file_name
            ...
            specs.addStringSubParam('pbs_stdout', 'PBS_STDOUT', "Save stdout to this location")
            specs.addStringSubParam('pbs_stderr', 'PBS_STDERR', "Save stderr to this location")
            if options.PBSEmulator:
                ...
                specs.addStringSubParam('pbs_project', 'PBS_PROJECT', "Identify this job submission with this project")
                specs['pbs_project'] = 'pbs_project = %s' % (options.pbs_project)


            # Squash cli_args list. Needs to happen before we attempt to replace template with valid parameters.
            ...
            # Write the cluster_launcher input file (this file is closed after all possible tests in
            # 'tests' has been processed. eg: misc/check_error/tests has 20+ tests and creates a
            # .cluster file with 20+ tests)
            options.cluster_handle.write(content + '\n')

            command = os.path.join(specs['moose_dir'], 'scripts', 'cluster_launcher.py') + ' ' + options.pbs + '.cluster'
