...
import os
import shutil
from TestHarness.testers import Tester
...
class RunApp(Tester):
    ...
    def validParams():
        ...
        params.addParam(
            "expect_out",
            "A regular expression or literal string that must occur in the output in order for the test to be considered passing (see match_literal).",
        )
        params.addParam(
            "match_literal",
            False,
        ...
        params.addParam(
            "delete_output_before_running",
            True,
            "Delete pre-existing output files before running test. Only set to False if you know what you're doing!",
        )
        params.addParam(
            "custom_evaluation_script",
            False,
            "A .py file containing a custom function for evaluating a test's success. For syntax, please check https://mooseframework.inl.gov/python/TestHarness.html",
        )
        ...
        params.addParam(
            "command_proxy",
            "A proxy command to run that will execute the underlying test via wrapper; the intended command is set in the env variable RUNAPP_COMMAND",
        )
        ...
        params.addParam(
            "capture_perf_graph",
            True,
            "Whether or not to enable the capturing of PerfGraph output via Outputs/perf_graph_json_file and --capture-perf-graph",
        )
        params.addParam(
            "perf_graph_live", False, "Whether to enable perf graph live printing"
        )
    ...
    def __init__(self, name, params):
        ...
        if not (
            params.isValid("input")
            or params.isValid("command")
            or params.isValid("command_proxy")
            or params["no_additional_cli_args"]
        ):
            raise Exception(
    ...
    def checkRunnable(self, options):
        ...
        if (
            ...
            and os.environ.get("APPTAINER_CONTAINER") is not None
        ):
    ...
    def getCommand(self, options):
        ...
        if specs.isValid("command"):
            return (
                os.path.join(specs["test_dir"], specs["command"])
                + " "
                + " ".join(specs["cli_args"])
            )
        ...
        if specs["no_additional_cli_args"]:
            ...
            cmd = (
                os.path.join(specs["test_dir"], specs["executable"])
                + " "
                + " ".join(specs["cli_args"])
            )
        ...
        if (
            "--error" not in cli_args
            and (not specs["allow_warnings"] or options.error)
            and not options.allow_warnings
        ):
        ...
        if (
            "--error-unused" not in cli_args
            and "--allow-unused" not in cli_args
        ...
        if (
            options.valgrind_mode.upper() == specs["valgrind"].upper()
            or options.valgrind_mode.upper() == "HEAVY"
            and specs["valgrind"].upper() == "NORMAL"
        ):
            command = (
                ...
                + " --leak-check=full --tool=memcheck --dsymutil=yes --track-origins=yes --demangle=yes -v "
                + command
            )
        elif nthreads > 1:
    ...
    def testFileOutput(self, moose_dir, options, runner_output):
        ...
        if specs["custom_evaluation_script"]:
            ...
            custom_mod_spec = importlib.util.spec_from_file_location(
                ...
                os.path.join(
                    self.getMooseDir(),
                    self.getTestDir(),
                    specs["custom_evaluation_script"],
                ),
            )
        ...
        for param, attr in params_and_msgs.items():
            if specs.isValid(param) and (
                options.method in attr["modes"] or attr["modes"] == ["ALL"]
            ):
                match_type = ""
                if specs["match_literal"]:
                    have_expected_out = util.checkOutputForLiteral(
                        runner_output, specs[param]
                    )
                    match_type = "literal"
                else:
