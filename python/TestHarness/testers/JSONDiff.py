...
class JSONDiff(SchemaDiff):
    ...
    def validParams():
        ...
        params.addParam(
            "skip_keys",
            [],
            "Deprecated. Items in the JSON that the differ will ignore. This is functionally identical to ignored_items inside of SchemaDiff.",
        )
        params.addParam(
            "ignored_regex_items",
            [],
            "Items (with regex enabled) to ignore, separated by '/' for each level, i.e., key1/key2/.* will skip all items in ['key1']['key2']['.*']",
        )
        params.addParam(
            "keep_system_information",
            False,
            "Whether or not to keep the system information as part of the diff.",
        )
        params.addParam(
            "keep_reporter_types",
            False,
            "Whether or not to keep the MOOSE Reporter type information as part of the diff.",
        )
        return params
    ...
    def __init__(self, name, params):
        ...
        if not params["keep_system_information"]:
            self.specs["ignored_items"].extend(
                [
                    "app_name",
                    "current_time",
                    "executable",
