...
class ImageDiff(FileTester):
    ...
    def validParams():
        ...
        params.addParam(
            "allowed", 0.98, "Absolute zero cutoff used in exodiff comparisons."
        )
        params.addParam(
            "allowed_linux",
            "Absolute zero cutoff used for linux machines, if not provided 'allowed' is used.",
        )
        params.addParam(
            "allowed_darwin",
            "Absolute zero cutoff used for Mac OS (Darwin) machines, if not provided 'allowed' is used.",
        )
