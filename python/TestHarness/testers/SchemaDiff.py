...
from TestHarness.testers import RunApp
...
class SchemaDiff(RunApp):
    ...
    def validParams():
        ...
        params.addParam(
            "gold_dir",
            "gold",
            'The directory where the "gold standard" files (the expected output to compare against) reside relative to the TEST_DIR: (default: ./gold/). This only needs to be set if the gold file is in a non-standard location.',
        )
        params.addParam(
            "gold_file",
            None,
            "Specify the file in the gold_dir that the output should be compared against. This only needs to be set if the gold file uses a different file name than the output file.",
        )
        params.addParam(
            "ignored_items",
            [],
            'Items in the schema that the differ will ignore. These can be keys or values, i.e. for "foo": "bar", either foo or bar can be chosen to be selected. Note that entering a value located inside a list will skip the whole list.',
        )
        params.addParam(
            "rel_err",
            5.5e-6,
            "Relative error value allowed in comparisons. If rel_err value is set to 0, it will work on the absolute difference between the values.",
        )
        params.addParam(
            "abs_zero",
            1e-10,
            "Absolute zero cutoff used in diff comparisons. Every value smaller than this threshold will be ignored.",
        ),
        return params
    ...
    def processResults(self, moose_dir, options, exit_code, runner_output):
        ...
        for file in specs["schemadiff"]:
            ...
            if not os.path.exists(
                os.path.join(self.getTestDir(), specs["gold_dir"], gold_file)
            ):
                output += "File Not Found: " + os.path.join(
                    self.getTestDir(), specs["gold_dir"], file
                )
    ...
    def do_deepdiff(self, orig, comp, rel_err, abs_zero, exclude_values: list = None):
                ...
                # but here we do the logic to diff the long string in case it sneaks in, or if for some reason, someone made a JSON like this.
                    ...
                    try:
                        ...
                        for i in range(len(split1)):
                            if (
                                not split1[i] and not split2[i]
                            ):  # if the two values are both just an empty str, continue.
                                continue
                            x = float(split1[i])
                            y = float(split2[i])
                            if x != y:
        ...
        exclude_paths = [
            "root['VTKFile']['@header_type']",
            "root['VTKFile']['@version']",
        ]
        if exclude_values:
        ...
        custom_operators = [
            testcompare(types=[str, float], rel_err=rel_err, abs_zero=abs_zero)
        ]
        args = [orig, comp]
        kwargs = {
