...
def findTestRoot() -> TestRoot:
    ...
    while os.path.dirname(root_dir) != root_dir:
        ...
        if os.path.exists(testroot_file) and os.access(testroot_file, os.R_OK):
            ...
            return TestRoot(
                root_dir=root_dir, app_name=app_name, args=args, hit_node=hit_node
            )
...
class TestHarness:
    ...
    def build(
        ...
        if test_root is None:
            ...
            test_root = TestRoot(
                root_dir=".", app_name=app_name, args=[], hit_node=pyhit.Node()
            )
    ...
    def __init__(
        ...
        if self.options.no_capabilities:
            ...
            checks["petsc_debug"] = get_option(
                "petsc_debug", from_type=bool, to_set=True
            )
            checks["slepc_version"] = get_option("slepc", from_version=True)
            checks["slepc"] = get_option(
                "slepc", from_version=True, to_set=True, to_bool=True
            )
            checks["exodus_version"] = get_option("exodus", from_version=True)
            checks["vtk_version"] = get_option("vtk", from_version=True)
            checks["vtk"] = get_option(
                "vtk", from_version=True, to_set=True, to_bool=True
            )
            ...
            checks["dof_id_bytes"] = get_option(
                "dof_id_bytes", from_type=int, to_set=True, no_all=True
            )
            ...
            checks["fparser_jit"] = set(
                sorted(["ALL", "TRUE" if fparser == "jit" else "FALSE"])
            )

            checks["libtorch"] = get_option(
                "libtorch", from_version=True, to_set=True, to_bool=True
            )
            checks["libtorch_version"] = get_option(
                "libtorch", from_version=True, to_none=True
            )
        ...
        if not self.options.no_capabilities:
            checks["installation_type"] = util.checkInstalled(
                self.executable, self.app_name
            )
                    ...
                    if "contrib" not in os.path.relpath(dirpath, os.getcwd()):
                        for file in filenames:
                            if self.options.use_subdir_exe and file == "testroot":
                                ...
                                app_name, args, root_params = readTestRoot(
    ...
    def cleanup(self):
        ...
        if (
            self.options.verbose or (self.num_failed != 0 and not self.options.quiet)
        ) and not self.options.dry_run:
            print(header("Final Test Results"))
            sorted_jobs = sorted(
                self.finished_jobs, key=lambda job: job.getJointStatus().sort_value
            )
    ...
    def parseCLArgs(self, argv):
        ...
        capabilitygroup.add_argument(
            "--no-capabilities", action="store_true", help="Disable Capability checks"
        )
        capabilitygroup.add_argument(
            "--pedantic-checks",
            action="store_true",
        ...
        if self.options.hpc_host and not self.options.hpc:
            ...
            if hpc_config is not None:
                ...
                print(
                    f"INFO: Setting --hpc={self.options.hpc} for known host {hpc_host}"
                )
    ...
    def checkAndUpdateCLArgs(self):
        ...
        if opts.check_input and opts.no_check_input:
            print(
                "ERROR: --check-input and --no-check-input cannot be used simultaneously"
            )
            sys.exit(1)
        has_flags = []
        for var, flag in [
    ...
    hpc_configs = {
        "sawtooth": HPCCluster(scheduler="pbs", apptainer_modules=inl_modules),
