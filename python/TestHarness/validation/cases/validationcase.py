...
class ValidationCase(MooseObject):
    ...
    def addResult(self, status: Status, message: str, **kwargs) -> None:
        ...
        status_value = ValidationResult(
            status=status, message=message, test=self._current_test, **kwargs
        )
    ...
    def _addData(
        self, data_type: typing.Type, key: str, value: Any, description: str, **kwargs
    ) -> ValidationData:
        """
    ...
    @staticmethod
    def checkBounds(
        value: float, min_bound: float, max_bound: float, units: Union[str, None]
    ) -> Tuple[Status, str]:
        """
    ...
    @staticmethod
    def checkRelativeError(
        value: float,
        nominal: float,
        rel_err: float,
        units: Union[str, None],
        abs_zero: float = 1.0e-10,
    ) -> Tuple[Status, str]:
        """
    ...
    def addScalarData(
        ...
        description: str,
        units: Optional[str],
        abs_zero: float = DEFAULT_ABS_ZERO,
        **kwargs,
    ) -> None:
        """
        ...
        if bounds is not None:
            ...
            kwargs["bounds"] = (
                self.toFloat(bounds[0], "bounds min"),
        ...
        data = self._addData(
            ValidationScalarData, key, value, description, units=units, **kwargs
        )

        result_kwargs = {"data_key": key, "validation": kwargs.pop("validation", True)}

        if data.rel_err is not None:
            if data.nominal is None:
                raise KeyError("Must provide 'nominal' with 'rel_err'")
            status, message = self.checkRelativeError(
                data.value, data.nominal, data.rel_err, data.units, abs_zero=abs_zero
            )
            self.addResult(status, message, **result_kwargs)

        if data.bounds is not None:
            status, message = self.checkBounds(
                data.value, data.bounds[0], data.bounds[1], data.units
            )
            self.addResult(status, message, **result_kwargs)
        ...
        **kwargs,
        ...
        if bounds is not None:
            ...
            if not len(bounds) == 2:
                raise TypeError("bounds: not of length 2 (min and max)")
            kwargs["bounds"] = (
                self.toListFloat(bounds[0], "bounds min:"),
        ...
        if data.bounds is not None:
            for i, v in enumerate(values):
                status, message = self.checkBounds(
                    v, data.bounds[0][i], data.bounds[1][i], data.units
                )
    ...
    def run(self):
        ...
        all_functions = [
            v[0]
            for v in inspect.getmembers(self.__class__, predicate=inspect.isfunction)
            if len(v)
        ]
        ...
        for status in ValidationCase.Status.list():
            results.append(
                f"{self.getNumResultsByStatus(status)} {status.name.lower()}"
            )
        if run_exceptions:
