...
class CSVValidationCase(ValidationCase):
    ...
    def validParams():
        ...
        params.addParam(
            "validation_abs_zero",
            ValidationCase.DEFAULT_ABS_ZERO,
            "Absolute zero cutoff used in comparisons",
        )
        return params
    ...
    def _getScalarCSV(self, key: str, index: int, gold: bool) -> float:
        ...
        if not self._called_csv_initialize:
            raise Exception(
                f"CSVValidationCase.initialize() was not called in {self.__class__.__name__}"
            )
