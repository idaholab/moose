...
class PBSJob(Job):
    def validParams():
        ...
        params.addStringSubParam(
            "pbs_o_workdir", "PBS_O_WORKDIR", "Move to this directory"
        )
        params.addStringSubParam(
            "pbs_project",
            "#PBS -P PBS_PROJECT",
            "Identify as PBS_PROJECT in the PBS queuing system",
        )
        params.addStringSubParam(
            "pbs_stdout", "#PBS -o PBS_STDOUT", "Save stdout to this location"
        )
        params.addStringSubParam(
            "pbs_stderr", "#PBS -e PBS_STDERR", "Save stderr to this location"
        )
        ...
        params.addStringSubParam(
            "queue", "#PBS -q QUEUE", "Which queue to submit this job to."
        )
        params.addStringSubParam(
            "cli_args", "CLI_ARGS", "Any extra command line arguments to tack on."
        )
        params.addStringSubParam(
            "notifications",
            "#PBS -m NOTIFICATIONS",
        ...
        params.addRequiredParam(
            "moose_application", "The full path to the application to run."
        )
    ...
    def copyFiles(self, job_file):
        ...
        for file in os.listdir("../"):
            if (
                os.path.isfile("../" + file)
                and file != job_file
                and (not params.isValid("no_copy") or file not in params["no_copy"])
                and (
    ...
    def prepareJobScript(self):
        ...
        if params.isValid("mpi_procs") and params.isValid("total_mpi_procs"):
            print(
                "ERROR: 'mpi_procs' and 'total_mpi_procs' are exclusive.  Only specify one!"
            )
            sys.exit(1)
        ...
        if params.isValid("mpi_procs"):
            ...
            params["mpi_procs_per_chunk"] = str(
                int(params["total_mpi_procs"]) / int(params["chunks"])
        ...
        for param in params.valid_keys():
            if param in params.substitute:
                params[param] = params.substitute[param].replace(
                    param.upper(), params[param]
                )
    ...
    def launch(self):
        ...
        my_process = subprocess.Popen(
            "qsub " + os.path.split(self.specs["template_script"])[1],
        ...
        print(
            ...
            self.specs["job_name"],
            "JOB_ID:",
            my_process.communicate()[0].split(".")[0],
            "TEST_NAME:",
