...
class Graph(base.ChiggerResultBase):
    ...
    def getOptions():
        ...
        opt.add(
            "xaxis", utils.AxisOptions.get_options(), "The settings for the x-axis."
        )
        opt.add(
            "yaxis", utils.AxisOptions.get_options(), "The settings for the y-axis."
        )
        opt.add(
            "x2axis",
            utils.AxisOptions.get_options(),
            "The settings for the secondary " "x-axis (top).",
        )
        opt.add(
            "y2axis",
            utils.AxisOptions.get_options(),
            "The settings for the secondary " "y-axis (right).",
        )
        opt.add(
            "legend",
            utils.LegendOptions.get_options(),
            "The settings for the graph legend.",
        )
        opt.add(
            "hidden_border",
            None,
            "Adjust the border of hidden axis; useful if axis text is " "cut-off.",
            vtype=int,
        )
    ...
    def needsUpdate(self):
        ...
        """
        return super(Graph, self).needsUpdate() or any(
            [p.needsUpdate() for p in self._plots]
        )
    ...
    def update(self, *args, **kwargs):
        ...
        # Update Axis options
        self._setAxisOptions(
            self._vtkchart.GetAxis(vtk.vtkAxis.BOTTOM), self.getOption("xaxis")
        )
        self._setAxisOptions(
            self._vtkchart.GetAxis(vtk.vtkAxis.LEFT), self.getOption("yaxis")
        )
        self._setAxisOptions(
            self._vtkchart.GetAxis(vtk.vtkAxis.TOP), self.getOption("x2axis")
        )
        self._setAxisOptions(
            self._vtkchart.GetAxis(vtk.vtkAxis.RIGHT), self.getOption("y2axis")
        )

        # Legend
        utils.LegendOptions.set_options(
            self._vtkchart, self._vtkrenderer, self.getOption("legend")
        )
        ...
        if not line.isOptionValid("color"):
            ...
            if self._vtkcolorseries is None:
                ...
                if self.isOptionValid("color_scheme"):
                    scheme = eval(
                        "vtk.vtkColorSeries." + self.getOption("color_scheme").upper()
                    )
    ...
    def _setAxisOptions(self, vtkaxis, opt):
        ...
        if (
            self._vtkchart.GetHiddenAxisBorder()
            < vtkaxis.GetLabelProperties().GetFontSize()
        ):
            self._vtkchart.SetHiddenAxisBorder(
                vtkaxis.GetLabelProperties().GetFontSize()
            )
