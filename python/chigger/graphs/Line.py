...
class Line(base.ChiggerObject):
    ...
    def getOptions():
        ...
        opt.add(
            "corner",
            "left-bottom",
            "The axis corner to place the line.",
        ...
        opt.add(
            "tracer",
            False,
            "Places both x and y tracing lines, (see 'xtracer' and " "'ytracer').",
        )
        opt.add(
            "xtracer",
            None,
            "Place a tracing line that follows the leading x-value (overrides "
            "'tracer' option).",
            vtype=bool,
        )
        opt.add(
            "ytracer",
            None,
            "Place a tracing line that follows the leading y-value (overrides "
            "'tracer' option).",
            vtype=bool,
        )
    ...
    def initialize(self):
        ...
        if self.getOption("xtracer"):
            if self._xtracer is None:
                self._xtracer = Line(
                    append=False, width=0.1, color=self.getOption("color")
                )
