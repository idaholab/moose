...
def create(base_type):
    ...
    class PlaneSourceMeta(source_type):
        ...
        This is a meta class because the vtkPlaneSource object can operate with both 2D and 3D actors, thus this create
        method should be called with either ChiggerSource or ChiggerSource2D as input.

        This has already been done in the "geometric" module (see geometric/__init__.py), therefore in your script or
        result objects containing geometric sources the following are available:
        ...
        def getOptions():
            ...
            opt.add('resolution', [1, 1], "Define the number of subdivisions in the x- and y-direction of the plane.")
            opt.add('data', None, "The VTK data to attach to the vtkMapper for this object, for used with the 'cmap' option.", vtype=vtk.vtkFloatArray)
        ...
        def update(self, **kwargs):
            ...
            if self.isOptionValid('cmap'):
                if self.isOptionValid('color'):
                    mooseutils.mooseWarning('The "color" and "cmap" options are both being set, the "color" will be ignored.')

                if not self.isOptionValid('data'):
                    mooseutils.mooseError('The "cmap" option requires that "data" option also be supplied.')
                ...
                if self.isOptionValid('data'):
                    ...
                    cmap_options = {key:self.getOption(key) for key in ['cmap', 'cmap_reverse', 'cmap_num_colors', 'cmap_range']}
