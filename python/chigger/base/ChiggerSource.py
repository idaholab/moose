...
class ChiggerSource(ChiggerFilterSourceBase):
    ...
    def getOptions():
        ...
        opt.add(
            "rotation",
            [0, 0, 0],
            "The rotation of the object about x, y, z axes.",
            vtype=list,
        )
    ...
    def __init__(
        self, vtkactor_type=vtk.vtkActor, vtkmapper_type=vtk.vtkPolyDataMapper, **kwargs
    ):
    ...
    def update(self, **kwargs):
        ...
        if self.isOptionValid("edges") and hasattr(
            self._vtkactor.GetProperty(), "SetEdgeVisibility"
        ):
            self._vtkactor.GetProperty().SetEdgeVisibility(self.getOption("edges"))

        if self.isOptionValid("edge_color") and hasattr(
            self._vtkactor.GetProperty(), "SetEdgeColor"
        ):
            self._vtkactor.GetProperty().SetEdgeColor(self.getOption("edge_color"))

        if self.isOptionValid("edge_width") and hasattr(
            self._vtkactor.GetProperty(), "SetLineWidth"
        ):
            self._vtkactor.GetProperty().SetLineWidth(self.getOption("edge_width"))

        if self.isOptionValid("point_size") and hasattr(
            self._vtkactor.GetProperty(), "SetPointSize"
        ):
