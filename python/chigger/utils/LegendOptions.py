...
def get_options():
    ...
    opt.add(
        "background",
        "Set the legend background color (defaults to graph background color).",
        vtype=list,
    )
    ...
    opt.add(
        ...
        "right",
        "The horizontal alignment of the legend with respect " "to the anchor point.",
        vtype=str,
    ...
    opt.add(
        "vertical_alignment",
        "top",
        "The vertical alignment of the legend with respect to the " "anchor point.",
        vtype=str,
...
def set_options(vtkchart, vtkrenderer, opt):
    ...
    if opt.isOptionValid("point"):
        ...
        legend.SetVerticalAlignment(
            eval("vtk.vtkChartLegend." + opt["vertical_alignment"].upper())
        )
        legend.SetHorizontalAlignment(
            eval("vtk.vtkChartLegend." + opt["horizontal_alignment"].upper())
        )
