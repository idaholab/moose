...
class ContourFilter(ChiggerFilterBase):
    ...
    def getOptions():
        ...
        opt.add(
            "count",
            10,
            "The number of contours to be automatically generated between the "
            "specified range.",
            vtype=int,
        )
        opt.add(
            "levels",
            None,
            "Explicitly define the contour levels, if this options is "
    ...
    def __init__(self, **kwargs):
        super(ContourFilter, self).__init__(
            vtkfilter_type=vtk.vtkContourFilter, **kwargs
        )
    ...
    def update(self, **kwargs):
        ...
        if varinfo.object_type != chigger.exodus.ExodusReader.NODAL:
            raise mooseutils.MooseException(
                "ContourFilter currently only works with nodal " "variables."
            )
