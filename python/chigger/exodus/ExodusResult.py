...
class ExodusResult(base.ChiggerResult):
    ...
    def getOptions():
        ...
        opt.add(
            "explode",
            None,
            "When multiple sources are being used (e.g., NemesisReader) "
        ...
        opt.add(
            "local_range",
            False,
            "Use local range when computing the default data range.",
        )
    ...
    def update(self, **kwargs):
        ...
        if self.isOptionValid("explode"):
            ...
            for src in self._sources:
                ...
                src.getVTKActor().AddPosition(
                    d[0] * factor, d[1] * factor, d[2] * factor
                )
    ...
    def getCenter(self):
        ...
        """
        a, b = self.getBounds()
        return ((b[0] - a[0]) / 2.0, (b[1] - a[1]) / 2.0, (b[2] - a[2]) / 2.0)
