...
class RenderWindow(base.ChiggerObject):
    ...
    def getOptions():
        ...
        opt.add(
            "size",
            [960, 540],
            "The size of the window, expects a list of two items",
            vtype=list,
        )
        opt.add(
            "style",
        ...
        opt.add(
            "test",
            False,
            "When True the interaction is disabled and the window closes "
            "immediately after rendering.",
        )
        opt.add(
            "motion_factor",
            "Control the interaction motion rate. "
        ...
        opt.add(
            "antialiasing",
            0,
            "Number of antialiasing frames to perform "
            "(set vtkRenderWindow::SetMultiSamples).",
            vtype=int,
        )
        ...
        opt.add(
            "observers",
            [],
            "A list of ChiggerObserver objects, once added they are not "
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__watermark = annotations.ImageAnnotation(
    ...
    def append(self, *args):
        ...
        for result in args:
            mooseutils.mooseDebug(
                "RenderWindow.append {}".format(type(result).__name__)
            )
    ...
    def setActive(self, result):
        ...
        if result not in self._results:
            mooseutils.mooseError(
                "The active result must be added to the RendererWindow prior to "
        ...
        if self.__vtkinteractor:
            self.__vtkinteractor.GetInteractorStyle().SetDefaultRenderer(
                result.getVTKRenderer()
            )
    ...
    def start(self, timer=None):
        ...
        if timer:
            msg = (
                "The timer argument is deprecated, please use the 'observers' setting."
            )
            mooseutils.mooseWarning(msg)

        mooseutils.mooseDebug(
            "{}.start()".format(self.__class__.__name__), color="MAGENTA"
        )
    ...
    def update(self, **kwargs):
        ...
        if self.isOptionValid("test") and self.getOption("test"):
            ...
            self.setOption(
                "style", None
            )  # avoids calling this function unless it changes
        ...
        if self.isOptionValid("motion_factor"):
            self.__vtkinteractor.GetInteractorStyle().SetMotionFactor(
                self.getOption("motion_factor")
            )
