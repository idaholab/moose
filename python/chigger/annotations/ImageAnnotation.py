...
class ImageAnnotation(base.ChiggerResult):
    ...
    def getOptions():
        ...
        opt.add(
            "position",
            (0.5, 0.5),
            "The position of the image center within the viewport, in "
        ...
        opt.add(
            "width",
            0.25,
            "The logo width as a fraction of the window width, this is ignored "
            "if 'scale' option is set.",
        )
        opt.add(
            "horizontal_alignment",
            "center",
            "The position horizontal position alignment.",
            allow=["left", "center", "right"],
        )
        opt.add(
            "vertical_alignment",
            "center",
            "The position vertical position alignment.",
            allow=["bottom", "center", "top"],
        )
        opt.add(
            "scale",
            None,
            "The scale of the image. (By default the image is scaled by the " "width.)",
            vtype=float,
        )
    ...
    def update(self, **kwargs):
        ...
        self._vtkcamera.SetPosition(size[0] / 2.0 + x, size[1] / 2.0 + y, d * 1 / scale)
        self._vtkcamera.SetFocalPoint(size[0] / 2.0 + x, size[1] / 2.0 + y, 0)
