...
class TestCombineCSV(unittest.TestCase):
    ...
    def testBasic(self):
        ...
        df_test = combine_csv.CombineCSV(
            self.__basename, "remove_me_54.csv", "large_number", write_header=True
        )
        self.assertTrue(df_test._ended)
        gold_df = pandas.read_csv("{}/combine_basic.csv".format(self.__goldpath))
        self.assertTrue(
            df_test._final_df.equals(gold_df),
    ...
    def testBasicTime(self):
        ...
        self.assertTrue(df_test._ended)
        gold_df = pandas.read_csv("{}/combine_basic_time.csv".format(self.__goldpath))
        self.assertTrue(
            df_test._final_df.equals(gold_df),
    ...
    def testBasicX(self):
        ...
        self.assertTrue(df_test._ended)
        gold_df = pandas.read_csv("{}/combine_basic_x.csv".format(self.__goldpath))
        self.assertTrue(
            df_test._final_df.equals(gold_df),
    ...
    def testBilinear(self):
        ...
        self.assertTrue(df_test._ended)
        gold_df = pandas.read_csv("{}/combine_bilinear.csv".format(self.__goldpath))

        self.assertTrue(
            df_test._final_df.shape == gold_df.shape,
        ...
        column_difference = df_test._final_df.columns.to_numpy(
            dtype=numpy.double
        ) - gold_df.columns.to_numpy(dtype=numpy.double)
        self.assertTrue(
            abs(data_difference.max()) < 1e-10,
            msg="Pandas data is different from gold CSV for bilinear usage.",
        )
        self.assertTrue(
            abs(column_difference.max()) < 1e-10,
