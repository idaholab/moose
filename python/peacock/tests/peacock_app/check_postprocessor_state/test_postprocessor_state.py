...
class TestPostprocessorState(Testing.PeacockImageTestCase):
    ...
    def setUp(self):
        ...
        self._window = (
            self._app.main_widget.tab_plugin.VectorPostprocessorViewer.currentWidget().FigurePlugin
        )
    ...
    def testTabChange(self):
        ...
        def get_active():
            return any(
                [
                    group._data._timer.isActive()
                    for group in vpp.currentWidget().PostprocessorSelectPlugin._groups
                ]
            )
