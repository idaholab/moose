...
class TestFilePlugin(Testing.PeacockImageTestCase):
    ...
    def testInitial(self):
        ...
        self.assertEqual(
            self._widget.FilePlugin.ComponentList.currentText(), "Magnitude"
        )
    ...
    def testDialogSingleFile(self, mock_exec, mock_open_files):
        ...
        self.assertEqual(
            [self._filenames[0]], [avail.itemData(i) for i in range(avail.count())]
        )
        self.assertEqual(
            [os.path.basename(self._filenames[0])],
            [avail.itemText(i) for i in range(avail.count())],
        )
    ...
    def testDialogMultipleFiles(self, mock_exec, mock_open_files):
        ...
        avail = self._widget.FilePlugin.FileList
        gold = [
            str(self._widget.FilePlugin.FileList.itemData(i))
            for i in range(self._widget.FilePlugin.FileList.count())
        ]
        self.assertEqual(self._filenames, gold)
        self.assertEqual(
            [os.path.basename(f) for f in self._filenames],
            [avail.itemText(i) for i in range(avail.count())],
        )
    ...
    def testVector(self):
        ...
        self._widget.FilePlugin.VariableList.currentIndexChanged.emit(1)
        self.assertEqual(
            os.path.basename(self._filenames[1]),
            str(self._widget.FilePlugin.FileList.currentText()),
        )
...
class TestFilePluginNewVariable(Testing.PeacockImageTestCase):
    ...
    def setUp(self):
        ...
        """
        self._filenames = Testing.get_chigger_input_list(
            "simple_diffusion_out.e", "simple_diffusion_new_var_out.e"
        )
        self._widget, self._window = main(size=[600, 600])
