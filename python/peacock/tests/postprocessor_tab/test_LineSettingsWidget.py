...
class TestLineSettingsWidget(Testing.PeacockImageTestCase):
    ...
    def testEmpty(self):
        ...
        self.assertFalse(
            self._toggle.ColorButton.isEnabled(), "ColorButton should be disabled."
        )
        self.assertFalse(
            self._toggle.PlotAxis.isEnabled(), "PlotAxis should be disabled."
        )
        self.assertFalse(
            self._toggle.LineStyle.isEnabled(), "LineStyle should be disabled."
        )
        self.assertFalse(
            self._toggle.LineWidth.isEnabled(), "LineWidth should be disabled."
        )
        self.assertFalse(
            self._toggle.MarkerStyle.isEnabled(), "MarkerStyle should be disabled."
        )
        self.assertFalse(
            self._toggle.MarkerSize.isEnabled(), "MarkerSize should be disabled."
        )
    ...
    def testMarkerStyleSize(self):
        ...
        self.assertTrue(
            self._toggle.MarkerSize.isEnabled(), "MarkerSize should be enabled."
        )
