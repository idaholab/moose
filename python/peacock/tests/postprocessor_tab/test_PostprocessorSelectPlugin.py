...
class TestPostprocessorSelectPlugin(Testing.PeacockImageTestCase):
    ...
    def setUp(self):
        ...
        self._control, self._widget, self._window = main(
            filenames, mooseutils.PostprocessorReader
        )
    ...
    def copyfiles(self, partial=False):
        ...
        if partial:
            shutil.copyfile(
                "../input/white_elephant_jan_2016_partial.csv", self._filename
            )
        else:
    ...
    def testRepr(self):
        ...
        self.assertIn(
            "data = mooseutils.PostprocessorReader('TestPostprocessorSelectPlugin_test.csv')",
            output,
        )
        self.assertIn("x = data('time')", output)
        self.assertIn("y = data('air_temp_set_1')", output)
        self.assertIn(
            "data = mooseutils.PostprocessorReader('TestPostprocessorSelectPlugin_test2.csv')",
            output,
        )
