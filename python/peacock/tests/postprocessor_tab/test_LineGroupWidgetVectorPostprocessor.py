...
class TestLineGroupWidgetVectorPostprocessor(Testing.PeacockImageTestCase):
    ...
    def testEmpty(self):
        ...
        self.assertFalse(
            self._control._toggles["index (Peacock)"].isEnabled(),
            "id toggle should be disabled.",
        )
    ...
    def testSelect(self):
        ...
        self.assertEqual(
            "index (Peacock)", self._window.axes()[0].get_xaxis().get_label().get_text()
        )
