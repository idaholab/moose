...
class GoldDiffPlugin(QtWidgets.QGroupBox, ExodusPlugin):
    ...
    def __init__(self, size=None):
        ...
        self.GoldVTKWindow = ExternalVTKWindowPlugin(
            self.GoldToggle, size=size, text="GOLD"
        )
        ...
        exodiff = os.path.join(
            os.getenv("MOOSE_DIR", moose_dir),
            "framework",
            "contrib",
            "exodiff",
            "exodiff",
        ...
        if os.path.isfile(exodiff):
            self.DiffVTKWindow = ExternalVTKWindowPlugin(
                self.DiffToggle, size=size, text="EXODIFF"
            )
    ...
    def updateOptions(self):
        ...
        if diff and (not self.DiffVTKWindow.isVisible()):
            ...
            (not diff)
            and (self.DiffVTKWindow is not None)
            and self.DiffVTKWindow.isVisible()
        ):
        ...
        if link:
            if gold and (self._gold_observer is None):
                self._gold_observer = (
                    self.GoldVTKWindow._window.getVTKInteractor().AddObserver(
                        "RenderEvent", self._callbackGoldRenderEvent
                    )
                )

            if diff and (self._diff_observer is None):
                self._diff_observer = (
                    self.DiffVTKWindow._window.getVTKInteractor().AddObserver(
                        "RenderEvent", self._callbackDiffRenderEvent
                    )
                )

        else:
            if self._gold_observer is not None:
                self.GoldVTKWindow._window.getVTKInteractor().RemoveObserver(
                    self._gold_observer
                )
                self._gold_observer = None

            if self._diff_observer is not None:
                self.DiffVTKWindow._window.getVTKInteractor().RemoveObserver(
                    self._diff_observer
                )
                self._diff_observer = None
...
def main(size=None):
    ...
    widget = ExodusPluginManager(
        plugins=[
            ...
            FilePlugin,
            lambda: GoldDiffPlugin(size=size),
        ]
    )
    widget.show()
