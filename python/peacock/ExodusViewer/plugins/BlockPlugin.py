...
class BlockPlugin(QtWidgets.QGroupBox, ExodusPlugin):
    ...
    def __init__(self, **kwargs):
        ...
        self.BlockSelector = BlockSelectorWidget(
            chigger.exodus.ExodusReader.BLOCK, title="Blocks"
        )
        self.SidesetSelector = BlockSelectorWidget(
            chigger.exodus.ExodusReader.SIDESET, title="Boundaries", enabled=False
        )
        self.NodesetSelector = BlockSelectorWidget(
            chigger.exodus.ExodusReader.NODESET, title="Nodesets", enabled=False
        )
    ...
    def onSetupResult(self, result):
        ...
        self._varinfo = reader.getVariableInformation(
            [chigger.exodus.ExodusReader.NODAL]
        )
    ...
    def onUpdateWindow(self, window, reader, result):
        ...
        """
        blocks = self._blocksChanged(
            reader, self.BlockSelector, chigger.exodus.ExodusReader.BLOCK
        )
        sideset = self._blocksChanged(
            reader, self.SidesetSelector, chigger.exodus.ExodusReader.SIDESET
        )
        nodeset = self._blocksChanged(
            reader, self.NodesetSelector, chigger.exodus.ExodusReader.NODESET
        )
    ...
    def _blocksChanged(self, reader, qobject, btype):
        ...
        current = [
            qobject.StandardItemModel.item(i).data(QtCore.Qt.UserRole)
            for i in range(1, qobject.StandardItemModel.rowCount())
        ]
    ...
    def _loadPlugin(self):
        ...
        if not self.BlockSelector.hasState(self.stateKey()):
            self.BlockSelector.StandardItemModel.item(0).setCheckState(
                QtCore.Qt.Checked
            )
        ...
        if not self.SidesetSelector.hasState(self.stateKey()):
            self.SidesetSelector.StandardItemModel.item(0).setCheckState(
                QtCore.Qt.Unchecked
            )
        ...
        if not self.NodesetSelector.hasState(self.stateKey()):
            self.NodesetSelector.StandardItemModel.item(0).setCheckState(
                QtCore.Qt.Unchecked
            )
...
def main(size=None):
    ...
    widget = ExodusPluginManager(
        plugins=[lambda: VTKWindowPlugin(size=size), FilePlugin, BlockPlugin]
    )
    widget.show()
...
if __name__ == "__main__":
    ...
    filenames = Testing.get_chigger_input_list(
        "mug_blocks_out.e", "mesh_only.e", "vector_out.e", "displace.e"
    )
