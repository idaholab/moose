...
class BackgroundPlugin(QtWidgets.QWidget, ExodusPlugin):
    ...
    def __init__(self, **kwargs):
        ...
        self._preferences.addBool(
            "exodus/backgroundGradient",
            "Use background gradient",
            True,
            "Turn on/off the background gradient",
        )

        self._preferences.addColor(
            "exodus/gradientTopColor",
        ...
        self._bottom = QtGui.QColor(
            self._preferences.value("exodus/gradientBottomColor")
        )
        self._solid = QtGui.QColor(
            self._preferences.value("exodus/solidBackgroundColor")
        )
    ...
    def addToMenu(self, menu):
        ...
        self.ColorbarBlackFontToggle.toggled.connect(
            self._callbackColorbarBlackFontToggle
        )
    ...
    def updateWindowOptions(self):
        ...
        if self.GradientToggle.isChecked():
            ...
            background = [bottom[0] / 255.0, bottom[1] / 255.0, bottom[2] / 255.0]
            background2 = [top[0] / 255.0, top[1] / 255.0, top[2] / 255.0]
            ...
            background = [solid[0] / 255.0, solid[1] / 255.0, solid[2] / 255.0]
            background2 = None
...
def main(size=None):
    ...
    widget = ExodusPluginManager(
        plugins=[lambda: VTKWindowPlugin(size=size), BackgroundPlugin, ColorbarPlugin]
    )
