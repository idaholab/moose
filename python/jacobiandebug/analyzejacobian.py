...
class REStruct(object):
    def __init__(self):
        ...
        self.MfdRE, self.MhcRE, self.MdiffRE = (
            re.compile(item) for item in (self.MfdRE, self.MhcRE, self.MdiffRE)
        )
...
def recursiveFindFile(current_path, p, executable):
    ...
    for afile in files:
        if p.match(afile) and (
            (not executable) or os.access(current_path + "/" + afile, os.X_OK)
        ):
        ...
        for j in range(nblocks):
            ...
            if norm[i][j] > rel_tol * fd[i][j] and norm[i][j] > abs_tol:
                ...
                if i == j:
                    ...
                    print(
                        "  (%d,%d) Off-diagonal Jacobian for variable '%s' %s"
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        default=1,
        type="int",
        help="Set the mesh dimensions to this number of elements along each dimension (defaults to 1, requires -r option).",
    )
    ...
    parser.add_option(
        ...
        "--only",
        dest="only",
        help="Test specified Jacobians only (space separated list of comma separated variable pairs).",
    )
    parser.add_option(
        "-D",
        "--on-diagonal-only",
    ...
    parser.add_option(
        ...
        dest="noauto",
        action="store_true",
        help="Do not add automatic options to the invocation of the moose based application. Requres a specially prepared input file for debugging.",
    )
    parser.add_option(
        "--rel-tol",
        dest="rel_tol",
        default=1e-4,
        type="float",
        help="The relative tolerance on the Jacobian elements between the hand-coded and those evaluated with finite difference.",
    )
    parser.add_option(
        "--abs-tol",
        dest="abs_tol",
        default=1e-12,
        type="float",
        help="The absolute tolerance on the Jacobian elements between the hand-coded and those evaluated with finite difference.",
    )
    parser.add_option(
        "--cli-args",
        type=str,
    ...
    if not options.noauto:
        ...
        mooseparams.extend(
            [
                "Problem/solve=false",
                "BCs/active=",
                "Outputs/" + dofoutname + "/type=DOFMap",
                "Outputs/active=" + dofoutname,
        ...
        try:
            child = subprocess.Popen(
                mooseparams, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
            )
    ...
    try:
        ...
        if child.returncode == -11:
            print(
                "The moose application crashed with a segmentation fault (try recompiling)"
            )
            sys.exit(1)
