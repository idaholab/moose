...
def verifyArgs(args):
    ...
    provided_custom_tol_params = [
        custom_tol_dict[x] for x in custom_tol_dict if custom_tol_dict[x] is not None
    ]
    ...
    if n_custom_columns > 0 and len(provided_custom_tol_params) == 0:
        allowed = ", ".join(sorted(custom_tol_dict.keys()))
        problems.append(
...
def parseArgs(args=None):
    ...
    parser.add_argument(
        ...
        metavar="tolerance",
        default="0",
        help="Value representing the acceptable absolute tolerance between comparisons (default: 0)",
    )
    parser.add_argument(
        "--relative-tolerance",
        metavar="tolerance",
