...
class TestReporterReader(unittest.TestCase):
    ...
    def setUp(self):
        ...
        """
        self._basicfile = os.path.abspath(
            "../../../test/tests/reporters/constant_reporter/gold/constant_reporter_out.json"
        )
        self._timefile = os.path.abspath(
            "../../../test/tests/reporters/accumulated_reporter/gold/accumulate_reporter_out.json"
        )
        self._onepertimefile = os.path.abspath(
            "../../../test/tests/outputs/json/one_file_per_timestep/gold/json_out_*.json"
        )
        self._partsfile = os.path.abspath(
            "../../../test/tests/reporters/mesh_info/gold/mesh_info_out.json"
        )
    ...
    def testBasic(self):
        ...
        # Test __getitem__
        self.assertEqual(
            data[("constant", "str_vec")], ["ten", "eleven", "twelve", "thirteen"]
        )
        self.assertEqual(
            data[[("constant", "num_2"), ("constant", "int_3")]], [5.0, -3]
        )
    ...
    def testTimeData(self):
        ...
        # Test __getitem__
        self.assertEqual(
            data[("accumulate", "rep:str")], ["two", "two", "two", "two", "two", "two"]
        )
