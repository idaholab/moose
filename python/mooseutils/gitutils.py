...
def git_is_config(key, value, working_dir=os.getcwd()):
    ...
    out = mooseutils.check_output(
        ["git", "config", "--get", key], cwd=working_dir
    ).strip()
    return out == value
...
def git_is_branch(name, working_dir=os.getcwd()):
    ...
    if git_is_repo(working_dir):
        out = mooseutils.check_output(
            ["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=working_dir
        ).strip()
        return out == name
    return False
...
def git_commit_message(sha, working_dir=os.getcwd()):
    ...
    """
    out = mooseutils.check_output(
        ["git", "show", "-s", "--format=%B", sha], cwd=working_dir
    )
    return out.strip(" \n")
...
def git_merge_commits(working_dir=os.getcwd()):
    ...
    """
    out = mooseutils.check_output(
        ["git", "log", "-1", "--merges", "--pretty=format:%P"], cwd=working_dir
    )
    return out.strip(" \n").split(" ")
...
def git_submodule_info(working_dir=os.getcwd(), *args):
    ...
    """
    out = dict()
    result = mooseutils.check_output(
        ["git", "submodule", "status", *args], cwd=working_dir
    )
    regex = re.compile(
        r"(?P<status>[\s\-\+U])(?P<sha1>[a-f0-9]{40})\s(?P<name>.*?)\s(?P<refs>(\(.*\))?)"
    )
...
def git_authors(loc=None):
    ...
    if not os.path.exists(loc):
        raise OSError(
            "The supplied location must be a file or directory: {}".format(loc)
        )
    loc = loc or os.getcwd()
    out = mooseutils.check_output(
        ["git", "shortlog", "-n", "-c", "-s", "--", loc], encoding="utf-8"
    )
    names = list()
...
def git_lines(filename, blank=False):
    ...
    regex = re.compile(
        r"^.*?\((?P<name>.*?)\s+\d{4}-\d{2}-\d{2}.*?\)\s+(?P<content>.*?)$",
        flags=re.MULTILINE,
    )
...
def git_repo(loc=os.getcwd(), remotes=["upstream", "origin"]):
    ...
    lookup = dict()
    for remote in (
        mooseutils.check_output(["git", "remote", "-v"], encoding="utf-8", cwd=loc)
        .strip(" \n")
        .split("\n")
    ):
    ...
    if address is None:
        raise OSError(
            "Unable to locate a remote with the name(s): {}".format(", ".join(remotes))
        )
...
def git_remotes(working_dir=None):
    ...
    lookup = dict()
    for remote in (
        mooseutils.check_output(
            ["git", "remote", "-v"], encoding="utf-8", cwd=working_dir
        )
        .strip(" \n")
        .split("\n")
    ):
...
def git_add_and_fetch_remote(url, name, branch, working_dir=None):
    ...
    """
    mooseutils.check_output(
        ["git", "remote", "add", name, url], cwd=working_dir, check=False
    )
