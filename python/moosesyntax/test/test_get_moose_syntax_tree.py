...
class TestSyntaxTree(unittest.TestCase):
    ...
    def testNodes(self):
        ...
        self.assertIn(
            "SetAdaptivityOptionsAction", [action.name for action in node.actions()]
        )
        ...
        self.assertTrue(
            node.source.endswith("framework/src/functions/MooseParsedFunction.C")
        )
        self.assertTrue(
            node.header.endswith("framework/include/functions/MooseParsedFunction.h")
        )
    ...
    def testRemove(self):
        ...
        remove = dict(
            first=["/Outputs", "/Kernels/Diffusion"], second=["/Kernels/ADDiffusion"]
        )
