...
class TestNodeBase(unittest.TestCase):
    ...
    def testRepr(self, *args):
        r = self._root
        self.assertIn(
            "LIGHT_RED:b: /b LIGHT_GREY:hidden=False removed=False group=None groups=set() test=False alias=None",
            repr(r(1)),
        )
        self.assertIn(
            "GREY:ab: /a/ab GREY:hidden=False removed=True group=None groups=set() test=False alias=None",
            repr(r(0, 1)),
        )
        self.assertIn(
            "RED:aa: /a/aa LIGHT_GREY:hidden=True removed=False group=None groups=set() test=False alias=None",
            repr(r(0, 0)),
        )
...
class TestSyntaxNode(unittest.TestCase):
    def setUp(self):
        ...
        self._objects = [
            MooseObjectNode(self._root, "mo0"),
            MooseObjectNode(self._root, "mo1"),
        ]
        self._actions = [
            ActionNode(self._root, "a0", parameters={"year": 1980}),
...
class TestObjectNodeBase(unittest.TestCase):
    ...
    def testProperties(self):
        source = os.path.abspath(
            os.path.join("..", "..", "..", "framework", "src", "kernels", "Diffusion.C")
        )
