...
if [[ "$kind" == "module" ]]; then
    ...
    echo "    2. Modify the moose/modules/combined/src/base/CombinedApp.C file"
    ...
    echo "    3. Modify the moose/modules/doc/config.yml file"
    echo "       a. Add the module to the content listing (alphabetical)"
    echo "    4. Initialize module SQA (reach out to the MOOSE development team with questions)"
    echo "       a. Navigate to moose/modules/$dstnamelow/doc"
    echo "       b. Run './moosedocs.py init sqa --module '$dstnamespace' --category '$dstnamelow'"
    echo "       c. This also registers the module, in alphabetical order, in the SQA extension"
    echo "          categories of modules/doc/config.yml and the Applications and Requirements"
    echo "          sections of modules/doc/sqa_reports.yml; review these before committing"
    echo "    5. Ensure that no stork files hang around before committing"
    echo "    6. Ensure that proper testing is performed for per-module tests (e.g. parallel, recover testing)"
    echo ""
