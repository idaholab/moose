...
MOOSE_DIR = os.environ.get("MOOSE_DIR", os.path.abspath(os.path.join(SCRIPT_DIR, "..")))
...
MOOSE_DEV_STACK = set(["libmesh-vtk", "peacock"]).union(set(TRACKING_LIBRARIES))

### All Libraries (versioner.yaml)
# This allows returning a hash for any library
# note: If adding to the following list below, see line 64 in
# python/MooseDocs/test/extensions/test_versioner.py
LIBRARIES = set(["seacas", "pyhit", "pprof"]).union(
    set(TRACKING_LIBRARIES).union(MOOSE_DEV_STACK)
)
...
class Versioner:
    ...
    def verify_recipes(self, args) -> None:
        ...
        all_influential = list(
            set(itertools.chain(*[p.all_influential for p in head.values()]))
        )
            ...
            if base_package is not None:
                ...
                for file, hash in base_influential.items():
                    if file not in influential:
                        ...
                        entries.append(
                            [package.name, colorize("REMOVED", "YELLOW"), file]
                        )
                ...
                if base_conda.install != head_conda.install:
                    ...
                    if different_version:
                        ...
                        if head_date is not None:
                            ...
                            if base_date is not None and date(*head_date) < date(
                                *base_date
                            ):
                                version_color = "RED"
                        ...
                        # Version is bumped, but build isn't zero
                            ...
                            head_package.build_number is not None
    ...
    def get_packages(ref: str) -> dict[str, Package]:
        ...
        for package in packages.values():
            influential = {
                dep.name: dep.all_influential.copy() for dep in package.dependencies
            }
            influential[package.name] = package.all_influential.copy()
            package.influential = {
                name: {file: None for file in influential[name]} for name in influential
            }
        ...
        files = list(
            set(itertools.chain(*[p.all_influential for p in packages.values()]))
        )
        ...
        for package in packages.values():
            ...
            for name in package.influential:
                package.influential[name] = {
                    k: hashes[k] for k in package.influential[name]
                }
        ...
        if not old_influential:
            for package in packages.values():
                ...
                for name in package.influential:
                    package.influential[name] = dict(
                        sorted(package.influential[name].items())
                    )
    ...
    def get_apptainer_package(package: Package) -> Union[ApptainerPackage, None]:
        ...
        if from_name:
            if from_name not in [dep.name for dep in package.dependencies]:
                Versioner().error(
                    f"apptainer.from for {package.name} " "is not a dependency"
                )
        ...
        values = {
            ...
            "def_path": os.path.abspath(
                os.path.join(MOOSE_DIR, f"apptainer/{def_package}.def")
            ),
        }
    ...
    @staticmethod
    def get_conda_package(package: Package, commit: str) -> Union[CondaPackage, None]:
        """
    ...
    def get_template_config() -> TemplateConfig:
        ...
        return TemplateConfig(
            prefix="__VERSIONER_", suffix="__", variables=["version", "build_number"]
        )
    ...
    def get_template_replacements(package: Package) -> list[Tuple[str, str]]:
        ...
        for p in [package] + package.dependencies:
            ...
            for var in config.variables:
                template_variable = (
                    f"{config.prefix}{name}_{var.upper()}{config.suffix}"
                )
    ...
    @staticmethod
    def augment_template(package: Package, file: os.PathLike) -> str:
        """
        ...
        if search:
            Versioner.error(
                f"Unused template variable {search.group(0)} " f"still exists in {file}"
            )
    ...
    def parse_args(argv: list[str], entities: list[str]) -> argparse.Namespace:
        ...
        parser = argparse.ArgumentParser(
            description="Supplies a hash for a given library"
        )
        parser.add_argument(
            "library",
            nargs="?",
        ...
        parser.add_argument(
            "--build-templates", action="store_true", help="Builds the templates"
        )
            ...
            if getattr(args, action):
                for other_action in actions:
                    if action != other_action and getattr(args, other_action):
                        Versioner.error(
                            f"Cannot use --{action} and " f"--{other_action} together"
                        )

        if args.verify and args.verify == "HEAD":
            Versioner.error(
                "You cannot verify against HEAD. You must choose a hash"
    ...
    @staticmethod
    def git_hash(
        file: os.PathLike, commit: str, repo_dir: os.PathLike = MOOSE_DIR
    ) -> str:
        ...
        raise Exception(
            f"Failed to obtain git hash for {file} in {repo_dir} at {commit}"
        )

    @staticmethod
    def git_ancestor(
        maybe_ancestor: str, descendant: str, repo_dir: os.PathLike = MOOSE_DIR
    ) -> bool:
    ...
    def render_jinja(contents: str) -> str:
        ...
        env = jinja2.Environment(
            loader=jinja2.DictLoader({"": contents}),
            trim_blocks=True,
            lstrip_blocks=True,
        )
    ...
    def get_app_info() -> Union[AppInfo, None]:
        ...
        process = subprocess.run(
            tree_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False
        )
        if process.returncode != 0:
            return None
        ...
        return AppInfo(name=app_name, git_root=git_root, hash=str(git_hash[0:7]))
    ...
    def git_rev_parse(ref: str, directory: os.PathLike = MOOSE_DIR) -> str:
        ...
        try:
            ...
            if e.returncode == 128:
                raise ValueError(
                    f"Reference {ref} is not valid in repo {directory}"
                ) from e
            raise
    ...
    def versioner_yaml_path(commit: str) -> str:
        ...
        use_module_hash = Versioner.git_is_commit(
            changed_commit
        ) and not Versioner.after_commit(changed_commit, commit)
    ...
    @staticmethod
    def get_influential_hashes(files: list[str], commit: str) -> dict[str, str]:
        """
