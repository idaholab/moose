...
class lldbAPI:
    ...
    def getStackTrace(self, process_tuple):
        ...
        if target.process.id is not 0:
            process = target.Attach(
                lldb.SBAttachInfo(target.process.id), lldb.SBError()
            )
        else:
        ...
        if process:
            ...
            for i in xrange(process.GetThreadAtIndex(0).GetNumFrames()):
                lldb_results.append(
                    process.GetThreadAtIndex(0).GetFrameAtIndex(i).__str__()
                )
...
class DebugInterpreter:
    ...
    def _parseStackTrace(self, gibberish):
        not_gibberish = re.findall(
            r"\(" + self.debugger + "\) (#.*)\(" + self.debugger + "\)",
            gibberish,
            re.DOTALL,
        )
...
class Server:
    ...
    def _launchClients(self):
        ...
        print(
            "Memory Logger running on Host:",
            self.host,
            "Port:",
            self.port,
            "\nNodes:",
            ", ".join(nodes),
            "\nSample rate (including stdout):",
            self.arguments.repeat_rate[-1],
            "s (use --repeat-rate to adjust)",
            "\nRemote agents delaying",
            self.arguments.pbs_delay[-1],
            "second/s before tracking. (use --pbs-delay to adjust)\n",
        )
        ...
        for node in nodes:
            command.append(
                [
                    ...
                    + "sleep "
                    + str(self.arguments.pbs_delay[-1])
                    + " && "
    ...
    def _launchJob(self):
        subprocess.Popen(
            self.arguments.run[-1].split(), stdout=self.agent.log, stderr=self.agent.log
        )
    ...
    def handleAgent(self):
        ...
        for agent_socket in self.client_connections:
            ...
            if new_data != None:
                ...
                if self.arguments.recover:
                    self.agent.agent_data[agent_uuid]["TIMESTAMP"] = (
                        GetTime().now - self.agent.delta
                    )
                else:
                        ...
                        if new_data[agent_uuid]["REQUEST"][request[0]] == "":
                            ...
                            for request_type in dir(self.arguments):
                                if request[0] == str(request_type):
                                    self.agent.agent_data[agent_uuid]["REQUEST"][
                                        request[0]
                                    ] = getattr(self.arguments, request[0])
...
class Client:
    def __init__(self, arguments):
        ...
        self.my_agent = Agent(
            arguments, str(uuid.uuid3(uuid.NAMESPACE_DNS, socket.gethostname()))
        )
        ...
        if self.arguments.call_back_host:
            ...
            self.my_agent.agent_data[self.my_agent.my_uuid] = (
                self.remote_server.sendData(self.my_agent.agent_data)
            )

            # Apply new instructions received from server (this basically updates our arguments)
            for request in self.my_agent.agent_data[self.my_agent.my_uuid][
                "REQUEST"
            ].iteritems():
                for request_type in dir(self.arguments):
                    if request[0] == str(request_type):
    ...
    def startProcess(self):
        ...
        if self.arguments.call_back_host == None:
            subprocess.Popen(
                self.arguments.run[-1].split(),
                stdout=self.my_agent.log,
                stderr=self.my_agent.log,
            )
        ...
        try:
            ...
            if self.arguments.call_back_host == None:
                print(
                    "Binary has exited and a log file has been written. You can now attempt to view this file by running"
                    "\nthe memory_logger with either the --plot or --read arguments:\n\n",
                    sys.argv[0],
                    "--plot",
                    self.arguments.outfile[0],
    ...
    def __init__(self, arguments, connection=None):
        ...
        if self.connection == None and self.arguments.call_back_host != None:
            ...
            self.connection.connect(
                (
                    self.arguments.call_back_host[0],
                    int(self.arguments.call_back_host[1]),
                )
            )
...
class Agent:
    ...
    def __init__(self, arguments, machine_id):
        ...
        if self.arguments.recover:
            if os.path.exists(self.arguments.outfile[-1]):
                ...
                print(
                    "Recovery options detected, but I could not find your previous memory log file."
                )
                sys.exit(1)
        else:
            self.delta = 0
    ...
    def _getMemory(self):
        ...
        if tmp_pids != {}:
            ...
            if memory_usage == 0:
                ...
                self.agent_data[self.my_uuid][
                    "DEBUG_LOG"
                ] = "I found the total memory usage of all my processes hit 0. Stopping"
        ...
        self.agent_data[self.my_uuid][
            "DEBUG_LOG"
        ] = "I found no processes running. Stopping"
        return 0
    ...
    def _getPIDs(self):
        ...
        if self.arguments.run:
            ...
            for single_pid in all_pids:
                if (
                    single_pid.find(self.track_process) != -1
                    and single_pid.find(__file__) == -1
                    and single_pid.find("mpirun") == -1
                    and single_pid.find(os.getenv("USER")) != -1
                    and single_pid.find("mpiexec") == -1
                ):
                    pid_list[int(single_pid.split()[0])] = []
                    pid_list[int(single_pid.split()[0])].extend(
                        [single_pid.split()[1], single_pid.split()[3]]
                    )
        return pid_list
...
class MemoryPlotter:
    ...
    def buildGraph(self):
        try:
            ...
            print(
                "Error importing matplotlib. Matplotlib not available on this system?"
            )
            sys.exit(1)
        ...
        for plot_name, value_list in plot_dictionary.iteritems():
            ...
            try:
                ...
                print(
                    ...
                    plot_name,
                    "is this a valid memory_logger file?",
                )
                sys.exit(1)
            ...
            if self.arguments.stdout:
                ...
                self.buildAnnotation(
                    plot_list[-1], tmp_stdout_x, tmp_stdout_y, stdout_msg, f.get_color()
                )

            if self.arguments.pstack:
                (pstack_line,) = plot_list[-1].plot(
                    tmp_pstack_x,
                    tmp_pstack_y,
                    "o",
                    picker=10,
    ...
    def buildAnnotation(self, fig, x, y, msg, c):
        for i in range(len(x)):
            fig.annotate(
                ...
                xytext=(
                    decimal.Decimal(x[i])
                    + decimal.Decimal(self.arguments.move_text[0]),
                    decimal.Decimal(y[i])
                    + decimal.Decimal(self.arguments.move_text[1]),
                ),
                color=c,
                horizontalalignment="center",
                verticalalignment="bottom",
                arrowprops=dict(
                    arrowstyle="->", connectionstyle="arc3,rad=0.5", color=c
                ),
            )
...
class ReadLog:
    ...
    def printHistory(self):
        ...
        for timestamp in self.memory_list:
            ...
            self.sorted_list.append(
                [
                    ...
                    + str(to.minute)
                    + ":"
                    + "{:02.0f}".format(to.second)
                    + "."
                    + "{:06.0f}".format(to.microsecond),
                    total_memory,
                    log,
                    pstack,
                    node_name,
        ...
        print(
            "Date Stamp"
            + " " * int(17)
            + "Memory Usage | Percent of MAX memory used: ( "
            + str("{:0,.0f}".format(largest_memory))
            + " K )"
        )
        for item in self.sorted_list:
            tmp_str = ""
            if decimal.Decimal(item[1]) == largest_memory:
                tmp_str = self.formatText(
                    largest_memory,
                    item[0],
                    item[1],
                    item[5],
    ...
    def formatText(
        ...
        if decimal.Decimal(total_memory) == largest_memory:
            ...
            percent = (
                str(decimal.Decimal(total_memory) / largest_memory)[2:4]
                + "."
                + str(decimal.Decimal(total_memory) / largest_memory)[4:6]
            )
        ...
        return (
            ...
            * (
                ...
                - (
                    int(node_position)
                    + ((len(message) - additional_correction) + footer)
                )
            )
            + RESET
            + "| "
...
def verifyArgs(args):
    ...
    if args.legend not in possible_positions:
        print(
            "Invalid legend position requested. Possible values are:\n\t",
    ...
    if args.outfile == None and args.run:
        ...
        if re.findall(r"-i (\w+)", args.run[0]) != []:
            ...
            args.outfile = [
                os.getcwd()
                + "/"
                + args.run[0].replace("..", "").replace("/", "").replace(" ", "_")
                + ".log"
            ]
    ...
    if args.pstack and (args.read is None and args.plot is None):
        if args.debugger is not None:
            ...
            print(
                "Invalid debugger selected. You must choose between gdb and lldb using the --debugger argument"
            )
            sys.exit(1)
    return args
...
def parseArguments(args=None):
    ...
    rungroup = parser.add_argument_group(
        "Tracking",
        "The following options control how the memory logger tracks memory usage",
    )
    rungroup.add_argument(
        "--run",
        nargs=1,
        metavar="command",
        help="Run specified command using absolute paths. You must encapsulate the command in quotes.",
    )
    rungroup.add_argument(
        "--pbs",
        dest="pbs",
        ...
        const=True,
        default=False,
        help="Instruct memory logger to tally all launches on all nodes\n ",
    )
    rungroup.add_argument(
        "--pbs-delay",
        dest="pbs_delay",
        ...
        type=float,
        default=[1.0],
        help="For larger jobs, you may need to increase the delay as to when the memory_logger will launch the tracking agents\n ",
    )
    rungroup.add_argument(
        "--sample-delay",
        dest="sample_delay",
        ...
        type=float,
        default=[0.25],
        help="The time to delay before taking the first sample (when not using pbs)",
    )
    rungroup.add_argument(
        "--repeat-rate",
        nargs=1,
        ...
        type=float,
        default=[0.25],
        help="Indicate the sleep delay in float seconds to check memory usage (default 0.25 seconds)\n ",
    )
    rungroup.add_argument(
        "--outfile",
        nargs=1,
        metavar="file",
        help="Save log to specified file. (Defaults based on run command)\n ",
    )

    readgroup = parser.add_argument_group(
        "Read / Display",
        "Options to manipulate or read log files created by the memory_logger",
    )
    readgroup.add_argument(
        "--read",
        nargs=1,
        metavar="file",
        help="Read a specified memory log file to stdout\n ",
    )
    readgroup.add_argument(
        "--separate",
        dest="separate",
        ...
        const=True,
        default=False,
        help="Display individual node memory usage (read mode only)\n ",
    )
    readgroup.add_argument(
        "--plot",
        nargs="+",
        metavar="file",
        help="Display a graphical representation of memory usage (Requires Matplotlib). Specify a single file or a list of files to plot\n ",
    )
    readgroup.add_argument(
        "--legend",
        metavar='"lower left"',
        default="lower left",
        help='Place legend in one of the following locations (default --legend "lower left") "center", "center left", "center right", "upper center", "lower center", "best", "right", "left", "upper right", "lower right", "upper left", "lower left"\n ',
    )

    commongroup = parser.add_argument_group(
        "Common Options",
        "The following options can be used when displaying the results",
    )
    commongroup.add_argument(
        ...
        const=True,
        default=False,
        help="Display/Record stack trace information (if available)\n ",
    )
    commongroup.add_argument(
        "--stdout",
        dest="stdout",
        ...
        const=True,
        default=False,
        help="Display stdout information\n ",
    )
    commongroup.add_argument(
        "--debugger",
        dest="debugger",
        metavar="gdb | lldb",
        nargs="?",
        help="Specify the debugger to use. Possible values: gdb or lldb\n ",
    )

    plotgroup = parser.add_argument_group(
        "Plot Options", "Additional options when using --plot"
    )
    plotgroup.add_argument(
        ...
        type=int,
        default=[30],
        help="Rotate stdout/pstack text by this ammount (default 30)\n ",
    )
    plotgroup.add_argument(
        "--move-text",
        nargs=2,
        metavar="int",
        default=["0", "0"],
        help="Move text X and Y by this ammount (default 0 0)\n ",
    )
    plotgroup.add_argument(
        "--trim-text",
        nargs=1,
        ...
        type=int,
        default=[15],
        help="Display this many characters in stdout/pstack (default 15)\n ",
    )
    plotgroup.add_argument(
        "--no-color",
        dest="no_color",
        ...
        action="store_const",
        const=False,
        help="When printing output to stdout do not use color codes\n ",
    )
    plotgroup.add_argument(
        "--darkmode",
        dest="darkmode",
        ...
        action="store_const",
        const=True,
        help="When you want to be cool\n ",
    )

    internalgroup = parser.add_argument_group(
        "Internal PBS Options",
        "The following options are used to control how memory_logger as a tracking agent connects back to the caller. These are set automatically when using PBS and can be ignored.",
    )
    internalgroup.add_argument(
        "--call-back-host",
        nargs=2,
        help="Server hostname and port that launched memory_logger\n ",
    )
