...
with open(
    os.path.join(MOOSE_DIR, "scripts", "tests", "versioner_hashes.yaml"), "r"
) as stream:
...
class Test(unittest.TestCase):
        ...
        for commit, libraries in OLD_HASHES.items():
            ...
            for name, values in libraries.items():
                ...
                for key in ["hash", "full_version"]:
                    ...
                    if value:
                        self.assertEqual(
                            value, getattr(package, key), f"{name}.{key} at {commit}"
                        )
    ...
    def testGitHash(self):
        self.assertEqual(
            Versioner.git_hash("LICENSE", "6724fe26513b2a4f458f5fe8dbd253c2059bda59"),
            "4362b49151d7b34ef83b3067a8f9c9f877d72a0e",
        )
        self.assertEqual(
            Versioner.git_hash("README.md", "21bace124ee2bfc46350b1f3540accab5d1ab0bb"),
            "d801a34a243d319e79991b299aa3eb8e8fe937c0",
        )
    ...
    def testGitFile(self):
        self.assertNotIn(
            "thm",
            Versioner.git_file(".coverage", "bc75e4a07af609e3cfbc5c789aa69a8b3fc2c099"),
        )
        self.assertIn(
            "thm",
            Versioner.git_file(".coverage", "3ce960385af649e3a0737ed445410bcb998b2267"),
        )
        ...
        self.assertEqual(
            f"Supplied path {out_of_repo_path} is not in {MOOSE_DIR}", str(e.exception)
        )
