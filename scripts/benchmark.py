...
os.environ["PYTHONPATH"] = (
    os.environ.get("PYTHONPATH", "") + ":" + os.path.join(MOOSE_DIR, "python")
)
...
def find_moose_python():
    ...
    if not os.path.exists(moosepython):
        raise Exception(
            "Unable to locate moose/python directory, please set MOOSE_DIR environment variable"
        )
...
def build_args():
    ...
    p.add_argument(
        "--db",
        type=str,
        default=os.environ.get("MOOSE_SPEED_DB", "speedtests.sqlite"),
        help="benchmark timings database",
    )
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="compare benchmark runs from the given revision (default prev of most recent)",
    )
    p.add_argument(
        "--new",
        type=str,
        default="",
        help="compare benchmark runs to the given revision (default most recent)",
    )
    ...
    p.add_argument(
        "--run",
        action="store_true",
        help="run all benchmarks on the current checked out revision",
    )
    p.add_argument(
        "--perflog", action="store_true", help="store perflog benchmark info"
    )
    p.add_argument(
        "--rev",
        type=str,
        default="",
        help="manually specify git revision for this set of benchmarks",
    )
    p.add_argument(
        "--benchlist",
        type=str,
    ...
    p.add_argument(
        ...
        type=float,
        default=60,
        help="cumulative time (secs) to run each benchmark",
    )
    p.add_argument(
        "--min-runs",
        type=int,
        default=40,
        help="minimum number of runs for each benchmark",
    )
    p.add_argument(
        "--list-revs",
        action="store_true",
        help="list all benchmarked revisions in the db",
    )
    p.add_argument(
        "--trends",
        action="store_true",
        help="generate plots of historical trends of all benchmarks",
    )
    p.add_argument(
        "--psig",
        type=float,
        default=0.01,
        help="the p-value cutoffused to determine comparison significance",
    )
    p.add_argument(
        "--baseurl",
        type=str,
        default="https://github.com/idaholab/moose/commit/",
        help="the url prefix for commit links in generated visualization/output",
    )
    return p
        ...
        with DB(args.db) as db:
            for bench in benches:
                ...
                t = Test(
                    ...
                    args=bench.args,
                    rootdir=rootdir,
                    perflog=bool(args.perflog),
                )
                b = Bench(
                    bench.name, test=t, cum_dur=args.cum_dur, min_runs=args.min_runs
                )
                b.run()
                if args.rev != "":
...
def buildpage(
    ...
    lastn=60,
    method="opt",
    baseurl="https://github.com/idaholab/moose/commit/",
):
    figpage = """
<!DOCTYPE html>
<meta charset="utf-8">
...
"""
    ...
    if len(revs) > 0:
        Comp = collections.namedtuple(
            "Comp", "revision1 revision2 date1 date2 link1 link2 text"
        )
    ...
    data = t.render(
        benchnames=plotnames, comparisons=comparisons, refcomparisons=refcomparisons
    )
...
def plot(
    ...
    plt.axhline(y=median, dashes=[48, 4, 12, 4], color="black", alpha=0.5)
    plt.axhline(y=median * 0.99, linestyle=":", linewidth=2, color="green", label="-1%")
    plt.axhline(
        y=median * 0.95,
        linestyle="--",
        linewidth=2,
