...
class ApptainerGenerator:
    ...
    def verify_args(args):
        ...
        requirements = [
            ("oras", args.action == "def" and not args.local),
    ...
    def parse_args(entities):
        ...
        def add_default_args(
            parser, write=True, remote=False, remote_fetch=False, repo=True
        ):
            parser.add_argument(
                "library", choices=entities, help="The library to act on"
            )
            if repo:
                parser.add_argument(
                    "--suffix", type=str, help="Suffix to add to the name"
                )
            ...
            if remote_fetch:
                parser.add_argument(
                    "--project-suffix", type=str, help="Suffix to add to the project"
                )

        exists_parser = action_parser.add_parser(
            "exists",
            parents=[parent],
            help="Checks if a container exists" + " on a remote",
        )
        add_default_args(exists_parser, write=False, remote=True, remote_fetch=True)

        def_parser = action_parser.add_parser(
            "def", parents=[parent], help="Generates a definition"
        )
        ...
        pull_parser = action_parser.add_parser(
            "pull", parents=[parent], help="Pull a container"
        )
        add_default_args(pull_parser, remote=True, remote_fetch=True)
        pull_parser.add_argument(
            "--pull-args", type=str, help="Arguments to pass to apptainer pull"
        )

        path_parser = action_parser.add_parser(
            "path", parents=[parent], help="Get the local path to a container"
        )
        add_default_args(path_parser, write=False)

        build_parser = action_parser.add_parser(
            "build", parents=[parent], help="Build a container"
        )
        add_def_args(build_parser)
        build_parser.add_argument(
            "--build-args", type=str, help="Arguments to pass to apptainer build"
        )
        build_parser.add_argument(
            "--sign", type=int, help="Sign the built container with the given key"
        )
        build_parser.add_argument(
            "--skip-tests",
            action="store_true",
        ...
        push_parser = action_parser.add_parser(
            "push", parents=[parent], help="Push a container"
        )
        add_default_args(push_parser, remote=True)
        push_parser.add_argument(
            "--to-tag", type=str, help="An alternate tag to push to"
        )
        push_parser.add_argument(
            "--to-tag-prefix", type=str, help="A prefix to add to the pushed tag"
        )
        push_parser.add_argument(
            "--to-project-suffix",
            type=str,
        ...
        uri_parser = action_parser.add_parser(
            "uri", parents=[parent], help="Get the URI to a container"
        )
        ...
        tag_parser = action_parser.add_parser(
            "tag", parents=[parent], help="Get the tag for a container"
        )
    ...
    def print(content, prefix_color="green", file=sys.stdout):
        ...
        prefix = (
            "["
            + ApptainerGenerator.add_color("\033[1mgenerator\033[0m", prefix_color)
            + "]"
        )
    ...
    def git_submodule_remote(dir, name):
        ...
        if remote.startswith("../.."):
            ...
            base_remote = subprocess.check_output(
                base_command, cwd=dir, encoding="utf-8"
            ).strip()
            host_re = re.search(r"^git@([a-zA-Z0-9_.-]+\.[a-zA-Z]+):", base_remote)
            if not host_re:
                host_re = re.search(
                    r"^https:\/\/([a-zA-Z0-9_.-]+\.[a-zA-Z]+)\/", base_remote
                )
            if not host_re:
                raise Exception(
                    f"Failed to replace ../../ in git submodule remote for {name}"
                )
    ...
    def apptainer_pull(
        self, project: str, name: str, tag: str, project_suffix=None, args=None
    ):
        """
        ...
        if args is not None:
            command += args
        if (
            hasattr(self.args, "disable_cache")
            and self.args.disable_cache
            and "--disable-cache" not in command
        ):
    ...
    def apptainer_push(
        self, project: str, name: str, from_tag: str, to_tag=None, project_suffix=None
    ):
        """
    ...
    def _dependency_from(self, package: Package) -> str:
        ...
        if not self.oras_exists(uri):
            ...
            uri = self.oras_uri(
                project, name, tag, project_suffix=self.args.project_suffix
            )
    ...
    def create_filename(app_root, section_key, actions):
        ...
        if os.path.exists(os.path.join(app_root, "apptainer", f"{section_key}.sh")):
            file_list = [
                (
                    f"SECTION_{section_key.upper()}",
        ...
        for method in ["pre", "post"]:
            ...
            if os.path.exists(
                os.path.join(app_root, "apptainer", f"{section_key}_{method}.sh")
            ):
                file_list.append(
                    (
                        f"SECTION_{section_key.upper()}_{method.upper()}",
                        os.path.join(
                            app_root, "apptainer", f"{section_key}_{method}.sh"
                        ),
                    )
                )
            ...
            for action in actions:
                file_path = os.path.join(
                    app_root, "apptainer", f"{section_key}_{method}_{action}.sh"
                )
                # Support sections that have method and an action (post_pre_configure.sh)
                if os.path.exists(file_path):
                    file_list.append(
                        (
                            f"SECTION_{section_key.upper()}_"
    ...
    def add_definition_vars(self, jinja_data):
        ...
        if self.args.library == "moose-dev":
            ...
            source = {
                k: v
                for element in package.conda.meta["source"]
        ...
        need_versions = {
            "petsc": [{"package": "petsc", "submodule": "petsc"}],
    ...
    def _action_exists(self):
        ...
        """
        uri = self.oras_uri(
            self.project, self.name, self.tag, project_suffix=self.args.project_suffix
        )
    ...
    def _action_pull(self):
        ...
        self.apptainer_pull(
            ...
            self.name,
            self.tag,
            project_suffix=self.args.project_suffix,
            args=args,
        )
    ...
    def _action_def(self):
        ...
        if self.args.dep:
            ...
            if not needs_from:
                self.error(
                    f"Library {self.name} does not need a dependency, but --dep was provided"
                )
        ...
        container_definition_path = self.container_path(
            self.name, self.tag, image=False
        )
    ...
    def _action_push(self):
        ...
        uri = self.oras_uri(
            self.project, self.name, to_tag, project_suffix=self.args.to_project_suffix
        )
        if self.oras_exists(uri):
            if self.args.overwrite:
        ...
        self.apptainer_push(
            ...
            from_tag,
            to_tag,
            project_suffix=self.args.to_project_suffix,
        )
