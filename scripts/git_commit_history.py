...
def getData(options):
  ...
  all_dates = sorted(set(run('git', 'log', '--reverse', '--format=%ad', '--date=short')))
  ...
  commits = run('git', 'log', '--format=%H\n%ad\n%aN', '--date=short', '--no-merges', '--reverse', '--shortstat', split=False)
...
if __name__ == '__main__':
  ...
  if options.unique:
    ...
    ax2.annotate('Unique Contributors', xy=(dates[i], contrib[i]), xytext=(datetime.date(2014,1,1), 60), ha='right', size=options.font, arrowprops=arrow)
  ...
  if options.stack: # stack plot
    handles = pylab.stackplot(dates, data['commits'])
    ...
    for i in range(len(contributors)):
      ...
      clr = color.next()
      h = ax1.fill_between(x, 0, y, label=label, linewidth=2, edgecolor=clr, facecolor=clr, alpha=0.5)
      handles.append(h)
    ...
    if not options.disable_legend:
      lgnd = pylab.legend(handles, legend_labels, loc='upper left', fontsize=options.font)
    ...
    if not options.disable_legend:
      lgnd = pylab.legend(handles, contributors, loc='upper left', fontsize=options.font)
      lgnd.draw_frame(False)
  ...
  if options.open_source:
    ...
    pylab.gca().add_patch(pylab.Rectangle((os, y_lim[0]), delta, y_lim[1]-y_lim[0], facecolor='green', alpha=0.2))
