...
def getData(options):
    ...
    if dev:
        ...
        contributors = [
            "MOOSE developers (" + str(len(contributors) - 1) + ")",
            contributors[-1],
        ]
    ...
    dates = [
        d1 + datetime.timedelta(days=x) for x in range(0, (d2 - d1).days, options.days)
    ]

    # Build the data arrays, filled with zeros
    N = numpy.zeros((len(contributors), len(dates)), dtype=int)
    data = {
        "commits": numpy.zeros((len(contributors), len(dates)), dtype=int),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Tool for building commit history of a git repository"
    )
    parser.add_argument(
        "--additions", action="store_true", help="Show additions/deletions graph"
    )
    parser.add_argument(
        "--days",
        type=int,
        default=1,
        help="The number of days to lump data (e.g., use 7 for weekly data)",
    )
    parser.add_argument(
        "--disable-legend", action="store_true", help="Disable display of legend"
    )
    parser.add_argument(
        "--stack",
        "-s",
        action="store_true",
        help="Show graph as stacked area instead of line plot",
    )
    parser.add_argument(
        "--unique",
        "-u",
        action="store_true",
        help="Show unique contributor on secondary axis",
    )
    parser.add_argument(
        "--unique-label",
        default="legend",
        choices=["none", "arrow", "legend"],
        help="Control how the unique contributor line is labeled.",
    )
    parser.add_argument(
        "--open-source",
        "-r",
        action="store_true",
        help="Show shaded region for open sourcing of MOOSE",
    )
    parser.add_argument(
        "--output",
        "-o",
        type=str,
        default="commit_history.pdf",
        help="The filename for writing the plot to a file",
    )
    parser.add_argument(
        "--authors",
        default=None,
        help='Limit the graph to the given number of entries authors, or use "moose" to limit to MOOSE developers',
    )
    parser.add_argument(
        "--moose-dev",
        action="store_true",
        help="Create two categories: MOOSE developers and other (this overrides --authors)",
    )
    parser.add_argument(
        "--framework",
        action="store_true",
        help="Limit the analysis to framework directory",
    )
    parser.add_argument(
        "--modules", action="store_true", help="Limit the analysis to modules directory"
    )
    ...
    if options.stack:  # stack plot
        ...
        if not options.disable_legend:
            ...
            lgnd = plt.legend(
                handles, contributors, loc="upper left", fontsize=options.font
            )
            lgnd.draw_frame(False)
