...
def getClones(args):
    ...
    urls = {
        "clones": "/".join([github, args.repo, "traffic/clones"]),
    ...
    for search_type, url in urls.iteritems():
        ...
        if r.status_code != 200:
            print(
                "There was an error while attempting to gather data:",
                r.status_code,
                "\n",
                r.text,
            )
...
def writeFile(args, stats):
    if os.path.isfile(args.write):
        ...
        if args.repo == file_data.split("\n")[0]:
            ...
            tmp_clones = old_clones[
                old_clones.index(stats["clones"][len(stats["clones"]) - 3]) + 3 :
            ]
            tmp_visitors = old_visitors[
                old_visitors.index(stats["views"][len(stats["views"]) - 3]) + 3 :
            ]
            ...
            print(
                "The file you attempted to write to contains stats for another repository ("
                + file_data.split("\n")[0]
                + ")\nwhile you supplied arguments to gather stats for ("
                + args.repo
...
def parseArgs(args=None):
    parser = argparse.ArgumentParser(
        description="Obtain cloning information using GitHub's API"
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="",
        help="Authenticate using specified token. Token must have admin priviledges for the 'repo' scope in your user settings.",
    )
    parser.add_argument(
        "--per-week",
        action="store_const",
        const=True,
        default=False,
        help="Obtain clones per week instead of day",
    )
