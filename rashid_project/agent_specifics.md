...
### Step 3.2 (2026-05-21) — Rashid objective stress rate (strategy-pattern refactor)

Added the **Rashid** objective rate from `plan_outline.pdf §3.2`:
`σ_{n+1} = r̂ (σ_n + Δσ) r̂^T` with `r̂ = exp(Δw)` (universal across all
`kinematic_approximation` options; eq. 26).

To support a rate that doesn't fit the existing linear advection template
without proliferating user-facing material classes, the per-rate logic was
lifted into a new **helper class hierarchy** (strategy pattern). The existing
material class `ComputeLagrangianObjectiveStress` stays the single user-facing
type and dispatches via its enum.

New files:
- `include/materials/lagrangian/LagrangianObjectiveRate.h`
- `src/materials/lagrangian/LagrangianObjectiveRate.C`

Class layout:
```
LagrangianObjectiveRate (abstract)
├── LagrangianLinearObjectiveRate     // owns advectStress, updateTensor,
│   │                                 // stressAdvectionDerivative, cauchyJacobian
│   ├── LagrangianTruesdellRate
│   ├── LagrangianJaumannRate
│   └── LagrangianGreenNaghdiRate
└── LagrangianRashidRate              // independent branch (eq. 22)
```

The bodies of `objectiveUpdateTruesdell` / `…Jaumann` / `…GreenNaghdi`
moved verbatim into the three concrete linear helpers (with `_foo[_qp]` →
`host._foo[host._qp]`); the four shared advection helpers became static
methods on `LagrangianLinearObjectiveRate`. The strategy pattern keeps the
host material's interface unchanged and the existing 30
`cartesian/total/rates/tests` cases pass byte-for-byte.

Material class changes (`ComputeLagrangianObjectiveStress`):
- `objective_rate` enum extended to `"truesdell jaumann green_naghdi rashid"`
  (default `truesdell`).
- `_rate` enum member dropped (was only read inside the dispatcher).
- New `std::unique_ptr<LagrangianObjectiveRate> _rate_strategy;` constructed
  in the ctor via `createObjectiveRate(getParam<MooseEnum>("objective_rate"))`.
- `computeQpCauchyStress()` reduced to: compute small stress; small-kinematics
  passthrough; or `_rate_strategy->update(*this, dS)`.
- Four `friend class Lagrangian*Rate;` declarations so the helpers can read
  the host's property handles directly.
- Dropped the three private `objectiveUpdate{Truesdell,Jaumann,GreenNaghdi}`
  methods and the four shared `advectStress` / `updateTensor` /
  `stressAdvectionDerivative` / `cauchyJacobian` helpers (now in the
  hierarchy).
- Dropped the `<FactorizedRankTwoTensor.h>` include (no longer needed —
  removed the Step-3.1 polar decomp inline; that's now in the strain calc).
- The unused `_def_grad` / `_def_grad_old` members are kept (still
  declared but currently unused) — drop in a future cleanup pass.

Rashid rate body (`LagrangianRashidRate::update`):
- `r̂ = exp(Δw)` via Rodrigues for skew `Δw = _vorticity_increment[qp]`,
  with closed-form `dR/dW` (rank-4) — see `rotationFromVorticity`. Small-angle
  fallback at `θ < 1e-7` (Taylor expansion of `R ≈ I + W + W²/2`).
- Cauchy update: `σ_{n+1} = r̂ (σ_n + Δσ) r̂^T`.
- Eigenstrain Jacobian: `-(r̂ ⊗ r̂) : small_jacobian` where
  `(r̂ ⊗ r̂)_{ijkl} = r̂_ik r̂_jl` is `J^{-1}` from eq. 24.
- Cauchy Jacobian = sum of three rank-4 contributions:
  - `T1 = (dr̂/d(dL)) S r̂^T`
  - `T2 = r̂ S (dr̂^T/d(dL))`
  - `T3 = (r̂ ⊗ r̂) : small_jacobian · d(Δd)/d(dL)`
  with `d(Δw)/d(dL) = _d_vorticity_increment_d_F · (_d_spatial_velocity_increment_d_F)^{-1}`
  and `d(Δd)/d(dL) = I^(4) − d(Δw)/d(dL)`.

Rodrigues `dR/dW` treats W as **unconstrained** for the chain rule. The
upstream `_d_vorticity_increment_d_F` only produces skew d(Δw), so any
symmetric perturbation gets multiplied by zero. PetscJacobianTester (which
sees the unconstrained derivative numerically) verifies the formula holds
on the full tensor space, not just on the skew subspace.

Tests added (10):
- 8 new `PetscJacobianTester` cases in `cartesian/total/rates/tests`
  (`rashid_jacobian{,_alpha05} × {linear, quadratic, rashid_approximate, rashid_eigen}`),
  all at `ratio_tol = 1E-7`.
- 2 new CSVDiff cases (`rashid_shear`, `rashid_rotation`) with default
  `kinematic_approximation=linear`, plus gold files
  `gold/rashid_{shear,rotation}_out.csv` generated by running the input
  once and freezing the output. Rashid produces visibly different
  trajectories from Jaumann at long shear time (sxx ≈ 0.57 vs 0.72 at
  `t = 20` for the shear test), as expected — the matrix-exponential vs
  linear-in-Δw difference accumulates.

Full `solid_mechanics` suite: 1854 / 1854 (1844 baseline + 10 new).

Bug encountered: my first cut at `J^{-1}_{ijmn} = r̂_im r̂_jn` used
`rhat.times<i_, m_, j_, n_>(rhat)` with `usingTensorIndices(i_, j_, k_, l_, m_, n_)`,
which assigns `m_=4, n_=5`. But the `RankTwoTensor::times<n,o,p,q>(RankTwoTensor)`
overload uses `std::size_t x[4]` internally and indexes `x[n]`, `x[o]`,
`x[p]`, `x[q]` — labels >3 are out-of-bounds (undefined behavior). The
simulation hung silently during auto-scaling. Fix: relabel as
`rhat.times<i_, k_, j_, l_>(rhat)` (output `_{ijkl} = r̂_ik r̂_jl`), which
keeps all template args ∈ {0..3}. Lesson: when using `usingTensorIndices`
with >4 names, the extra ones can only be used in the
`times<>(RankFourTensor)` 6-arg variant (which uses `x[5]`), not in the
4-arg form for `times<>(RankTwoTensor)`.

Notes:
- The plan also flagged the duplicated polar-decomp-derivative algebra
  between the strain calc's `computeRashidEigenIncrement` (for f^{-1}) and
  the strain calc's new `computeQpPolarDecomposition` (for F, added in
  Step 3.1). Step 3.2 doesn't address that — extraction into a
  `MathUtils::polarRotationDerivative(R, U)` utility is still recommended
  for a future cleanup pass.
- Future Step 4 (physics action): use this Rashid rate when the user
  selects "old kernel style Rashid model" — see the top-of-file Plan
  section 4.2 / 4.3.
