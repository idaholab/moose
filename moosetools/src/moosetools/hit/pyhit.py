...
class Node(tree.Node):
    """
    An [moosetree/Node.md#tree.node] object for building a [!ac](HIT) tree.

    Add a new node to the input file tree that is a child of *parent*. The supplied *parent* must
    be `hit.Node` object. The *hitnode* is name of the node, as a string.
...
def load(filename, root=None):
    ...
    The function return a `hit.Node` object which is the root node of the loaded tree. The
    ...
    if os.path.exists(filename):
        ...
        raise RuntimeError("Unable to load the hit file ", filename)
...
def parse(content, root=None, filename=""):
    """
    Parse a hit tree from a *content* string and return a `hit.Node` object.
    ...
    if not content.strip():
        raise ValueError("hit cannot parse null content.")
