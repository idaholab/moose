...
/*******************************************************************************/
/* Python _external module definition                                          */
/*******************************************************************************/

PyDoc_STRVAR(capabilities_doc, "Interface to the Moose::Capabilities system.");

static struct PyModuleDef _external_module = {
    PyModuleDef_HEAD_INIT,
    "_external",   /* name of module */
...
{
  // get exception type
  auto exception_type = getPythonAttribute("moosetools.capabilities.exceptions", "CapabilityException");
...
setPythonUnknownCapabilitiesException(const UnknownCapabilitiesException & e)
{
  // get exception type
  auto exception_type =
      getPythonAttribute("moosetools.capabilities.exceptions", "UnknownCapabilitiesException");
  if (!exception_type || !PyCallable_Check(exception_type))
    return;
...
buildPythonCheckResult(const CapabilityRegistry::CheckResult & result)
{
  ...
  auto checkstate_type = getPythonAttribute("moosetools.capabilities", "CheckState");
  ...
  auto checkresult_type = getPythonAttribute("moosetools.capabilities.dataclasses", "CheckResult");
                ...
                CapabilityRegistry::CheckOptions & check_options)
{
  ...
  auto checkoptions_type = getPythonAttribute("moosetools.capabilities.dataclasses", "CheckOptions");
  ...
  if (!PyObject_TypeCheck(check_options_instance, (PyTypeObject *)checkoptions_type))
  {
    ...
    setPythonError(PyExc_TypeError,
                   "options must be of type moosetools.capabilities.dataclasses.CheckOptions");
    return false;
  }
  Py_DECREF(checkoptions_type);
...
/*******************************************************************************/
/* Python _external.CheckState: IntEnum                                        */
/*******************************************************************************/
...
/*******************************************************************************/
/* Python _external.AUGMENTED_CAPABILITY_NAMES: Set[str]                       */
/*******************************************************************************/
...
/*******************************************************************************/
/* Python _external.Capabilities object                                        */
/*******************************************************************************/

/// C++ state stored with a moosetools.capabilities.Capabilities object
struct CapabilitiesState
{
  CapabilityRegistry registry;
};

/// Python state stored with a _external.Capabilities object
typedef struct
{
...
/// Instantiate CapabilitiesState for a _external.Capabilities object
static PyObject *
...
/// _external.Capabilities.__init__()
static int
...
Capabilities_check(CapabilitiesObject * self, PyObject * args, PyObject * kwargs)
{
  ...
  if (!PyArg_ParseTupleAndKeywords(args,
    ...
    return returnPythonError(
        ...
        "Optional[moosetools.capabilities.dataclasses.CheckOptions] = None, add_capabilities: "
...
/// _external.Capabilities destruction
static void
...
/// _external.Capabilities definition of methods
...
/// _external.Capabilities definition of members
...
/// _external.Capabilities definition
static PyTypeObject CapabilitiesType = {
    PyVarObject_HEAD_INIT(nullptr, 0).tp_name = "_external.Capabilities",
...
/*******************************************************************************/
/* Python _external module init                                                */
/*******************************************************************************/

PyMODINIT_FUNC
PyInit__external(void)
{
  if (PyType_Ready(&CapabilitiesType) < 0)
    return nullptr;

  auto module = PyModule_Create(&_external_module);
