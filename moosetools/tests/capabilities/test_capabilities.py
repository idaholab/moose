...
from moosetools.capabilities import Capabilities, CheckState
from moosetools.capabilities.dataclasses import CheckOptions, CheckResult
from moosetools.capabilities.exceptions import (
    CapabilityException,
    UnknownCapabilitiesException,
)

CERTAIN_FAIL = CheckState.CERTAIN_FAIL
POSSIBLE_FAIL = CheckState.POSSIBLE_FAIL
POSSIBLE_PASS = CheckState.POSSIBLE_PASS
CERTAIN_PASS = CheckState.CERTAIN_PASS
...
class TestCapabilities(unittest.TestCase):
    ...
    def testInit(self):
        ...
        with self.assertRaisesRegex(
            ...
            Capabilities({"name": {"doc": "foo", "value": True, "explicit": True}})
        with self.assertRaisesRegex(
            CapabilityException,
            r"Capability::setExplicit\(\): Capability 'name' is "
            "bool-valued and cannot be set as explicit",
    ...
    def testCapabilitiesCheck(self):
        """Test Capabilities.check with no optional arguments."""
        cap = {
        ...
        err = "options must be of type moosetools.capabilities.dataclasses.CheckOptions"
        with self.assertRaisesRegex(TypeError, err):
            capabilities.check("unused", options=True)

        def check(
            req: str,
            expect_status: CheckState,
            ...
            state = result.state
            self.assertIsInstance(state, CheckState)
    ...
    def testCapabilitiesCheckAdd(self):
        """Test Capabilities.check with add_capabilities."""
    ...
    def testCapabilitiesCheckNegate(self):
        """Test Capabilities.check with negate_capabilities."""
        caps = {
    ...
    def testCapabilitiesCheckAddNegate(self):
        """Test Capabilities.check with [add,negate]_capabilities."""
        caps = {
            "cap1": {"doc": "doc", "value": "cap1"},
            "cap2": {"doc": "doc", "value": "cap2"},
        }
