...
"""Test moosetools.control.runners.interfaces.SubprocessRunnerInterface."""
...
from moosetools.control.runners.interfaces.subprocessrunnerinterface import (
    DEFAULT_CLEANUP_KILL_TIMEOUT,
)
from moosetools.control.runners.utils import SubprocessReader
...
class SubprocessRunnerInterfaceTest(SubprocessRunnerInterface):
    """Specialized SubprocessRunnerInterface for testing."""

    def get_additional_command(self):
        """Get the faked additional command."""
        return ADDITIONAL_COMMAND


def patch_runner(name: str, **kwargs):
    """Patch a method on the SubprocessRunnerInterface."""
    return patch(
        f"moosetools.control.runners.interfaces.SubprocessRunnerInterface.{name}",
        **kwargs,
    )
...
def get_process_output(
    ...
    """Read the process output from the SubprocessReader."""
    return [
...
class TestSubprocessRunnerInterface(MooseControlTestCase):
    ...
    def run_test_initialize_dummy_process(self, use_subprocess_reader: bool):
        """Run a test for initialize() with a dummy process."""
        self._caplog.clear()
    ...
    def test_initialize_dummy_process_with_reader_thread(self):
        """Test initialize() with a dummy process with the reader thread."""
        self.run_test_initialize_dummy_process(True)

    def test_initialize_dummy_process_without_reader_thread(self):
        """Test initialize() with a dummy process with the reader thread."""
        self.run_test_initialize_dummy_process(False)

    def test_initialize_process_directory(self):
        """Test initialize() starting the process with the specified directory."""
        self._caplog.clear()
    ...
    def test_finalize_wait_nothing(self):
        """Test waiting for nothing in finalize()."""
        runner = SubprocessRunnerInterfaceTest(**ARGS, use_subprocess_reader=False)
        runner.finalize()
        self.assert_log_size(0)

    def test_finalize_wait_process(self):
        """Test waiting for a dummy process in finalize()."""
        runner = SubprocessRunnerInterfaceTest(
    ...
    def test_finalize_wait_subprocess_reader(self):
        """Test running a dummy reader thread and waiting for it in finalize()."""
        runner = SubprocessRunnerInterfaceTest(
    ...
    def test_finalize_wait_subprocess_reader_not_alive(self):
        """Test running a dummy reader and waiting for it in initialize."""
        runner = SubprocessRunnerInterfaceTest(
            **ARGS, directory=self.directory.name, use_subprocess_reader=True
        )
    ...
    def test_cleanup_wait_subprocess_reader(self):
        """Test cleanup() waiting for the reader thread to finish."""
        self.allow_log_warnings = True
    ...
    def test_start_process_kwargs(self):
        """Test that start_process() passes its kwargs to popen."""
        with patch("subprocess.Popen", wraps=subprocess.Popen) as mock_popen:
    ...
    def test_get_pid_no_process(self):
        """Test get_pid() when there is no process."""
        runner = SubprocessRunnerInterfaceTest(**ARGS)
    ...
    def test_is_process_running_no_process(self):
        """Test is_process_running() when there is no process."""
        runner = SubprocessRunnerInterfaceTest(**ARGS)
        self.assertIsNone(runner._process)
        self.assertFalse(runner.is_process_running())

    def test_kill_process(self):
        """Test kill_process() on a dummy process."""
        runner = SubprocessRunnerInterfaceTest(
            **ARGS,
            directory=self.directory.name,
        )
    ...
    def test_kill_process_no_process(self):
        """Test kill_process() when a process doesn't exist."""
        runner = SubprocessRunnerInterfaceTest(
            **ARGS,
            directory=self.directory.name,
        )
    ...
    def test_get_return_code_running(self):
        """Test get_return_code() when the process is running."""
        runner = SubprocessRunnerInterfaceTest(**ARGS, use_subprocess_reader=False)
