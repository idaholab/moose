...
"""Test moosetools.control.runners.baserunner.SocketRunner."""
...
from moosetools.control import MooseControl, SocketRunner
from moosetools.control.requests_unixsocket import Session
...
def patch_runner(name: str, **kwargs):
    ...
    return patch(f"moosetools.control.SocketRunner.{name}", **kwargs)


def patch_base(name: str, **kwargs):
    """Patch a method on the BaseRunner."""
    return patch(f"moosetools.control.runners.BaseRunner.{name}", **kwargs)


class TestSocketRunner(MooseControlTestCase):
    """Test moosetools.control.runners.socketrunner.SocketRunner."""
    ...
    def test_initialize_finalize(self):
        ...
        with (
            patch(
                "moosetools.control.requests_unixsocket.Session.get",
