...
def run_moose(dt, time_integrator):
    ...
    command_line_args = ['../../../moose_test-opt', '-i', 'dot_convergence_study_nodot.i',
                         'Executioner/dt={}'.format(dt),
                         'Executioner/scheme={}'.format(time_integrator)]
...
#                         'GlobalParams/implicit={}'.format(implicit_flag)]
    ...
    last_row = get_last_row('dot_convergence_study_nodot_out.csv')
    return float(last_row[1]) - 0.067667641618306
...
time_integrators = ['implicit-euler', 'explicit-euler', 'crank-nicolson', 'bdf2', 'explicit-midpoint',
                    'dirk', 'explicit-tvd-rk-2']
