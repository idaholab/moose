...
if not (
    importlib.util.find_spec("moose_stochastic_tools")
    and importlib.util.find_spec("moose_stochastic_tools.StochasticControl")
):
    ...
    if MOOSE_DIR is None:
        ...
        if os.path.exists(os.path.join(HOMEDIR, "source", "moose")):
            ...
            raise ValueError(
                "Cannot find MOOSE. Please set the $MOOSE_DIR environment variable"
            )
        sys.path.append(os.path.join(MOOSE_DIR, "python"))
        sys.path.append(os.path.join(MOOSE_DIR, "modules/stochastic_tools/python"))
        sys.path.append(
            os.path.join(
                MOOSE_DIR, "modules/stochastic_tools/python/moose_stochastic_tools"
            )
        )
    else:
        sys.path.append(os.path.join(MOOSE_DIR, "python"))
...
def run_test(moose_exe: str, input_file: str, work_dir: str) -> None:
    """Check that controllable parsedpostprocessor works."""
