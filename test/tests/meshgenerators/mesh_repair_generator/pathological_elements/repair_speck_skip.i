# Two speck QUAD4s sharing an edge (a degenerate cluster), attached to a healthy unit
# quadrilateral (which sets the mesh scale so the tiny quads are flagged). Merging either tiny quad's
# vertices would collapse the other to a repeated node, so the speck repair conservatively
# leaves both in place and reports them (cluster removal is not implemented). A later routine may
# still act on them; this test only checks the speck routine's "left in place" report.
[Mesh]
  [healthy]
