...
class TestOutflow(unittest.TestCase):
    def test(self):
        ...
        for label, value in fig.label_to_slope.items():
            if label == "L2u":
                self.assertTrue(bottom_bound(value, 1.0, 0.05))
            else:
                self.assertTrue(bottom_bound(value, 2.0, 0.05))
...
class TestOutflowMinMod(unittest.TestCase):
    def test(self):
        ...
        for label, value in fig.label_to_slope.items():
            if label == "L2u":
                self.assertTrue(bottom_bound(value, 1.5, 0.05))
            else:
                self.assertTrue(bottom_bound(value, 2.0, 0.05))
...
class UpwindLimiter(unittest.TestCase):
    def test(self):
        df1 = mms.run_spatial(
            "limited-advection.i",
            7,
            "FVKernels/advection_u/limiter='upwind'",
            y_pp=["L2u"],
        )
        fig = mms.ConvergencePlot(xlabel="Element Size ($h$)", ylabel="$L_2$ Error")
        fig.plot(
...
class MinModLimiter(unittest.TestCase):
    def test(self):
        df1 = mms.run_spatial(
            "limited-advection.i",
            9,
            "FVKernels/advection_u/limiter='min_mod'",
            y_pp=["L2u"],
        )
        fig = mms.ConvergencePlot(xlabel="Element Size ($h$)", ylabel="$L_2$ Error")
        fig.plot(
...
class SOULimiter(unittest.TestCase):
    def test(self):
        df1 = mms.run_spatial(
            "limited-advection.i",
            9,
            "FVKernels/advection_u/limiter='sou'",
            y_pp=["L2u"],
        )
        fig = mms.ConvergencePlot(xlabel="Element Size ($h$)", ylabel="$L_2$ Error")
        fig.plot(
...
class QUICKLimiter(unittest.TestCase):
    def test(self):
        df1 = mms.run_spatial(
            "limited-advection.i",
            15,
            "FVKernels/advection_u/limiter='quick'",
            y_pp=["L2u"],
        )
        fig = mms.ConvergencePlot(xlabel="Element Size ($h$)", ylabel="$L_2$ Error")
        fig.plot(
...
class KTLimitedVanLeer(unittest.TestCase):
    def test(self):
        ...
        for key, value in fig.label_to_slope.items():
            self.assertTrue(bottom_bound(value, 2.5, 0.05))
            print("%s slope, %f" % (key, value))
