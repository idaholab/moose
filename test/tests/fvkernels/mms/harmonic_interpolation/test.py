...
class TestHarmonicTriangles(unittest.TestCase):
    def test(self):
        df1 = mms.run_spatial(
            "diffusion.i", 5, "FVKernels/diff/coeff_interp_method=harmonic"
        )
        ...
        for _, value in fig.label_to_slope.items():
            self.assertTrue(fuzzyEqual(value, 1.8, 0.05))


class TestAverageTriangles(unittest.TestCase):
    def test(self):
        df1 = mms.run_spatial(
            "diffusion.i", 5, "FVKernels/diff/coeff_interp_method=average"
        )
...
class TestHarmonicQuads(unittest.TestCase):
    def test(self):
        df1 = mms.run_spatial(
            "diffusion.i",
            4,
            "FVKernels/diff/coeff_interp_method=harmonic",
            "Mesh/gen_mesh/elem_type=QUAD",
        )
...
class TestAverageQuads(unittest.TestCase):
    def test(self):
        df1 = mms.run_spatial(
            "diffusion.i",
            5,
            "FVKernels/diff/coeff_interp_method=average",
            "Mesh/gen_mesh/elem_type=QUAD",
        )
