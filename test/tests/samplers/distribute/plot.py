...
def execute(name, ylabel, yscale=1, ext="pdf"):
    ...
    add_plot(
        ax, "distribute_none", name, "(a) Baseline w/o sample data", yscale, marker="s"
    )
...
def add_plot(ax, prefix, name, label, yscale, marker="o"):
    ...
    ax.plot(
        data["n_procs"].to_numpy(),
        data[name].to_numpy() / yscale,
        label=label,
        marker=marker,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-e", "--extension", default="pdf", type=str, help="The plot file extension."
    )
