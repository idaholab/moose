...
class TestDiffusion1DSymmetry(unittest.TestCase):
    def test(self):
        df1 = run_spatial('diffusion-1d-symmetry.i', 4, file_base="diffusion-1d-symmetry")
...
class TestAdvection1DSymmetry(unittest.TestCase):
    def test(self):
        df1 = run_spatial('advection-1d-symmetry.i', 4, file_base="advection-1d-symmetry")
        fig = mms.ConvergencePlot(xlabel='Element Size  ($h$)', ylabel='$L_2$ Error')
        fig.plot(df1,
                 label='l2error',
                 marker='o',
...
class TestDiffusion2DSymmetry(unittest.TestCase):
    def test(self):
        df1 = run_spatial('diffusion-2d-symmetry.i', 4, file_base="diffusion-2d-symmetry")
        fig = mms.ConvergencePlot(xlabel='Element Size  ($h$)', ylabel='$L_2$ Error')
        fig.plot(df1,
                 label='l2error',
                 marker='o',
...
class TestAdvection2DSymmetry(unittest.TestCase):
    def test(self):
        df1 = run_spatial('advection-2d-symmetry.i', 4, file_base="advection-2d-symmetry")
        fig = mms.ConvergencePlot(xlabel='Element Size  ($h$)', ylabel='$L_2$ Error')
        fig.plot(df1,
                 label='l2error',
                 marker='o',
...
class TestDiffusion2DSymmetryNonOrthogonal(unittest.TestCase):
    def test(self):
        df1 = run_spatial('diffusion-2d-symmetry.i', 5, "Mesh/gmg/elem_type=TRI3", file_base="diffusion-2d-symmetry")
        fig = mms.ConvergencePlot(xlabel='Element Size  ($h$)', ylabel='$L_2$ Error')
        fig.plot(df1,
                 label='l2error',
                 marker='o',
...
class TestAdvection2DSymmetryNonOrthogonal(unittest.TestCase):
    def test(self):
        df1 = run_spatial('advection-2d-symmetry.i', 5, "Mesh/gmg/elem_type=TRI3 Convergence/linear/max_iterations=20", file_base="advection-2d-symmetry")
        fig = mms.ConvergencePlot(xlabel='Element Size  ($h$)', ylabel='$L_2$ Error')
        fig.plot(df1,
                 label='l2error',
                 marker='o',
        ...
        for _,value in fig.label_to_slope.items():
            print("The current slope: ", value)
            self.assertTrue(fuzzyEqual(value, 1., .11)) # Why is this 1st order? It is 1st order with NonlinearFV as well. It is first order with a regular inlet-outlet flow as well. We will have to revisit this at some point.
