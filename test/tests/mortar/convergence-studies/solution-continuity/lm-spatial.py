...
class TestContinuity(unittest.TestCase):
    ...
    def secondary_and_primary_plots(
        self, fine, num_refinements, additional_cli_args, name, plots_to_do=None, mpi=1
    ):
    ...
    def run_geometric_discretization(
        self, geom_disc, fine_values, num_refinements, mpi=1
    ):
        if geom_disc == "p2p2":
            cli_args = [
        ...
        for fine_value in fine_values:
            ...
            fig = self.secondary_and_primary_plots(
                fine_value, num_refinements, cli_args, name, mpi=mpi
            )
