...
class TestContinuity(unittest.TestCase):
    ...
    def secondary_and_primary_plots(
        self, num_refinements, additional_cli_args, name, plots_to_do=None, mpi=1
    ):
    ...
    def run_geometric_discretization(
        self, geom_disc, elem_types, num_refinements, mpi=1
    ):
        if geom_disc == "p1p0":
            cli_args = [
        ...
        for elem_type in elem_types:
            ...
            fig = self.secondary_and_primary_plots(
                num_refinements, cli_args, name, mpi=mpi
            )
