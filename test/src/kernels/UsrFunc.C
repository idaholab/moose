                                      ...
                                      Real& Ak, Real& Bk, Real& Ck, Real& W, Real &t, bool& is_transient)
{
   ...
   if(is_transient)
      return libMesh::pi*(Ck*std::sin(libMesh::pi*t*W)*(3.*B0*std::cos(3.*libMesh::pi*y) + C0*std::cos(libMesh::pi*y)*std::sin(libMesh::pi*x))
           ...
           + (Au + Bu*x + Cu*y)*std::sin(libMesh::pi*t*W)*(3.*A0*std::cos(3.*libMesh::pi*x) + C0*std::cos(libMesh::pi*x)*std::sin(libMesh::pi*y))
           - libMesh::pi*(Ak + Bk*x + Ck*y)*std::sin(libMesh::pi*t*W)*(9.*A0*std::sin(3.*libMesh::pi*x)
           + C0*std::sin(libMesh::pi*x)*std::sin(libMesh::pi*y)) + W*std::cos(libMesh::pi*t*W)*(A0*std::sin(3.*libMesh::pi*x)
           + C0*std::sin(libMesh::pi*x)*std::sin(libMesh::pi*y) + B0*std::sin(3.*libMesh::pi*y))
           - libMesh::pi*(Ak + Bk*x + Ck*y)*std::sin(libMesh::pi*t*W)*(C0*std::sin(libMesh::pi*x)*std::sin(libMesh::pi*y)
           + 9.*B0*std::sin(3.*libMesh::pi*y)));
   else
      return libMesh::pi*(3.*A0*(Au + Bk + Bu*x + Cu*y)*std::cos(3.*libMesh::pi*x)
           + 3.*B0*(Av + Ck + Bv*x + Cv*y)*std::cos(3.*libMesh::pi*y)
           - 9.*A0*libMesh::pi*(Ak + Bk*x + Ck*y)*std::sin(3.*libMesh::pi*x)
           + C0*(Au + Bk + Bu*x + Cu*y)*std::cos(libMesh::pi*x)*std::sin(libMesh::pi*y)
           + C0*std::sin(libMesh::pi*x)*((Av + Ck + Bv*x + Cv*y)*std::cos(libMesh::pi*y)
  		        ...
  		        const std::string&) // unk_name, not needed
{
   ...
   gradu(0) = 3.*libMesh::pi*A0*std::cos(3.*libMesh::pi*p(0))+C0*libMesh::pi*std::sin(libMesh::pi*p(1))*std::cos(libMesh::pi*p(0));
   gradu(1) = 3.*libMesh::pi*B0*std::cos(3.*libMesh::pi*p(1))+C0*libMesh::pi*std::cos(libMesh::pi*p(1))*std::sin(libMesh::pi*p(0));
   gradu(2) = 0.;
