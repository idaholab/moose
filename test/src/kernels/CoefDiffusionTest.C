...
#include "CoefDiffusionTest.h"

registerMooseObject("MooseTestApp", CoefDiffusionTest);

InputParameters
CoefDiffusionTest::validParams()
{
...
CoefDiffusionTest::CoefDiffusionTest(const InputParameters & parameters)
  : Kernel(parameters), _coef(getParam<Real>("coef"))
{
}

Real
CoefDiffusionTest::computeQpResidual()
{
  return _coef * _grad_test[_i][_qp] * _grad_u[_qp];
}

Real
CoefDiffusionTest::computeQpJacobian()
{
