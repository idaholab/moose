...
CN2AdvDiffReaction1::CN2AdvDiffReaction1(const std::string & name, InputParameters parameters)
...
Real
CN2AdvDiffReaction1::computeQpResidual()
{
  // Reaction:
  Real told = _t-_dt;     
  Real QpResidual = _test[_i][_qp]*( ManSol4ADR2src(_q_point[_qp],_A0,_B0,_C0,_Au,_Bu,_Cu,_Av,_Bv,_Cv,_Ak,_Bk,_Ck,_omega0,_t)
                                     +ManSol4ADR2src(_q_point[_qp],_A0,_B0,_C0,_Au,_Bu,_Cu,_Av,_Bv,_Cv,_Ak,_Bk,_Ck,_omega0,told))/2.;
  // Advection:
  VectorValue<Number> vel(_Au+_Bu*_q_point[_qp](0)+_Cu*_q_point[_qp](1),
                          _Av+_Bv*_q_point[_qp](0)+_Cv*_q_point[_qp](1),0);
  QpResidual += -_test[_i][_qp]*vel*(_grad_u[_qp]+_grad_u_old[_qp])/2.;

  // Diffusion:
  Real diff   = _Ak+_Bk*_q_point[_qp](0)+_Ck*_q_point[_qp](1);    
  QpResidual += diff*_grad_test[_i][_qp]*(_grad_u[_qp]+_grad_u_old[_qp])/2.;
...
Real
CN2AdvDiffReaction1::computeQpJacobian()
{     
  // Advection:
  VectorValue<Number> vel(_Au+_Bu*_q_point[_qp](0)+_Cu*_q_point[_qp](1),
                          _Av+_Bv*_q_point[_qp](0)+_Cv*_q_point[_qp](1),0);
