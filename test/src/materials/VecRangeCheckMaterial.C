...
InputParameters validParams<VecRangeCheckMaterial>()
{
  ...
  params.addRequiredRangeCheckedParam<std::vector<int> >("iv3", "iv3_size = 3", "Int vector of length 3");
  params.addRequiredRangeCheckedParam<std::vector<Real> >("rvp", "rvp > 0", "Real vector of all positive values");
  params.addRequiredRangeCheckedParam<std::vector<unsigned int> >("uvg", "uvg_0 > uvg_1", "Unsigned int vector where component 0 is bigger than component 1");
  params.addRequiredRangeCheckedParam<std::vector<long> >("lvg", "lvg_0 > lvg_1", "Long vector where component 0 is bigger than component 1");
