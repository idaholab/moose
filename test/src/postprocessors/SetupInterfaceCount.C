...
InputParameters setupInterfaceCountParameters()
{
  InputParameters parameters = emptyInputParameters();
  MooseEnum count_type("initial timestep subdomain linear nonlinear initialize finalize execute threadjoin");
  parameters.addRequiredParam<MooseEnum>("count_type", count_type, "Specify the count type to return.");
  return parameters;
}
