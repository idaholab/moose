...
public:
  ...
  // Expose Action's protected overload for testing relationship managers declared by object params.
...
TEST_F(GhostPrimaryFaceTest, nodeFaceDerivedConstraintAddsDisabledRMByDefault)
{
  ...
  ASSERT_TRUE(action->addRelationshipManagers(Moose::RelationshipManagerType::GEOMETRIC,
                                              tiedValueParams(false)));
