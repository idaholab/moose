...
// traverse diagnostics and build formatted list for easy testing and viewing

void
format_diagnostics(const wasp::DataArray & diagnostics_array, std::stringstream & diagnostics_strm)
{
  ...
  for (size_t i = 0; i < diagnostics_size; i++)
  {
    ...
    diagnostics_strm << "line:" << diagnostic_start_line << " column:" << diagnostic_start_character
                     << " - " << diagnostic_message << std::endl;
  }
}

// traverse symbolpaths and build formatted list for easy testing and viewing

void
format_symbolpaths(const wasp::DataObject & symbols_response, std::stringstream & paths_strm)
{
  ...
  for (std::vector<int> indices{0}; indices.back() < (int)si.getChildSize(); indices.back()++)
  {
    ...
    indices.push_back(-1);
    paths_strm << si.getPath() << std::endl;
...
TEST(MooseServerTest, DiagnosticsAndSymbols)
{
  auto moose_unit_app = AppFactory::createAppShared("MooseUnitApp", 0, nullptr);
  MooseServer moose_server(*moose_unit_app);

  // test initialize and server capabilities
  {
...
line:9 column:2 - parameter 'Mesh/nx' supplied multiple times
  ...
  // test document open and hierarchical symbols
  {
...
/Outputs/term (21:0)
  ...
  // test document change and diagnostics
  {
...
line:9 column:2 -       Did you mean 'type'?
  ...
  // test document change and hierarchical symbols
  {
    // symbols test parameters
