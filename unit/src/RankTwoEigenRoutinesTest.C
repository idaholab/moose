...
TEST(RankTwoEigenRoutines, symmetricEigenvaluesEigenvectors_dual_number_finite_difference)
{
  ...
  ADRankTwoTensor T{-7.1, 2.3, 5.7, -1.4, -5.1, 1.2};
  ...
  for (unsigned int ii = 0; ii < 3; ++ii)
    for (unsigned int jj = 0; jj < 3; ++jj)
    {
      ...
      for (unsigned int i = 0; i < 3; ++i)
        EXPECT_NEAR(e1[i].derivatives()[dd], MetaPhysicL::raw_value(e2[i] - e1[i]) / eps, eps);

      for (unsigned int i = 0; i < 3; ++i)
        for (unsigned int j = 0; j < 3; ++j)
          EXPECT_NEAR(
              E1(i, j).derivatives()[dd], MetaPhysicL::raw_value(E2(i, j) - E1(i, j)) / eps, eps);
