...
/**
 * orient2d() is exactly zero on collinear input, where the textbook determinant is not.
 *
 ...
 * orient2d() reports a nonzero determinant here.
 */
TEST(GeometricPredicatesTest, collinearIsExactlyZero)
{
  ...
  EXPECT_EQ(0.0, orient2d(pa, pb, pc));

  // Zero has no sign, so the argument order cannot change the answer.
  EXPECT_EQ(0.0, orient2d(pb, pa, pc));
  EXPECT_EQ(0.0, orient2d(pc, pb, pa));
  ...
  EXPECT_EQ(0.0, orient2d(small_a, small_b, small_c));
}
...
TEST(GeometricPredicatesTest, orientationSign)
{
  ...
  EXPECT_LT(orient2d(pa, pc, pb), 0.0);
}

/**
 * incircle() is exactly zero on cocircular input, and flips sign across the circle.
 *
...
TEST(GeometricPredicatesTest, cocircularIsExactlyZero)
{
  ...
  ASSERT_GT(orient2d(east, north, west), 0.0);
  EXPECT_EQ(0.0, incircle(east, north, west, south));

  const double delta = 1.0 / 1024.0;
  const double south_inside[2] = {0.0, -radius + delta};
  const double south_outside[2] = {0.0, -radius - delta};
  EXPECT_GT(incircle(east, north, west, south_inside), 0.0);
  EXPECT_LT(incircle(east, north, west, south_outside), 0.0);
  ...
  ASSERT_GT(orient2d(sw, se, ne), 0.0);
  EXPECT_EQ(0.0, incircle(sw, se, ne, nw));

  const double center[2] = {0.0, 0.0};
  const double corner_outside[2] = {-2.0, 2.0};
  EXPECT_GT(incircle(sw, se, ne, center), 0.0);
  EXPECT_LT(incircle(sw, se, ne, corner_outside), 0.0);
}

/**
 * A cyclic rotation of orient2d()'s arguments preserves the sign, and swapping any two of them
 * negates it.
 */
TEST(GeometricPredicatesTest, orient2dPermutationAntisymmetry)
{
  ...
  const int ccw = predicateSign(orient2d(pa, pb, pc));
  ASSERT_EQ(1, ccw);

  EXPECT_EQ(ccw, predicateSign(orient2d(pb, pc, pa)));
  EXPECT_EQ(ccw, predicateSign(orient2d(pc, pa, pb)));

  EXPECT_EQ(-ccw, predicateSign(orient2d(pb, pa, pc)));
  EXPECT_EQ(-ccw, predicateSign(orient2d(pa, pc, pb)));
  EXPECT_EQ(-ccw, predicateSign(orient2d(pc, pb, pa)));
}

/**
 * Swapping two of incircle()'s first three arguments reverses the counter-clockwise order they must
 * be in, and so negates the sign; a cyclic rotation of them preserves it.
 */
TEST(GeometricPredicatesTest, incirclePermutationAntisymmetry)
{
  ...
  ASSERT_GT(orient2d(pa, pb, pc), 0.0);

  const int inside = predicateSign(incircle(pa, pb, pc, pd));
  ASSERT_EQ(1, inside);

  EXPECT_EQ(inside, predicateSign(incircle(pb, pc, pa, pd)));
  EXPECT_EQ(inside, predicateSign(incircle(pc, pa, pb, pd)));

  EXPECT_EQ(-inside, predicateSign(incircle(pb, pa, pc, pd)));
  EXPECT_EQ(-inside, predicateSign(incircle(pa, pc, pb, pd)));
  EXPECT_EQ(-inside, predicateSign(incircle(pc, pb, pa, pd)));
}
