...
show them the draft, then post what they approve.

When reviewing a GitHub PR and the user confirms:

- A single structured review with the report body:
  ```bash
  gh pr review <num> --comment --body-file <draft.md>
  ```
  (Use `--comment`, not `--approve`/`--request-changes`, unless the user is a CCB member and
  explicitly asks to set that state.)
- For line-specific items, post inline comments. For typos/grammar/small fixes, use a GitHub
  **suggestion** block so the author can accept it in one click:
  ````
  ```suggestion
  <corrected line(s)>
  ```
  ````
  Inline comments are created against `path` + `line` on the PR head commit via
