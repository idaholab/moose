...
result = chigger.exodus.ExodusResult(
    reader, variable="T", cmap="viridis", camera=camera, range=[263.15, 273.15]
)
cbar = chigger.exodus.ExodusColorBar(result, location="bottom")
cbar.setOptions("primary", title="Temperature [K]", font_size=32)
text = chigger.annotations.TextAnnotation(
    font_size=32, position=[0.5, 0.3], justification="center"
)
