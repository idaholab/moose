...
def frames():
    ...
    phase = chigger.exodus.ExodusResult(
        reader,
        camera=camera,
        variable="phi",
        range=[0, 1],
    ...
    cbar = chigger.exodus.ExodusColorBar(
        phase, length=0.8, colorbar_origin=[0.1, 0.85], location="top"
    )
    cbar.setOptions(
        "primary",
    ...
    window = chigger.RenderWindow(
        phase, cbar, graph, size=[1920, 1080], motion_factor=0.1, background=[1, 1, 1]
    )
...
def movie():
    chigger.utils.img2mov(
        "output/step10_micro*.png",
        "step10_micro_result.mp4",
        duration=30,
        num_threads=6,
    )
