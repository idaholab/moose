...
We want to minimize the squared distance between the knwon function ($sin(x)$)
...
We can move the derivative with recpect to $c_i$ into the integral:

!equation
\frac{\partial}{\partial \vec{c_i}} = \int\limits_{0.25}^{1.0} 2 \textcolor{red}{x^i} (sin(x)-\sum_{0}^2 c_i x^i) dx,
