...
def frames():
    ...
    out_flux = chigger.graphs.Line(
        color=[1, 0.2, 0.2], width=4, tracer=True, corner="right-bottom"
    )

    graph = chigger.graphs.Graph(avg_temp, out_flux, viewport=[0, 0, 0.5, 0.65])
    graph.setOptions("legend", visible=False)
    graph.setOptions(
        "xaxis", title="Time (s)", lim=[0, 100], font_color=[0, 0, 0], font_size=28
    )
    graph.setOptions(
        "yaxis",
        title="Temperature (K)",
    ...
    for i, t in enumerate(reader.getTimes()):
        ...
        line_temp.setOptions(
            x=vpp["temperature"].tolist(), y=vpp["y"].multiply(100).tolist()
        )
...
def movie():
    chigger.utils.img2mov(
        "output/step08_*.png", "step08_result.mp4", duration=30, num_threads=6
    )
