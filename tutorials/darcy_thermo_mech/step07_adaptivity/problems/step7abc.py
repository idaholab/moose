...
def frames():
    ...
    temp_a = chigger.exodus.ExodusResult(
        reader_a,
        camera=camera,
    ...
    temp_b = chigger.exodus.ExodusResult(
        reader_b,
        camera=camera,
        variable="temperature",
    ...
    temp_c = chigger.exodus.ExodusResult(
        reader_c,
        camera=camera,
        variable="temperature",
    ...
    cbar.setOptions(
        "primary", title="Temperature (K)", font_size=28, font_color=[0, 0, 0]
    )
...
def movie():
    chigger.utils.img2mov(
        "output/step07abc_*.png", "step07abc_result.mp4", duration=20, num_threads=6
    )
