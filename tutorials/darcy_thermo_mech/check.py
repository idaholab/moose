...
def compare(dir0, dir1, link=False):
    ...
    for root, _, files in os.walk(dir1):
        for fname in files:
            ...
            if (
                os.path.isfile(full_name0)
                and os.path.isfile(full_name1)
                and not os.path.islink(full_name1)
                and filecmp.cmp(full_name0, full_name1)
            ):
                ...
                if link:
                    ...
                    subprocess.call(
                        cmd, cwd=root
                    )  # os.symlink doesn't handle files w/o extension
                    print(" ".join(cmd))
                else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="A helper tool for keeping the symlinks correct."
    )
    parser.add_argument(
        "--link", action="store_true", help="Generate missing symlinks."
    )
    options = parser.parse_args()

    # Compare the sorted directories and show/create symlinks
    steps = sorted(
        [d for d in os.listdir(os.path.dirname(__file__)) if os.path.isdir(d)]
    )
