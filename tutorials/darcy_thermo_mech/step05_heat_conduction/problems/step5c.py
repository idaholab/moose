...
pressure = chigger.exodus.ExodusResult(
    reader, camera=camera, variable="temperature", range=[300, 350], cmap="viridis"
)
cbar = chigger.exodus.ExodusColorBar(
    pressure, length=0.6, colorbar_origin=[0.2, 0.7], location="top"
)
time = chigger.annotations.TextAnnotation(
    position=[0.5, 0.2], font_size=48, text_color=[0, 0, 0], justification="center"
)
cbar.setOptions("primary", title="Temperature (K)", font_size=48, font_color=[0, 0, 0])

window = chigger.RenderWindow(
    pressure, cbar, time, size=[1920, 1080], background=[1, 1, 1]
)
...
chigger.utils.img2mov(
    "output/step5c_*.png", "step05c_result.webm", duration=20, num_threads=6
)
# window.start()
